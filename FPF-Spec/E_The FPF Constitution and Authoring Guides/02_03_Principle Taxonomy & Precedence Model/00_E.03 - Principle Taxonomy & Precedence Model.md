...
## [[00_E.03 - Principle Taxonomy & Precedence Model|E.3]]:1 - Problem frame
Pattern [[00_E.02 - The Eleven Pillars|E.2]] supplies eleven immutable pillars, yet experience shows that a **flat list of principles invites ambiguity**: reviewers cannot decide which pillar overrules another  and “dead‑letter” rules accumulate.

## [[00_E.03 - Principle Taxonomy & Precedence Model|E.3]]:2 - Problem
...
## [[00_E.03 - Principle Taxonomy & Precedence Model|E.3]]:3 - Forces
| Force                                 | Tension                                                            |
| ------------------------------------- | ------------------------------------------------------------------ |
...
## [[00_E.03 - Principle Taxonomy & Precedence Model|E.3]]:4 - Solution

### [[00_E.03 - Principle Taxonomy & Precedence Model|E.3]]:4.1 - **Principle Taxonomy**
 ...
 #### [[00_E.03 - Principle Taxonomy & Precedence Model|E.3]]:4.2 - **Precedence Stack**

   | Level | Governing Artefact                    | Overrides        |
   | ----- | ------------------------------------- | ---------------- |
   | 0     | **Vision & Mission** ([[00_E.01 - Vision & Mission- “Operating System for Thought”]])            | everything       |
   | 1     | **Eleven Pillars** ([[00_E.02 - The Eleven Pillars]])              | all below        |
...
## [[00_E.03 - Principle Taxonomy & Precedence Model|E.3]]:5 - Conformance Checklist
...
## [[00_E.03 - Principle Taxonomy & Precedence Model|E.3]]:6 - Illustrative Conflict Resolution
...
**Precedence (high → low).** Law & Regulation → **[[00_E.05 - Four Guard-Rails of FPF|E.5]] Guard‑Rails** → **[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] Trust & Assurance** → **[[00_E.03 - Principle Taxonomy & Precedence Model|E.3]] governance decisions** → **E/E‑LOG policies** (editioned) → **BLP ([[00_E.02 - The Eleven Pillars|E.2]])** → Product Policies → Implementation Tactics.

**Notes.**
* BLP is a constitutional policy (see [[00_E.02 - The Eleven Pillars|E.2]] / “BLP”), but **does not supersede** [[00_E.05 - Four Guard-Rails of FPF|E.5]] Guard‑Rails nor [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] assurance floors; it **does govern** ties among lawful, comparable‑assurance options.
* Wherever **NQD/E/E‑LOG** promotes illumination telemetry to dominance (via an explicit **CAL** policy; **policy‑id recorded in SCR**), **BLP adopts that lens** rather than overriding it (see [[00_E.02 - The Eleven Pillars|E.2]] BLP‑6).
...
* **BLP Override (Waiver).** When a narrower hand‑engineered method is selected over a general/scalable alternative **within declared tolerances** (α = budget, δ = assurance), the DRR **MUST** include:
  - a **BLP Scale‑Audit** (see [[00_E.02 - The Eleven Pillars|E.2]] **BLP‑1**) covering compute/data/**freedom‑of‑action** sweeps and slope/uncertainty reporting,
  - the **tolerances** α/δ and objective vector used ([[00_E.02 - The Eleven Pillars|E.2]] **BLP‑1e**),
  - a **Heuristic‑Debt** entry (responsible role, scope, expiry/review, de-hardening plan) per [[00_E.02 - The Eleven Pillars|E.2]] **BLP‑4**,
  - an **AutonomyProfileId** (see **[[00_E.03 - Principle Taxonomy & Precedence Model|E.3]]‑ABL**) and the GateDecision authority (see **Gate‑decision authority map** below).
**Set-returning parity.** All precedence decisions that compare methods **MUST** use the G.5/G.9 parity harness and **Pareto** dominance; scalarisation across mixed scales/units is **prohibited** ([[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).

**BLP — Bitter‑Lesson Hooks into Precedence**
1) **Tie‑breaking.** If two lawful options are **within δ** assurance and **within α** budget, prefer the option whose **slope vector Pareto‑dominates** over the audited window; if no dominance, prefer the **more general** method. ([[00_E.02 - The Eleven Pillars|E.2]] **BLP‑2**.)
2) **Script‑vs‑Search conflicts.** For conflicts between **procedural scripts** and **general search/learning**, scripts prevail **only** when mandated by [[00_E.05 - Four Guard-Rails of FPF|E.5]] or regulation, or when a DRR records a **BLP‑waiver** with expiry and hazard rationale ([[00_E.02 - The Eleven Pillars|E.2]] **BLP‑3/6**).
3) **Publication.** Precedence rulings that reference BLP **MUST** publish editioned policy‑IDs, edition pins, and **Resrc‑CAL** accounts to the SCR ([[00_E.02 - The Eleven Pillars|E.2]] **BLP‑1d**; [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]).
...
* **Profile binding & change visibility.** Every CallPlan **MUST** declare the active profile id. Any profile change is a **GateCrossing** ([[00_E.18 - Transformation Flow Structure|E.18]]) and **MUST** be published (DecisionLog entry + pinned policy‑ids), so an auditor can reconstruct which profile governed which Window.
* **Assurance floors.** **[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]** WLNK minima on **F** and **R** apply at all profiles. Any profile‑specific tightening (e.g., higher required **R_eff** or stricter CL/Φ policies for broader FoA) **MUST** be declared on the profile and pinned by policy‑id. Pre‑deployment **assurance deltas** MUST be recorded for L2+.
...
* **BLP conformance.** For L2+, selection MUST apply **BLP** ([[00_E.02 - The Eleven Pillars|E.2]] **BLP‑2**) with **α/δ** tolerances declared in the plan policy. Any admitted heuristic requires a **Heuristic‑Debt** entry ([[00_E.02 - The Eleven Pillars|E.2]] **BLP‑4**).
* **Learning/Adaptation.** L3–L4 MAY enable **feedback‑driven adaptation** within [[00_E.05 - Four Guard-Rails of FPF|E.5]] Guard‑Rails and privacy controls; L0–L2 default **off** unless a DRR documents mitigation ([[00_E.02 - The Eleven Pillars|E.2]] **BLP‑5**).
* **Human‑in‑the‑Loop (HITL).** HITL obligations are expressed as **gate decisions and pause/resume hooks**, not an implicit “approval ladder”:
  ...
  * **L4:** continuous telemetry review; the default execution context is **sandboxed**; leaving the sandbox requires an explicit **GateCrossing** with a published CrossingBundle ([[00_E.18 - Transformation Flow Structure|E.18]] + [[00_F.09 - Alignment & Bridge across Contexts|F.9]]/[[00_F.17 - Unified Term Sheet (UTS)|F.17]]/[[03_E.17 - Multi-View Publication Kit|E.17]], with [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] when a gate decision is live).
...
* **Promote** a profile when repeated **BLP‑consistent** results show stable assurance within δ and budget adherence within α for ≥ **N_policy** runs (declare **N_policy** in the active profile). Promotion is not implicit: a **GateDecision** **MUST** authorize the profile change and cite the slope evidence ([[00_E.02 - The Eleven Pillars|E.2]] **BLP‑1c**).
...
## [[00_E.03 - Principle Taxonomy & Precedence Model|E.3]]:7 - **Conformance Checklist — [[00_E.03 - Principle Taxonomy & Precedence Model|E.3]] ↔ BLP Interop**
...
| **CC‑E3.11** | Every DRR that overrides BLP **MUST** include a **Scale‑Audit** ([[00_E.02 - The Eleven Pillars]] **BLP‑1**) and a **Heuristic‑Debt** entry ([[00_E.02 - The Eleven Pillars]] **BLP‑4**). | Prevents silent heuristic drift. |
...
## [[00_E.03 - Principle Taxonomy & Precedence Model|E.3]]:8 - Consequences
*Positive* — Turns subjective debate into objective, traceable decisions; high‑impact conflicts surface early.

## [[00_E.03 - Principle Taxonomy & Precedence Model|E.3]]:9 - Rationale
...
## [[00_E.03 - Principle Taxonomy & Precedence Model|E.3]]:10 - Relations
...
## [[00_E.03 - Principle Taxonomy & Precedence Model|E.3]]:End
