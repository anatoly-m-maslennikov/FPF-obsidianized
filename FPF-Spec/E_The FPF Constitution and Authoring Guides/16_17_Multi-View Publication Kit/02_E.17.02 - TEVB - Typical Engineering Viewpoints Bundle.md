...
**Builds on.**
* **[[00_E.17.00 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]] — `U.MultiViewDescribing`.** Supplies the generic notion of `U.Viewpoint`, `U.View`, and `ViewFamily` over an `EntityOfConcernClass ⊑ U.Entity` (here: `EntityOfConcernClass = U.Holon`).
* **[[01_E.17.01 - U.ViewpointBundleLibrary - Reusable Viewpoint Bundles|E.17.1]] — `U.ViewpointBundleLibrary`.** Provides the generic `U.ViewpointBundle`/`ViewFamilyId` structure; TEVB is a concrete bundle (`VF.TEVB.ENG`) in the core library.
* **[[00_A.01 - U.Holon, U.System, and U.Episteme|A.1]] — Holon.** Holon kinds `U.System` and `U.Episteme` as the typical engineering entities of concern.
...
* **[[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] and [[04_E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]] - Strict Distinction: EntityOfConcern, Description episteme, and Description episteme admitted for specification use.** TEVB uses DescriptionContext; engineering descriptions and specifications under TEVB are Description epistemes and specification-use cases with explicit `ViewpointRef`.
* **[[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]] — `U.EpistemeSlotRelation`.** Provides `EntityOfConcernSlot`, `ViewpointSlot`, `ViewSlot` and the slot discipline ([[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]) used by TEVB-aligned Description epistemes and specification-use Description epistemes.

**Used by.**
* **[[00_E.18 - Transformation Flow Structure|E.18]]:5.12 — transformation-flow viewpoint-family map.** As a canonical consumer, [[00_E.18 - Transformation Flow Structure|E.18]] binds its engineering transformation-flow families (Functional, Procedural, Role-Enactor or Device-Structure, Module-Interface) to TEVB viewpoints `VP.Functional`, `VP.Procedural`, `VP.RoleEnactor`, `VP.ModuleInterface`.
* **[[03_E.17 - Multi-View Publication Kit|E.17]] (MVPK).** Publication of engineering morphisms uses TEVB engineering viewpoints on the Description-episteme and specification-use side and separate publication-side viewpoints over publication faces and forms.
...
## [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:1 - Problem frame  *(informative)*
...
In FPF, [[00_E.17.00 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]] and [[01_E.17.01 - U.ViewpointBundleLibrary - Reusable Viewpoint Bundles|E.17.1]] give the **generic machinery**:
...
[[00_E.18 - Transformation Flow Structure|E.18]] already uses a **canonical engineering viewpoint-family map** with names like “Functional”, “Procedural”, “Role-Enactor (Device-Structure)”, “Module-Interface”. Without a formal bundle tying these together, those names drift and the mapping between [[00_E.18 - Transformation Flow Structure|E.18]], MVPK, EntityOfConcern, Description-episteme boundary, and specification use becomes fragile.
...
## [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:2 - Problem  *(informative)*
...
3. **Viewpoints conflated with publication faces/forms and files.** “Functional view” is used both for the underlying viewpoint and for a concrete document or dashboard; MVPK faces/forms, [[00_E.18 - Transformation Flow Structure|E.18]] transformation-flow families, and EntityOfConcern and Description-episteme and specification-use distinctions become entangled.
4. **Role leakage into EntityOfConcern and Description-episteme boundary and specification-use discipline.** Engineering views that are about role-enactors are written directly in terms of `U.Role`, blurring the boundary between RoleEnactment ([[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]) and description and specification-use lanes, and breaking [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] and [[04_E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]].
...
TEVB makes engineering viewpoint families **first‑class reusable bundles** and pins them to an explicit `EntityOfConcernClass` (engineering holons) so that [[00_E.18 - Transformation Flow Structure|E.18]], MVPK and discipline-packs can align on the same vocabulary.

## [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:3 - Forces  *(informative)*
...
## [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:4 - Solution — TEVB as a core `U.ViewpointBundle` for holons  *(normative)*

### [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:4.1 - TEVB bundle identity
...
The selection `{VP.Functional, VP.Procedural, VP.RoleEnactor, VP.ModuleInterface}` is the current **NQD-frontier** for engineering holon viewpoints in Part G: it realises a Function-Behaviour-Structure-plus-Role (`F-B-S+R`) cut that is non-dominated against candidate families including explicit information or data, assurance or safety, and mission or context viewpoints under the N, U, C, and D characteristics ([[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]], [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]). Part G records the SoTA candidate set and rejected alternatives; TEVB only fixes the **core four** where each `VP.* : U.Viewpoint` is defined below. These four are the **only** viewpoints in the core TEVB bundle.

  > **Note.** Other ViewFamilyId values used in the [[00_E.18 - Transformation Flow Structure|E.18]] transformation-flow viewpoint-family map (e.g., *Assurance‑Oriented*, *Interoperability‑Oriented*, *Information/Data‑Oriented*, *Operational/Deployment*, *Mission/Context*) remain **lexical families only** for transformation-flow species ([[00_E.18 - Transformation Flow Structure|E.18]]:5.12). They do not add viewpoints to TEVB; they are orthogonal to TEVB’s `viewpoints` set.

### [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:4.2 - TEVB engineering viewpoints
...
* `AllowedEpistemeKinds : FinSet(U.EpistemeKindRef)` — Description-episteme and specification-use kinds admissible under this viewpoint (all obeying EntityOfConcern and Description-episteme boundary, specification use, and [[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]] slot disciplines);
...
#### [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:4.2.1 - `VP.Functional` - functional behavior and capability viewpoint
...
* **ConformanceRules (examples).**
  ...
  * Functional ports/signatures are separated from module interfaces unless an [[12_A.06.M - Module Relation Repair|A.6.M]] module-interface claim is current.
  * When functional views participate in retargeting patterns, they satisfy the relevant [[08_A.06.04 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] retargeting constraints; concrete consumer patterns such as [[00_E.18 - Transformation Flow Structure|E.18]] may impose additional rules.
...
#### [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:4.2.2 - `VP.Procedural` — process & control viewpoint
...
* **AllowedEpistemeKinds (shape).**
  ...
  * `MethodDescription`, `MethodSpec` for operational procedures ([[01_A.03.01 - U.Method- Context-Defined Way of Doing|A.3.1]]–[[02_A.03.02 - U.MethodDescription- Description Episteme for a Way of Doing|A.3.2]]).
  ...
  * `WorkflowDescription`, `WorkflowSpec` (business processes, orchestration logic).
  ...
  These epistemes:
  * respect the **order discipline** (Γ_method, Γ_ctx) and [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] (Role–Method–Work alignment);
  * carry E.10.D2-conformant DescriptionContext with `ViewpointRef = VP.Procedural`.

* **ConformanceRules (examples).**
  * Pre/post‑conditions at step boundaries are explicit and type‑checked ([[00_A.03 - Transformer Constitution (Quartet)|A.3]].1/A.3.2, Γ_method).
  * No embedding of Work or calendars inside procedural descriptions ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] and [[04_E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]]).
  * Failure modes and recovery actions are declared and traceable to safety analyses ([[00_F.15 - SCR-RSCR Harness for Unification|F.15]] harnesses where relevant).
...
#### [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:4.2.3 - `VP.RoleEnactor` - role, bearer, and device-structure viewpoint
...
* **AllowedEpistemeKinds (shape).**
  ...
  * `RoleEnactmentDescription` for mappings `Holder#Role:Context` under [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]].
  * `TransformerBearerDescription` for `U.System bearing TransformerRole@Context` around an [[00_A.03.04 - U.Transformation- Bounded Change Under Conditions|A.3.4]] transformation.
...
* **ConformanceRules (examples).**
  * Role vs method vs work vs capability separation is upheld through [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] and [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]].
...
#### [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:4.2.4 - `VP.ModuleInterface` - module and interface viewpoint
...
* **ConformanceRules (examples).**
  * Interfaces are typed and explicitly bound to standards or signature declarations where applicable ([[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]], [[12_A.06.M - Module Relation Repair|A.6.M]], [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]).
  * Functional ports are not treated as module interfaces unless the module-interface or substitutability claim is current.
  * No inlining of methods, work, or functional behavior into module structure; use [[00_A.03 - Transformer Constitution (Quartet)|A.3]].4/A.6.F/A.15 for those claims.
...
## [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:5 - Archetypal grounding  *(informative)*
...
Each `VP.*` viewpoint is a `U.Viewpoint` as in [[00_E.17.00 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]], with:
...
Each of these is a **family of Description epistemes and specification-use cases** with `DescriptionContext = ⟨EntityOfConcernRef(Plant_X or ControlStack_X), BoundedContextRef, ViewpointRef=VP.*⟩` and TEVB ensures that [[00_E.18 - Transformation Flow Structure|E.18]] and MVPK can rely on this common structure.

## [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:6 - Conformance checklist  *(normative)*
...
Each `VP.*` viewpoint MUST be a well‑formed `U.Viewpoint` per [[00_E.17.00 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]]:
...
TEVB-aligned descriptions and specification-use cases MAY reference `U.RoleEnactor` families in `StakeholderFamilies` but SHALL NOT add `Role` or `RoleEnactor` as characteristics in Description episteme or specification-use case signatures beyond what [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] and [[04_E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]] already provides. Role semantics stay in RoleEnactment patterns; TEVB just selects concerns.
...
## [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:7 - Rationale & SoTA echoing  *(informative)*

### [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:7.1 - NQD‑grounded choice of the core four
...
Other candidates (e.g. dedicated information, assurance, or mission viewpoints) remain important but either duplicate concerns already captured by TEVB (when specialised to engineering holons) or are better modelled as orthogonal quality bundles ([[00_C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]]) or non-engineering viewpoint bundles (business and governance viewpoint bundles). TEVB therefore pins only the core four and leaves the rest to specialised families.

### [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:7.2 - Alignment with post‑2015 engineering practice
...
## [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:8 - Relations  *(informative)*

* **Builds on.** [[00_E.17.00 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]] (`U.MultiViewDescribing`), [[01_E.17.01 - U.ViewpointBundleLibrary - Reusable Viewpoint Bundles|E.17.1]] (`U.ViewpointBundleLibrary`), [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] and [[04_E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]] (EntityOfConcern and Description-episteme boundary plus specification use), [[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]] (EpistemeSlotRelation), A.6.2-A.6.4 (episteme morphisms).
* **Constrains.** [[00_E.18 - Transformation Flow Structure|E.18]]:5.12 (transformation-flow viewpoint-family map), engineering description and specification-use patterns, MVPK engineering publication guidance.
* **Coordinates with.** MVPK publication face, publication form, publication unit, and publication carrier discipline; F-R family (Role, RoleDescription, RoleSpec); [[00_F.18 - Local-First Unification Naming Protocol|F.18]] (naming discipline for ViewFamilyId, ViewpointId, EngineeringVPId, and publication-side viewpoint ids).
...
## [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:End
