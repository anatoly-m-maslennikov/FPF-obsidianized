...
TransformationFlowStructure:
  ...
  mathematicalDescriptionRef?: [[E_The FPF Constitution and Authoring Guides/17_18_Transformation Flow Structure/02_E.18.02 - Transformation Flow Mathematical Description|E.18.2]] only if a graph, algebra, or category expression is being used.
```
...
**Structure ontology.** [[00_E.18 - Transformation Flow Structure|E.18]] keeps these distinctions primary:
...
| transformation locus | an [[00_E.18 - Transformation Flow Structure]] locus, path, path slice, substructure, or valuation used to express, decompose, constrain, or locate a bounded `U.Transformation` | governed by [[00_A.03.04 - U.Transformation- Bounded Change Under Conditions]] for transformation identity and slots |
...
When a sentence says that a system performs a functional transformation at one point in a flow, [[00_E.18 - Transformation Flow Structure|E.18]] carries only the selected flow structure, locus, path, slice, crossing, valuation, and pins. The bounded transformation, transformer or candidate bearer, input/output or functional-port boundary, functioning relation, method/algorithm, mechanism, and performed work are recovered through [[00_A.03.04 - U.Transformation- Bounded Change Under Conditions|A.3.4]], [[11_A.06.F - Function and Functional Precision Restoration (RPR-FUNCTION)|A.6.F]], [[04_C.30.ASV - Architecture Structural View Adequacy (ASV)|C.30.ASV]], [[12_A.06.M - Module Relation Repair|A.6.M]], [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]], and the [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] family as applicable. A computational algorithm may fill `MethodRef?` or `MethodDescriptionRef?`; a physical-world way of transforming may fill `U.Method`; neither is inferred from [[00_E.18 - Transformation Flow Structure|E.18]] structure membership.

**Not this pattern when.** Use [[00_A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] for internal step validity, [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] for gate-decision publication, [[00_E.20 - Mechanism Introduction Protocol|E.20]] for mechanism-governing-definition placement, [[00_A.03.04 - U.Transformation- Bounded Change Under Conditions|A.3.4]] for bounded transformation under conditions, [[02_E.18.02 - Transformation Flow Mathematical Description|E.18.2]] for mathematical descriptions of the selected structure, [[01_C.27.TA - Temporal Aspect- Time Windows, Rhythm, Cadence, and Currentness|C.27.TA]] for temporal aspects, [[00_C.27 - Temporal Claim Adequacy- State Readings, Temporal Trends, and Intervention-Sensitive Temporal Change|C.27]] for temporal-claim adequacy or supported-use claims, the [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] family for work planning or performed work, [[03_E.17 - Multi-View Publication Kit|E.17]] for publication faces, and [[00_E.10 - Unified Lexical Rules for FPF|E.10]] for wording-use repair when the current EntityOfConcern is not the selected structure, path, crossing, or flow valuation.
...
**What this buys.** [[00_E.18 - Transformation Flow Structure|E.18]] keeps selected structure, publication pins, crossings, CV/GF separation, and refresh locality in one current structure pattern without turning every domain-specific path into its own flow doctrine or every mathematical graph description into the selected structure.

## [[00_E.18 - Transformation Flow Structure|E.18]]:2 - Problem frame

Teams can produce many **well-typed flow valuations** for transformations of the same context holon under `VP.Functional`, for example for a declared `U.Capability` or transformation claim. The holon is the described/context object; the [[00_E.18 - Transformation Flow Structure|E.18]] `EntityOfConcern` remains the selected `TransformationFlowStructure` over transformations and adjacent governed loci. The P2W reference path is:
`U.Signature(profile=FormalSubstrate) -> U.PrincipleFrame -> U.Mechanism -> U.ContextNormalization (UNM) -> SelectionAndTuning locus (G.5/selector relation) <-> WorkPlanning locus ([[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/02_A.15.02 - U.WorkPlan|A.15.2]] U.WorkPlan or plan-item relation) -> U.Work -> EvaluatingAndRefreshing locus (G.11/refresh relation)`
...
MVPK already fixes publication drift at the **single-arrow** scope; [[00_E.18 - Transformation Flow Structure|E.18]] lifts those **publication and comparability laws** to the **selected transformation-flow structure as a whole**.

## [[00_E.18 - Transformation Flow Structure|E.18]]:3 - Problem
...
## [[00_E.18 - Transformation Flow Structure|E.18]]:4 - Forces
...
* **Scopes:** `Gamma_time` (budgets, horizons), `PublicationScope` for faces ([[03_E.17 - Multi-View Publication Kit|E.17]]), and **slice ids** for refresh ([[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]).

 **CtxState (PS‑projection; closed slots):** `CtxState = ⟨L, P, E⃗, D⟩` is the **projection of [[03_E.17 - Multi-View Publication Kit|E.17]] Publication Scope**.
 **Slot definitions (normative):**
  ...
  • `P := ReferencePlane` — the reference plane/units registry id; **no plane/unit declarations or translations** occur in CV; crossings remain gated ([[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]).
 ...
 **Data-shape location.** [[00_E.18 - Transformation Flow Structure|E.18]] names the structure and valuation obligations for `PathId`, `PathSliceId`, Gamma pins, and lineage: flow is a valuation over `U.Transfer`, raw transfer preserves `CtxState`, and path or slice evidence is carried through this pattern plus [[00_A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]], with [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]] or [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] for cases involving evidence-path visibility or refresh wiring. These are the current structure loci for path and slice currentness.

 * **Locus kinds:** `Transformation`, `Signature`, `Mechanism`, `WorkPlanning`, `Work`, `Check`, and `StructuralReinterpretation` are the current minimal structure-positioned locus baseline. Domain-specific species are open-world and non-exhaustive, but each species binds to one of the locus kinds or requires an explicit [[00_E.18 - Transformation Flow Structure|E.18]] update. These are positioned loci in the selected structure, not a local taxonomy of new FPF kinds.
  **Exact identification (no local ontology):**
  — `Transformation` **≡** **[[00_A.03.04 - U.Transformation- Bounded Change Under Conditions|A.3.4]]** `U.Transformation` when the structure locus, path, path slice, substructure, or flow valuation is being used as a bounded transformation under conditions.
  — `Signature` **≡** **[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]** `U.Signature` (universal, law-governed declaration).
  — `Mechanism` **≡** **[[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]** `U.Mechanism` (law-governed application over a SubjectKind/BaseType), with placement and stabilization relations in [[00_E.20 - Mechanism Introduction Protocol|E.20]] when current.
  — `WorkPlanning` **≡** **[[02_A.15.02 - U.WorkPlan|A.15.2]]** `U.WorkPlan` or its current plan-item relation when planning is the governed value.
  — `Work` **≡** **[[01_A.15.01 - U.Work|A.15.1]]** `U.Work` / `U.WorkEnactment` (dated world-contact; `FinalizeLaunchValues` only here).
  — `Check` **≡** `OperationalGate(profile)` (universal **gate**; [[00_A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] governs CV when internal step validity is current, and [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] governs gate profile, check aggregation, decision, and publication minima when gate fit or gate decision is current).
  — `StructuralReinterpretation` **≡** the [[00_E.18 - Transformation Flow Structure|E.18]] placement of **[[08_A.06.04 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]]** `U.EpistemicRetargeting` as a structure-positioned locus; it is not a new retargeting kind. **All retargeting semantics** (slot-scoped discipline, `EntityOfConcernSlot`/`GroundingHolonSlot` behaviour, invariants, Bridges, witnesses) come from **[[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]]** and **[[06_A.06.02 - U.EffectFreeEpistemicMorphing - Effect-free morphisms of epistemes|A.6.2]]–[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]**; [[00_E.18 - Transformation Flow Structure|E.18]] does **not** introduce a local variant of retargeting.

  `OperationalGate` is the [[00_E.18 - Transformation Flow Structure|E.18]] check locus with DecisionLog aggregation. A check-locus label names only the current gate or check value that the selected structure positions: [[00_A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] governs internal constraint validity when that claim is current, [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] governs gate profile, aggregation, decision, and publication minima when gate fit or gate decision is current, and [[00_A.03.04 - U.Transformation- Bounded Change Under Conditions|A.3.4]] governs the bounded transformation claim that the check constrains.
  The only extra discipline [[00_E.18 - Transformation Flow Structure|E.18]] adds for `StructuralReinterpretation` is **structure-local**: CtxState and GateCrossing behaviour are governed by **CC-E18-06-EX** and **CC-E18-11** (projection-preserving w.r.t. `⟨L,P,E⃗,D⟩`, PathSlice-local, and "no plane/unit change without a gate"). `StructuralReinterpretation` is not a gate exception; it carries a recorded witness condition for saying no GateCrossing occurred. If any `CtxState` slot, plane/unit, edition, or design/run boundary changes, the case is a GateCrossing again.
> **MVPK integration (import).** Every locus with an external publication face is published via **MVPK** faces (`PlainView`, `TechCard`, `AssuranceLane`, `InteropCard`) under a declared **PublicationScope** ([[03_E.17 - Multi-View Publication Kit|E.17]]). [[00_E.18 - Transformation Flow Structure|E.18]] **reuses** MVPK's publication laws (pins, declared-order discipline, "no new numeric claims / no I/O re-listing") and only adds structure-scope constraints in S3 and **CC-E18-09/10**; it does **not** define a second, local publication semantics.
...
* where a Bridge is involved: the **BridgeCard** ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]) and its disclosed fields (`BridgeId`, `bridgeChannel`, **CL** and loss notes; **`CL^k`** when `bridgeChannel=Kind`; **`ReferencePlane(src,tgt)`**);
* where planes differ: **`CL^plane`** and the active **`Φ_plane`** as a **`PolicyIdRef`** (policy-id + resolvable refs; [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:8.1);
* the active penalty policy identifiers **`Φ(CL)`** (and **`Ψ(CL^k)`** if used) as **`PolicyIdRef`** bundles (policy-id + `PolicySpecRef` + `MintDecisionRef?`; [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:8.1);
* any additional pins mandated by the active **GateProfile** / GateChecks ([[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]) for this crossing.
...
* **Eulerian (structural):** define a **function on transfer relations** ("how much/what is associated with each relation under a given regime"), with gate laws. [[00_E.18 - Transformation Flow Structure|E.18]] deliberately fixes the **Eulerian semantics of flow** at the selected-structure scope: "flow (= valuation) + publication log", while change over time appears as **re-valuation** over a **PathSlice** (the execution/republishing window) under gate rules and the SquareLaw. This yields comparability, reproducibility, and slice-local refresh.

### [[00_E.18 - Transformation Flow Structure|E.18]]:5.3 - S3 - Publication discipline (faces)

[[00_E.18 - Transformation Flow Structure|E.18]] **imports [[03_E.17 - Multi-View Publication Kit|E.17]]** wholesale **and associates MVPK faces with `PublicationScope` (USM)**.
...
[[00_E.18 - Transformation Flow Structure|E.18]] **does not re-specify** these laws; it only adds **structure-scope obligations** for faces published over transformation-flow paths:
...
2. **Gate requirement on cited editions.** Any face that references editions of `CG-Spec`, `ComparatorSet`, or `UNM.TransportRegistryPhi` includes **`BridgeCard + UTS row`**; faces without this are treated as **non-consumable downstream**. Bridge and terminology-synchronization checks are governed by [[00_F.09 - Alignment & Bridge across Contexts|F.9]], [[00_F.17 - Unified Term Sheet (UTS)|F.17]], [[03_E.17 - Multi-View Publication Kit|E.17]], and [[00_E.18 - Transformation Flow Structure|E.18]]; selection and comparator pressure stays with [[12_A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]], [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]], [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]], [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]], or [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] for current selector or comparator cases.
...
4. **Gamma_time on compare and launch faces.** All compare and launch faces on [[00_E.18 - Transformation Flow Structure|E.18]] paths pin `Gamma_time`; implicit *latest* is not admissible. [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] carries current GateProfile binding and minimum profile semantics; [[00_E.18 - Transformation Flow Structure|E.18]] paths include the pin. **CHR avoids acceptance thresholds** (*NoThresholdsInCHR*); thresholding and launches are carried by [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], Part G, and `U.Work` for current launch or thresholding cases. Unknowns remain tri-state (`pass|degrade|abstain`) and fold per the active GateProfile ([[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]).

> **Reminder.** MVPK already bans "signature" on faces, I/O re-listing, arithmetic on faces, and unpinned numeric content ([[03_E.17 - Multi-View Publication Kit|E.17]] §5.4-5.5). [[00_E.18 - Transformation Flow Structure|E.18]] **does not weaken or override** those rules; it only constrains how they are used along transformation-flow paths.
...
**Decision stability & idempotency (gate-local).** Gate decisions are stable under a declared equivalence relation over the pins used by [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]; the witness is recorded as `DecisionLog` or `EquivalenceWitnessRef`, with [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]] or [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] used for cases involving evidence-path visibility or refresh implications. [[00_E.18 - Transformation Flow Structure|E.18]] **does not** prescribe storage formats, key shapes, or hashing schemes.
...
Treat a step as a **EntityOfConcernRef/kind** transition (including `StructuralReinterpretation` under CC-E18‑06‑EX) **iff** the **UTS row**:
  — satisfies the minimal bridge and terminology-synchronization obligations of [[00_F.09 - Alignment & Bridge across Contexts|F.9]], [[00_F.17 - Unified Term Sheet (UTS)|F.17]], [[03_E.17 - Multi-View Publication Kit|E.17]], and [[00_E.18 - Transformation Flow Structure|E.18]] for the current crossing case (identity, `ReferencePlane`, `CL/CL^plane`, edition pins for `CG-Spec`, `ComparatorSet`, `UNM.TransportRegistryPhi`, `ComparatorSetRef`, `BridgeId`, and `Phi-RuleIds`), and
  — is additionally marked as a **KindBridge** per [[00_C.03 - Kinds, Intent-Extent, and Typed Reasoning (Kind-CAL)|C.3]] (`bridgeChannel=Kind`, `CL^k`, mapping or signature‑translation, order‑preservation claims, loss notes, definedness area, determinism).
...
### [[00_E.18 - Transformation Flow Structure|E.18]]:5.4 - S4 - Assurance‑operations on `U.Transfer` (counterfactual admissibility)
...
### [[00_E.18 - Transformation Flow Structure|E.18]]:5.5 - S5 - Comparability & aggregation (normalize‑then‑compare; counterfactual form)
...
**Edition-aware set/archive publication records (e.g., QD archives) pin `DescriptorMapRef.edition`, `DistanceDefRef.edition`, and `CharacteristicSpaceRef.edition` when applicable; refresh is slice-local. Comparator, archive, and refresh checks are governed by [[12_A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]], [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]], [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]], [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]], [[00_G.09 - Parity and Benchmark Harness|G.9]], and [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] for current archive or refresh cases.**

### [[00_E.18 - Transformation Flow Structure|E.18]]:5.6 - S6 - Cycle discipline (Selection ↔ Planning)
...
### [[00_E.18 - Transformation Flow Structure|E.18]]:5.7 - S7 - Selector semantics ([[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]) & parity harness ([[00_G.09 - Parity and Benchmark Harness|G.9]])
[[00_E.18 - Transformation Flow Structure|E.18]] keeps set-return, archive preservation, and comparator refs visible along the path. It does not define selector, archive, dominance, or comparator semantics; those remain with [[12_A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]], [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]], [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]], [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]], [[00_G.09 - Parity and Benchmark Harness|G.9]], and [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] for current selector or comparator cases.
...
### [[00_E.18 - Transformation Flow Structure|E.18]]:5.8 - S8 - Guard aggregation assignment and handling (USM §1.2)
...
**GateProfile data shape (cross-reference).** [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] carries the current GateProfile binding and minimum profile semantics. [[00_E.18 - Transformation Flow Structure|E.18]] names the structure only where crossings need it; fuller profile-matrix material is not a separate current authority unless a current governing pattern explicitly admits it.

**Bridge-aware guards (cross-reference).** USM guards apply bridge-translation semantics (`translate(Bridge, Scope)`) with CL penalties in R-lane; guard vocabulary is governed by **[[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]**, while gate aggregation remains in [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]].

**Error/timeout/unknown (profile-bound).** GateCheck errors/timeouts fold to **`degrade`** under `Lean\|Core` and to **`block`** under `SafetyCritical\|RegulatedX`; `unknown` follows the GateCheck's governing rule (safety-default: `degrade`). The [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] DecisionLog record and equivalence witness carry decision stability; [[00_E.18 - Transformation Flow Structure|E.18]] does not define storage or key structures.

### [[00_E.18 - Transformation Flow Structure|E.18]]:5.9 - S9 - Transport & crossings
* Cross-Context or cross-plane transfer relations appear as **GateCrossings** that include a **Bridge** with **CL** policy; **Phi(CL)/Phi_plane** are published; penalties appear **in R only**; **Scope membership** (USM) is unchanged by crossings. **SquareLaw is checked within a single `DesignRunTag`; a `T^D<->T^R` change is modelled as a pair of coordinated gates with `DesignRunTagFrom/To` and the selected [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] work or publication locus for the current case.**
...
### [[00_E.18 - Transformation Flow Structure|E.18]]:5.10 - S10 - Non‑mechanism boundary
...
### [[00_E.18 - Transformation Flow Structure|E.18]]:5.11 - S11 - Coordination wording labels (when current)
...
### [[00_E.18 - Transformation Flow Structure|E.18]]:5.12 - S12 - Viewpoint Families To [[00_E.18 - Transformation Flow Structure|E.18]] Constructs (neutral, holonic)
**S12 status.** S12 is secondary viewpoint-mapping input for a current viewpoint-family mapping claim. It is not the ordinary [[00_E.18 - Transformation Flow Structure|E.18]] core for naming a selected structure, flow valuation, path slice, or crossing.

[[00_E.18 - Transformation Flow Structure|E.18]] does not mint new viewpoint or view kinds. It **imports** the generic multi-view machinery of [[00_E.17.00 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]] `U.MultiViewDescribing`, bundles from [[01_E.17.01 - U.ViewpointBundleLibrary - Reusable Viewpoint Bundles|E.17.1]], and the TEVB engineering bundle from [[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]. S12 only describes how these existing `U.Viewpoint` / `U.ViewpointBundle` ids are *used* in transformation-flow structures and in `UTS.ViewpointMap`; intent/concern semantics are governed by E.17.0-E.17.2.

**Two-part use of TEVB and MVPK (ISO 42010 summary, no local re‑definition).**

* **Engineering viewpoints.** For engineering holons, [[00_E.18 - Transformation Flow Structure|E.18]] assumes a TEVB bundle with `ViewFamilyId = VF.TEVB.ENG`. `EngineeringVPId` is one of `{VP.Functional, VP.Procedural, VP.RoleEnactor, VP.ModuleInterface}`, and TEVB is the normative source for their semantics. [[00_E.18 - Transformation Flow Structure|E.18]] does not refine these viewpoints.
* **Publication viewpoints.** Publication viewpoints come from MVPK ([[03_E.17 - Multi-View Publication Kit|E.17]]); `PublicationVPId` is a `MVPK.ViewpointId` that governs faces under a `PublicationScope`.
* **Architecture relation.** [[00_E.18 - Transformation Flow Structure|E.18]] can supply the selected transformation-flow structure used by an `ArchitectureOf@Context` claim when `DescriptionContext`, described holon, bounded context, and structure kind are explicit. It does not define architecture itself, and a transformation-flow structure is not the functional architecture by default. Use [[00_C.30 - Grounded Architecture and Selected-Structure Adequacy|C.30]], [[04_C.30.ASV - Architecture Structural View Adequacy (ASV)|C.30.ASV]], and the architecture transformation-flow relation pattern when the selected structure is used in an architecture-flow relation. Crossings and penalties follow [[00_E.18 - Transformation Flow Structure|E.18]] gating rules (S9; CC-E18-11/23) but do not change viewpoint semantics.
* **Separation of roles.** `VP.*` from TEVB are **EngineeringVPId** values only; they are not publication faces. `PublicationVPId` values are defined in MVPK. The mapping between them is entirely via ISO-style correspondences and the `UTS.ViewpointMap`; [[00_E.18 - Transformation Flow Structure|E.18]] does not define a second notion of viewpoint.

**Context object scope (summary).**

* **Engineering described object.** The engineering entity described by TEVB may be a holon (`U.System` or `U.Episteme`) per TEVB's `EntityOfConcernClassSpec`. That holon is the context object whose transformations, work, interfaces, or viewpoints may be structured. [[00_E.18 - Transformation Flow Structure|E.18]] does not make that holon its own EoC unless the selected transformation-flow structure itself is under concern.
  Transformation, method, procedure/control, role-enactor structure, structural architecture, module, interface, and allocation terms are viewpoint concern/content about that holon unless a concrete [[00_E.18 - Transformation Flow Structure|E.18]] use explicitly opens [[08_A.06.04 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] retargeting with `KindBridge (CL^k)`, retargeting witness, and the applicable species rule.
* **Publication described object.** MVPK can treat the *architecture description* itself as an EntityOfConcern; publication viewpoints for that AD are defined in MVPK, not here. [[00_E.18 - Transformation Flow Structure|E.18]] only checks that such faces honor MVPK discipline and [[00_E.18 - Transformation Flow Structure|E.18]] crossing rules when they publish selected transformation-flow material.

**Naming rules (aligned with [[03_E.17 - Multi-View Publication Kit|E.17]].0/E.17.1/E.17.2).**
* `ViewFamilyId` is the `U.ViewpointBundle.viewFamilyId` (e.g. `VF.TEVB.ENG` for TEVB); its lexical and ontological discipline is governed by [[01_E.17.01 - U.ViewpointBundleLibrary - Reusable Viewpoint Bundles|E.17.1]].
* `EngineeringVPId : ViewpointId` is always a `U.ViewpointId` drawn from some bundle (for TEVB, one of `{VP.Functional, VP.Procedural, VP.RoleEnactor, VP.ModuleInterface}`). [[00_E.18 - Transformation Flow Structure|E.18]] never defines new `VP.*` ids.
* `PublicationVPId : ViewpointId` is a `MVPK.ViewpointId` defined in [[03_E.17 - Multi-View Publication Kit|E.17]]; TEVB viewpoints are **never** reused as publication viewpoints (per TEVB guard and MVPK).
...
* Within S12, “viewpoint”, “view” and “correspondence” have exactly the meanings given in [[00_E.17.00 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]]; “publication face” means an MVPK face (`PlainView`, `TechCard`, `InteropCard`, `AssuranceLane`) under some `PublicationVPId`.
* Faces are **carriers for views**: a face is part of a view only when linked via an ISO‑style `CorrespondenceRef` to an engineering `U.View` under some `EngineeringVPId`; S12 does not add extra conditions beyond [[03_E.17 - Multi-View Publication Kit|E.17]].0/E.17.2.
...
**Purpose.** Provide a neutral ([[00_F.18 - Local-First Unification Naming Protocol|F.18]]) mapping from TEVB engineering viewpoint families - bundle `VF.TEVB.ENG` with `VP.Functional / VP.Procedural / VP.RoleEnactor / VP.ModuleInterface` - to [[00_E.18 - Transformation Flow Structure|E.18]] constructs so that the same holon can be described through functional, procedural, role-enactor, or module-interface viewpoints while the [[00_E.18 - Transformation Flow Structure|E.18]] construct scope remains explicit. S12 does not introduce new `U.Viewpoint` or `U.View` kinds, and it does not claim that all such views share one underlying transformation-flow structure unless the structure, EntityOfConcernRef, and correspondence refs are declared.

**Holon target.** The mapping applies to any holon, with the constraint that only `U.System` enacts `U.Work` (A.3/A.15). Supervisory and structural hierarchies remain distinct ([[04_B.02.05 - Supervisor-Subholon Feedback Loop|B.2.5]]).

**Viewpoint family to primary [[00_E.18 - Transformation Flow Structure|E.18]] constructs (TEVB-aligned)**
*All four families referenced below are TEVB engineering viewpoints; the "what ..." clauses are interpretive glosses for how they *use* [[00_E.18 - Transformation Flow Structure|E.18]] constructs. Formal intent/concerns/allowed episteme kinds remain in TEVB ([[02_E.17.02 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]).*
1) **Function-Oriented View (`EngineeringVPId = VP.Functional`, capability and transformation viewpoint)** - "what transformation is achieved under roles"
    ...
    * **Publication:** MVPK publication faces per [[03_E.17 - Multi-View Publication Kit|E.17]]; comparable claims pin to `CG‑Spec/ComparatorSet` editions; crossings are published through `Bridge+UTS` and `CL/CL^plane` (penalties → **R‑lane** only).
    * **Checks:** [[00_A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] (CV) inside transformations; [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] (GateFit) at gates; comparator, set-return, and No-Hidden-Scalarization discipline is carried through [[12_A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]], [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]], [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]], [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]], [[00_G.09 - Parity and Benchmark Harness|G.9]], and [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] for current selector or comparator cases.
...
2) **Procedure‑Oriented View (`EngineeringVPId = VP.Procedural`, step/time storyboard)** — “what steps occur and when”
    * **FPF constructs:** `U.WorkPlan` ([[02_A.15.02 - U.WorkPlan|A.15.2]]) for intent/schedule; `U.WorkEnactment` for enactment.
    ...
    * **Holonic note:** Applies to any `U.System` scope (single holon or a supervised sub‑holon cluster); supervisory structure is handled by roles rather than structural mereology ([[04_B.02.05 - Supervisor-Subholon Feedback Loop|B.2.5]]).
3) **Role‑Enactor / Device‑Structure View (`EngineeringVPId = VP.RoleEnactor`)** — “what carrier/ports/constraints exist; who typically enacts it”
    ...
    * **Publication:** MVPK faces are **typed projections**, not `U.Work` records or execution carriers; faces add **no new numeric claims** ([[03_E.17 - Multi-View Publication Kit|E.17]]). Constraints and compatibility appear as CV checks ([[00_A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]]).
    * **Holonic note:** Structural mereology (part/whole of the carrier) is modeled in Part A; [[00_E.18 - Transformation Flow Structure|E.18]] ties interface/exposure semantics to mathematical-lens expressions and gates only when those are current.
    ...
    * **Role‑label guard.** `TypicalEnactorRoleName` is **pedagogical only** and is not used as a GateFit role; GateFit uses `U.Role` ([[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]).
4) **Module‑Interface View (`EngineeringVPId = VP.ModuleInterface`, physical/logical module structure)** — “what modules exist and how they specify commitments and constraints across interfaces”
    ...
    * **Holonic note:** The same module can appear as a holon in multiple views; supervisory loops ([[04_B.02.05 - Supervisor-Subholon Feedback Loop|B.2.5]]) remain orthogonal to structural composition.
This is an expandable list of viewpoint families; [[00_E.18 - Transformation Flow Structure|E.18]] is intentionally viewpoint-neutral. Additional engineering bundles beyond TEVB (safety, mission, information, ...) are introduced as separate `U.ViewpointBundle` species via [[03_E.17 - Multi-View Publication Kit|E.17]].1/E.17.2; S12 does not define them.

**View-family label discipline for transformation-flow loci (recognition-only).**
*Scope.* When a viewpoint-family mapping claim is current, a pattern or domain profile may declare `LocusViewFamilyLabels[]` for transformation-flow locus labels so practitioners can recognize familiar engineering wording while the selected structure stays governed by [[00_E.18 - Transformation Flow Structure|E.18]]. Semantics come from the referenced `U.ViewpointBundle`, [[00_E.18 - Transformation Flow Structure|E.18]] locus binding, and MVPK correspondences; labels are recognition aids, not loci, viewpoints, publication faces, checks, or work records.
...
4. Twin registers are allowed as Tech and Plain labels per [[00_E.10 - Unified Lexical Rules for FPF|E.10]]; naming follows [[00_F.18 - Local-First Unification Naming Protocol|F.18]] local-first discipline.
...
Publish a UTS block named `ViewpointMap` only when an engineering or publication viewpoint-family mapping claim is made or consumed. Ordinary [[00_E.18 - Transformation Flow Structure|E.18]] use does not require `UTS.ViewpointMap` when the question under repair is only the selected structure, flow valuation, path slice, or crossing.
...
(iii) Rows that carry edition identifiers also include `BridgeCard + UTS` rows through [[00_F.09 - Alignment & Bridge across Contexts|F.9]], [[00_F.17 - Unified Term Sheet (UTS)|F.17]], [[03_E.17 - Multi-View Publication Kit|E.17]], and [[00_E.18 - Transformation Flow Structure|E.18]]; edition-bearing faces that lack such rows are not admissible for downstream consumption.
...
## [[00_E.18 - Transformation Flow Structure|E.18]]:6 - Archetypal Grounding (Tell–Show–Show; concise)
...
## [[00_E.18 - Transformation Flow Structure|E.18]]:7 - Conformance — **Unified checklist (normative)**
...
| **CC-E18-02 — Locus kinds bind governed values** | Loci are structure-positioned bindings to governed values. The current minimal locus baseline is `{Transformation, Signature, Mechanism, WorkPlanning, Work, Check, StructuralReinterpretation}`. Domain-specific species are open-world and non-exhaustive; they bind to one of these locus kinds or require an explicit [[00_E.18 - Transformation Flow Structure]] update. The baseline is **not** a local ontology: `Transformation -> [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)]].4`, `Signature -> [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/00_A.06 - Signature Stack & Boundary Discipline]].0`, `Mechanism -> [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/00_A.06 - Signature Stack & Boundary Discipline]].1/E.20`, `WorkPlanning -> [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/00_A.15 - Role-Method-Work Alignment (Contextual Enactment)]].2`, `Work -> [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/00_A.15 - Role-Method-Work Alignment (Contextual Enactment)]].1`, `Check -> [[A_Kernel Architecture Cluster/20_U.Flow.ConstraintValidity - Eulerian/00_A.20 - U.Flow.ConstraintValidity - Eulerian]] or A.21`, and `StructuralReinterpretation -> [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/08_A.06.04 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism]] plus E.18/A.20` where CV is current. A morphism expression is a mathematical-lens view when current, not the FPF kind of every locus. | Type registry shows at least the listed locus kinds; additional species map to one of them; checks are realized as `OperationalGate` when a gate/check locus is present (see CC-E18-06-EX/11). **Lint:** registry/table exposes `{species -> {locusKind, governingPattern}}`; missing or mismatched governing pattern fails. |
...
| **CC-E18-06-EX (strictly limited) — Projection retargeting without gate** | A locus of kind **`StructuralReinterpretation`** retargets the **published projection** without invoking `OperationalGate` **only if all hold**: **(a)** `CtxState` is preserved; **(b)** any **EntityOfConcernRef** change has a **KindBridge** (`CL^k`) entry on MVPK/**UTS**; **(c)** a **SquareLaw-retargeting witness** is present (on UTS); **(d)** the operation is **PathSlice-local** (`PathSliceId` pinned); **(e)** **no plane/unit change** occurs (plane/unit changes remain gated); **(f)** **CV.ReinterpretationEquivalence** ([[00_A.20 - U.Flow.ConstraintValidity - Eulerian]]) is `pass`; **(g)** **NoHiddenScalarization** - if the step concerns a comparable return shape, set/partial-order semantics are preserved and comparators remain ref-only (current comparator and set-return loci). | UTS row includes `bridgeChannel=Kind` and `CL^k`; SquareLaw-retargeting witness present; PathSliceId pinned; CV status recorded; no scalarization detected. |
...
> **Scope note ([[00_E.18 - Transformation Flow Structure|E.18]] vs neighbor patterns):** Detailed mechanism-scoped checks and publication obligations are governed by the current neighbor patterns named in this pattern's Relations. [[00_E.18 - Transformation Flow Structure|E.18]] fixes only selected-structure obligations: single `U.Transfer` relation kind, gate crossings, valuation, publication pins, CV/GF boundary, and slice-local refresh.
...
* *Signature locus* - structure-positioned use of **[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]** `U.Signature` (universal block). It is a governed value bound into the selected structure, not a local kind and not a `C.3.2 KindSignature`.
* *KindSignature ([[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]])* - definition of a `U.Kind` by intent, extent, and formality; **unrelated** to [[00_E.18 - Transformation Flow Structure|E.18]] locus kinds; never a `genus`.
* *Species (domain-scoped)* — typed specialisations `speciesOf(kind=...)` that declare `KindDefinition=<current governing pattern id>` (e.g., `kind=Mechanism; KindDefinition=[[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/00_A.06 - Signature Stack & Boundary Discipline|A.6]].1`).
...
* **GateCheckKind boundary.** `GateCheckKind` is a publication/check lexeme used inside `GateCheckRef`, not a structure locus kind. No `GateCheckKind` becomes an [[00_E.18 - Transformation Flow Structure|E.18]] `Check` locus unless an `OperationalGate(profile)` locus is actually present.

* **GateCheckRef shape (publication lexeme governed by [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]).** Where publication faces over a selected structure carry GateChecks, a **GateCheckRef** is a record
  defined by [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]; [[00_E.18 - Transformation Flow Structure|E.18]] constrains only where such faces carry those refs along transformation-flow paths.
...
* **GateFit (aspect, not an entity).** GateFit names the **aspect** of checks that evaluate **profile‑fit**; there is no separate GateFit entity. “Gate decision under GateFit” means “the gate’s decision computed from GateChecks with `aspect=GateFit`”.
  ...
  This shape is publication-only; it introduces no new execution steps and no arithmetic on faces.  (Couples to [[00_A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] or [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] without duplicating their check catalogs.)
* *VALATA (VA/LA/TA)* — value-annotation scheme used on **AssuranceLane**; **carriers** are referenced via **SCR/RSCR**; detailed evidence obligations are governed by [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] and the named evidence, publication, or crossing pattern for the current case. Included here so evidence pins are self-describing in Part E texts.
...
## [[00_E.18 - Transformation Flow Structure|E.18]]:8 - Gating Profiles (applied to [[00_E.18 - Transformation Flow Structure|E.18]])
This table is a selected-structure coverage table for [[00_E.18 - Transformation Flow Structure|E.18]] crossings and path slices. It is not the source of `GateProfile` semantics. [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] governs gate decision semantics, folds, `DecisionLog` minima, and the GateFit check-catalog boundary.

> Gating is expressed as **publication-gating** per [[03_E.17 - Multi-View Publication Kit|E.17]] profiles. The structure model aligns with the **CC items** listed for the chosen profile; broader obligation profiles include all narrower-profile items.
...
## [[00_E.18 - Transformation Flow Structure|E.18]]:10 - Consequences
...
c) **Tooling alignment:** some incumbent DAG-only orchestrators conflict with budgeted cycles and set-return semantics; adapters project [[00_E.18 - Transformation Flow Structure|E.18]] semantics to their interop boundary, while [[02_E.18.02 - Transformation Flow Mathematical Description|E.18.2]] carries the mathematical graph-description relation when that projection matters.

## [[00_E.18 - Transformation Flow Structure|E.18]]:11 - Rationale

[[00_E.18 - Transformation Flow Structure|E.18]] states **strict separation of concerns** (selected-structure scope only); **specialized semantics are governed by the patterns named below for those current relations**:
...
## [[00_E.18 - Transformation Flow Structure|E.18]]:12 - SoTA‑Echoing (post‑2015, multi‑Tradition)

> Each row states the source idea, the FPF invariant [[00_E.18 - Transformation Flow Structure|E.18]] adopts, the practitioner move it changes, and the shortcut it rejects. Vendor, tool, and literature tokens are informative; the invariant and practitioner move carry the pattern explanatory work.
...
| **Operads, wiring diagrams, and hypergraph categories** (Spivak, *The operad of wiring diagrams*, arXiv [`1305.0297`](https://arxiv.org/abs/1305.0297); Baez & Fong, *A Compositional Framework for Passive Linear Networks*, arXiv [`1504.05625`](https://arxiv.org/abs/1504.05625)). | Typed ports and interface junctions motivate Bridge/CL/Phi pins at crossings; [[00_E.18 - Transformation Flow Structure]] adapts the math by requiring publication pins that the math alone does not supply. | When an interface or boundary crossing matters, publish the Bridge, UTS row, CL/CL^plane, and R-lane penalty placement instead of leaving an unpinned junction. | Treating an interface diagram, wiring diagram, or decorated cospan as sufficient crossing evidence. |
...
*Cross-tradition note.* Rows 1-3 (compositional graph practice), rows 4-5 (publication and reproducibility practice), row 6 (controls/robotics), row 7 (evolutionary search), and row 8 (PL/semantics) jointly position [[00_E.18 - Transformation Flow Structure|E.18]] across multiple traditions per [[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]], but each row is retained only because it changes a practitioner move or rejected overread.

## [[00_E.18 - Transformation Flow Structure|E.18]]:13 - Bias‑Annotation (per [[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]] SG‑bias slot)

* **Acyclic-bias risk.** Tooling accustomed to DAGs may discourage admissible feedback loops; [[00_E.18 - Transformation Flow Structure|E.18]] explicitly permits loops with budget/sentinel controls (CC-E18-13, -18).
* **Scalarization-bias risk.** Cultural defaults to single-score rankings can suppress Pareto/QD sets; [[00_E.18 - Transformation Flow Structure|E.18]] keeps declared order relations and return sets visible (CC-E18-10, CC-E18-12).
* **Interop-dominance risk.** File/format ecosystems (CWL/RO-Crate/lineage) can be mistaken for semantic sources; [[00_E.18 - Transformation Flow Structure|E.18]] places them in **InteropCard** and keeps governing semantics in loci/gates.
* **Over-formalization risk.** Category-theoretic formalisms can obscure operational guard-rails; [[00_E.18 - Transformation Flow Structure|E.18]] grounds crossings in Bridge/UTS/CL/Phi pins and SquareLaw audits (CC-E18-11, -17).
* **Retrospective rewrite risk.** Global rewrites break replay; [[00_E.18 - Transformation Flow Structure|E.18]] confines them to edition bumps and slice-local refresh (CC-E18-16).
...
## [[00_E.18 - Transformation Flow Structure|E.18]]:14 - Relations (explicit pattern-to-pattern relations)

> Relation rows are typed as **builds_on / constrains / coordinates / specializes / publishes_on / requires / provides_checks_for**.

**Foundations**
* **[[00_E.18 - Transformation Flow Structure|E.18]] -> builds_on -> [[03_E.17 - Multi-View Publication Kit|E.17]] MVPK (for publications of selected structure material).** Faces, pins, lanes, functorial publication, Lean/Core/Regulated profiles.
* **[[00_E.18 - Transformation Flow Structure|E.18]] -> builds_on -> [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] U.Signature / [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] U.Mechanism.** Locus kinds and governing-definition content boundaries.
* **[[00_E.18 - Transformation Flow Structure|E.18]] -> builds_on -> [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] Strict Distinction (EntityOfConcern, Description episteme, Description episteme admitted for specification use, and publication/carrier separation).** No new claims on faces; publication faces project selected structure, crossing, or flow-valuation information without becoming the governed selected structure, Description episteme, specification use, evidence, gate decision, work occurrence, or carrier.

**Flow semantics & checks**
* **[[00_E.18 - Transformation Flow Structure|E.18]] -> coordinates -> [[00_A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] U.Flow (ConstraintValidity scope).** CV checks apply inside transformations; no declaration/translation of planes/units in CV; **error/timeout/unknown folds** follow **CC-E18-22** as the **minimum default** (profiles can be stricter).
  **Terminology discipline ([[00_A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] boundary).** In CV scope, publications use **status/witness** language; **GateDecisionRationale/GateDecisionExplanation** are reserved for gating and do not apply to CV.
* **[[00_E.18 - Transformation Flow Structure|E.18]] -> coordinates -> [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] GateProfilization (GateFit scope).** **GateFit-scoped GateChecks** are aggregated by `OperationalGate(profile)` with CV=>GF activation; the **enumeration and publication shape** of GateChecks are governed by **[[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]**. **Equivalently:** a GateFit decision different from `abstain` appears only when aggregated `ConstraintValidity = pass`; otherwise the **GateDecisionExplanation (GateFit-oriented)** does not apply.
* **[[00_E.18 - Transformation Flow Structure|E.18]] -> uses -> USM.CompareGuard and USM.LaunchGuard.** Guards publish scope and responsible gate; guard failures are handled by the declared gate.
* **[[00_E.18 - Transformation Flow Structure|E.18]] -> constrains -> F.9/F.17 bridge and terminology-synchronization loci (Bridge+UTS, CL/CL^plane, Phi -> R).** A transition is treated as a **Crossing** iff `Bridge+UTS` and the appropriate `CL/CL^plane` are published; otherwise this crossing explanation does not apply. Where Phi defines penalties, they appear in the R-lane only.
...
**UNM & comparability**
* **[[00_E.18 - Transformation Flow Structure|E.18]] -> constrains -> UNM declaration and use loci.** `CG-Spec`, `ComparatorSet`, and `UNM.TransportRegistryPhi` declarations are governed by UNM; normalize-then-compare is mandatory.
* **[[00_E.18 - Transformation Flow Structure|E.18]] -> constrains -> [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] SelectionAndTuning.** Set-returning, comparator-pinned decisions, no hidden scalarization; `MethodTuning` without launch-value slot filling.
* **[[00_E.18 - Transformation Flow Structure|E.18]] -> constrains -> [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] EvaluatingAndRefreshing.** EditionBumpProposal, two-phase update through the UNM declaration locus, path-local refresh.

**Work boundary**
* **[[00_E.18 - Transformation Flow Structure|E.18]] -> coordinates with -> [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] U.WorkEnactment (`FinalizeLaunchValuesOnlyInWork`).** Single point of `FinalizeLaunchValues`; `FreshnessUpToDate` hard at LaunchGate; acceptance/telemetry published here.

**Structure & reuse**
* **[[00_E.18 - Transformation Flow Structure|E.18]] -> provides selected-structure base for transformation-flow families.** Flow patterns such as P2W and EvaluatingAndRefreshing use [[00_E.18 - Transformation Flow Structure|E.18]] for selected structure, valuation, crossings, guards, MVPK faces, and slice-local refresh. The current ontology is: [[00_A.03.04 - U.Transformation- Bounded Change Under Conditions|A.3.4]] governs each bounded `U.Transformation`, [[00_E.18 - Transformation Flow Structure|E.18]] governs the selected compound structure over transformations and adjacent governed loci, and neighbor patterns govern method, work, mechanism, evidence, publication, gate, decision, and refresh claims when those claims are current.
* **[[00_E.18 - Transformation Flow Structure|E.18]] -> coordinates with -> architecture transformation-flow relation patterns.** When a selected transformation-flow structure is used in an architecture-flow relation, the architecture transformation-flow relation pattern records the relation between `TransformationFlowStructure` and `ArchitectureOf@Context`; [[00_E.18 - Transformation Flow Structure|E.18]] keeps selected structure, crossing, and flow-valuation discipline.
* **[[00_E.18 - Transformation Flow Structure|E.18]] -> publishes_on -> [[03_E.17 - Multi-View Publication Kit|E.17]] MVPK views** (`PlainView`, `TechCard`, `InteropCard`, `AssuranceLane`) for every transfer/locus where publication occurs; Lean mode applies only as per profile.

## [[00_E.18 - Transformation Flow Structure|E.18]]:15 - Conformance Use Checks
...
Relation boundary: [[00_E.18 - Transformation Flow Structure|E.18]] governs selected transformation-flow structures for structures of atomic `U.Transformation` values and their structure-positioned slot-filler loci. It does not define a second change ontology, a work sequence, a method, a mechanism, a mathematical graph expression, or a publication record. When a selected-structure use raises bounded-transformation, dynamics-episteme, temporal-aspect, temporal-claim adequacy, work planning, performed work, evidence, assurance, gate, decision, architecture, structural-view, mechanism, selector, comparison, refresh, publication, or wording-use claims, name the governing pattern for that relation before relying on the structure.

When a selected structure locus, selected path, path slice, substructure, or flow valuation expresses, decomposes, or constrains one bounded transformation relation, use [[00_A.03.04 - U.Transformation- Bounded Change Under Conditions|A.3.4]] for the atomic `U.Transformation` claim and use [[00_E.18 - Transformation Flow Structure|E.18]] for the selected structure, containing locus, pins, locus kind, crossing, publication, comparability, and refresh discipline. [[00_E.18 - Transformation Flow Structure|E.18]] locus kinds do not automatically fill neighboring slots: `Transformation` points to [[00_A.03.04 - U.Transformation- Bounded Change Under Conditions|A.3.4]], `Signature` points to [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]], `Mechanism` points to [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] and [[00_E.20 - Mechanism Introduction Protocol|E.20]], `WorkPlanning` and `Work` point to the [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] work family, and `Check` points to [[00_A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] or [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] according to the current claim.

## [[00_E.18 - Transformation Flow Structure|E.18]]:15a - [[01_E.18.01 - Principles-to-Work Carry-Through|E.18.1]] P2W Child-Pattern Relation

[[01_E.18.01 - Principles-to-Work Carry-Through|E.18.1]] is a child pattern for principles-to-work carry-through. It inherits this pattern's selected structure, path, flow-valuation, transfer, crossing, and gate minimum, then adds the local P2W relation from accepted problem-side output to the next FPF kind named by value, relation, record, or application. Pilot examples for one specialization belong in the selected child pattern that uses them; [[00_E.18 - Transformation Flow Structure|E.18]] keeps only the selected-structure law and this short child-pattern relation.

## [[00_E.18 - Transformation Flow Structure|E.18]]:End
