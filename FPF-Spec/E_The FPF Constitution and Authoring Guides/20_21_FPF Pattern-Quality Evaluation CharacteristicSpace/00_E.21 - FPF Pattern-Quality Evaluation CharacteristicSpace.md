...
## [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:2 - Problem
...
[[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] solves this by measuring the pattern of concern against one complete coordinate set. The coordinates ask whether the pattern is usable, coherent, current, precise, affordable, mature enough for its claim, and safe from proxy improvement.

## [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:3 - Forces
...
## [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:4 - Solution

[[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] is the FPF pattern-quality specialization of [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]. It evaluates one pattern of concern under one declared quality claim.
...
| `EvaluationEvidenceBasis` | Evidence loci named by value for the evaluation: pattern body version, host or monolith section, README scenario, ToC row, [[00_E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline]] entry-distribution locus, [[00_I.02 - Expanded Entry Disambiguation Cases]] expanded entry-disambiguation case when corpus-facing, card or retrieval cue when claimed, source-currentness locus when SoTA/currentness is valued, mature comparator set when maturity is valued, and worked case or absence of worked case when case coverage is valued. |
| `QualityEvaluationQuestionFrameRef` | [[00_E.22 - Improvement-Oriented Quality Evaluation Question Framing]] frame when purpose, floor, trade-offs, absorption, or proposal expectation needs to be declared. |
...
| `PatternQualityStatus` | Scoped pattern-quality result assigned by [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]]; it is not an [[00_E.19 - Pattern Quality Gates- Review and Refresh Profiles]] admission or refresh decision by itself. |
| `StopCondition` | Why improvement may stop, continue, refresh, or hold. |
Names are local to pattern-quality evaluation unless [[00_F.18 - Local-First Unification Naming Protocol|F.18]] promotes a durable name. They are not project evidence, release state, review state, or assurance.

### [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:4.2 - Evaluation record
...
### [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:4.3 - Ordinal scale, result row, and adjacent-value rationale
...
|---|---:|---|
| `<[[E_The FPF Constitution and Authoring Guides/20_21_FPF Pattern-Quality Evaluation CharacteristicSpace/00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]] coordinate>` | `<0..5>` | `<assigned-value basis; why the lower adjacent value would understate the evidence; why the higher adjacent value would overstate the evidence, or for 5 what evidence makes 4 too weak and what would lower or reopen>` |

A two-column coordinate-and-value table, a narrative paragraph, a table whose comment lacks adjacent-value comparison, or a result whose value depends on unchecked external loci is not an [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] result. It is only draft evaluation material until every coordinate has a `ShortRationale` row and the result names the `EvaluationEvidenceBasis` used for values that depend on source, comparator, corpus, projection, or worked-case evidence.

A `ShortRationale` is allowed to be compact, but it is not allowed to be evidenceless. When the value depends on a source-currentness row, mature comparator, README scenario, ToC row, [[00_E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline|E.11]] entry-distribution locus, [[00_I.02 - Expanded Entry Disambiguation Cases|I.2]] expanded entry-disambiguation case, card, retrieval cue, monolith section, worked slice, near-miss, or anti-case, the rationale names that locus by value or says that the locus was missing or unchecked. "By value" means a recoverable section, row, case, checklist item, relation, source row, projection row, comparator id plus selected ingredient, or specific absent locus; a category list such as "entry, first move, boundaries, SoTA, checklist, relations" is not by-value discharge. Missing or unchecked evidence lowers the value for the coordinate that needs it; it does not create a separate "not evaluated" result.
...
### [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:4.3a - Precision-restoration profile
...
PrecisionRestorationProfile:
  ...
  wordHeadUsePrecision: <clean | [[E_The FPF Constitution and Authoring Guides/09_10_Unified Lexical Rules for FPF/00_E.10 - Unified Lexical Rules for FPF|E.10]], [[E_The FPF Constitution and Authoring Guides/09_10_Unified Lexical Rules for FPF/01_E.10.ARCH - Wording-Use Ontological Precision Restoration Architecture|E.10.ARCH]], [[F_The Unification Suite (U-Suite)- Concept-Sets, SenseCells & Contextual Role Assignment/18_Local-First Unification Naming Protocol/00_F.18 - Local-First Unification Naming Protocol|F.18]], or governing pattern needed | lowers coordinates>
  ...
  phraseApparatus: <clean | [[F_The Unification Suite (U-Suite)- Concept-Sets, SenseCells & Contextual Role Assignment/19_Ontology-First Plain Technical Rewriting/00_F.19 - Ontology-First Plain Technical Rewriting|F.19]] needed | lowers coordinates>
...
### [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:4.4 - RequiredPatternQualityCoordinates

Every [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] evaluation of an FPF pattern of concern evaluates every coordinate below.
...
| `ProxyForValueSubstitutionResistance` | Whether the evaluation asks what became worse when visible quality coordinates improved, and applies [[00_E.13 - Pragmatic Utility and Value Alignment]] when a visible quality value, metric, review result, or release cue is being used as the practical value itself. |
...
| `CorpusEntryProjectionAndEcologyFit` | Whether README scenarios, ToC query cues, Preface cues, [[00_E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline]] entry-distribution loci, [[00_I.02 - Expanded Entry Disambiguation Cases]] expanded entry-disambiguation cases, cards, summaries, retrieval snippets, durable names, relations, and corpus ecology preserve the scoped quality result without becoming authority faces, stale echoes, or pattern content. Corpus-entry and projection evidence belongs in the [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]] result, [[00_E.19 - Pattern Quality Gates- Review and Refresh Profiles]] run record, README, ToC, [[00_E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline]], [[00_I.02 - Expanded Entry Disambiguation Cases]], retrieval or card publication locus, or other quality evaluation locus unless the pattern of concern's own `EntityOfConcern` and user move are that projection or evaluation work. |
...
If the declared use is `Stable`, landing-input, release-input, external-review-ready, or another corpus-facing use, the evaluation must use evidence for corpus entry and projection coordinates. A host-only body evaluation can still evaluate the pattern body, but it cannot silently turn missing README, ToC, [[00_E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline|E.11]], [[00_I.02 - Expanded Entry Disambiguation Cases|I.2]], card, retrieval, monolith, or projection evidence into a high `CorpusEntryProjectionAndEcologyFit` value.

### [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:4.5 - Status and stop condition
...
An all-`5` result is a local exceptional result under the declared scope and qualification window. It is not a permanent end of development. [[00_E.23 - Quality Improvement Loop Method|E.23]] can reopen improvement when use, source, comparison set, front, affordability, or payoff changes.

### [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:4.6 - Compact result form

An [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] result uses this result-bearing form:

```text
[[E_The FPF Constitution and Authoring Guides/20_21_FPF Pattern-Quality Evaluation CharacteristicSpace/00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] result:
...
Status is not assigned from a two-column table, a prose summary, a checklist count, an [[00_E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]] pass or fail row, a table missing `ShortRationale`, a result missing the required `PrecisionRestorationProfile`, or a result missing the evidence basis needed for the values it claims. Such material can support a subsequent evaluation, but it is not the [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] result. Conversely, an [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] status is a pattern-quality status, not a release crossing: [[00_E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]] or the release or admission process named by value still checks gate-specific carry-through, projection, monolith, packaging, authority, and non-overread conditions.

### [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:4.7 - Finding and proposal rows

```text
[[E_The FPF Constitution and Authoring Guides/20_21_FPF Pattern-Quality Evaluation CharacteristicSpace/00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] finding:
  Pattern of concern: <PatternOfConcernRef>
...
## [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:5 - Worked slices
...
**QualityEvidenceLeakage in the pattern.** The pattern says that corpus projection, README, ToC, [[00_E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline|E.11]], or [[00_I.02 - Expanded Entry Disambiguation Cases|I.2]] alignment, retrieval or cold-reader evidence, monolith parity, external-review readiness, landing evidence, `PatternQualityStatus`, all-`4` or all-`5` result framing, or another quality-result locus is what the user should do with the pattern's `EntityOfConcern`, or records developer, reviewer, or executor correspondence as if it were pattern content. The defect is not limited to `Problem frame`, `Solution`, examples, or checklist; notes, appendices, `Relations`, `Rationale`, `SoTA-Echoing`, tables, and conformance rows are also parts of the pattern in hosts and the monolith. That evidence may be required for [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]], [[00_E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]], landing, or retrieval loci, but it is not automatically a user action in the pattern of concern. Lower `EntityOfConcernPrimacyAndSemioBiasResistance`, `ActionPathGuidance`, `UseAffordabilityAndApparatusProportionality`, and `CorpusEntryProjectionAndEcologyFit` when this evidence enters the pattern. Repair by moving the evidence to the [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] result, [[00_E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]] run record, README, ToC, [[00_E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline|E.11]], [[00_I.02 - Expanded Entry Disambiguation Cases|I.2]], card, retrieval, projection, or release or landing evidence locus, and keeping in the pattern only the user-facing move or boundary that follows from that evidence.
...
## [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:6 - Conformance checklist

| Check | Requirement |
|---|---|
| `CC-E21-1` | Recover `ClaimScope` from the governing request, [[00_E.22 - Improvement-Oriented Quality Evaluation Question Framing]] frame, campaign seam, landing check, release check, or review assignment; then name `PatternOfConcernRef`, `ClaimScope`, `WorkingReaderScope`, `IntendedUse`, `QualificationWindow`, and `EvaluationEvidenceBasis`. |
| `CC-E21-2` | Evaluate the full `RequiredPatternQualityCoordinates` set. |
| `CC-E21-2a` | Before assigning coordinate values, record one `PrecisionRestorationProfile` with word, head, and use; phrase-apparatus; repetition-and-distribution; ontic-slot; description-publication-source; and pattern-application layers. A missing, grouped, or memory-only profile makes the [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]] result incomplete. |
| `CC-E21-3` | Use the result-bearing three-column table: coordinate, value, and `ShortRationale`; a two-column coordinate-and-value table is not an [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]] result. |
...
| `CC-E21-10` | Keep measurement, score, scale, formal, causal, mathematical, QL, simulation, representation, or learned-lens claims under [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)]], [[00_A.17 - Canonical “Characteristic” (A.CHR-NORM)]], [[00_A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC-KERNEL)]], [[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)]], or the pattern that governs the claim when the evaluated pattern makes those claims. |
...
## [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:7 - Common anti-patterns and repairs
...
| **Quality-evidence leakage.** Corpus projection, retrieval evidence, README, ToC, [[00_E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline]], or [[00_I.02 - Expanded Entry Disambiguation Cases]] alignment, monolith parity, `PatternQualityStatus`, developer, reviewer, or executor correspondence, or other quality evidence is written anywhere in the pattern as method, problem, note, appendix, relation, rationale, or status content about the pattern. | Move the evidence to the [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]] result, [[00_E.19 - Pattern Quality Gates- Review and Refresh Profiles]] run record, README, ToC, [[00_E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline]], [[00_I.02 - Expanded Entry Disambiguation Cases]], card, retrieval, projection, or release or landing evidence locus; keep only the user move or boundary that the evidence justifies. |
| **Apparatus overwrap.** A simple FPF claim is wrapped in extra role, publication-form, locus, flow, state, status, text-state, package, or process words, such as `current pattern text`, `current object`, `active record`, `field used in the current pass`, or route-like pattern talk where no real state or use-position is named, so the reader sees a bureaucratic apparatus instead of the object, relation, action, or boundary. | Apply [[00_F.19 - Ontology-First Plain Technical Rewriting]]; record the scalar effect in `PrecisionRestorationProfile`, then lower the affected coordinates or name the completed repair. |
...
| **Quality veto theatre.** "Not ready" has no [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]] coordinate named by value, evidence, status effect, and repair. | Rewrite as an [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]] finding or remove the veto. |

## [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:8 - Consequences
...
| Stop decisions become less taste-based. | Open-ended improvement remains possible through [[00_E.23 - Quality Improvement Loop Method]] when a stronger aim is requested. |

## [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:9 - Rationale

[[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] keeps the measuring device simple: one object kind, one ordinal scale, one required coordinate set, one status set, and one stop condition. The evaluation never asks whether a coordinate is active. It asks what value the current pattern and its named evidence basis earn under the declared use.
...
## [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:10 - SoTA-Echoing

| Claim | Source-use disposition | Concrete [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]] effect |
|---|---|---|
...
| Proxy optimization can make intended value worse. | Adopt from Goodhart and Campbell, management-accounting surrogation, reward-hacking, and specification-gaming lines. | `ProxyForValueSubstitutionResistance`, `PrecisionRestorationProfile`, [[00_E.13 - Pragmatic Utility and Value Alignment]], and stop condition ask what got worse; `5`, all-`5`, discharge count, and proof apparatus cannot replace pattern content or pragmatic value. |
| Evaluation results are not governance, safety, or compliance proof. | Adopt as non-overread boundary from current evaluation-governance practice. | Neighbour authority and status boundaries keep project claims outside [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]]. |

## [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:11 - Relations

| Neighbour | Relation |
|---|---|
| [[01_A.19.ECS - Evaluation CharacteristicSpace Construction]] | Constructs or repairs the general evaluation `CharacteristicSpace`; [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]] is one specialization. |
| [[01_E.08.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form]] | Publishes an evaluation `CharacteristicSpace` as an FPF pattern when that form is selected. |
| [[00_E.08 - FPF Authoring Conventions & Style Guide]] | Authors the pattern body whose quality [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]] evaluates. |
| [[00_E.19 - Pattern Quality Gates- Review and Refresh Profiles]] | Runs admission and refresh review profiles; it can consume or request [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]], but it does not assign [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]] coordinate values or replace the required pattern-quality table. |
| [[00_E.22 - Improvement-Oriented Quality Evaluation Question Framing]] | Frames purpose, floor, trade-offs, and proposal expectation before an evaluation. |
| [[00_E.23 - Quality Improvement Loop Method]] | Runs repeated improvement using [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]] values and stop meanings for pattern versions. |
| [[00_E.13 - Pragmatic Utility and Value Alignment]] | Governs pragmatic utility and proxy-to-value alignment when quality values, visible measures, review results, all-`5` result framing, or release cues are used as practical value, target, incentive, gate, or improvement proof. |

| [[01_E.09.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace]] | Evaluates upstream `DRR` decision adequacy when pattern-quality defects trace to decisions. |
| [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)]], [[00_A.17 - Canonical “Characteristic” (A.CHR-NORM)]], [[00_A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC-KERNEL)]], [[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)]] | Govern scale, coordinate, and measurement legality. |
| [[00_F.18 - Local-First Unification Naming Protocol]], [[00_E.10 - Unified Lexical Rules for FPF]], [[09_A.06.P - Relational Precision Restoration (RPR) - Kind-Explicit Qualified Relation Discipline]], [[00_C.02.P - Epistemic Precision Restoration]], [[01_C.16.P - Characteristic and Scale Precision Restoration]], [[02_C.16.Q - Quality-Term Precision Restoration]] | Govern naming and wording-use precision when quality defects are lexical or ontological. |
| [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]], [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]], [[00_A.20 - U.Flow.ConstraintValidity - Eulerian]], [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]], [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)]] | Govern project evidence, assurance, local CV state, gates, and work authority. |
| [[00_E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline]] and [[00_I.02 - Expanded Entry Disambiguation Cases]] | Govern entry-distribution and expanded entry-disambiguation cues; [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]] supplies only the scoped quality result. |

## [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]:End
