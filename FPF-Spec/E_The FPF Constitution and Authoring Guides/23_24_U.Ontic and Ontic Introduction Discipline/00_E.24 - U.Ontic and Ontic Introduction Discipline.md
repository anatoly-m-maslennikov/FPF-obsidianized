...
builds_on:
  ...
  - "[[01_E.24.CD - Ontic Candidate Detection|E.24.CD]]"
...
coordinates_with:
  ...
  - "[[01_C.02.P.DR - Declarative Representation Precision Restoration|C.2.P.DR]]"
...
## [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:0 - Use This When
...
- If the issue is only one wording-use repair row, use [[00_E.10 - Unified Lexical Rules for FPF|E.10]] and [[01_E.10.ARCH - Wording-Use Ontological Precision Restoration Architecture|E.10.ARCH]].
- If the issue is only a new or revised mechanism meaning, use [[00_E.20 - Mechanism Introduction Protocol|E.20]].
- If the issue is only durable naming, use [[00_F.18 - Local-First Unification Naming Protocol|F.18]].
- If the issue is only a pattern publication-form or section-order matter, use [[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]].

## [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:1 - Problem Frame

Some FPF governed objects are small enough to define with one relation or one record. Others require a durable ontic. `U.Episteme` is the central example: it needs identity criteria, typed slots, slot-filling discipline, filled assignments, card and publication species, description boundary, publication-form boundary, relation to `U.Signature`, and dependent episteme-morphism and publication patterns. [[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]] works because it makes the small ontic slot relation explicit.
...
[[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] governs that ontic-introduction decision.

## [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:2 - Problem
...
## [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:3 - Forces
...
- If the name has independent `EntityOfConcern` identity, stable identity criteria, a governing pattern, admissible use boundaries, and dependent-pattern reliance, it may remain or become a `U.*` kind after an [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] decision.
...
3. **Filled value assignment or ordinary-use core.** Give a compact filled instance or first-use frame only when users need one concrete application shape. It fills the type-level slots; it is not a second ontology and not a competing slot relation. Under [[00_C.29 - Mathematical Lens Use|C.29]], that filled assignment may be viewed as a tuple when tuple reasoning is current.
4. **Description episteme and publication.** Claims about the ontic, its slots, its slot fillers, or relations among those values use [[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]]; a pattern section, table, diagram, publication, card, or view may describe the ontic, but it is not the ontic.
...
Not every ontic needs every named layer. Add a named signature or kind level only when dependent patterns must rely on slot constraints across species or morphisms. Add a named filled-value assignment only when patterns need a publication-form-agnostic filled value; use [[00_C.29 - Mathematical Lens Use|C.29]] tuple-view language only when tuple reasoning is current. Add named card, view, or publication species only when holonic working forms, views, or publication forms are themselves governed by the ontic. Name attached or non-slot components only when common adjacent structures must stay recoverable while remaining outside the ontic identity.

Keep annotation proportional. [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] requires source-ontology recovery only for wording that can change the ontic, slot, filled value, governing pattern, admissible use, or dependent-pattern obligation. If a domain sentence already preserves those values, do not replace it with a typed paraphrase merely to show that an ontic exists.
...
[[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] is compatible with modular ontology and ontology-design-pattern practice: modular ontology libraries and ontology design patterns show why reusable small ontology structures matter, and recent process-modeling work shows that implicit process patterns must be made explicit for reuse. [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] is narrower and more FPF-specific: it selects when FPF should introduce a durable action-facing ontic, rather than importing an external microtheory or treating every reusable repair table as ontology.

If the choice between "write an ontic" and "keep the existing pattern constellation" needs reusable scoring, build a separate evaluation `CharacteristicSpace` through [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]. The evaluated object is then the FPF pattern-set architecture alternative, not the ontic itself: current constellation, local frame, or durable ontic. Candidate architecture characteristics include cross-pattern coupling, subject cohesion, explicit `onticSlotRelation` and `SlotSpec` discipline, invariant recoverability, duplicate-ontology resistance, dependent-pattern thinness, change-impact locality, first-use burden, and FPF ecology fit. [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] uses these as diagnostic pressure for the introduction decision; it does not itself become the full architecture-characteristic evaluation pattern.

Do not grow [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] by adding full publication-section rules, adequacy scales, wording-use restoration rules, or a general architecture-characteristic theory. [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] keeps only the object split and the ontic-introduction decision needed before dependent patterns rely on a durable ontic.

Use the current split this way:

- use [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] for `U.Ontic` identity, type-level `onticSlotRelation`, semantic area, ontological neighborhood, dependent-pattern obligations, and non-use boundary;
- use [[01_E.24.CD - Ontic Candidate Detection|E.24.CD]] when the current problem is detection of an ontic candidate, hidden-form classification, or sufficiency rationale for deciding whether a recurring construct is a durable ontic, a local use frame, direct governing-pattern use, or source wording only;
- use [[02_E.24.PUB - Ontic Description and Publication Discipline|E.24.PUB]] when the current problem is the distinction among the ontic, the ontic-description episteme, the publication of that description, and publication forms such as cards, records, tables, schemas, diagrams, views, or source packets;
- use [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] only when the contested question is how to construct an evaluation `CharacteristicSpace` for comparing pattern-set architecture alternatives, such as current constellation, local frame, or durable ontic.

This split keeps [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] ontic-first. Candidate detection, publication discipline, and contested evaluation remain neighboring governed objects rather than sections that make [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] a general discovery, documentation, or scoring pattern.


Introduce or rely on a durable FPF ontic only after the ontic-introduction decision satisfies four checks.

### [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:4.1 - Check 1: Existing Governing Pattern Check

Name the current claim under decision and ask whether an existing pattern already carries it.

Use direct governing patterns first. If the case is method semantics, use [[01_A.03.01 - U.Method- Context-Defined Way of Doing|A.3.1]]; if it is method description, use [[02_A.03.02 - U.MethodDescription- Description Episteme for a Way of Doing|A.3.2]]; if it is mechanism meaning, use [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] and [[00_E.20 - Mechanism Introduction Protocol|E.20]]; if it is work planning or dated work, use [[02_A.15.02 - U.WorkPlan|A.15.2]] or [[01_A.15.01 - U.Work|A.15.1]]; if it is evidence, gate, source, assurance, decision, release, or publication use, use that governing pattern.
...
### [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:4.2 - Check 2: Stable Identity Test
...
### [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:4.3 - Check 3: Typed Slot Relation Test
...
- names that pass [[00_F.18 - Local-First Unification Naming Protocol|F.18]];
- evaluation pattern for the resulting pattern or DRR, usually [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] for a pattern and [[01_E.09.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]] for the DRR.
...
### [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:4.5 - Local Use Frame Decision
...
## [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:5 - Archetypal Grounding
...
The same test applies to any draft-only locus. If the label has no current governing text, do not cite it as ontology. Either cite current governing patterns, keep the label as investigation context, or open an [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] ontic-introduction decision.

### [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:5.4 - System-Like Head Concepts

`system`, `episteme`, `architecture`, `method`, `mechanism`, `temporal claim`, `dynamics`, and `change` can each appear as a broad head for many dependent FPF patterns. That breadth is not itself enough to create a durable FPF ontic. Apply [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] before treating a broad head as current governing ontology: name the primary `EntityOfConcern`, stable identity, `onticSlotRelation`, selected `semanticArea`, selected `ontologicalNeighborhood`, dependent patterns, description boundary, and publication boundary. If those rows are missing, use the current governing patterns that already carry the claim and do not cite the broad head as if it supplied current slot discipline.

### [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:5.5 - Mature Comparator Discharge

[[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] is mature only when its selected mature-pattern ingredients are present in the body, not only in a separate planning or evaluation note.

| Comparator | Selected mature ingredient | Current [[00_E.24 - U.Ontic and Ontic Introduction Discipline]] locus | Lowering condition |
| --- | --- | --- | --- |
| [[01_C.02.01 - U.Episteme - Epistemes and their slot relation]] | stable identity plus small typed slot relation for a durable ontic | `E.24:4.2`, `E.24:4.3`, `E.24:5.1` | Lower if [[00_E.24 - U.Ontic and Ontic Introduction Discipline]] asks for fields but no longer asks what preserves or changes identity. |
| [[00_E.20 - Mechanism Introduction Protocol]] | introduction discipline for one governed subject family | `E.24:4.1`, `E.24:4.4`, `E.24:8` | Lower if mechanism-specific doctrine is copied here instead of left with [[00_E.20 - Mechanism Introduction Protocol]], [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind]], and related patterns. |
| [[00_E.08 - FPF Authoring Conventions & Style Guide]] | publication-form and section-order boundary | `E.24:0`, `E.24:4.4`, `E.24:6`, `E.24:8` | Lower if [[00_E.24 - U.Ontic and Ontic Introduction Discipline]] starts regulating pattern format instead of the ontic-introduction decision. |
...
| [[00_F.18 - Local-First Unification Naming Protocol]] | durable naming after ontology is settled | `E.24:4.4`, `E.24:6`, `E.24:7` | Lower if a new name substitutes for identity, slot, and dependent-pattern settlement. |

### [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:5.6 - Bias-Annotation
...
### [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:5.7 - Rationale
...
### [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:5.8 - SoTA-Echoing and Currentness

[[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] does not claim to replace ontology engineering, OWL-style formal ontology, or UFO-style foundational ontology. Its governing reason is the current FPF need for action-facing ontology compactness, plus a narrow SoTA echo:

| Source family | Current lesson for [[00_E.24 - U.Ontic and Ontic Introduction Discipline]] | FPF decision |
| --- | --- | --- |
...
| `CC-E24-8` | The selected name passes [[00_F.18 - Local-First Unification Naming Protocol]]; the name does not hide a second ontology or one umbrella for several kinds. |
| `CC-E24-8a` | Durable `U.*` names, reusable SlotKind heads, species or record-form names, public ids, Core-facing heads, and cross-context labels use [[00_F.18 - Local-First Unification Naming Protocol]]; `F.17 UTS` and Name Card material is opened only when that name becomes public, Core-facing, or cross-context, and never replaces [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)]] SlotSpec discipline. |
| `CC-E24-9` | Pattern-quality and DRR-adequacy checks stay in [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]] and [[01_E.09.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace]]; they are not copied as user-facing ontic or subject-matter content. |
...
| `CC-E24-14` | Candidate detection, publication-form discipline, and contested evaluation are sent to [[01_E.24.CD - Ontic Candidate Detection]], [[02_E.24.PUB - Ontic Description and Publication Discipline]], and [[01_A.19.ECS - Evaluation CharacteristicSpace Construction]] respectively; [[00_E.24 - U.Ontic and Ontic Introduction Discipline]] itself stays centered on `U.Ontic` identity, slot relation, semantic area, ontological neighborhood, and dependent-pattern obligations. |


## [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:7 - Common Anti-Patterns

| Anti-pattern | Symptom | Repair |
| --- | --- | --- |
| Shadow-kind by repetition | The same local record appears in several patterns and starts being cited as an object. | Apply [[00_E.24 - U.Ontic and Ontic Introduction Discipline]]; either write a durable ontic pattern or rename it as a local use frame. |
...
## [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:8 - Relations

- **Builds on:** [[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]], [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]], [[01_E.09.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]], [[00_E.10 - Unified Lexical Rules for FPF|E.10]], [[01_E.10.ARCH - Wording-Use Ontological Precision Restoration Architecture|E.10.ARCH]], [[00_E.20 - Mechanism Introduction Protocol|E.20]], [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]], [[00_F.18 - Local-First Unification Naming Protocol|F.18]], [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]], [[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]], [[01_E.24.CD - Ontic Candidate Detection|E.24.CD]], and [[02_E.24.PUB - Ontic Description and Publication Discipline|E.24.PUB]].
- **Coordinates with:** governing patterns that describe durable ontics or their filled values, especially [[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]] for epistemes, [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] and [[00_E.20 - Mechanism Introduction Protocol|E.20]] for mechanisms, [[01_A.03.01 - U.Method- Context-Defined Way of Doing|A.3.1]] and [[02_A.03.02 - U.MethodDescription- Description Episteme for a Way of Doing|A.3.2]] for method and method description, [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] for role-method-work alignment, and precision-restoration patterns such as [[00_C.02.P - Epistemic Precision Restoration|C.2.P]], [[01_C.02.P.DR - Declarative Representation Precision Restoration|C.2.P.DR]], and [[03_C.30.STRAT - Stratification Wording Precision Restoration|C.30.STRAT]].
...
## [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:9 - Consequences
...
## [[00_E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:End
