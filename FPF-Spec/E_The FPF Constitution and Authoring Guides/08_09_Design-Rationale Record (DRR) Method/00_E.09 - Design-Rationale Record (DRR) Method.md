...
## [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]:0 - Use this when
...
## [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]:0.2 - What this buys
...
**Cheap stop.** If the change is ordinary local wording repair, application of an already accepted pattern, or editorial cleanup that does not change FPF semantics, obligations, boundaries, names, admissible uses, or normative force, do not open a full DRR. Use the lighter governing pattern for the local repair: [[01_E.17.AUD.LHR - PublicationUnit Stability Discipline and Local Head Restoration - repair the overloaded local lexical head before the publication unit inherits it|E.17.AUD.LHR]] for one overloaded local lexical head inside one publication unit, [[00_C.02.P - Epistemic Precision Restoration|C.2.P]] for one episteme, publication, or source-use phrase requiring local epistemic precision restoration, [[00_E.10 - Unified Lexical Rules for FPF|E.10]] for general lexical repair, [[00_F.18 - Local-First Unification Naming Protocol|F.18]] only when a durable reusable name is being minted, and [[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]] for authoring-form correction. Leave [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]] for bounded content decisions that need rationale by value.

**Kind-or-boilerplate diagnostic.** When a DRR proposes wording for selected patterns, apply [[00_F.19 - Ontology-First Plain Technical Rewriting|F.19]] to separate boilerplate apparatus from remaining content before any wording is treated as pasteable pattern prose. If the remaining content still hides wording-use, naming, relation, claim, admissible-use, selected-locus, user-move, or flow-role precision, the DRR names the applied [[00_E.10 - Unified Lexical Rules for FPF|E.10]], [[01_E.10.ARCH - Wording-Use Ontological Precision Restoration Architecture|E.10.ARCH]], [[00_F.18 - Local-First Unification Naming Protocol|F.18]], or governing pattern. Process, architecture, review, or reference apparatus belongs in its own carrier, not in pasteable pattern prose.
...
## [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]:1 - Problem frame
...
## [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]:2 - Problem
...
## [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]:3 - Forces
...
## [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]:4 - Solution — the DRR as a structured argument and temporary convergence record
...
### [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]:4.1 - Minimum decision-inspection content blocks
...
| **Naming, ontology, and wrong-carrier-confusion account** | Head/branch/object/move/outside-work separation, tempting wrong-pattern assignment or wrong non-pattern FPF kind-reference assignment, and any load-bearing [[00_F.18 - Local-First Unification Naming Protocol]] naming obligation needed to keep the selected answer truthful by value. | Problem frame, Decision, and Rationale. |
...
per the Pattern Template, [[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]]). The full DRR remains external as
provenance.
...
### [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]:4.1a - Process-source method admission into FPF
...
## [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]:5 - Archetypal Grounding (System / Episteme)
...
## [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]:6 - Bias-Annotation
...
Scope: this bias annotation is universal for FPF semantic changes governed by [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]. It does not turn project-management state, helper state, or review logistics into DRR content.

## [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]:7 - Conformance Checklist
...
## [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]:8 - Common Anti-Patterns and How to Avoid Them
...
## [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]:10 - Rationale
...
[[00_E.09 - Design-Rationale Record (DRR) Method|E.9]] sets a **floor, not a ceiling**: every conforming DRR must make
...
## [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]:11 - SoTA-Echoing
...
## [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]:12 - Relations

* **Instantiates:** P‑10 Open‑Ended Evolution, P‑2 Didactic Primacy
* **Template governed by:** `pat:authoring/pattern‑template` ([[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]])
* **Interacts with:** `pat:guard/bias‑audit` ([[04_E.05.04 - Cross-Disciplinary Bias Audit|E.5.4]]) via lens check
* **Complemented by:** [[01_E.09.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]] when one concrete `DRR` follows [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]] form but its adequacy for downstream drafting, host amendment, accepted-decision carry-through, source-use carry-through, or selected-locus distribution is disputed or materially relevant. [[01_E.09.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]] reads the `DRR` decision-adequacy claim; it is not a second DRR form, review gate, or mandatory ordinary editorial step. Also complemented by `pat:authoring/code-of-conduct` ([[00_E.12 - Didactic Primacy & Cognitive Ergonomics|E.12]]) for etiquette in DRR debate.

* **Coordinates with:** [[00_E.23 - Quality Improvement Loop Method|E.23]] when one `DRR` is being improved through repeated quality-improvement passes. [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]] keeps the `DRR` kind and decision-record form; [[01_E.09.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]] supplies the decision-adequacy object-under-improvement evaluation when adequacy is being improved; [[00_E.23 - Quality Improvement Loop Method|E.23]] governs the repeated method rather than turning the DRR into final pattern prose.

## [[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]:End
