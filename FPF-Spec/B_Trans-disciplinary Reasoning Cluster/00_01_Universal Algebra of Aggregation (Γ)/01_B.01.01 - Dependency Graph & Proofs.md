...
## [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:1 - Problem frame
...
`D` is the *only* admissible input shape for Γ. It must capture **part–whole** structure faithfully ([[00_A.01 - U.Holon, U.System, and U.Episteme|A.1]], [[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]) while staying neutral to order (handled by Γ\_ctx and Γ\_method), time (Γ\_time), and accounting (Γ\_work). If `D` is sloppy—mixing kinds of relations or scopes—Γ becomes unpredictable and the Quintet invariants (IDEM, COMM, LOC, WLNK, MONO) fail in subtle ways.
...
## [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:2 - Problem
...
## [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:3 - Forces
...
| **Parsimony vs. Expressiveness**   | Keep the vocabulary small ([[00_A.11 - Ontological Parsimony (C-5)]]) ↔ include the minimal extra relations that engineers actually use ([[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases]]).                          |
...
## [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:4 - Solution

### [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:4.1 - The shape: a typed, scoped, acyclic graph
...
* **V (nodes):** each `v ∈ V` is a `U.Holon` with:
  ...
  * `DesignRunTag ∈ {design, run}` ([[00_A.04 - Temporal Duality & Open-Ended Evolution Principle|A.4]]) — **single, uniform per D**
  * a declared `U.Boundary` ([[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]])
  * optional characteristics (e.g., F–G–R, CL, Agency metrics) for use by downstream patterns ([[00_B.01 - Universal Algebra of Aggregation (Γ)|B.1]].2/3; [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]; [[00_A.13 - The Agential Role & Agency Spectrum|A.13]])
...
* **acyclicity:** `D` **MUST** be a DAG. Any cycle requires refactoring or elevation to a Meta‑Holon ([[00_B.02 - Meta-Holon Transition (MHT)- Recognizing Emergence and Re-identifying Wholes|B.2]]).

> **Strict distinction ([[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]).**
...
### [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:4.2 - Normative edge vocabulary `V_rel` ([[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] compliant)

Only the following **four** **mereological** relations are allowed in `E` ([[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]):
...
* `SerialStepOf`, `ParallelFactorOf` — **order/concurrency** edges of Γ_method/Γ_ctx; **not** parthood; keep them out of `E` (see § 4.1 [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] and Part [[05_B.01.05 - Γmethod - Order-Sensitive Method Composition & Work Enactment|B.1.5]]).
* `MemberOf` — **non‑mereological** collective membership; model in Γ_collective (B.1.7), **not** in `E` (**see § 9**).
 * `RepresentationOf`, `MapsTo`, `Implements` — these are **mappings**, not parthood; model them at the value level ([[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]) or as `U.Interaction` between holons.
* `RoleBearerOf` — links a `U.System` to a `U.Role`; **not** parthood (see [[00_A.12 - External Transformer & Reflexive Split|A.12]], [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]).
* Any “is‑a” (`subClassOf`) taxonomic relation — orthogonal to parthood.

### [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:4.3 - Minimal axioms & type guards per relation

| Relation             | Axioms (informal)                                                 | Guards / When to use                                                                                               |
| -------------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| **ComponentOf**      | anti‑symmetric; transitive; acyclic                               | Physical assemblies; interfaces compose via BIC ([[02_B.01.02 - System-specific Aggregation Γ-sys]]). Do **not** use for collections or pipelines.              |
...
> **Carrier identity for `PhaseOf`.** The same carrier identity across phases must be explicit (e.g., *this* frame across heat/dwell/quench; *this* theory across revisions). If identity changes, you are modelling a **Transformer** creating a **new** holon ([[00_A.12 - External Transformer & Reflexive Split|A.12]]) — not a phase.

### [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:4.4 - Selection guide (didactic, normative in spirit)

Use this **one‑page decision** to pick the edge correctly:

1. **Is it a part–whole relation at all?**
   If it is mapping, influence, or reference → **not** parthood. Use `U.Interaction` or value‑level links ([[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]).
...
4. **Is it order‑sensitive execution?**
   If step order changes semantics -> **apply [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] (ordered relations)** and aggregate with **Γ_ctx / Γ_method**.
...
> • Treating representation (file ↔ thing) as parthood → keep as value‑level mapping ([[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]), not in `D`.

### [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:4.5 - **Γ_m (Compose‑CAL)** — structural aggregators & trace shape

**Purpose.** Provide a **minimal constructional generator** for **structural mereology** that keeps the kernel small (C-5), aligns with **[[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]** (Portions/Phases/Components discipline), and feeds Working-Model layer publication in LOG without importing tooling or notations.
...
Γ_m.set(elems : Multiset[U.Entity])  → C : U.Holon
  ...
  // outward **MemberOf** remains a non‑mereological signal per [[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] (does not build holarchies)
...
Notes capture boundary tags ([[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]), scope (`design|run`), and any independence declarations used by the Quintet proofs (below).

**Invariant footprint on Γ_m traces (inherits [[00_B.01 - Universal Algebra of Aggregation (Γ)|B.1]] Quintet).**
...
* **WLNK** — aggregate cannot exceed the weakest limiting attribute among parts; synergy escalates via **[[00_B.02 - Meta-Holon Transition (MHT)- Recognizing Emergence and Re-identifying Wholes|B.2]] Meta‑Holon Transition**.
...
### [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:4.6 - Scope and boundary rules (make graphs foldable)
...
3. **Acyclicity:** if a cycle is detected, either (a) refactor (e.g., split a collective from an assembly), or (b) escalate to **Meta‑Holon Transition** ([[00_B.02 - Meta-Holon Transition (MHT)- Recognizing Emergence and Re-identifying Wholes|B.2]]) if a new “whole” with novel properties is intended.
...
5. **Resource routing:** do **not** encode costs with structural edges; route to Γ\_work ([[06_B.01.06 - Γ-work - Work as Spent Resource|B.1.6]]) across declared boundaries.

### [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:4.7 - What “Proofs” mean here (preview of Part 2)
...
## [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:5 - Didactic mini‑examples
...
## [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:6 - The Proof Kit (ready‑made templates for Γ on D)
...
### [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:6.1 - Independence declaration (for COMM/LOC)
...
### [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:6.2 - Weakest‑link cutset (WLNK)
...
### [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:6.3 - Monotone coordinates (MONO)
...
### [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:6.4 - Idempotence witness (IDEM)
...
### [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:6.5 - Scope & boundary attestations
...
### [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:6.6 - Flavour‑specific summary table
...
## [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:7 - Archetypal grounding (worked micro‑examples)

> Each row is self‑contained and can be used as a template.

### [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:7.1 - U.System (assembly & production)
...
### [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:7.2 - U.Episteme (paper & dataset)
...
## [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:8 - Conformance Checklist (normative checklist)
...
| **CC‑B1.1.10** | Order/time/resource **SHALL NOT** be encoded via structural edges; they must be routed to Γ\_ctx/Γ\_method, Γ\_time, Γ\_work respectively.                   | Maintain [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)]] Strict Distinction.   |
| **CC‑B1.1.11** | If a cycle or a locality violation persists, the modeller **SHALL** either refactor or declare a **Meta‑Holon Transition ([[00_B.02 - Meta-Holon Transition (MHT)- Recognizing Emergence and Re-identifying Wholes]])**.                          | Make emergence explicit.            |
...
## [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:9 - Anti‑pattern diagnostics (before → after)
...
| **History encoded as structure** | `v2 ComponentOf v1`                                            | Use `PhaseOf` for time slices of the *same* carrier, or a Transformer creating a new holon ([[00_A.12 - External Transformer & Reflexive Split]]) if identity changes.                                  |
...
## [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:10 - Consequences
...
## [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:11 - Rationale (informative)

This pattern operationalizes **[[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] (Mereology Extension)** and **[[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] (Strict Distinction)** for the universal algebra of [[00_B.01 - Universal Algebra of Aggregation (Γ)|B.1]]. +… By limiting `E` to **four** well‑formed **mereological** relations, we prevent the three recurrent category errors: **mapping≠parthood**, **order/time≠structure**, **collection≠stock**. The Proof Kit converts the Quintet from abstract slogans into concrete obligations that engineers can check in everyday models. Γ‑flavours then remain simple and domain‑appropriate, while proofs remain small and reusable.

## [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:12 - Relations

* **Builds on:** [[00_A.01 - U.Holon, U.System, and U.Episteme|A.1]] **Holonic Foundation**; [[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] **Mereology Extension**; [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] **Strict Distinction**; [[00_A.12 - External Transformer & Reflexive Split|A.12]] **Transformer Principle**.
* **Constrained by:** [[00_B.01 - Universal Algebra of Aggregation (Γ)|B.1]] **Universal Γ** and the **Invariant Quintet**.
* **Used by:** [[02_B.01.02 - System-specific Aggregation Γ-sys|B.1.2]] **Γ\_sys**, [[03_B.01.03 - Γepist - Knowledge-Specific Aggregation|B.1.3]] **Γ\_epist**, [[04_B.01.04 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]] **Γ\_ctx/Γ\_time**, [[05_B.01.05 - Γmethod - Order-Sensitive Method Composition & Work Enactment|B.1.5]] **Γ\_method**, [[06_B.01.06 - Γ-work - Work as Spent Resource|B.1.6]] **Γ\_work**.
* **Triggers:** [[00_B.02 - Meta-Holon Transition (MHT)- Recognizing Emergence and Re-identifying Wholes|B.2]] **Meta‑Holon Transition (MHT): Recognizing Emergence and Re‑identifying Wholes** when cycles or WLNK violations indicate a new emergent whole.
* **Feeds:** [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] **Trust & Assurance Calculus (F–G–R with Congruence)** via explicit declaration of monotone characteristics and provenance.
...
## [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]]:End
