...
## [[01_B.03.03 - Assurance Subtypes & Levels|B.3.3]]:1 - **Problem Frame**
...
## [[01_B.03.03 - Assurance Subtypes & Levels|B.3.3]]:2 - **Problem**
...
## [[01_B.03.03 - Assurance Subtypes & Levels|B.3.3]]:3 - **Solution**
...
### [[01_B.03.03 - Assurance Subtypes & Levels|B.3.3]]:3.1 - Assurance Subtypes: The Three Pillars of Trust
...
### [[01_B.03.03 - Assurance Subtypes & Levels|B.3.3]]:3.2 - Computed Assurance Levels: Evidence-support progression
...
*   **CC-B3.3.2 (L1 Typing Mandate):** A ToA at `AssuranceLevel:L1` or higher **MUST** be supported by **Typing Assurance (TA)**. This includes, at a minimum, that its core concepts are mapped via the Role-Projection bridge (Pattern [[00_B.05 - Canonical Reasoning Cycle|B.5]]) and it conforms to its declared schema.
...
*   **CC-B3.3.4 (Concept-Bridge Completeness):** For any mechanism used in a model at `AssuranceLevel:L1` or higher, all of its mandatory U.Types **MUST** be mapped to domain concepts via the Role-Projection bridge (Pattern [[00_B.05 - Canonical Reasoning Cycle|B.5]]).
*   **CC-B3.3.5 (Scope Separation):** Assurance claims **MUST** maintain a strict separation between `design-time` and `run-time` scopes (Pattern [[00_A.04 - Temporal Duality & Open-Ended Evolution Principle|A.4]]). An assurance tuple for a `MethodDescription` (design-time) SHALL NOT be conflated with one for its corresponding `Work`/`Trace` (run-time). The Evidence Graph Ref (`verifiedBy`, `validatedBy`) must point to evidence carriers or records with the appropriate scope.
* **CC-B3.3.6 (CT2R‑LOG Handshake):** If a ToA depends on **structural** claims, those claims **SHALL** be published as **Working‑Model** relations and, when used to justify `L2`, **SHALL** declare `validationMode=axiomatic` and provide **Constructive** grounding with `tv:groundedBy → Γₘ.(sum|set|slice)` (see [[03_B.03.05 - Working-Model Relations & Grounding (CT2R-LOG)|B.3.5]] and [[00_C.13 - Constructional Mereology (Compose-CAL)|C.13]]).
* **CC-B3.3.7 (Downward‑Only Dependence):** Assurance publications or records (Mapping, Logical, Constructive, and Evidence) **SHALL NOT** impose vocabulary or layout back onto the Working‑Model surface ([[00_E.14 - Human-Centric Working-Model|E.14]]).

## [[01_B.03.03 - Assurance Subtypes & Levels|B.3.3]]:5 - **Common Anti-Patterns and How to Avoid Them**
...
## [[01_B.03.03 - Assurance Subtypes & Levels|B.3.3]]:6 - **Consequences**
...
## [[01_B.03.03 - Assurance Subtypes & Levels|B.3.3]]:7 - **Rationale**
...
## [[01_B.03.03 - Assurance Subtypes & Levels|B.3.3]]:8 - **Relations**
...
*   **Enables:** Objective quality gates in the Canonical Evolution Loop ([[00_B.04 - Canonical Evolution Loop|B.4]]) and reliable inputs for the Trust-Aware Mediation Calculus (D.4).

## [[01_B.03.03 - Assurance Subtypes & Levels|B.3.3]]:End
