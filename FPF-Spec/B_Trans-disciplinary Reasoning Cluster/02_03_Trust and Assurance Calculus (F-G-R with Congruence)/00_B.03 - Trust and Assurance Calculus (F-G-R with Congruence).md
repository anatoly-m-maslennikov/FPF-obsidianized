...
> [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] defines how assurance or trust is assigned, aggregated, and reused for physical systems, epistemes, and publication or evidence records that are used to claim assurance. It uses one typed assurance tuple, F-G-R: `F` and `R` as characteristics, `G` as the scope value, plus edge-scoped `CL`; the aggregation rules stay conservative, respect the Gamma invariants, and keep the [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] EntityOfConcern and description strict distinction. It treats the [[00_E.14 - Human-Centric Working-Model|E.14]] Working-Model layer as the publication-facing assertion layer for claims, with assurance inputs attached downward from Mapping, Logical, Constructive, and Empirical Validation layers.

**Use this when.** Use [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] when a claim, label, dashboard, evidence bundle, model, report, gate decision, or assurance-input package is being used to raise assurance, trust, readiness, compliance, safety, release confidence, `F`, `G`, `R`, or `CL` for a named claim.
...
**Not this pattern when.** If the encountered source or publication face is only a cue, action invitation, boundary wording, evidence question, currentness question, gate decision, release decision, role assertion, status-value assertion, commitment, or work occurrence, use [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]], [[00_A.06 - Signature Stack & Boundary Discipline|A.6]], [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]], [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], [[00_A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]], [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]], [[08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]], [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]], or [[01_A.15.01 - U.Work|A.15.1]] as appropriate.

**Assurance result selection.** Use the lightest assurance result that can decide the assurance use being claimed. A cue or source pointer gets no [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] tuple. A local, non-release, non-compliance, non-safety, non-reused claim may be written as a compact bounded assurance claim statement that names claim, assurance use carried by the assurance tuple or relying context, evidence pointer, limit, and stop or reopen condition. Reserve a full typed `Assurance(H, C | K, S)` claim for readiness, compliance, safety, release confidence, trust, `F`, `G`, `R`, `CL`, or reused assurance input.
...
## [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:1 - Problem frame
...
To make such claims comparable and auditable across domains, [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] introduces a **Trust and Assurance Calculus** that:
...
* accounts for **integration quality** via **Congruence Level (CL)** along the edges of a `DependencyGraph` ([[01_B.01.01 - Dependency Graph & Proofs|B.1.1]], [[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]),
...
[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] is **conceptual and normative**: it defines *which assurance components must be published and how they propagate*. How those components improve (for example by formalizing, replicating, reconciling, or widening or narrowing scope under declared operation rules) is handled by KD-CAL improvement moves; those knowledge-dynamics references are descriptive, not required to read here.

**Mechanism linkage.** For law-governed operation families (for example **USM** and **UNM**) authored as **mechanisms**, use [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] — U.Mechanism to publish **OperationAlgebra**, **LawSet**, **AdmissibilityConditions**, and the **Transport** clause (Bridge-only; `CL`, `CL^k`, and `CL^plane`). All such penalties reduce `R_eff` only; `F` and `G` remain invariant.

**Working-Model handshake (alignment with [[00_E.14 - Human-Centric Working-Model|E.14]], [[03_B.03.05 - Working-Model Relations & Grounding (CT2R-LOG)|B.3.5]], and [[00_C.13 - Constructional Mereology (Compose-CAL)|C.13]]).**
Assurance consumes two inputs declared in the **Working-Model** assertion layer (CT2R-LOG, [[03_B.03.05 - Working-Model Relations & Grounding (CT2R-LOG)|B.3.5]]): the **justification declaration** `validationMode ∈ {postulate, inferential, axiomatic}` and, where present, the **grounding link** `tv:groundedBy`. Structural claims that aspire to the strongest guarantees rely on **Constructive** grounding as a **Gamma-m** (Compose-CAL) narrative referenced via `tv:groundedBy`. No assurance record or publication **defines** Working-Model wording or layout; dependence remains downward-only under [[00_E.14 - Human-Centric Working-Model|E.14]].

## [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:2 - Problem
...
## [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:3 - Forces

| Force                                    | Tension                                                                                                                             |
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **Conservatism vs. Synthesis**           | Avoid overclaiming (WLNK) ↔ recognize real gains from better integration (raise CL) or true emergence ([[00_B.02 - Meta-Holon Transition (MHT)- Recognizing Emergence and Re-identifying Wholes]]).                            |
...
| **Simplicity vs. Fidelity**              | Keep the assurance tuple small and typed ([[00_A.11 - Ontological Parsimony (C-5)]]) ↔ capture enough structure to be informative and improvable by KD-CAL moves.           |
...
## [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:4 - Solution — **Part 1: The assurance tuple and the universal aggregation skeleton**

[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] defines **what** the assurance components are, **where** they are assigned on nodes and edges of the dependency graph, and the **shape** of the aggregation that any Γ-flavour must honor when producing an *assurance result*.

### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:4.1 - The F-G-R assurance components (typed; `F` and `R` as CHR, `G` as USM)
...
2. **ClaimScope (G)** — *the declared set of `U.ContextSlice` values where the result applies.*
   ...
   * **Type:** **set-valued USM scope value** ([[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]), **not** a CHR characteristic.
...
> **EntityOfConcern and description strict distinction ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]).**
>
...
> **Mapping** raises **TA** (typing, fit, and CL). **Logical** and **Constructive** contribute to **VA** (intended relation semantics; Gamma-m extensional identity for structure). **Empirical Validation** contributes to **LA** (evidence in a bounded context). These assurance inputs attach **downward** from the Working-Model assertion layer ([[00_E.14 - Human-Centric Working-Model|E.14]]).

### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:4.2 - Assurance as a typed claim

[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] speaks about **assurance of a specific typed claim** `C` over a holon `H` under context `K` and scope `S ∈ {design, run}`:
...
* `Notes` include the **SCR** (all sources, [[03_B.01.03 - Γepist - Knowledge-Specific Aggregation|B.1.3]]), **OrderSpec** or **TimeWindow** where applicable ([[04_B.01.04 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]), cutsets, and evidence citations ([[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]).
...
Each published Working-Model assertion declares **`validationMode ∈ {postulate, inferential, axiomatic}`** ([[00_E.14 - Human-Centric Working-Model|E.14]]).
...
* *axiomatic* -> constructive identity; **structural** edges provide a Gamma-m narrative and a **`tv:groundedBy`** pointer ([[00_C.13 - Constructional Mereology (Compose-CAL)|C.13]], [[03_B.03.05 - Working-Model Relations & Grounding (CT2R-LOG)|B.3.5]]).

**Design versus run (no chimeras).** Assurance tuples for **design-time** and **run-time** are reported **separately** and are not composed into a single score; see the *Scope drift* hazard in [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:2 and the obligations in [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:4.6.

### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:4.2a - Authority-looking labels and dashboard tiles
...
[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] dispositions for such a source or publication face are:

| Disposition | Use when | Output |
| --- | --- | --- |
| No assurance use | The encountered source or publication face is only a cue, source pointer, evidence question, currentness question, gate decision, role assertion, status-value assertion, commitment, boundary wording, or work occurrence. | Use [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)]], [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]], [[00_A.06 - Signature Stack & Boundary Discipline]], [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]], [[00_A.20 - U.Flow.ConstraintValidity - Eulerian]], [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment]], [[08_A.02.08 - U.Commitment (Deontic Commitment Object)]], [[09_A.02.09 - U.SpeechAct (Communicative Work Object)]], or [[01_A.15.01 - U.Work]]; no tuple is needed. |
...
Build a [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] assurance claim only when the next work move or reliance move depends on a typed assurance claim. The typed assurance claim names:
...
| Manager or release role | Which desired decision or work or reliance use is outside [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] and must instead use [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)]], [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]], [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]], or another named source? |
...
Contestability and redress relation: when the [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] material-reliance threshold is met, the [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] result should name the claim being contested, evidence path, limitation or decay condition, contest forum or decision forum, safe interim disposition, and what evidence or scope change would reopen the assurance claim.

If those fields are missing, the encountered publication face, rendering, or cue remains an orientation label, source pointer, evidence pointer, documentation record, or unsubstantiated confidence cue. Use [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] when the question is whether that lane may guide work or reliance, [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] when the question is evidence, currentness, or provenance, and [[00_A.06 - Signature Stack & Boundary Discipline|A.6]] when the question is mixed policy, API, or schema wording.
...
| --- | --- | --- |
| Source-backed release dashboard tile | If the tile is a current view of [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]] `GateDecision` or `DecisionLogRef` plus an [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] evidence path, it may carry gate-passage reliance outside [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] for the named release and environment. [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] is used only when the tile is also asked to raise readiness, safety, compliance, trust, or release-confidence assurance. | Release approval by display, compliance proof, rollback success, work occurrence, or assurance increase without a typed assurance claim. |
...
Model cards, datasheets, data cards, assurance documents, and assurance-looking notes are external documentation records or source records unless they are mapped into existing `FPF` claims and publication faces. They do not add MVPK face kinds and do not bypass [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] when the use under repair is an assurance claim.

**Lint trigger.** A model card, datasheet, or data card cited as readiness, safety, compliance, release confidence, or assurance proof requires documented intended-use match, evaluation condition, limitations, an [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence path, and one typed `Assurance(H, C \| K, S)` claim for the named assurance claim. Without those, classify the use as `no assurance use` or as a rejected or downgraded assurance claim.

Positive repaired example: a model card plus documented bounded-use statement or external intended-use field, evaluation condition, version, window, limitations, an [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence path, and a typed `Assurance(H, C \| K, S)` claim may carry assurance for that named model claim in that evaluated context. The same documentation still does not carry another deployment context, gate passage, release work occurrence, or compliance proof unless those sources are separately present.

### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:4.2b - Minimum reliance safety assurance record

Use this [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] section when the [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] material-reliance threshold is met: reliance on a visible source may materially change behavior, safety, release, compliance, public or protocol behavior, access, resource allocation, people or team status value, operational action, or controlled-entity regulation. The first [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] move is to decide whether an assurance claim is being made; if it is, write the minimum reliance safety assurance record for the named reliance use. Mere attention shift, learning, orientation, source-finding, or source-wording correction is not enough.

`RelianceSafetyCase` is the local Tech label for this [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] assurance-record form. The plain phrase is **minimum reliance safety assurance record**. The label is not a new FPF pattern, Core kind, safety authority, gate, policy source, approval, certificate, compliance method, or general safety-case ontology.

Assurance-record role: the trigger and non-trigger table is a [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] recognition aid, the minimum assurance-record table is a minimum local record aid, and the worked reliance-threshold slices are examples for users of the pattern. They are not a universal project checklist, sign-off sequence, untyped status vocabulary, or replacement for `Assurance(H, C | K, S)`; use them only when the named material reliance trigger is met. This local section keeps the attempted reliance inside the [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] assurance relation; it does not create an extra SEMIO authority or cross-pattern relation vocabulary.

Affordability card: orientation or source-finding stays outside [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]; bounded local reliance stays with the local evidence, explanation, CV, gate, or pattern-quality relation unless an assurance claim is being made; threshold reliance uses the minimum reliance safety assurance record only when the [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] material-reliance threshold is met. Plain wording remains ordinary unless it changes bounded use, source relation, evidence, gate, assurance, work, decision, or selected governing pattern.

Common wrong first classification: a safety-looking note, safety case, compliance-looking label, or dashboard warning is a certificate, approval, or gate. First honest entry: state one typed [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] assurance claim with [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence path, assumptions, limitations, defeaters, residual uncertainty, monitoring or stop condition, contest and redress relation, bounded assurance use, and unsupported attempted use.

First [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] move: name the reliance use, the assurance claim, the affected context or audience, the trigger that meets the [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] material-reliance threshold, the [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence path, the argument, limitations, defeaters, contest and redress relation, stop or monitoring condition, bounded assurance use, and unsupported attempted use. If those pieces are absent, use [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]], [[04_E.17.EFP - ExplanationFaithfulnessProfile - explanation-use discipline over existing MVPK faces|E.17.EFP]], [[00_A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]], [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], [[00_E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]], or the local relation that actually governs the source use rather than inventing assurance by label.

Trigger and non-trigger cases:

| Encountered source use | [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] disposition | Minimum response |
| --- | --- | --- |
| Ordinary source-backed report, citation, model card, datasheet, data card, or documentation record with no assurance use and no met [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] material-reliance threshold | No [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] assurance use. | Stay in [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] with claim, source record or publication face, evidence path, window, bounded evidence use, unsupported attempted use, and reopen trigger. |
| Generated explanation, generated summary, or didactic reconstruction used only for source-finding or learning | No [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] assurance use. | Stay in [[04_E.17.EFP - ExplanationFaithfulnessProfile - explanation-use discipline over existing MVPK faces]] unless operative claims are relied on through [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] evidence paths or another source relation that evidences the operative claim. |
| Local conformance label, `CV.Status`, benchmark result, or score near a release conversation but not used to raise assurance | No [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] assurance use. | Keep `CV.Status` in [[00_A.20 - U.Flow.ConstraintValidity - Eulerian]], gate-decision publication in [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]], pattern-quality result in [[00_E.19 - Pattern Quality Gates- Review and Refresh Profiles]], measurement or marker relation in [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)]] or [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]], and no assurance tuple unless an assurance claim is being made. |
| Confidence, calibration, prediction interval, or abstention reason tied to one reversible local act | Compact bounded assurance claim only when the act depends on assurance; otherwise no [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] use. | State act, context, window, calibration condition, stop condition, bounded evidence use, and unsupported attempted use; use [[00_C.27 - Temporal Claim Adequacy- State Readings, Temporal Trends, and Intervention-Sensitive Temporal Change]] or [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator]] when time, expiry, refresh, or monitoring changes the move. |
| Safety-looking note, compliance-looking label, public warning, dashboard value, generated operational explanation, or status-value display is intended or reasonably foreseeable to meet the [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] material-reliance threshold: reliance materially changes behavior, safety, release, compliance, public or protocol behavior, access, resource allocation, people or team status value, operational action, or controlled-entity regulation. | Minimum reliance safety assurance record is required. | Build the [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] assurance record with [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] evidence path and any relevant [[00_A.20 - U.Flow.ConstraintValidity - Eulerian]], [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]], [[00_E.19 - Pattern Quality Gates- Review and Refresh Profiles]], [[00_C.27 - Temporal Claim Adequacy- State Readings, Temporal Trends, and Intervention-Sensitive Temporal Change]], [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator]], [[04_B.02.05 - Supervisor-Subholon Feedback Loop]], or representation and retargeting dependency. |
...
| Dependencies | Any relevant [[00_A.20 - U.Flow.ConstraintValidity - Eulerian]] CV status, [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]] gate decision, [[00_E.19 - Pattern Quality Gates- Review and Refresh Profiles]] pattern-quality result, [[00_C.27 - Temporal Claim Adequacy- State Readings, Temporal Trends, and Intervention-Sensitive Temporal Change]] temporal claim, [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator]] refresh and decay relation, [[04_B.02.05 - Supervisor-Subholon Feedback Loop]] control relation, or representation and retargeting relation. |
| Monitoring, rollback, or stop condition | What observation, incident, drift, contest, expiry, changed [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability]] identification or realizability profile, changed [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]] gate profile, changed evaluation condition, changed source record, or failed check stops, narrows, reopens, or withdraws the reliance. |
...
Misuse guard: an incoming or attempted-reliance `RelianceDisposition=safety-case-required` must name the trigger that meets the [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] material-reliance threshold. A source producer, dashboard-value publisher or maintainer, model producer, documentation producer, or status-value label issuer cannot self-clear a threshold-bearing reliance by attaching the label. Where the [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] material-reliance threshold is met, the assurance record must expose an accountable review role and a contest relation capable of changing the disposition.
...
| Slice | [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] move | Boundary |
| --- | --- | --- |
...
| An SRE dashboard changes incident behavior or resource allocation. | Use [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] only when the dashboard is asked to raise assurance or safety-bearing reliance; keep ordinary evidence and currentness in [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]]. | Use [[04_B.02.05 - Supervisor-Subholon Feedback Loop]] only for a control relation being claimed and [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]] only for a gate decision being claimed. |
| A public warning or synthetic-content label changes perceived meaning but there is no evidence that it changed the behavior claimed to change, release risk, safety claim, or control relation. | Keep the label as [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] evidence or source-finding and orientation cue; require audience-effect or behavior-effect evidence before [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] reliance. | Do not infer safety, compliance, behavior change, or control effect from label presence alone. |
| A manufacturing conformance label appears near release. | Keep local CV or conformance evidence in [[00_A.20 - U.Flow.ConstraintValidity - Eulerian]], [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]], [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)]], or [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]]; use [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] only when assurance, safety, compliance, or release-confidence reliance is being claimed. | Conformance presence is not safety acceptance or release permission. |
| A software supply-chain attestation is cited as runtime safety. | Use [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] for origin, build, and process claims and [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] only for the named assurance claim with argument, limitations, defeaters, and stop condition. | Build provenance is not runtime safety or operational permission. |
...
| A standards-document clause is reused as approval. | Use [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] for evidence of the clause; use the named approval, commitment, gate, or assurance relation only when that relation is being claimed by value. | A cited clause is not project approval, gate passage, or assurance by quotation. |

Do not treat the assurance record as a graded scale, standalone status value, universal assurance checklist, release certificate, or new safety-case disposition family. [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] consumes the assurance record only as typed assurance input for the named claim and reliance use.

### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:4.3 - Where the numbers are assigned (and where they are not)
...
* **Not inside Γ:** Γ consumes `D` and produces a composed holon; [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] governs how `F, G, R, CL` **propagate** to the **Assurance** tuple for that composed holon. This keeps Γ algebra and assurance calculus **separable** and reviewable.
...
### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:4.4 - Universal aggregation skeleton (domain‑neutral)
...
2. **ClaimScope (G):**
   ...
   * Across independent evidence lines for the same claim, [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] may publish a `SpanUnion` of the path scopes, but only when the independence assumption and evidence relation are explicit.
...
4. **SCR and Notes:**
   * The aggregate produces an SCR listing all contributing nodes and edges, with their F, G, R, CL, scopes, and evidence links ([[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]).
   * The SCR also displays the **EntityOfConcernRef** (`entityOfConcernRef and groundingHolonRef`) and the **ReferencePlane** for the claim, and presents a separable TA, VA, and LA table of evidence contributions with valid-until or decay marks and the **Epistemic-Debt** per § [[02_B.03.04 - Evidence Decay & Epistemic Debt|B.3.4]].
   * If order or time mattered for the claim, attach the OrderSpec or TimeWindow identifiers ([[04_B.01.04 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]).
...
### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:4.5 - System vs. Episteme - same shape, different interpretations
...
### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:4.6 - Scale discipline (CHR guard‑rails)
...
### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:4.7 - What improves the tuple (improvement-pattern overview)

[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] remains neutral about *how* improvement happens, but for didactic clarity:
...
### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:4.8 - Prohibition (normative) — F–G–R is not a CharacteristicSpace
...
### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:4.9 - Assurance consequence for unsupported causal-use claims

[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] consumes `CausalUseSupportVerdict`, `CausalEvidenceSupportBasis`, and relevant profile refs from [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] and [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] when an assurance claim depends on a [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] causal-use verdict:

```text
CausalUseSupportVerdict = supported | bounded | unsupported | abstain
```

`CausalAssuranceTupleTrigger` is narrower than local causal-use repair. A local [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] downgrade, redirection to a relation governing the asserted use, or abstain disposition does not require a new [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] assurance tuple by itself. Create or update a [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] tuple only when the causal-use claim is assurance-bearing, publication-bearing, release-bearing, or reused as an input to assurance, trust, certification, risk acceptance, or downstream selection. Exploratory causal wording, local causal wording repair, or a [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] cheap stop remains outside [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] until it changes assurance or publication use.

An unsupported causal-use shift lowers, blocks, or abstains from `R` for the affected causal-use claim. If `CounterfactualSamplingRealizabilityProfile.verdict = nonrealizable`, [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] lowers or blocks `R` for claims that require direct counterfactual-comparison sampling evidence. If `CounterfactualSamplingRealizabilityProfile.verdict = unknown`, direct-realization claims are unsupported, while identified, bounded, or simulation-only bounded use may remain available when [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] declares the bounded use and unsupported use.
...
What this does not authorize: [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] does not determine the [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] target `CausalityLadderRung`, estimand, causal identification, evidence design, or realizability profile; it applies assurance consequences to the `CausalUseSupportVerdict` supplied by [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] and the evidence path supplied by [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]].

## [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:5 Proof obligations (attach these when producing an Assurance tuple)

These obligations refine the generic Proof Kit from **[[01_B.01.01 - Dependency Graph & Proofs|B.1.1]] §6** for **assurance** outputs. Each Γ-flavour that emits an *Assurance(H, C | K, S)* tuple attaches the applicable obligations below.

### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:5.1 - Common obligations (all Γ‑flavours)
...
* **ASS‑MAN (SCR).**
  Produce a **SCR** listing all contributing nodes and edges with `(F, G, R)` and `CL` values, their **DesignRunTag**, and Evidence Graph Ref ([[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]). If order or time affect the claim, include the **OrderSpec** or **TimeWindow** identifiers from **[[04_B.01.04 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]**.

* **ASS‑MONO (Declared monotone characteristics).**
  List the characteristics along which local improvement cannot reduce the aggregate (this is used by future evolution, [[00_B.04 - Canonical Evolution Loop|B.4]]).

### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:5.2 - Γ\_sys (systems) — additional obligations

* **CORE‑BIC (Interface congruence).**
  Reference the **Boundary‑Inheritance Standard** (BIC) from **[[02_B.01.02 - System-specific Aggregation Γ-sys|B.1.2]]** and record any interface mismatches; these contribute to `CL_min`.
...
### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:5.3 - Γ\_epist (epistemes) — additional obligations
...
### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:5.4 - Γ\_ctx and Γ\_method (order‑sensitive) — additional obligations

* **CTX‑ORD (OrderSpec).**
  Attach the partial or total order `σ` and any **join-soundness** conditions (types, preconditions, and postconditions).
  (See [[04_B.01.04 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]] for NC‑1..3 invariants; [[05_B.01.05 - Γmethod - Order-Sensitive Method Composition & Work Enactment|B.1.5]] adds duration/capability typing.)

### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:5.5 - Γ\_time (temporal) — additional obligations
...
## [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:6 - Archetypal grounding (worked examples)

### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:6.1 - System archetype — **Battery pack safety claim**
...
### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:6.2 - Episteme archetype — **Meta-analysis claim**
...
### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:6.3 - Order and process-sequence archetype - **Manufacturing process-sequence assurance**
...
### [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:6.4 - Temporal archetype — **Versioned model credibility**
...
## [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:7 - Conformance checklist
...
| **CC-B3.6** | An Assurance SCR lists node and edge values, Evidence Graph Ref, and any OrderSpec or TimeWindow identifiers; it also displays the `describe(EntityOfConcernRef->GroundingHolonRef)` binding for the claim, the declared ReferencePlane value of world, concept, or episteme, a separable TA, VA, and LA evidence breakdown per **CC-KD-08**, decay or valid-until indicators on empirical bindings, and the Epistemic-Debt tally from **[[02_B.03.04 - Evidence Decay & Epistemic Debt]]**. | Provide auditability through [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] without collapsing evidence families. |
| **CC-B3.7** | Agency-CHR values ([[00_A.13 - The Agential Role & Agency Spectrum]]) do not override WLNK or `Φ(CL)` penalties; if agency grade change alters capabilities, model it as a Meta-Holon Transition. | Preserve safety; keep agency separate. |
...
| **False assurance relation** | Badge, dashboard color, credential display, compliance mark, provenance label, model card, datasheet, data card, assurance document, attestation label, or generated confidence phrase is used as an assurance claim. | Keep it as orientation or source pointer unless a typed assurance claim and [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] evidence path make the intended assurance use bounded and evidenced. |
| **Minimum reliance safety assurance record inflation** | Ordinary evidence, source-finding explanation, local CV, documentation, or reversible local calibration use is forced into a safety assurance record; or the assurance record is used as approval, release permission, gate passage, safety acceptance, or compliance proof. | State the trigger that meets the [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] material-reliance threshold. If the trigger is absent, use [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]], [[04_E.17.EFP - ExplanationFaithfulnessProfile - explanation-use discipline over existing MVPK faces]], [[00_A.20 - U.Flow.ConstraintValidity - Eulerian]], [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]], [[00_E.19 - Pattern Quality Gates- Review and Refresh Profiles]], or the local relation that actually governs the use. If the threshold is met, write only the minimum assurance record and contest and redress relation needed for the named reliance use. |

## [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:9 - Consequences
...
## [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:10 - Rationale (informative)

[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] distills mature post‑2015 practice across several fields into a single, small calculus:
...
* **Assurance documentation, provenance, and release-status practice** treats labels, model cards, datasheets, C2PA provenance marks, SLSA and in-toto attestations, credential displays, generated confidence phrases, and dashboards as scoped documentation or source pointers, not automatic assurance claims. [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] adopts claim, argument, and evidence discipline and scoped assurance-documentation use, adapts model cards, datasheets, data cards, attestations, provenance marks, dashboards, and generated confidence phrases as possible documentation or evidence inputs for a named assurance claim, and rejects visible-label promotion into readiness, compliance, safety, trust, `R`, `F`, `G`, `CL`, or release confidence without a typed tuple and [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence path.

Practical result from that safety-case and assurance-documentation practice: safety notes, compliance-looking labels, assurance documents, dashboards, provenance marks, model cards, datasheets, data cards, and generated confidence phrases do not become certificates, approvals, gates, safety acceptance, or assurance by appearance. The local [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] result is one typed assurance claim or minimum reliance safety assurance record for the named reliance use, with [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence path, assumptions, limitations, defeaters, residual uncertainty, monitoring or stop condition, contest and redress relation, bounded assurance use, unsupported attempted use, and reopen when evidence, source record, context, [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] identification or realizability profile, [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] gate profile, evaluation condition, monitoring, or challenge evidence admitted by the contest relation materially changes the disposition.

This arrangement preserves **[[00_A.11 - Ontological Parsimony (C-5)|A.11]] Parsimony** (few characteristics), aligns with **[[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]**, **[[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]**, and **[[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]** (clear separation of structure, order, time, cost, values), and leaves Context for domain-specific refinements that do not break the invariants.

## [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:11 - Relations

* **Builds on:** [[00_B.01 - Universal Algebra of Aggregation (Γ)|B.1]] (Universal Γ), [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]] (Proof Kit), [[02_B.01.02 - System-specific Aggregation Γ-sys|B.1.2]] (Γ_sys and BIC), [[03_B.01.03 - Γepist - Knowledge-Specific Aggregation|B.1.3]] (Γ_epist and SCR), [[04_B.01.04 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]] (Γ_ctx and Γ_time), [[00_A.12 - External Transformer & Reflexive Split|A.12]] (Transformer), [[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] (Mereology), [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] (EntityOfConcern and Description strict distinction) and [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] (role, method, and work alignment), **[[00_C.13 - Constructional Mereology (Compose-CAL)|C.13]] (Compose-CAL)**.
* **Coordinates with:** **[[00_E.14 - Human-Centric Working-Model|E.14]] (Human‑Centric Working‑Model)** for publication-facing assertion discipline and **[[03_B.03.05 - Working-Model Relations & Grounding (CT2R-LOG)|B.3.5]] (CT2R‑LOG)** for Working‑Model relation aliasing and grounding (`tv:*`, `validationMode`).
* **Coordinates with:** [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] for `CausalUseSupportVerdict`, `CausalityLadderRung`, `CausalEvidenceSupportBasis`, identification profile refs, realizability profile refs, supported causal use, and unsupported causal use; [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] for the evidence graph path carrying causal-evidence refs.
* **Coordinates with:** [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] for work disposition and reliance disposition, [[00_A.06 - Signature Stack & Boundary Discipline|A.6]] for mixed authority wording, [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] for `OperationalGate(profile)`, `GateDecision`, and `DecisionLogRef`, [[00_A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] for `ConstraintValidity` status or witness, and [[01_A.15.01 - U.Work|A.15.1]] for release or deployment work occurrence. [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] only handles typed assurance use; labels and evidence pointers stay with the source relation that governs them when assurance is not being claimed.
* **Used by:** KD-CAL improvement patterns (to plan improvements), [[00_B.04 - Canonical Evolution Loop|B.4]] (Evolution loops that raise `F`, `G`, `R`, or `CL` over time).
* **Triggers:** [[00_B.02 - Meta-Holon Transition (MHT)- Recognizing Emergence and Re-identifying Wholes|B.2]] (Meta‑Holon Transition (MHT): Recognizing Emergence and Re‑identifying Wholes) when genuine new capabilities emerge that change the applicable cutsets or envelopes.
...
## [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:11a - Assurance relation for quantum-like claims

Quantum-like wording does not raise the claim-assurance requirement by default. A local [[00_C.26 - Quantum-Like Modeling Lens|C.26]] modeling note can remain lightweight when it only prevents a representational mistake and is not used for a work-guiding use, reliance use, audit-closure claim, readiness-certification claim, or empirical-superiority claim.
...
4. If the claim is used for release, readiness, audit, compliance, assurance, or threshold-bearing work or reliance, build the [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] assurance claim over named evidence refs and scope.
...
| Claim-use requirement | [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] expectation | Output |
| --- | --- | --- |
...
- no [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] assurance use when QL is only a local representational lens;
...
## [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:11b - [[00_C.29 - Mathematical Lens Use|C.29]] mathematical-lens use relation

> If a mathematical lens is used as input to assurance, readiness, reliability, release confidence, safety, trust, or engineering justification, write the assurance relation in [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] with the relevant evidence path and residual-use limits. A [[00_C.29 - Mathematical Lens Use|C.29]] output may be cited only as a lens-use result; mathematical elegance, validation regime, or a declared structure-preserving mapping does not raise assurance by itself. Evidence paths remain [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]; measurement construction and comparability remain [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]].

## [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:End
