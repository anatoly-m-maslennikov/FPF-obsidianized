...
**Stage.** design‑time (calibration + publication) + run‑time (sentinel‑driven telemetry emission; orchestration governed by **[[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]**)
**Primary output.** A bridge calibration kit that turns **[[00_G.02 - SoTA Harvester & Synthesis|G.2]]**’s BridgeMatrix rows into **[[00_F.09 - Alignment & Bridge across Contexts|F.9]]** `BridgeCard`s and publishes: a `BridgeCalibrationTable (BCT)` + `CalibrationLedger` + `RegressionSet` + `SentinelSet`, plus UTS‑visible crossing rows and RSCR‑ready sentinel triggers scoped to `PathSliceId` / `PatternScopeId`.
**Primary hooks.** [[00_G.Core - Part G Core Invariants|G.Core]] (Part‑G invariants + RSCR trigger catalogue + Default Governing Definition Index), **[[00_G.02 - SoTA Harvester & Synthesis|G.2]]** (BridgeMatrix), **[[00_F.09 - Alignment & Bridge across Contexts|F.9]]** (BridgeCard + CL/CL^k), **F.3/F.7** (SenseCell anchoring; row bottleneck discipline), **E.18/A.21** (GateCrossing + CrossingBundle checks), **[[00_G.06 - Evidence Graph & Provenance Ledger|G.6]]** (PathId/PathSliceId citation surface), **[[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]** (downstream consumer for eligibility/selection), **[[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]** (refresh orchestration consumer), **[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]** (assurance lanes + penalty policies), **[[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]]** (DHC accounts such as AlignmentDensity), **[[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]] and [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]]** (QD/OEE pins when relevant), **[[00_C.23 - MethodFamily Evidence & Maturity (Method-SoS-LOG)|C.23]]** (SoS‑LOG clauses as explainability gates for cross‑Tradition choices), **[[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]** (Acceptance hooks/thresholds when bridges are used as selector gates), **[[00_E.10 - Unified Lexical Rules for FPF|E.10]]** (LEX / strict distinction discipline).
**Working‑Model first.** Prefer a minimal, auditable calibration procedure and worked micro‑cases; escalate to heavier harnesses only where risk warrants (per **[[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]]**).
**Non‑duplication note.** Universal Part‑G invariants (no shadow specs; Bridge‑only crossings; penalty routing to `R_eff` only; P2W split; typed/id‑based RSCR causes; defaults with one governing definition; Δ‑discipline) are governed by [[00_G.Core - Part G Core Invariants|G.Core]] and are *cited* via `CC‑GCORE‑*`. This pattern defines only the *bridge calibration kit* and its surfaces.

## [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:1 - Problem frame

SoTA synthesis (**[[00_G.02 - SoTA Harvester & Synthesis|G.2]]**) can legitimately preserve pluralism by exporting a **BridgeMatrix**: a Tradition×Tradition inventory of “comparable constructs” with preliminary notes (candidate correspondences, likely losses, tentative levels). Downstream patterns (CHR/CAL/selector/logging/shipping) cannot consume this safely unless cross‑Context reuse is:
...
[[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] packages this into a kit: `BCT` + `BridgeCard` publication + `RegressionSet`/`SentinelSet` wiring, so that later patterns can satisfy core invariants without re‑inventing cross‑Tradition machinery.

## [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:2 - Problem
...
## [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:3 - Forces
...
## [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:4 - Solution — Bridge calibration kit (BCT + BridgeCards + RegressionSet/Sentinels)

### [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:4.1 - [[00_G.Core - Part G Core Invariants|G.Core]] linkage (normative)
...
* **Expansion rule.** Effective `CoreConformanceIds`, `RSCRTriggerKindIds`, and `CorePinsRequired` are obtained by expanding the cited profile/set ids and unioning with the explicit ids above (see [[00_G.Core - Part G Core Invariants|G.Core]] nil‑elision + expansion rule).
* **Conditional pins.**
...
### [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:4.2 - Kit objects (surface governed by this patterns)

This pattern defines the *bridge calibration kit* as a set of minimal, checkable surfaces. Semantics of `BridgeCard` and CL typing are governed by **[[00_F.09 - Alignment & Bridge across Contexts|F.9]]**; [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] adds calibration records and publication/wiring surfaces.
...
**Source provenance (when sourced from [[00_G.02 - SoTA Harvester & Synthesis|G.2]]).** If the BCT is derived from a [[00_G.02 - SoTA Harvester & Synthesis|G.2]] BridgeMatrix, publish `BridgeMatrixId` (+ `BridgeMatrixRef.edition` when editioned) and row‑level linkage via `G.7:Ext.MatrixIntake` (wiring‑only), rather than duplicating [[00_G.02 - SoTA Harvester & Synthesis|G.2]] semantics in core.
...
#### [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:4.2.1 - CL / CL^k admissibility regime and plane guard (kit‑local; normative)

This subsection is kit-governed ([[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]) and complements (but does not duplicate) [[00_G.Core - Part G Core Invariants|G.Core]] penalty routing and tri‑state guard semantics.

**Admissibility regimes (row‑level, minimal).**
* `RowCL_min` MUST take a value in `{3,2,1,0}` (value set and CL meaning are governed by [[00_F.09 - Alignment & Bridge across Contexts|F.9]]; [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] governs the admissibility regime).
...
* any “blocking” outcome must be representable downstream via [[00_G.Core - Part G Core Invariants|G.Core]] tri‑state guard (`abstain` or a policy‑bound `degrade(mode=…)`), without introducing additional statuses in [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]];
* plane effects MUST NOT rewrite `CL/CL^k`; their impact is routed via the pinned policy ids and [[00_G.Core - Part G Core Invariants|G.Core]] penalty semantics.
...
### [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:4.3 - Minimal calibration procedure (auditable; table‑backed; bridge‑first)

For each Tradition‑pair and each comparable construct row from **[[00_G.02 - SoTA Harvester & Synthesis|G.2]]**:

1. **Materialise bridge artefacts.** Produce (or reuse) **[[00_F.09 - Alignment & Bridge across Contexts|F.9]]** `BridgeCard`s for the concrete `SenseCell`‑level alignments required by the row scope.
   *Note.* “SenseCell anchoring” is a kit requirement: if a row is authored at a coarser token level, the SenseCell anchors must be explicitly cited ([[00_F.03 - Intra-Context Sense Clustering|F.3]] discipline).
...
4. **Policy pins for penalty routing.** Record the policy id pins needed to audit penalty routing (ids only). Penalty semantics cite `CC‑GCORE‑PEN‑1` through [[00_G.Core - Part G Core Invariants|G.Core]]; [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]’s responsibility is to make the pins explicit and published.
5. **Row bottleneck discipline.** When a row aggregates multiple bridge cells, row summarisation uses bottleneck semantics ([[00_F.07 - Concept-Set Table|F.7]]) and carries a counterexample citation whenever any cell is loss‑noted.
...
### [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:4.4 - Publication surfaces (UTS + GateCrossing harness)

A conformant [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] publication:
...
* ensures evidence-facing citations are pin-complete: whenever bridge calibration is cited in SCR/Evidence surfaces, the citation MUST include `{BCT.id, RegressionSetId}` and the active policy id pins `{Φ(CL), Ψ(CL^k)?, Φ_plane?}` (ids only; representation is governed by [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]]/SCR).

### [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:4.5 - Worked mini‑examples (informative; post‑2015; row scopes + loss notes)
...
### [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:4.6 - Extensions (pattern‑scoped; non‑core)
...
* **GoverningPatternId:** [[00_G.02 - SoTA Harvester & Synthesis|G.2]] *(BridgeMatrix semantics and comparable-construct inventory)*
* **Uses:** `{[[G_Discipline SoTA Patterns Kit/03_02_SoTA Harvester & Synthesis/00_G.02 - SoTA Harvester & Synthesis|G.2]], [[F_The Unification Suite (U-Suite)- Concept-Sets, SenseCells & Contextual Role Assignment/09_Alignment & Bridge across Contexts/00_F.09 - Alignment & Bridge across Contexts|F.9]]}`
* **⊑/⊑⁺:** `∅`
* **RequiredPins/EditionPins/PolicyPins (minimum):**
  ...
  * `ComparableConstructId[]` *(row keys; if the source does not supply a stable id, [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] mints one while preserving `BridgeMatrixRowRef` as the provenance anchor)*
  * `LossNoteRef[]?` *(if exported by [[00_G.02 - SoTA Harvester & Synthesis|G.2]]; otherwise authored in [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] and cited from the `CalibrationLedger`)*
...
* **Notes (wiring‑only):** This module binds “row candidates” from [[00_G.02 - SoTA Harvester & Synthesis|G.2]] to the BCT/Ledger intake without copying [[00_G.02 - SoTA Harvester & Synthesis|G.2]] semantics into [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]].
...
* **GoverningPatternId:** [[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]] *(DHC metric semantics, including AlignmentDensity)*
...
* **Notes (wiring‑only):**
  * [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] stores the *counts and declared units* as a surface; [[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]] governs the meaning and legality constraints.
  * When reporting AlignmentDensity, the counted bridge set is typically restricted to `CL ≥ 2` (treat `CL=3` as “free substitution”, `CL=2` as “guarded” for reporting); conformance is enforced by `CC‑G7‑DHC‑Units‑1` while semantics remain governed by [[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]].
...
* **GPatternExtensionKind:** `InteropSpecific`
* **GoverningPatternId:** [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]] *(QD artefact semantics; uses [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]] for exploration/logging pins as needed)*
...
* **GPatternExtensionKind:** `InteropSpecific`
* **GoverningPatternId:** [[00_C.23 - MethodFamily Evidence & Maturity (Method-SoS-LOG)|C.23]] *(SoS‑LOG rule and branch semantics; [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] does not redefine meaning)*
* **Uses:** `{[[C_Kernel Extension Specifications/11_23_MethodFamily Evidence & Maturity (Method-SoS-LOG)/00_C.23 - MethodFamily Evidence & Maturity (Method-SoS-LOG)|C.23]], [[G_Discipline SoTA Patterns Kit/07_06_Evidence Graph & Provenance Ledger/00_G.06 - Evidence Graph & Provenance Ledger|G.6]]}`
* **⊑/⊑⁺:** `∅`
* **RequiredPins/EditionPins/PolicyPins (minimum; conditional on use):**
  * `SoSLogRuleId[]` *(or governing definition‑equivalent ids)*
  * `FailureBehaviorPolicyId?` *(policy id, when degrade behavior is bound)*
  * `PathId/PathSliceId` citations for explainability (via [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]])
...
* **Notes (wiring‑only):** Ensures cross‑Tradition bridge reuse decisions can be justified by citing SoS‑LOG clauses and evidence paths, without embedding SoS‑LOG semantics into [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]].
...
* **GoverningPatternId:** [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] *(Acceptance/threshold/unknown handling; [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] does not define thresholds)*
...
## [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:7 - Conformance Checklist (normative) — **CC‑G7**

| ConformanceId             | Requirement                                                                                                                                                                                                                                                                               | Purpose                                                                        |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| **CC‑G7‑CoreRef**         | [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)]] is conformant only if it satisfies the effective [[00_G.Core - Part G Core Invariants]] obligations declared by the `GCoreLinkageManifest` in **§4.1** (after nil‑elision and expansion of profile/set/pinset ids), including any explicit deltas listed there. | Make universal invariants one governing definition and enforce citation‑based reuse.       |
...
| **CC‑G7‑BridgeCard‑1**    | Any bridge published by [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)]] **MUST** be consumable as an **[[00_F.09 - Alignment & Bridge across Contexts]]** `BridgeCard` and **MUST** be SenseCell‑anchored (directly or via explicit SenseCell anchor refs).                                                                                                                        | Prevent “Context‑only” or ambiguous bridges.                                   |
| **CC‑G7‑UTS‑1**           | [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)]] outputs **MUST** mint/publish UTS‑citable ids (NameCards/twin labels as applicable) for (a) each BridgeCard (or its NameCard) and (b) each GateCrossing/crossing row that makes bridge use checkable; and **MUST** expose the resulting `UTSRowId[]` in the BCT/Ledger/crossing bundles. *(UTS discipline is delegated to `CC‑GCORE‑UTS‑1`.)* | Make bridge calibration externally citable and checkable.                      |
...
| **CC‑G7‑SoSLOG‑Pins‑1**   | When `G.7:Ext.SoSLogClauses` is in use, [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)]] outputs **MUST** expose the cited SoS‑LOG rule ids and the relevant `PathId/PathSliceId` evidence citations; any change in those pins **MUST** be RSCR‑relevant per `CC‑GCORE‑TRIG‑1…TRIG‑4`.                                               | Keep cross‑Tradition reuse explainable without embedding [[00_C.23 - MethodFamily Evidence & Maturity (Method-SoS-LOG)]] semantics.        |
| **CC‑G7‑Acceptance‑1**    | When `G.7:Ext.AcceptanceHooks` is in use, [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)]] outputs **MUST** expose the Acceptance clause ids/policy ids used as gates; thresholds/unknown handling remain governed by Acceptance; any change **MUST** be RSCR‑relevant per `CC‑GCORE‑TRIG‑1…TRIG‑4`.                                           | Keep thresholds and unknowns out of bridges while preserving auditability.     |
| **CC‑G7‑RowBottleneck‑1** | If a comparable construct row aggregates multiple bridge cells, row summaries (e.g., `RowCL_min`) **MUST** follow bottleneck discipline ([[00_F.07 - Concept-Set Table]]) and cite a counterexample whenever a cell carries a loss note.                                                                              | Forbid “CL averaging” and enforce loss‑aware summaries.                        |
| **CC‑G7‑PolicyPins‑1**    | [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)]] outputs **MUST** publish the *policy id pins* required to audit penalty routing and plane effects (ids only), as required by `CC‑GCORE‑LINK‑1/2` and `CC‑GCORE‑PEN‑1`. [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)]] MUST NOT duplicate policy tables or redefine penalty semantics.                                           | Keep penalty routing auditable while preserving single‑governing-pattern policy semantics. |
...
| **CC‑G7‑Sentinels‑1**     | [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)]] **MUST** register `BridgeSentinel` entries for bridges used by live scopes and **MUST** emit typed RSCR triggers (canonical `RSCRTriggerKindId`; see `CC‑GCORE‑TRIG‑1…TRIG‑4`) on calibration‑relevant edits, scoped to the watched `PathSliceId[]` or `PatternScopeId[]`, with the minimum payload pins from §4.1. | Enable targeted refresh rather than pack‑wide reruns.                          |
| **CC‑G7‑QD‑Pins‑1**       | When `G.7:Ext.QDParityPins` is in use, [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)]] outputs **MUST** include `{DescriptorMapRef.edition, DistanceDefRef.edition, InsertionPolicyRef}` and treat any change to those pins as RSCR‑relevant per `CC‑GCORE‑TRIG‑1…TRIG‑4`.                                                          | Prevent silent QD telemetry drift.                                             |
| **CC‑G7‑DHC‑Units‑1**     | When AlignmentDensity (or related DHC accounts) are reported, [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)]] outputs **MUST** (a) restrict the counted bridge set to rows with `RowCL_min ≥ 2` (treat `CL=3` as “free substitution”, `CL=2` as “guarded” for reporting), (b) include declared units, and (c) cite the relevant DHC method semantics ([[00_C.21 - Field Health & Structure (Discipline-CHR)]]). [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)]] MUST NOT invent arithmetic over ordinal/illegal surfaces. | Keep dashboards and discipline‑health metrics lawful and interpretable.        |

## [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:8 - Common Anti-Patterns and How to Avoid Them
...
* **SenseFamily jump (scope‑bridge used as kind‑bridge).**
  **Avoid:** keep channel/sense‑family constraints governed by **[[00_F.09 - Alignment & Bridge across Contexts|F.9]]** visible; use `RowScopeId` to state which channel is claimed, and require `CL^k` + `Ψ(CL^k)` pins when a kind‑channel bridge is invoked (do not “upgrade” a scope‑channel bridge into kind substitution).
...
## [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:9 - Consequences
...
## [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:10 - Rationale
...
## [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:11 - SoTA‑Echoing (post‑2015, for orientation; non‑normative)
...
## [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:12 - Relations

**Builds on:** [[00_G.Core - Part G Core Invariants|G.Core]], [[00_G.02 - SoTA Harvester & Synthesis|G.2]], [[00_F.03 - Intra-Context Sense Clustering|F.3]], [[00_F.07 - Concept-Set Table|F.7]], [[00_F.09 - Alignment & Bridge across Contexts|F.9]], [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], [[00_E.10 - Unified Lexical Rules for FPF|E.10]], [[00_E.18 - Transformation Flow Structure|E.18]], [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]], [[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]].
**Optionally uses via Extensions:** **[[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]** (Acceptance hooks), **[[00_C.23 - MethodFamily Evidence & Maturity (Method-SoS-LOG)|C.23]]** (SoS‑LOG clauses), **[[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]] and [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]]** (QD/OEE pins).
**Used by / prerequisite for:** **[[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]** (cross‑Tradition eligibility/selection), **[[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]** (refresh orchestration), **[[00_G.09 - Parity and Benchmark Harness|G.9]]** (parity across Traditions where bridges are required), **[[00_G.10 - SoTA Pack Shipping|G.10]]** (shipping surfaces that must cite bridge calibration ids), **[[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]** (DHC dashboards when bridge counts/units are surfaced).
**Publishes to:** **UTS** (bridge and crossing rows; twin labels as applicable) and emits RSCR‑ready telemetry/trigger payloads for **[[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]**.
**Constrains:** Any downstream consumer that claims cross‑Context/Tradition reuse must use the calibrated bridge artefacts/pins surfaced by this kit (governed by [[00_G.Core - Part G Core Invariants|G.Core]] crossing invariants apply).

## [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:End
