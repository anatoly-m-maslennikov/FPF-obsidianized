...
**Primary hooks.** [[00_G.Core - Part G Core Invariants|G.Core]] (Part‑G core invariants + trigger catalogue + Default Governing Definition Index), [[00_G.02 - SoTA Harvester & Synthesis|G.2]] (SoTA Synthesis Pack), [[00_G.03 - CHR Authoring for a CG-Frame- Characteristics, Scales, Levels, Coordinates|G.3]] (CHR Pack), [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] (CAL Pack), [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] (selector & registries), [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]] (EvidenceGraph + PathId/PathSliceId), [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] (BridgeMatrix + CL/planes), [[00_G.08 - SoS-LOG Bundles & Maturity Ladders|G.8]] (SoS‑LOG bundle surfaces), [[00_G.09 - Parity and Benchmark Harness|G.9]] (parity harness), [[00_G.10 - SoTA Pack Shipping|G.10]] (shipping), [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] (refresh orchestration), [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]] (dashboards), [[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]] (CN‑Spec), [[00_A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC-KERNEL)|A.18]] (CSLC legality), [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]] (CG‑Spec), [[00_F.17 - Unified Term Sheet (UTS)|F.17]] (UTS), [[00_F.09 - Alignment & Bridge across Contexts|F.9]] (BridgeCard / CL), [[03_E.17 - Multi-View Publication Kit|E.17]] (publication faces), [[02_E.05.02 - Notational Independence|E.5.2]] (notation independence), `E.18/A.21` (GateCrossing/CrossingBundle checks).

**Status.** Stable (Phase‑2 universalized; [[00_G.Core - Part G Core Invariants|G.Core]] linkage explicit)
**Normativity.** Normative when used (when any [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]] surface is authored/emitted/consumed); informative otherwise.

**Non‑duplication note (Phase‑2 universalization).** This pattern **does not restate** Part‑G‑wide invariants (CN/CG spec-ref governing-definition assignment, crossing visibility, penalty routing, set‑return discipline, typed RSCR triggers, Default Governing Definition Index, Δ‑discipline). Those are governed in [[00_G.Core - Part G Core Invariants|G.Core]] and referenced here via the linkage manifest and CC delegations (*cite, don’t duplicate*).

## [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:1 - Problem frame
...
[[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]] provides the missing kit: **conceptual registration, alignment, and telemetry hooks** that let external sources be wired into the Part‑G pipeline (**[[00_G.02 - SoTA Harvester & Synthesis|G.2]] → [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] → [[00_G.09 - Parity and Benchmark Harness|G.9]] → [[00_G.10 - SoTA Pack Shipping|G.10]] → [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]**, and optionally **[[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]**) while preserving Part‑G invariants via [[00_G.Core - Part G Core Invariants|G.Core]].

## [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:2 - Problem
...
## [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:3 - Forces
...
## [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:4 - Solution — Conceptual interop kit: registered sources, alignment cards, feature derivations, and RSCR‑wired telemetry

### [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:4.1 - [[00_G.Core - Part G Core Invariants|G.Core]] linkage (normative)

**Builds on:** [[00_G.Core - Part G Core Invariants|G.Core]].
...
`GCoreLinkageManifest := ⟨
  ...
  CorePinsRequired := {
    // Interop pins ([[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]‑specific; avoid duplicating `GCorePinSetId.PartG.CrossingVisibilityPins`)
...
### [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:4.2 - Interop kit objects & surfaces (pattern-governed; notation‑independent)
...
* **`ClaimMapperCard@Context`** — a conceptual “mapping recipe” that yields FPF‑native artefacts from an external source.
  ...
  * This is **not** a shadow legality gate. It is an interop surface that **cites** governing definitions ([[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]], [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]], [[00_G.03 - CHR Authoring for a CG-Frame- Characteristics, Scales, Levels, Coordinates|G.3]], [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]) and publishes the required pins for downstream audit/refresh.
  * When cross‑plane or cross‑context reuse is implicated, the alignment outputs must use the existing crossing bundles (see [[00_G.Core - Part G Core Invariants|G.Core]] linkage).
...
* **`ScaleEmbeddingSpec@Context`** — optional constraints for representation/space alignment used inside an alignment recipe.
  ...
  **LEX/UTS note (informative).** `ScaleEmbeddingSpec` is a new LEX head; when it mints a public id it must be published to UTS with twin labels (see [[00_G.Core - Part G Core Invariants|G.Core]] / UTS profile).
...
* **`IndexTelemetryPin`** — an emitted refresh input that makes interop changes RSCR‑visible.
  ...
  **Publication.** Emitted to [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] as refresh input; recorded with canonical `RSCRTriggerKindId` causes.
...
### [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:4.3 - Generation‑first interop flow (notation‑independent; governing-definition delegating)
...
3. **Produce FPF‑native inputs.** Use the alignment recipe outputs as inputs to:
   ...
   * [[00_G.02 - SoTA Harvester & Synthesis|G.2]] harvesting (ClaimSheets / operator & object inventories / candidate bridge hints),
   * [[00_G.03 - CHR Authoring for a CG-Frame- Characteristics, Scales, Levels, Coordinates|G.3]] CHR typing (when numeric signals are formalized as CHR characteristics/scales/coordinates),
   * [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] acceptance/threshold policies (when a downstream decision requires explicit CAL policy rather than telemetry),
   * [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]] dashboards (when derived SoS features are used as DHC slots).
4. **Feed selection/parity/shipping without smuggling semantics.**

   * [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] consumes the produced artefacts under its own governing spec refs and returns set‑valued outcomes (selector semantics remain governed by [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] + [[00_G.Core - Part G Core Invariants|G.Core]]).
   * [[00_G.09 - Parity and Benchmark Harness|G.9]] parity consumes pinned editions/windows and produces traceable parity reports.
   * [[00_G.10 - SoTA Pack Shipping|G.10]] shipping may include interop surfaces **as cited publications or records**; [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]] does not govern shipping.
5. **Emit telemetry and refresh causes.** Any change in external editions, alignment policies, plane maps, or embedding specs emits:
   ...
   * a canonical `RSCRTriggerKindId` (per [[00_G.Core - Part G Core Invariants|G.Core]]),
   * a scope (`PathSliceId[]` and/or `PatternScopeId`),
   * payload pins (editions/policies/UTS rows),
     enabling [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] to plan slice‑scoped refresh.

### [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:4.4 - Interfaces — minimal I/O standard (conceptual; kit‑only)

| ID   | Interface   | Consumes  | Produces   |
| ---- | ----------- | --------- | ---------- |
| **[[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)]]‑1 `Register_ExternalIndex`**  | Register `ExternalIndexCard@Context` | Provider metadata, scope, **ExternalEdition**, freshness, entityOfConcern anchor   | `ExternalIndexCard@Context` (+ UTS row when published)   |
...
| **[[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)]]‑3 `Derive_SoSFeatures`**  | Produce CHR‑typed SoS features  | ClaimSheets / external signals refs, CHR typing refs, legality proof hooks | `SoSFeatureSet@Context` (CHR‑typed; provenance pinned)   |
| **[[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)]]‑4 `Publish_InteropSurface`**  | Publish interop summary | outputs of [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)]]‑2/‑3, UTS refs | `InteropSurface@Context` (+ UTS rows/twins) |
| **[[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)]]‑5 `Emit_IndexTelemetryPin`** | Emit refresh input  | edition/policy changes + scope + payload pins  | telemetry to [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator]] (typed causes + payload pins)   |
| **[[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)]]‑6 `Wire_To_SoTA_Pack`** | Provide shipping hook  | `InteropSurface@Context` + citations to upstream artefacts  | [[00_G.10 - SoTA Pack Shipping]] pack hooks (as cited payload; no serialisation mandated)  |

## [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:5 - Extensions (pattern‑scoped; non‑core)
...
### [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:5.1 - `G.13:Ext.ExternalIndexProviderWiring` *(Phase‑3 seed)*
...
### [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:5.2 - `G.13:Ext.EmbeddingBasedAlignment` *(Phase‑3 seed; method‑specific wiring stub)*
...
### [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:5.3 - `G.13:Ext.EntityResolutionAndAliasDocking` *(interop‑specific; Phase‑3 seed)*
...
**Uses:** `{[[F_The Unification Suite (U-Suite)- Concept-Sets, SenseCells & Contextual Role Assignment/17_Unified Term Sheet (UTS)/00_F.17 - Unified Term Sheet (UTS)|F.17]], [[E_The FPF Constitution and Authoring Guides/09_10_Unified Lexical Rules for FPF/00_E.10 - Unified Lexical Rules for FPF|E.10]]}`
**⊑/⊑⁺:** `∅`
**RequiredPins/EditionPins/PolicyPins (minimum):**
...
## [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:6 - Archetypal grounding (informative; SoTA‑oriented)

**System.** *Software architecture portfolio design.*
Register an external scholarly index edition for “software architecture” concept neighborhoods. Align extracted technique/tactic claims into ClaimSheets and derive a CHR‑typed feature set (e.g., evidence depth, maturity). Use [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] to select a **set** of tactics under multi‑objective tradeoffs, and ship a SoTA pack that cites the interop surface.
...
## [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:7 - Bias‑Annotation (informative)
...
## [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:8 - Conformance Checklist (CC‑G13; applies when [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]] surfaces are used)

1. **CC‑G13‑CoreRef.** *(normative)* [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]] implementations **MUST** satisfy the *effective* [[00_G.Core - Part G Core Invariants|G.Core]] obligations declared by `G.13:4.1` (`GCoreLinkageManifest`), including trigger typing, Default Governing Definition Index citation, and crossing‑visibility pin discipline.
...
7. **CC‑G13‑EditionDisciplineForInteropCards.** *(local)* `ExternalIndexCard@Context` and `ClaimMapperCard@Context` **MUST** expose edition pins (`ExternalIndexRef.edition`, `ClaimMapperRef.edition`). Any interop surface published to UTS **MUST** cite the relevant `…Ref.edition` values (incl. `PlaneMapRef.edition?`, `ScaleEmbeddingSpecRef.edition?`) when present.
   ...
   In particular, interop transforms **MUST NOT** perform illicit arithmetic on ordinal/compare‑only scales (e.g., averaging or subtraction); any aggregation must be via lawful CAL operators with explicit scale legality (cite [[00_A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC-KERNEL)|A.18]] / `CC‑G0‑CSLC`).

8. **CC‑G13‑SoSFeaturesAreCHRTypedAndLegal.** *(local; governing-definition citing)* If `SoSFeatureTransform@Context` is used, produced SoS features **MUST** be CHR‑typed via `FeatureTypingRefs{CharacteristicId/ScaleId/CoordinateId}` (governed by [[00_G.03 - CHR Authoring for a CG-Frame- Characteristics, Scales, Levels, Coordinates|G.3]]) and any legality/units obligations must be satisfied via CSLC/CG governing definitions (cite [[00_A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC-KERNEL)|A.18]] / [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]] / [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]; do not invent interop‑local legality gates).
...
11. **CC‑G13‑NotationIndependence.** *(local)* Conformance is judged on the conceptual objects in `G.13:4.2`. Any serialisation is non‑normative and must not redefine semantics.
   *(Cites [[02_E.05.02 - Notational Independence|E.5.2]] for notation independence.)*

## [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:9 - Common Anti‑Patterns and How to Avoid Them
...
## [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:10 - Consequences
...
## [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:11 - Rationale
...
## [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:12 - SoTA‑Echoing (post‑2015, for orientation; non‑normative)
...
## [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:13 - Relations

**Builds on:** [[00_G.Core - Part G Core Invariants|G.Core]].
**Imports:** [[00_G.02 - SoTA Harvester & Synthesis|G.2]], [[00_G.03 - CHR Authoring for a CG-Frame- Characteristics, Scales, Levels, Coordinates|G.3]], [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]], [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]], [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]], [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]], [[00_G.09 - Parity and Benchmark Harness|G.9]], [[00_G.10 - SoTA Pack Shipping|G.10]], [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]], [[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]], [[00_A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC-KERNEL)|A.18]], [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]], [[00_F.17 - Unified Term Sheet (UTS)|F.17]], [[02_E.05.02 - Notational Independence|E.5.2]], [[00_E.18 - Transformation Flow Structure|E.18]].
**Publishes to:** UTS (twin labels where applicable); refresh inputs to [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]; shipping hook surfaces to [[00_G.10 - SoTA Pack Shipping|G.10]] (as cited publications or records).
**Relates to:** [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]] (dashboards), [[00_G.08 - SoS-LOG Bundles & Maturity Ladders|G.8]] (SoS-LOG bundle surfaces) when interop-derived publications or records are consumed there.

## [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:14 - Author’s quick checklist (informative)
...
## [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]]:End
