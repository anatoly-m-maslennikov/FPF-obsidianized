...
RSCR tests, Worked‑Examples, and a `TaskMap@Context` (`TaskMap`; handoff record consumed by [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]).

**Primary hooks.** [[00_G.Core - Part G Core Invariants|G.Core]] (Part‑G invariants + RSCR trigger catalogue + Default Governing Definition Index), [[00_G.01 - CG-Frame-Ready Generator|G.1]] (CG‑FrameContext), [[00_G.02 - SoTA Harvester & Synthesis|G.2]] (SoTA Synthesis Pack), [[00_G.03 - CHR Authoring for a CG-Frame- Characteristics, Scales, Levels, Coordinates|G.3]] (CHR Pack), [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]] (CG‑Spec legality gate), [[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]] (CN‑Spec), [[00_A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC-KERNEL)|A.18]] (CSLC), [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] (provenance anchors), [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (trust / freshness / decay), [[00_E.18 - Transformation Flow Structure|E.18]] + [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] + [[00_F.09 - Alignment & Bridge across Contexts|F.9]]/[[00_F.17 - Unified Term Sheet (UTS)|F.17]]/[[03_E.17 - Multi-View Publication Kit|E.17]] (GateCrossing / CrossingBundle harnesses), [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]] (EvidenceGraph / PathId / PathSliceId; wired via Extensions), [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] (Selector & Dispatch), [[00_G.10 - SoTA Pack Shipping|G.10]] (shipping), [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] (refresh orchestration), plus Contexts/UTS/LEX disciplines already fixed elsewhere in the spec.

**Non‑duplication note.** Universal Part‑G invariants (no shadow specs, crossing visibility, tri‑state guard, penalties→`R_eff`‑only, set‑return semantics, P2W split, typed RSCR causes, Default Governing Definition Index, shipping boundary) are governed in [[00_G.Core - Part G Core Invariants|G.Core]] and are pulled into [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] only through the `G.Core linkage` manifest in **[[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4.1** (and via explicit delegations in CC).

## [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:1 - Problem frame
...
This pattern provides the design‑time authoring kit and the publication surface for CAL artifacts, while delegating Part‑G‑wide invariants to [[00_G.Core - Part G Core Invariants|G.Core]] and CN-Spec and CG-Spec legality to `CG‑Spec`/`CN‑Spec`.

## [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:2 - Problem
...
## [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:3 - Forces
...
## [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4 - Solution — CAL authoring kit and publication surface

### [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4.1 - [[00_G.Core - Part G Core Invariants|G.Core]] linkage (normative)

**Builds on:** [[00_G.Core - Part G Core Invariants|G.Core]] (Part‑G core invariants; citation/delegation hub)

**GCoreLinkageManifest (normative).** Canonical shape, Nil‑elision, and the Expansion rule are defined in [[00_G.Core - Part G Core Invariants|G.Core]].
...
By the [[00_G.Core - Part G Core Invariants|G.Core]] Expansion rule, the effective conformance ids / trigger kinds / pin obligations for [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] are the expansions of the referenced profiles/sets/pin‑sets plus the explicit deltas above.

Notes (normative intent, delegated semantics):

* The semantics of tri‑state outcomes, penalty routing, set‑return discipline, crossing visibility, P2W split, typed RSCR causes, and the Default Governing Definition Index are governed in [[00_G.Core - Part G Core Invariants|G.Core]] and are not redefined here.
* EvidenceGraph/Path pins (when used) are declared only via **`G.4:Ext.EvidenceGraphWiring`** in **[[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4.5** (so `G.Core linkage` stays minimal and does not “pull in” [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]] by default).
* Method‑specific pins (e.g., QD descriptor/distance/insert policy pins; open‑ended transfer rules pins) MUST appear only in **Extensions** blocks (see **[[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4.5**) and MUST NOT introduce competing defaults.

### [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4.2 - `CAL Pack@CG-Frame` surface (kit governed by this pattern)
...
* `CAL.Charter@Context` — scope anchor for this CAL pack:
  ...
  * emits `TaskMap@Context` (`TaskMap`) as the canonical handoff record to [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] (task→gates/flows/evidence pins).
...
* **Optional** `CAL.NQD[]` — QD/OEE‑related calculus surfaces when declared:
  ...
  * semantics are governed by method‑specific governing definitions (e.g., [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]], [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]]) and not redefined by CAL.
...
* **No refresh governing-definition assignment**: CAL does not govern refresh orchestration; it only publishes pins/payload for refresh (governing definition: [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]).
...
### [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4.3 - CAL authoring chassis C1–C9 (kit governed by this pattern)
...
* emits a `TaskMap@Context` (`TaskMap`) that binds each declared `TaskSignature` (or task family) to:
  ...
  * and any required policy pins/edition pins for reproducibility.
  This is the canonical “handoff manifest” consumed by [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] (thresholds remain only inside `CAL.Acceptance`).
...
* `UnknownHandling` as a tri‑state choice (via [[00_G.Core - Part G Core Invariants|G.Core]] semantics),
...
* `GateCrossingId[]` / `CrossingBundleId[]` **iff** the clause relies on cross-context, cross-plane, or cross-edition imports (no “silent reuse”).
  Missing required crossing artefacts is a conformance failure and blocks publication of the affected clause/flow (GateCrossing harness: [[00_E.18 - Transformation Flow Structure|E.18]]/[[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]/[[00_F.09 - Alignment & Bridge across Contexts|F.9]]/[[00_F.17 - Unified Term Sheet (UTS)|F.17]]/[[03_E.17 - Multi-View Publication Kit|E.17]]; crossing invariants: [[00_G.Core - Part G Core Invariants|G.Core]]).
...
* provenance anchor references ([[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]‑style carriers/anchors, cited by id),
* lane tags (`F/G/R`) for each evidence contribution (no implicit lane mixing; penalties route only to `R_eff` as governed by [[00_G.Core - Part G Core Invariants|G.Core]]),
...
* **does not** redefine QD/OEE semantics (those remain governed by method‑specific patterns such as [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]] / [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]] and are wired via `Extensions`).
...
Any cross-context, cross-plane, or cross-edition import required by CAL publication is handled through GateCrossing/CrossingBundle discipline (as governed by [[00_G.Core - Part G Core Invariants|G.Core]]), and CAL publication is blocked if required crossing artifacts are missing.
...
* emit RSCR‑relevant trigger payload pins (editions/policies/UTS ids/paths) for refresh orchestration (governing definition: [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]).

### [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4.4 - Interfaces (minimal I/O surface)
...
### [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4.5 - Extensions (pattern‑scoped; non‑core)

[[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] supports method‑family and discipline‑specific calculus variations exclusively via pattern‑scoped extensions.
...
- **GoverningPatternId:** [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]]
- **Uses:** `{[[G_Discipline SoTA Patterns Kit/07_06_Evidence Graph & Provenance Ledger/00_G.06 - Evidence Graph & Provenance Ledger|G.6]]}`
- **⊑/⊑⁺:** `∅`
...
- **Uses:** `{[[C_Kernel Extension Specifications/06_18_Open-Ended Search Calculus (NQD-CAL)/00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]]}`
- **⊑/⊑⁺:** `∅`
- **RequiredPins/EditionPins/PolicyPins (minimum):**
...
## [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:5 - Archetypal Grounding
...
Downstream, [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] consumes only the handoff manifest: clause ids, operator ids, and evidence profile ids (no embedded thresholds).
...
## [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:6 - Bias-Annotation
...
## [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:7 - Conformance Checklist (normative)

| ConformanceId     | Statement                                                                                                                                                                                                                                                                                                      |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **CC‑G4‑CoreRef** | Conformance with [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring]] requires satisfying the effective [[00_G.Core - Part G Core Invariants]] obligations referenced by the `GCoreLinkageManifest` in **[[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring]]:4.1** (profiles, pin sets, consumed defaults, and trigger kinds).                                                                                                              |
...
## [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:9 - Consequences
...
* Downstream patterns ([[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]], [[00_G.08 - SoS-LOG Bundles & Maturity Ladders|G.8]], [[00_G.09 - Parity and Benchmark Harness|G.9]], [[00_G.10 - SoTA Pack Shipping|G.10]], [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]) can reference stable ids/pins without redefining acceptance or operator semantics.
...
## [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:10 - Rationale
...
* [[00_G.Core - Part G Core Invariants|G.Core]] governs Part‑G invariants and trigger/default discipline,
* [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] governs the CAL kit: authoring objects, publication surface, and handoff manifest.
...
## [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:11 - SoTA-Echoing
...
All of these remain method‑specific semantics and therefore belong in `Extensions` blocks (or their governing patterns), while [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] keeps the calculus kit stable and auditable.

## [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:12 - Relations

**Builds on:** [[00_G.Core - Part G Core Invariants|G.Core]] (and the pattern template discipline in [[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]]).

**Uses:** [[00_G.01 - CG-Frame-Ready Generator|G.1]] (CG‑FrameContext), [[00_G.02 - SoTA Harvester & Synthesis|G.2]] (SoTA Synthesis Pack), [[00_G.03 - CHR Authoring for a CG-Frame- Characteristics, Scales, Levels, Coordinates|G.3]] (CHR Pack), [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]] (CG‑Spec legality gate), [[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]] (CN‑Spec), [[00_A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC-KERNEL)|A.18]] (CSLC), [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] (provenance anchors), [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (trust/freshness/decay), [[00_E.18 - Transformation Flow Structure|E.18]] + [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] + [[00_F.09 - Alignment & Bridge across Contexts|F.9]]/[[00_F.17 - Unified Term Sheet (UTS)|F.17]]/[[03_E.17 - Multi-View Publication Kit|E.17]] (GateCrossing harness).

**Uses (via Extensions):** [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]] (EvidenceGraph/Path citation; when `G.4:Ext.EvidenceGraphWiring` is present), [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]] (NQD), [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]] (E/E‑LOG), [[00_C.23 - MethodFamily Evidence & Maturity (Method-SoS-LOG)|C.23]] (SoS‑LOG).

**Used by:** [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] (selector/dispatcher), [[00_G.08 - SoS-LOG Bundles & Maturity Ladders|G.8]] (SoS‑LOG bundles), [[00_G.09 - Parity and Benchmark Harness|G.9]] (parity), [[00_G.10 - SoTA Pack Shipping|G.10]] (shipping), [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] (refresh orchestration).
**Publishes to:** UTS (public ids and public-id continuity records), RSCR (tests and trigger emissions), [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] (handoff manifest), and, as cited payload, shipped packs governed by [[00_G.10 - SoTA Pack Shipping|G.10]].
...
## [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:End
