...
## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:0 - Use this when
...
## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:0.1 - What goes wrong if missed
...
- selected-set publication gets hidden inside [[00_C.11 - Decision Theory (Decsn-CAL)|C.11]], [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]], or [[00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]], so the published result no longer makes clear whether it carries local choice, pool policy, enactment, or publication
...
## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:0.2 - What this buys
...
## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:0.3 - First-minute questions
...
## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:0.4 - First output
...
If that first output still cannot be written honestly, the current publication result is not finished [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] publication yet.

[[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] keeps the dispatcher/registry object set here and leaves universal Part-G invariants to [[00_G.Core - Part G Core Invariants|G.Core]]; method- or generator-specific semantics stay in their named source patterns and arrive here only through explicit pins.

When [[00_C.11 - Decision Theory (Decsn-CAL)|C.11]] has already emitted one local choice result, [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]] one pool-policy result, or [[00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]] one enactment-facing next move, [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] begins where the question becomes selector-facing publication of the retained set or narrowed handoff result rather than one more explanation of why the result looked reasonable. A conformant [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] pass should therefore publish the retained set, narrowed handoff, or abstain result directly, with its declared outcome kind, any applicable public selected-set label, and basis pins explicit in the result itself.
...
## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:1 - Problem frame

A `CG‑FrameContext` (from **[[00_G.01 - CG-Frame-Ready Generator|G.1]]**) and a `SoTA Synthesis Pack@CG‑Frame` (from **[[00_G.02 - SoTA Harvester & Synthesis|G.2]]**) expose multiple rival, internally coherent **method families** (and sometimes **generator families**) that can plausibly act on the same `EntityOfConcernRef` and ReferencePlane.
...
## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:2 - Problem
...
* cleanly separates:
  ...
  * **universal Part‑G invariants** (carried by [[00_G.Core - Part G Core Invariants|G.Core]]),
  * **method/generator specifics** (wired only via `Extensions` blocks).

## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:3 - Forces
...
## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:4 - Solution

### [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:4.6a - Causal method dispatch declarations
...
What this does not authorize: [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] does not identify causal effects, decide fairness, certify off-policy causal evaluation, or compare cross-rung causal methods as one undifferentiated improvement set; it keeps method dispatch and selected-set publication while [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] governs causal-use support.

### [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:4.1 - [[00_G.Core - Part G Core Invariants|G.Core]] linkage (normative)
...
### [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:4.2 - Dispatcher & Registry object set (notation‑independent)

[[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] defines the **object-set components** below. Their purpose is to make dispatch **possible and auditable** without embedding any method-family semantics in the selector kernel.
...
* `EligibilityStandardRef`: a typed predicate record (tri‑state per [[00_G.Core - Part G Core Invariants|G.Core]]), expressed in CHR/CAL terms and pinned to the relevant editions.
...
* `EvidencePins`: citations to [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]] (`PathId/PathSliceId`) for claims/guarantees where such claims are asserted.
...
A registry row for families that generate tasks/environments and/or co‑evolve solver families. [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] carries the registry-entry shape, not the generator semantics:
...
[[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] treats `TaskSignatureRef` as an input record; it does not define CHR/CAL semantics.
...
When the real selector question is acquisition of usable specialization on a declared task family, the selector may publish one `TaskFamilySpecializationProfile@Context` for each candidate, one `SpecialistHandoff`, or one narrowed handoff plan. Here `profile` means one selector-time comparison record for bounded specialization, not a new kernel type and not a generic narrative profile. [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] carries this selector-time specialization question here; it does not re-govern the adaptation-signature field vocabulary from [[01_C.22.01 - Task-family adaptation signature|C.22.1]].
...
A library of composition shapes (preconditioner → solver → verifier; cascades; meta‑selectors) **as templates**, legality‑checked and pinned. Concrete strategy semantics stay in the referenced method families; [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] only carries the composition template.
...
* telemetry pins to refresh orchestration ([[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]), without governing orchestration.
...
### [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:4.3 - Selector head and narrower selector families

Selection/dispatch stays one generic selector head. Narrower selector families may refine it, but they do not redefine the universal invariants pinned through [[00_G.Core - Part G Core Invariants|G.Core]], do not add hidden mandatory inputs beyond pinned policy or edition refs, and do not mutate SlotKinds.
...
### [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:4.4 - Interfaces (minimal I/O record)

| Interface                         | Consumes                                                                                                                                                     | Produces                                                                                                                                                                                                                                                   |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **[[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]]‑1 RegisterFamily**          | `SoTA` family cards (from [[00_G.02 - SoTA Harvester & Synthesis]]), CHR/CAL pins (from `G.3/G.4`), `CNSpecRef.edition`, `CGSpecRef.edition`, `ContextId`                                       | A `MethodFamily` registry row (`MethodFamilyId`, `EligibilityStandardRef`, `AssuranceProfileRef`, `UTSRowId`, pinned refs)                                                                                                                                 |
| **[[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]]‑2 RegisterGeneratorFamily** | `SoTA` generator family cards (from [[00_G.02 - SoTA Harvester & Synthesis]]), `ContextId`, pinned refs (including `TransferRulesRef.edition` when applicable)                                  | A `GeneratorFamily` registry row (`GeneratorFamilyId`, `GeneratorSignatureRef`, `UTSRowId`, pinned refs)                                                                                                                                                   |
| **[[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]]‑3 Select**                  | `TaskSignatureRef`, `MethodFamilyId[]` (in scope), pinned `CNSpecRef/CGSpecRef` (editions), policy refs (if any), audit citation pins (`PathId/PathSliceId`) | `CandidateSet` (set‑returning), declared selector result with `PortfolioMode` recorded, `DRR + SCR` pins; if no admissible candidate exists: return `CandidateSet=∅` plus an escalation hint (`ActionHint`) and the pins required to plan next steps (P2W split applies) |
| **[[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]]‑4 Compose**                 | `CandidateSet`, composition template refs, pinned legality constraints                                                                                       | Composite strategy template (template‑level; legality‑checked; pinned)                                                                                                                                                                                      |
| **[[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]]‑5 Telemetry**               | run outcomes + citations + policy/edition pins                                                                                                               | refresh cues (typed RSCR causes + payload pins), parity deltas (if parity harness is in use), telemetry pins (selector‑side; orchestration governing definition is [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator]])                                                                                              |

### [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:4.4a - Worked selector slice

- A catalyst-search team is choosing among three method families for the same declared `TaskSignature` and [[01_C.22.01 - Task-family adaptation signature|C.22.1]] adaptation signature.
...
- When one upstream [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]] pass has already narrowed the live pool to one internal retained subset over registered families, [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] may publish that result as one `Shortlist` with one `ShortlistId` and explicit basis pins only when selector-facing publication is now the question. Until that emission occurs, the internal retained subset is not yet one public shortlist result.
- When one upstream [[00_C.11 - Decision Theory (Decsn-CAL)|C.11]] pass has already fixed one local choice over one declared source set, or one [[00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]] pass has already produced one enactment-facing narrowed handoff, [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] may publish the selected-set or narrowed-handoff result only when selector-facing publication is now the question. Until this [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] emission occurs, the `ChoiceResult`, `CallPlan`, or `CheckpointReturn` is not itself one public `Shortlist`, `RankedShortlist`, or `ShortlistId`-bearing result.

### [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:4.4b - Published selected-set result and closure rule

A finished [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] pass should publish one explicit selected-set result from the dispatcher/registry question rather than one selector trace that leaves the public result implicit.
...
If the publication still does not state what public result was emitted, who remained in it, whether order belongs to it, and which pins justify it, then the selector has not yet published one finished [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] result.

### [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:4.4c - Publication quick card

The smallest useful [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] publication card usually states:
...
If the card does not already state what was published, who survived, whether order belongs to the result, and which pins justify it, the publication is still unfinished [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] work.

### [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:4.4ca - Derived tradition-view publication stays derived over one declared palette
...
### [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:4.4d - Worked publication closure slice

Three short contrasts keep the publication law practical.

**Several survivors, no public order belongs to the result.**
When the selector has retained more than one admissible family but no downstream public order belongs to the published result, [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] should close as one `Shortlist` over the registered surviving rows:
...
When one ordered public handoff is required, [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] should say so directly instead of leaving order implicit:
...
When no family clears the pinned legality or evidence gates, [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] should close as one abstain or escalation result rather than as one empty shortlist pretending to be progress:
...
* `Uses`: `{[[C_Kernel Extension Specifications/07_19_Explore-Exploit Governor (E-E-LOG)/00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]]}`
* `⊑/⊑⁺`: `∅`
* `RequiredPins/EditionPins/PolicyPins (minimum):`

  * `EELensPolicyRef` *(or equivalent lens/policy id carried by [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]])*
  * `RiskBudgetRef?`
  * `ProbeAccountingRef?`
...
* `RequiredPins/EditionPins/PolicyPins (minimum):`
  ...
  * `MaturityRungId[]?` *(when maturity ladders are used as gates; semantics come from [[00_C.23 - MethodFamily Evidence & Maturity (Method-SoS-LOG)|C.23]])*
...
* `Notes (extension discipline; semantics cited):`
  ...
  * [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] core remains QD‑agnostic; QD semantics are governed by [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]].
...
* `GoverningPatternId`: [[00_G.02 - SoTA Harvester & Synthesis|G.2]]
* `Uses`: `{[[G_Discipline SoTA Patterns Kit/03_02_SoTA Harvester & Synthesis/00_G.02 - SoTA Harvester & Synthesis|G.2]], [[C_Kernel Extension Specifications/07_19_Explore-Exploit Governor (E-E-LOG)/00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]], [[C_Kernel Extension Specifications/11_23_MethodFamily Evidence & Maturity (Method-SoS-LOG)/00_C.23 - MethodFamily Evidence & Maturity (Method-SoS-LOG)|C.23]]}`
* `⊑/⊑⁺`: `∅`
* `RequiredPins/EditionPins/PolicyPins (minimum):`
...
* `Notes (extension discipline; semantics cited):`
  ...
  * This block enables declared sets of `{Environment, MethodFamily}` pairs without redefining generator semantics in [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]].
  * Post‑2015 examples typically referenced via [[00_G.02 - SoTA Harvester & Synthesis|G.2]] family cards: POET‑class and later open‑ended/co‑evolutionary regimes, including enhanced variants where transfer policies and validity gates must be edition‑pinned.

### [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:4.4e - Selector-facing outcome kinds
...
## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:5 - Archetypal Grounding
...
The admissible [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] result is therefore one `RankedShortlist` with explicit ordering, `ShortlistId`, and handoff-facing `nextUse`, so the publication itself states whether the order is public.
...
The truthful [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] result is one abstain or escalation publication that names the blocking pins and the next downstream use boundary, not one empty shortlist that leaves downstream users unsure whether selection silently failed or admissibly stopped.

## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:6 - Bias-Annotation
...
## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:7 - Conformance Checklist (normative)

| ConformanceId   | Statement |
| --------------- | ----------|
| `CC‑G5‑CoreRef` | **Core conformance bridge.** [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]] is conformant only if the **effective** [[00_G.Core - Part G Core Invariants]] obligations referenced by `G.5:4.1 (GCoreLinkageManifest)` are satisfied (after profile/set expansion and explicit deltas). |
| `CC‑G5.0`       | Core standards **SHALL** remain notation‑independent; vendor/tool keywords are forbidden in registry, eligibility, assurance, or selector‑kernel obligations ([[00_E.05 - Four Guard-Rails of FPF]].*). |
...
| `CC‑G5.3`       | **Delegated (ID‑continuity).** Cross‑Context use **MUST** follow [[00_G.Core - Part G Core Invariants]] crossing visibility and penalty routing. **Delegation targets:** `CC‑GCORE‑CROSS‑1`, `CC‑GCORE‑PEN‑1`.  |
...
| `CC‑G5.6`       | Method and generator family identities **SHALL** be published to UTS with the required naming discipline (twin labels where applicable; deprecations follow lexical continuity rules). *(Core conformance applies; [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]] adds the registry‑specific publication obligation.)* |
...
| `CC‑G5.11`      | **Delegated (ID‑continuity).** Tri‑state eligibility/acceptance semantics and unknown handling are pinned through [[00_G.Core - Part G Core Invariants]]. **Delegation target:** `CC‑GCORE‑GUARD‑1`. *(Includes the rule that `degrade(...)` is expressed via a pinned FailureBehavior/SoS‑LOG branch id — not as a fourth status.)* |
| `CC‑G5.12`      | **No “universal” cross‑Tradition scoring.** Cross‑Tradition selection **MUST NOT** rely on a single numeric formula not justified by pinned CHR/CAL constraints and the constraint-bearing spec refs. If a triad or selected set **claims universality**, it **MUST** satisfy **explicit, pinned** heterogeneity gates (ids/pins), e.g., `FamilyCoverage ≥ k` and `MinInterFamilyDistance ≥ δ_family`, where `k` and `δ_family` are declared by the pinned policy/TaskSignature/SoTA pack, and cite the relevant **Context Card id ([[00_F.01 - Domain-Family Landscape Survey]])** in DRR/SCR; otherwise treat the outcome as Context‑local.  |
...
| `CC‑G5.23a`     | **Parity‑run publication.** If parity harness is in use, a selector/generator **MUST** publish a parity run and `ParityCard` to **UTS** (see [[00_G.09 - Parity and Benchmark Harness]]). This obligation remains mandatory irrespective of dominance/`PortfolioMode` policy. |
...
| `CC‑G5.30`      | **No Strategy minting.** Within [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]], “strategy” is a policy‑bound composition template; the pattern **SHALL NOT** mint a new universal `U.Type` named `Strategy` ([[00_E.10 - Unified Lexical Rules for FPF]] discipline). If a stable reference is needed, publish composition/policy ids (e.g., UTS entries) rather than minting a universal type. |
...
| `CC‑G5.35`      | **Publication closure.** Any published selector result **MUST** state the declared `SelectorOutcomeKind`, any applicable public selected-set label, retained members or narrowed handoff content, ordering status (when applicable), and basis pins directly in the emitted result rather than relying on upstream [[00_C.11 - Decision Theory (Decsn-CAL)]], [[00_C.19 - Explore-Exploit Governor (E-E-LOG)]], or [[00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)]] notes. |
| `CC‑G5.36`      | **Neighboring-pattern boundary.** If the current question is still local choice among already-available options, pool policy over still-live candidate lines, or enactment planning after choice, [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]] **MUST** consume the published result from [[00_C.11 - Decision Theory (Decsn-CAL)]], [[00_C.19 - Explore-Exploit Governor (E-E-LOG)]], or [[00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)]] rather than restating those patterns as if publication itself decided the matter. |
...
## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:8 - Common Anti-Patterns and How to Avoid Them
...
* **Anti‑pattern: “Selector as a shadow spec.”**
  ...
  *Avoid:* route all constraint semantics via `CNSpecRef/CGSpecRef` and pinned CAL records; keep [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] core as a facade.
...
* **Anti‑pattern: “Publication hidden in upstream reasoning.”**
  *Symptom:* the retained set exists only as one implication inside [[00_C.11 - Decision Theory (Decsn-CAL)|C.11]], [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]], or [[00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]], while [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] never names the published selected-set label.
...
* **Anti‑pattern: “Published result without closure record.”**
  ...
  *Avoid:* publish the head, retained members, ordering status, abstain or escalation condition, and basis pins directly in [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]].

## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:9 - Consequences
...
* **Core stability.** Universal invariants are pinned through [[00_G.Core - Part G Core Invariants|G.Core]]; method/generator innovation does not churn the selector head.
...
## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:10 - Rationale
...
## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:11 - SoTA-Echoing
...
## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:12 - Relations

**Builds on (normative):** [[00_G.Core - Part G Core Invariants|G.Core]] (core invariants + linkage discipline).
...
* Causal-use method dispatch: [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] when method selection involves causal effect, counterfactual comparison, causal fairness, causal policy, causal RL, or simulation-only causal-use claims.
* Optional method/generator extensions via `G.5:Ext.*`: [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]], [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]], [[00_C.23 - MethodFamily Evidence & Maturity (Method-SoS-LOG)|C.23]], plus any future extension-bearing patterns that add extra selector pins.
* Mathematical-lens use: apply [[00_C.29 - Mathematical Lens Use|C.29]] when a selector input depends on a claim-relevant comparator, distance, descriptor geometry, embedding, normalization, surrogate model, learned representation, QD archive descriptor, model-family label, or model-selection basis whose mathematical object, mapping mode, preserved/lost structure, or stop condition is not yet recoverable. [[00_C.29 - Mathematical Lens Use|C.29]] may return `NoMathLensUseNeeded`, `MathLensUse.LensCandidateNote`, `MathLensUse.OneLine`, `MathLensUse.MiniCard`, `MathLensUse.FullCard`, or `NeighborGoverningPatternNote` for the stated selector use. It does not publish the selected set, selector policy, registry row, shortlist, ranked shortlist, or selector evidence pins; those stay in [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] and its governing refs.

**Publishes to:** `UTS` (family ids, selector policy records, and selected-set identities such as `ShortlistId` when one public result is emitted), [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]] (audit citations), RSCR emission records (typed triggers + payload pins), and downstream packs via [[00_G.10 - SoTA Pack Shipping|G.10]] shipping publications.

**Coordinates with:** [[00_C.11 - Decision Theory (Decsn-CAL)|C.11]] for local choice results, [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]] for pool-policy records, [[00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]] for enactment-facing next-move records, and the accepted Q-Front shortlist-family continuity line when the published selected-set label is one shortlist-family result.

## [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]:End
---
