...
**Tag.** Architectural pattern (foundational Standard; constrains [[00_G.01 - CG-Frame-Ready Generator|G.1]]–[[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]])
...
**Non-duplication note.** Universal Part‑G invariants are governed by [[00_G.Core - Part G Core Invariants|G.Core]] and are satisfied here **only via delegation** (`CC‑G0‑CoreRef` → `CC‑GCORE‑*`). Single‑governing definition CN/CG spec-ref discipline is enforced via `CC‑GCORE‑CN‑CG‑1` (no shadow specs; no competing defaults).

## [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:1 - Problem frame

A team defines or evolves a `CG‑Frame` (e.g., a frame for creativity measurement, decision quality, architecture trade‑offs, or selected-set publication). Downstream mechanisms ([[00_G.01 - CG-Frame-Ready Generator|G.1]]–[[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] and beyond) must compare, aggregate, and publish CHR‑typed observations in ways that are:
...
## [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:2 - Problem
...
## [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:3 - Forces
...
## [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:4 - Solution — CG‑Spec as the design-time legality gate
...
### [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:4.1 - [[00_G.Core - Part G Core Invariants|G.Core]] linkage (normative)

**Builds on:** [[00_G.Core - Part G Core Invariants|G.Core]] (Part-G core invariants; governing-pattern citation)
...
### [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:4.2 - CG‑Spec object model (normative)
...
### [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:4.3 - CG‑Spec conceptual model (normative)
...
* `MinimalEvidenceEntry` MUST declare: lane requirements, evidence carriers, freshness window (if any), and explicit failure behavior wiring. The semantics of `{pass|degrade|abstain}` and `degrade(mode=…)` are delegated to [[00_G.Core - Part G Core Invariants|G.Core]].

### [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:4.4 - Interfaces (normative)

| Interface          | Consumes                             | Produces / constrains                                                      |
| ------------------ | ------------------------------------ | -------------------------------------------------------------------------- |
| **[[00_G.0 - Frame Standard and Comparability Governance - CG-Spec]]‑1 Charter**  | CG‑Frame brief, USM scope signals    | `CG‑Spec.Scope`, `entityOfConcern`, `ReferenceMap`                         |
| **[[00_G.0 - Frame Standard and Comparability Governance - CG-Spec]]‑2 SCP**      | CHR pack refs ([[00_G.03 - CHR Authoring for a CG-Frame- Characteristics, Scales, Levels, Coordinates]]), legality proofs | `CG‑Spec.SCP` + bindings to lawful operators/aggregators                   |
| **[[00_G.0 - Frame Standard and Comparability Governance - CG-Spec]]‑3 Evidence** | SoTA inputs ([[00_G.02 - SoTA Harvester & Synthesis]]), carriers ([[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]])   | `CG‑Spec.MinimalEvidence`, `Γ‑fold` segment pins, `CL‑Routing`, `Φ` ids    |
| **[[00_G.0 - Frame Standard and Comparability Governance - CG-Spec]]‑4 Publish**  | All above                            | Versioned `CG‑Spec@UTS` plus Name Cards, public-id continuity records, and RSCR tests and trigger kinds  |
| **[[00_G.0 - Frame Standard and Comparability Governance - CG-Spec]]‑5 Expose_CrossingHooks** | `CG‑Spec` + crossing/plane/policy pins | GateCrossing inputs for `GateChecks` (`E.18/A.21`): plane checks, lane purity, lexical SD pins |
| **→ [[00_G.01 - CG-Frame-Ready Generator]]**          | `CG‑Spec`                            | Generator guardrails (Comparator/SCP/MinEv pins); degrade/abstain wiring   |
| **→ [[00_G.02 - SoTA Harvester & Synthesis]]**          | `CG‑Spec`                            | Harvesting inclusion/exclusion and crossing policy constraints             |
| **→ [[00_G.03 - CHR Authoring for a CG-Frame- Characteristics, Scales, Levels, Coordinates]]**          | `CG‑Spec`                            | Required CHR characteristics/scales/operators to exist                     |
| **→ [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring]]**          | `CG‑Spec`                            | Acceptance templates; evidence minima; Γ‑fold override proof hooks         |
| **→ [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]]**          | `CG‑Spec`                            | Eligibility gates and explainability pins (Path/UTS/policy ids)            |
| **→ [[00_G.06 - Evidence Graph & Provenance Ledger]]**          | `CG‑Spec`                            | EvidenceGraph/SCR pinning surface (policy ids + Path/PathSlice discipline) |

### [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:4.5 - CG‑Spec authoring chassis (informative)
...
4. **Declare MinimalEvidence.** For each characteristic: required lanes/carriers, freshness window, crossing allowances (if any), and explicit failure behavior wiring (tri-state semantics delegated to [[00_G.Core - Part G Core Invariants|G.Core]]).
...
### [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:4.6 - Extensions (pattern-scoped; non-core)
...
* **GoverningPatternId:** [[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]]
...
* **Notes (wiring-only):** Crossing semantics and penalty routing are delegated to [[00_G.Core - Part G Core Invariants|G.Core]]; this module only lists the required pins used by `CG‑Spec` entries.
...
* **Notes (wiring-only):** Any SoTA palette/tradition semantics are governed by [[00_G.02 - SoTA Harvester & Synthesis|G.2]]. [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]] only requires that `CG‑Spec` entries cite the needed SoTA artefacts for auditability.
...
* **Uses:** `{[[C_Kernel Extension Specifications/06_18_Open-Ended Search Calculus (NQD-CAL)/00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]], [[C_Kernel Extension Specifications/07_19_Explore-Exploit Governor (E-E-LOG)/00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]], [[C_Kernel Extension Specifications/11_23_MethodFamily Evidence & Maturity (Method-SoS-LOG)/00_C.23 - MethodFamily Evidence & Maturity (Method-SoS-LOG)|C.23]]}`
* **⊑/⊑⁺:** `∅`
* **RequiredPins/EditionPins/PolicyPins (minimum):**
...
## [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:5 - Archetypal Grounding — Tell–Show–Show; System / Episteme

### [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:5.1 - Archetype 1: System comparability under mixed evidence and unit constraints
...
* declares explicit failure behavior wiring (tri-state semantics delegated to [[00_G.Core - Part G Core Invariants|G.Core]]),
...
### [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:5.2 - Archetype 2: Epistemic comparability for selected-set publication across traditions
...
* makes `AcceptanceStubs` explicit as templates while locating thresholds in CAL ([[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]),
* ensures RSCR triggers are emitted when comparator or policy pins change.

## [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:6 - Bias-Annotation
...
## [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:7 - Conformance Checklist (normative)

| ConformanceId | Statement |
| --- | --- |
| **CC‑G0‑CoreRef** | [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec]] is conformant only if the applicable core obligations listed in `G.0:4.1` are satisfied (delegation to `CC‑GCORE‑*`; no shadow specs, no competing defaults, typed RSCR triggers, explicit pins). |
...
| CC‑G0‑07 | If crossing penalties are used, `CL‑Routing` and `Φ` policy ids are explicit and auditable (policy ids are exposed as pins/refs) **and are required pins for downstream SCR publication on penalised claims** (see [[00_G.06 - Evidence Graph & Provenance Ledger]]). |
| CC‑G0‑08 | `AcceptanceStubs` in `CG‑Spec` are templates only; any context-local thresholds/acceptance policies are governed by CAL acceptance artefacts ([[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring]]) and are cited, not duplicated. |
...
| **CC‑G0‑CSLC** | Scale/unit/polarity legality MUST be proven before any aggregation; illegal arithmetic on ordinal/nominal values is nonconformant. (Governed by the relevant legality patterns; [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec]] only binds and cites.) |

## [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:8 - Common Anti-Patterns and How to Avoid Them
...
## [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:9 - Consequences
...
## [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:10 - Rationale

`CG‑Spec` centralises frame-level comparability constraints so that:

* CHR authorship ([[00_G.03 - CHR Authoring for a CG-Frame- Characteristics, Scales, Levels, Coordinates|G.3]]) remains about *measurement meaning* rather than implicit thresholds,
* CAL ([[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]) governs context-local acceptance/threshold policies and proof ledgers,
* selectors and dispatchers ([[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]) remain policy-governed and auditable rather than encoding hidden legality assumptions,
* refresh ([[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]) can treat legality edits and pin changes as explicit causes with canonical trigger ids.

## [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:11 - SoTA‑Echoing
...
## [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:12 - Relations

**Builds on:** [[00_G.Core - Part G Core Invariants|G.Core]], `A.19 (CN‑Spec)`, `A.10 (evidence carriers)`, `A.17–[[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]] / [[C_Kernel Extension Specifications/04_16_Measurement & Metrics Characterization (MM-CHR)/00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]] (MM‑CHR legality)`, `A.18 (CSLC)`, `B.3 (trust / Γ‑fold family)`, `F.* (contexts, bridges, CL, UTS)`, `E.10 (lexical rules)`, `E.5.* (notation independence discipline)`.
**Used by:** [[00_G.01 - CG-Frame-Ready Generator|G.1]] (generator guards), [[00_G.02 - SoTA Harvester & Synthesis|G.2]] (harvesting constraints), [[00_G.03 - CHR Authoring for a CG-Frame- Characteristics, Scales, Levels, Coordinates|G.3]] (required CHR), [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] (acceptance templates / proof hooks), [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] (eligibility gates), [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]] (evidence/pin surfaces), and downstream parity/shipping/refresh where `CG‑Spec` is pinned.
**Publishes to:** `UTS` (Name Cards + editioned `CG‑Spec` segments).

## [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]:End
