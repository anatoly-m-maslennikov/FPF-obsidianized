...
**Primary hooks:** **[[00_G.Core - Part G Core Invariants|G.Core]]** (core invariants, linkage catalogues, RSCR trigger catalogue, Default Governing Definition Index), **[[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]]** (DHC slots + `DHCPack` / `DHCMethodSpec` / `DHCSeries` artefacts), **[[00_G.06 - Evidence Graph & Provenance Ledger|G.6]]** (EvidenceGraph; `PathId`/`PathSliceId` citation), **[[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]** (Bridge calibration / CL & `Φ/Ψ/Φ_plane` policy surfaces; when crossings/plane routing is used), **[[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]** (telemetry‑driven refresh/decay orchestration), **[[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]** (selector set-result / set‑returning outputs, when dashboard consumes performance trade‑offs), **[[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]]** (CN‑Spec governance card), **[[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]** (CG‑Spec legality gate), **F.17/F.18** (UTS + twin labels), **[[02_E.05.02 - Notational Independence|E.5.2]]** (notation independence), **[[00_E.10 - Unified Lexical Rules for FPF|E.10]]** (LEX discipline).
*(Optional, extension‑gated hooks:* **[[00_G.02 - SoTA Harvester & Synthesis|G.2]]** (SoTA palette & DHC alignment hooks), **[[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]] and [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]]** (QD / E‑E / OEE telemetry pins), **[[00_G.08 - SoS-LOG Bundles & Maturity Ladders|G.8]]** (SoS‑LOG bundle & maturity ladder view), **[[00_G.10 - SoTA Pack Shipping|G.10]]** (shipping inclusion of dashboard slices).)*

**Why this exists.** **[[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]]** defines *what* lawful “discipline health” slots are (CHR‑typed; scale/legality aware; freshness‑windowed), but it does not, by itself, provide a **generation‑first** method for producing **edition‑pinned, evidence‑citable DHC time series** that remain refreshable under RSCR.
**[[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]** is that dashboard method: it defines the **dashboard kit surfaces** (`DHCSeries@Context`, `DHCRow@Context`, `DashboardSlice@Context`, telemetry pins) and a pipeline for computing and publishing DHC readings **without shadow specs**, **without illicit arithmetic**, and **without smuggling scalar winners** out of partial orders or telemetry.

**Modularity note.** [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]] governs **dashboard publication units and wiring** only. It **does not** govern CN-Spec, CG-Spec, CHR, CAL, selection semantics, evidence semantics, shipping, or refresh heuristics. It binds to those governing definitions via refs/pins/editions/policy‑ids and keeps any method‑/generator‑specific panels strictly inside **Extensions** (`GPatternExtension` blocks).

## [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]:1 — Intent

Produce **admissible, reproducible, refresh-aware discipline-health dashboards** by turning **[[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]]** DHC definitions into:
...
3. **telemetry pins** that allow **[[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]** to plan **slice‑scoped refresh** (rather than “rerun everything”).

## [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]:2 — Problem frame
...
We need a **dashboard kit** that makes the *method of obtaining dashboard values* explicit and auditable, while keeping universal invariants governed in **[[00_G.Core - Part G Core Invariants|G.Core]]**.

## [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]:3 — Forces

* **Legality and comparability are governed by CN-Spec and CG-Spec.** Dashboards must not invent local legality/acceptance/normalization “mini‑specs”; they pin and cite **CN‑Spec** and **CG‑Spec** surfaces as required by **[[00_G.Core - Part G Core Invariants|G.Core]]** conformance.
...
## [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]:4 — Solution — Compute and publish DHC series admissibly, with RSCR-ready telemetry

### [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]:4.0 — [[00_G.Core - Part G Core Invariants|G.Core]] linkage (normative)

This pattern is **core‑invariant‑bearing** and therefore binds to **[[00_G.Core - Part G Core Invariants|G.Core]]** by declaration (not by restating invariants here).

**GCoreLinkageManifest ([[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]])** *(normative; expands per `G.Core:4.2`)*
...
* `CorePinsRequired` *(pattern delta; pin names only; all are id‑valued unless noted)* := {
  ...
  `DHCSlotId[]` *(typed DHC slots governed by [[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]]; each resolves to `CharacteristicId` + scale/unit/polarity + reference plane binding + lane discipline)*,
...
### [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]:4.1 — Objects (LEX heads; twin‑register discipline)

All objects below are **notation‑independent**; serialisations (if any) live under shipping or interop governance, not here.

**(1) `DHCSeries@Context`** *(UTS‑published dashboard series; [[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]]‑grounded)*
A time‑indexed publication of computed DHC readings for a `Discipline × ContextSlice`, aligned with `U.DHCSeries` semantics from **[[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]]** and pinned to method/governing spec refs.
...
A conceptual telemetry pin emitted to refresh/orchestration (governed by **[[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]**) with canonical trigger kind ids.
...
**Ref discipline.** `.edition` SHALL appear only on `…Ref` (per **[[00_E.10 - Unified Lexical Rules for FPF|E.10]]**). Dashboard artefacts that mint public ids publish **Tech/Plain twins** (UTS discipline).

### [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]:4.2 — Method‑of‑Obtaining Output (generation‑first; design‑time → run‑time)

**Stage A — Author & bind (design‑time)**

A1. **Select the DHC slot set (governed by [[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]]).**
Choose `DHCSlotId[]` from **[[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]]** (typed DHC slots), and declare the series scope explicitly as `TargetSlice` (USM tuple) plus an explicit time selector (`Γ_time` per row; optionally a `WindowSpec` that generates the row windows). Do not restate slot semantics in the dashboard kit; cite the [[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]] governing definitions.

A2. **Bind governance card and legality gate (governing definitions: [[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]], [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]).**
...
If the dashboard series/slice actually uses cross‑Context or cross‑plane routing, it MUST additionally pin the relevant crossing and penalty‑policy surfaces as ids (Bridge/CL/plane ids, `Φ/Ψ/Φ_plane` policy‑ids, `PlaneMapRef.edition?`) and cite their governing patterns (typically [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] for bridge calibration/CL kits, pinned through [[00_G.Core - Part G Core Invariants|G.Core]]). The dashboard MUST NOT encode a dashboard‑local “penalty regime”.

A3. **Pin computation methods (governed by [[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]]).**
For each slot/method used to compute a time series value, record `DHCMethodSpecRef.edition` and `DHCMethodRef.edition` (table‑backed, per [[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]]). The dashboard series is edition‑aware: if a method spec changes, the dashboard either forks the series edition or emits telemetry and refreshes under explicit pins.
...
B1. **Resolve evidence by Path (governed by [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]]).**
Compute rows from evidence cited as `PathSliceId[]` (and `PathId[]` when needed), under the declared window/freshness regime. Preserve lane discipline and handle missingness using tri‑state stances governed by **[[00_G.Core - Part G Core Invariants|G.Core]]**.

B2. **Compute slot values using pinned methods (governed by [[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]]).**
...
Any cross-Context or cross-plane use is expressed only via explicit crossing pins (Bridge/Plane routing) and policy ids governed by **[[00_G.Core - Part G Core Invariants|G.Core]]**.

B3. **Emit RSCR‑actionable telemetry pins (governing definition: [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]).**
...
Dashboards emit pins; refresh orchestration remains governed by **[[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]**.

### [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]:4.9 — Extensions (pattern‑scoped; non‑core)
...
**GoverningPatternId:** [[00_G.02 - SoTA Harvester & Synthesis|G.2]] *(SoTA palette + DHC alignment hooks semantics are governed by [[00_G.02 - SoTA Harvester & Synthesis|G.2]]; [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]] only wires them)*
...
**GoverningPatternId:** [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] *(`PortfolioMode` citation plus selected-set semantics and set‑return discipline)*
**Uses:** `{[[G_Discipline SoTA Patterns Kit/06_05_Multi-Method Dispatcher & MethodFamily Registry/00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]], [[G_Discipline SoTA Patterns Kit/07_06_Evidence Graph & Provenance Ledger/00_G.06 - Evidence Graph & Provenance Ledger|G.6]]}`
**⊑/⊑⁺:** `∅`
...
**GPatternExtensionKind:** `MethodSpecific`
**GoverningPatternId:** [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]] *(QD / NQD‑CAL semantics; descriptor/distance/insertion policy)*
...
* Illumination/coverage signals are treated as telemetry. Any promotion of telemetry into selection dominance is governed elsewhere (typically CAL policy; pinned through [[00_G.Core - Part G Core Invariants|G.Core]]).
...
**GoverningPatternId:** [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]] *(E/E‑LOG & exploration accounting; generator/transfer telemetry wiring)*
**Uses:** `{[[C_Kernel Extension Specifications/07_19_Explore-Exploit Governor (E-E-LOG)/00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]], [[G_Discipline SoTA Patterns Kit/06_05_Multi-Method Dispatcher & MethodFamily Registry/00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]], [[G_Discipline SoTA Patterns Kit/12_11_Telemetry-Driven Refresh & Decay Orchestrator/00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]}`
**⊑/⊑⁺:** `∅`
...
**GoverningPatternId:** [[00_G.08 - SoS-LOG Bundles & Maturity Ladders|G.8]] *(maturity ladder semantics in SoS‑LOG bundle/maturity cards)*
**Uses:** `{[[G_Discipline SoTA Patterns Kit/09_08_SoS-LOG Bundles & Maturity Ladders/00_G.08 - SoS-LOG Bundles & Maturity Ladders|G.8]], [[G_Discipline SoTA Patterns Kit/07_06_Evidence Graph & Provenance Ledger/00_G.06 - Evidence Graph & Provenance Ledger|G.6]], [[G_Discipline SoTA Patterns Kit/12_11_Telemetry-Driven Refresh & Decay Orchestrator/00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]}`
**⊑/⊑⁺:** `∅`
...
**GPatternExtensionKind:** `InteropSpecific`
**GoverningPatternId:** [[00_G.10 - SoTA Pack Shipping|G.10]] *(shipping semantics are governed by [[00_G.10 - SoTA Pack Shipping|G.10]])*
...
* `CNSpecRef.edition`, `CGSpecRef.edition` *(as shipped pins, per [[00_G.10 - SoTA Pack Shipping|G.10]] wiring)*
...
## [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]:5 — Interfaces (conceptual; kit surface)

| ID  | Interface   | Consumes   | Produces  |
| --- | ----------- | ---------- | --------- |
| **[[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)]]‑1 `Create_DHCSeries`** | Create/bind a DHC series scope ([[00_C.21 - Field Health & Structure (Discipline-CHR)]]‑grounded; edition‑aware) | `DHCSlotId[]`, `DHCPackRef.edition?`, `DHCMethodSpecRef.edition`, `TargetSlice` (USM), `WindowSpec?`, `ReferencePlane`, `CNSpecRef.edition`, `CGSpecRef.edition` | `DHCSeries@Context` (UTS publication unit; edition‑aware) |
| **[[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)]]‑2 `Update_DHCSeries`** | Compute/update one or more rows under pinned conditions (run‑time; Work/Audit‑citable) | `PathSliceId[]`, `EvidenceGraphId?`, `DHCMethodRef.edition`, `DHCMethodSpecRef.edition`, `Γ_time`, crossing pins (if any) | `DHCRow@Context[]` (UTS publication units; stance and pins; `DesignRunTag = run`) |
| **[[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)]]‑3 `Integrate_PortfolioTelemetry`** *(extension‑gated)* | Integrate selector set-result evidence into a slice/series | See `G.12:Ext.PortfolioTelemetry` | Extension‑gated fields / telemetry pins |
| **[[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)]]‑4 `Integrate_QDTelemetry`** *(extension‑gated)* | Integrate QD illumination/archive telemetry | See `G.12:Ext.QDTelemetry` | Extension‑gated fields / telemetry pins |
| **[[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)]]‑5 `Integrate_OEETelemetry`** *(extension‑gated)* | Integrate open‑endedness / transfer telemetry | See `G.12:Ext.OpenEndedTelemetry` | Extension‑gated fields / telemetry pins |
| **[[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)]]‑6 `Publish_DashboardSlice`** | Publish a view slice as a projection over computed rows | `DHCSeriesId(UTS)[]`, `DHCRowId(UTS)[]?`, `SliceAnnotations?` | `DashboardSlice@Context` (UTS publication unit; view‑only) |
| **[[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)]]‑7 `Emit_TelemetryPins`** | Emit RSCR‑actionable telemetry pins for refresh | `RSCRTriggerKindId`, `scope`, `payloadPins` | `DHCTelemetryPin[]` (consumed by [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator]]) |

(*No file formats are introduced here; serialisation recipes live under shipping or interop governance.*)

## [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]:6 — Conformance checklist (CC‑G12, normative)

| CC ID   | Requirement  | Verification notes  |
| ------- | ------------ | ------------------- |
| **CC‑G12‑CoreRef** | The pattern satisfies the **effective** [[00_G.Core - Part G Core Invariants]] obligations declared by `GCoreLinkageManifest ([[G_Discipline SoTA Patterns Kit/13_12_DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)/00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)]])` (profiles/sets/deltas expanded per `G.Core:4.2`).    | Evidence: the manifest is present; required pins/defaults/triggers are accounted for; no local restatement overrides core governing definitions.  |
| **CC‑G12.1** | **DHC slot typing ([[00_C.21 - Field Health & Structure (Discipline-CHR)]]‑grounded).** Every published dashboard value is indexed by a **[[00_C.21 - Field Health & Structure (Discipline-CHR)]]‑authored** `DHCSlotId` (typed DHC slot: `CharacteristicId` + scale/unit/polarity + reference plane binding + lane discipline) and is scoped by an explicit `TargetSlice` + `Γ_time`. | Evidence: row/series references `DHCSlotId` and pins `ReferencePlane` and `Γ_time` (or a series `WindowSpec` that yields row Γ_time). |
...
## [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]:7 — Bias‑Annotation (informative)

* **Didactic:** dashboard publication units publish pins and paths first; views second.
* **Architectural:** no “dashboard‑local governing spec refs”; invariant citation is via [[00_G.Core - Part G Core Invariants|G.Core]].
...
## [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]:8 — Consequences
...
* **Selective maintenance becomes possible.** Telemetry pins let [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] refresh what changed (path slice / window / method edition), rather than rerunning the entire pipeline.
...
## [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]:9 — Relations

**Builds on:** [[00_G.Core - Part G Core Invariants|G.Core]], [[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]], [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]], [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]], [[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]], [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]], `F.17/F.18`, [[02_E.05.02 - Notational Independence|E.5.2]], [[00_E.10 - Unified Lexical Rules for FPF|E.10]].
**Coordinates with:** [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] *(when selector set-result outputs are consumed)*, [[00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] *(when crossings/plane routing or `CL/Φ/Ψ/Φ_plane` policy pins are used)*, [[00_G.08 - SoS-LOG Bundles & Maturity Ladders|G.8]] *(when maturity ladder view is included)*, [[00_G.10 - SoTA Pack Shipping|G.10]] *(when dashboard slices are shipped)*.
...
## [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]:10 — Author’s quick checklist
...
2. Select `DHCSlotId[]` and cite **[[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]]** (do not restate slot semantics).
...
6. Emit canonical telemetry pins (`RSCRTriggerKindId` + scope + payload pins) for [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]].
...
## [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]:11 — Worked micro‑examples (informative; SoTA‑oriented)

**(A) Decision‑making discipline dashboard (multi‑tradition).**
Slots (from **[[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]]**): *ReproducibilityRate* (freshness‑windowed), *StandardisationLevel* (ordinal), *AlignmentDensity* (bridge density over DHC‑SenseCells), *MetaDiversity* (operator family diversity), *DisruptionBalance* (target‑band metric).
...
## [[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]:End
