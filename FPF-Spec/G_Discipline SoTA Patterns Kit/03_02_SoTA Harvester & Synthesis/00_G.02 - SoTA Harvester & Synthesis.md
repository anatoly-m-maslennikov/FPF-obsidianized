...
> * CHR authoring ([[00_G.03 - CHR Authoring for a CG-Frame- Characteristics, Scales, Levels, Coordinates|G.3]]),
> * CAL authoring ([[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]),
> * method/generator registries and dispatch ([[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]).
>
> **Scope note.** This pattern **governs** the harvesting + synthesis *generator* in Part G. Shipping governing-definition assignment is in **[[00_G.10 - SoTA Pack Shipping|G.10]]**, refresh orchestration governing-definition assignment is in **[[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]**.
>
...
## [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:1 - Problem frame
...
## [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:2 - Problem
...
## [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:3 - Forces
...
## [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:4 - Solution

### [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:4.1 - [[00_G.Core - Part G Core Invariants|G.Core]] linkage (normative)
...
*(Canonical form, Nil‑elision, and Expansion rule are defined in [[00_G.Core - Part G Core Invariants|G.Core]].)*
...
`GCoreLinkageManifest := ⟨
  ...
  CorePinsRequired := {
    // Scope pins ([[00_G.02 - SoTA Harvester & Synthesis|G.2]]‑specific)
    CG-FrameContext,
    Tradition[],
...
### [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:4.2 - Kit: `SoTA Synthesis Pack@CG‑Frame` (surface governed by this pattern)

A conforming [[00_G.02 - SoTA Harvester & Synthesis|G.2]] publication produces a **notation‑independent pack** whose internal organisation is free, but whose exported **named components and views** are stable and citable:
...
* explicit evidence anchors/citations ([[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] and/or EvidenceGraph refs when available),
* explicit freshness window notes and risk/trust cues *(cite [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] governing definitions when using trust/decay language)*.
...
5. **`G.2e MicroExamples`**
   Worked micro‑examples for load‑bearing claims, each citing [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] carriers, declaring context + `entityOfConcern`, and annotating assurance type(s) (`TA`/`VA`/`LA`, where applicable).
...
10. **`G.2j MethodFamilyCards`**
    ...
    When the pack targets downstream registry/dispatch, MethodFamily cards **SHOULD** include the declared refs and pins [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] needs (eligibility predicate refs, assurance profile cues, and the pack ids that justify the family).
...
### [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:4.3 - Harvester loop (conceptual choreography; pattern-governed)

A conforming [[00_G.02 - SoTA Harvester & Synthesis|G.2]] pack publication is built by iterating the following conceptual loop until the declared gates are satisfied:
...
6. **(Alias: [[00_G.02 - SoTA Harvester & Synthesis|G.2]]‑F) Produce Γ_epist synthesis records when fusion/substitution is asserted.**
   If a [[00_G.02 - SoTA Harvester & Synthesis|G.2]] pack publication asserts fusion or substitution across sources or across `Tradition` records (beyond mere “parallel divergent claims”), it **MUST** emit `GammaEpistSynthId` records per `G.2:Ext.GammaEpistSynthesis` (provenance union + explicit object alignment refs + assurance tuple refs), and it **MUST** keep penalties routed to `R_eff` only by delegation (`CC‑GCORE‑PEN‑1`).

7. **Publish teachable micro‑groundings.**
   Attach worked micro‑examples to load‑bearing claims, each tied to [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] carriers and declaring context + `entityOfConcern`.
...
* [[00_G.03 - CHR Authoring for a CG-Frame- Characteristics, Scales, Levels, Coordinates|G.3]] (CHR authoring),
* [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] (CAL authoring),
* [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] (registry/dispatch),
...
### [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:4.4 - Interfaces (minimal I/O Standard)

| Interface         | Consumes                                                      | Produces                                                                    |
| ----------------- | ------------------------------------------------------------- | --------------------------------------------------------------------------- |
| **[[00_G.02 - SoTA Harvester & Synthesis]]‑1 Harvest** | `CG-FrameContext`, initial `Tradition[]`, `HarvestPolicyRef?`  | `SoTA Synthesis Pack@CG‑Frame` (G.2a–G.2l)                                  |
| **[[00_G.02 - SoTA Harvester & Synthesis]]‑2 Extend**  | existing Pack + new sources/anchors + updated policy pins     | updated Pack + RSCR‑relevant trigger emissions (canonical kinds)            |
| **[[00_G.02 - SoTA Harvester & Synthesis]]‑3 HandOff** | Pack                                                          | `CHR‑handoff` (to [[00_G.03 - CHR Authoring for a CG-Frame- Characteristics, Scales, Levels, Coordinates]]), `CAL‑handoff` (to [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring]]), `Registry‑handoff` (to [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]]) |

*Note:* Orchestration of re‑runs is governed by [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]; this pattern only defines what a conforming (re)harvest produces and what pins it must expose.

### [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:4.5 - Extensions (pattern‑scoped; non‑core)
...
##### [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:4.5.1 - GPatternExtension: GammaEpistSynthesis
...
**Uses:** `{[[G_Discipline SoTA Patterns Kit/00_Core_Part G Core Invariants/00_G.Core - Part G Core Invariants|G.Core]], [[B_Trans-disciplinary Reasoning Cluster/02_03_Trust and Assurance Calculus (F-G-R with Congruence)/00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], [[F_The Unification Suite (U-Suite)- Concept-Sets, SenseCells & Contextual Role Assignment/09_Alignment & Bridge across Contexts/00_F.09 - Alignment & Bridge across Contexts|F.9]], [[G_Discipline SoTA Patterns Kit/07_06_Evidence Graph & Provenance Ledger/00_G.06 - Evidence Graph & Provenance Ledger|G.6]]}` *(penalty routing + trust/decay cues + bridges/CL + evidence path citation when used)*
**⊑/⊑⁺:** `∅`
**RequiredPins/EditionPins/PolicyPins (minimum):**

* `GammaEpistSynthId[]` *(pack‑local ids of synthesis records; emitted iff fusion/substitution is asserted)*
* `EvidenceAnchorRef[]` *(provenance union; [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] carriers)*
...
* `PathId/PathSliceId?` *(only when citing via [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]])*
...
* This extension **does not** redefine `Γ‑fold`, `Φ`, or penalty semantics; it only requires the pins/refs needed for replayability and auditability (see [[00_G.Core - Part G Core Invariants|G.Core]] delegations).

##### [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:4.5.2 - GPatternExtension: HarvestProtocols
...
##### [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:4.5.3 - GPatternExtension: DHCAlignmentHooks

**PatternScopeId:** `G.2:Ext.DHCAlignmentHooks`
**GPatternExtensionId:** `DHCAlignmentHooks`
**GPatternExtensionKind:** `DisciplineSpecific`
**GoverningPatternId:** [[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]] *(DHC semantics are governed by [[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]])*
**Uses:** `{[[C_Kernel Extension Specifications/09_21_Field Health & Structure (Discipline-CHR)/00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]], [[G_Discipline SoTA Patterns Kit/07_06_Evidence Graph & Provenance Ledger/00_G.06 - Evidence Graph & Provenance Ledger|G.6]], [[G_Discipline SoTA Patterns Kit/08_07_Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)/00_G.07 - Cross-Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]}` *(DHC series + evidence path citations + bridge/CL regimes when alignment density is claimed)*
**⊑/⊑⁺:** `∅`
**RequiredPins/EditionPins/PolicyPins (minimum):**
...
* `PathId[]` / `PathSliceId[]` *(when alignment summaries cite evidence paths via [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]])*
...
* Units/constraints (governing pattern: [[00_C.21 - Field Health & Structure (Discipline-CHR)|C.21]]) must be **pinned, not redefined** here (e.g., `bridges_per_100_DHC_SenseCells`, `CL_min = 2` for cross‑Context counting, and the “CL=3 implies free substitution” interpretation when used).

##### [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:4.5.4 - GPatternExtension: NQDAnnex

**PatternScopeId:** `G.2:Ext.NQDAnnex`
**GPatternExtensionId:** `NQDAnnex`
**GPatternExtensionKind:** `MethodSpecific`
**GoverningPatternId:** [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]] *(NQD-CAL semantics are governed by [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]]; explore/exploit logging is governed by [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]] when used)*
...
##### [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:4.5.5 - GPatternExtension: InteropForms
...
### [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:4.6 - Palette first
...
### [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:4.7 - Atlas views stay optional neighboring interpretation over one declared palette and declared set results
...
- `TraditionAtlasView` is the [[00_G.02 - SoTA Harvester & Synthesis|G.2]] use-site specialization of `DeclaredSubstrateAtlasView`; keep the generic interpretive-view declaration in [[04_A.19.DECLARED-SUBSTRATE-INTERPRETIVE-VIEW - Declared-Substrate Interpretive View|A.19.DECLARED-SUBSTRATE-INTERPRETIVE-VIEW]].
...
- If one atlas reading would materially change the base source-to-outcome relation or distortion posture, reopen the substrate declaration instead of treating that change as one local [[00_G.02 - SoTA Harvester & Synthesis|G.2]] convenience.
...
- If the question is naming-side only, use [[00_F.18 - Local-First Unification Naming Protocol|F.18]] for that wording choice rather than letting atlas-form interpretation language carry the naming decision by itself.

## [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:5 - Archetypal Grounding (System / Episteme)
...
| **Show (repair)**  | A conformant [[00_G.02 - SoTA Harvester & Synthesis]] pack keeps parallel Claim Sheets per `Tradition`, publishes explicit alignment/loss notes where reuse is attempted, and emits hand‑offs so CHR/CAL/selection can be authored without re‑inventing semantics.                                          | A conformant [[00_G.02 - SoTA Harvester & Synthesis]] pack preserves plural claims, publishes explicit bridge‑backed alignment where justified, represents indicators as families/variants, and makes evidence anchors and freshness windows visible so downstream re‑audits are possible. |

## [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:6 - Bias-Annotation (informative)
...
* **Didactic bias (Did).** Micro‑examples can steer interpretation toward familiar domains.
  *Mitigation:* require heterogeneous substrates and explicit [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] anchors.

## [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:7 - Conformance Checklist (normative) — **CC‑G2**

| ConformanceId             | Requirement                                                                                                                                                                                                                                                                                                                                        | Purpose / Notes                                                                     |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| **CC‑G2‑CoreRef**         | A conforming [[00_G.02 - SoTA Harvester & Synthesis]] artefact **MUST** satisfy the **effective** core obligations declared by the `GCoreLinkageManifest` in `G.2:4.1` (per [[00_G.Core - Part G Core Invariants]] Expansion rule).                                                                                                                                                                                 | Phase‑2 bridge clause: ensures universal invariants are not redefined inside [[00_G.02 - SoTA Harvester & Synthesis]]. |
...
| **CC‑G2‑MicroExamples‑1** | For every load‑bearing claim family, a conforming pack **MUST** include **at least two** worked micro‑examples on **heterogeneous substrates**, each with explicit [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] carrier anchors, declared context + `entityOfConcern`, and an assurance tag (`TA`/`VA`/`LA`, where applicable).                                                          | Makes the synthesis teachable and anchor‑grounded.                                  |
...
| **CC‑G2‑HandOff‑1**       | A conforming pack **MUST** emit hand‑off manifests to [[00_G.03 - CHR Authoring for a CG-Frame- Characteristics, Scales, Levels, Coordinates]], [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring]], and [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]] that cite pack components by id and identify which families/operators are intended for downstream formalisation or registry entry.                                                                                                                                   | Prevents downstream re‑authoring and drift.                                         |
...
| **CC‑G2‑Ext‑DHC‑1**       | If `G.2:Ext.DHCAlignmentHooks` is used, the pack **SHALL** (a) expose the required pins listed in that extension, including `DHCSenseCellId[]`, and (b) declare the unit/constraint pins required by [[00_C.21 - Field Health & Structure (Discipline-CHR)]] (e.g., `bridges_per_100_DHC_SenseCells`, `CL_min=2`) without redefining their semantics locally (governing pattern: [[00_C.21 - Field Health & Structure (Discipline-CHR)]]).                                                             | Keeps DHC extension pins auditable and non‑shadowing.                              |
...
## [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:8 - Common Anti‑Patterns and How to Avoid Them
...
* **AP‑G2‑4: Unanchored pedagogy.**
  ...
  **Do instead:** bind micro‑examples to [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] anchors and declare `entityOfConcern`.
...
## [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:9 - Consequences
...
## [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:10 - Rationale

SoTA synthesis is a bottleneck for new `CG‑Frame` work: without a disciplined harvest, downstream formalization (CHR/CAL) and operational selection ([[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]) either (i) inherit hidden semantic collisions, or (ii) re‑invent incompatible “mini‑standards.”
[[00_G.02 - SoTA Harvester & Synthesis|G.2]] resolves this by treating SoTA work as a **publishable kit**: explicit plurality, explicit crossings, explicit evidence anchors, and explicit hand‑offs.

## [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:11 - SoTA-Echoing (informative)
...
## [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:12 - Relations

* **Builds on:**

  * [[00_G.Core - Part G Core Invariants|G.Core]] (core invariants, typed RSCR causes, Default Governing Definition Index)
  * [[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]] (pattern template discipline)
  * [[00_E.10 - Unified Lexical Rules for FPF|E.10]] (lexical/ontological rules; strict distinction; kind‑suffix discipline)
  * [[00_E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]] (conformance discipline)
  * [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] (provenance anchors / carriers)
  * [[04_A.19.DECLARED-SUBSTRATE-INTERPRETIVE-VIEW - Declared-Substrate Interpretive View|A.19.DECLARED-SUBSTRATE-INTERPRETIVE-VIEW]] (generic interpretive-view and atlas discipline when `TraditionAtlasView` is used)
  * [[09_A.06.P - Relational Precision Restoration (RPR) - Kind-Explicit Qualified Relation Discipline|A.6.P]] (space/view/publication precision restoration when palette/support claims collapse)
  * [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (trust, freshness/decay as cited governing patterns)
  * [[00_F.09 - Alignment & Bridge across Contexts|F.9]] (bridges and CL as cited governing patterns)
  * [[00_F.17 - Unified Term Sheet (UTS)|F.17]] (UTS publication discipline; via delegation)
  * [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]] (CG‑Spec legality gate; cited when legality surfaces are referenced)
  * [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]] (EvidenceGraph / path citation surfaces when used)

* **Used by:**

  * [[00_G.01 - CG-Frame-Ready Generator|G.1]] (generator chassis consumes harvested SoTA sets)
  * [[00_G.03 - CHR Authoring for a CG-Frame- Characteristics, Scales, Levels, Coordinates|G.3]] (CHR authoring consumes operator/object inventory and claim sheets)
  * [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] (CAL authoring consumes operator stubs, acceptance branch scaffolding)
  * [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] (registry/dispatch consumes MethodFamily/GeneratorFamily cards)
  * [[00_G.10 - SoTA Pack Shipping|G.10]] (shipping cites the pack as payload)
  * [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] (refresh orchestration can re‑invoke harvest via typed causes)

* **Relates to:**

  * [[00_G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]] (interop surfaces when external indices are used)
  * [[00_F.18 - Local-First Unification Naming Protocol|F.18]] (naming-side support wording when the question is label choice rather than synthesis geometry)

## [[00_G.02 - SoTA Harvester & Synthesis|G.2]]:End
