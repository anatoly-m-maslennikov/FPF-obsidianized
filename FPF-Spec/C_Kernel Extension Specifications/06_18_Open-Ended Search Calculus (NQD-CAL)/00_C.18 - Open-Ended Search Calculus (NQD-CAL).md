...
**Depends on.** A‑kernel ([[00_A.01 - U.Holon, U.System, and U.Episteme|A.1]]–[[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]), **MM‑CHR** ([[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]]) for measurements, **KD‑CAL** for similarity/corpora, **Sys‑CAL** for carriers, **Decsn‑CAL** (objectives; advisory), **Compose‑CAL** (set aggregation; advisory).

**Coordinates with.** **[[02_B.05.02.01 - Creative Abduction with NQD|B.5.2.1]]** (binding), **[[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]] Creativity‑CHR** (characteristics & scales), **[[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]] E/E‑LOG** (policies: emitter selection, explore/exploit).
...
## [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]]:1 - Problem frame
Open‑ended search (NQD) equips FPF with illumination‑style generation and Pareto / selected-set selection in multi‑criteria, partially ordered spaces; it feeds [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] without scalarising ordinal or mixed‑scale characteristics.

## [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]]:2 - Problem
...
## [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]]:3 - Forces
...
## [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]]:4 - Solution
...
**Type notes.**
- `U.DescriptorMap (Tech; twin‑labelled Plain) : Hypothesis → ℝ^d` (declares encoder, invariances, version, **CharacteristicSpaceRef**). Publish Tech/Plain per **[[00_E.10 - Unified Lexical Rules for FPF|E.10]]**; declare `DescriptorMapRef.edition` and `DistanceDefRef.edition`. **Dimensionality rule.** **Require `d≥2` only when QD/illumination surfaces are active**; for non‑QD contexts `d≥1` is lawful.
...
- `U.EmitterPolicyRef` points to a named policy in **[[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]] E/E‑LOG**.
...
- `U.IlluminationSummary` is a **telemetry summary** over `Diversity_P` (see [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]), not a dominance characteristic.
...
## [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]]:5 - Conformance Checklist
...
- **C18‑2** Record `EmitterPolicyRef` (policy id from [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]]) and parameter set.
- **C18‑3** Compute `D = ΔDiversity_P(h | Pool)` under the same DescriptorMap & TimeWindow as the Pool (see [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]).
- **C18‑4** Exclude Illumination from dominance unless policy explicitly promotes it.
- **C18‑5** Keep `Use‑Value` separate from assurance scores; do not alter `F/G/R` semantics (see [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]] §Use‑Value).
...
**Defaults.** Default-governance responsibility is split on purpose: [[00_G.Core - Part G Core Invariants|G.Core]] and [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] govern `DefaultId.DominanceRegime` and selector-facing default routing, while [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]] governs emitter, insertion, and pool-policy defaults. [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]] consumes those defaults and records the active refs instead of restating them locally. Minimum provenance remains: `DescriptorMapRef.edition` and `DistanceDefRef.edition`, `DHCMethodRef.edition`, `EmitterPolicyRef`, `InsertionPolicyRef`, `TimeWindow`, `Seeds`, `DedupThreshold?`; also record `FamilyCoverage/MinInterFamilyDistance`.
...
## [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]]:6 - Archetypal Grounding
...
## [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]]:7 - Bias‑Annotation
Lexical firewall and notation independence apply; no vendor/tool tokens; ordinal characteristics never averaged; illumination treated as report‑only telemetry unless a policy promotes it. ([[01_E.05.01 - DevOps Lexical Firewall|E.5.1]], [[02_E.05.02 - Notational Independence|E.5.2]], [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]])

## [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]]:8 - Consequences
...
## [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]]:9 - Rationale
...
## [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]]:10 - Relations
Builds on: [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]], [[00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]. Coordinates with: [[02_B.05.02.01 - Creative Abduction with NQD|B.5.2.1]] (binding), [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]], [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]], [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]], [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]], [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]].

## [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]]:End
