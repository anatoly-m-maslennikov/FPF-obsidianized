...
**Placement.** Part C (Kinds), identifier **[[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]** (Annex). Audience: engineering managers, editors, reviewers, assurance leads.

**Depends on.**
— **[[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] USM**: `U.ContextSlice`, `U.ClaimScope` (**G**), `U.WorkScope`, ∈/∩/**SpanUnion**/translate, **Γ\_time** policy, Bridge + **CL** (scope).
— **[[01_C.03.01 - U.Kind & SubkindOf (Core)|C.3.1]]**: `U.Kind`, `U.SubkindOf (⊑)`; **[[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]**: `KindSignature` (with **F**) and `Extension/MemberOf`; **[[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]**: **KindBridge** + `CL^k` (kind‑congruence) & loss notes; **[[04_C.03.04 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]**: **RoleMask**.
— **[[05_C.03.05 - KindAT - Intentional Abstraction Facet for Kinds (K0…K3)|C.3.5]]**: **KindAT** (facet) — **explicitly forbidden** in guards.
— **[[03_C.02.02 - Reliability R in the F-G-R triad|C.2.2]] F–G–R**: weakest‑link, penalties to **R** only; **[[05_C.02.03 - Unified Formality Characteristic F|C.2.3]] F**: F0…F9 (expression rigor).
— **Part B Bridges & CL**: scope bridge semantics and CL ladder.

## [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:1 - Purpose & Audience
...
## [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:2 - Context & Problem
...
**[[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]** fixes this by supplying guard macros that:
...
## [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:3 - Solution Overview (what these guards do)
...
## [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:4 - Normative Guard Macros
...
### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:4.1 - **Guard\_TypedClaim** — admit a claim quantified over a kind
...
1. **ScopeCoverage.** `U.ClaimScope(C) covers TargetSlice`. *(USM [[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]])*
2. **Γ\_time declared.** TargetSlice **SHALL** specify **Γ\_time** (point/window/policy). No “latest”. *([[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]])*
3. **Kind definedness.** `MemberOf(?, k, TargetSlice)` is **defined and deterministic**. *([[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]] K‑05/K‑07)*
4. **Typed compatibility.**
   4.1 **same Context**: if C expects `k′`, require `k ⊑ k′`. *([[01_C.03.01 - U.Kind & SubkindOf (Core)|C.3.1]])*
   4.2 **Cross Context**: if Contexts differ, require a declared **KindBridge** that maps `k → k′` and publishes **`CL^k ≥ c`** with loss notes. *([[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]])*
5. **Assurance penalties (R only).** If step 4.2 used a KindBridge, the guard **SHALL** apply a monotone penalty **Ψ(`CL^k`)** to **R**. If a **Scope bridge** was used to move C’s Scope (USM), apply **Φ(CL)** to **R**. *([[03_C.02.02 - Reliability R in the F-G-R triad|C.2.2]] + [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]] + Part B)*
6. **Evidence freshness (if trust is implied).** Freshness windows and expiry checks **SHALL** be separate predicates (not Scope). *([[03_C.02.02 - Reliability R in the F-G-R triad|C.2.2]])*
7. **Formality threshold (if ESG mandates).** If the Context gates rigor, require `U.Formality(C) ≥ F_k`. *([[05_C.02.03 - Unified Formality Characteristic F|C.2.3]])*

**Prohibitions.**
— **AT forbidden.** KindAT **MUST NOT** appear in this guard. *([[05_C.03.05 - KindAT - Intentional Abstraction Facet for Kinds (K0…K3)|C.3.5]] AT‑01/02)*
— **No “domain” placeholders.** Guards **SHALL** name an addressable **TargetSlice**, not a fuzzy “domain”.

### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:4.2 - **Guard\_TypedJoin** — compose two typed claims/specs (A → B)
...
2. **ScopeSerial.** Compute `Scope_serial = ClaimScope(A) ∩ ClaimScope(B)`. Require `Scope_serial covers TargetSlice`. *([[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]])*
3. **Penalties (R only).** Apply **Ψ(`CL^k`)** if a KindBridge was used; apply **Φ(CL)** if a Scope bridge was used. *([[03_C.02.02 - Reliability R in the F-G-R triad|C.2.2]] / Part B / [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]])*
...
### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:4.3 - **Guard\_MaskedUse** — use a RoleMask with a kind

**Intent.** Use `U.Kind` **k** under a **RoleMask** **m** in Context **R**.

**Guard\_MaskedUse(k, m, TargetSlice)** — **SHALL** include:

1. **MaskRegistered.** `RoleMask(k, m, version)` is **registered and versioned**. *([[04_C.03.04 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]] RM‑06)*
...
5. **Cross‑context use.** If `TargetSlice.Context ≠ owner(k).Context`, require:
   ...
   5.3 Penalty **Ψ(`CL^k`)** to **R**. *(RM‑07 + [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]])*
6. **ScopeCoverage.** `U.ClaimScope(artifact) covers TargetSlice`. *([[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]])*

**Prohibitions.**
— **Mask ≠ Kind.** Guards **MUST NOT** treat the mask name as a synonym for the Kind. *(RM‑06)*

### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:4.4 - **Guard\_SpanUnion\_Typed** — publish parallel coverage across independent support lines
...
2. **Independence justification.** Publisher **SHALL** include a partition or certificate showing that essential components of `Lᵢ` are **disjoint** from `Lⱼ` (no shared weakest link). *([[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] §7.3)*
...
### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:4.5 - **Guard\_XContext\_Typed** — Cross‑context typed reuse (both bridges)
...
2. **Kind bridge.** There **exists** a KindBridge **b\_k** `(source = owner(k).Context, target = TargetSlice.Context)` with **`CL^k ≥ c_k`**. *([[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]])*
...
## [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:5 - Evaluation Semantics & Order (normative)
...
**E‑04 (No AT in guards).** AT is an editorial facet and **MUST NOT** be referenced. *([[05_C.03.05 - KindAT - Intentional Abstraction Facet for Kinds (K0…K3)|C.3.5]] AT‑01/02)*
...
## [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:6 - Conformance Checklist (normative)
...
### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:6.1 - What counts as “proven‑equivalent” (editorial rule)
...
### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:6.2 - Where penalties land (assurance calculus hook)
...
### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:6.3 - Minimal conceptual formulas (informative)
...
## [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:7 - Decision Trees (informative)
...
## [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:8 - Guard Anti‑patterns & Remedies (informative)
...
## [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:9 - Worked Examples (informative, brief)

> Detailed scenarios remain in **[[00_C.03 - Kinds, Intent-Extent, and Typed Reasoning (Kind-CAL)|C.3]] §11**. This Annex sketches how the macros apply; cross‑reference as needed.
...
## [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:10 - Rationale (why an Annex) (informative)
...
### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:Annex A - How typed reasoning plugs into **Compliance & Regulatory Alignment**  \[A/I]
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:[[00_A.01 - U.Holon, U.System, and U.Episteme|A.1]] Purpose & fit
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:[[00_A.02 - Role Taxonomy|A.2]] Normative obligations

**Conformance.** A model or authoring action conforms to [[00_A.02 - Role Taxonomy|A.2]] iff it satisfies **C‑REG‑1..C‑REG‑8** below.

**C‑REG‑1 (Regulatory kinds).** Regulatory categories **SHALL** be represented as `U.Kind` in the authority’s Context (e.g., `AdultPerson@RegY`, `MedicalDeviceClassII@FDA`, `PersonalData@GDPR`, `Lease@IFRS`). Each such kind **SHALL** have a `KindSignature` with a declared **F** level ([[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]).

**C‑REG‑2 (KindBridge).** Cross‑context reuse of a regulatory category **MUST** declare a **KindBridge** with a kind‑bridge congruence level (**CL^k**) and **loss notes** ([[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]). The mapping **SHALL** preserve the “is‑a / subkind‑of” direction and **MUST NOT** invert it.
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:[[00_A.03 - Transformer Constitution (Quartet)|A.3]] Guard macros (ready to use)
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:[[00_A.04 - Temporal Duality & Open-Ended Evolution Principle|A.4]] Worked examples  \[I]
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:[[00_A.05 - Open-Ended Kernel & Extension Layering|A.5]] Design guidance & pitfalls  \[I]
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:[[00_A.06 - Signature Stack & Boundary Discipline|A.6]] Migration checklist  \[I]
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:[[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] Manager’s one‑page pattern  \[I]
...
### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:Annex B - How typed reasoning plugs into **Assurance Lanes (VA/LA/TA) & Evidence design**  \[A/I]
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:[[00_B.01 - Universal Algebra of Aggregation (Γ)|B.1]] What you get with typed assurance  \[I]
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:[[00_B.02 - Meta-Holon Transition (MHT)- Recognizing Emergence and Re-identifying Wholes|B.2]] Normative obligations for evidence design
...
* **Kind movement** **SHALL** use a **KindBridge** (§ [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]) with **CL^k** and apply the **kind‑bridge penalty** to **R**.
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] Designing the **evidence matrix**  \[I]
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:[[00_B.04 - Canonical Evolution Loop|B.4]] VA lane — proofs that match the kind  \[A/I]
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:[[00_B.05 - Canonical Reasoning Cycle|B.5]] LA lane — tests & monitoring that cover the right variants  \[A/I]
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:B.6 TA lane — tool qualification where it belongs  \[A/I]
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:B.7 Guard macros for evidence planning & attachment
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:B.8 Anti‑patterns & remedies
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:B.9 End‑to‑end example (manager’s cheat‑sheet)  \[I]
...
### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:Annex C. ESG guards
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:C.1 **ESG** guard obligations (normative)
...
6. **Formality threshold (if gated).**
   If the ESG state requires rigor, enforce `U.Formality(Claim) ≥ F_k` ([[05_C.02.03 - Unified Formality Characteristic F|C.2.3]]).
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:[[00_C.02 - Epistemic holon composition (KD-CAL)|C.2]] - **Method–Work** guard obligations (normative)
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:[[00_C.03 - Kinds, Intent-Extent, and Typed Reasoning (Kind-CAL)|C.3]] - Guard macros (ready‑to‑use)
...
4      assert membership_defined(k, TargetSlice)               // [[C_Kernel Extension Specifications/01_03_Kinds, Intent-Extent, and Typed Reasoning (Kind-CAL)/02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]] K-07
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:C.4 - Worked examples (manager‑focused)
...
#### [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:C5 - Anti‑patterns & remedies (normative where noted)
...
## [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]:End
