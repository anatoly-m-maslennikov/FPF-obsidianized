...
> **One‑line summary.** Specifies the **intent and extent** of kinds: (**i**) a **`KindSignature(k)`** (the intensional definition of kind `k`) that **declares its own Formality F**; (**ii**) an **`Extension(k, slice) ⊆ U.EntitySet(slice)`** and the **membership predicate** `MemberOf(e, k, slice)` that are **deterministic per `U.ContextSlice`**; (**iii**) **monotonicity** of extension under `SubkindOf`; (**iv**) a **definedness policy** that fails **closed** outside its domain. **Kinds still carry no Scope** (that rule lives in [[01_C.03.01 - U.Kind & SubkindOf (Core)|C.3.1]]); Scope stays on **claims/capabilities** (USM). This pattern gives managers and reviewers the **observable basis** to check “what counts as a member here and now” without entangling applicability (G) or assurance (R).

**Status.** Normative in **Part C**. Identifier **[[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]**.
**Audience.** Engineering managers, architects, assurance leads, editors.

**Depends on.**

* **[[01_C.03.01 - U.Kind & SubkindOf (Core)|C.3.1]]** (*U.Kind & SubkindOf Core*): kinds are context‑local; `⊑` is a partial order; kinds carry **no Scope**.
* **[[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] USM** (*Context slices & Scopes*): Claim scope (G) and Work scope live on claims/capabilities; algebra `∈` (membership), `∩` (intersection), `SpanUnion` (union across independent lines), `translate` (scope mapping).
* **[[05_C.02.03 - Unified Formality Characteristic F|C.2.3]] U.Formality (F)**: ordinal F0…F9; no arithmetic; weakest‑link composition applies to content that depends on the signature.
* **[[03_C.02.02 - Reliability R in the F-G-R triad|C.2.2]] F–G–R**: assurance calculus; CL penalties feed **R**, not **F/G**.
* **Part B (Scope Bridges & CL).** CL (scope congruence) and scope translation live in Part B/USM; **kind‑congruence `CL^k`** and kind mapping live in **[[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]** (KindBridge).

**Non‑goals.**

* No Scope semantics here (USM); no bridge semantics here ([[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]).
* No repository/notation mandates; this is concept‑level, not tooling.

## [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:1 - Purpose & Audience
...
## [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:2 - Context
...
## [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:3 - Problem
...
## [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:4 - Forces
...
## [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:5 - Solution — Objects & Standards (overview)

* **`KindSignature(k)`** — the **intensional** definition of kind `k` in the Context; it **declares `U.Formality`** per [[05_C.02.03 - Unified Formality Characteristic F|C.2.3]].
...
## [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:6 - Norms & Invariants (normative)

> IDs **C3.2‑K‑03…K‑08** correspond to the rules announced in [[00_C.03 - Kinds, Intent-Extent, and Typed Reasoning (Kind-CAL)|C.3]]; additional local rules use **C3.2‑S‑\***.

### [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:6.1 - Signature & Formality

**C3.2‑K‑03 (Signature F).** Every `KindSignature(k)` **SHALL declare `U.Formality`** per [[05_C.02.03 - Unified Formality Characteristic F|C.2.3]] (F0…F9).
...
### [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:6.2 - Extension & Membership
...
— Implication: **“latest” is forbidden**; `Γ_time` must be explicit on `slice` ([[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]).
...
### [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:6.3 - Entity set & time
...
## [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:7 - Interactions & Placement (informative)

* **With [[01_C.03.01 - U.Kind & SubkindOf (Core)|C.3.1]].** Kinds carry identity and `⊑`; **no Scope** on kinds. This pattern adds the **intent/extent** layer under those constraints.
* **With [[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] (USM).** A typed claim’s guard normally evaluates, in the order specified by **Annex [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]] §5 (E‑01)**: (1) typed compatibility, (2) **Scope coverage** at `TargetSlice`, (3) **`MemberOf(?, k, TargetSlice)`** definedness and any instantiation, followed by penalties to **R** and freshness checks. Use **Guard_TypedClaim** / **Guard_TypedJoin** rather than ad‑hoc shapes.
* **With [[05_C.02.03 - Unified Formality Characteristic F|C.2.3]] (F).** Signature F influences claims **only if** the claim **depends on** the signature content; weakest‑link min applies along the claim’s support path.
* **With [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]] (KindBridge).** When `MemberOf` is computed via a **kind mapping across Contexts**, kind‑congruence `CL^k` contributes a **monotone penalty to **R** only (Ψ(`CL^k`)); **F/G MUST NOT** be adjusted.
* **With Role‑CAL ([[04_C.03.04 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]).** A **RoleMask** may **narrow** membership (context‑local adaptation). Frequent masks that encode stable narrowing **SHOULD** be promoted to subkinds (`⊑`).

## [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:8 - Authoring & Review Guidance (informative)

### [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:8.1 - Authoring `KindSignature`
...
### [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:8.2 - Authoring membership
...
### [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:8.3 - Review checklist (10 minutes)
...
6. Any **Cross‑context** classification? If yes, is **KindBridge** referenced ([[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]])?

## [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:9 - Worked Examples (informative)

### [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:9.1 - Vehicle (signature F4) and membership
...
### [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:9.2 - AuthenticatedRequest (definedness & fail‑closed)
...
### [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:9.3 - Clinical cohort (low‑F signature; deterministic membership)
...
## [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:10 - Anti‑patterns & Remedies (informative)
...
## [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:11 - Rationale (informative)

### [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:11.1 - Why give **F** to `KindSignature`?
...
### [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:11.2 - Why **Extension** is not **Scope**
...
### [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:11.3 - Why **determinism** and **fail‑closed**?
...
## [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:12 - Conformance checklist (normative)
...
## [[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]:End
