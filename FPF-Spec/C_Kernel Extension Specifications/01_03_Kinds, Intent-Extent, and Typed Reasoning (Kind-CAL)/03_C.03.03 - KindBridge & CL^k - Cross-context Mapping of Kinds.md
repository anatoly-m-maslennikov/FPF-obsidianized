...
**Status.** Normative in **Part C**. Identifier **[[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]**.
**Audience.** Engineering managers, architects, assurance leads, editors.

**Depends on.**

* **[[01_C.03.01 - U.Kind & SubkindOf (Core)|C.3.1]] — U.Kind & SubkindOf (Core):** kinds are context-local `U.Kind` records; `⊑` is a partial order; kinds **do not carry Scope**.
* **[[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]] — KindSignature (+F) & Extension/MemberOf:** signature declares its own **F**; membership `MemberOf(e,k,slice)` is **deterministic** per `U.ContextSlice`.
* **[[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] — USM (Context slices & Scopes):** Claim scope (**G**) and Work scope live on claims/capabilities; scope bridging and **CL** penalties are defined there.
* **[[03_C.02.02 - Reliability R in the F-G-R triad|C.2.2]] — F–G–R:** weakest‑link; penalties land in **R**, not **F/G**.
* **[[05_C.02.03 - Unified Formality Characteristic F|C.2.3]] — U.Formality (F):** signature rigor.
...
## [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:1 - Purpose & Audience
...
**[[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]** gives managers an explicit, auditable channel for **(2)**, so a team can say, with evidence: *“`Vehicle` in Lab maps to `TransportUnit` in Plant with `CL^k=2`; the EV subkind collapses; here’s what we lost.”* Guards stay deterministic; assurance math stays clean (penalties in **R** only).

## [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:2 - Context
...
## [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:3 - Problem
...
## [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:4 - Forces
...
## [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:5 - Solution — The **KindBridge** object (overview)
...
## [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:6 - Norms & Invariants (normative)

> The following formalize the **KB‑01…KB‑12** rules announced in [[00_C.03 - Kinds, Intent-Extent, and Typed Reasoning (Kind-CAL)|C.3]].

### [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:6.1 - Subject & Scope of a KindBridge
...
**KB‑02 (No Scope).** A KindBridge **MUST NOT** map Claim/Work scope (**G**). Scope translation uses the **USM Bridge + CL** channel ([[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]], Part B).

**No blended score.** Congruence for Scope (**CL**) and for Kind (**CL^k**) **MUST NOT** be aggregated into a single “interoperability” score in guards; each channel is assessed and penalized **separately**. See **Annex [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]] §5 (E‑06)**.

### [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:6.2 - Declaration & Shape
...
### [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:6.3 - Order & Monotonicity
...
### [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:6.5 - Loss Notes & Definedness
...
## [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:7 - Interactions (informative)

### [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:7.1 - With USM Scope bridges (two channels)
...
**Discipline:** compute both; **do not** collapse them into one “interoperability score.”
 ...
 See **Annex [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]] §5 (E‑01)** for the normative evaluation order in guards.

### [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:7.2 - With membership ([[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]])
...
### [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:7.3 - With Role masks ([[04_C.03.04 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]])
...
### [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:7.4 - With guards (Annex [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]])

Use the **`Guard_XContext_Typed`** macro (Annex [[06_C.03.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]), which requires **both bridges** and applies **both penalties** to **R**:
...
## [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:8 - Authoring, Review & Rating Guidance (informative)

### [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:8.1 - Authoring a KindBridge
...
### [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:8.2 - Review playbook (10 minutes)
...
### [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:8.3 - Rating `CL^k` (rules of thumb)
...
## [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:9 - Worked Examples (informative)

### [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:9.1 - Vehicle → TransportUnit (manufacturing)
...
### [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:9.2 - AuthenticatedRequest across services (software)
...
### [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:9.3 - AdultPatient across jurisdictions (clinical)
...
## [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:10 - Anti‑patterns & Remedies (informative)
...
## [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:11 - Conformance Checklist (normative)
...
## [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]:End
