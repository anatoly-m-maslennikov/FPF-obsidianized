...
> **Type:** Architectural subpattern under [[00_C.30 - Grounded Architecture and Selected-Structure Adequacy|C.30]]
> **Status:** Stable
> **Normativity:** Normative for FPF pattern, companion-text, and project-description use that claims architecture-specific residuals across declared scopes.

## [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:1 - Problem frame
...
What goes wrong if [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] is missed: a local improvement, control layer, scale label, interface grammar, or evidence reuse is treated as whole-holon architecture adequacy while the residual moves into another declared holon level or declared scope.

What [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] buys in practice: the practitioner can keep useful conflict or frustration language as an entry label while governing the architecture residual itself: affected holon levels or scopes, the selected structure that carries them, residual-bearing locus, and one admissible architecture move or governing-pattern application.
...
The useful architecture intuition is narrower than a new `Frustration` kind: local optimization at one declared holon level or declared scope can create a persistent residual in another declared holon level, declared scope, or level-bearing structure relation. Depending on the claim being made, that residual is governed by [[00_C.29 - Mathematical Lens Use|C.29]] only when a recoverable multilevel mapping, scale mapping, or coarse-graining mapping is being claimed; by [[02_C.31.ASAP - Architecture Scale-Amenability Preference|C.31.ASAP]] when architecture scale preference over declared alternatives is being claimed; or by [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] when residual-reducing candidate architecture moves form a candidate set being used. An ordinary conflict between structures is not enough for the RG lens or frustration mathematical lens, but a conflict between structures assigned to different declared holon levels or scale windows may be enough when the mapping, preserved-structure line, and lost-structure line are recoverable. The first [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] output is only the grounded triage record.

Without a pattern, teams either discuss the residual as vague `complexity`, treat it as ordinary stakeholder conflict, jump into measurement, use mathematical frustration language as proof, or jump to candidate generation too early. [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] keeps the first move small: identify whether the residual is architecture-shaping and name the first admissible architecture move or governing-pattern application.
...
## [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:3 - Forces
...
## [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:4 - Solution
...
**Multilevel optimization boundary.** [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] can recognize that local optimization in one declared holon level or declared scope degrades another declared holon level or declared scope. It does not optimize the architecture and does not prove that one global function exists. Use [[00_C.29 - Mathematical Lens Use|C.29]] with `MLU.Description@MultilevelLearningFrustration` only when the mathematical representation supplies a recoverable mapping between declared levels, scopes, scale windows, or coarse-graining steps and states what structure is preserved and lost. Conflicting structures can enter this lens only when each structure is assigned to a declared holon level, scope, scale window, or coarse-graining step and the mapping shows why the conflict is interlevel. If scale window, RG relation, coarse-graining relation, preserved structure, lost structure, or conflict residual slope becomes an architecture scale-preference claim, use [[02_C.31.ASAP - Architecture Scale-Amenability Preference|C.31.ASAP]] and keep any mathematical-lens claim in [[00_C.29 - Mathematical Lens Use|C.29]]. If the practitioner needs to generate, compare, or publish residual-reducing candidate architecture moves, use [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] for the candidate set and [[00_C.11 - Decision Theory (Decsn-CAL)|C.11]] when a final local choice is being made; [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] stops at the residual and first admissible move. If the case is only a conflict between two selected structures with no declared multilevel mapping or scale mapping, keep it in [[00_C.30 - Grounded Architecture and Selected-Structure Adequacy|C.30]], [[04_C.30.ASV - Architecture Structural View Adequacy (ASV)|C.30.ASV]], `D.3`, `D.4`, [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]], evidence, assurance, or decision patterns as applicable.
...
|---|---|
| measurement or characteristic claim | [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)]] or the characteristic pattern that governs the characteristic under evaluation |
| scale window, RG relation, coarse-graining relation, preserved structure, lost structure, or conflict residual slope | [[02_C.31.ASAP - Architecture Scale-Amenability Preference]] when an architecture scale-preference claim is being made; use [[00_C.29 - Mathematical Lens Use]] when mathematical-lens use is being claimed |
| multilevel learning or frustration mathematical-lens use with recoverable level mapping or scale mapping | [[00_C.29 - Mathematical Lens Use]] with `MLU.Description@MultilevelLearningFrustration` |
| candidate generation or residual-reducing candidate architecture moves | [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]] for the candidate set; [[00_C.11 - Decision Theory (Decsn-CAL)]] when final local choice is being made |
| final local choice | [[00_C.11 - Decision Theory (Decsn-CAL)]] |
| causal outcome claim | [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability]] |
| evidence or assurance | [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]], [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]], or [[00_G.06 - Evidence Graph & Provenance Ledger]] |
| ethical or stakeholder mediation | `D.3` or `D.4` |

**D.3 and D.4 boundary.** `D.3` and `D.4` handle conflict topology, values, stakeholder mediation, and ethical negotiation. [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] handles architecture-specific recognition: whether the conflict is borne by declared holon levels or declared scopes inside a selected structure: structural views, allocation, interfaces, control rates, work reuse, evidence reuse, scale windows, or coarse-graining loss. It is a triage and architecture-move pattern, not a negotiation pattern.
...
| Local autonomy conflicts with control scope | add control layer; change allocation; apply [[05_C.30.LCA - Control Structure View Adequacy (LCA)]] | Treat autonomy label as causal or safety proof. |
| Evidence reuse hides source loss | add evidence scope; add source-return condition; apply [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] or [[00_G.06 - Evidence Graph & Provenance Ledger]] | Treat reused evidence as automatically valid in the wider scope. |
| A scale window changes the residual | apply [[02_C.31.ASAP - Architecture Scale-Amenability Preference]], with [[00_C.29 - Mathematical Lens Use]] when scale-lens use is being made | Treat two observations as a universal scale law. |
| A frustration lens with recoverable level mapping or scale mapping makes candidate moves comparable | use [[00_C.29 - Mathematical Lens Use]] for lens adequacy and [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]] for the candidate set | Treat an unassigned or same-scope structure conflict as RG mathematics or frustration mathematics, or treat an interlevel residual without recoverable mapping as a global optimizer, proof, or selected architecture. |

**Worked slice A - clean module layout, bad flow.** A product team redraws modules so each component has an explicit responsibility relation or enactor relation, but order-to-cash flow now crosses more work transfers and exceptions rise. [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] names the module structure, selected transformation-flow structure, affected work scope, cross-scope residual, and first move: expose hidden coupling or apply [[07_C.30.TFS-REL - Architecture Transformation-Flow Structure Relation|C.30.TFS-REL]]. It does not turn the exception count into a modularity measure until [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]] or the characteristic pattern governing the characteristic under evaluation is applied.

**Worked slice B - AI agent control conflict.** A local agent optimizes its local objective and violates a supervisor's allowed-mode constraint. [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] names the agent scope, supervisor scope or control scope, control relation, local optimization claim, residual-bearing locus, and local repair attempted. The first move may be add control layer, change allocation, or apply [[05_C.30.LCA - Control Structure View Adequacy (LCA)|C.30.LCA]]. Safety, causality, and gate claims use their governing patterns.

**Worked slice C - evidence scope residue.** A reusable certification evidence set removes repeated evidence work for several product variants, but one variant has a hidden environment difference. [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] names the work scope or evidence scope and source-return condition. It applies [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] or [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]] when an evidence-validity claim is being made.

**Worked slice D - frustration residual before synthesis.** Several decompositions reduce local module work but each creates a different integration, control-rate, or evidence-reuse residual in another declared scope. [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] records the residuals and first architecture moves. If the team needs a residual-reducing candidate palette, [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] stops and applies [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] to the candidate set. If the team claims a multilevel-learning lens or frustration lens, [[00_C.29 - Mathematical Lens Use|C.29]] carries the lens-use fields and stop condition only after the level mapping, scope mapping, scale-window mapping, or coarse-graining mapping and preserved structure and lost structure are recoverable.

## [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:5 - Archetypal Grounding

| Archetype | Without [[06_C.30.ILC - Cross-Scope Architecture Residual Triage]] | With [[06_C.30.ILC - Cross-Scope Architecture Residual Triage]] |
|---|---|---|
...
## [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:6 - Bias-Annotation
...
* **Generic-structure-conflict bias.** A conflict between selected structures is treated as interlevel conflict even though no declared holon level, scale window, or coarse-graining relation is recoverable. Repair by keeping the case in [[00_C.30 - Grounded Architecture and Selected-Structure Adequacy|C.30]], [[04_C.30.ASV - Architecture Structural View Adequacy (ASV)|C.30.ASV]], or the governing pattern unless the structures are assigned to different declared holon levels or scale windows.
...
* **Global-optimizer bias.** Local optimization in one declared holon level or declared scope is used as if the architecture literally optimizes one global function. Repair by keeping the local optimization claim as a triage input unless [[00_C.29 - Mathematical Lens Use|C.29]] supplies an admissible mathematical-lens use with recoverable level mapping or scale mapping and the candidate-set or decision pattern carries any synthesis or selection claim.
* **Measurement-first bias.** A residual is measured before its level-bearing structure and scope grounding are declared. Repair by applying [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]] or the characteristic pattern governing the characteristic under evaluation only after triage names the affected characteristic or measurement relation.
...
* **Synthesis-jump bias.** A local residual immediately triggers candidate generation. Repair by identifying the first admissible architecture move before applying [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] to a residual-reducing candidate set.
...
## [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:7 - Conformance Checklist
...
| CC-ILC-9 | If multilevel learning or frustration is used as mathematics, the record names [[00_C.29 - Mathematical Lens Use]], `MLU.Description@MultilevelLearningFrustration`, the recoverable level mapping or scale mapping, and preserved structure and lost structure; if residual-reducing candidate moves form a candidate set being used, the record applies [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]] to the candidate set. | Preserves the useful multilevel optimization line without importing ontology, proof, or a hidden selector. |

## [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:8 - Common Anti-Patterns and How to Avoid Them
...
| Structure conflict as interlevel conflict | Two structures are in tension, but no declared holon level, scale window, or coarse-graining relation is recoverable. | Use [[00_C.30 - Grounded Architecture and Selected-Structure Adequacy]], [[04_C.30.ASV - Architecture Structural View Adequacy (ASV)]], `D.3`, `D.4`, [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability]], evidence, assurance, or decision patterns as applicable; use [[06_C.30.ILC - Cross-Scope Architecture Residual Triage]] only when the conflict is across declared levels or scopes. |
| Frustration-as-ontology | `Frustration` is treated as a new FPF kind, psychological state, physics kind, biology kind, assurance score, or proof. | Keep frustration as an entry label; recover `FrustrationResidual`, conflict carriers, residual-bearing locus, and governing patterns for [[00_C.29 - Mathematical Lens Use]], evidence, assurance, causal, mediation, or synthesis claims when those claim kinds are being made. |
| Optimization-as-global-proof | A local optimization claim is treated as proof that the whole architecture optimizes one global objective. | Record local and wider-scope claims separately; use [[00_C.29 - Mathematical Lens Use]] only for an admissible mathematical lens with recoverable level mapping or scale mapping and use candidate-generation, comparison, or decision patterns for synthesis and selection. |
| Measurement-first conflict | The team starts measuring before declaring what is in conflict. | Run ILC triage first; apply [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)]] or the characteristic pattern governing the characteristic under evaluation only when the measured characteristic is under evaluation. |
...
| Hidden candidate generation | The residual immediately spawns many designs. | State the first admissible move; apply [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]] to a residual-reducing candidate set only when candidate generation is being claimed. |
...
## [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:9 - Consequences
...
The cost is that [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] refuses to solve the whole problem. It identifies the first architecture move or governing-pattern application. Measurement, scale relation, RG relation, coarse-graining relation, mathematical lens use, stakeholder mediation, candidate generation, evidence, assurance, and final choice remain outside until those claims are being made.

This makes multilevel optimization usable rather than decorative. [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] identifies the residual that makes optimization relevant; [[00_C.29 - Mathematical Lens Use|C.29]] carries an admissible mathematical-lens use only when level mapping or scale mapping and preserved structure and lost structure are recoverable; [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] carries residual-reducing candidate sets; and the decision pattern carries any final selected architecture.

## [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:10 - Rationale
...
| Vanchurin, Wolf, Katsnelson, and Koonin multilevel learning and frustration line, plus related multilevel optimization and complexity-frustration materials. | Local optimization at one declared holon level, scope, or level-bearing structure relation can create persistent residual in another; frustrated optimization can be a candidate mathematical lens when a level mapping or scale mapping is recoverable. | Adopt and adapt: [[06_C.30.ILC - Cross-Scope Architecture Residual Triage]] uses this line for architecture triage only. [[00_C.29 - Mathematical Lens Use]] with `MLU.Description@MultilevelLearningFrustration` carries mathematical-lens adequacy with preserved structure and lost structure; no `U.Frustration`, physics or biology ontology transfer, global optimizer proof, causal proof, assurance proof, or stakeholder-mediation takeover. | First recover holon levels or scopes, level-bearing structure, conflict carriers, residual-bearing locus, local repair, and the first architecture move. Apply [[00_C.29 - Mathematical Lens Use]] only when the lens mapping is being claimed; apply [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]] only when residual-reducing candidate moves form a candidate set being used. |
| Control and cyber-physical systems practice. | Local autonomy, feedback, supervisor relations, and rate separation can create cross-scope conflict. | Reuse through [[05_C.30.LCA - Control Structure View Adequacy (LCA)]], [[04_B.02.05 - Supervisor-Subholon Feedback Loop]], [[00_C.27 - Temporal Claim Adequacy- State Readings, Temporal Trends, and Intervention-Sensitive Temporal Change]], and [[03_A.03.03 - U.Dynamics- State-Space and Transition-Law Episteme]]; do not let ILC carry control proof. | A control conflict is governed by control-structure or dynamics patterns only when those claims are being made. |
...
## [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:12 - Relations

* Builds on [[00_C.30 - Grounded Architecture and Selected-Structure Adequacy|C.30]] and [[04_C.30.ASV - Architecture Structural View Adequacy (ASV)|C.30.ASV]] for grounded architecture, selected-structure, and structural-view adequacy.
* Uses [[00_A.22 - Structure and Structural Views (STRUCT-CAL)|A.22]] for structure and structural-view discipline.
* Coordinates with [[07_C.30.TFS-REL - Architecture Transformation-Flow Structure Relation|C.30.TFS-REL]], [[05_C.30.LCA - Control Structure View Adequacy (LCA)|C.30.LCA]], [[11_A.06.F - Function and Functional Precision Restoration (RPR-FUNCTION)|A.6.F]], and [[12_A.06.M - Module Relation Repair|A.6.M]] when the residual concerns flow, control, function, allocation, module, or interface structure.
* Applies [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]] or the characteristic pattern that governs the characteristic under evaluation for measurement or characteristic claims.
* Applies [[00_C.29 - Mathematical Lens Use|C.29]] with `MLU.Description@MultilevelLearningFrustration` only when multilevel learning or frustration is used as a mathematical lens with recoverable level mapping or scale mapping and preserved structure and lost structure; applies [[02_C.31.ASAP - Architecture Scale-Amenability Preference|C.31.ASAP]] for architecture scale-preference claims and [[00_C.29 - Mathematical Lens Use|C.29]] for mathematical-lens claims when scale, RG, coarse-graining, preserved structure, lost structure, or scale-window adequacy is being claimed.
* Applies [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] for candidate generation and residual-reducing candidate architecture moves.
* Applies [[00_C.11 - Decision Theory (Decsn-CAL)|C.11]] for final local choice, [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] for causal outcome claims, [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]], [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], or [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]] for evidence or assurance, and `D.3` or `D.4` for ethical or stakeholder mediation.

Neighboring claims stay with their governing patterns: [[00_C.30 - Grounded Architecture and Selected-Structure Adequacy|C.30]] for grounded architecture and selected-structure adequacy, [[04_C.30.ASV - Architecture Structural View Adequacy (ASV)|C.30.ASV]] for structural-view adequacy, [[00_A.22 - Structure and Structural Views (STRUCT-CAL)|A.22]] for structure and structural-view discipline, [[07_C.30.TFS-REL - Architecture Transformation-Flow Structure Relation|C.30.TFS-REL]] for architecture-transformation-flow relation, [[05_C.30.LCA - Control Structure View Adequacy (LCA)|C.30.LCA]] for control-structure view relation, [[11_A.06.F - Function and Functional Precision Restoration (RPR-FUNCTION)|A.6.F]] for function-use repair, [[12_A.06.M - Module Relation Repair|A.6.M]] for module-interface repair, [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]] or the local characteristic pattern for the characteristic under evaluation, [[00_C.29 - Mathematical Lens Use|C.29]] for mathematical-lens use, [[02_C.31.ASAP - Architecture Scale-Amenability Preference|C.31.ASAP]] for architecture scale-preference, [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]] for candidate-set generation, [[00_C.11 - Decision Theory (Decsn-CAL)|C.11]] for final local choice, [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] for causal use, [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]], [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], or [[00_G.06 - Evidence Graph & Provenance Ledger|G.6]] for evidence or assurance, and `D.3` or `D.4` for ethical or stakeholder mediation. [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] governs only cross-scope architecture residual triage.

## [[06_C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:End
