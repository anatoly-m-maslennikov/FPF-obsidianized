...
**Scope & exports.** A substrate‑neutral calculus for composing **epistemic holons** (`U.Episteme`) and reasoning about their motion and equivalence. Exports: (i) three **point‑characteristics**—**Formality F**, **ClaimScope G**, **Reliability R**—that locate a single episteme; (ii) a **pairwise ladder** of **Congruence Levels (CL 0…3)**; (iii) four **Δ‑moves** (*Formalise, Generalise/Specialise, Calibrate/Validate, Congrue*); (iv) **composition rules** (Γ_epist) for aggregates; (v) propagation laws for CL through mappings and notation bridges. KD‑CAL is typed by `U.EpistemeSlotRelation` and never confuses `ClaimGraph`, `EntityOfConcernSlot`, `GroundingHolonSlot`, `Viewpoint`, `View`, `ReferenceScheme`, notation, publication form, or carrier. All F–G–R computations are **context‑local**; Cross‑context traversals **require** an explicit **Bridge** with **CL** and apply the **[[B_Trans-disciplinary Reasoning Cluster/02_03_Trust and Assurance Calculus (F-G-R with Congruence)/00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]** congruence penalty **Φ(CL)** to **R**.  // Contexts ≡ U.BoundedContext; substitution is plane‑preserving only.

**Formality F** is the rigor characteristic defined **normatively in [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/05_C.02.03 - Unified Formality Characteristic F|C.2.3]]**. All KD‑CAL computations and guards **SHALL** use `U.Formality` (F0…F9) as specified there; **no parallel “mode” ladders** are allowed.

## [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]:1 - Problem Frame

FPF fixes two archetypal sub‑holons: **`U.System`** (physical/operational) and **`U.Episteme`** (knowledge holon). KD‑CAL is the primary composition pattern for `U.Episteme`, giving engineers a compact, testable way to say (a) how strictly an episteme is written (**F**), (b) how much structure it manages (**G**), (c) how well it is warranted by evidence or severe tests (**R**), and (d) how closely **two** epistemes coincide (**CL**). KD‑CAL is built atop **[[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]] U.Episteme — Epistemes and their slot relation**, which reifies every episteme through `U.EpistemeSlotRelation`: `ClaimGraph`, `EntityOfConcernSlot`, `GroundingHolonSlot`, `Viewpoint`, `View`, and `ReferenceScheme`. Notation, publication forms, carriers, and work occurrences remain outside episteme content and are linked by their own FPF relations.

## [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]:2 - Problem
...
## [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]:3 - Forces
...
## [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]:4 - Solution

### [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]:4.1 - Coordinates and the episteme slot relation
...
### [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]:4.2 - Four Δ‑moves (epistemic motion)
...
### [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]:4.3 - Composition (Γ\_epist) and propagation
...
### [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]:4.4 - What **must not** be conflated (normative guards)

* **Representation structure ≠ carrier.** Files, PDFs, or repositories are **carriers** outside the episteme; they never count as parts of `U.Episteme` (**see [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]] EP‑1; CC‑EPI‑2/3**).
* **Epistemes do not act.** Only **systems** perform work; epistemes carry constraints and evaluation criteria through their `ClaimGraph`, EntityOfConcern, grounding holon, scope, and evidence bindings (**per Core [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] / CC‑EPI‑3**).
* **CL is not a score.** It is a **qualitative ladder** of preservation classes; do not average it.

## [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]:5 - ✱ Archetypal Grounding (Tell–Show–Show)
...
## [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]:6 - Bias‑Annotation
...
* **Category slip.** Equating a notation or carrier with `ClaimGraph`, EntityOfConcern, or grounding holon; mitigation: slot-relation and carrier separation under [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]].
* **Analogy inflation.** Presenting CL‑0/1 as identity; mitigation: always name the **CL rung** for cross‑mappings.

## [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]:7 - Conformance Checklist

1. **C2‑1 (Slot relation).** Every `U.Episteme` **MUST** satisfy [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]] slot discipline for `ClaimGraph`, `EntityOfConcernSlot`, `GroundingHolonSlot`, `Viewpoint`, `View`, and `ReferenceScheme`; carriers link through SCR/RSCR or other exact carrier relations and are never parts of the episteme.
2. **C2‑2 (Coordinates).** Each episteme **SHALL** declare `[F,G,R]` with a brief rationale; **F** is `U.Formality ∈ {F0…F9}` per **[[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/05_C.02.03 - Unified Formality Characteristic F|C.2.3]]**, **exactly one episteme‑level F** computed as the **min over essential parts**. CL is declared for **pairs only**. Sub‑anchors: ** Contexts **MAY** mint named sub‑anchors (e.g., `F4[OCL]`, `F7[HOL]`), which **MUST** preserve the global order and **map to their parent anchor** from [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/05_C.02.03 - Unified Formality Characteristic F|C.2.3]].
...
## [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]:8 - Consequences
...
## [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]:9 - Rationale

KD‑CAL turns the coarse legacy semiotic picture into **holonic composition** over `U.EpistemeSlotRelation`, where formal structure and claim scope (**F,G**), evidence (**R**), and cross‑mapping congruence (**CL**) are visible and composable. The explicit [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]] slot relation prevents carrier confusion; the characteristics provide a **manager‑readable** yet **formalisation‑ready** scale (with **G** grounded in **scope/envelope**, not part‑count); the CL scale replaces overloaded “alignment” with a typed sameness relation.

## [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]:10 - Relations

* **Depends on:** `U.Episteme — Epistemes and their slot relation` ([[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]]): identity invariants, slot definitions, carrier separation, and evidence bindings.
...
## [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]:11 - Worked mini‑examples (post‑2015 flavours)
...
## [[C_Kernel Extension Specifications/00_02_Epistemic holon composition (KD-CAL)/00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]:End
