...
terms:
  - "U.CausalUseQuestion"
builds_on:
  - "[[A_Kernel Architecture Cluster/13_The Agential Role & Agency Spectrum/00_A.13 - The Agential Role & Agency Spectrum|A.13]]"
  ...
  - "[[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/09_A.06.P - Relational Precision Restoration (RPR) - Kind-Explicit Qualified Relation Discipline|A.6.P]]"
...
**At a glance.** [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] is the choice-calculus pattern for the moment when options already exist and the working question is which option to choose, including whether another probe is worth its cost before commitment.

## [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:1 - Problem frame
...
**Typical reroutes.** [[C_Kernel Extension Specifications/06_18_Open-Ended Search Archive and Front Stewardship/00_C.18 - Open-Ended Search Archive and Front Stewardship|C.18]] when the real question is still inventing or reframing options; [[C_Kernel Extension Specifications/07_19_Explore-Exploit Live-Pool Governor/00_C.19 - Explore-Exploit Live-Pool Governor|C.19]] when the working question is how broadly to explore or exploit the candidate pool; [[C_Kernel Extension Specifications/12_24_Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)/00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]] when one option is already chosen and the work has become sequencing or enactment; `A.13 / C.9` when the hard question is agenthood rather than choice; `A.18 / A.19` when the mathematical support question itself becomes primary.

**Common neighboring-pattern mistakes.** Do not use [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] to hide search work inside "decision", to hide candidate-pool policy inside one local choice, or to hide execution planning inside one generic rationality account. Do not treat selected-set publication or shortlist semantics as if they were the same question as deciding.
...
Decision work often fails not because no options exist, but because the choice among existing options is never typed as its own question. [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] starts from one narrower and more useful center: one decision subject choosing among already-available options, including whether more probing is worth the cost before the choice is fixed.

## [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:2 - Problem
...
## [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:3 - Forces

| Force | Tension |
| --- | --- |
| Choice doctrine versus option generation | [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)]] must govern choice among already-available options without swallowing [[C_Kernel Extension Specifications/06_18_Open-Ended Search Archive and Front Stewardship/00_C.18 - Open-Ended Search Archive and Front Stewardship]] search and candidate-generation work. |
...
## [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4 - Solution
### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.1a - Causal-use hook for choice records

When the admissible choice among an existing `OptionSet` depends on an effect claim, intervention claim, counterfactual comparison, causal policy claim, or off-policy causal evaluation, the `ChoiceResult` keeps the decision-theory question local and cites [[C_Kernel Extension Specifications/16_28_CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability/00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] for the causal-use question and support basis.
...
What this does not authorize: [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] does not identify causal effects, certify target-trial emulation, validate off-policy causal evaluation, or decide counterfactual sampling realizability; it emits one `ChoiceResult` and redirects the causal-use question to [[C_Kernel Extension Specifications/16_28_CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability/00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]].

### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.1 - Primary EntityOfConcern and admissible choice result

[[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] governs theory-side choice among already-available options. Its selected decision result states what should be chosen from the current `OptionSet`, including whether further probing, information gathering, or computation is rational before the choice is fixed.
...
### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.2 - Decision discipline over a live option set

A conforming [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] pass does not stop at naming schools of decision theory. It carries one usable choice discipline over a live `OptionSet`, and it ends with one explicit `ChoiceResult` under one explicit `ChoiceRule`.
...
2. **Freeze the current option set.**
   ...
   If the hard work is still inventing, expanding, or reframing the options, stop here and apply [[C_Kernel Extension Specifications/06_18_Open-Ended Search Archive and Front Stewardship/00_C.18 - Open-Ended Search Archive and Front Stewardship|C.18]].
...
5. **Run the probe-worthiness test before commitment.**
   ...
   This rule is intentionally local or myopic: it judges the best next feasible probe over the current `OptionSet` and current comparison basis, not one full sequential or non-myopic experimental program. Richer `OED` lines may strengthen this doctrine, but the local [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] closure rule already has to decide whether the next feasible probe can still change the current choice.
   ...
   If one choice result is already fixed and the remaining probe would change only execution-path description, call-plan ordering, enactment budget, or checkpointing of that chosen option, stop treating the probe as local choice doctrine and apply [[C_Kernel Extension Specifications/12_24_Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)/00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]].
...
6. **Apply one `ChoiceRule` and emit one `ChoiceResult` plus the next question.**
   ...
   A [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] pass is done only when it names the lawful choice result and the reason that result is lawful.

### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.2.1 - Well-formed comparison state

Well-formedness constraint: a live [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] comparison state is usable only when the decision record states all of the following:
...
### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.2.1a - Minimal admissible decision semantics
...
The following are lawful in this [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] body when they are stated explicitly:
...
The following are not admissible in this [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] body:
...
This is why [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] is more than one note-taking protocol. The body already supports local incompleteness, partial order, explicit trade-off measures, and wider chooser-bearing cases, but it requires those semantic facts to change the lawful result rather than remain hidden beneath one elegant summary line.

### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.2.2 - Probe-worthiness rule
...
This is the current local or myopic probe-worthiness rule for [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]: judge the best next feasible probe over the current `OptionSet`, not one whole non-myopic experiment design over longer horizons. Later sequential or non-myopic `OED` may strengthen this doctrine, but they do not relocate the local-choice question outside [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]].
...
If the next probe would no longer change which option survives but would only change how one already-chosen option gets enacted, budgeted, or checkpointed, the question has already crossed to [[C_Kernel Extension Specifications/12_24_Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)/00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]].

### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.2.3 - `ChoiceRule` versus `ChoiceResult`
...
### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.2.4 - Closure rule over the current `OptionSet`
...
- to [[C_Kernel Extension Specifications/06_18_Open-Ended Search Archive and Front Stewardship/00_C.18 - Open-Ended Search Archive and Front Stewardship|C.18]] when the option set itself is still under invention or reframing;
- to [[C_Kernel Extension Specifications/07_19_Explore-Exploit Live-Pool Governor/00_C.19 - Explore-Exploit Live-Pool Governor|C.19]] when the question is now how broadly to keep exploring or exploiting one candidate pool;
- to [[C_Kernel Extension Specifications/12_24_Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)/00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]] when one choice result already exists and the next task is now sequencing, enactment, or execution-path probe work;
- to [[G_Discipline SoTA Patterns Kit/06_05_Multi-Method Dispatcher and MethodFamily Registry/00_G.05 - Multi-Method Dispatcher and MethodFamily Registry|G.5]] when the next task is now selected-set surfacing or publication.
...
### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.2.5 - Minimal decision-record form

A minimal [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] decision record has this shape:
...
### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.2.6 - Resource-aware choice is one lens over declared source families
...
- The kernel floor here is:
  - [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/12_A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]] remains the cited set-return floor
  - `SelectionSlot` remains the selector output floor
  - if later selector-facing publication is required, that set-returning floor may support one `Shortlist` or one `RankedShortlist` in [[G_Discipline SoTA Patterns Kit/06_05_Multi-Method Dispatcher and MethodFamily Registry/00_G.05 - Multi-Method Dispatcher and MethodFamily Registry|G.5]] rather than one forced single winner

#### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.3.1 - Classical evidential baseline
...
#### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.3.2 - Causal repair
...
#### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.3.3 - Success-first or subjunctive repair
...
#### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.3.4 - Active-inference neighboring repair
...
This minimal choice doctrine makes that social-expectation pressure explicit, but it does not yet operationalize one full `ROE` or `SocialExpectationRegime` object model inside [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]. If that heavier machinery is itself what the case hinges on, the decision record should say so honestly rather than pretending the local [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] floor has already settled it.
...
[[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] keeps the choice question visible there, but [[A_Kernel Architecture Cluster/13_The Agential Role & Agency Spectrum/00_A.13 - The Agential Role & Agency Spectrum|A.13]] and `C.9` still govern the narrower question of what kind of agent or agential system is in play, and [[C_Kernel Extension Specifications/12_24_Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)/00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]] still governs later sequencing and enactment once a choice result has already been fixed.
...
#### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.3.5 - Quantum-like neighboring repair
...
This minimal choice doctrine keeps the branch at that measurement-sensitive recognition point. It does not yet claim one full quantum-like state-space package inside [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]; it claims only that the live comparison may need one explicit measurement-class or order-sensitive repair rather than one plain commutative reading.
...
### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.4 - Reroute as soon as the question stops being local choice

Use [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] while the question remains: from this current `OptionSet`, what should the `DecisionSubject` choose, and is another probe worth its cost before commitment?

Reroute immediately when the question changes:

- If the hard question is still what options should exist at all, or whether the current option set needs to be expanded or reframed, leave this pattern and work in [[C_Kernel Extension Specifications/06_18_Open-Ended Search Archive and Front Stewardship/00_C.18 - Open-Ended Search Archive and Front Stewardship|C.18]] first.
- If the options already exist but the question is how broadly to keep exploring or exploiting the candidate pool, leave this pattern and work in [[C_Kernel Extension Specifications/07_19_Explore-Exploit Live-Pool Governor/00_C.19 - Explore-Exploit Live-Pool Governor|C.19]], where the next useful output is one explicit pool-policy result rather than one local `ChoiceResult`.
- If one option is already chosen and the question is how to sequence, budget, or enact that choice, leave this pattern and work in [[C_Kernel Extension Specifications/12_24_Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)/00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]], where the next useful output is one enactment-facing call plan or `CheckpointReturn`.
- If the question has shifted from deciding to surfacing, publishing, or naming the selected set, leave this pattern and work in [[G_Discipline SoTA Patterns Kit/06_05_Multi-Method Dispatcher and MethodFamily Registry/00_G.05 - Multi-Method Dispatcher and MethodFamily Registry|G.5]], where the next useful output is one published shortlist, ranked shortlist, narrowed handoff plan, or explicit abstain outcome rather than one more local choice result.

`ProbeBudget` stays here while it means the epistemic or deliberative budget for one more probe before choice and while that probe can still change which option survives or whether the current set should be rejected. When the same word now means execution budget, call budget, enactment budget, or execution-path scouting after one choice result already exists, the question has moved to [[C_Kernel Extension Specifications/12_24_Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)/00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]].

`ValueOfInformation` and `ValueOfComputation` also stay theory-side here as comparative criteria while the question is still local choice among the current options. If one more probe could still change which option survives or whether the current set should be rejected, stay in [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]. If the choice result is already fixed and those criteria now govern only execution-path sequencing, call-plan ordering, or enactment of the chosen option, the question has crossed to [[C_Kernel Extension Specifications/12_24_Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)/00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]]. [[C_Kernel Extension Specifications/07_19_Explore-Exploit Live-Pool Governor/00_C.19 - Explore-Exploit Live-Pool Governor|C.19]] and [[C_Kernel Extension Specifications/12_24_Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)/00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]] may consume the criteria, but they do not become the doctrine authorities for them.
...
### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.5 - Minimal inventory and mathematical floor
...
### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.5.1 - Always explicit versus conditionally activated objects

The following objects should be explicit in every usable [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] decision record:
...
- the chooser in [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] need not be one person-like agent;
...
### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.5.2 - Boundary on multilevel and social-expectation doctrine
...
Those absences are not hidden exceptions. They are explicit scope boundaries of this [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] body. If one of those heavier questions is already live in the case, the decision record should say that the local [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] floor is being used only as the current typed floor and should keep the unresolved aggregation, `ROE`, or multilevel support question visible by value.

### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:4.6 - Minimal decision tuple and finish condition

A [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] decision record is complete only when it states:
...
A [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] pass is finished enough for practical use when all three conditions hold:
...
## [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:5 - Archetypal Grounding

### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:5.1 - System grounding
...
**Show.** [[C_Kernel Extension Specifications/06_18_Open-Ended Search Archive and Front Stewardship/00_C.18 - Open-Ended Search Archive and Front Stewardship|C.18]] is still the place for inventing new plans, [[C_Kernel Extension Specifications/07_19_Explore-Exploit Live-Pool Governor/00_C.19 - Explore-Exploit Live-Pool Governor|C.19]] is still the place for broader exploration policy over the plan pool, and [[C_Kernel Extension Specifications/12_24_Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)/00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]] is still the place for the run sheet and execution order after the choice is made.

### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:5.2 - Episteme grounding
...
**Show.** [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] governs the decision doctrine over the current explanation set: one `BeliefState`, one `OutcomeModel`, one explicit `PreferenceOrder` or `EvaluativeMeasure`, and, when the case needs it, one `InterventionModel`, `CounterfactualModel`, or `SubjunctiveDependenceRelation` rather than one thinner evidential comparison. When another model comparison pass is on the table, `ValueOfComputation` belongs here as part of the current choice doctrine rather than as one later planning afterthought.
...
### [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:5.3 - Collective and contextual grounding
...
**Show.** [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] keeps the case legible by typing the chooser as one `DecisionSubject` at explicit `DecisionSubjectGranularity`, keeping the available actions as one current `OptionSet`, keeping one explicit `BeliefState` and `OutcomeModel` around those actions, and asking whether another test belongs in the `ProbeActionSet` with enough expected value to justify its `CostToProbe`.

**Show.** Active-inference-adjacent pressure is visible because the chooser is embodied, online, and socially coupled; quantum-like pressure is visible because context and question order change the comparison state. [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] keeps both repaired limitations visible without pretending that the whole pattern has already become one full active-inference or quantum-like formal package.
...
- `ChoiceResult = the next question is governed by [[C_Kernel Extension Specifications/12_24_Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)/00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]] because the board has already chosen escalation and the next task is now treatment sequencing rather than local choice`.
...
## [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:6 - Bias-Annotation
...
## [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:7 - Conformance Checklist

| ID | Requirement | Purpose |
| --- | --- | --- |
| `CC-C11.1` | The pattern **SHALL** state that [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)]] governs choice among already-available options rather than candidate generation. | Keeps [[C_Kernel Extension Specifications/06_18_Open-Ended Search Archive and Front Stewardship/00_C.18 - Open-Ended Search Archive and Front Stewardship]] outside and prevents search takeover. |
| `CC-C11.2` | The pattern **SHALL** keep `DecisionSubject` as the default chooser term, and **SHALL NOT** use `Agent` as the generic chooser term unless one explicit agency claim is governed by [[A_Kernel Architecture Cluster/13_The Agential Role & Agency Spectrum/00_A.13 - The Agential Role & Agency Spectrum]] or `C.9`. | Prevents unwanted narrowing of the chooser. |
| `CC-C11.3` | The pattern **SHALL** state the `C.11 / [[C_Kernel Extension Specifications/06_18_Open-Ended Search Archive and Front Stewardship/00_C.18 - Open-Ended Search Archive and Front Stewardship]] / [[C_Kernel Extension Specifications/07_19_Explore-Exploit Live-Pool Governor/00_C.19 - Explore-Exploit Live-Pool Governor]] / [[C_Kernel Extension Specifications/12_24_Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)/00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)]] / G.5` split explicitly in the body. | Prevents collapse of choice doctrine, candidate generation, candidate-pool policy, planning, and selected-set publication. |
| `CC-C11.4` | `Solution` **SHALL** state one inspectable decision procedure from `DecisionSubject` and `OptionSet` through comparison basis, dependence layer, probe-worthiness test, one explicit `ChoiceRule`, and one emitted `ChoiceResult`. | Keeps [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)]] as one operational answer to the choice question rather than one survey of schools. |
...
| `CC-C11.7` | Relation-heavy terms such as `PreferenceOrder`, `CounterfactualModel`, and `SubjunctiveDependenceRelation` **SHALL** remain answerable to [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/09_A.06.P - Relational Precision Restoration (RPR) - Kind-Explicit Qualified Relation Discipline]] together with [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/14_A.06.05 - U.RelationSlotDiscipline - SlotKind, ValueKind, RefKind, and slot-operation discipline]]. | Keeps dependence language inspectable and deconflicted. |
...
| `CC-C11.10` | `ProbeBudget` **SHALL** stay in [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)]] while it means the budget for further probing before choice, and `ValueOfInformation` / `ValueOfComputation` **SHALL** stay theory-side comparative criteria even when [[C_Kernel Extension Specifications/07_19_Explore-Exploit Live-Pool Governor/00_C.19 - Explore-Exploit Live-Pool Governor]] or [[C_Kernel Extension Specifications/12_24_Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)/00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)]] later consume their outputs. | Preserves the bounded-resource bridge without letting neighboring patterns steal the doctrine. |
| `CC-C11.11` | Shortlist or selected-set publication semantics **SHALL NOT** be treated as part of [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)]]; if the question shifts to surfacing or publishing the selected set, the text **SHALL** apply [[G_Discipline SoTA Patterns Kit/06_05_Multi-Method Dispatcher and MethodFamily Registry/00_G.05 - Multi-Method Dispatcher and MethodFamily Registry]]. | Preserves selector-facing publication placement and keeps publication semantics out of local choice doctrine. |
...
| `CC-C11.15` | If a `ChoiceResult` is supported by a causal effect, counterfactual comparison, causal policy, or off-policy causal evaluation claim, it **SHALL** carry `ChoiceResult.causalUseSpec?` with `targetCausalityLadderRung`, `causalUseClaimKind: CausalUseClaimKind`, supported use and unsupported use, and the relevant [[C_Kernel Extension Specifications/16_28_CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability/00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability]] support refs. | Prevents decision-theory vocabulary from certifying causal-use support. |

## [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:8 - Common Anti-Patterns and How to Avoid Them
...
| --- | --- | --- | --- |
| Search takeover | The text starts treating option generation as if it were already part of decision doctrine. | [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)]] loses its decision-theory EntityOfConcern and silently absorbs [[C_Kernel Extension Specifications/06_18_Open-Ended Search Archive and Front Stewardship/00_C.18 - Open-Ended Search Archive and Front Stewardship]]. | The option set is stated as already existing, and search questions are handled by [[C_Kernel Extension Specifications/06_18_Open-Ended Search Archive and Front Stewardship/00_C.18 - Open-Ended Search Archive and Front Stewardship]]. |
| Policy collapse | Exploration or exploitation governance over a candidate pool is written as if it were identical with choosing among current options. | Choice doctrine and candidate-pool policy become indistinguishable. | [[C_Kernel Extension Specifications/07_19_Explore-Exploit Live-Pool Governor/00_C.19 - Explore-Exploit Live-Pool Governor]] remains explicit as the neighboring pattern for selection policy and exploration governance. |
| Planning collapse | Sequencing, replanning, and enactment budgeting are written as if they were already part of the choice calculus. | Planning-side question moves out of [[C_Kernel Extension Specifications/12_24_Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)/00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)]] by accident. | Execution order and operational budgeting remain in [[C_Kernel Extension Specifications/12_24_Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)/00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)]], even when [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)]] says more probing is rational. |
...
| Bounded-resource bridge loss | `ProbeBudget`, `ValueOfInformation`, or `ValueOfComputation` are mentioned, but the text silently lets [[C_Kernel Extension Specifications/07_19_Explore-Exploit Live-Pool Governor/00_C.19 - Explore-Exploit Live-Pool Governor]] or [[C_Kernel Extension Specifications/12_24_Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)/00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)]] own them. | The theory-side doctrine disappears into neighboring policy or planning prose. | Keep those objects theory-side in [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)]]; let neighboring patterns consume their outputs without minting the concepts. |
| Publication collapse | The text starts treating shortlist or selected-set publication semantics as if they were identical with deciding. | Choice doctrine silently absorbs selector-facing publication question and collides with the [[G_Discipline SoTA Patterns Kit/06_05_Multi-Method Dispatcher and MethodFamily Registry/00_G.05 - Multi-Method Dispatcher and MethodFamily Registry]] placement. | Keep selected-set publication outside [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)]] and apply [[G_Discipline SoTA Patterns Kit/06_05_Multi-Method Dispatcher and MethodFamily Registry/00_G.05 - Multi-Method Dispatcher and MethodFamily Registry]] when the question becomes surfacing or publishing the selected set. |
...
## [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:9 - Consequences
...
## [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:10 - Rationale
...
| Some quantum-like lines also claim one practical representational gain from linear state dynamics over harder nonlinear underlying processes. | Quantum-like modeling in biology presents linear Hilbert-space dynamics as one simplifying and potentially faster information-processing lens over nonlinear classical biophysical dynamics, while treating this as representational modeling rather than proof that the modeled system is physically quantum. | [Quantum-like modeling in biology with open quantum systems and instruments](https://www.sciencedirect.com/science/article/pii/S0303264720301994) | [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)]] takes this only as one possible practical reason to keep the quantum-like branch available when measurement-sensitive effects are real; it does not treat quantum-like choice as one claim of physical quantumness. | **Adapt cautiously.** |
| Broader contextual and multilevel lines pressure decision texts to keep one typed substrate rather than pure verbal drift. | Current multilevel-learning and evolution-as-inference work argues for one shared formal lens across levels even when the heavier final geometry is still unsettled. | [Multilevel selection as Bayesian inference, major transitions in individuality as structure learning](https://royalsocietypublishing.org/doi/10.1098/rsos.190202) | [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)]] therefore keeps one minimal typed floor and one wider chooser-bearing scope while stating by value that full aggregation doctrine, cross-scale or cross-collective conflict doctrine, and heavier multilevel mathematics remain outside this local choice body. | **Adapt.** |
...
## [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:12 - Relations

- **Builds on:** [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/09_A.06.P - Relational Precision Restoration (RPR) - Kind-Explicit Qualified Relation Discipline|A.6.P]], [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/14_A.06.05 - U.RelationSlotDiscipline - SlotKind, ValueKind, RefKind, and slot-operation discipline|A.6.5]], [[A_Kernel Architecture Cluster/13_The Agential Role & Agency Spectrum/00_A.13 - The Agential Role & Agency Spectrum|A.13]], `C.9`, [[A_Kernel Architecture Cluster/18_Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC-KERNEL)/00_A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC-KERNEL)|A.18]], [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]]
- **Read next when this question leaves local choice:** [[C_Kernel Extension Specifications/06_18_Open-Ended Search Archive and Front Stewardship/00_C.18 - Open-Ended Search Archive and Front Stewardship|C.18]] for candidate generation and open-ended search, [[C_Kernel Extension Specifications/07_19_Explore-Exploit Live-Pool Governor/00_C.19 - Explore-Exploit Live-Pool Governor|C.19]] for one explicit pool-policy result over exploration or exploitation governance, [[C_Kernel Extension Specifications/12_24_Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)/00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]] for one enactment-facing call plan or `CheckpointReturn`, [[G_Discipline SoTA Patterns Kit/06_05_Multi-Method Dispatcher and MethodFamily Registry/00_G.05 - Multi-Method Dispatcher and MethodFamily Registry|G.5]] for shortlist-family public selected-set label and emitted selected-set semantics, [[C_Kernel Extension Specifications/16_28_CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability/00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] when the choice result depends on causal-use support
...
3. If the issue is question order, incompatible questions, response-replicability tension, non-shared comparison frames, or CbD-style variable identity / joint-availability trouble inside the current option set, keep the QL repair inside [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]].
4. If the current option set itself is suspect, incomplete, generated by the wrong frame, or still needs expansion/reframing/search, leave [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] and apply [[C_Kernel Extension Specifications/06_18_Open-Ended Search Archive and Front Stewardship/00_C.18 - Open-Ended Search Archive and Front Stewardship|C.18]], [[C_Kernel Extension Specifications/07_19_Explore-Exploit Live-Pool Governor/00_C.19 - Explore-Exploit Live-Pool Governor|C.19]], [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]], or [[B_Trans-disciplinary Reasoning Cluster/04_05_Canonical Reasoning Cycle/02_Abductive Loop/00_B.05.02 - Abductive Loop|B.5.2]] as appropriate.
...
When the question leaves local choice, apply the governing pattern instead of stretching [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:

| Question that appears in the decision case | First pattern |
| --- | --- |
| Boundary interaction, API read, workshop, dashboard, or message changes the represented state | [[C_Kernel Extension Specifications/14_26_Quantum-Like Modeling Lens/01_C.26.01 - Probe-Coupled Boundary Interaction]] only for the remaining state or probe question, with [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/00_A.06 - Signature Stack & Boundary Discipline]], [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/02_A.06.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work-Effects)]], and [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/09_A.06.P - Relational Precision Restoration (RPR) - Kind-Explicit Qualified Relation Discipline]] still active |
| Cross-context export, translation, or same-label comparison loses state or comparability | [[F_The Unification Suite (U-Suite)- Concept-Sets, SenseCells & Contextual Role Assignment/09_Alignment and Bridge across Contexts/00_F.09 - Alignment and Bridge across Contexts]] |
| Coordinated work evidences a state no report faithfully carries | [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/00_A.15 - Role-Method-Work Alignment (Contextual Enactment)]] plus evidence patterns, with [[C_Kernel Extension Specifications/14_26_Quantum-Like Modeling Lens/02_C.26.02 - Enacted Distributed State Evidence]] only for the remaining low-recoverability distributed-state reading |
| Measurement, metric, survey, or score frame changes the represented state | [[C_Kernel Extension Specifications/04_16_Measurement & Metrics Characterization (MM-CHR)/00_C.16 - Measurement & Metrics Characterization (MM-CHR)]] plus evidence patterns |
| Viability envelope, adaptation cost, or boundary-maintenance decision is load-bearing | [[C_Kernel Extension Specifications/13_25_Q-Bundle- Authoring --ilities- as Structured Quality Bundles/00_C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles]] first, with [[C_Kernel Extension Specifications/14_26_Quantum-Like Modeling Lens/03_C.26.03 - Viability-Envelope Boundary Regulation]] only for the remaining probe/export/frame/coarsening viability question |
| Current option set is suspect, incomplete, frame-generated, or still being expanded/reframed/searched | [[C_Kernel Extension Specifications/06_18_Open-Ended Search Archive and Front Stewardship/00_C.18 - Open-Ended Search Archive and Front Stewardship]], [[C_Kernel Extension Specifications/07_19_Explore-Exploit Live-Pool Governor/00_C.19 - Explore-Exploit Live-Pool Governor]], [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)]], or [[B_Trans-disciplinary Reasoning Cluster/04_05_Canonical Reasoning Cycle/02_Abductive Loop/00_B.05.02 - Abductive Loop]] before returning to [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)]] |
...
[[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] may cite [[C_Kernel Extension Specifications/14_26_Quantum-Like Modeling Lens/00_C.26 - Quantum-Like Modeling Lens|C.26]] as the common quantum-like modeling lens only for the residual question after the local-choice split is applied. It is not the whole-cluster pattern for boundary, bridge/export, distributed-evidence, viability, or state-representation coarsening claims.

## [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:12b - [[C_Kernel Extension Specifications/17_29_Mathematical Lens Use/00_C.29 - Mathematical Lens Use|C.29]] mathematical-lens use relation

> [[C_Kernel Extension Specifications/17_29_Mathematical Lens Use/00_C.29 - Mathematical Lens Use|C.29]] may supply a lens-supported prediction, distinction, obstruction, diagnostic boundary, or rival-lens note that a decision record can cite. If the output is a `ChoiceResult`, local choice record, selected-set publication, or selected option set, [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]] governs the decision discipline and any [[G_Discipline SoTA Patterns Kit/06_05_Multi-Method Dispatcher and MethodFamily Registry/00_G.05 - Multi-Method Dispatcher and MethodFamily Registry|G.5]]/[[G_Discipline SoTA Patterns Kit/10_09_Parity and Benchmark Harness/00_G.09 - Parity and Benchmark Harness|G.9]] selector or benchmark publication remains separate. [[C_Kernel Extension Specifications/17_29_Mathematical Lens Use/00_C.29 - Mathematical Lens Use|C.29]] does not select the option by mathematical elegance.

## [[C_Kernel Extension Specifications/02_11_Decision Theory (Decsn-CAL)/00_C.11 - Decision Theory (Decsn-CAL)|C.11]]:End
