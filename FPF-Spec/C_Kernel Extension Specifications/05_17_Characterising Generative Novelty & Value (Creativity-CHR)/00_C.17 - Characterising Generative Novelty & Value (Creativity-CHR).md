...
**Depends on.** A‑kernel ([[00_A.01 - U.Holon, U.System, and U.Episteme|A.1]]–[[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]), **CHR‑CAL** (C.7), **MM‑CHR** measurement infrastructure ([[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]]), **KD‑CAL** and **Sys‑CAL** for carriers and holons, **Decsn‑CAL** (utility), **Norm‑CAL** (constraints/ethics).
**Coordinates with.** **[[02_B.05.02.01 - Creative Abduction with NQD|B.5.2.1]] NQD** (abductive generator) for search instrumentation, **Agency-CHR** (C.9) for agential capacity, B-cluster trust/assurance ([[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]), Canonical Evolution Loop ([[00_B.04 - Canonical Evolution Loop|B.4]]), Role Assignment & Enactment Cycle (Six-Step) ([[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]) and Naming Discipline for U.Types & Role Names ([[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]).
...
This pattern exports **Characteristics** and measurement templates **only**. It **does not** export any Γ\_\* operators, retained-set composition rules, `Front`/`Archive`/`Shortlist` heads, or selection/scalarization policies; those live in **[[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]] NQD-CAL**, **[[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]] E/E-LOG**, and **[[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]** (or **Decsn-CAL** for decision lenses). A Context _publishes_ the measurement space and admissible policies; later choice using that space is attributed to a declared `DecisionSubject` at explicit `DecisionSubjectGranularity` under a named lens.
...
* **Relations (informative; not exported):** dominance relation (partial order in the space), frontier predicate (Pareto set), retained-set composition view. *[[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]] exports no operators and does not mint public set-result family heads; these are mathematical relations only.*
*
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:1 - Motivation & Intent (manager’s read‑first)
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:2 - Forces
...
| **Local truth vs. Cross‑context reuse** | Meaning is context‑local ([[01_A.01.01 - U.BoundedContext Semantic Frame]]); yet we need Bridges to compare across organisations/disciplines.                                         |
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:3 - Solution Overview — The context‑local CreativitySpace
...
* A **system** (person, team, service) performs **`U.Work`** under a role ([[00_A.02 - Role Taxonomy|A.2]]).
...
with **scale**/**unit** metadata from **MM‑CHR** ([[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]]), and Context‑specific **measurement methods** bound by **MethodDescription**.

**DesignRunTag split ([[00_A.04 - Temporal Duality & Open-Ended Evolution Principle|A.4]]):**

* **Design‑time**: score **concepts** or **specs** against **surrogate value models** and **priors**; record **assumptions** (USM scopes; [[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]).
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:4 - Vocabulary (CHR terms & D‑stubs)

> Names are **context‑local**; below are kernel terms. Roles like “Designer/Reviewer” are contextual ([[00_A.02 - Role Taxonomy|A.2]]). **Documents don’t act** (A.7/A.12); they are **evaluated**.
...
6. **`U.CreativityProfile`** *(D).* The **coordinate tuple** in `U.CreativitySpace` with provenance to the above inputs and **USM scopes**.
   **Conformance (CP‑1):** profile **must** include **scales/units**, **scopes**, **confidence bands** ([[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]), and the **edition** of space definitions.

## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:5 - The Core Characteristics (kernel nucleus)

Each characteristic is specified per **MM‑CHR ([[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]])** with: **name**, **intent**, **carrier**, **polarity**, **scale type**, **measurement template**, **evidence**, **scope (USM)**, and **didactic cues**. *Context profiles MAY add characteristics; kernel characteristics MAY NOT be removed without a Bridge.*

### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:5.1 - `Novelty@context` — “How unlike the known set is this?”
...
### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:5.2 - `Use‑Value` *(alias: `ValueGain`)* — “What good did this add under our objective?”
...
### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:5.3 - `Surprise` — “How improbable under our learned world?”
...
* **Scope.** Training slice declared as **ContextSlice**; Bridges penalise **R** (trust), not the value itself ([[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]).
* **Didactic cues.**

  * **Novelty vs Surprise:** high novelty under one kernel may be low surprise under a broad prior; publish both.

### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:5.4 - `ConstraintFit` — “Did it honour the non‑negotiables?”
...
* **Scope.** Constraints are **context‑local**; Cross‑context requires **Bridge**; waivers are **SpeechAct Work** with RSG gates ([[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]).
...
### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:5.5 - `Diversity_P` *(declared retained-set / portfolio-level)* — “Are we exploring the space?”
...
* **Alignment note.** The **illumination/coverage** view corresponds to *IlluminationScore* used by **[[02_B.05.02.01 - Creative Abduction with NQD|B.5.2.1]] NQD‑Generate**; no separate characteristic is introduced here—measure it as part of `Diversity_P`.
...
**Lexical special case ([[00_F.18 - Local-First Unification Naming Protocol|F.18]] naming).**
For **lexical CandidateSets** used by Name Cards ([[00_F.18 - Local-First Unification Naming Protocol|F.18]]), **Diversity_P SHALL be computed over head-term families, not over raw strings**. Variants that share the same lexical head (e.g., “Reference plane”, “Plane of reference”, “Planar reference”) **MUST** be treated as one family for coverage and distance; only candidates with distinct heads contribute to lexical Diversity_P. This aligns lexical use of Diversity_P with `FamilyCoverage` / `AliasRisk` and prevents inflating diversity by near-synonyms of a single head.

### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:5.6 - `AttributionIntegrity` — “Did we credit sources and licences correctly?”
...
* **Default role.** `AttributionIntegrity` is **measurable but non‑dominant**. It MAY serve as a **policy filter/tie‑break** ([[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]]). If certain attribution duties are **must‑constraints**, they belong to **ConstraintFit** (Norm‑CAL) and act as **eligibility gates**. It is **not** part of the default dominance set.
* **Dominance & gating note (normative).** `AttributionIntegrity` is a measurable **Characteristic**; it is **not** in the default dominance set. Contexts MAY use it as a **filter** or **tie‑break** via policy ([[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]]). Legal/ethical **must‑fit** checks live in **ConstraintFit** (Norm‑CAL); failing those blocks eligibility **before** dominance.

### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:5.7 - `EffortCost` *(advisory)* — “What did it take?”
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:6 - Conformance Checklist (first tranche)
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:7 - Manager’s Quick‑Start (apply in 5 steps)
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:8 - Archetypal Grounding (three domains)
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:9 - Anti‑Patterns (fast fixes)
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:10 - Relations

* **[[00_A.02 - Role Taxonomy|A.2]] Role & [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] Run‑alignment.** Creative **Work** is performed by **systems in roles**; outcomes are **epistemes**. Creativity is **measured by `U.Evaluation`**, not “done by a document.”
* **[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] Trust/Assurance.** Coordinates carry **confidence bands**; Bridges lower **R** by **CL**. Evidence roles ([[04_A.02.04 - U.EvidenceRole|A.2.4]]) bind datasets/benchmarks used in measurements.
...
* **[[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] USM (Scope).** All measurements sit on **ContextSlices**; `G‑ladder` is explicitly **not** used ([[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]] follows [[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]’s set‑valued scopes).
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:11 - Authoring Aids (didactic cards)
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:12 - CSLC recap and the Creativity CharacteristicSpace
...
> **Strict Distinction ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]) reminders.**
...
### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:12.1 - The Creativity CharacteristicSpace (CHR‑SPACE)
...
| **Use‑Value**            | Benefit vs a **declared objective**                          | `U.Episteme` value or `U.Evaluation` | Ordinal (Fail/Partial/Pass) or scalar KPI          | [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] Evidence & `U.Evaluation`      |
...
> **Locality.** **Every characteristic is context‑local** (e.g., **Novelty\@context**). Cross‑context claims **must** use a **Bridge** and record **CL** penalties ([[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]). No global novelty.

### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:12.2 - Context extensions & policy‑level characteristics (non‑kernel)
...
> **Compatibility note.** This split removes duplicate “core lists” and aligns [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]] with **[[02_B.05.02.01 - Creative Abduction with NQD|B.5.2.1]] NQD** and **C.16/A.17–[[00_A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC-KERNEL)|A.18]]**: the **kernel nucleus** captures creativity *qualities*; the items above instrument Work, policy, or declared retained-set shaping without renaming `Front`, `Archive`, or `Shortlist`.

### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:12.3 - Scale choices (CSLC discipline)
...
* *Novelty\@context.*
  Coordinate = `1 − max_similarity(candidate, corpus)` with a declared encoder (text, graph, CAD). Unitless in \[0..1]. Document encoder & corpus freeze ([[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] Evidence Graph Ref).
* *Use‑Value.*
...
### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:12.4 - Metric templates (normative kernels + manager‑ready variants)
...
#### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:12.4.1 - Templates (kernel definitions)
...
* **scale:** ratio \[0..1].
* **EvidencePin:** `{ReferenceSetId, EncoderId, Version}`; frozen by [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]].
...
#### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:12.4.2 - Manager’s quick checks (plain‑language adapters)
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:13 - Novelty & transfer are **context‑local** (Bridges mandatory)

**Rule N‑1 (Locality).** `Novelty@context` is defined **only** within its `U.BoundedContext`. **Never** compare scores across Contexts without an **Alignment Bridge** ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]).
...
**Rule N‑3 (Penalty to R, not to G).** Cross‑context novelty **does not** change scope **G**; it **reduces R** (reliability) by the **CL penalty** ([[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]), unless validated by pilot Work in the target Context.
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:14 - Anti‑Goodhart guard (use creativity metrics safely)
...
### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:14.1 - Guard‑rails (normative)

* **G‑1 Paired appraisal.** **Never** assess **Novelty** in isolation; pair it with **Use‑Value** or **ConstraintFit** to avoid proxy myopia
* **G‑2 Frozen references.** Novelty requires **frozen corpus + encoder**; changes create a **new edition** and **RSCR** rerun. Portfolio-publication heuristics and selection heuristics are **policy-level** (see **[[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]]**); do not “reward” Illumination beyond its role as a report-metric.
...
### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:14.2 - Conformance Checklist — **CC‑C17‑M (metrics & guards)**

| ID             | Requirement                                                                                                                            | Practical test                                                              |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| **CC‑C17‑M.1** | Each metric instance **MUST** cite its **Context**, **edition**, and **evidence hooks** (corpus/encoder, acceptanceSpec, constraint set). | Scorecard lists `ContextId`, `Edition`, and hook ids resolvable via [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]]. |
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:15 - Worked mini‑cases (engineer‑manager focus)
...
### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:15.1 - Case A — Hardware ideation sprint (manufacturing design)
...
### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:15.2 - Case B — Data‑science hypothesis generation (health analytics)
...
### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:15.3 - Case C — Product squad reframing (software UX)
...
### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:15.4 What these cases illustrate (tie‑backs)
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:16 - Working examples

### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:16.1 - Software (algorithmic/architectural ideation)
...
* Evidence prior at **L0** ([[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]) until benchmarks exist; regression tests act as **ObserverRole** evidence.

**Context card (one screen).**

* Γ\_bundle: Cost = sum; ConstraintFit = AND; Novelty = subadditive; Evidence = min (chain) / SpanUnion (indep).

### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:16.2 - Hardware (mechanical/electro‑mechanical concepting)**
...
**Context card.**
* Γ\_bundle: Hazard = max; ConstraintFit = AND; Cost = sum+coupling; Evidence = min on chain; Scope via **WorkScope** ([[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]).

### [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:16.3 - Policy design (rules/standards/programs)
...
**Context card.**
* Γ\_bundle: EthicalRisk = max; ConstraintFit = AND (legal & operational); Cost = sum; Evidence = min/SpanUnion; Scope = ClaimScope ([[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]).

## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:17 - Consequences & fit (for engineer‑managers)
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:17.1 - Cross-scale characteristics need one visible distortion account
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:17.2 - `Use-Value` is not the whole `Q-set` by default
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:18 - Relations

* **Builds on**: [[00_B.01 - Universal Algebra of Aggregation (Γ)|B.1]] Γ‑algebra (WLNK/COMM/IDEM/MONO), [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] Trust & Assurance (F–G–R, CL), [[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] USM (Claim/Work scopes), [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] Evidence Graph Referring.
* **Coordinates with**: [[00_A.02 - Role Taxonomy|A.2]] Role suite (Observer/Evidence roles for probes), [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] (Work & plans for probes), [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]] MM‑CHR (scale polarity & units). **[[00_C.18 - Open-Ended Search Calculus (NQD-CAL)|C.18]] NQD-CAL** (generation/illumination operators Γ_nqd.\*) and **[[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]] E/E-LOG** (policies, selection, and declared retained-set rules). This CHR remains measurement-only.
* **Defers to**: [[00_F.09 - Alignment & Bridge across Contexts|F.9]] Bridges for Cross‑context transfers; D‑cluster for ethical/speech‑act gates.

## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:19 - Quick reference cards (tear‑out)
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:20 - Conformance Checklist (pattern‑level, normative)
...
Exploration/exploitation **MUST** be expressed as a **dynamics on the declared retained-set measure(s)** (e.g., exploration share as a function of marginal value of information), *not* as a prescriptive budget recipe. Objective, constraint, and decision-policy statements belong to Decsn‑CAL / [[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]]; [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]] may cite them, but does not own or restate them.
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:21 - Worked‑Context Handbooks (concept cards, not runbooks)
...
These cards are *conceptual fixtures*; **Tooling** may implement them, **Pedagogy** may teach them, but **[[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]** only standardises their content as **thinking supports**.

## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:22 - Placement sanity‑check across the pattern language *(avoid scope creep)*

* **MM‑CHR ([[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]]):** defines **Characteristic/Scale/Unit/Measure** and the *characterisation discipline*. **All** CS dimensions live there; [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]] **uses** them, never re‑defines scales.
* **A.CHR‑SPACE ([[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]]):** exports **CharacteristicSpace & Dynamics hooks**; [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]] is a **Contexted specialisation** for creative reasoning (profiles / retained sets / selection).
* **Decsn‑CAL ([[00_C.11 - Decision Theory (Decsn-CAL)|C.11]]):** defines **objective functions, constraints, preference orders, utility proofs**, and the **search–exploit dynamics** as decision policies. [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]] only **names** the hooks (objective, rule), keeps policy math out.
* **KD‑CAL ([[00_C.02 - Epistemic holon composition (KD-CAL)|C.2]]) & [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (Trust):** carry **evidence provenance**, **assurance** and **congruence penalties (CL)** for Cross‑context reuse. [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]] requires anchors; it does not invent confidence calculus.
* **Compose‑CAL ([[00_C.13 - Constructional Mereology (Compose-CAL)|C.13]]):** governs **set/union/slice** aggregation; a declared retained set is a **Γ\_m.set** over profiles; frontier is derived **without** ad‑hoc geometry.
* **[[00_B.04 - Canonical Evolution Loop|B.4]] Canonical Evolution Loop:** where *Run→Observe→Refine→Deploy* sits. [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]] supplies the **view** in which refinement is judged.
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:23 - Anti‑patterns & canonical rewrites (conceptual hygiene)
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:24 - Minimal didactic cards (one screen each)
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:25 - Consequences (informative)
...
| **Method independence**    | Any tooling can implement the cards; [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)]] remains a conceptual API for thought.                                                   |
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:26- Open questions (non‑normative, research hooks)**
...
## [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]:End
