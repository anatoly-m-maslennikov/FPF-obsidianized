...
## [[00_A.02 - Role Taxonomy|A.2]]:1 - Problem frame

Pattern [[00_A.01 - U.Holon, U.System, and U.Episteme|A.1]] established the **substantial** characteristic of the core (`Entity → Holon → {System, Episteme, …}`), cleanly separating identity from structure and aggregation. The present pattern introduces the **functional** characteristic: how a holon participates in purposes **within a bounded context** and for some interval. This extends the early sketch of [[00_A.02 - Role Taxonomy|A.2]] and tightens its alignment with [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] (Strict Distinction): roles are *not* parts and *not* behaviours; they are contextual **masks** that a holon wears while behaviours are handled by **Method**/**Work**.

## [[00_A.02 - Role Taxonomy|A.2]]:2 - Problem
...
## [[00_A.02 - Role Taxonomy|A.2]]:3 - Forces
...
## [[00_A.02 - Role Taxonomy|A.2]]:4 - Solution
...
### [[00_A.02 - Role Taxonomy|A.2]]:4.1 - S‑level definitions (normative)

* **`U.Role`** — a **context-bound** capability/obligation schema that a holon **may bear (play)** for a time interval. A role has **no structural parts** (it does not participate in [[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] `partOf`) and **no resource deltas** of its own. Role refinement/bundling is expressed via in‑Context relations (`≤`, `⊥`, `⊗`) rather than mereology. *(A7 guard)*
* **`U.RoleAssignment`** — a first-class assignment record recording that a holon **bears (plays)** a role **in** a bounded context over an optional **Window**. Keep the signature aligned with **[[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]] Role Assignment Standard**; governance metadata (authority/justification/provenance) is captured via `U.RoleAssigning` and the evidence graph ([[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]).
...
### [[00_A.02 - Role Taxonomy|A.2]]:4.2 - Temporal & behavioural alignment
...
### [[00_A.02 - Role Taxonomy|A.2]]:4.4 - Taxonomic frame (within a context)
...
## [[00_A.02 - Role Taxonomy|A.2]]:5 - Archetypal Grounding (Tell–Show–Show: System / Episteme)
...
## [[00_A.02 - Role Taxonomy|A.2]]:6 - Bias-Annotation
...
## [[00_A.02 - Role Taxonomy|A.2]]:7 - Authoring guidance (for engineers and leads)
...
|             **CC‑A2.6** | If two roles are declared **incompatible inside the same context**, a bearer SHALL NOT hold them over **overlapping** spans.                                                                                                                                                         | Check the context’s role‑compatibility grid; if overlaps exist, either split the Work by `PhaseOf` or change staffing ([[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases]]; [[00_B.01 - Universal Algebra of Aggregation (Γ)]].4/Γ\_time).                                                  |
|             **CC‑A2.7** | For any **Work** item, `performedBy` MUST reference the **concrete RoleAssignment** of the performer, and its window MUST cover the Work’s window. | Assert `performedBy(Work) = RA` and `RA.window ⊇ window(Work)`; split Work or update assignments if the performer changes mid‑window ([[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment]], [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)]]). |
|             **CC‑A2.8** | Every **Method** bound to a role SHALL be `isDescribedBy ≥ 1` **MethodDescription** (`U.Episteme`) and every **Work** SHALL be `isExecutionOf` exactly one **MethodDescription** version.                                                                                                          | If a Work lacks `isExecutionOf`, or a Method lacks `MethodDescription`, the audit fails ([[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)]]; [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] evidencing hook).                                                                                |
...
|             **CC‑A2.9** | **Evidence** for claims about roles and execution MUST anchor to **symbol carriers** (SCR/RSCR); self‑evidence is forbidden.                                                                                                                                                         | A role effectiveness claim without SCR/RSCR or with cyclic provenance fails ([[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]]).                                                                                                           |
|            **CC‑A2.10** | When a Role assignment implies **order** or **temporal** structure, the pattern SHALL use **Γ\_ctx**/**Γ\_time** rather than overloading role edges.                                                                                                                               | If argument order matters, use Γ\_ctx folds and record OrderSpec; version/evolution goes via Γ\_time ([[03_B.01.03 - Γepist - Knowledge-Specific Aggregation]] §4.5).                                                                            |
|            **CC‑A2.11** | Use of legacy nouns “creator/actor/agent” in Core text is prohibited unless they are explicitly typed as **roles** with bearers; the term **“Transformer”** is a local alias, **not** a type.                                                                                        | Scan for bare nouns; replace with “system bearing TransformerRole” or define an alias in the Glossary ([[00_A.07 - Strict Distinction (Clarity Lattice)]] canonical rewrites; [[00_E.10 - Unified Lexical Rules for FPF]] registers).                                               |
...
> **Note.** CC‑A2.2 aligns with **[[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] Role‑domain guards** (“behavioural roles’ domain = system; epistemes bear non‑behavioural roles only”).

## [[00_A.02 - Role Taxonomy|A.2]]:9 - Common Anti-Patterns and How to Avoid Them
...
4. **“Context leakage.”**
   ...
   ✓ *“`Pluto#Planet:Early20thCenturyAstronomy`; `Pluto#DwarfPlanet:IAU_2006_Definition`.”* No contradiction—different bounded contexts. (Illustrative of `U.RoleAssignment` semantics carried forward from the [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]].)
...
## [[00_A.02 - Role Taxonomy|A.2]]:10 - Consequences

| Benefit                     | Why it matters                                                                                                       | Trade‑off / Mitigation                                                                       |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| **Category‑error immunity** | Clear firewall between **identity** (holarchies) and **function** (roles) prevents mixing “parts” with “masks”.      | Slight modelling overhead; templates provide checklists ([[00_A.07 - Strict Distinction (Clarity Lattice)]], [[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases]]).                         |
...
| **Cross-context pluralism** | Same bearer can hold different roles across contexts without contradiction; differences are explicit in the assignment. | Requires declaring the **bounded context**; [[00_E.10 - Unified Lexical Rules for FPF]] eases the ceremony with registers/aliases.  |
...
5. **Knowledge ecosystems & safety cases (2018–2025).** Modern assurance relies on **traceable evidence** and conservative integration (no “truth averaging”): our [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] anchors (SCR/RSCR) and Γ\_epist’s **weakest‑link** fold implement that discipline and forbid self‑evidence.

> Summing up: post‑2015 science and engineering converge on **roles as contextual capabilities**, **externalised control**, and **traceable evidence**. [[00_A.02 - Role Taxonomy|A.2]] codifies these insights in a substrate‑neutral way, keeping the Core small yet powerful.

## [[00_A.02 - Role Taxonomy|A.2]]:12 - SoTA-Echoing (post‑2015 alignment, informative)

| Claim ([[00_A.02 - Role Taxonomy]] need) | SoTA practice (post‑2015) | Primary source (post‑2015) | Alignment with [[00_A.02 - Role Taxonomy]] | Adoption status |
| --- | --- | --- | --- | --- |
...
## [[00_A.02 - Role Taxonomy|A.2]]:13 - Relations

* **Builds on:**
  **[[00_A.01 - U.Holon, U.System, and U.Episteme|A.1]] Holonic Foundation** (role/mereology split), **[[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] Strict Distinction** (role ≠ behaviour; episteme ≠ carrier), **[[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] Advanced Mereology** (no roles in holarchies).
* **Specialises / Coordinates with:**
  **[[00_A.13 - The Agential Role & Agency Spectrum|A.13]] Agential Role & Agency Spectrum** (behavioural roles over systems; graded agency), **[[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] Role–Method–Work Alignment** (bindsMethod / isExecutionOf discipline).
* **Constrains / Used by B‑cluster:**
  **[[00_B.01 - Universal Algebra of Aggregation (Γ)|B.1]] Universal Algebra of Aggregation (Γ)** (keep order/time in Γ\_ctx/Γ\_time; keep provenance in Γ\_epist), **[[00_B.02 - Meta-Holon Transition (MHT)- Recognizing Emergence and Re-identifying Wholes|B.2]] Meta‑Holon Transition** (promotion when supervision/closure appears), **[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] Trust & Assurance** (evidence & congruence).
* **Interlocks with E‑cluster (governance & language):**
  **[[00_E.10 - Unified Lexical Rules for FPF|E.10]] Lexical Discipline** (registers, progression-label disambiguation, local aliases like “Transformer”), **[[01_E.05.01 - DevOps Lexical Firewall|E.5.1]] DevOps Lexical Firewall** (ban tooling tokens in Core patterns).
* **Reinforces:**
  **[[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] Evidence Graph Referring** (external transformer; SCR/RSCR), **[[00_A.12 - External Transformer & Reflexive Split|A.12]] External Transformer Principle** (agent externalisation).

## [[00_A.02 - Role Taxonomy|A.2]]:End
