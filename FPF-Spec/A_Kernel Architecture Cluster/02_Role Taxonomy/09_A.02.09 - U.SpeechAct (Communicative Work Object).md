...
> **Status:** Stable
> **Normativity:** Normative (unless explicitly marked informative)
> **Placement:** Part A → **[[00_A.02 - Role Taxonomy|A.2]] Roles & Agency Kernel**
> **Refines:** [[00_A.02 - Role Taxonomy|A.2]] (Role Taxonomy)
> **Builds on:** [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]] (RoleAssignment), [[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] (`Γ_time` / windows), [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] (EntityOfConcern / Description episteme / carrier), [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] (SCR/RSCR carrier discipline), [[01_A.15.01 - U.Work|A.15.1]] (`U.Work`)
...
> [[00_F.18 - Local-First Unification Naming Protocol|F.18]] currently frames `U.SpeechAct` as the “utterance” label in the promise/utterance/commitment triad; [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]] keeps that as **naming intuition** while putting the ontology and conformance discipline in Part A where it can be linted and reused.

## [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:1 — Problem frame
...
* Governance patterns and boundary writing ([[00_A.06 - Signature Stack & Boundary Discipline|A.6]] stack) need **provenance**: “this obligation/commitment/permission was instituted by *that* act”.
...
## [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:2 — Problem
...
## [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:3 — Forces
...
| Separation discipline  | Must preserve [[00_A.07 - Strict Distinction (Clarity Lattice)]] splits: **act object** ≠ **utterance description** ≠ **carrier/traces**.                              |

## [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:4 — Solution
...
### [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:4.1 — Normative definition
...
### [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:4.2 — Minimal structure (normative)
...
* **(SA‑C4) If used for gating/audit, it must be observable.** If a speech act is used as a checklist criterion, guard condition, or provenance hook for a `U.Commitment`, the model **SHALL** include at least one observable handle: `utteranceRefs` and/or `carrierRefs`. When the act is used as evidence, at least one carrier reference **SHOULD** be SCR/RSCR‑resolvable per [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]].
...
### [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:4.3 — `SpeechActRef` discipline (normative)
...
### [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:4.4 — Separation rules with `U.Commitment` and `U.PromiseContent` (normative)

1. **Speech act is not the deontic binding.**
   A speech act may **institute** a `U.Commitment`, but the deontic relation itself is the `U.Commitment` object ([[08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]). Do not encode obligations/permissions inside `U.SpeechAct` as prose; instead, create/point to `U.Commitment` IDs in `institutes.commitments`.
...
### [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:4.5 — Multi-function and multi-party support (normative)
...
## [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:5 — Archetypal Grounding (Tell–Show–Show)

### [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:5.1 — Tell (universal rule)
...
### [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:5.2 — Show #1 (system archetype: change-control approval gates a deployment)
...
### [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:5.3 — Show #2 (episteme archetype: publishing a spec edition without making the spec an agent)
...
## [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:6 — Bias-Annotation
...
## [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:7 — Conformance Checklist (normative)

1. **CC‑[[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]‑1 (Accountable performer).** A normative `U.SpeechAct` record **MUST** identify `performedBy` as an accountable `RoleAssignmentRef` and **MUST NOT** use a specification episteme, interface-description episteme, or document-carried episteme as performer.
2. **CC‑[[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]‑2 (ActTypes declared).** A `U.SpeechAct` record **MUST** include at least one `SpeechActTypeRef` recognized in its judgement context.
3. **CC‑[[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]‑3 (Window explicit).** A `U.SpeechAct` record **MUST** have an explicit `window` (or inherit a window from its parent work record) so freshness and gating can be evaluated.
4. **CC‑[[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]‑4 (Observable when used for gating/provenance).** If a speech act is cited by a checklist/guard or by `U.Commitment.source.speechActRef`, it **SHALL** have at least one `utteranceRef` or `carrierRef` that supports observation/audit in the given context; evidence‑critical uses **SHOULD** anchor at least one carrier via SCR/RSCR per [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]].
5. **CC‑[[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]‑5 (Effects by reference).** If the act is intended to institute/update commitments/roles or statuses, those effects **SHOULD** be referenced in `institutes.*` by stable IDs.
6. **CC‑[[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]‑6 (Bridge-only cross-context use).** If a `SpeechActRef` is interpreted for gating/provenance in a different bounded context than the act’s judgement context, the referencing object **MUST** cite the Bridge/policy that licenses that cross-context interpretation (no “same label implies same force”).

## [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:8 — Common Anti-Patterns and How to Avoid Them
...
## [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:9 — Consequences
...
* Requires recording a small structured object for communicative events; mitigated by allowing minimal stubs that still satisfy CC‑[[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]‑1…4.
...
## [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:10 — Rationale

FPF already relies on communicative acts (approvals, notices, overrides) as operationally meaningful events, but without a kernel object they blur into examples, naming choices, or prose. [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]] anchors speech acts where they belong: as a **Work-kind** with explicit performer, scope, and time, and with disciplined links to utterance surfaces, carriers, and deontic bindings (`U.Commitment`).

This also improves modularity:

* **[[00_F.18 - Local-First Unification Naming Protocol|F.18]]** can remain a **lexical anchor** for naming (why “SpeechAct/utterance” as a label family is useful),
* while **[[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]** carries the ontology and conformance discipline for how speech acts behave as objects and how they connect to commitments and evidence.

## [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:11 — SoTA‑Echoing (informative; post‑2015 alignment)

> **Informative.** Alignment notes; not normative requirements.

* **Adopt — ISO 24617‑2:2020 / multi-dimensional communicative functions.** Modern dialogue‑act standards treat communicative behavior as potentially multi‑functional. [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]] mirrors this by allowing `actTypes` to be a **set** and by supporting shared carriers across multiple acts.
* **Adapt — commitment-based semantics for communication (multi-agent/protocol practice, 2015+).** A pragmatic way to avoid mental-state modeling is to track communication by its **social/institutional effects**, especially on commitments and protocol states. [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]] reflects this via `institutes.commitments` and explicit links to `U.Commitment` without modeling sincerity or intention.
* **Adopt (warning) — illocutionary pluralism in multiparty discourse (2015+).** One utterance commonly performs multiple recognizable functions. [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]] avoids the “single force” trap by permitting multi‑type acts and/or multiple acts sharing the same utterance and carriers.

## [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:12 — Relations

**Uses / builds on**

* Uses **[[01_A.15.01 - U.Work|A.15.1]] (`U.Work`)** for the event/work backbone (performedBy + window + stance).
* Uses **[[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]** for the strict act≠description≠carrier split.
* Coordinates with **[[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]** for scope/window discipline.

**Used by**

* **[[08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]] (`U.Commitment`)** as a concrete target for `source.speechActRef` provenance.
* **[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]] (RSG checklists/guards)** when “presence of authorization/approval act” is a criterion.
* **[[03_A.06.C - Contract Unpacking for Boundaries|A.6.C]] (Contract unpacking)** as the “utterance/instituting act” hook that prevents episteme-as-agent claims and improves provenance.

## [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]:End
