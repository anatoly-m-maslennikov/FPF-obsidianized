...
## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:1 - Purpose & scope (why this exists)
...
## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:2 - Problem frame (what goes wrong without an RSG)
...
**Consequences.** Violations of **Strict Distinction ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]])** and **Didactic Primacy ([[00_E.12 - Didactic Primacy & Cognitive Ergonomics|E.12]])**: ambiguous authority to act, unsafe SoD, and non‑reproducible evaluations.

## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:3 - Core idea (didactic)
...
## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:4 - Minimal vocabulary (this pattern only)
...
* **RCS (Role Characterisation Space).** The **characteristic bundle** that characterises this role in this Context (e.g., *CalibrationAge*, *AuthorizationScope*, *FatigueIndex*, *IndependenceFlag*, *EvidenceFreshness*). *(Defined in [[00_A.02 - Role Taxonomy|A.2]] Role Taxonomy / RoleDescription.)*
...
## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:5 - What an RSG is **not** (guardrails)
...
* **Not a global status set.** RSG lives **inside one Context**; the label *Ready* in another Context is **a different state** unless bridged ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]).
* **Not a log.** RSG is not a history. Histories are **StateAssertions** over Windows; **`U.Work`** is the record of enactments.
* **Not a document-state sequence.** Epistemic role RSGs can *look like* document-state sequences, but they remain **role‑status graphs**; carrier history and carrier replacement stay separate ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]], `U.Carrier`).

## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:6 - Invariants (preview)
...
## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:7 - Formal structure of an RSG (intensional, context‑local)
...
* **`Guard`** — for each transition (and optionally for state maintenance), a **predicate over**:
  ...
  * the role’s **RCS snapshot** at a **Window** (values on named characteristics; see [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]), and
...
2. **RSG‑N2 (Disjoint labels).** State names are **unique** within `(Role, Context)`; reusing global labels (e.g., “Ready”) across contexts is allowed **only** via Bridges ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]).
...
## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:8 - Enactability & Checklist semantics (how a state is *known*, now)

An RSG **does not** determine history; it determines **what counts as being in a state**, and which states **authorize enactment**.

### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:8.1 - State Checklists (description, not the state)

For each `s ∈ S`, the **RoleDescription** ([[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]) includes a **State Checklist** `Checklist(s)` — a **named set of criteria** that can be evaluated at a **Window** to test “**holder is in state `s`**”.
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:8.2 - From Checklist to **StateAssertion** (verdict of `U.Evaluation`)
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:8.3 - Transitions & guards (admission, maintenance, exit)
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:8.4 - The **Green‑Gate Law** (enactment gating)
...
## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:9 - Interaction with role algebra (`≤`, `⊥`, `⊗`) and refinement

### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:9.1 - Specialization (`≤`) — RSG refinement map
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:9.2 - Incompatibility (`⊥`) — state‑aware SoD
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:9.3 - Bundles (`⊗`) — conjunction without product explosion
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:9.4 - Readiness monotonicity across specialization & bundles
...
## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:10 - Guard design (types and discipline)
...
* **RSG‑G5 (Context‑local semantics).** Guard semantics are **scoped to Context**; Cross‑context reuse requires a Bridge (§14 in Part 1/4, [[00_F.09 - Alignment & Bridge across Contexts|F.9]]).
...
## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:11 - Putting it together (one‑screen mental model)

At any **Window**:

1. **RoleAssignment exists** ([[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]): `Holder#Role:Context`.
...
## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:12 - Archetypal RoleStateGraphs (cross‑domain patterns)

Below are **didactic, reusable** RSG skeletons for the three principal **behavioural** role families and for **epistemic/status** roles. Names and criteria are **context‑local**; treat them as **templates** to specialise inside your `U.BoundedContext` ([[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]). For each RSG we list:
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:12.1 - AgentialRole (decision‑capable actor)
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:12.2 - TransformerRole (non‑agential executor of change)
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:12.3 - ObserverRole (measurement actor, incl. SOSA/SSN style)
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:12.4 - Epistemic/status roles (no enactment)

These roles are **status‑only**; **`S_en = ∅`**. They **gate decisions** (e.g., can be cited, can constrain), but can never authorize `U.Work`.

#### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:12.4.1 - NormativeStandardRole
...
#### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:12.4.2 - EvidenceRole
...
#### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:12.4.3 - RequirementRole
...
## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:13 - One‑screen authoring templates (didactic cards)

Keep each RSG **teachable on one screen**. Use the following **notation‑neutral** templates when drafting RoleDescriptions ([[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]).

### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:13.1 - RSG card (per Role, per Context)
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:13.2 - State checklist snippet (per State)
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:13.3 - Specialization refinement map (R' ≤ R)
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:13.4 - SoD focus (⊥) — enactable pairs
...
## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:14 - Cross‑context adjustments (via Bridges, not imports)

RSGs are **context‑local**. When similar roles appear in different Contexts, relate them with an **Alignment Bridge** ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]), never by silently importing state names.

### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:14.1 - State name correspondence (lossy mapping)
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:14.2 - Authorization vocabulary drift (deontic vs operational)
...
> **Payoff.** Bridges keep **local honesty** while enabling **Cross‑context reasoning** with explicit penalties ([[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).

## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:15 - Author conformance (write good RSGs)

When you define or revise an RSG, check these **concept‑level** rules. They are easy to hold in mind; no tooling implied.

**CC‑RSG‑01 (Locality).** State names and meanings are **scoped** to `(Role, Context)`. Reuse across contexts **only via a Bridge** ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]).
...
**CC‑RSG‑04 (Guard discipline).** Guards **gate change**, checklists **recognise state**. Don’t smuggle **task order** into guards; task order belongs to `U.MethodDescription` and [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] work alignment.
...
**CC‑RSG‑10 (Status/behaviour split).** Epistemic/status roles: **`S_en = ∅`**. They gate **decisions**, not **Work**. Behavioural roles require `U.System` holders ([[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]).

## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:16 - Extended grounding across four disciplines
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:16.1 - Clinical surgery (medicine)
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:16.2 - Software operations (SRE)
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:16.3 - Laboratory metrology
...
### [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:16.4 - Governance / compliance
...
## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:17 - Acceptance harness (static conformance)

Author‑facing checks; **notation‑free**, **concept‑level**. Use them when drafting or reviewing an RSG.

**SCR‑[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]‑S01 - Local scope.** Every state name is qualified by `(Role, Context)`. No global states.
**SCR‑[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]‑S02 - Enactability mark.** The set **S\_en** is explicit; each ◉ state is listed.
**SCR‑[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]‑S03 - Observable checklists.** Each state has a Checklist of **observable** predicates (Observation / Evaluation / SpeechAct / Work evidence).
**SCR‑[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]‑S04 - Green‑Gate wiring.** Every `MethodDescription` step that names the Role either (a) names its ◉ state(s) or (b) relies on the default “any ◉” policy; the RSG declares which.
**SCR‑[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]‑S05 - Guard discipline.** Guards only **gate transitions**; they do not encode task order.
**SCR‑[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]‑S06 - SoD by state.** Incompatibilities (⊥) are declared over **states** (or “any ◉”), not over bare role names.
**SCR‑[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]‑S07 - Specialisation entailment.** For every `R' ≤ R`, a refinement map `π: S(R')→S(R)` is provided; each mapped pair has an entailment note (why `Checklist_R' ⇒ Checklist_R`).
**SCR‑[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]‑S08 - Human scale.** `|S| ≤ 7` unless a one‑line didactic rationale is recorded.
**SCR‑[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]‑S09 - Status‑only roles.** If `S_en=∅`, the Role is explicitly tagged **status‑only**; it cannot open the Green‑Gate.
**SCR‑[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]‑S10 - Bridge discipline.** Any cross‑context reuse is via an Alignment Bridge ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]) with recorded `CL` and losses; no silent imports.

## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:18 - Regression harness (evolution checks)

Use when **adding/removing states**, **changing criteria**, or **bridging** across contexts.

**RSCR‑[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]‑R01 - State churn impact.** For every added/removed/renamed state, list affected `MethodDescription` steps and `Work` validators; confirm the Green‑Gate policy remains decidable.
**RSCR‑[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]‑R02 - Entailment stability.** When `R' ≤ R` changes, update the `π` map and re‑justify entailments; fail the check if any previously valid entailment breaks.
**RSCR‑[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]‑R03 - SoD coverage.** After edits, recompute the set of **enactable pairs**; verify declared ⊥ still blocks all intended conflicts and no longer blocks permitted cases.
**RSCR‑[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]‑R04 - Evidence freshness.** If any checklist predicate uses **age/freshness**, ensure default Windows are documented and existing state assertions re‑evaluate accordingly.
**RSCR‑[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]‑R05 - Bridge congruence drift.** If a Bridge maps states with `CL=k`, and either side’s checklist changes, revisit the mapping; **do not** keep `CL` unchanged by default—raise or lower with a short rationale.
**RSCR‑[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]‑R06 - Status/behaviour split.** Verify behavioural roles still require `U.System` holders ([[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]); status‑only roles still have `S_en=∅`.
**RSCR‑[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]‑R07 - One‑screen rule.** If cumulative edits push the RSG beyond one screen, split states or tighten criteria; record a one‑line teaching rationale if you must exceed.

## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:19 - Common failure modes (and quick remedies)
...
## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:20 - Didactic script (90 seconds): how [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]] ties to [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]] & [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]

> \*“A role assignment says **who wears which mask where** ([[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]). The **RoleStateGraph** says **when that mask is actually wearable**. Each role’s RSG is a **small named state space** with **checklists** for each state. Some states are **enactable** (◉): they open the **Green‑Gate** for `Work`. Others are **status‑only**: they gate decisions, never execution.
>
> A **RoleDescription** ([[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]) is where you publish the role’s **RCS** (characteristics), its **RSG** (states + checklists + guards), and any **role algebra** (≤, ⊥, ⊗) specific to your context.
>
...
## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:21 - Relations (quick pointers)

* **Builds on:** [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]] `U.RoleAssignment` (the binding that can assert states); [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]] `U.RoleDescription` (the carrier of RSG); [[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] (Context discipline).
* **Enables.** [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] (Role‑Method‑Work Alignment via Green‑Gate); [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (Trust penalties when crossing Bridges with lower `CL`).
* **Interacts with.** D‑cluster deontics (speech‑acts gate **Authorized**‑like states for agential roles); [[00_F.09 - Alignment & Bridge across Contexts|F.9]] (state‑level alignment across contexts).

## [[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]:End
