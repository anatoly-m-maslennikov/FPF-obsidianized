...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:0.1 - Kind Settlement
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:0 - Use This When
...
**Not this pattern when.** If the current EntityOfConcern is the promised content, use [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]; if it is the communicative act that instituted or revoked the commitment, use [[A_Kernel Architecture Cluster/02_Role Taxonomy/09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]; if it is a gate or admissibility claim, use the gate or boundary pattern; if it is performed work, use [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/01_A.15.01 - U.Work|A.15.1]].
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:1 - Problem frame
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:3 - Forces
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:4 - Solution
...
* how it is used as the canonical payload for **D-quadrant** claims ([[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/02_A.06.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work-Effects)|A.6.B]]),
* and what must be stated for a commitment to be considered **auditable**.

### [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:4.1 - Normative definition
...
Per [[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]], a `U.Commitment` is **not** the text that states it: it is an object that is typically **instituted by** (and recorded via) one or more **speech acts and utterance descriptions** and may be carried by utterance carriers or publication carriers.

### [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:4.2 - Minimal structure (normative)
...
PromiseContentRef ::=
  ObjectIdRef
  // MUST resolve to a `U.PromiseContent` object ([[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]). (Some chapters may call this a “promise content”.)
...
SpeechActRef ::=
  ObjectIdRef
  // MUST resolve to a `U.SpeechAct` Work occurrence ([[A_Kernel Architecture Cluster/02_Role Taxonomy/09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]).
...
### [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:4.3 - Interaction rules (normative)
...
5. **Commitment changes are explicit (no silent mutation).**
   When a commitment is updated, narrowed, broadened, superseded, or revoked, the change **SHOULD** be represented as a new `U.Commitment` (new ID) and an instituting `U.SpeechAct` ([[A_Kernel Architecture Cluster/02_Role Taxonomy/09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]]) that references the affected commitment IDs (e.g., via `U.Commitment.source.speechActRef` and a status/supersession claim), rather than editing a published commitment in place without an auditable change record.

### [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:4.4 - Canonical use in boundary claim registers (recommended)

When using the [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/00_A.06 - Signature Stack & Boundary Discipline|A.6]] stack, represent each **D-quadrant** atomic claim as a `U.Commitment` payload with:
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:5 - Archetypal Grounding (Tell–Show–Show)

### [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:5.1 - Tell (universal rule)
...
### [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:5.2 - Show #1 (system archetype: incident response SLO discipline, post‑2015 SRE practice)
...
### [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:5.3 - Show #2 (episteme archetype: protocol specification with behavioural typing motif)
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:6 - Bias-Annotation
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:7 - Conformance Checklist (normative)

1. **CC‑[[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]‑1 (Accountable subject).** A normative `U.Commitment` **MUST** name an accountable `subject` (role assignment, role enactor, or party) and **MUST NOT** use a specification episteme, interface-description episteme, or document-carried episteme as subject.

2. **CC‑[[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]‑2 (Explicit modality).** A normative `U.Commitment` **MUST** specify `modality` as `DeonticModalityToken` (with any RFC-keyword synonyms normalized to it).

3. **CC‑[[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]‑3 (Scope & validity explicit).** A normative `U.Commitment` **MUST** specify `scope` (`U.ClaimScope`) and `validityWindow` (qualification-window policy), or explicitly cite the context policy that supplies defaults (do not rely on “implied defaults”).

4. **CC‑[[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]‑4 (Referents present and by ID).** `referents` **MUST** be non‑empty. If the bound content exists as claim IDs, the commitment **SHOULD** reference those IDs in `referents` rather than restating their content.

5. **CC‑[[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]‑5 (Auditable commitments have hooks).** If the commitment is intended to be auditable, it **SHALL** include `adjudication.evidenceRefs` referencing the evidence claims (typically `E-*`) that make adjudication possible.
6. **CC‑[[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]‑6 (Evidence separation).** If an `E-*` claim is referenced only for measurement/verification, it **SHALL** appear in `adjudication.evidenceRefs` (not in `referents`).

## [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:8 - Common Anti-Patterns and How to Avoid Them
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:9 - Consequences

**Benefits**

* Makes deontic statements **first-class and lintable** (subject/modality/scope/referents/hooks).
* Enables clean integration with boundary claim classification ([[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/02_A.06.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work-Effects)|A.6.B]]) and contract unpacking ([[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/03_A.06.C - Contract Unpacking for Boundaries|A.6.C]]) without embedding ontology in naming patterns.
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:10 - Rationale
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:11 - SoTA-Echoing (informative; post‑2015 alignment)
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:12 - Relations

**Uses / builds on**

* [[A_Kernel Architecture Cluster/02_Role Taxonomy/01_A.02.01 - U.RoleAssignment - Contextual Work-Role Assignment|A.2.1]] for identifying accountable roles vs role-enactors (role assignments).
* [[A_Kernel Architecture Cluster/02_Role Taxonomy/06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] for expressing scope and time/window (`U.ClaimScope`, qualification-window policy).
* [[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] for keeping source “binding” wording distinct from utterance descriptions and carriers.

**Used by**

* [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/02_A.06.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work-Effects)|A.6.B]] (Quadrant D) as the canonical payload shape for deontic statements.
* [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/03_A.06.C - Contract Unpacking for Boundaries|A.6.C]] (Contract Unpacking) as the formal governing pattern for the “Commitment” component of the bundle.
* Part D governance/ethics patterns, when current, for expressing layered, conflicting, multi-authority commitments.

**Coordinates with**

* [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]] (`U.PromiseContent`): services are promise clauses; commitments assign accountable subjects to those clauses.
* **[[A_Kernel Architecture Cluster/02_Role Taxonomy/09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]] (`U.SpeechAct`)**: `U.Commitment.source.speechActRef` points to the instituting communicative work occurrence when provenance matters.
* [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/01_A.15.01 - U.Work|A.15.1]] (`U.Work`) and evidence patterns: adjudication hooks refer to evidence in work, not to text.

## [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]:End
