...
**Builds on:** [[00_A.01 - U.Holon, U.System, and U.Episteme|A.1]] **Holonic Foundation**, [[01_A.01.01 - U.BoundedContext Semantic Frame|A.1.1]] **`U.BoundedContext`**, [[00_A.02 - Role Taxonomy|A.2]] **Role Taxonomy**.
**Coordinates with:** [[00_A.13 - The Agential Role & Agency Spectrum|A.13]] **Agential Role & Agency Spectrum**, [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] **Role–Method–Work Alignment**, [[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **D.CTX (Context discipline)**, [[04_E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]] **Strict Distinction**.
**Lexical discipline.** *Context* ≡ `U.BoundedContext` ([[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]). *Appointment* is **colloquial only**; the canonical term in this specification is **Role Assignment** (see **CC‑LX‑1**).
...
## [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:1 - Problem frame
...
## [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:2 - Problem
...
## [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:3 - Forces

| Force                            | Resolution in this pattern                                                                                                        |
| -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| **Universality vs locality**     | One mechanism (`U.RoleAssignment`), but every meaning is **context‑local** (Context); cross‑context sameness only via **Bridge** ([[00_F.09 - Alignment & Bridge across Contexts]]). |
...
| **Specification vs description** | Role **RCS/RSG** are recorded in **RoleDescription**; upgrade to **RoleSpec** only after a test harness exists ([[04_E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline]]).         |

## [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4 - Solution

### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.1 - Canonical definition (notation‑free)
...
### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.2 - Role Enactment (distinct from the assignment)
...
### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.3 - Role Characterisation Space (RCS) & Role State Graph (RSG)

These are **intensional facets** of a **Role**, not containers “inside” the Role. They are **recorded in** the **RoleDescription** (or **RoleSpec** once harnessed), per [[04_E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]].
...
### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.4 - Shorthand & reading
...
### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.5 - No role chains (use algebra, not badge‑of‑badge)
...
Taxonomic inheritance between role names is declared explicitly in the Context’s role algebra. For example, if (per [[00_A.13 - The Agential Role & Agency Spectrum|A.13]]) your Context treats every transformer as a kind of agent, state:
...
### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.6 - Eligibility across holon kinds (normative matrix)
...
#### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.6.1 - Holder kinds (recap)
...
#### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.6.2 - Role‑kind × holder matrix
...
### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.7 - Role algebra within a single Context (meaning relations)

Role algebra

The in‑Context role algebra relates **role types** inside **one** `U.BoundedContext`. It is **not** mereology. Its operators (`≤`, `⊥`, `⊗`) is specified normatively in **[[07_A.02.07 - U.RoleAlgebra- In-Context Role Relations|A.2.7]] `U.RoleAlgebra`**.

[[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]] relies on it for (i) `requiredRoles` substitution checks (`≤`), (ii) separation‑of‑duties validation (`⊥`), and (iii) conjunctive bundles (`⊗`), but does not restate the operator semantics here.

### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.8 - Time & state transition calculus (windows, RSG, enactability)
...
#### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.8.1 - Windows and overlap
...
#### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.8.2 - RSG gating of enactment

Each Role’s **RoleDescription/RoleSpec** defines an **RSG** with named states; some states are **enactable**.

* **Delegation.** Normative author‑facing requirements for `U.RoleStateGraph` structure (including enactability marking and per‑state checklists) and the shape of `StateAssertion` evidence are defined in **[[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]**; [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]] only relies on the resulting enactment gate.
...
#### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.8.3 - Suspensions, revocations, probation
...
* **Discipline ([[05_A.02.05 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]).** RSG transitions are explicit; no implicit “back to Active”.

#### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.8.4 - Typical temporal patterns (didactic)
...
### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.9 - Integration with [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] (Role–Method–Work alignment)
...
#### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.9.1 - Design‑time Standard (inside `U.MethodDescription`)
...
#### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.9.2 - Run‑time check (inside `U.Work`)
...
#### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.9.3 - Evaluation & acceptance (link to services & deontics)
...
#### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.9.4 - Planning & scheduling (design‑time “who will enact”)
...
### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.10 - Cross‑Context bridges in practice (with CL penalties)

Cross‑Context role substitution is **Bridge‑only** and is specified in **[[00_F.09 - Alignment & Bridge across Contexts|F.9]]** (with CL/waiver regimes) and **[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]** (CL‑penalty routing).

[[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]’s only rule is *no substitution by label*: any “`Role_B@B` satisfies `Role_A@A`” claim used for checking or enactment **MUST** cite an explicit Bridge (direction, CL, loss notes) and **MUST NOT** override in‑Context `≤`, `⊥`, or `⊗`.

### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:4.11 - Everyday pattern snippets (didactic moves)
...
## [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:5 - Archetypal Grounding (three disparate arenas)
...
### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:5.1 - Industrial operations (welding cell)
...
### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:5.2 - Software & cloud operations (continuous delivery / SRE)
...
### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:5.3 - Knowledge governance (standards & requirements)
...
## [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:6 - Bias-Annotation
...
## [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:7 - Conformance Checklist (normative)

### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:7.1 - SCR (compact, memorable)
...
2. **CC‑CTX‑2.** Authors and validators **MUST NOT** assume cross‑Context equivalence by label; any cross‑Context relation used for substitution or checking **MUST** be represented only in **Bridges** ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]).
...
See **[[07_A.02.07 - U.RoleAlgebra- In-Context Role Relations|A.2.7]] `U.RoleAlgebra`** (CC‑ALG‑1, 2, 3).
...
### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:7.2 - RSCR (regression harness)
...
## [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:8 - Common Anti-Patterns and How to Avoid Them
...
| **A6** | **Edition blur**      | Context “ITIL” with no version             | Sense slippage                     | Context card must carry edition ([[00_E.10 - Unified Lexical Rules for FPF]].D1/F.1)                      |
| **A7** | **Bridge‑by‑name**    | Equating roles across Contexts by label    | Cross‑context drift                | Use [[00_F.09 - Alignment & Bridge across Contexts]] Bridge with CL & loss notes                                |

## [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:9 - Consequences
...
### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:9.1 - Teaching distillation (60‑second recap)
...
### [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:9.2 - So what? Adoption test (1 minute)

If a team claims to “use [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]”, a random audit sample should pass all of these in minutes:
...
## [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:10 - Rationale

* **Strict Distinction ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]).** Keeps **identity** (Holon) separate from **assignment** (RoleAssignment), **behaviour** (Method/Work), and **knowledge** (Episteme).
* **Ontological Parsimony ([[00_A.11 - Ontological Parsimony (C-5)|A.11]]).** One universal binding, three tiny in‑Context relations (`≤, ⊥, ⊗`), no global role types.
* **Universal core ([[00_A.08 - Universal Core Principle (C-1)|A.8]]).** The same mechanism works across systems (machines, software, teams) and epistemes (standards, requirements), demonstrated in §5.
* **Lexical discipline ([[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] & [[04_E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]]).** Roles are **context‑local**; descriptions (RCS, RSG) are **descriptions of intensional roles**, not the roles themselves.
* **Assurance posture.** Windows + RSG + StateAssertions make authorisation explicit and reviewable; Bridges + CL make cross‑Context reuse explicit and risk‑graded ([[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).

## [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:11 - SoTA-Echoing (notes)

| Topic this pattern leans on | Post‑2015 anchor (example) | How [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment]] uses it | Status |
| --- | --- | --- | --- |
...
## [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:12 - Relations

**Builds on / depends on**

* **[[00_A.01 - U.Holon, U.System, and U.Episteme|A.1]] Holonic Foundation** — `U.Holon` (holders).
* **[[01_A.01.01 - U.BoundedContext Semantic Frame|A.1.1]] `U.BoundedContext`** — the Context of meaning.
* **[[00_A.02 - Role Taxonomy|A.2]] Role Taxonomy** — role kinds for Systems vs Epistemes; context‑local naming.
* **[[07_A.02.07 - U.RoleAlgebra- In-Context Role Relations|A.2.7]] `U.RoleAlgebra`** — in‑Context `≤/⊥/⊗` relations used for substitution, SoD, and bundles.
* **[[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] (D.CTX)** & **[[04_E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]] (Strict Distinction of intensional vs description)** — locality & description discipline.

**Enables / instantiated by**

* **[[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] Role–Method–Work Alignment** — step gating, performer linking, evaluation hooks.
* **[[00_B.01 - Universal Algebra of Aggregation (Γ)|B.1]] Γ‑algebra** — constructors/observers are simply roles enacted by systems.
* **[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] Trust & Assurance Calculus** — CL penalties on Bridges; evidence from StateAssertions.
* **D.2 Multi‑Scale Ethics** — duties attach to roles; SoD encoded via `⊥`.
* **F‑cluster (Unification Method)** — Context definitions ([[00_F.01 - Domain-Family Landscape Survey|F.1]]–[[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]) and Bridges ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]) consumed here.
...
## [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]:End
