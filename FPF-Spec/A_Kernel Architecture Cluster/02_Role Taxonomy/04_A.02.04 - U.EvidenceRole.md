...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:1 - Context and intent
...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:2 - Problem
...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:3 - Forces
...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:4 - Solution — Term and definition
...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:5 - Role family and specialisations
### [[04_A.02.04 - U.EvidenceRole|A.2.4]]:5.3 - Causal evidence-role specialisations
...
The corresponding `CausalEvidenceSupportBasis` values are governed by [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]]: `observationalAssociationSupportBasis`, `interventionalActionSupportBasis`, `realizedCounterfactualSampleSupportBasis`, `identifiedCounterfactualEstimateSupportBasis`, and `simulationOnlyCounterfactualOutputBasis`. [[04_A.02.04 - U.EvidenceRole|A.2.4]] only classifies evidence roles held by epistemes; it does not mint a second causal support-basis value set.

What this does not authorize: [[04_A.02.04 - U.EvidenceRole|A.2.4]] does not decide the causal-use question, estimand, identification, or counterfactual sampling realizability; it preserves the evidence-role assignment and the authority-reference boundary so [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] and [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] can judge the causal-use claim without vocabulary laundering.
...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:6 - Clear distinctions (Strict Distinction, litmus tests)
...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:7 - Core invariants (concept level)
...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:8 - Facets and semantics (normative)
...
### [[04_A.02.04 - U.EvidenceRole|A.2.4]]:8.1 - Claim-scope schema

Every `U.EvidenceRole` definition **within a `U.BoundedContext`** **MUST** declare a claim-scope record. This record ties the role’s meaning to the target claim named by value and its claim scope, and aligns with the typed-claim form used in [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:
...
| `assuranceUse`  | `TA` / `VA` / `LA`                 | Declares whether the evidence functions as typing, verification, or validation input ([[01_B.03.03 - Assurance Subtypes & Levels]]).       |
...
### [[04_A.02.04 - U.EvidenceRole|A.2.4]]:8.2 - Timespan and decay
...
When the relevance window closes (`validUntil` reached), the evidence incurs **Epistemic Debt (ED)**. Per [[02_B.03.04 - Evidence Decay & Epistemic Debt|B.3.4]], debt must trigger one of three managed actions:
...
### [[04_A.02.04 - U.EvidenceRole|A.2.4]]:8.3 - Provenance hooks

Each `U.EvidenceRole` **MUST** anchor into the **Evidence–Provenance DAG** ([[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]):
...
* SCR/RSCR anchors ([[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]) are mandatory for all carriers.
...
### [[04_A.02.04 - U.EvidenceRole|A.2.4]]:8.4 - Contribution to assurance

A `U.EvidenceRole` classifies an episteme; its contribution to the target claim’s assurance tuple ⟨F, G, R⟩ is computed in [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] using:
...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:9 - Worked examples

### [[04_A.02.04 - U.EvidenceRole|A.2.4]]:9.1 - Formal line — *Proof as evidence for a theorem*
...
#### [[04_A.02.04 - U.EvidenceRole|A.2.4]]:9.2 - Empirical line — *Sensor calibration as evidence for an accuracy claim*
...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:10 - Conformance checklist (normative)
...
**CC-ER-06 (Provenance)**
Every evidence-role assignment **MUST** anchor into the EPV-DAG ([[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]). For empirical line, `fromWorkSet` must point to external `U.Work`; self-evidence is prohibited.
...
Every binding **SHALL** declare `assuranceUse ∈ {TA | VA | LA}` and, for **empirical** (LA) bindings, expose `timespan/valid_until` and `decayPolicy` so that SCR can report lane‑separated contributions and freshness ([[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).

## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:11 - Anti-patterns and remedies
...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:12 - Operators (conceptual, tooling-agnostic)
...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:13 - Relations

**Builds on:**
[[00_A.02 - Role Taxonomy|A.2]] `U.Role`, [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]] `U.RoleAssignment` (role as mask, binding as assignment), [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] Evidence Graph Referring (EPV-DAG), [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] Trust & Assurance Calculus.

**Coordinates with:**
[[02_A.03.02 - U.MethodDescription- Description Episteme for a Way of Doing|A.3.2]] `U.MethodDescription` (protocols, proof obligations), E.6.1 Epistemic Roles via `U.RoleAssignment` (didactic gateway), [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] when an evidence role is used to support causal-use claims and must stay distinct as observational, interventional, realized counterfactual sample, identified counterfactual estimate, or simulation-only counterfactual output.
...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:14 - Migration notes (quick wins)
...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:15 - Didactic quick cards (engineer-manager ready)
...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:16 - SCR/RSCR audit stubs (assurance scaffolding)
...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:17 - Minimal evidence-role assignment schema (informative)
...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:18 - Memory hooks and acceptance cross-checks (informative)
...
## [[04_A.02.04 - U.EvidenceRole|A.2.4]]:End
