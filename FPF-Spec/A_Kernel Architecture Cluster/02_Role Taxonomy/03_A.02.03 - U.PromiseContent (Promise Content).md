...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:0.1 - Kind Settlement
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:0 - Use This When
...
**Not this pattern when.** If the current EntityOfConcern is the accountable deontic relation, use [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]; if it is the performed delivery work, use [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/01_A.15.01 - U.Work|A.15.1]]; if it is the access point or delivery system, use system and architecture patterns plus [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] service wording repair; if it is contract-bundle unpacking, use [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/03_A.06.C - Contract Unpacking for Boundaries|A.6.C]].

## [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:1 - Problem frame
...
FPF therefore reserves **`U.PromiseContent`** for one kernel meaning: a consumer-facing **promise content** clause. Any other “service” sense MUST be modeled explicitly as `U.System`, `U.RoleAssignment` or principal, `U.MethodDescription`, or `U.Work` inside an appropriate `U.BoundedContext` and, in normative prose, MUST be written with an explicit **facet head phrase** per **[[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] (RPR-SERV)**.
...
**Modularity note.** [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]] defines only the promise-content object (the **promise content**) and its direct links to roles, access specification, acceptance criteria, and work evidence. The multi-facet "service situation" bundle that also names provider principals, systems, access points, commitments, and acts is handled as a precision-restoration lens in **[[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] (`serviceSituation(...)`)**. Contract-talk unpacking and classification of "contract", "SLA", and "guarantee" language is handled by **[[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/03_A.06.C - Contract Unpacking for Boundaries|A.6.C]]**, which applies [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] when service-cluster tokens appear.
...
> Intuition: the **consumer-facing promise clause** is what you advertise and are judged by (`U.PromiseContent`); **work** is what you do to keep that promise; **method description or specification** is how you know what to do. (See [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] for full "service" polysemy unpacking.)
...
Normative prose therefore SHALL treat the bare head noun **service** as **always‑unpack** (PTG=Guarded): every head‑noun occurrence MUST be rewritten to a facet head phrase (promise content, service provider principal, service access point, service delivery system, and so on) or to the correct underlying EntityOfConcern or project-side FPF kind (team, ticket, endpoint host, procedure, work item), per **[[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] (RPR‑SERV)**.

[[E_The FPF Constitution and Authoring Guides/09_10_Unified Lexical Rules for FPF/00_E.10 - Unified Lexical Rules for FPF|E.10]]’s lexical trigger **L-SERV** SHOULD be implemented as “pointer + lint rule” to [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]: the short rule names the hazard, while [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] provides the full rewrite recipe and the facet head phrase set.

## [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:2 - Problem
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:3 - Forces
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:4 - Solution — The unified concept `U.PromiseContent`
...
* **Prose head (normative):** *promise content* (Tech), *service offering clause* (Plain; *service promise clause* acceptable synonym). Both twins retain an explicit **clause** head-kind to avoid act/content ambiguity and to comply with [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] headword governance.

### [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:4.1 - Core structure (minimal fields)
...
U.PromiseContent {
  ...
  unitOfDelivery?: Episteme,           // how delivered units are counted or measured (unit + countingRule; see [[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10)
  version?       : SemVer or Text,
  timespan?      : Interval
}
```

* `promisedOutcomeSpecRef` MUST point to a `U.OutcomeSpec` ([[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10). It is the promise-facing outcome template (work-only, result-only, or composite), not a `U.Work` episode and not an extensional delivered-result referent.
...
* **Internal delivery methods and runbooks are not part of the promise content.** Model them as `U.MethodDescription` and relate them to the clause via `serviceSituation(...)` ([[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]) or explicit context relations; providers retain **Method autonomy**.

### [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:4.1.1 - Promised outcome spec (disambiguation: work vs post-work result)
...
* **promiseOutcomeSpec** → `U.OutcomeSpec` ([[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10), referenced via `promisedOutcomeSpecRef`.
* **promiseOutcome** → an **extensional delivered outcome instance**. It is not a single kernel object; it is the **run‑time reality** that satisfies the outcome spec, understood according to `U.OutcomeSpec.mode`:
  ...
  * `Composite` → the pair: **(delivery Work episode(s), post‑work state)**.
  ...
  FPF points to this extensional delivered outcome instance by citing: (i) the relevant `U.Work` occurrence(s) and (ii) their **Delta anchors** (affected referents plus pre-state and post-state anchors) on the declared state-plane ([[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/01_A.15.01 - U.Work|A.15.1]]:4.2 item 10). Evidence carriers and telemetry are **epistemic witnesses** used to justify those anchors and acceptance verdicts - they are not themselves the delivered outcome.
...
A conforming `U.OutcomeSpec` uses the canonical shape from [[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10.2:
...
U.OutcomeSpec ::= {
  ...
  resultSpec?: {
    ...
    statePlaneRef?: StatePlaneRef,                // where the predicate lives ([[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:3 pins)
...
* **Counting is not part of `U.OutcomeSpec`.** Counting lives on `U.PromiseContent.unitOfDelivery` as the `countingRule` mini‑schema ([[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10.3). Outcome specs say *what* counts as delivery; unit‑of‑delivery specs say *how much* to count and how to avoid double counting.
...
### [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:4.1.2 - Recommended `acceptanceSpec` mini‑schema *(informative, non‑kernel)*
...
* **`Gamma_timePolicyRef`** keeps windowing explicit and non-retroactive ([[F_The Unification Suite (U-Suite)- Concept-Sets, SenseCells & Contextual Role Assignment/10_Status Families Mapping- Evidence, Standard, and Requirement Status/00_F.10 - Status Families Mapping- Evidence, Standard, and Requirement Status|F.10]] and [[F_The Unification Suite (U-Suite)- Concept-Sets, SenseCells & Contextual Role Assignment/12_Service Acceptance-Work Evidence Link/00_F.12 - Service Acceptance-Work Evidence Link|F.12]]): it states whether judgement is per Work episode, per reporting window, per population, etc.
...
### [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:4.2 - What `U.PromiseContent` is **not**

* **Not a provider:** use `System#ServiceProviderRole:Context` `U.RoleAssignment`.
* **Not a deontic commitment:** that is `U.Commitment` ([[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]) referencing the promise content as payload.
* **Not an access point:** addressable "services", servers, desks, or endpoints are `U.System` (see [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]: *service access point* and *service delivery system*).
...
* **Not a scope label:** do **not** use *applicability*, *envelope*, *generality*, or *validity* as names for **scope objects**; declare **Claim scope (G)** or **Work scope** explicitly where needed ([[A_Kernel Architecture Cluster/02_Role Taxonomy/06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]).

### [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:4.3 - Position in the enactment chain

* **Design‑time:**
  The context **declares Claim scope (G)** for acceptance (operating conditions, populations, locales) per [[A_Kernel Architecture Cluster/02_Role Taxonomy/06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]].
...
### [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:4.4 - Didactic card: The service delivery chain (clause → commitment → situation → work → acceptance)

> **Didactic (non‑normative).** This is a one‑screen “map” that stitches the modular pieces together:
> `U.PromiseContent` ([[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]) → `U.Commitment` ([[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]) → provider `U.RoleAssignment` ([[A_Kernel Architecture Cluster/02_Role Taxonomy/01_A.02.01 - U.RoleAssignment - Contextual Work-Role Assignment|A.2.1]]) → *serviceSituation(...)* facet slots ([[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] lens) → `U.Work + carriers` ([[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]) → acceptance verdict ([[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]).
>
...
flowchart LR
  ...
  subgraph LENS["Optional lens ([[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]): serviceSituation(...)"]
...
* `serviceSituation(...)` ([[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]) is a **facet-recovery lens** that names the common "service talk" participants (access spec, access point, delivery system, and delivery method) **without** collapsing them into the clause.
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:5 - Archetypal grounding (engineer‑manager friendly)
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:6 - Mapping the common “service” picture to FPF (didactic bridge)
...
* **Service Level Agreement (SLA)** (obligation-bearing source) -> `U.Commitment` referencing the relevant `U.PromiseContent` (and, where needed, its acceptance specs and evidence specs); use **[[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/03_A.06.C - Contract Unpacking for Boundaries|A.6.C]] Contract Bundle** when packaging "the SLA" as a bundle of commitments, evidence specs, and publication carriers.
* **SLA document or published terms** -> `U.SpeechAct` (promise act or offer act) + the clause carrier (`U.Episteme`), per [[A_Kernel Architecture Cluster/02_Role Taxonomy/09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]] + [[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]].
* **Operating conditions / “where the promise holds”** → `claimScope : U.ClaimScope (G)` (or embedded in `acceptanceSpec`) per [[A_Kernel Architecture Cluster/02_Role Taxonomy/06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]].
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:7 - Conformance Checklist (normative)

**CC‑A2.3‑0 (Prose head phrase).**
In normative prose, an instance of `U.PromiseContent` SHALL be referred to as a **promise content** (or **service offering clause** or **service promise clause**) and SHALL NOT be referenced by the bare head noun *service*. Unqualified *service* usage (and the co-moving cluster *service provider* or *server*) SHALL be unpacked per [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] (RPR-SERV).
...
`providerRole` and (if used) `consumerRole` **MUST** be **role kinds** (see [[A_Kernel Architecture Cluster/02_Role Taxonomy/00_A.02 - Role Taxonomy|A.2]]). Actual performers at run‑time are `U.RoleAssignment`s.
...
If the acceptance criteria mention measurable characteristics (availability, latency, accuracy, cost, safety, etc.), each such characteristic MUST be introduced via the Characterization patterns ([[C_Kernel Extension Specifications/04_16_Measurement & Metrics Characterization (MM-CHR)/00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]] and [[C_Kernel Extension Specifications/13_25_Q-Bundle- Authoring --ilities- as Structured Quality Bundles/00_C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]]): an explicit `U.Characteristic` (with scale and unit plus measurement procedure and evidence carrier), referenced by id rather than only by a bare KPI name.
...
When declared, `unitOfDelivery` **MUST** include a **countingRule** that maps accepted delivery work episodes (`W✓`) to unit counts ([[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10). If omitted, the default is “1 unit per accepted delivery work episode”.

**CC‑A2.3‑7 (No actuals on Promise Content).**
Resource and time **actuals** and incident logs **MUST** attach to `U.Work` only ([[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/01_A.15.01 - U.Work|A.15.1]]). Promise contents carry no actuals.
...
Unqualified head-noun uses of *service* (and the co-moving cluster *service provider* or *server*) in normative prose **MUST** be disambiguated per **[[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] (RPR-SERV)** and its lexical trigger **L-SERV** ([[E_The FPF Constitution and Authoring Guides/09_10_Unified Lexical Rules for FPF/00_E.10 - Unified Lexical Rules for FPF|E.10]]).

**CC‑A2.3‑11 (No mereology).**
Do **not** place a promise content clause in PBS or SBS, or treat it as a part or component. Structural assemblies live in PBS and SBS; the promise clause is an episteme ([[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]) and "service" talk must be facet-unpacked ([[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]).

**CC‑A2.3‑12 (Plan–run split).**
Windows and calendars belong to `U.WorkPlan` ([[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/02_A.15.02 - U.WorkPlan|A.15.2]]). Fulfilment evidence belongs to `U.Work` ([[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/01_A.15.01 - U.Work|A.15.1]]).

**CC-A2.3-13 (Scope lexicon & guards).**
Deprecated labels *applicability*, *envelope*, *generality*, and *validity* **MUST NOT** appear as names for scope objects in guards or conformance blocks. Use **`U.ClaimScope (G)`** for epistemes and **`U.WorkScope`** for capabilities ([[A_Kernel Architecture Cluster/02_Role Taxonomy/06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] and [[A_Kernel Architecture Cluster/02_Role Taxonomy/02_A.02.02 - U.Capability - System Ability Envelope and Measures|A.2.2]]). Scope-sensitive guards **MUST** use **ScopeCoverage** with explicit **Gamma_time** selectors.

**CC-A2.3-14 (Bridges & CL).**
Cross-context mappings via Bridges keep **F** and **G** stable; **CL** penalties apply to **R**. A mapping **MAY** recommend **narrowing** the mapped **Claim scope (G)** as best practice ([[A_Kernel Architecture Cluster/02_Role Taxonomy/06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] and B-line).

**CC-A2.3-15 (OutcomeSpec typing).**
`promisedOutcomeSpecRef` MUST resolve to `U.OutcomeSpec` ([[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10). It MUST NOT be used to point at a concrete `U.Work` episode or at an extensional delivered-result referent.

**CC-A2.3-16 (OutcomeSpec is explicit and mode‑complete).**
`promisedOutcomeSpecRef` MUST be present and MUST reference an `U.OutcomeSpec` that declares `mode ∈ {WorkOnly, ResultOnly, Composite}` and satisfies [[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10 mode completeness:
...
(You MAY materialize this as `deliversPromisedOutcome(Work, OutcomeSpec)` per [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:8.1 for auditability.)
...
If `unitOfDelivery` is present, then it MUST declare how delivered units are computed from Work evidence (duration, quantity, cases, kWh, etc) per [[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10.3. The default “1 unit per fulfilment Work” is permitted only when `unitOfDelivery` is a pure count of fulfilment episodes.

## [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:8 - Evidence relations & operators (Promise content ⇄ Work)

To keep the promise-to-evidence relation explicit:

### [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:8.1 - Core relations
...
### [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:8.2 - Service‑clause performance operators
...
* **Delivered units:** `delivered(SC, T)` is computed from the set `W✓(SC, T)` using `unitOfDelivery`’s **countingRule** ([[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10). Default (when `unitOfDelivery` is absent): `delivered(SC, T) = |W✓(SC, T)|` (one unit per accepted delivery work).
...
* **Cost‑to‑serve:** sum of `Γ_work` over `W✓` per resource category ([[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/01_A.15.01 - U.Work|A.15.1]]).
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:9 - Common Anti-Patterns and How to Avoid Them

* **“The microservice **is** the service.”**
  Rewrite to facet-explicit terms ([[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]): the microservice is typically a **service delivery system** (`U.System`), a **service access point** (`U.System`), or both. Keep the consumer-facing promised-outcome statement in `U.PromiseContent`, and represent accountability via `U.Commitment` if needed.
...
* **“Attach cost to the service.”**
  Actual cost and time attach to `U.Work` only ([[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/01_A.15.01 - U.Work|A.15.1]]). Service metrics are computed **from** Work.
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:10 - Migration notes (quick wins)
...
4. **Choose metrics.** For each promise content, including local service labels that resolve to promise content, define 2-4 KPIs and the declared Work-based formulas (availability, lead-time, rejection rate, cost-to-serve), declare the **Claim scope (G)** and **Gamma_time** policy used for each KPI, and - when KPIs are numeric or comparable - define the underlying `U.Characteristic` plus measurement procedure and evidence ([[C_Kernel Extension Specifications/04_16_Measurement & Metrics Characterization (MM-CHR)/00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]] and [[C_Kernel Extension Specifications/13_25_Q-Bundle- Authoring --ilities- as Structured Quality Bundles/00_C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]]) and pin `{UnitType, ScaleKind, ReferencePlane, EditionId}`.
...
6. **Tidy language.** Apply **[[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/17_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] (RPR-SERV)** and **L-SERV**: ban unqualified "service" as a synonym for server, team, process, or ticket in normative prose; map them explicitly.

## [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:10.1 - Consequences
...
| Commitments stay distinct | A promise-content clause can be reused as payload for `U.Commitment` without becoming the deontic commitment relation itself. | Accountability still needs [[A_Kernel Architecture Cluster/02_Role Taxonomy/08_A.02.08 - U.Commitment (Deontic Commitment Object)]], role assignment, and source relations when those claims are current. |
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:10.3 - SoTA-Echoing

Service-management, product, utility, platform, and public-service practice all distinguish offers, providers, access channels, service levels, work execution, and evidence of fulfilment, even when everyday language calls all of them "the service". [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]] keeps that practical distinction in FPF by giving the consumer-facing promise clause its own episteme value and by returning provider, access, commitment, work, and evidence claims to their governing patterns.
...
## [[A_Kernel Architecture Cluster/02_Role Taxonomy/03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:End
