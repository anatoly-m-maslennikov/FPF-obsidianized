...
## [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:1 - Context
...
**Modularity note.** [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]] defines only the promise‑content object (the **promise content**) and its direct links to roles, access specification, acceptance criteria, and work evidence. The multi‑facet “service situation” bundle that also names provider principals/systems/access points/commitments/acts is handled as a precision‑restoration lens in **[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] (`serviceSituation(…)`)**. Contract-talk unpacking and classification of “contract / SLA / guarantee” language is handled by **[[03_A.06.C - Contract Unpacking for Boundaries|A.6.C]]**, which applies [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] when service‑cluster tokens appear.
...
> Intuition: the **consumer-facing promise clause** is what you advertise and are judged by (`U.PromiseContent`); **work** is what you do to keep that promise; **method/spec** is how you know what to do. (See [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] for full “service” polysemy unpacking.)
...
Normative prose therefore SHALL treat the bare head noun **service** as **always‑unpack** (PTG=Guarded): every head‑noun occurrence MUST be rewritten to a facet head phrase (promise content, service provider principal, service access point, service delivery system, and so on) or to the correct underlying EntityOfConcern or project-side FPF kind (team, ticket, endpoint host, procedure, work item), per **[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] (RPR‑SERV)**.

[[00_E.10 - Unified Lexical Rules for FPF|E.10]]’s lexical anchor **L‑SERV** SHOULD be implemented as “pointer + lint rule” to [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]: the short rule names the hazard, while [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] provides the full rewrite recipe and the facet head phrase set.

## [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:2 - Problem
...
## [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:3 - Forces
...
## [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:4 - Solution — The unified concept `U.PromiseContent`
...
* **Prose head (normative):** *promise content* (Tech) / *service offering clause* (Plain; *service promise clause* acceptable synonym). (Both twins retain an explicit **clause** head‑kind to avoid act/content ambiguity and to comply with [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] headword governance.)

### [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:4.1 - Core structure (minimal fields)
...
U.PromiseContent {
  ...
  unitOfDelivery?: Episteme,           // how delivered units are counted/measured (unit + countingRule; see [[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10)
  version?       : SemVer/Text,
  timespan?      : Interval
}
```

* `promisedOutcomeSpecRef` MUST point to a `U.OutcomeSpec` ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10). It is the promise-facing outcome template (work-only, result-only, or composite), not a `U.Work` episode and not an extensional delivered-result referent.
...
* **Internal delivery methods/runbooks are not part of the promise content.** Model them as `U.MethodDescription` and relate them to the clause via `serviceSituation(…)` ([[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]) or explicit context relations; providers retain **Method autonomy**.

### [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:4.1.1 - Promised outcome spec (disambiguation: work vs post-work result)
...
* **promiseOutcomeSpec** → `U.OutcomeSpec` ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10), referenced via `promisedOutcomeSpecRef`.
...
A conforming `U.OutcomeSpec` uses the canonical shape from [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10.2:
...
* **Counting is not part of `U.OutcomeSpec`.** Counting lives on `U.PromiseContent.unitOfDelivery` as the `countingRule` mini‑schema ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10.3). Outcome specs say *what* counts as delivery; unit‑of‑delivery specs say *how much* to count and how to avoid double counting.
...
### [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:4.1.2 - Recommended `acceptanceSpec` mini‑schema *(informative, non‑kernel)*
...
### [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:4.2 - What `U.PromiseContent` is **not**

* **Not a provider:** use `System#ServiceProviderRole:Context` `U.RoleAssignment`.
* **Not a deontic commitment:** that is `U.Commitment` ([[08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]) referencing the promise content as payload.
* **Not an access point:** addressable “services/servers/desks/endpoints” are `U.System` (see [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]: *service access point* / *service delivery system*).
...
* **Not a scope label:** do **not** use *applicability*, *envelope*, *generality*, or *validity* as names for **scope objects**; declare **Claim scope (G)** or **Work scope** explicitly where needed ([[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]).

### [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:4.3 - Position in the enactment chain

* **Design‑time:**
  The context **declares Claim scope (G)** for acceptance (operating conditions, populations, locales) per [[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]].
...
### [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:4.4 - Didactic card: The service delivery chain (clause → commitment → situation → work → acceptance)

> **Didactic (non‑normative).** This is a one‑screen “map” that stitches the modular pieces together:
> `U.PromiseContent` ([[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]) → `U.Commitment` ([[08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]]) → provider `U.RoleAssignment` ([[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]) → *serviceSituation(...)* facet slots ([[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] lens) → `U.Work + carriers` ([[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]) → acceptance verdict ([[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]).
>
...
flowchart LR
  ...
  subgraph LENS["Optional lens ([[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]): serviceSituation(...)"]
...
* `serviceSituation(...)` ([[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]) is a **facet‑binding lens** that names the common “service talk” participants (access spec / access point / delivery system / delivery method) **without** collapsing them into the clause.
...
## [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:5 - Archetypal grounding (engineer‑manager friendly)
...
## [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:6 - Mapping the common “service” picture to FPF (didactic bridge)
...
* **Service Level Agreement (SLA)** (binding obligation) -> `U.Commitment` referencing the relevant `U.PromiseContent` (and, where needed, its acceptance/evidence specs); use **[[03_A.06.C - Contract Unpacking for Boundaries|A.6.C]] Contract Bundle** when packaging “the SLA” as a bundle of commitments, evidence specs, and publication carriers.
* **SLA document / published terms** → `U.SpeechAct` (promise/offer act) + the clause carrier (`U.Episteme`), per [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]] + [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]].
* **Operating conditions / “where the promise holds”** → `claimScope : U.ClaimScope (G)` (or embedded in `acceptanceSpec`) per [[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]].
...
## [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:7 - Conformance Checklist (normative)

**CC‑A2.3‑0 (Prose head phrase).**
In normative prose, an instance of `U.PromiseContent` SHALL be referred to as a **promise content** (or **service offering clause** or **service promise clause**) and SHALL NOT be referenced by the bare head noun *service*. Unqualified *service* usage (and the co‑moving cluster *service provider* / *server*) SHALL be unpacked per [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] (RPR‑SERV).
...
`providerRole` and (if used) `consumerRole` **MUST** be **role kinds** (see [[00_A.02 - Role Taxonomy|A.2]]). Actual performers at run‑time are `U.RoleAssignment`s.
...
When declared, `unitOfDelivery` **MUST** include a **countingRule** that maps accepted delivery work episodes (`W✓`) to unit counts ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10). If omitted, the default is “1 unit per accepted delivery work episode”.

**CC‑A2.3‑7 (No actuals on Promise Content).**
Resource/time **actuals** and incident logs **MUST** attach to `U.Work` only ([[01_A.15.01 - U.Work|A.15.1]]). Promise contents carry no actuals.
...
Unqualified head‑noun uses of *service* (and the co‑moving cluster *service provider* / *server*) in normative prose **MUST** be disambiguated per **[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] (RPR‑SERV)** and its lexical anchor **L‑SERV** ([[00_E.10 - Unified Lexical Rules for FPF|E.10]]).

**CC‑A2.3‑11 (No mereology).**
Do **not** place a promise content clause in PBS/SBS or treat it as a part/component. Structural assemblies live in PBS/SBS; the promise clause is an episteme ([[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]) and “service” talk must be facet‑unpacked ([[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]).

**CC‑A2.3‑12 (Plan–run split).**
Windows and calendars belong to `U.WorkPlan` ([[02_A.15.02 - U.WorkPlan|A.15.2]]). Fulfilment evidence belongs to `U.Work` ([[01_A.15.01 - U.Work|A.15.1]]).

**CC-A2.3-13 (Scope lexicon & guards).**
Deprecated labels *applicability/envelope/generality/validity* **MUST NOT** appear as names for scope objects in guards or conformance blocks. Use **`U.ClaimScope (G)`** for epistemes and **`U.WorkScope`** for capabilities ([[00_A.02 - Role Taxonomy|A.2]].6/A.2.2). Scope-sensitive guards **MUST** use **ScopeCoverage** with explicit **Γ_time** selectors.

**CC-A2.3-14 (Bridges & CL).**
Cross-context mappings via Bridges keep **F/G** stable; **CL** penalties apply to **R**. A mapping **MAY** recommend **narrowing** the mapped **Claim scope (G)** as best practice ([[00_A.02 - Role Taxonomy|A.2]].6/B-line).

**CC-A2.3-15 (OutcomeSpec typing).**
`promisedOutcomeSpecRef` MUST resolve to `U.OutcomeSpec` ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10). It MUST NOT be used to point at a concrete `U.Work` episode or at an extensional delivered-result referent.

**CC-A2.3-16 (OutcomeSpec is explicit and mode‑complete).**
`promisedOutcomeSpecRef` MUST be present and MUST reference an `U.OutcomeSpec` that declares `mode ∈ {WorkOnly, ResultOnly, Composite}` and satisfies [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10 mode completeness:
...
(You MAY materialize this as `deliversPromisedOutcome(Work, OutcomeSpec)` per [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:8.1 for auditability.)
...
If `unitOfDelivery` is present, then it MUST declare how delivered units are computed from Work evidence (duration, quantity, cases, kWh, etc) per [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10.3. The default “1 unit per fulfilment Work” is permitted only when `unitOfDelivery` is a pure count of fulfilment episodes.

## [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:8 - Evidence relations & operators (Promise content ⇄ Work)

To keep the promise → evidence path explicit:

### [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:8.1 - Core relations
...
### [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:8.2 - Service‑clause performance operators
...
* **Delivered units:** `delivered(SC, T)` is computed from the set `W✓(SC, T)` using `unitOfDelivery`’s **countingRule** ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10). Default (when `unitOfDelivery` is absent): `delivered(SC, T) = |W✓(SC, T)|` (one unit per accepted delivery work).
...
* **Cost‑to‑serve:** sum of `Γ_work` over `W✓` per resource category ([[01_A.15.01 - U.Work|A.15.1]]).
...
## [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:9 - Anti‑patterns (and the right move)

* **“The microservice **is** the service.”**
  Rewrite to facet‑explicit terms ([[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]): the microservice is typically a **service delivery system** (`U.System`) and/or a **service access point** (`U.System`). Keep the **promise content** as a **promise content** in `U.PromiseContent`, and bind accountability via `U.Commitment` if needed.
...
* **“Attach cost to the service.”**
  Actual cost/time attach to `U.Work` only ([[01_A.15.01 - U.Work|A.15.1]]). Service metrics are computed **from** Work.
...
## [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:10 - Migration notes (quick wins)
...
6. **Tidy language.** Apply **[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] (RPR‑SERV)** / **L‑SERV**: ban unqualified “service” as a synonym for server/team/process/ticket in normative prose; map them explicitly.

## [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:11 - Relations

* **Builds on:** [[01_A.01.01 - U.BoundedContext Semantic Frame|A.1.1]] `U.BoundedContext`; [[00_A.02 - Role Taxonomy|A.2]] `U.Role`; [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]] `U.RoleAssignment`; [[02_A.02.02 - U.Capability|A.2.2]] `U.Capability`; **[[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] `U.Scope` / `U.ClaimScope (G)` / `U.WorkScope`**.
* **Coordinates with:** [[01_A.03.01 - U.Method- Context-Defined Way of Doing|A.3.1]] `U.Method`; [[02_A.03.02 - U.MethodDescription- Description Episteme for a Way of Doing|A.3.2]] `U.MethodDescription`; [[01_A.15.01 - U.Work|A.15.1]] `U.Work`; [[02_A.15.02 - U.WorkPlan|A.15.2]] `U.WorkPlan`; **[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] (RPR‑SERV)** for normative prose unpacking of the service cluster; **B-line Bridges & CL (CL→R; may recommend ΔG narrowing)**.
* **Constrained by lexical rules:** **[[00_E.10 - Unified Lexical Rules for FPF|E.10]] L‑SERV** (service disambiguation); also **L‑FUNC**, **L‑PROC**, **L‑SCHED**, **L‑ACT**.
* **Informs:** Reporting/assurance patterns (service KPIs, SLA dashboards); catalog/exposure patterns in domain contexts.

## [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:12 - Didactic quick cards (engineer‑manager ready)
...
## [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:End
