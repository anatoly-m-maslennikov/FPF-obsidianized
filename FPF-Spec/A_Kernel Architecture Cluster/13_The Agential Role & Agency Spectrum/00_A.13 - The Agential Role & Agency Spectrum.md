...
## [[00_A.13 - The Agential Role & Agency Spectrum|A.13]]:1 - Intent & Context
...
This pattern builds directly upon the foundations laid in the FPF Kernel to provide that definition. [[00_A.01 - U.Holon, U.System, and U.Episteme|A.1]] established that only a **`U.System`** can be the bearer (`holder`) of behavioral roles.  [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]] defined the universal `U.RoleAssignment` (`Holder#Role:Context`) as the canonical way to assign roles. [[00_A.03 - Transformer Constitution (Quartet)|A.3]] and [[00_A.12 - External Transformer & Reflexive Split|A.12]] defined the `TransformerRole` and the principle of the external agent.
...
## [[00_A.13 - The Agential Role & Agency Spectrum|A.13]]:2 - Problem
...
1.  **Episteme-as-Actor:** Models might incorrectly assign agency to knowledge epistemes or publications (`U.Episteme`), leading to nonsensical claims like "the specification decided to update the system." This is a direct violation of **Strict Distinction ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]])**.
...
3.  **Unfalsifiable Claims:** Without a measurable basis, "agency" becomes a subjective label. A team might call their system an "agent" for marketing purposes, but this claim has no verifiable meaning and cannot be audited, violating **Evidence Graph Referring ([[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]])**.
...
## [[00_A.13 - The Agential Role & Agency Spectrum|A.13]]:3 - Forces
...
| **System-only Action vs. Collective Action**| How does agency apply to groups like teams or swarms? This requires a clear link to the rule from [[00_A.01 - U.Holon, U.System, and U.Episteme]] that any *acting group* must be modeled as a `U.System`. |

## [[00_A.13 - The Agential Role & Agency Spectrum|A.13]]:4 - Solution

FPF's solution is threefold: it defines an Agent via `U.RoleAssignment` ([[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]), makes agency measurable with a dedicated Characterization, and provides a didactic summary via a graded scale.

### [[00_A.13 - The Agential Role & Agency Spectrum|A.13]]:4.1 - The Core Definition: Agent as a Contextual Role Assignment
...
### [[00_A.13 - The Agential Role & Agency Spectrum|A.13]]:4.2 - The `AgentialRole` and its Specializations
...
### [[00_A.13 - The Agential Role & Agency Spectrum|A.13]]:4.3 - Measuring Agency: The `Agency-CHR` and the Spectrum
...
The `Agency-CHR` profile is grounded in contemporary research (e.g., Active Inference, Basal Cognition) and includes the following key characteristics. Each is measured for a specific agent in a specific context and must be backed by evidence ([[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]).
...
#### [[00_A.13 - The Agential Role & Agency Spectrum|A.13]]:4.3.1 - Context-bounded task-family specialization claims
...
### [[00_A.13 - The Agential Role & Agency Spectrum|A.13]]:4.4 - The Agency Grade (Didactic Layer)
...
## [[00_A.13 - The Agential Role & Agency Spectrum|A.13]]:5 - Archetypal Grounding
...
## [[00_A.13 - The Agential Role & Agency Spectrum|A.13]]:6 - Conformance Checklist
...
| :--- | :--- | :--- |
| **CC-A13.1 (Holder Type)** | The `holder` of a `U.RoleAssignment` with `role: U.AgentialRole` **MUST** be a `U.System`. | Prevents the "episteme-as-actor" category error. Enforces **Strict Distinction ([[00_A.07 - Strict Distinction (Clarity Lattice)]])**. |
...
| **CC-A13.3 (CHR Evidence)** | Any claim about an Agent's Agency Grade or autonomy profile **MUST** be substantiated by an auditable `Agency-CHR` profile with Evidence Graph Ref ([[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]]). | Makes claims of agency falsifiable and prevents "agency by marketing." |
...
| **CC-A13.5 (Collective as System)** | To claim agency for a collective (e.g., a team, a swarm), the collective **MUST** first be modeled as a `U.System` with a defined `U.Boundary` and a coordination `U.Method`. | Prevents the error of assigning agency to a mere set or collection (`MemberOf`). Aligns with **[[00_A.01 - U.Holon, U.System, and U.Episteme]]** and **[[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases]]**. |
| **CC-A13.6 (MHT for Emergent Agency)** | If a collection of systems, previously non-agential or at a lower grade, develops a new supervisory structure and crosses a documented `Agency-CHR` threshold, a **Meta-Holon Transition (MHT, [[00_B.02 - Meta-Holon Transition (MHT)- Recognizing Emergence and Re-identifying Wholes]])** **MUST** be declared. | Makes the emergence of collective agency an explicit, auditable event, preventing "magic" emergence. |

## [[00_A.13 - The Agential Role & Agency Spectrum|A.13]]:7 - Consequences
...
## [[00_A.13 - The Agential Role & Agency Spectrum|A.13]]:8 - Rationale
...
## [[00_A.13 - The Agential Role & Agency Spectrum|A.13]]:9 - Relations
...
## [[00_A.13 - The Agential Role & Agency Spectrum|A.13]]:End
