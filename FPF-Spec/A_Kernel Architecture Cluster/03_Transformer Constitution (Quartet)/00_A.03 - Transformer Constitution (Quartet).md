...
## [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:1 - Intent

Establish a single, substrate‑neutral way to say **who acts**, **under which role**, **according to which description**, **by which capability**, and **what actually happened**—without “self‑magic” and without blurring design‑time and run‑time. The pattern fixes the **Transformer Quartet** so all kernel and Γ‑patterns reuse the *same four anchors*. It builds directly on **Holon‑Role Duality ([[A_Kernel Architecture Cluster/02_Role Taxonomy/00_A.02 - Role Taxonomy|A.2]])** and **Temporal Duality ([[A_Kernel Architecture Cluster/04_Temporal Duality & Open-Ended Evolution Principle/00_A.04 - Temporal Duality & Open-Ended Evolution Principle|A.4]])** and is guarded by **Strict Distinction ([[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]])** and **Evidence Graph Referring ([[A_Kernel Architecture Cluster/10_Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph/00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]])**.

## [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:2 - Context
...
## [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:3 - Problem
...
5. **Scope leak:** design‑time and run‑time mixed; run traces lack carriers/method ties.
   A.2/A.4/A.7/A.10 collectively forbid these, but [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]] must give the **canonical quartet** that authors can apply consistently.

## [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:4 - Forces
...
## [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:5 - Solution — The Transformer Quartet

[[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]] defines four anchors, tied together by **Role Assignment (`U.RoleAssignment`)** and aligned with **Temporal Duality**.

### [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:5.1 - The four anchors (terms & types)
...
### [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:5.2 - Contextual Role Assignmnent (`U.RoleAssignment`) for transformations
...
### [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:5.3 - Boundary & externality

Every transformation is modelled with **two sides** and an explicit **U.Interaction** boundary: **acting** (system bearing TransformerRole) and **target** (system being transformed, or the **carrier** of an episteme). There is **no self‑doing**; “self‑like” stories are handled by the **reflexive split** (regulator vs regulated subsystems) or by promoting a meta‑holon and keeping evidence external ([[A_Kernel Architecture Cluster/12_Acting-Side Externalization and Reflexive Split/00_A.12 - Acting-Side Externalization and Reflexive Split|A.12]]).

### [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:5.4 - Temporal alignment ([[A_Kernel Architecture Cluster/04_Temporal Duality & Open-Ended Evolution Principle/00_A.04 - Temporal Duality & Open-Ended Evolution Principle|A.4]] bridge)
...
### [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:5.5 - Evidence Graph Referring
...
### [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:5.6 - Didactic dictionary (safe mappings)
...
## [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:6 - Illustrative scenarios (substrate‑neutral)

### [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:6.1 - Physical system — Cooling loop
...
### [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:6.2 - Epistemic change — Proof revision
...
### [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:6.3 - Reflexive maintenance — “calibrates itself”
...
## [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:7 - Conformance Checklist (normative)
...
This is the canonical way to say *who acts, in which role, where (semantically), and when*. See **[[A_Kernel Architecture Cluster/02_Role Taxonomy/01_A.02.01 - U.RoleAssignment - Contextual Work-Role Assignment|A.2.1]]** for the universal **`U.RoleAssignment`** Standard and its invariants.

**CC‑A3‑1 - External transformer discipline.**
The **bearer** of `TransformerRole` MUST NOT be the same model instance as the **object-under-change** within the same assignment. Self-modification is modelled via two **`U.RoleAssignment`s** (same holder playing two roles) or via an explicit controller-plant split. This upholds **acting-side externalization** ([[A_Kernel Architecture Cluster/12_Acting-Side Externalization and Reflexive Split/00_A.12 - Acting-Side Externalization and Reflexive Split|A.12]]).

**CC‑A3‑2 - Design–Run separation.**
`U.MethodDescription` (recipe, definition) is a design-time `U.Episteme`; `U.Method` (mask-of-work) and `U.Work` (executed work) are run-time kinds. It is non-conformant to mutate a `MethodDescription` inside a `Work` log or to treat a `Work` as a `MethodDescription`. This enforces the kernel’s **Temporal Duality** ([[A_Kernel Architecture Cluster/04_Temporal Duality & Open-Ended Evolution Principle/00_A.04 - Temporal Duality & Open-Ended Evolution Principle|A.4]]) and the [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] alignment.
...
Conflicts among roles (including `TransformerRole`) are only **within the same bounded context**; cross‑context differences are admissible if bridges are declared. Non‑conformance arises only when a context’s own incompatibility rules are violated. ([[A_Kernel Architecture Cluster/02_Role Taxonomy/01_A.02.01 - U.RoleAssignment - Contextual Work-Role Assignment|A.2.1]] `U.RoleAssignment` invariants.)

**CC‑A3‑10 - Γ‑compatibility.**
Descriptions MUST be sufficient for the relevant Γ‑aggregations to run: Γ\_method for recipe composition, Γ\_work for resource deltas, Γ\_sys for boundary integration, Γ\_time for ordering. Each Γ flavour declares its [[A_Kernel Architecture Cluster/14_Advanced Mereology- Components, Portions, Aspects & Phases/00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] hooks (Portion/Phase) and inherits B‑invariants.

## [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:8 - Consequences
...
## [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:10 - Relations

**[[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] Strict Distinction.**
[[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]] operationalises [[A_Kernel Architecture Cluster/07_Strict Distinction (Clarity Lattice)/00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] by keeping **target EntityOfConcern ≠ MethodDescription ≠ observation/log Work**:
...
**[[A_Kernel Architecture Cluster/12_Acting-Side Externalization and Reflexive Split/00_A.12 - Acting-Side Externalization and Reflexive Split|A.12]] Acting-Side Externalization & External Transformer.**
[[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]'s CC-A3-1 is the mechanical guard-rail for [[A_Kernel Architecture Cluster/12_Acting-Side Externalization and Reflexive Split/00_A.12 - Acting-Side Externalization and Reflexive Split|A.12]]: even in self-modification, the *modelling split* keeps the acting-side holder distinct from the object-under-change.

**[[A_Kernel Architecture Cluster/13_The Agential Role & Agency Spectrum/00_A.13 - The Agential Role & Agency Spectrum|A.13]] Agential Role.**
When the bearer is an acting system or collective system with an agential role assignment, [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]] keeps identity, role assignment, method, plan, work, and evidence separate while [[A_Kernel Architecture Cluster/13_The Agential Role & Agency Spectrum/00_A.13 - The Agential Role & Agency Spectrum|A.13]] supplies the agency-characteristic profile. This is where policy, plan, and action pipelines remain tied to explicit role assignments and Γ compatibility.

**[[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] Role–Method–Work Alignment.**
[[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]] relies on [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]’s vocabulary guard‑rails (roles are not parts; methods are masks of work; specs are recipes). CC‑A3‑2/‑4 are enforceable precisely because [[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] fixes the naming discipline.

**[[A_Kernel Architecture Cluster/14_Advanced Mereology- Components, Portions, Aspects & Phases/00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] Advanced Mereology.**
[[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]] consumes [[A_Kernel Architecture Cluster/14_Advanced Mereology- Components, Portions, Aspects & Phases/00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]’s **PortionOf** (for quantitative deltas) and **PhaseOf** (for versioning) and forbids role/recipe leakage into part–whole trees. Γ‑flavours declare which [[A_Kernel Architecture Cluster/14_Advanced Mereology- Components, Portions, Aspects & Phases/00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] hooks they use.

**B‑cluster (Γ‑sections).**
[[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]] is executable only because Γ‑operators provide aggregation and invariants:
...
## [[A_Kernel Architecture Cluster/03_Transformer Constitution (Quartet)/00_A.03 - Transformer Constitution (Quartet)|A.3]]:End
