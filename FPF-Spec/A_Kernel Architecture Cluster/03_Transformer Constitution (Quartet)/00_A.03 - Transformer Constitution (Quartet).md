...
## [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:1 - Intent

Establish a single, substrate‑neutral way to say **who acts**, **under which role**, **according to which description**, **by which capability**, and **what actually happened**—without “self‑magic” and without blurring design‑time and run‑time. The pattern fixes the **Transformer Quartet** so all kernel and Γ‑patterns reuse the *same four anchors*. It builds directly on **Holon‑Role Duality ([[00_A.02 - Role Taxonomy|A.2]])** and **Temporal Duality ([[00_A.04 - Temporal Duality & Open-Ended Evolution Principle|A.4]])** and is guarded by **Strict Distinction ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]])** and **Evidence Graph Referring ([[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]])**.

## [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:2 - Context
...
## [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:3 - Problem
...
5. **Scope leak:** design‑time and run‑time mixed; run traces lack carriers/method ties.
   A.2/A.4/A.7/A.10 collectively forbid these, but [[00_A.03 - Transformer Constitution (Quartet)|A.3]] must give the **canonical quartet** that authors can apply consistently.

## [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:4 - Forces
...
## [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:5 - Solution — The Transformer Quartet

[[00_A.03 - Transformer Constitution (Quartet)|A.3]] defines four anchors, tied together by **Role Assignment (`U.RoleAssignment`)** and aligned with **Temporal Duality**.

### [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:5.1 - The four anchors (terms & types)
...
### [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:5.2 - Contextual Role Assignmnent (`U.RoleAssignment`) for transformations
...
### [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:5.3 - Boundary & externality

Every transformation is modelled with **two sides** and an explicit **U.Interaction** boundary: **acting** (system bearing TransformerRole) and **target** (system being transformed, or the **carrier** of an episteme). There is **no self‑doing**; “self‑like” stories are handled by the **reflexive split** (regulator vs regulated subsystems) or by promoting a meta‑holon and keeping evidence external ([[00_A.12 - External Transformer & Reflexive Split|A.12]]).

### [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:5.4 - Temporal alignment ([[00_A.04 - Temporal Duality & Open-Ended Evolution Principle|A.4]] bridge)
...
### [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:5.5 - Evidence Graph Referring
...
### [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:5.6 - Didactic dictionary (safe mappings)
...
## [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:6 - Illustrative scenarios (substrate‑neutral)

### [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:6.1 - Physical system — Cooling loop
...
### [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:6.2 - Epistemic change — Proof revision
...
### [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:6.3 - Reflexive maintenance — “calibrates itself”
...
## [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:7 - Conformance Checklist (normative)
...
This is the canonical way to say *who acts, in which role, where (semantically), and when*. See **[[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]** for the universal **`U.RoleAssignment`** Standard and its invariants.

**CC‑A3‑1 - External transformer discipline.**
The **bearer** of `TransformerRole` MUST NOT be the same model instance as the **object‑under‑change** within the same assignment. Self-modification is modelled via two **`U.RoleAssignment`s** (same holder playing two roles) or via an explicit controller–plant split. This upholds **Agent Externalization** ([[00_A.12 - External Transformer & Reflexive Split|A.12]]).

**CC‑A3‑2 - Design–Run separation.**
`U.MethodDescription` (recipe, definition) is a design-time `U.Episteme`; `U.Method` (mask-of-work) and `U.Work` (executed work) are run-time kinds. It is non-conformant to mutate a `MethodDescription` inside a `Work` log or to treat a `Work` as a `MethodDescription`. This enforces the kernel’s **Temporal Duality** ([[00_A.04 - Temporal Duality & Open-Ended Evolution Principle|A.4]]) and the [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] alignment.
...
Conflicts among roles (including `TransformerRole`) are only **within the same bounded context**; cross‑context differences are admissible if bridges are declared. Non‑conformance arises only when a context’s own incompatibility rules are violated. ([[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]] `U.RoleAssignment` invariants.)

**CC‑A3‑10 - Γ‑compatibility.**
Descriptions MUST be sufficient for the relevant Γ‑aggregations to run: Γ\_method for recipe composition, Γ\_work for resource deltas, Γ\_sys for boundary integration, Γ\_time for ordering. Each Γ flavour declares its [[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] hooks (Portion/Phase) and inherits B‑invariants.

## [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:8 - Consequences
...
* **No category errors.** Keeping methods/roles out of mereology and enforcing DesignRunTag separation prevents the usual “process‑as‑part” and “version‑as‑component” mistakes. ([[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] + [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]].)
...
## [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:10 - Relations

**[[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] Strict Distinction.**
[[00_A.03 - Transformer Constitution (Quartet)|A.3]] operationalises [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] by keeping **target EntityOfConcern ≠ MethodDescription ≠ observation/log Work**:
...
**[[00_A.12 - External Transformer & Reflexive Split|A.12]] Agent Externalization & External Transformer.**
[[00_A.03 - Transformer Constitution (Quartet)|A.3]]’s CC‑A3‑1 is the mechanical guard‑rail for [[00_A.12 - External Transformer & Reflexive Split|A.12]]: even in self‑modification, the *modelling split* keeps the agent (transformer bearer) distinct from the object‑under‑change.

**[[00_A.13 - The Agential Role & Agency Spectrum|A.13]] Agential Role.**
When the bearer is an **Agent**, [[00_A.03 - Transformer Constitution (Quartet)|A.3]] keeps identity and states management in Agent‑CAL (`U.Agent`, `U.Intent`, `U.Action`), while still requiring `RoleAssigning` + Γ compatibility. This is where policy/plan/action pipelines live.

**[[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] Role–Method–Work Alignment.**
[[00_A.03 - Transformer Constitution (Quartet)|A.3]] relies on [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]’s vocabulary guard‑rails (roles are not parts; methods are masks of work; specs are recipes). CC‑A3‑2/‑4 are enforceable precisely because [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] fixes the naming discipline.

**[[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] Advanced Mereology.**
[[00_A.03 - Transformer Constitution (Quartet)|A.3]] consumes [[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]’s **PortionOf** (for quantitative deltas) and **PhaseOf** (for versioning) and forbids role/recipe leakage into part–whole trees. Γ‑flavours declare which [[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] hooks they use.

**B‑cluster (Γ‑sections).**
[[00_A.03 - Transformer Constitution (Quartet)|A.3]] is executable only because Γ‑operators provide aggregation and invariants:
...
## [[00_A.03 - Transformer Constitution (Quartet)|A.3]]:End
