...
**Status.** Transitional stub (informative). This section defines no dedicated “module” subsystem. Enforceable boundary discipline lives in **[[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind over a RangedValueKind|A.6.0]] `U.Signature`** and **[[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/05_A.06.01 - U.Mechanism - Law-governed application to a SubjectKind over a RangedValueKind|A.6.1]] `U.Mechanism`**, with guard‑rails in **[[E_The FPF Constitution and Authoring Guides/04_05_Four Guard-Rails of FPF/03_E.05.03 - Unidirectional Dependency|E.5.3]]** (Unidirectional Dependency) and **[[E_The FPF Constitution and Authoring Guides/09_10_Unified Lexical Rules for FPF/00_E.10 - Unified Lexical Rules for FPF|E.10]]** (LEX‑BUNDLE stratification).

## [[A_Kernel Architecture Cluster/05_Open-Ended Kernel & Extension Layering/00_A.05 - Open-Ended Kernel & Extension Layering|A.5]]:1 - Problem frame
...
## [[A_Kernel Architecture Cluster/05_Open-Ended Kernel & Extension Layering/00_A.05 - Open-Ended Kernel & Extension Layering|A.5]]:2 - Problem
...
## [[A_Kernel Architecture Cluster/05_Open-Ended Kernel & Extension Layering/00_A.05 - Open-Ended Kernel & Extension Layering|A.5]]:3 - Forces
...
## [[A_Kernel Architecture Cluster/05_Open-Ended Kernel & Extension Layering/00_A.05 - Open-Ended Kernel & Extension Layering|A.5]]:4 - Solution
...
2. **Boundary packaging via `U.Signature` ([[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind over a RangedValueKind|A.6.0]]).** Reusable bundles are published as signatures with an explicit `SignatureManifest` (`imports`, `provides`).
3. **Dependency vs specialisation are separate relations.** `imports` forms a dependency DAG constrained by **[[E_The FPF Constitution and Authoring Guides/04_05_Four Guard-Rails of FPF/03_E.05.03 - Unidirectional Dependency|E.5.3]]**; refinement/extension (`⊑`, `⊑⁺`) is expressed separately (e.g., **[[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/05_A.06.01 - U.Mechanism - Law-governed application to a SubjectKind over a RangedValueKind|A.6.1]] `U.MechMorph`**) and should not be conflated with `imports`.
...
This section is intentionally lightweight: it provides architectural intent and neighboring-pattern pointers only. Any new enforceable modularity constraints belong in the [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/00_A.06 - Signature Stack & Boundary Discipline|A.6]].* boundary patterns (or in E.* guard‑rails), not here.

## [[A_Kernel Architecture Cluster/05_Open-Ended Kernel & Extension Layering/00_A.05 - Open-Ended Kernel & Extension Layering|A.5]]:End
