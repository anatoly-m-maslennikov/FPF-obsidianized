...
related:
  ...
  - "[[06_A.19.CHR - CHRMechanismSuite|A.19.CHR]]"
  - "[[05_A.19.CN - CN-frame (comparability & normalization)|A.19.CN]]"
  ...
  - "[[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]"
...
> **Status:** Stable
> **Normativity:** Normative (unless explicitly marked informative)
> **Placement:** Part A / CN‑Spec cluster ([[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]]) / CHR mechanism-governing patterns
> **Governing-pattern note (Phase‑3 canonicalization):** This pattern governs the meaning of `UNM.IntensionRef` (per [[00_E.20 - Mechanism Introduction Protocol|E.20]]). The canonical publication anchor for `UNM.IntensionRef` remains [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]], while [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] governs the `U.Mechanism.Intension` **template**.
> **Boundary note:** The `CN_Spec` surface itself (incl. `CN_Spec.normalization` and `CN_Spec.comparability`) remains governed by [[05_A.19.CN - CN-frame (comparability & normalization)|A.19.CN]]; this pattern specifies only UNM’s stable semantic surface and how UNM **consumes/interprets** the CN‑frame routing fields (no shadow CN‑spec).
...
## [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:0 - At a glance — didactic, informative
...
## [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:1 - Problem frame
...
## [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:2 - Problem
...
## [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:3 - Forces
...
## [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:4 - Solution
...
### [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:4.0 - Vocabulary (normative)
...
**NormalizationMethodDescription.** An editioned epistemic description of a normalization method (bounds, validity region/window, scope constraints, and evidence links governed by [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]]).
...
**Alias:** `UNM.TransportRegistryPhi` is an ASCII‑safe alias token (dock via [[00_F.18 - Local-First Unification Naming Protocol|F.18]]); it is **not** a competing head.
...
### [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:4.1 - UNM as a `U.Mechanism.Intension` (normative)

**Scope note.** This Mechanism.Intension is authored to the `U.Mechanism.Intension` **shape** governed by [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]. It defines only UNM’s stable *semantic surface*. It does **not** bind project pins (editions/policy‑ids), which belong to the P2W seam ([[03_A.15.03 - SlotFillingsPlanItem|A.15.3]] + [[06_A.19.CHR - CHRMechanismSuite|A.19.CHR]]), and it does **not** emit `GateDecision`/`GateLog`. It may emit tri‑state `GuardDecision` and Audit pins.
...
- [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] (shape: `U.Mechanism.Intension`, specialization discipline)
- [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] (slot discipline; SlotIndex is a projection)
...
- [[05_A.19.CN - CN-frame (comparability & normalization)|A.19.CN]] (CN-frame routing: `normalization`, `comparability.mode`)
- [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]] (CG-frame legality gates where required downstream)
- [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]] (evidence carriers; calibration/validity for normalization legitimacy)
...
- **UNM‑L0 (Values, not characteristics).** UNM produces `NCV` as a **value** under declared invariants; it does not redefine the underlying characteristic meaning (measurement meaning remains governed by A.17/A.18 and evidence by [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]]).
...
- **UNM‑L3 (Fail‑closed).** If admissibility/evidence is insufficient (or required inputs are missing/stale), UNM does not silently coerce; it yields `abstain` or `degrade` (tri‑state guard discipline) and may surface an explicit freshness/work request (see [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:4.5).
...
- any emitted `FreshnessRequest` / work request identifiers (when applicable; see [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:4.5)

### [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:4.2 - CN-frame wiring: `normalization` and comparability routing (normative-by-reference)
...
(See CN-frame definition in [[05_A.19.CN - CN-frame (comparability & normalization)|A.19.CN]]; [[05_A.19.CN - CN-frame (comparability & normalization)|A.19.CN]] remains the governing pattern of the CN-frame surface. This section only states the UNM consumption/interpretation constraints and does not introduce a shadow spec.)

### [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:4.3 - Evidence and calibration are governed by MM‑CHR (normative-by-reference)
...
### [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:4.4 - Didactic rule: quotients or fixes, never “labels” (normative)
...
### [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:4.5 - P2W and transformation-flow integration note (normative-by-reference)

When UNM is used inside transformation-flow structures/graphs (e.g., [[00_E.18 - Transformation Flow Structure|E.18]]):
...
## [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:5 - Archetypal Grounding (Tell–Show–Show)
...
## [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:6 - Bias‑Annotation
...
## [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:7 - Conformance Checklist

- [ ] **Template compliance:** canonical [[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]] sections 1–13 present in order; pattern ends with `### [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:End`.
...
## [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:8 - Common Anti‑Patterns and How to Avoid Them
...
## [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:9 - Consequences
...
## [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:10 - Rationale
...
## [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:11 - SoTA‑Echoing (post‑2015 practice alignment)

UNM does not prescribe algorithms, but it is designed to wire in SoTA normalization families via `NormalizationMethodDescriptionRef` + evidence pins (typically shipped as [[00_G.02 - SoTA Harvester & Synthesis|G.2]] SoTA packs and wired via `GPatternExtension` modules, not as mutations of UNM’s surface). Examples of post‑2015 method families that often appear as evidence-backed normalization candidates (domain-dependent):
- **SoTA ≠ popular.** Method families enter UNM through [[00_G.02 - SoTA Harvester & Synthesis|G.2]] claim structures + edition pins + evidence pins; “widely used” is not a validity claim by itself.
...
## [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:12 - Relations

**Builds on / cites**
- [[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]] (pattern template)
- [[00_E.20 - Mechanism Introduction Protocol|E.20]] (governing-pattern discipline for mechanism‑intension content)
- [[03_A.15.03 - SlotFillingsPlanItem|A.15.3]] (P2W planned baseline seam, when UNM is used in flows)
- [[00_F.18 - Local-First Unification Naming Protocol|F.18]] (alias docking / token continuity, when renaming or retiring legacy UNM tokens)
- [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] (U.Mechanism.Intension shape; specialization discipline)
- [[06_A.19.CHR - CHRMechanismSuite|A.19.CHR]] (CHR suite boundary; slot lexicon; suite protocols)
- [[05_A.19.CN - CN-frame (comparability & normalization)|A.19.CN]] (CN_Spec normalization + comparability routing)
- [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]] (MM‑CHR evidence/calibration carriers)
- [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]] (CG-frame legality gates used downstream)
- [[00_G.02 - SoTA Harvester & Synthesis|G.2]] (SoTA synthesis packs as the method‑family ingress; wiring‑only integration)
- [[00_E.18 - Transformation Flow Structure|E.18]] (when UNM is used in transformation-flow structures/graphs; P2W freshness/work routing)
- [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (congruence/quotient intuition, when referenced)

**Used by**
- CHR suite protocols (normalize stage), when `comparability.mode` requires normalization-based comparability.

## [[07_A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]:End
