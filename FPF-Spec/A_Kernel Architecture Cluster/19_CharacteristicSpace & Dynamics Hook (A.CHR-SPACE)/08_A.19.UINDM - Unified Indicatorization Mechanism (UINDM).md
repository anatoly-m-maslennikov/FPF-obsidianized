...
> [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/05_A.06.01 - U.Mechanism - Law-governed application to a SubjectKind over a RangedValueKind|A.6.1]] governs the **template** of `U.Mechanism.Intension`.
>
> **Canonicalization hook (ID‑continuity‑safe):** any other appearances of the UINDM intension (e.g., a legacy grounding stub in [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/05_A.06.01 - U.Mechanism - Law-governed application to a SubjectKind over a RangedValueKind|A.6.1]] or suite prose in [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/06_A.19.CHR - CHRMechanismSuite|A.19.CHR]]) SHALL be reduced to a **Tell + Cite** stub pointing to **`A.19.UINDM:4.1`**, while preserving the original section headings and their public `PatternId:SectionPath` IDs for continuity (alias‑dock legacy tokens rather than deleting them).
> Such stubs MUST NOT restate SlotIndex / LawSet / Admissibility content (no “second center of gravity” via near‑duplicate prose).

## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:0 - At a glance (didactic, informative)
...
* **P2W seam:** concrete edition/policy pins are bound in planned baseline plan items ([[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/03_A.15.03 - SlotFillingsPlanItem|A.15.3]] + `A.19.CHR:4.7.2`); executions only record effective refs/pins in `Audit`.
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:1 - Problem frame
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:2 - Problem
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:3 - Forces
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:4 - Solution
...
Method semantics (“how to pick indicators”) remain out of suite core: they belong in SoTA packs ([[G_Discipline SoTA Patterns Kit/03_02_SoTA Harvester & Synthesis/00_G.02 - SoTA Harvester & Synthesis|G.2]]) and wiring‑only extension modules (`GPatternExtension` blocks), while UINDM remains the stable mechanism boundary.

### [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:4.1 - Mechanism.Intension (normative)
...
* **LawSet** (CHR‑lawful indicatorization):
  ...
  3. **No implicit NCV⇒indicator:** measurability/NCV is not sufficient; indicators exist only via `IndicatorChoicePolicySlot` (cites [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/05_A.19.CN - CN-frame (comparability & normalization)|A.19.CN]] `indicator_policy`).
...
### [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:4.2 - Interpretation notes (informative)
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:5 - Archetypal Grounding (informative)

### [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:5.1 - Tell
...
### [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:5.2 - Show (U.System) — cross‑unit engineering dashboard
...
### [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:5.3 - Show (U.Episteme) — robust evaluation across environments
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:6 - Bias-Annotation (informative)
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:7 - Conformance Checklist
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:8 - Common Anti‑Patterns and How to Avoid Them
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:9 - Consequences
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:10 - Rationale
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:11 - SoTA-Echoing
...
**Pack note (Phase‑3):** this pattern does not currently cite a UINDM‑specific [[G_Discipline SoTA Patterns Kit/03_02_SoTA Harvester & Synthesis/00_G.02 - SoTA Harvester & Synthesis|G.2]] SoTA pack/ClaimSheet. If/when such a pack is introduced, replace the bibliographic pointers below with the pack’s `ClaimSheetId` citations, keeping the mechanism semantics unchanged.

### [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:11.1 - SoTA alignment map (normative)
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:12 - Relations

* **Builds on**

  * [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/05_A.19.CN - CN-frame (comparability & normalization)|A.19.CN]] (CN‑Spec, specifically `indicator_policy`).
...
* **Used by**
  ...
  * [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/06_A.19.CHR - CHRMechanismSuite|A.19.CHR]] (suite membership and suite protocols; UINDM is the `indicatorize` stage).
* **Coordinates with**

  * [[G_Discipline SoTA Patterns Kit/01_00_Frame Standard and Comparability Governance - CG-Spec/00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]] (CG‑Spec / MinimalEvidence) when indicator choice is evidence‑gated.
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]:End
