...
## [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:1 - Problem frame

Use [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] when an object version is to be improved or judged, but the evaluation that says what "better" means is not yet available, not yet explicit, or not yet adequate for the object.

[[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]] says how a `CharacteristicSpace` is structured: declared characteristics, declared scales, slots, value sets, declared coordinate groups, and no hidden normalization or aggregation. [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] says how to make such a `CharacteristicSpace` for the evaluated object, so that an evaluation can later evaluate that object and [[00_E.23 - Quality Improvement Loop Method|E.23]] can run an improvement loop without inventing values.
...
**Not this pattern when.** If a suitable evaluation already exists, cite it and use [[00_E.22 - Improvement-Oriented Quality Evaluation Question Framing|E.22]] for question framing or [[00_E.23 - Quality Improvement Loop Method|E.23]] for repeated improvement. Use [[00_A.17 - Canonical “Characteristic” (A.CHR-NORM)|A.17]], [[00_A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC-KERNEL)|A.18]], and [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]] when the live problem is one characteristic, one scale, or measurement legality. Use [[01_C.16.P - Characteristic and Scale Precision Restoration|C.16.P]] first when candidate coordinate wording still hides whether the use under repair is a characteristic, scale, coordinate, score, metric label, quality-term repair, or governing-pattern relation. Use [[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]] when the live problem is the structure of `CharacteristicSpace` itself. Use [[00_C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]] when the evaluated EntityOfConcern is a composite engineering quality family that already fits Q-Bundle form. Use [[00_F.18 - Local-First Unification Naming Protocol|F.18]] when the live problem is durable naming. Use [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]], [[01_E.09.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]], or [[01_E.02.DA - FPF Pillar-Adequacy Evaluation CharacteristicSpace|E.2.DA]] when the evaluated EntityOfConcern is respectively one FPF pattern version, one `DRR`, or one FPF-level Pillar-adequacy evaluated EntityOfConcern.
...
**What this buys.** [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] gives improvement work a way to create the missing evaluation before the loop starts. It keeps [[00_E.23 - Quality Improvement Loop Method|E.23]] universal and simple: [[00_E.23 - Quality Improvement Loop Method|E.23]] changes the object and asks an evaluation to re-evaluate it; [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] helps build that evaluation when none is yet adequate.
...
## [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:2 - Problem
...
## [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:3 - Forces
...
| **Measurement legality vs ordinal judgment** | Some coordinates are measured through [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)]]; others are evidence-backed ordinal content values. The evaluation must say which is which. |
...
## [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:4 - Solution
...
### [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:4.1 - Local names and kind settlement
...
| `E23StartCondition` | Minimum condition for using this evaluation inside [[00_E.23 - Quality Improvement Loop Method]]. | Not the improvement loop itself. |
...
### [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:4.2 - Construction moves
...
11. **Start [[00_E.23 - Quality Improvement Loop Method|E.23]] only after evaluation values exist.** A repeated improvement loop can start only when the evaluated object version, evidence basis, result form, and evaluation are recoverable enough for re-evaluation.

### [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:4.3 - Evaluation specification minimum

[[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] does not prescribe a publication or record form. It states which evaluation characteristic-space elements must be recoverable before an evaluation characteristic space is reusable for judgement or improvement. The selected publication or record form may be an FPF pattern, local engineering standard, rubric, table, review form, model card section, protocol note, or project rule, but that form is not governed here. The evaluation characteristic-space specification must make these items recoverable by value:
...
This minimum is a content requirement, not a file-format requirement. For an FPF pattern publication form, [[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]] still governs the authoring form. [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] only states what the evaluation must make recoverable so that [[00_E.22 - Improvement-Oriented Quality Evaluation Question Framing|E.22]] can frame an improvement-oriented quality evaluation and [[00_E.23 - Quality Improvement Loop Method|E.23]] can run a repeated improvement loop.
...
### [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:4.4 - Discriminating-case test
...
### [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:4.5 - Scale-set improvement
...
Use [[00_E.23 - Quality Improvement Loop Method|E.23]] for the repeated improvement method over the scale set when the improvement aim is live. The evaluation for that meta-level improvement may be:

- this pattern's conformance checklist for whether the scale set is constructible and usable;
- [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] when the evaluation characteristic-space specification is itself an FPF pattern version;
- [[01_E.09.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]] when the decision record selecting the scale set is the `DRR` decision-adequacy object being evaluated;
- [[01_E.02.DA - FPF Pillar-Adequacy Evaluation CharacteristicSpace|E.2.DA]] when the scale set changes FPF-level Pillar adequacy;
- [[00_F.18 - Local-First Unification Naming Protocol|F.18]] when the live problem is name choice for the scale-set heads;
- [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]], [[00_A.17 - Canonical “Characteristic” (A.CHR-NORM)|A.17]], [[00_A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC-KERNEL)|A.18]], or [[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]] when the live problem is measurement or characteristic-space legality.
...
## [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:5 - Worked slices

**Show, FPF pattern quality.** The evaluated object kind is one FPF pattern version. The existing evaluation is [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]], so [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] stays closed unless [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] itself is being redesigned. [[00_E.23 - Quality Improvement Loop Method|E.23]] may improve the pattern version under [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]].

**Show, DRR adequacy.** The evaluated object kind is one `DRR` version for a declared campaign-decision use. The existing evaluation is [[01_E.09.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]]. If a campaign needs a different DRR adequacy coordinate, [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] can test whether that coordinate belongs inside [[01_E.09.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]], another evaluation, or no current FPF pattern.

**Show, FPF Pillar adequacy.** The evaluated object is FPF as a corpus or release candidate. [[00_E.02 - The Eleven Pillars|E.2]] gives the Pillars; [[01_E.02.DA - FPF Pillar-Adequacy Evaluation CharacteristicSpace|E.2.DA]] is the evaluation. [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] explains why [[01_E.02.DA - FPF Pillar-Adequacy Evaluation CharacteristicSpace|E.2.DA]] needs evaluated object, use, eligibility, coordinates, evidence loci, stop meanings, and neighbour governing relations rather than a Pillar essay.

**Show, name improvement.** The evaluated object is a durable term candidate. [[00_F.18 - Local-First Unification Naming Protocol|F.18]] already supplies a grouped lexical quality vector: `SemanticFidelity`, `CognitiveErgonomics`, `MorphologicalActionFit`, and `AliasRisk`, plus NQD discipline over candidate names. [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] treats [[00_F.18 - Local-First Unification Naming Protocol|F.18]] as an existing local evaluation for naming, not as a reason to build another one.

**Show, no evaluation yet.** A team says "make this onboarding method better" but cannot say better for whom, by what values, or with what stop. [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] opens before [[00_E.23 - Quality Improvement Loop Method|E.23]]: it names evaluated object kind, user, use, contrast cases, candidate characteristics, scales, floors, missingness, protected trade-offs, and neighbour governing relations. Only then can [[00_E.22 - Improvement-Oriented Quality Evaluation Question Framing|E.22]] frame an improvement-oriented quality evaluation and [[00_E.23 - Quality Improvement Loop Method|E.23]] improve the method.

## [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:6 - Conformance checklist
...
| `CC-A19ECS-9` | If made reusable beyond one local use, the evaluation characteristic-space specification SHALL make the minimum items in `A.19.ECS:4.3` recoverable by value. If the selected publication form is an FPF pattern, [[00_E.08 - FPF Authoring Conventions & Style Guide]] also applies to that publication form. | Prevents underspecified evaluations. |
...
## [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:7 - Common anti-patterns
...
## [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:8 - Relations

| Pattern | Relation |
|---|---|
| [[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)]] | Defines `CharacteristicSpace`. [[01_A.19.ECS - Evaluation CharacteristicSpace Construction]] gives the method for constructing one evaluation `CharacteristicSpace` for an evaluated object. |
| [[00_A.17 - Canonical “Characteristic” (A.CHR-NORM)]], [[00_A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC-KERNEL)]], [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)]] | Govern characteristics, scales, scale values, coordinates, measures, units, and measurement legality. [[01_A.19.ECS - Evaluation CharacteristicSpace Construction]] uses them by reference for each slot. |
| [[00_C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles]] | Governs Q-Bundle normal form for composite engineering quality families. [[01_A.19.ECS - Evaluation CharacteristicSpace Construction]] may select or repair the characteristic-space part before a Q-Bundle endpoint is used. |
| [[00_E.22 - Improvement-Oriented Quality Evaluation Question Framing]] | Frames one improvement-oriented quality-evaluation question after an evaluation is declared. [[01_A.19.ECS - Evaluation CharacteristicSpace Construction]] constructs the missing or inadequate evaluation. |
| [[00_E.23 - Quality Improvement Loop Method]] | Governs repeated improvement after evaluated object version and evaluation are declared. [[01_A.19.ECS - Evaluation CharacteristicSpace Construction]] provides the evaluation when it is missing or underdesigned. |
| [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]] | Existing evaluation for one FPF pattern version. [[01_A.19.ECS - Evaluation CharacteristicSpace Construction]] explains the construction shape but does not replace [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace]]. |
| [[01_E.09.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace]] | Existing evaluation for one `DRR` decision-adequacy claim. [[01_A.19.ECS - Evaluation CharacteristicSpace Construction]] does not replace it. |
| [[01_E.02.DA - FPF Pillar-Adequacy Evaluation CharacteristicSpace]] | Existing evaluation for FPF-level Pillar adequacy. [[01_A.19.ECS - Evaluation CharacteristicSpace Construction]] explains why it must publish evaluated object, coordinates, values, evidence loci, status, and stop meanings. |
| [[00_F.18 - Local-First Unification Naming Protocol]] | Existing naming discipline with a grouped lexical quality vector. Use [[00_F.18 - Local-First Unification Naming Protocol]] for durable term and name improvement. |
| [[01_C.16.P - Characteristic and Scale Precision Restoration]], [[02_C.16.Q - Quality-Term Precision Restoration]], [[00_E.10 - Unified Lexical Rules for FPF]], [[09_A.06.P - Relational Precision Restoration (RPR) - Kind-Explicit Qualified Relation Discipline]], [[00_C.02.P - Epistemic Precision Restoration]] | Repair overloaded characteristic/scale/score, quality, lexical, relation, and source-use wording before it becomes a coordinate or status value. |
| [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)]], [[00_C.19 - Explore-Exploit Governor (E-E-LOG)]], [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]], [[00_G.09 - Parity and Benchmark Harness]], [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator]] | Govern OEE/NQD novelty, diversity, archive, pool, selected-set, parity, and refresh semantics. An evaluation may supply `Q` values, but it does not govern the rest of OEE/NQD. |
| [[00_C.29 - Mathematical Lens Use]] | Governs mathematical-lens use when a mathematical structure is used to define or justify coordinates. |
...
## [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:9 - Consequences

A conforming [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] result lets [[00_E.22 - Improvement-Oriented Quality Evaluation Question Framing|E.22]] ask a useful improvement-oriented quality-evaluation question and lets [[00_E.23 - Quality Improvement Loop Method|E.23]] run a repeated improvement loop without inventing values during the loop. It also gives object-specific evaluation patterns such as [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]], [[01_E.09.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]], [[01_E.02.DA - FPF Pillar-Adequacy Evaluation CharacteristicSpace|E.2.DA]], and [[00_F.18 - Local-First Unification Naming Protocol|F.18]] a common construction shape: evaluated object kind, use, contrast cases, coordinates, value meanings, evidence basis, result-row shape, calibration points, coordinate-specific payloads, protected trade-offs, status meanings, and local stop or reopen condition.
...
| Improvement pressure can damage the intended value when the evaluation is a weak proxy. | Current proxy-risk anchors: `Goodhart's Law in Reinforcement Learning` (ICLR 2024) and current catastrophic-Goodhart reward-misspecification work (NeurIPS 2024); retained lineage: Goodhart taxonomy. | [[01_A.19.ECS - Evaluation CharacteristicSpace Construction]] requires evidence rules, missingness rules, protected trade-offs, and lowering/reopen conditions before a loop can treat a value as improved. | It is not an anti-measurement rule; it makes the measurement or ordinal evaluation explicit enough to be challenged. |
| OEE/NQD separates the quality side from novelty, diversity, archive, pool, and selected-set semantics. | Current QD and OEE/NQD neighbour basis: QD uses quality pressure with novelty/diversity and archive/front practice, while current FPF [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)]], [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)]], [[00_C.19 - Explore-Exploit Governor (E-E-LOG)]], [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry]], [[00_G.09 - Parity and Benchmark Harness]], and [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator]] keep archive, pool, selected-set, parity, and refresh semantics named by value. | The evaluation may supply `Q` values, while novelty, diversity, archive, front, pool, selected-set publication, parity, and refresh remain with neighboring pattern governing the claims. | [[01_A.19.ECS - Evaluation CharacteristicSpace Construction]] does not govern OEE/NQD generation, selection, archive, parity, or refresh. |

## [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:11 - Rationale
...
Placing this method under [[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]] keeps the ontology clean. [[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]] governs the structure of `CharacteristicSpace`; [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] governs the construction method for evaluations of declared EntityOfConcern kinds and uses. [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] governs the selected characteristics, scales, coordinate construction, and evaluation-use boundaries of the evaluation characteristic space, not its publication or record form. An FPF pattern is only one possible publication form when the evaluation belongs in FPF; a local rubric, standard, table, or project rule is enough when the use is local. [[00_E.23 - Quality Improvement Loop Method|E.23]] stays a universal loop method because it does not need to know how every domain chooses its scales. Domain and FPF-specific evaluations such as [[00_E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]], [[01_E.09.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]], [[01_E.02.DA - FPF Pillar-Adequacy Evaluation CharacteristicSpace|E.2.DA]], and [[00_F.18 - Local-First Unification Naming Protocol|F.18]] keep coordinate choices inside those evaluations.

## [[01_A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:End
