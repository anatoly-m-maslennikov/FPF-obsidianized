...
> **Status:** Stable
> **Normativity:** Normative (unless explicitly marked informative)
> **Placement:** Part A / CN‑Spec cluster ([[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]]) / CHR mechanism-governing patterns (Phase‑3)
> **Source:** FPF / CHR Phase‑3 mechanism-governing patterns
> **Modified:** 2026-01-20

**Governing-pattern note (Phase‑3 canonicalization):** this pattern governs the canonical `U.Mechanism.Intension` for `ULSAM.IntensionRef` (CHR suite stage `fold_Γ?`). Mechanism-intension semantics are governed by explicitly designated governing patterns ([[E_The FPF Constitution and Authoring Guides/19_20_Mechanism Introduction Protocol/00_E.20 - Mechanism Introduction Protocol|E.20]]).
[[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/05_A.06.01 - U.Mechanism - Law-governed application to a SubjectKind over a RangedValueKind|A.6.1]] governs the **template** of `U.Mechanism.Intension` and the `U.MechAuthoring` discipline; this pattern governs the **ULSAM-specific** slots, operations, laws, admissibility, and audit obligations for that template.

**ID continuity note.** When migrating away from any legacy “card location”, preserve public anchors: keep the legacy section heading/ID as a **Tell + Cite stub** (or dock aliases via [[F_The Unification Suite (U-Suite)- Concept-Sets, SenseCells & Contextual Role Assignment/18_Local-First Unification Naming Protocol/00_F.18 - Local-First Unification Naming Protocol|F.18]]) rather than deleting or silently renaming it.

**Canonicalization hook (ID‑continuity‑safe):** any other appearances of ULSAM intension content (e.g., a legacy grounding stub in [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/05_A.06.01 - U.Mechanism - Law-governed application to a SubjectKind over a RangedValueKind|A.6.1]] or suite prose in [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/06_A.19.CHR - CHRMechanismSuite|A.19.CHR]]) SHALL be reduced to a **Tell + Cite** stub pointing to **`A.19.ULSAM:4.1`**, while preserving the original section headings and their public `PatternId:SectionPath` IDs for continuity (alias‑dock legacy tokens rather than deleting them).
...
* **Alias‑dock, don’t break:** if any legacy tokens exist, dock them via [[F_The Unification Suite (U-Suite)- Concept-Sets, SenseCells & Contextual Role Assignment/18_Local-First Unification Naming Protocol/00_F.18 - Local-First Unification Naming Protocol|F.18]] + [[E_The FPF Constitution and Authoring Guides/09_10_Unified Lexical Rules for FPF/00_E.10 - Unified Lexical Rules for FPF|E.10]] rules; do not silently replace tokens “by смысл”.
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:0 - At a glance (didactic, informative)
...
* **P2W seam:** edition/policy binding for `ΓFoldRef` / `MinimalEvidenceRef` is selected in planned baseline ([[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/03_A.15.03 - SlotFillingsPlanItem|A.15.3]] + CHR P2W hook), not invented at run time.
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:1 - Problem frame (normative)
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:2 - Problem (normative)
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:3 - Forces (normative)
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:4 - Solution (normative)
...
Method semantics (“which aggregation family to use”) remain out of suite core: they belong in SoTA packs ([[G_Discipline SoTA Patterns Kit/03_02_SoTA Harvester & Synthesis/00_G.02 - SoTA Harvester & Synthesis|G.2]]) and wiring‑only extension modules (`GPatternExtension` blocks), while ULSAM remains the stable mechanism boundary.

### [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:4.1 - Mechanism.Intension (canonical; normative)
...
### [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:4.2 - Interpretation notes (didactic, informative)
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:5 - Archetypal grounding (didactic, informative)

### [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:5.1 - Tell
...
### [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:5.2 - Show
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:6 - Bias-Annotation (informative)
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:7 - Conformance Checklist (normative)

| ID | Requirement |
|----|-------------|
| **CC‑A19ULSAM‑0** | **MechAuthoring discipline:** the canonical ULSAM Mechanism.Intension in `A.19.ULSAM:4.1` MUST satisfy [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/05_A.06.01 - U.Mechanism - Law-governed application to a SubjectKind over a RangedValueKind]] `U.MechAuthoring` and the relevant `CC‑UM.*` checks; this pattern does not override the `U.Mechanism.Intension` shape. |
...
| **CC‑A19ULSAM‑9** | **P2W separation:** planned baseline plan items MUST bind `ΓFoldRef`/`MinimalEvidenceRef`/editions ([[A_Kernel Architecture Cluster/15_Role-Method-Work Alignment (Contextual Enactment)/03_A.15.03 - SlotFillingsPlanItem]] + CHR P2W hook); these bindings MUST NOT be invented as run-time decisions inside the suite protocol. |
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:8 - Common anti-patterns (didactic, informative)
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:9 - Consequences (didactic, informative)
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:10 - Rationale (didactic, informative)
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:11 - Known uses (didactic, informative)
...
- In [[E_The FPF Constitution and Authoring Guides/17_18_Transformation Flow Structure/00_E.18 - Transformation Flow Structure|E.18]] transformation-flow structures: ULSAM appears as a mechanism instance node whose `ΓFoldRef` / `MinimalEvidenceRef` are bound in planned baseline (P2W), while Audit records the effective pins used at run time.

## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:12 - Builds on / Relates to

**Builds on (cite, don’t duplicate).**
- [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/05_A.06.01 - U.Mechanism - Law-governed application to a SubjectKind over a RangedValueKind|A.6.1]] (`U.Mechanism.Intension` shape; `U.MechAuthoring`; CC‑UM discipline).
- [[A_Kernel Architecture Cluster/06_Signature Stack & Boundary Discipline/14_A.06.05 - U.RelationSlotDiscipline - SlotKind, ValueKind, RefKind, and slot-operation discipline|A.6.5]] (slot discipline; SlotIndex as a projection).
- [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/06_A.19.CHR - CHRMechanismSuite|A.19.CHR]] (CHR suite boundary; stage `fold_Γ?`; CHR SlotKind Lexicon).
- [[G_Discipline SoTA Patterns Kit/01_00_Frame Standard and Comparability Governance - CG-Spec/00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]] (`CG-Spec.Γ_fold`, `CG-Spec.SCP`, `CG-Spec.MinimalEvidence`; admissibility gate).
- [[A_Kernel Architecture Cluster/18_Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC-KERNEL)/00_A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC-KERNEL)|A.18]] (CSLC).
- [[B_Trans-disciplinary Reasoning Cluster/02_03_Trust and Assurance Calculus (F-G-R with Congruence)/00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (Γ‑fold defaults for `R_eff`, including WLNK; trust skeleton).

**Relates to (coordination, not governing-pattern assignment).**
- [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/05_A.19.CN - CN-frame (comparability & normalization)|A.19.CN]] (`CN‑Spec`), via `CNSpecSlot.acceptance` gating in admissibility.
- [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]], [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/09_A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]], [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/11_A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]], and [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/12_A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]] as adjacent CHR stages (Uses contour; no governing-pattern assignment transfer).
- Part G SoTA packs and wiring ([[G_Discipline SoTA Patterns Kit/03_02_SoTA Harvester & Synthesis/00_G.02 - SoTA Harvester & Synthesis|G.2]] + `G.*:Ext.*`) for method family selection and edition/policy binding.

## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:13 - SoTA-Echoing (informative; not a center of gravity)

SoTA here is treated as **method-family source publications and [[G_Discipline SoTA Patterns Kit/03_02_SoTA Harvester & Synthesis/00_G.02 - SoTA Harvester & Synthesis|G.2]] claim sheets to be wired** through `G.*:Ext.*` wiring, not as kernel semantics. ULSAM’s contribution is the stable boundary: explicit, admissible, auditable folding.

**SoTA vs popular note.** This section records alignment to post‑2015 evidence‑backed practice. It is **not** a mandate to use fashionable methods; method semantics stay in SoTA packs ([[G_Discipline SoTA Patterns Kit/03_02_SoTA Harvester & Synthesis/00_G.02 - SoTA Harvester & Synthesis|G.2]]) and wiring modules, while this pattern fixes the stable mechanism boundary.

**Pack note (Phase‑3):** this pattern does not currently cite a ULSAM‑specific [[G_Discipline SoTA Patterns Kit/03_02_SoTA Harvester & Synthesis/00_G.02 - SoTA Harvester & Synthesis|G.2]] SoTA pack/ClaimSheet. If/when such a pack is introduced, replace the bibliographic pointers below with the pack’s `ClaimSheetId` citations, keeping the mechanism semantics unchanged.
...
## [[A_Kernel Architecture Cluster/19_CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)/10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:End
