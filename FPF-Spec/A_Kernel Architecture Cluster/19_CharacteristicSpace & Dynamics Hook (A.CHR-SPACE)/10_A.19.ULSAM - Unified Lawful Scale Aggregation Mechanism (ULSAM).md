...
**Governing-pattern note (Phase‑3 canonicalization):** this pattern governs the canonical `U.Mechanism.Intension` for `ULSAM.IntensionRef` (CHR suite stage `fold_Γ?`). Mechanism-intension semantics are governed by explicitly designated governing patterns ([[00_E.20 - Mechanism Introduction Protocol|E.20]]).
[[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] governs the **template** of `U.Mechanism.Intension` and the `U.MechAuthoring` discipline; this pattern governs the **ULSAM-specific** slots, operations, laws, admissibility, and audit obligations for that template.
...
**Canonicalization hook (ID‑continuity‑safe):** any other appearances of ULSAM intension content (e.g., a legacy grounding stub in [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] or suite prose in [[06_A.19.CHR - CHRMechanismSuite|A.19.CHR]]) SHALL be reduced to a **Tell + Cite** stub pointing to **`A.19.ULSAM:4.1`**, while preserving the original section headings and their public `PatternId:SectionPath` IDs for continuity (alias‑dock legacy tokens rather than deleting them).
...
* **Alias‑dock, don’t break:** if any legacy tokens exist, dock them via [[00_F.18 - Local-First Unification Naming Protocol|F.18]] + [[00_E.10 - Unified Lexical Rules for FPF|E.10]] rules; do not silently replace tokens “by смысл”.
...
## [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:0 - At a glance (didactic, informative)
...
* **P2W seam:** edition/policy binding for `ΓFoldRef` / `MinimalEvidenceRef` is selected in planned baseline ([[03_A.15.03 - SlotFillingsPlanItem|A.15.3]] + CHR P2W hook), not invented at run time.
...
## [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:1 - Problem frame (normative)
...
## [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:2 - Problem (normative)
...
## [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:3 - Forces (normative)
...
## [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:4 - Solution (normative)
...
Method semantics (“which aggregation family to use”) remain out of suite core: they belong in SoTA packs ([[00_G.02 - SoTA Harvester & Synthesis|G.2]]) and wiring‑only extension modules (`GPatternExtension` blocks), while ULSAM remains the stable mechanism boundary.

### [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:4.1 - Mechanism.Intension (canonical; normative)
...
### [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:4.2 - Interpretation notes (didactic, informative)
...
## [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:5 - Archetypal grounding (didactic, informative)

### [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:5.1 - Tell
...
### [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:5.2 - Show
...
## [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:6 - Bias-Annotation (informative)
...
## [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:7 - Conformance Checklist (normative)

| ID | Requirement |
|----|-------------|
| **CC‑A19ULSAM‑0** | **MechAuthoring discipline:** the canonical ULSAM Mechanism.Intension in `A.19.ULSAM:4.1` MUST satisfy [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind]] `U.MechAuthoring` and the relevant `CC‑UM.*` checks; this pattern does not override the `U.Mechanism.Intension` shape. |
...
| **CC‑A19ULSAM‑9** | **P2W separation:** planned baseline plan items MUST bind `ΓFoldRef`/`MinimalEvidenceRef`/editions ([[03_A.15.03 - SlotFillingsPlanItem]] + CHR P2W hook); these bindings MUST NOT be invented as run-time decisions inside the suite protocol. |
...
## [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:8 - Common anti-patterns (didactic, informative)
...
## [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:9 - Consequences (didactic, informative)
...
## [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:10 - Rationale (didactic, informative)
...
## [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:11 - Known uses (didactic, informative)
...
- In [[00_E.18 - Transformation Flow Structure|E.18]] transformation-flow structures: ULSAM appears as a mechanism instance node whose `ΓFoldRef` / `MinimalEvidenceRef` are bound in planned baseline (P2W), while Audit records the effective pins used at run time.

## [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:12 - Builds on / Relates to

**Builds on (cite, don’t duplicate).**
- [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] (`U.Mechanism.Intension` shape; `U.MechAuthoring`; CC‑UM discipline).
- [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] (slot discipline; SlotIndex as a projection).
- [[06_A.19.CHR - CHRMechanismSuite|A.19.CHR]] (CHR suite boundary; stage `fold_Γ?`; CHR SlotKind Lexicon).
- [[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]] (`CG‑Spec.Γ_fold`, `CG‑Spec.SCP`, `CG‑Spec.MinimalEvidence`; legality gate).
- [[00_A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC-KERNEL)|A.18]] (CSLC).
- [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (Γ‑fold defaults for `R_eff`, including WLNK; trust skeleton).

**Relates to (coordination, not governing-pattern assignment).**
- [[05_A.19.CN - CN-frame (comparability & normalization)|A.19.CN]] (`CN‑Spec`), via `CNSpecSlot.acceptance` gating in admissibility.
- [[08_A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]], [[09_A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]], [[11_A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]], and [[12_A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]] as adjacent CHR stages (Uses contour; no governing-pattern assignment transfer).
- Part G SoTA packs and wiring ([[00_G.02 - SoTA Harvester & Synthesis|G.2]] + `G.*:Ext.*`) for method family selection and edition/policy binding.

## [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:13 - SoTA-Echoing (informative; not a center of gravity)

SoTA here is treated as **method-family source publications and [[00_G.02 - SoTA Harvester & Synthesis|G.2]] claim sheets to be wired** through `G.*:Ext.*` wiring, not as kernel semantics. ULSAM’s contribution is the stable boundary: explicit, admissible, auditable folding.

**SoTA vs popular note.** This section records alignment to post‑2015 evidence‑backed practice. It is **not** a mandate to use fashionable methods; method semantics stay in SoTA packs ([[00_G.02 - SoTA Harvester & Synthesis|G.2]]) and wiring modules, while this pattern fixes the stable mechanism boundary.

**Pack note (Phase‑3):** this pattern does not currently cite a ULSAM‑specific [[00_G.02 - SoTA Harvester & Synthesis|G.2]] SoTA pack/ClaimSheet. If/when such a pack is introduced, replace the bibliographic pointers below with the pack’s `ClaimSheetId` citations, keeping the mechanism semantics unchanged.
...
## [[10_A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]:End
