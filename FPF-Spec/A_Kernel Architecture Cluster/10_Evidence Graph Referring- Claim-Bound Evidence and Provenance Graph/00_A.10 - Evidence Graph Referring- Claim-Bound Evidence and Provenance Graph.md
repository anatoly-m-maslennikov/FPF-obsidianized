...
## [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]:1 - Problem frame
...
3. **Self‑justifying loops.** A holon attempts to evidence itself (violates [[00_A.12 - External Transformer & Reflexive Split|A.12]] externality), producing cyclic provenance and unverifiable conclusions.
...
The business effect is predictable: confidence badges cannot be defended, cross‑scale consistency ([[00_A.09 - Cross-Scale Consistency (C-3)|A.9]]) is broken, and iteration slows because every review re‑litigates “where did this come from?”.

## [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]:3 - Forces
...
| **Externality vs. reflexivity** | Evidence must be produced by an external TransformerRole ([[00_A.12 - External Transformer & Reflexive Split]]) ↔ Some systems adapt themselves (need reflexive modelling without self‑evidence). |
...
### [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]:4.2 - Evidence relations (two relations, two flavours)
...
### [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]:4.3 - SCR and RSCR Symbol Carrier Registers
Every `Γ_epist` aggregation **SHALL** emit an **SCR**: an exhaustive register of **symbol carriers** substantively used in the aggregate, with id, type, version, date, source conditions, and optional `PortionOf` ([[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]) for sub-carriers.
...
### [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]:4.4 - Scope alignment across Role-Method-Work
...
### [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]:4.5 - External TransformerRole
The system that produces or interprets evidence is **external** to the holon under evaluation. If true reflexivity is essential, model a **meta‑holon** ([[00_A.12 - External Transformer & Reflexive Split|A.12]]): the self‑updating holon becomes the *object* of a meta-holon external transformer (the “mirror”), restoring objectivity.

### [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]:4.6 - Gamma-flavour evidence connection points
...
| Verifiable credential, credential view, or register excerpt | Treat as an [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] carrier with issuer or trust root, governing status register when one exists, register entry or source-record id and version, holder or subject binding, verifier, proof result, status result, currentness, relying context, effective window, revocation window, and acceptance rule. When those checks pass, it may evidence credential-currentness for that holder and relying context. It evidences permission, authorization, role assignment, status assertion, or gate passage only when the register entry or another source named by value such as [[08_A.02.08 - U.Commitment (Deontic Commitment Object)]], [[09_A.02.09 - U.SpeechAct (Communicative Work Object)]], [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment]], [[02_A.06.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work-Effects)]], or [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]] creates or states that effect for the bounded context. |
| Copied approval or review summary | Show the original [[09_A.02.09 - U.SpeechAct (Communicative Work Object)]] `SpeechActRef` or issuing act when approval or authorization is claimed, or the original reviewed source when only review-content currentness is claimed. Add copy relation, currentness, scope, window, evidence-producing work occurrence, and whether a separate commitment relation or work relation is being claimed. Copy evidence is not approval by itself. |
...
| Dashboard status tile | For gate-passage or release reliance, show dashboard query, source, time, window, currentness, source order, freshness policy, rival explanation, and the current [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]] `GateDecision` or `DecisionLogRef` with gate profile, gate version, release target, and work target; the [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] evidence path evidences that source chain. A status display is not gate passage or work occurrence by itself. |
| Rollback command-like cue | Show command source, authorization source, actor, affected work target or claim target, scope, window, and whether the cue is only an [[10_A.06.A - U.ActionInvitationPrecisionRestoration - Affordance and Action-Invitation Precision Restoration (ACT-INV)]] action invitation. A command cue is not performed-work evidence. |
| Rollback performed-work result | Show [[01_A.15.01 - U.Work]] `U.Work` occurrence, method trace or work trace, logs, outcome evidence, and time window. Performed-work evidence is not approval, assurance, or gate passage by itself. |
| Generated explanation | Use [[04_E.17.EFP - ExplanationFaithfulnessProfile - explanation-use discipline over existing MVPK faces]] to classify the explanation relation and source-finding use. For reliance, show claim-bound attribution alignment: every operative claim relied on maps to a source passage, carrier, or `governingPatternRef` or `authoritySourceRef` named by value that evidences that claim in the relying context. When that mapping is complete, [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] may evidence those operative claims as source-backed evidence; the explanation itself still does not issue, approve, authorize, pass a gate, evidence performed work, or raise assurance. |
| Model card or datasheet used as evidence | Show documented bounded-use statement or external intended-use field, version, window, evaluation condition, limitations, evidence carriers, and whether a [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] assurance claim is being made. Documentation does not become readiness or assurance by presence. |
| Extracted-source chain to gate or release claim | Name the source reference, the first lossy or non-commutative transform step, the FPF relation or pattern governing that transform ([[02_A.06.03.CR - ConservativeRetextualization- EntityOfConcern-Preserving Textual Re-Expression]], [[03_A.06.03.RT - Representation-Scheme Transition- EntityOfConcern-Preserving Representation-Scheme Transition]], [[01_A.06.03.CSC - Controlled Semantic Coarsening]], [[04_E.17.EFP - ExplanationFaithfulnessProfile - explanation-use discipline over existing MVPK faces]], [[00_E.17.ID.CR - ComparativeReviewUnit - bounded comparison over comparative review units]], or [[00_E.18 - Transformation Flow Structure]] where applicable), the bounded inference relation after the step, the `governingPatternRef` or `authoritySourceRef` named by value that carries the claim being made, the source reopen trigger, and the gate claim or release claim blocked until those source relations are recoverable. |
...
**Repeated missing-source indicator.** If the same visible carrier family repeatedly returns stale, contested, no-source, or no-currentness [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] results, record a source-system repair action: instrument the source, expose decision-source refs, add currentness checks and status checks, preserve claim-bound source links for generated or copied outputs, require credential views to show status windows and currentness windows, require model documentation and data documentation to expose intended-use and evaluation-condition fields, or require provenance labels and attestation labels to name their bounded claim type. Repetition is an indicator that the source relation or display needs repair; it is not a reason to make each acting user rebuild the evidence path manually.
...
What this does not authorize: [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] does not approve, authorize work or reliance, pass a gate, release, create permission, create a commitment, assign a role, record a work occurrence, or raise assurance. It supplies the evidence path and evidence-use classification that [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]], [[00_A.06 - Signature Stack & Boundary Discipline|A.6]], [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] gate-decision sources, [[00_A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] constraint-validity sources, [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]] speech-act sources, [[08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]] commitment sources, [[01_A.15.01 - U.Work|A.15.1]] work-occurrence sources, or another `governingPatternRef` or `authoritySourceRef` named by value may consume.

### [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]:4.6b - Local evidence-use classifier and `RelianceDisposition` for source-looking evidence uses

Use this subsection when a visible source is being treated as evidence for a claim, act, work move, gate, release, review claim, assurance use, or problem-side P2W use. The first [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] move is to recover the evidence kind and the bounded evidence use. Broad source words such as `source`, `metric`, `confidence`, `conformant`, `safe`, `ready`, `certified`, `approval`, or `permission` are only recovery prompts; they do not name the evidence relation by themselves.

This subsection uses a local reliance-use classifier, not a Core evidence-kind ontology. Its practical gain is a smaller next move: recover the evidence relation, name the bounded evidence use and unsupported attempted use, then either stay inside [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] or apply the governing pattern for the stronger claim being made. It is not a required project review step and does not ask the practitioner to inspect every source-looking carrier or display.

Section role: the first table is an [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] recognition aid, the `RelianceDisposition` table is a minimum local record aid, and the worked source-overread slices are regression slices and review slices. They are not project checklists, a required sequence, a new evidence ontology, or a general source classifier. Use only the row that answers the attempted evidence use, then stop when the bounded evidence relation, unsupported attempted use, and reopen condition are clear. This local section keeps the attempted use inside the [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence relation; it does not create an extra SEMIO authority or cross-pattern relation vocabulary.

Affordability card: orientation or source-finding remains a cue and stops here; bounded reliance states one bounded evidence use, unsupported attempted use, window, and reopen condition; threshold reliance applies the minimum governing pattern only when the [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] material-reliance threshold is met: behavior, safety, release, compliance, public or protocol behavior, access, resource allocation, people status, team status, operational action, or controlled-object regulation would materially change. Plain wording remains ordinary unless it changes bounded use, source relation, evidence, gate, assurance, work, decision, or neighboring governing-pattern claim.

Cheap stop: if a bounded claim, current carrier, evidence path, window, bounded evidence use, unsupported attempted use, and reopen trigger are present, and there is no assurance claim, gate relation, work relation, control-bearing relation, release relation, or met [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] material-reliance threshold, stay in [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]. Do not open [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], [[04_B.02.05 - Supervisor-Subholon Feedback Loop|B.2.5]], or a broad evidence pack merely because the source looks official, quantitative, generated, credentialed, or safety-related.

Common wrong first classification: a visible source is approval, permission, safety, or readiness. First honest entry: recover the [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence path for one bounded claim or use; approval, permission, safety, readiness, gate passage, and work authority stay with their governing patterns when those relations are being claimed.

Plain move palette: `RelianceDisposition=pass` means proceed only inside the bounded evidence use; `RelianceDisposition=degrade` means use only a narrower or reversible version; `RelianceDisposition=abstain` means do not decide yet; `RelianceDisposition=reopen` means changed or contested evidence relation defeated the previous evidence-use classification; `RelianceDisposition=evidence-needed` means ask for the named missing evidence at the named decision point; `RelianceDisposition=safety-case-required` means apply [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] because the [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] material-reliance threshold is met; `RelianceDisposition=blocked-current-use` means block the current attempted use until the evidence path or governing source relation changes.

| Source-looking evidence use or attempted use | First [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] move | Escalation trigger | Forbidden overread |
| --- | --- | --- | --- |
| Ordinary source-backed report, record, citation, observation, model card, datasheet, data card, or publication excerpt | Name the claim, evidence carrier, producer or method trace, evidence path, currentness window, bounded evidence use, unsupported attempted use, and reopen trigger. | Open [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] only when an assurance claim is being made or the [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] material-reliance threshold is met; open [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]] for a gate decision currently being relied on, [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)]] or [[01_A.15.01 - U.Work]] for work, or another governing neighboring pattern only when that relation is being claimed; open [[04_B.02.05 - Supervisor-Subholon Feedback Loop]] only when a controlled object is regulated through a feedback channel, evidence channel, cadence, window, supervisory relation, or control relation. | Evidence presence as approval, gate passage, assurance, release permission, work authority, control authority, or safety acceptance. |
| Confidence, calibration, prediction interval, abstention reason, or selective-action cue | Name the act, context, window, calibration population, exchangeability condition, shift condition, applicability condition, and stop condition for the bounded evidence use. Use `RelianceDisposition=pass` or `RelianceDisposition=degrade` only for that bounded use, and state the unsupported attempted use beside it. | Open [[00_C.27 - Temporal Claim Adequacy- State Readings, Temporal Trends, and Intervention-Sensitive Temporal Change]] or [[00_G.11 - Telemetry-Driven Refresh & Decay Orchestrator]] when timing, expiry, refresh, distribution shift, monitoring, or applicability change alters the bounded act; open [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] when an assurance claim is being made or the [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] material-reliance threshold is met. | Confidence as global permission, trust, readiness, safety, release reliance, or engineering justification. |
| Generated explanation, generated summary, or didactic reconstruction | Keep the rendering in [[04_E.17.EFP - ExplanationFaithfulnessProfile - explanation-use discipline over existing MVPK faces]] as explanation or source-finding unless each relied-on operative claim has an [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] evidence path or another source relation that evidences the operative claim. | Apply [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]], [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]], [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]], [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)]], or another governing pattern only for the operative claim being relied on. | Explanation wording as evidence, assurance, approval, gate passage, work occurrence, or permission. |
| Conformance label, `CV.Status`, benchmark result, score, semantic-fidelity marker, or CV-looking publication near release | Recover the declared relation: measurement or marker relation, [[00_A.20 - U.Flow.ConstraintValidity - Eulerian]] step-local CV status, [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]] gate check, [[00_E.19 - Pattern Quality Gates- Review and Refresh Profiles]] pattern-quality result, [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)]] characterization, or external-rule source named by value. | Open [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]] only when an `OperationalGate(profile)` consumes effective gate-check refs and emits a `GateDecision`; open [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] only when an assurance claim is being made. | Conformance or score as value, adequacy, release confidence, work occurrence, safety, trust, or gate passage outside the declared relation. |
...
For [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] use, `RelianceDisposition` is a local disposition over the evidence path and the bounded reliance use. Outside a table column already headed `RelianceDisposition`, write the qualified form `RelianceDisposition=...` and bind it to the named attempted use, currentness and window when relevant, bounded evidence use, unsupported attempted use, and reopen or stop condition; it is not `CV.Status`, `GateDecision`, selector result, or `ProblemCard@Context` state.

Observed-effect or consequence evidence may be used only for what happened or is credibly recorded. If the attempted use says the source caused, prevented, would have changed, or is responsible for that effect, leave ordinary [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] reliance and open [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] plus any relevant evidence, work, or assurance relation.

If a proxy marker, benchmark, confidence value, dashboard metric, or score becomes the primary driver for action, release, resource allocation, people status, team status, or P2W priority, check whether the claim being made also raises an [[00_E.13 - Pragmatic Utility and Value Alignment|E.13]] proxy-to-objective question. Do not open [[00_E.13 - Pragmatic Utility and Value Alignment|E.13]] for every metric; open it only when the proxy is being used as the target or decision driver.
...
| `RelianceDisposition` | [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] classification | Minimum [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] statement |
| --- | --- | --- |
...
| `RelianceDisposition=safety-case-required` | The [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] material-reliance threshold is met: reliance on the visible source may materially change behavior, safety, release, compliance, public or protocol behavior, access, resource allocation, people status, team status, operational action, or controlled-object regulation. | State the threshold trigger and apply [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] for the minimum reliance safety assurance record, with [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] evidence paths for the source claims. |
...
| Slice | [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] usable classification | Unsupported lift |
| --- | --- | --- |
| Software supply-chain attestation is cited near a release conversation. | The attestation may evidence bounded origin, build method or production trace, verifier-rule, holder, and currentness claims. | Runtime safety, release approval, gate passage, or assurance unless [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]], [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)]], or another relation governing the asserted use is asserted for that use. |
...
### [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]:4.7 - Causal evidence relation values in evidence paths

Evidence graph paths used for causal-use claims must carry the [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]]-governed `CausalEvidenceSupportBasis` value without redefining causal estimands or causal-use authority. In this subsection, `SupportBasis` is a [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] field-value name; it is not the loose FPF prose word "support".

The [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] values that [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] may carry in an evidence path are:
...
[[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] consumes this value set from [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]]; it does not add `causalAssumptionOnlySupport` or `noCausalEvidenceSupport` as causal-evidence values. Assumption-only and no-evidence-use cases are represented by causal assumptions, a [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] causal-use verdict, bounded use, unsupported attempted use, or abstain in [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]]/[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], not by a second causal-evidence vocabulary.
...
What changes in practice: an evidence path can show that a carrier evidences a causal-use claim, but it must also show the causal evidence relation value and the relevant [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] references when the claim changes from observation to intervention or from intervention to counterfactual comparison.

What this does not authorize: [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] does not identify causal effects, create an estimand, certify target-trial emulation, or decide counterfactual sampling realizability; it stores and makes recoverable the evidence graph path and the [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] causal-evidence refs needed by [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] and [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]].

## [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]:5 - Archetypal Grounding
...
## [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]:6 - Conformance Checklist
...
| **CC‑A10.6 (Externality)**              | The evidencing `TransformerRole` MUST be **external** to the holon under evaluation ([[00_A.12 - External Transformer & Reflexive Split]]). Reflexive cases require modelling a meta-holon and an external mirror.                                                                      | Prevents self-creation and self-evidence paradoxes.            |
...
| **CC-A10.13 (Causal evidence-value path)** | If an evidence path is used for a causal-use claim, it **MUST** carry `CausalEvidenceSupportBasis` from [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability]] and any relevant `CausalIdentificationProfile`, `CounterfactualSamplingRealizabilityProfile`, or `CausalUseEvidenceDesignRecord` refs; [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]] **MUST NOT** identify causal effects or create a second causal-evidence value set. | Keeps evidence graph path recoverable without moving causal authority out of [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability]]. |
...
| **Feeds [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] typed assurance claims** | Evidence relations provide evidence inputs such as `R` and `CL` only for a named typed assurance claim. | [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] is not a generic trust or assurance score; cite the claim named by value and relying context. |

## [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]:8 - Rationale and SoTA alignment
...
* **Holonic parsimony.** By drawing a firewall between **mereology** ([[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]) and **provenance**, [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] prevents semantic leakage and keeps the holarchy well‑typed.
* **Role–Method–Work clarity.** Evidence relationing explicitly rides on [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]: **roles** act via **methods** specified at design‑time and produce **work** observed at run‑time. This keeps agency, policy, and execution disentangled yet connected.
* **Credential, provenance, attestation, status-register, and generated-source currentness.** Verifiable-credential and digital-identity practice separates issuer or trust root, holder binding, proof result, status result, revocation, effective window, audience, and relying context. Some bounded contexts also treat a register entry or status-source entry as the source that creates or changes role assignment, status assertion, permission, duty, or gate state; a credential view, pass, badge, dashboard cell, API response, screenshot, or certificate excerpt is then a publication of that source, not automatically the source itself. C2PA content provenance plus SLSA and in-toto attestations separate bounded origin, history, build, and process claims from truth, approval, release, safety, gate passage, permission, or assurance; their consumer-side verifier or policy acceptance rule is part of the relying context, not implied by source-carrier presence. LLM citation and generated-explanation practice requires claim-bound attribution alignment before operative claims are relied on. [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] adopts issuer, holder, verifier, status, currentness recoverability, status-source recoverability, and claim-bound attribution as evidence-path invariants, adapts credential practice, provenance practice, attestation practice, model documentation, data documentation, register-backed status display, and generated-explanation practice as FPF source-role and carrier-relation inputs, and rejects visual display, copied text, generated text, provenance mark, credential display, register excerpt, or attestation form as evidence of an operative action invitation, gate, role assignment, status assertion, work occurrence, assurance, or bounded work effect without source relation named by value.

Practical result from that cited practice: provenance, attestation, credential, status-register, and generated-source practice rejects the shortcut that provenance means truth, safety, release, permission, or assurance. The local [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] result is bounded origin, history, build, holder or status currentness, generated-claim source mapping, bounded evidence use, unsupported attempted use, and reopen when the verifier, trust model, status or currentness rule, source mapping, or source-order relation changes.

## [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]:9 - Relations

* **Builds on:** [[00_A.01 - U.Holon, U.System, and U.Episteme|A.1]] Holonic Foundation; [[00_A.04 - Temporal Duality & Open-Ended Evolution Principle|A.4]] Temporal Duality; **[[00_A.12 - External Transformer & Reflexive Split|A.12]] Transformer Externalization**; **[[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] Advanced Mereology**; **[[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] Role–Method–Work Alignment**.
* **Constrains / used by:** [[00_B.01 - Universal Algebra of Aggregation (Γ)|B.1]] (all Γ‑flavours: `Γ_sys`, `Γ_epist`, `Γ_method`, `Γ\_time`, `Γ_work`); [[01_B.01.01 - Dependency Graph & Proofs|B.1.1]] (Dependency Graph & Proofs).
* **Enables:** **[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] Trust Calculus** (R inputs, CL inputs, auditability); [[00_B.04 - Canonical Evolution Loop|B.4]] Canonical Evolution Loop (clean DesignRunTag bridges).
* **Coordinates with:** [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] when an evidence path is used for a causal-use relation; [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] carries the evidence-provenance path, while [[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] governs the causal-use question, `CausalEvidenceSupportBasis` value, identification, realizability, bounded use, and unsupported attempted use.
* **Coordinates with:** [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] for work or reliance disposition, [[00_A.06 - Signature Stack & Boundary Discipline|A.6]] for mixed boundary wording, [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] for assurance, [[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] for `OperationalGate(profile)`, `GateDecision`, and `DecisionLogRef`, [[00_A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] for `ConstraintValidity` status or witness, [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]] for speech-act refs, [[08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]] for commitments, and [[01_A.15.01 - U.Work|A.15.1]] for work occurrences. [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] supplies evidence paths for those sources; it does not create their gate decision, commitment, role effect, status effect, work-occurrence, assurance, bounded work effect, or bounded reliance effect.

## [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]:10 - Legacy source interpretation and neighboring-pattern notes
...
When a legacy source also claims approval, permission, gate passage, assurance, causal authority, measured comparability, representation shift, or publication-face effect, keep [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] to the evidence-provenance graph relation and apply the neighboring governing pattern for that extra claim.

## [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]:10a - Evidence carriers for quantum-like statements

Use [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] when a quantum-like statement needs evidence rather than only a local modeling note. The practical question is not "is this quantum-like source impressive?" but "which carrier evidences which minimal claim, under which time window and method?"
...
6. Choose the next pattern: stay in [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] for carrier evidence relation, apply [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] for assurance claims, apply [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]] for measurement legality, apply [[00_F.09 - Alignment & Bridge across Contexts|F.9]] for bridge or export loss, or apply a `C.26.*` pattern for the remaining probe, state, or envelope question.
...
- a [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] assurance tuple when the claim will feed readiness, audit, release, compliance, or comparative assurance;
...
## [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]:10b - [[00_C.29 - Mathematical Lens Use|C.29]] mathematical-lens use relation

> If a mathematical lens needs evidence relation, write the evidence path, source currentness, provenance, and any model-card or datasheet evidence use in [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]. A [[00_C.29 - Mathematical Lens Use|C.29]] output may state only the C.29-local lens-use value for the mathematical-lens use claim; it is not an evidence path, currentness proof, provenance record, or evidence-carrier substitute. Assurance or release confidence goes to [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]; measurement construction or comparability goes to [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]].

## [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]:End
