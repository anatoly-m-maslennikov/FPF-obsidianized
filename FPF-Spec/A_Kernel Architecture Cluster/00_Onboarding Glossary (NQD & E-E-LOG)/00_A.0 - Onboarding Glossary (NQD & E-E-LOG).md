...
**Builds on.** [[00_E.02 - The Eleven Pillars|E.2]] (**P‑10 Open‑Ended Evolution; P‑2 Didactic Primacy**), [[00_A.05 - Open-Ended Kernel & Extension Layering|A.5]], [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]–[[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]] - **Coordinates with.** [[00_E.07 - Archetypal Grounding Principle|E.7]], [[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]], [[00_E.10 - Unified Lexical Rules for FPF|E.10]]; [[00_F.17 - Unified Term Sheet (UTS)|F.17]] (UTS); [[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]], [[00_G.09 - Parity and Benchmark Harness|G.9]]–[[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]] - **Constrains.** Any pattern/UTS row that **describes a generator, selector, typed portfolio publication, or set-return publication surface**.
...
## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:1 - Problem Frame
...
## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:2 - Problem
...
## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:3 - Forces
...
## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:4 - Solution - Normative onboarding glossary and publication hooks

### 4.1 Plain one‑liners (normative on‑ramp; formal anchors in [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]–[[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]])

| Term                      | Plain definition (on‑ramp)                                                                                                                                   | See        |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------- |
| **Novelty (N)**           | *How unlike the known set in your declared **CharacteristicSpace**. **Compute admissibly** (declared `DescriptorMapRef` + `DistanceDefRef`; no ad-hoc normalisation). | [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)]], [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)]] |
| **Use‑Value (U / ValueGain)** | *What it helps you achieve now under your **CG‑Frame**; tie to acceptance/tests; **publish units, scale kind, polarity, ReferencePlane**.                   | [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)]], [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)]] |
| **Constraint‑Fit (C)**    | *Satisfies must‑constraints (Resource/Risk/Ethics)*; legality via **CG‑Spec**; **unknowns propagate** (never coerce to zero).                                | [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)]], [[00_G.04 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring]]  |
| **Diversity_P (declared retained set)** | *Adds a new niche to the declared retained set or portfolio-publication surface; measured against the **active archive/grid**, not a single list; declare **ReferencePlane** for each head.          | [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)]], [[00_C.18 - Open-Ended Search Calculus (NQD-CAL)]] |
| **E/E‑LOG**               | *Named, versioned **explore↔exploit** policy*; governs when to widen space vs refine candidates; **policy‑id is published**.                                   | [[00_C.19 - Explore-Exploit Governor (E-E-LOG)]]       |
| **ReferencePlane**        | *Where a value lives:* **world** (system), **concept** (definition), **episteme** (about a claim). **Plane‑crossings add CL^plane** (penalties to **R only**); cite policy‑id. | [[00_F.09 - Alignment & Bridge across Contexts]], [[00_G.06 - Evidence Graph & Provenance Ledger]]   |
| **Scale Variables (S)**  | *The **monotone knobs** along which improvement is expected* (e.g., parameterisation breadth, data exposure, iteration budget, resolution). **Declare S** for any generator/selector claimed to scale. | [[01_C.18.01 - Scaling-Law Lens Binding (SLL)]]       |
| **Scale Elasticity (χ)** | *Qualitative class of improvement when moving along S* (e.g., **rising**, **knee**, **flat** in the declared window). Used as a **selection lens**; numeric laws live in domain contexts.              | [[01_C.18.01 - Scaling-Law Lens Binding (SLL)]]       |
| **BLP (Bitter‑Lesson Preference)**  | *Default policy that **prefers general, scale‑amenable methods** over domain‑specific heuristics, unless forbidden by deontics or overturned by a scale‑probe.*                                        | [[01_C.19.01 - Bitter-Lesson Preference (BLP)]], [[00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)]] |
| **Iso‑Scale Parity**  | *Fair comparison across candidates at equalised **scale budgets** along S*; may also include **scale‑probes** (two points) to test elasticity.                                                         | [[00_G.09 - Parity and Benchmark Harness]], [[01_C.18.01 - Scaling-Law Lens Binding (SLL)]]  |
...
1. **UTS surface (Part F).** When a **UTS row describes a generator, selector, typed portfolio publication, or set-return publication surface**, it **MUST** surface **N, U, C, Diversity_P, E/E‑LOG `policy‑id`, `ReferencePlane`**, with **units, scale, and polarity** typed under **MM‑CHR** and **CG‑Spec**, and admissible references to `DescriptorMapRef` and `DistanceDefRef`. *(Row schema: [[00_F.17 - Unified Term Sheet (UTS)|F.17]]; shipping via [[00_G.10 - SoTA Pack Shipping|G.10]].)*
...
4) Apply **[[00_G.05 - Multi-Method Dispatcher & MethodFamily Registry|G.5]]** selection/dispatch with parity pins; **return a declared set result** (`Front`, `Archive`, `Shortlist`, or `RankedShortlist` as appropriate), not a single score or an unnamed "portfolio".
5) **Publish to UTS** + **PathIds/PathSliceId**; **Illumination Map** is **report‑only telemetry** by default.

## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:5 - Archetypal Grounding
...
## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:6 - Bias-Annotation
...
## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:7 - Conformance Checklist (SCR/RSCR stubs)
...
| **CC‑A0‑6** | Apply **E.7/E.8**: include a **U.System** and a **U.Episteme** illustration when claiming generative behaviour; obey **[[00_E.10 - Unified Lexical Rules for FPF]]** register hygiene; use the exact subsection title **“Archetypal Grounding.”** | Locks didactic primacy; prevents jargon drift.                                  |
...
## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:8 - Consequences
...
## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:9 - Rationale

This pattern **instantiates P‑10 Open‑Ended Evolution** by making *generation‑selection‑publication* **operational** at the on‑ramp: readers get just enough shared vocabulary to run *search as standard practice*. It aligns with **Didactic Primacy (P‑2)** and **LEX‑BUNDLE ([[00_E.10 - Unified Lexical Rules for FPF|E.10]])** by keeping definitions *plain‑first* and scale‑lawful, and with **Patterns Layering (P‑5)** by pointing to [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]–[[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]] for formal anchors without tool lock‑in. The post‑2015 line (MAP‑Elites → CMA‑ME/MAE → Differentiable QD/MEGA → QDax; POET/Enhanced‑POET/Darwinian Goedel Machine) normalised **quality‑diversity** and **open‑endedness** as first‑class search objectives; this glossary surfaces those ideas as **publication standards**, not tool recipes.

## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:10 - Relations

**Builds on.** **[[00_E.02 - The Eleven Pillars|E.2]] Pillars** (P-10, P-2, P-6), **[[00_A.05 - Open-Ended Kernel & Extension Layering|A.5]]** (Open-Ended Kernel), **B.5/B.5.2.1** (Abductive loops + NQD integration), **[[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]–[[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]]** (Creativity-CHR, NQD-CAL, E/E-LOG).

**Coordinates with.** **E.7/E.8** (Archetypal Grounding; Authoring template), **[[00_E.10 - Unified Lexical Rules for FPF|E.10]]** (LEX‑BUNDLE), **[[00_F.17 - Unified Term Sheet (UTS)|F.17]]** (UTS), **G.5/G.9–[[00_G.12 - DHC Dashboards (Discipline-Health time-series; admissible telemetry; generation-first)|G.12]]** (set‑returning selectors, **iso‑scale** parity, shipping & refresh).
...
**Editor’s cross‑reference.** For agentic orchestration of scalable tool‑calls under **BLP**/**SLL**, see **[[00_C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]] (Agent‑Tools‑CAL)**.

## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:QF.0a - Scope of this glossary

This pattern is an **on‑ramp**: it **does not replace** [[00_C.17 - Characterising Generative Novelty & Value (Creativity-CHR)|C.17]]–[[00_C.19 - Explore-Exploit Governor (E-E-LOG)|C.19]]. It binds Plain definitions to **publication/telemetry** expectations so newcomers can *use* NQD/E/E‑LOG immediately while experts follow the formal trails.

## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:QF.1 - Early set-result and metric-kind vocabulary
...
## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:QF.1a - Helper declarations for set-result language
...
## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:QF.2 - First public wording for shortlisted results
...
## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:QF.2a - Set/space reading reading glosses
...
## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:QF.2b - Practitioner-facing reading cue
...
## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:QF.2c - First-use classification check
...
## [[00_A.0 - Onboarding Glossary (NQD & E-E-LOG)|A.0]]:End
