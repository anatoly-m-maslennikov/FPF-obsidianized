...
**Placement.** Part A (Kernel), **before [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]** (“U.Mechanism”).
**Builds on.** **[[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]** (USM: context slices and scopes), **[[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]]** (pattern form and section order), **[[00_E.10 - Unified Lexical Rules for FPF|E.10]]** LEX-BUNDLE (registers, naming, stratification), **[[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]** D.CTX (Context discipline).

**Coordinates with.** **[[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]** (U.Mechanism), **[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]** (`U.RelationSlotDiscipline` for n-ary arguments), **[[03_E.05.03 - Unidirectional Dependency|E.5.3]]** (Unidirectional Dependency), **[[00_E.10 - Unified Lexical Rules for FPF|E.10]]** (LEX-BUNDLE), and **Part F** (harnesses and cross-context transport; naming). Conformance keywords: RFC 2119.

## [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:0 - Use and boundary
...
## [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:1 - Problem frame

FPF already uses “signatures” to stabilise public promises of **reusable extension vocabularies** and, via **[[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]**, of **mechanisms**. But declaration publishers also need stable, minimal declarations for **theories**, **methods** (operational families), and **disciplines** (regulated vocabularies). Without **one** universal notion of signature:
...
[[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]] demands one publication voice and section order; [[00_E.10 - Unified Lexical Rules for FPF|E.10]] demands lexical discipline across strata. [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] provides the common kernel shape these patterns presuppose.

## [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:2 - Problem
...
## [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:3 - Forces
...
A `U.Signature` is conceptual: it contains no implementation, no packaging or CI metadata, and no Γ-builders. If a family wants to expose a Γ‑like *builder or aggregator*, publish it **outside** the Signature Block (typically as an **[[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]** Mechanism‑level operator) and **namespace** it under the Signature id; do not treat Γ as part of the canonical Vocabulary.

### [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:4.1 - The **Signature Block (universal form)**
...
1. **SubjectBlock** — ⟨**SubjectKind**, **BaseType**, **SliceSet**, **ExtentRule**, **ResultKind?**⟩.
   *SubjectKind* names the EntityOfConcern kind declared by the signature ([[00_C.03 - Kinds, Intent-Extent, and Typed Reasoning (Kind-CAL)|C.3]]); *BaseType* is the `U.Type` the signature ranges over (CHR Spaces appear here **as types**, not as field names); *SliceSet* addresses the quantification domain (USM; e.g., **ContextSliceSet**); *ExtentRule* computes `Extension(SubjectKind, slice)` ([[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]); *ResultKind?* (optional) is the output kind when outputs differ from the SubjectKind.
   ...
   * **SubjectKind (EntityOfConcern kind).** The EntityOfConcern kind declared by the signature ([[01_C.03.01 - U.Kind & SubkindOf (Core)|C.3.1]]), ordered by `⊑`. It carries no Scope.
   ...
   * **ExtentRule (extent).** A rule yielding `Extension(SubjectKind, slice)` ([[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]); this is the quantifier’s domain, computed per slice.
...
#### [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:4.1.1 - SlotSpec for argument positions (normative; see [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]])
...
where:
* **SlotKind_i** is a named position in the relation or operator (Tech name with `…Slot` suffix) whose semantics are documented (see [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]).
...
Full discipline and lexical rules for **SlotKind, ValueKind, and RefKind** are given in [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] `U.RelationSlotDiscipline` and [[00_E.10 - Unified Lexical Rules for FPF|E.10]] (§8.1). [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] requires that every vocabulary‑level relation or operator that takes arguments **declare** these SlotSpecs; downstream patterns MAY provide templates for common shapes (e.g., episteme slots in [[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]]).
...
### [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:4.2 - Profile specialisations (normative; structure‑preserving)
...
* **`profile = FormalSubstrate`** — *formal‑deductive specialization*
  ...
  **MVPK pins:** **`No‑Realization` pin (mandatory)** on `PlainView` and `TechCard` asserting that handler semantics live only in **[[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] `U.Mechanism::U.EffectRealization`**.
...
### [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:4.3 - Effect-discipline and handler-semantics split (normative)

If a Signature’s **Vocabulary** includes an **EffectDiscipline** (operation and effect signatures), the Signature **SHALL NOT** declare **handler semantics** or any **EffectRealization**. Such realizations are declared only under **[[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] `U.Mechanism`** and cited by **ref-id** on faces where needed. This mirrors the modern algebraic-effects separation between *operation signatures* and *handlers* while keeping [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] purely conceptual.

### [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:4.4 - Declaration Rules (strict-distinction-aware; lexically disciplined)
...
* **Context discipline.** Bind Applicability to a **`U.BoundedContext`**. If cross‑context use is intended, **name** the crossing and **reference** the Bridge (Part F and [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]); [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] does **not** prescribe **compatibility and loss tables (CL, including `CL^plane`)** or penalty formulas.
...
#### [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:4.4.1 - SignatureManifest (imports and provides; normative)
...
- **Stratum dependency.** `imports` **MUST** respect **[[03_E.05.03 - Unidirectional Dependency|E.5.3]]** (Unidirectional Dependency) and **[[00_E.10 - Unified Lexical Rules for FPF|E.10]]** strata and token-class discipline; do not import from a lower stratum or across a forbidden dependency direction.
...
- **Law reference.** When downstream text depends on laws or constraints from an imported signature, it SHOULD cite the corresponding **ClaimId** ([[02_A.06.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work-Effects)|A.6.B]]), not paraphrase prose.

The [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] four-row Block remains the canonical meaning locus for Vocabulary, Laws, and Applicability. The manifest only declares dependency edges and exported names.
...
*MVPK publication discipline for Signatures (normative).* When publishing a `U.Signature` via MVPK ([[03_E.17 - Multi-View Publication Kit|E.17]]), faces **SHALL** be typed projections that add **no new claims**; any numeric or comparable statement **MUST** pin unit, scale, reference-plane, and **EditionId** to **CG-Spec and MM-CHR** where applicable. For `profile=FormalSubstrate` signatures, faces **MUST** carry a **No-Realization pin** (handlers and realizers absent). For Principle-level signatures, faces **MUST NOT** introduce units, ReferencePlane, or Transport (first mention occurs in UNM).

### [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:4.5 - Specialisation knobs (for downstream patterns)

[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] exposes **three** conceptual knobs; downstream patterns ([[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]], method or discipline specifications) may **tighten** them:

1. **Builder policy.** The Block MUST NOT export Γ-builders. If a family publishes a Γ-like builder or aggregator, it MUST be outside the Block (typically as an **[[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]** Mechanism-level operator), explicitly marked optional, and namespaced under the Signature id.

2. **Transport clause.** If cross-context or cross-plane use is part of the design, the signature **may declare** a conceptual Transport clause; **[[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]** gives a concrete schema (Bridge, **CL, CL^k, and CL^plane**; Bridges per **[[00_F.09 - Alignment & Bridge across Contexts|F.9]]**, penalties per **[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]**, **CL^plane** per **[[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]]**), but [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] remains agnostic about penalty shapes.

3. **Morphisms.** Families may define `SigMorph` (refinement, conservative extension, equivalence, quotient, product) to relate signatures; **[[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]** instantiates this for mechanisms. Where such morphisms, or downstream **substitution and retargeting** laws (e.g., **[[06_A.06.02 - U.EffectFreeEpistemicMorphing - Effect-free morphisms of epistemes|A.6.2]]–[[08_A.06.04 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]]**), act on **n‑ary relations or morphisms**, they **SHALL** express their access, update, and retargeting discipline in terms of **SlotSpecs**  (SlotKind, ValueKind, RefKind) rather than unnamed parameter indices, using **[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] `U.RelationSlotDiscipline`** as the normative slot calculus.

### [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:4.6 - Profile‑specialisation as a structure‑preserving morphism (normative)
...
— No **AdmissibilityConditions**, **operational guards**, or **handler semantics** are introduced by the profile (those live under **[[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]**).
...
## [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:5 - Archetypal Grounding (Tell–Show–Show)
...
| **Applicability (Scope and Context)** | Context: *industrial robotics*; stance: design; time notion: discrete ticks. Cross-context transport not declared. | Context: *clinical metrics*; stance: analysis; validity windows declared; cross-context transport via Bridge (concept only; details per [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind]]). Numeric comparability bound to CHR and CG-Spec. |

*Why these two?* [[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]] requires pairs from **U.System** and **U.Episteme** to demonstrate trans‑disciplinary universality.

### [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:5.1 - Near-miss and anti-cases

**Near-miss: handler hidden in a `U.Signature(profile=FormalSubstrate)` declaration.** A `U.Signature(profile=FormalSubstrate)` declaration for an algebraic-effects calculus may list operation symbols, inference kinds, and equational laws. If the same block says how a database handler commits transactions, the text has crossed into [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] `U.Mechanism`: keep the operation and effect signature in [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]], and publish the handler realization as a mechanism that cites this signature.
...
## [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:6 - Bias-Annotation (lenses and defaults)
...
* **Register hygiene.** Keep Tech vs Plain register pairs; avoid tooling or vendor talk in Kernel prose ([[00_E.10 - Unified Lexical Rules for FPF|E.10]]).

## [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:7 - Conformance Checklist (normative)

| ID | Requirement |
| --- | --- |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑1** | A conformant text labelled **`U.Signature`** **SHALL** expose the **four‑row Signature Block**: *SubjectBlock; Vocabulary; Laws; Applicability*. A visual split of SubjectBlock into **Subject** and **Quantification** lines is allowed; it still counts as **one** conceptual row. |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑2** |  The Signature Block MUST remain conceptual: no code or CI metadata, no tool bindings, no execution steps, no implementation details, and no Γ-builder exports. Dependency and export metadata belongs in the `SignatureManifest` (§4.4.1), not inside the four-row Block. |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑3** | Applicability **binds** a `U.BoundedContext`; if cross-context use is intended, a **Transport clause** is *named* (Bridge reference) without re-stating Part F and [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]] details (including any **CL^plane**). |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑4** | Where numeric comparability is implied, Applicability **binds** to **CG-Spec and MM-CHR** legality (normalize-then-compare; scale and unit alignment). |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑5** | Families that specialise [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]] (e.g., [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind]], method profiles, or discipline profiles) MAY add extra constraints and projection views, but MUST preserve the four-row Block as the canonical core (no extra semantic rows). |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑6** | Under [[00_E.10 - Unified Lexical Rules for FPF]] and [[00_E.05 - Four Guard-Rails of FPF]], tokens MUST respect strata and family segregation: never redefine Kernel tokens in an Extension, Context, or Instance signature; instead, import and align. |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑7** | The **Laws** row contains **axioms and invariants** only; **AdmissibilityConditions** and operational admissions **MUST** appear only in **[[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind]] Mechanisms** that consume this Signature. |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑8 (No‑Realization on Signatures with EffectDiscipline).** | If **EffectDiscipline** appears in **Vocabulary**, faces **MUST** carry a **`No‑Realization` pin** and **MUST NOT** publish handler semantics; any **EffectRealization** is referenced ([[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind]]) by id only. |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑9 (CHR‑binding without units or Transport).** | Signatures that declare **measurability intent** (e.g., PrincipleFrame) **SHALL NOT** publish **units, ReferencePlane, ComparatorSet, or Transport**; those are declared only by **UNM** and cited by edition or ref-id where consumers require numeric comparability. |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑10 (UNM‑priority on faces).** | Any numeric or comparable claim on a Signature face **pins** **CG-Spec and ComparatorSet edition ids** and, where scale or plane conversion occurs, **UNM.TransportRegistry edition** with **CL and CL^plane policy-ids**; **penalties are recorded only in `R` or `R_eff`**. |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑11 (Bridge‑only crossings).** | Cross-context or cross-plane reuse of Signature claims **MUST** name a **Bridge** (UTS row) and **MUST NOT** imply implicit equivalence by label; losses are recorded via **CL** (penalties → **R**). |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑12 (Profile conformance).** | If the Signature declares `profile=FormalSubstrate` or `profile=PrincipleFrame`, the corresponding **profile pins** in §4.2 are **mandatory**; failure to emit them makes the Signature **non‑conformant** for that profile. |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑13 (Profile morphism discipline).** | Applying a profile **SHALL** satisfy §4.6 (structure‑preserving morphism: SubjectBlock preserved, Vocabulary and Laws monotone, Applicability restrictive, no admissibility or handlers). |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑14 (SlotSpec for argument positions).** | Any `U.Signature` whose **Vocabulary** declares n‑ary relations or operators **SHALL** provide, for each argument position, a **SlotSpec** triple `⟨SlotKind, ValueKind, refMode⟩` (with `refMode ∈ {ByValue \| RefKind}`) as per [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)]] `U.RelationSlotDiscipline`. |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑15 (Slot and Ref lexical discipline on signatures).** | Names of SlotKinds and RefKinds used in SlotSpecs **MUST** obey [[00_E.10 - Unified Lexical Rules for FPF]] and [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)]] lexical guards: tokens ending with **`…Slot`** denote SlotKinds only; tokens ending with **`…Ref`** denote either RefKinds or episteme fields whose type is a RefKind; no ValueKind ends with these suffixes. |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑16 (SlotSpecs for n‑ary relations).** | Any `U.Signature` whose **Vocabulary** declares an **n‑ary relation or morphism** **SHALL** assign to each parameter position a `SlotSpec_i = ⟨SlotKind, ValueKind, refMode⟩` as defined in **[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)]] `U.RelationSlotDiscipline`**; SlotSpecs live inside the Vocabulary row’s per‑relation parameter block and **MUST NOT** introduce additional rows beyond the four‑row Block. |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑17 (SlotSpec-based substitution laws).** | Specialisations of [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]] that define **substitution, retargeting, or profile application** over n-ary relations or morphisms (e.g., **[[06_A.06.02 - U.EffectFreeEpistemicMorphing - Effect-free morphisms of epistemes]]–[[08_A.06.04 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism]]**) **SHALL** phrase their rules in terms of **SlotSpecs** (SlotKind, ValueKind, and RefKind) rather than unnamed parameter indices and **SHALL** obey the `…Slot` and `…Ref` lexical discipline in **[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)]]** and **[[00_F.18 - Local-First Unification Naming Protocol]]**. |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑18 (Manifest required for reuse).** | If a signature is intended to be imported or reused, it MUST include a `SignatureManifest` (§4.4.1) with explicit `id`, `version`, `imports`, and `provides`. |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑19 (Imports acyclicity).** | If `imports` is present, it MUST be acyclic (no cycles in the signature import graph). |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑20 (No redeclare across imports).** | If `imports` is present, `provides(S)` MUST NOT re‑declare any symbol already provided by any transitive import of `S`. |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑21 (No ghost dependencies).** | If `imports` is present, any non-Kernel **SymbolId** referenced in the **SubjectBlock** or **Vocabulary** rows that is **not** provided by this signature MUST be provided by some imported signature. ClaimIds, BridgeIds, policy-ids, and EditionIds are exempt. |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑22 (Realization opacity).** | If a family publishes any Realization of a `U.Signature`, that Realization **MUST** treat imported Signatures as **opaque** (depend only on their `provides` symbols and cited ClaimIds), and **MUST NOT** reference internal structure of imported Signatures. |
| **CC‑[[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]]‑23 (Monotone Realization).** | A Realization **MAY** tighten but **MUST NOT** relax the Signature’s Laws; if weaker laws are needed, publish a new Signature (or publish an explicit refinement morphism) rather than weakening the existing Signature Laws. |

## [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:8 - Consequences
...
* **Hard decoupling.** Boundary interfaces stay stable: the [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] Block defines the signature and laws, while mechanisms and realizations can evolve behind it (with monotone strengthening and explicit guard boundaries).

**Didactic cohesion.** Readers see the same four conceptual rows across the spec, satisfying [[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]]’s comparability goal.

## [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:9 - Rationale

**Why “SubjectBlock”?** [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] showed that making the **ranged-over type explicit** (here: *BaseType*) avoids category mistakes when moving between domains (e.g., *set‑algebra on context slices* vs *equivalence‑classes of normalisations*). [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] lifts this to the kernel so every signature can declare **what it is about** before saying **what it provides**.
**Why one universal Block?** Experience with extension and mechanism signatures shows the value of a single canonical shape for Vocabulary, Laws, Applicability, and Alignment; [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] factors that universal core so other families can add headers and views without fragmenting the Kernel.
...
## [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:10 - Relations

* **Specialises and is specialised by:** **[[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]** (adds `OperationAlgebra`, `LawSet`, `AdmissibilityConditions`, and `Transport` for mechanisms) and any domain‑profiled signature publications that preserve the four‑row Block.
* **Constrained by:** [[00_E.10 - Unified Lexical Rules for FPF|E.10]] LEX-BUNDLE (registers, strata); D.CTX for Context binding; **Part F** (Bridges and cross-context transport; naming).
...
## [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:10a - P2W `U.Signature(profile=FormalSubstrate)` Use Relation

When [[01_E.18.01 - Principles-to-Work Carry-Through|E.18.1]] uses a first-principles or mathematical cue to select, declare, or cite a `U.Signature(profile=FormalSubstrate)` declaration, this pattern governs only that declaration: SubjectBlock, Vocabulary, Laws, Applicability, effect discipline, inference kinds, imported-symbol dependencies, and the no-realization pin. [[01_E.18.01 - Principles-to-Work Carry-Through|E.18.1]] may carry the cue and select the next admissible relation. [[00_C.29 - Mathematical Lens Use|C.29]] governs whether a mathematical-lens use is admissible for the stated use.

### [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:10a.1 - `profile=FormalSubstrate` signature, mathematical object, and lens-use slot discipline
...
|---|---|---|---|
| `U.Signature(profile=FormalSubstrate)` declaration | [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]] | `U.Signature(profile=FormalSubstrate)` with SubjectBlock, Vocabulary, Laws, Applicability, effect discipline, inference kinds, imports and provides, and no-realization pin. | The declaration is not a mechanism, empirical identity claim, evidence proof, work authorization, gate passage, or mathematical-lens use result. |
| Mathematical-lens use | [[00_C.29 - Mathematical Lens Use]] | Candidate mathematical object or formalism, mapping mode, preserved structure, lost structure, visible payoff, admissible use, non-admissible use, and stop condition. | Lens-use adequacy does not declare the signature profile and does not settle handler semantics, mechanism realization, empirical truth, evidence, work, gate, or decision authority. |
| Mechanism consumption or realization | [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind]] and downstream mechanism patterns | A mechanism cites the signature by import or reference, publishes operation algebra, law set, admissibility conditions, transport, and any monotone realization relation when that relation is being made. | A mechanism does not rewrite the imported signature laws by use, and a realization does not become a new `U.Signature(profile=FormalSubstrate)` declaration unless a new signature is declared. |
| P2W carry-through cue | [[01_E.18.01 - Principles-to-Work Carry-Through]] | Source cue, carried distinction, live next relation, selected application, stop condition, and any return trigger. | P2W does not mint `U.SubstrateFormalization`, does not decide mathematical-lens admissibility, and does not replace [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType]] or [[00_C.29 - Mathematical Lens Use]]. |

Old or source-local wording such as `SubstrateFormalization` recovers as a move to author, select, or cite a `U.Signature(profile=FormalSubstrate)` unless the claim being made is actually a [[00_C.29 - Mathematical Lens Use|C.29]] mathematical-lens use, an [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] mechanism relation, or another neighboring relation. In slot terms, the mathematical object can fill a `CandidateMathObject` position in [[00_C.29 - Mathematical Lens Use|C.29]], a vocabulary or law position in a `U.Signature(profile=FormalSubstrate)` declaration, or an imported-signature position in a mechanism. Those are relation positions, not separate object kinds and not `U.Role`s.

The Rodin-style lesson used here is constructive rather than slogan-like: formal languages, axioms, rules, and mathematical objects help model a world-facing or episteme-facing EntityOfConcern only when their representational and operational limits are declared. [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] therefore stores the formal-deductive declaration. [[00_C.29 - Mathematical Lens Use|C.29]] stores the declared use of a mathematical lens. [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]], bridge, measurement, evidence, work, gate, and decision patterns store the later relations that apply, test, authorize, or use that declaration.

## [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:10b - P2W PrincipleFrame Input Order

When [[01_E.18.01 - Principles-to-Work Carry-Through|E.18.1]] carries ontology, UTS, kind-relation, identity, context, boundary, characteristic, measurement, scale, comparator, or result-measurement wording toward a `PrincipleFrame`, write the input order explicitly. `PrincipleFrame` publishes postulates plus CHR observability in a bounded context; ontology editions, UTS rows, CHR editions, UNM, comparator, transport, normalization, bridge, and measurement relations stay with their own governing patterns.

## [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:10c - PrincipleFrame And CHR Observability Relation
...
## [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:10d - PrincipleFrame Name And Profile Boundary For P2W
...
## [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:10e - P2W Boundary Summary For `U.Signature(profile=FormalSubstrate)` And PrincipleFrame
...
## [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:11 - Lowering, repair, and refresh conditions
...
* a renamed SubjectKind, BaseType, SlotKind, RefKind, or exported SymbolId no longer recovers the same FPF kind under [[00_E.10 - Unified Lexical Rules for FPF|E.10]] and [[00_F.18 - Local-First Unification Naming Protocol|F.18]].
...
## [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]:End
