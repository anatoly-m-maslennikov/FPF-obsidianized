...
**Placement.** Part A, cluster A.IV “Signature Stack & Boundary Discipline”; directly under [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] `U.Signature` and alongside [[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]–[[08_A.06.04 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]].
**Depends on.**
– [[00_A.01 - U.Holon, U.System, and U.Episteme|A.1]] `U.Holon` (holonic carrier model).
– [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] `U.Signature` (universal morphism/relationship signatures).
– [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] (Strict Distinction; EntityOfConcern / Description / specification-use vs Surface).
– [[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]] (pattern authoring order & SoTA discipline).
– [[00_E.10 - Unified Lexical Rules for FPF|E.10]] (LEX‑BUNDLE: Tech/Plain registers, naming guards).

**Coordinates with.**
– [[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]] `U.EpistemeSlotRelation` (episteme slots: EntityOfConcern, GroundingHolon, ClaimGraph, Viewpoint, View, ReferenceScheme).
– [[00_C.03 - Kinds, Intent-Extent, and Typed Reasoning (Kind-CAL)|C.3]].* Kind‑CAL (Kinds, KindSignature, KindBridge).
– [[00_F.18 - Local-First Unification Naming Protocol|F.18]] (name governance; twin‑register discipline).

## [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:1 - Problem frame

FPF relies heavily on **n‑ary relations and morphisms**:

* episteme component layouts (`U.EpistemeKind` in [[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]]),
...
This produces subtle bugs (elaborated in [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:2):

* misuse of “Subject/Object” as SlotKind‑like names for very different ValueKinds (explicitly banned for episteme Tech names by [[00_E.10 - Unified Lexical Rules for FPF|E.10]]),
...
## [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:2 - Problem (symptoms in FPF)
...
5. **Substitution rules not localisable.**
   ...
   * “this Bridge only changes RefKind, not ValueKind”.
   ...
   This blocks clean use of [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] `U.Signature` as a shared calculus for method/role/episteme signatures.

6. **Episteme‑specific slots not standardised.**
   For epistemes, the positions “what is this about?”, “in which holon is it grounded?”, “what ClaimGraph is inside?” re‑appear across patterns. Without a shared slot discipline, each pattern names these ad‑hoc, breaking the ability to state **universal laws** over episteme morphisms ([[06_A.06.02 - U.EffectFreeEpistemicMorphing - Effect-free morphisms of epistemes|A.6.2]]–[[08_A.06.04 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]]).
...
The result: **local convenience, global incoherence** — exactly what [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] and [[00_E.10 - Unified Lexical Rules for FPF|E.10]] are supposed to prevent.

## [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:3 - Forces
...
* **F2 - Cross‑disciplinary reuse.**
  ...
  * episteme structures ([[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]]),
  * systems/roles/methods (A/B),
  * services and APIs (including method/service interfaces and ports),
  * cells in tables and databases,
  * guards, bridges, and flows in [[00_E.18 - Transformation Flow Structure|E.18]],
  * and publication operations ([[03_E.17 - Multi-View Publication Kit|E.17]]).
...
* **F4 - EntityOfConcern / Description / specification-use discipline.**
  Strict distinction ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]], [[04_E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]]) already separates **entities of concern**, their **Description epistemes**, and **specification uses**. The same discipline is needed inside relations:
...
* **F5 - Didactic primacy and naming discipline.**
  [[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]] and [[00_E.10 - Unified Lexical Rules for FPF|E.10]] demand patterns that are:
...
## [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:4 - Solution — SlotKind / ValueKind / RefKind triple (plus a slot‑operation lexicon)

### [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:4.1 - Three layers for every argument position
...
In **normative** text, the canonical word is **slot**, and the canonical carrier is a **SlotSpec** triple ([[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]]).
...
2. **ValueKind (kind of slot filler).**
   *Which kinds of things may fill this position in principle (at the intensional level).*
   ...
   – ValueKind is a **Kind** ([[00_C.03 - Kinds, Intent-Extent, and Typed Reasoning (Kind-CAL)|C.3]].\*) or another kernel‑level type; it is **not** a slot and never carries `*Slot`/`*Ref` suffixes.
...
3. **RefKind (how we store / refer).**
   *What reference/identifier we actually store in episteme when we fill this slot.*
   ...
   – RefKind is **about references, not values**; it usually points to an editioned referent ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]], [[00_F.15 - SCR-RSCR Harness for Unification|F.15]]) and carries the `.edition` field when pinning a phase.
...
### [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:4.2 - Naming discipline: `*Slot` and `*Ref`

This pattern introduces the following **lexical constraints**, aligned with [[00_E.10 - Unified Lexical Rules for FPF|E.10]]:
...
3. **ValueKind names carry no `*Slot`/`*Ref`.**
   ...
   * ValueKinds are named using standard FPF conventions (A/E/F, [[00_E.10 - Unified Lexical Rules for FPF|E.10]]), without `*Slot`/`*Ref`.
...
These rules become part of the **LEX‑BUNDLE guards** and are enforced by [[00_F.18 - Local-First Unification Naming Protocol|F.18]] / name‑acceptance harnesses.

### [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:4.3 - Integration with `U.Signature` ([[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]])
...
### [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:4.4 - Typed substitution discipline
...
2. **ValueKind compatibility.**
   The new content **MUST** be of the same ValueKind (or a declared subkind) as `SlotSpec_i.value`; Kind‑CAL governs this (`⊑` in [[01_C.03.01 - U.Kind & SubkindOf (Core)|C.3.1]]–[[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]). If a Context uses EntityOfConcernClass species constraints ([[02_C.03.02 - KindSignature (+F) & Extension-MemberOf|C.3.2]]), those act as additional guards but do **not** change the SlotKind.

3. **RefKind correctness.**
   If `refMode=RefKind`, the stored field is of that RefKind; substitutions operate on references, not on underlying values. Edition pinning is handled as usual by `.edition` fields in F‑patterns ([[00_F.15 - SCR-RSCR Harness for Unification|F.15]], etc.).

4. **By‑value vs by‑ref awareness.**
   Substitutions at by‑value slots (e.g. `ClaimGraphSlot`) are **content edits** to the episteme or relation instance; they may affect formality F or assurance lanes. Substitutions at ref slots are **retargetings** of entityOfConcern or context, and their legality is governed by [[06_A.06.02 - U.EffectFreeEpistemicMorphing - Effect-free morphisms of epistemes|A.6.2]]–[[08_A.06.04 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] and Bridge/CL rules. Tooling SHOULD surface this difference explicitly in authoring surfaces (e.g. separate “Ref” vs “embedded content” columns).
...
### [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:4.5 - Slot operation lexicon (binding / filling / assignment / retargeting / mutation)
...
#### [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:4.5.1 - Four‑way separation: Identifier vs Slot vs Slot‑content vs Referent
...
This separation is the anchor for all “binding time” talk in [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:4.6.

#### [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:4.5.2 - Canonical verbs (Tech register) for slot operations *(normative)*
...
8. **mutate / modify** (Referent internal change; content unchanged).
   ...
   **FPF note:** In edition-disciplined contexts, prefer to describe intentional change as **revise**, issue a **re-edition**, and **retarget**, rather than “mutate”, because the Core treats editioned referents as stable per edition ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]], [[00_F.15 - SCR-RSCR Harness for Unification|F.15]]).
...
#### [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:4.5.3 - Canonical nouns *(normative)*
...
#### [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:4.5.4 - Operator naming guidance for slot‑writing operators *(normative, but intentionally lightweight)*
...
Recommended patterns (examples only; Contexts may adopt their own naming style via [[00_F.18 - Local-First Unification Naming Protocol|F.18]]):
...
This preserves the [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] stratification and prevents importing accidental semantics from a specific programming language.

## [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:5 - Archetypal Grounding (Tell‑Show‑Show)

Following [[00_E.07 - Archetypal Grounding Principle|E.7]], we ground the pattern in a **System** example and an **Episteme** example.

### [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:5.1 - System example — authentication pipeline signature
...
### [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:5.2 - Episteme example — model evaluation result
...
The SlotKind/ValueKind/RefKind discipline makes these moves **local and explicit**: the pattern describes which moves are allowed where, and [[06_A.06.02 - U.EffectFreeEpistemicMorphing - Effect-free morphisms of epistemes|A.6.2]]–[[08_A.06.04 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] then constrain how episteme morphisms may change ClaimGraphs and references.

### [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:5.3 - Didactic micro‑examples — substitution by SlotKind / ValueKind / RefKind *(informative)*

The following short examples are intended for a didactic guide or for cross‑references from [[00_A.06 - Signature Stack & Boundary Discipline|A.6]].0/A.6.x/C.2.1. In all of them:
...
#### [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:5.3.1 - Mechanics — stool on a test rig
...
#### [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:5.3.2 - Microservices — switching the authentication provider
...
#### [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:5.3.3 - Data/ML — swapping dataset or target characteristic
...
#### [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:5.3.4 - Operational practice — the same runbook in different operating centres
...
#### [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:5.3.5 - SLO/SLA requirements — changing the target characteristic vs changing the threshold
...
## [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:6 - Bias‑Annotation
...
* **Episteme‑first entityOfConcern.**
  The examples and cross‑references prioritise episteme use‑cases ([[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]], [[06_A.06.02 - U.EffectFreeEpistemicMorphing - Effect-free morphisms of epistemes|A.6.2]]–[[08_A.06.04 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]]) where entityOfConcern and retargeting are subtle. System‑only usages (e.g. method signatures) are absolutely allowed but not the driving case; they inherit the same discipline without additional obligations.

## [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:7 - Conformance Checklist (normative)

**CC‑[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]‑1 - SlotSpec for every parameter.**
...
**CC‑[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]‑2 - `*Slot` discipline.**
Any Tech name ending with `…Slot` **MUST** denote a SlotKind; SlotKinds **MUST NOT** be used as ValueKinds or RefKinds.

**CC‑[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]‑3 - `*Ref` discipline.**
...
**CC‑[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]‑4 - ValueKind purity.**
...
**CC‑[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]‑5 - Episteme core SlotKinds.**
For episteme kinds (`U.EpistemeKind`), the following SlotKinds **SHALL** be used (or their documented refinements) in [[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]] / C.2.x:
...
**CC‑[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]‑6 - No “Role” as SlotKind head.**
...
**CC‑[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]‑7 - Substitution checks.**
...
**CC‑[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]‑8 - Cross‑pattern consistency.**
...
**CC‑[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]‑9 - Migration of legacy `…Ref`/`…Slot` usage.**
...
**CC‑[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]‑10 - Pattern integration.**
New or revised patterns in Part A/B/C/E that introduce n‑ary relations, morphisms, or signatures **SHALL** reference [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] in their Relations section and attest that they follow SlotKind/ValueKind/RefKind discipline.

**CC‑[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]‑11 - Slot‑content terminology.**
...
**CC‑[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]‑12 - Slot‑operation verb discipline.**
Any normative description of a change “to a slot” **MUST** specify which operation class it is (initialize vs assign/set vs retarget vs by‑value edit vs resolve vs mutate/revise), using the canonical verbs in [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:4.5.2 or explicitly mapping local terms to them.

**CC‑[[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]‑13 - Binding‑time clarity.**
...
## [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:8 - Consequences
...
* **Stricter lexical rules.**
  Some legacy names will need migration (`EpistemicObject`, ad‑hoc `…Ref` types). Mitigation: migration notes in [[00_F.18 - Local-First Unification Naming Protocol|F.18]] and dedicated anti‑pattern sections; transitional aliases allowed but marked deprecated.

* **Learning curve.**
  Authors must learn to think “SlotKind/ValueKind/RefKind” *and* distinguish “retarget vs edit vs resolve” before writing `id` or `subject`. Mitigation: Tell‑Show‑Show examples and a didactic micro‑guide on slot operations referenced from [[00_A.06 - Signature Stack & Boundary Discipline|A.6]].0/C.2.1/E.17.0.

## [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:9 - Rationale
...
In practice, “Ref” types tend to be quietly used as if they were values, eroding the EntityOfConcern / Description / specification-use split and making edition discipline invisible. By insisting that ValueKind is always the conceptual kind (“what sort of thing is this about?”) and RefKind is always the reference/identifier kind (“how do we point at it in Episteme?”), the pattern aligns with [[04_E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]]’s intension/description/specification discipline and with modern resource‑aware logics that keep values and resources distinct.

**Why add a slot‑operation lexicon.**
The triple only buys safety if authors and tools can see it at a glance **and** can narrate changes without collapsing layers. [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:4.5 makes the common “put something in a slot” moves explicit: initialization vs assignment vs retargeting vs by‑value editing vs resolution. This directly reduces ambiguity in episteme morphism descriptions ([[06_A.06.02 - U.EffectFreeEpistemicMorphing - Effect-free morphisms of epistemes|A.6.2]]–[[08_A.06.04 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]]) and prevents accidental imports from a specific PL’s terminology.

**Why standardise episteme SlotKinds.**
entityOfConcern and grounding recur across epistemes; standard SlotKinds (`EntityOfConcernSlot`, `GroundingHolonSlot`, `ClaimGraphSlot`, etc.) let [[06_A.06.02 - U.EffectFreeEpistemicMorphing - Effect-free morphisms of epistemes|A.6.2]]–[[08_A.06.04 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] and [[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]] talk about substitutions and retargetings once, instead of re‑defining “what this is about” in every pattern.
...
## [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:10 - SoTA‑Echoing (post‑2015 practice alignment)

**Purpose.** To situate SlotKind/ValueKind/RefKind discipline with respect to contemporary typed and relational approaches, without importing any external calculus into the Core. All items are used as conceptual comparators; concrete reuse in a `U.BoundedContext` would happen only via explicit Bridges ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]) with declared CL penalties.
...
3. **Relational models of types‑and‑effects (adapt).**
   ...
   **Adapted:** [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] stratifies *slot / value / reference* instead of *value / effect*, and applies the discipline not only to programs but also to epistemes, roles, methods, and services. ([15])
...
4. **Optics / lenses as disciplined projections (echo).**
   ...
   **Echoed:** SlotKind plays the role of the focus coordinate; ValueKind is the focus type; RefKind determines whether the focus is stored by value or via a handle. This perspective informs later use of SlotKind discipline in EpistemicViewing ([[00_A.06.03 - U.EpistemicViewing - EntityOfConcern-preserving morphism|A.6.3]]) and multi‑view publication ([[03_E.17 - Multi-View Publication Kit|E.17]]). ([16])

**Cross‑Context reuse and Bridges.** When a `U.BoundedContext` chooses to adopt a concrete row‑typing discipline, relational logic, or optics library, it **SHALL** do so via explicit Bridges ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]) with CL and (for plane crossings) `Φ(CL)`/`Φ_plane` policy‑ids, keeping numerical policies and notations Context‑local. [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] only constrains the **slot discipline** that such Bridges must respect.

## [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:11 - Relations (with other patterns)

**Specialises [[09_A.06.P - Relational Precision Restoration (RPR) - Kind-Explicit Qualified Relation Discipline|A.6.P]] Relational Precision Restoration (RPR).**
...
**Builds on [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] `U.Signature`.**
...
**Constrains [[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]] `U.EpistemeSlotRelation`.**
Fixes core episteme SlotKinds (`EntityOfConcernSlot`, `GroundingHolonSlot`, `ClaimGraphSlot`, `ViewpointSlot`, `ViewSlot`, `ReferenceSchemeSlot`) and their ValueKinds/`ByValue` vs Ref discipline. [[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]] and its extensions SHALL use these SlotKinds (or documented refinements) so that episteme morphisms can be expressed uniformly over slots.

**Supports [[06_A.06.02 - U.EffectFreeEpistemicMorphing - Effect-free morphisms of epistemes|A.6.2]]–[[08_A.06.04 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] (episteme morphisms and viewing).**
EntityOfConcern‑preserving vs entityOfConcern‑retargeting morphisms can now be stated as constraints on which SlotKinds’ ValueKinds/RefKinds they may change. Retargeting becomes “retargeting at `EntityOfConcernSlot` under a Kind‑Bridge” rather than an ad‑hoc parameter tweak. The operation lexicon in [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:4.5 makes “retarget vs edit vs resolve” explicit in these morphism descriptions.

**Coordinates with [[00_B.05 - Canonical Reasoning Cycle|B.5]].* (RoleEnactment).**
Role/assignment relations may declare SlotKinds such as `HolderHolonSlot`, `RoleSlot`, `ContextSlot`, `WindowSlot` with clear ValueKinds/RefKinds, instead of overloading “role” for both holonic roles and relation positions. This keeps `U.Role` semantics ([[00_A.02 - Role Taxonomy|A.2]], [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]) separate from slot discipline.

**Coordinates with [[03_E.17 - Multi-View Publication Kit|E.17]] `U.MultiViewDescribing`.**
...
**Feeds [[00_F.18 - Local-First Unification Naming Protocol|F.18]] (LEX‑BUNDLE) and [[00_E.10 - Unified Lexical Rules for FPF|E.10]] (LEX).**
Provides lexical guards for `*Slot` and `*Ref`, and (via [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:4.5) for operation verbs:
...
**Used by [[00_A.19 - CharacteristicSpace & Dynamics Hook (A.CHR-SPACE)|A.19]] `CharacteristicSpace` and measurement patterns.**
Characteristic‑space slots already behave as positions with attached kinds; slot discipline in [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] gives a uniform story for how such slots appear inside relation signatures, episteme cards, and service definitions, and how substitution over those slots is checked.
...
## [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]:End
