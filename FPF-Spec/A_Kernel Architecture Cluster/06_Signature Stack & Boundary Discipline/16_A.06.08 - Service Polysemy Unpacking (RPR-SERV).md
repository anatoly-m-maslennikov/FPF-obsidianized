...
> **Status:** Stable
> **Normativity:** Normative
> **Placement:** Part A → [[00_A.06 - Signature Stack & Boundary Discipline|A.6]] (Precision restoration / stack discipline)
> **Builds on:** [[09_A.06.P - Relational Precision Restoration (RPR) - Kind-Explicit Qualified Relation Discipline|A.6.P]] (RPR recipe), [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] (slot discipline), [[02_A.06.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work-Effects)|A.6.B]] (L/A/D/E claim classification), [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]] (`U.PromiseContent`), [[08_A.02.08 - U.Commitment (Deontic Commitment Object)|A.2.8]] (`U.Commitment`), [[09_A.02.09 - U.SpeechAct (Communicative Work Object)|A.2.9]] (`U.SpeechAct`), [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] (`U.Work`), [[00_E.10 - Unified Lexical Rules for FPF|E.10]] (LEX, incl. L‑SERV, LEX‑BUNDLE & PTG stances), [[00_F.17 - Unified Term Sheet (UTS)|F.17]] (UTS — Unified Term Sheet), [[00_F.18 - Local-First Unification Naming Protocol|F.18]] (Name Cards / NQD‑front; promise ≠ utterance ≠ commitment).
> **Coordinates with:** [[03_A.06.C - Contract Unpacking for Boundaries|A.6.C]] (agreement/specification/contract shorthand unpacking into promise content, commitment, work/evidence, and interface/boundary claims), [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] (EntityOfConcern / Description episteme / carrier), G.* evidence discipline (EvidenceGraph / SCR), Context/Bridge policy for cross‑Context reuse, [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] (Mint/Reuse), [[00_E.15 - Lexical Authoring & Evolution Protocol (LEX-AUTH)|E.15]] (LEX‑AUTH when refactoring existing prose at scale).
...
> **Impact radius:** Any normative prose that uses the “service” cluster (`service`, `service provider`, `server`); LEX rules (L‑SERV / LEX‑BUNDLE); UTS blocks ([[00_F.17 - Unified Term Sheet (UTS)|F.17]]); boundary/agreement/specification patterns that already talk about services (esp. [[03_A.06.C - Contract Unpacking for Boundaries|A.6.C]]); any automated repair/lint pipeline used for bulk refactors ([[00_E.15 - Lexical Authoring & Evolution Protocol (LEX-AUTH)|E.15]] / LEX‑AUTH).
 **Mint vs reuse:** Mints the `serviceSituation(…)` QRR lens id and the facet headphrase set defined in §4.3. Reuses `U.PromiseContent`, `U.Commitment`, `U.SpeechAct`, `U.System`, `U.Work`, `U.MethodDescription`, and the [[00_A.06 - Signature Stack & Boundary Discipline|A.6]].P/QRR recipe.
...
**Non‑goal (modularity guard).** This pattern does **not** redefine the semantics or field structure of the promise‑content object (the **promise content**). That kernel meaning is defined in **[[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]] (`U.PromiseContent`)**. [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]] is a precision‑restoration + lexicon discipline that (i) forces facet‑typed head phrases and (ii) provides an optional QRR lens to bind already‑defined kinds without collapsing them. Agreement/specification/contract shorthand unpacking is handled by **[[03_A.06.C - Contract Unpacking for Boundaries|A.6.C]]**, which applies this pattern when that shorthand contains the service cluster.

## [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:1 - Problem frame
...
## [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:2 - Problem
...
Result: readers can’t apply [[02_A.06.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work-Effects)|A.6.B]] routing, and engineers are incentivized to preserve ambiguity (“service” as a convenient metonym) because it avoids committing to a model.

## [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:3 - Forces
...
## [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:4 - Solution

### [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:4.0 — UTS + LEX preparation (mandatory for authoring/repair)

“Service” is a **polysemy cluster**, not a single token. Therefore, before applying the rewrite rules below to normative prose, the author/editor SHALL create or update a **thread‑local UTS block** ([[00_F.17 - Unified Term Sheet (UTS)|F.17]]) and its paired **LEX‑BUNDLE entries** ([[00_E.10 - Unified Lexical Rules for FPF|E.10]]) for the **service cluster** (Tech/Plain twins and PTG stance).
...
**Context selection (universality guard).** The UTS block MUST cite **ContextName@Edition** in each SenseCell ([[00_F.17 - Unified Term Sheet (UTS)|F.17]]), and the cited contexts SHOULD span at least **three** distinct “service traditions” reflected in this pattern’s SoTA‑Echoing set (e.g., ITSM/service management, EA/modelling, speech‑act/coordination, microservices/SRE practice). This prevents a “FPF‑only” meaning loop and keeps facet names portable.
...
* A conforming **normative Tech** text SHALL treat the bare word **service** (unqualified) as **PTG=Guarded** ([[00_E.10 - Unified Lexical Rules for FPF|E.10]]): it is valid only under this pattern’s rewrite rules and only as part of a qualified head phrase.
* If a new facet head phrase must be introduced, it SHALL be treated as a **LexicalAct** with an explicit **Mint/Reuse** decision ([[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]), and its **CandidateSet + rationale** SHOULD be recorded via a Name Card ([[00_F.18 - Local-First Unification Naming Protocol|F.18]] / NQD‑front) to avoid “clever” but unstable vocabulary.
...
### [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:4.1 — Trigger rule
...
### [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:4.2 — Stable lens: the Service Situation Bundle
...
* `promiseContentRef : PromiseContentRef`
  *Promise content* — the `U.PromiseContent` referent ([[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]). **Plain head:** *promise content* / *service offering clause* / *service promise clause*.
* `promisedOutcomeSpecRef? : OutcomeSpecRef`
  The **promised outcome template** described by the clause (`U.OutcomeSpec`, [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10). It may constrain:
...
* `providerAssignmentRef? : RoleAssignmentRef`
  The concrete **role enactor assignment** that holds `providerRoleRef` in the relevant Context/window ([[00_E.10 - Unified Lexical Rules for FPF|E.10]] / [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]). This is what everyday talk calls “the service provider” (team/shop/vendor/system).
...
* `accessSpecRef? : MethodDescriptionRef`
  The **service access spec**, a request-facing interface description (API signature, OpenAPI, endpoint interface description, intake SOP, desk procedure). This is typically `promiseContentRef.accessSpec` ([[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]) and is a `U.MethodDescription`.
...
* `deliveryWorkRef? : WorkRef`
  ...
  **Invariant: SERV‑INV‑3 (Outcome anchoring).**
  ...
  (i) explicitly assert `deliversPromisedOutcome(deliveryWorkRef, promisedOutcomeSpecRef)` ([[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:8.1), or
  ...
  **Invariant: SERV‑INV‑4 (Unit-of-delivery measurability).**
  If `promiseContentRef.unitOfDelivery` is present, then its `countingRule` is stated (per [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]:5.10.3, with [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] defaults available) and the cited Work carries the measurements required by that rule (duration, quantity, cases, kWh, etc).
...
Qualifier slots (as needed per [[00_A.06 - Signature Stack & Boundary Discipline|A.6]].P/A.6.B):

* `scope? : ClaimScope`
* `Γ_time?` (explicit Γ_time selector per [[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]; time windows are explicit when the surrounding passage is time‑sensitive)
* `viewpoint? : ViewpointRef`
* `referenceScheme? / representationScheme?` (only when needed)

**Guidance (didactic).** In normative prose, prefer facet‑explicit predicates: if a predicate targets a specific facet (addressability, deontic force, actuals, mechanism), apply it to the corresponding slot rather than to an untyped “service” noun phrase. (Enforced by CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑3/4/6/9.)
...
* The **promisor / commitment‑holder** is the **provider principal** (or its role assignment) unless the Context explicitly models a system as an agent with standing. *(See CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑8.)*
...
When a `serviceSituation(…)` binds a principal/role assignment to systems (access point / delivery system), the relation kinds are explicit (prefer [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]] base relations when available). **Implicit “system implies provider” readings are invalid.**
* Mechanism/process claims target `deliverySystemRef` and/or `deliveryMethodRef` (and sometimes `accessSpecRef` if the claim is strictly about interface signature), not `promiseContentRef`. *(See CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑9.)*
...
### [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:4.3 — Facet headwords (mandatory lexical rule)
...
(Normative enforcement is via CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑1 and CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑8.)

### [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:4.4 — Addressability rule (the “can you call it?” test)
...
### [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:4.4b — Method/mechanism rule (the “how does it work?” test)
...
A conforming text **SHALL NOT** attach mechanism/process predicates to the **promise content**; the clause may constrain outcomes or acceptance criteria, but mechanism claims belong to design descriptions or method descriptions. *(See CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑9.)*

### [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:4.5 — Deontic rule (the “must/shall” test)
...
**No hidden agency rule (normative):** A conforming text **SHALL NOT** use an access object (e.g., endpoint/access point) as the grammatical subject of an RFC‑keyword sentence. It **SHALL** use the accountable principal (or role assignment) as subject and then state the operational condition on the access point as a predicate/evidence claim. *(See CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑4 and CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑8.)*

### [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:4.6 — Speech‑act rule (the performative verb test)
...
### [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:4.7 — Runtime/telemetry rule (the “actuals” test)
...
### [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:4.8 — Change‑class lexicon (service‑specific narrations)

When the draft describes “service changes”, narrate changes using stable change classes ([[09_A.06.P - Relational Precision Restoration (RPR) - Kind-Explicit Qualified Relation Discipline|A.6.P]]), specialized to the serviceSituation lens:
...
### [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:4.9 — Disambiguation guide (rewrite/selection)
...
## [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:5 - Archetypal grounding
...
## [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:6 - Bias-Annotation
...
## [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:7 - Conformance Checklist (CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]])

0. **CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑0 — UTS/LEX block exists for the service cluster.**
   Any document that applies this pattern (or that introduces normative “service” language) SHALL publish:
   (a) a local **UTS block** ([[00_F.17 - Unified Term Sheet (UTS)|F.17]]), and
   (b) paired **LEX‑BUNDLE entries** ([[00_E.10 - Unified Lexical Rules for FPF|E.10]]) for the Tech/Plain twins and PTG stances used here.
   ...
   Any newly introduced facet head phrase SHALL have an explicit Mint/Reuse decision ([[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]) and SHOULD have a Name Card rationale ([[00_F.18 - Local-First Unification Naming Protocol|F.18]]).

1. **CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑1 — Unqualified “service” (and cluster stand-ins) is unsupported in normative prose.**
...
2. **CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑2 — `U.PromiseContent` is referred to as a “promise content” in prose.**
...
3. **CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑3 — Addressability implies `accessPointRef` (system), not clause.**
...
4. **CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑4 — Deontic language requires a commitment.**
...
5. **CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑5 — Performative verbs require a speech act.**
...
6. **CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑6 — Actuals require work + evidence.**
...
7. **CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑7 — Bundle lens is used when multiple facets are in play.**
...
8. **CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑8 — Commitments and promises have an accountable principal.**
...
9. **CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑9 — “How it works” claims belong under method or system, not under the clause.**
...
## [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:8 - Common Anti-Patterns and How to Avoid Them
...
## [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:9 - Consequences

* **Pros:**

  * Removes the incentive to keep “service” conveniently vague.
  * Enables [[02_A.06.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work-Effects)|A.6.B]] routing: clause (L), commitment (D), acts/work/evidence (E), mechanisms/interfaces (A/L depending on placement).
...
1) Unqualified head‑noun “service” occurrences in normative prose are **0** (CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑1).
2) Every deontic (“must/shall/guarantee/SLA”) sentence about service delivery references a **service commitment** / `U.Commitment` (CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑4).
3) Every runtime/telemetry “service is down/slow/…” claim is routed to **work + evidence** and, when relevant, distinguishes access‑point symptoms from delivery‑system symptoms (CC‑[[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]‑6 + §4.7).

## [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:10 - Rationale
...
This aligns with [[09_A.06.P - Relational Precision Restoration (RPR) - Kind-Explicit Qualified Relation Discipline|A.6.P]]’s requirement to replace umbrella tokens with explicit kind+slots forms and to provide rewrite guides and guardrails.

## [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:11 - SoTA-Echoing
...
## [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:12 - Relations

* **Specialises:** [[09_A.06.P - Relational Precision Restoration (RPR) - Kind-Explicit Qualified Relation Discipline|A.6.P]] (RPR) for the lexical/semantic ambiguity cluster around “service”.
...
* **Requires (authoring discipline):** a local UTS block ([[00_F.17 - Unified Term Sheet (UTS)|F.17]]) and published Tech/Plain twins ([[00_E.10 - Unified Lexical Rules for FPF|E.10]]) for the service/provider/server cluster; this is the “anti‑FPF‑only loop” guard.
* **Coordinates with:** [[03_A.06.C - Contract Unpacking for Boundaries|A.6.C]] (agreement/specification/contract shorthand unpacking). When that shorthand includes *service* tokens, apply RPR‑SERV first to select **promise content** vs **commitment** vs **access point/system** vs **work/evidence**, then unpack the resulting atomic statements with [[03_A.06.C - Contract Unpacking for Boundaries|A.6.C]] and classify them with [[02_A.06.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work-Effects)|A.6.B]] (L/A/D/E).

## [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:12a - Service/cell analogy correction and quantum-like route
...
### [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:12a.1 - Service/cell analogy correction
...
### [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:12a.2 - QL route after service-facet unpacking

Use QL wording only after the service facets have been unpacked and an ordinary [[00_A.06 - Signature Stack & Boundary Discipline|A.6]], [[02_A.06.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work-Effects)|A.6.B]], [[00_F.09 - Alignment & Bridge across Contexts|F.9]], [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]], [[00_C.16 - Measurement & Metrics Characterization (MM-CHR)|C.16]], or [[00_C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]] governing pattern still leaves a residual probe, export, coarsening, or viability-envelope support load.

QL route action path:

1. If an API read/export is later used as passive state reading while it changes or thins the state, route that remainder through [[01_C.26.01 - Probe-Coupled Boundary Interaction|C.26.1]].
2. If a service situation is being kept viable by boundary/exchange/adaptation work, route that remainder through [[03_C.26.03 - Viability-Envelope Boundary Regulation|C.26.3]].
3. If a coarsened representation of the service state is used, route admissible use and non-admissible downstream use through CSC/RT and the [[00_C.26 - Quantum-Like Modeling Lens|C.26]] coarsening support section only when a QL cue remains.
...
- a [[01_C.26.01 - Probe-Coupled Boundary Interaction|C.26.1]] note only for passive-read or export mistakes caused by the service interaction;
- a [[03_C.26.03 - Viability-Envelope Boundary Regulation|C.26.3]] note only when service viability is the actual envelope-regulation problem.

## [[16_A.06.08 - Service Polysemy Unpacking (RPR-SERV)|A.6.8]]:End
