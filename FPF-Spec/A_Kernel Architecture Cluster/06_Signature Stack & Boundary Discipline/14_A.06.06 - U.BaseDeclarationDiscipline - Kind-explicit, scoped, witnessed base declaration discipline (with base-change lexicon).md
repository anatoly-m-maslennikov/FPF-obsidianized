...
**Placement.** Part A, cluster A.IV “Signature Stack & Boundary Discipline”; adjacent to [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] `U.RelationSlotDiscipline`.

**Depends on.**
– [[04_A.06.00 - U.Signature - Universal, law-governed declaration for a SubjectKind on a BaseType|A.6.0]] `U.Signature` (universal signature carrier).
– [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] `U.RelationSlotDiscipline` (SlotKind/ValueKind/RefKind stratification + slot-operation lexicon).
– [[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] (Scope discipline; explicit `Γ_time`; implicit “latest/current” is forbidden).
– [[04_A.02.04 - U.EvidenceRole|A.2.4]] `U.EvidenceRole` (timespan + evidence-role discipline for decision-relevant witness sets).
- [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] (Strict Distinction; EntityOfConcern vs Description-episteme and specification-use cases vs publication face, form, unit, carrier, and rendering lanes).
– [[00_E.08 - FPF Authoring Conventions & Style Guide|E.8]] (pattern authoring order & SoTA discipline).
– [[00_E.10 - Unified Lexical Rules for FPF|E.10]] (LEX‑BUNDLE discipline; D.CTX lexical guardrails).

**Coordinates with.**
– [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] Evidence–Provenance DAG discipline (`verifiedBy`, `validatedBy`).
– [[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] per-edge constructive grounding (`tv:groundedBy`) and `validationMode` discipline.
– [[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]] `U.EpistemeSlotRelation` grounding slots (`GroundingHolonSlot`, `EntityOfConcernSlot`).
– [[00_A.06.03 - U.EpistemicViewing - EntityOfConcern-preserving morphism|A.6.3]] `U.EpistemicViewing` (`EntityOfConcernRef`-preserving view operators; base-relative “how” without retargeting).
– [[08_A.06.04 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] `U.EpistemicRetargeting` (base-change along `KindBridge`; retargeting lexicon and continuity rules).
– [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]] `U.KindBridge` & `CL^k` (explicit repair/translation when endpoint kinds or Contexts differ; no silent re-typing).
– [[00_E.18 - Transformation Flow Structure|E.18]] assurance-operations on `U.Transfer` (`CalibrateTo`, `CiteEvidence`, `AttributeTo`, `ConstrainTo`, …).
– [[00_F.09 - Alignment & Bridge across Contexts|F.9]] Bridges & CL (cross-context and cross-plane base declarations cite Bridge ids + CL policy).
– [[00_F.15 - SCR-RSCR Harness for Unification|F.15]] F‑Suite validation harness (SCR/RSCR pins and refresh governance).
- [[00_F.18 - Local-First Unification Naming Protocol|F.18]] naming governance (Tech/Plain twins and publication-lane naming boundaries).

**Aliases (informative; discouraged for normative prose).**
– “anchoring / anchor” (source umbrella colloquial; a red-flag token for *under-described dependence*). **Prohibited in Tech register** as a meaning-surrogate; treat it as a defect to be rewritten into an explicit `baseRelation(dependent, base)` form. Allowed only when referring to a **pattern-defined primitive** that already reserves the word (e.g., [[00_E.10 - Unified Lexical Rules for FPF|E.10]] MG‑DA *Domain Anchoring*; evidence/pin “anchors” where the term is explicitly reserved), or inside quoted source text that is immediately followed by a conformant rewrite.
...
It **reuses** [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] SlotSpec discipline (SlotKind/ValueKind/RefMode), [[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] Scope discipline (`U.Scope`, explicit `Γ_time` when time matters), and [[04_A.02.04 - U.EvidenceRole|A.2.4]] witness semantics (`U.EvidenceRole`) as the enforcement substrate.

## [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:1 - Problem frame
...
> **Deconfliction note (lexical).** This pattern is about *base-dependence in content* (“X is usable relative to B”). It is not about [[00_E.10 - Unified Lexical Rules for FPF|E.10]]’s **Domain Anchoring** (MG-DA), where “anchoring” is a *lexical* primitive for binding token morphology to the term's selected EntityOfConcern. When you see `anchor*` in a basedness sentence, treat it as a defect unless an explicit baseRelation token is present.
>
> **Deconfliction note (context/meaning).** This pattern is also not a license to reintroduce “anchor” as a surrogate for **Context**, **SenseCell**, or “where meaning lives”. Any such use is an *anchor‑relapse* and SHALL be rewritten into explicit Context/SenseCell/ConceptSet lane constructs ([[00_E.10 - Unified Lexical Rules for FPF|E.10]] D.CTX), not into SWBD.
>
...
## [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:2 - Problem
...
7. **Grounding conflation.**
   ...
   * situational/empirical grounding of an episteme via a grounding holon ([[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]]),
...
## [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4 - Solution — The `U.ScopedWitnessedBaseDeclaration` object and its lexicon

### [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4.1 - Canonical object

**Definition.** A **`U.ScopedWitnessedBaseDeclaration`** (SWBD) is a first-class base-declaration record *shape* (a signature in the [[00_A.06 - Signature Stack & Boundary Discipline|A.6]].0/A.6.5 sense): it reifies “dependent is usable relative to base via baseRelation, under scope/time, witnessed by pins”.
...
**Notation.** `SlotContent(VK, refMode)` is a compact shorthand for “a slot whose SlotSpec declares `ValueKind=VK` and `refMode ∈ {ByValue | RefKind}` ([[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]])”.

**SlotSpec note.** `VK_*` / `RK_*` / `refMode_*` above are **not** “anything goes”: they are fixed by the chosen `BaseRelationSlot` vocabulary entry and must be declared as SlotSpecs ([[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]). In other words, SWBD is a reusable *shape*, but each Context’s declared `baseRelation` vocabulary entry makes it a concrete, typed signature.

**Instance/prose notation note (convention).** In the prose and examples below, the slot fillers are written as `dependent`, `base`, `baseRelation`, `scope`, `Γ_time`, `witnesses` (no `*Slot` suffix). The `*Slot` suffix is reserved for SlotKinds/positions only ([[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] / [[00_E.10 - Unified Lexical Rules for FPF|E.10]]).
...
* **WF‑BD‑6 (No silent cross-context reuse).** An SWBD that relates dependent and base across Contexts/planes (or requires scope translation) is admissible only if it cites the Bridge ids + CL policy that make the reuse admissible ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]). No informal “it is the same entity anyway” prose is an admissible substitute.

This is the discipline-level analogue of [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]’s move: disambiguation is achieved by making the missing structural component explicit and non-optional in decision-relevant contexts.

### [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4.2 - Underlying mathematical lens
...
### [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4.3 - Slot discipline for SWBD

Any signature that introduces SWBD (or SWBD-like relations) SHALL define SlotSpecs per [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]: every position declares **SlotKind / ValueKind / RefKind-or-ByValue**.
...
* **Token discipline (LEX).** The token SHALL satisfy [[00_E.10 - Unified Lexical Rules for FPF|E.10]] token-class morphology for relations/verbs; it SHALL NOT use metaphor heads (`Anchor*`, `Ground*`, `Attach*`) as a meaning-surrogate. If a source synonym must be kept for continuity, record it through [[00_F.18 - Local-First Unification Naming Protocol|F.18]] alias discipline while keeping the relation-specific token specific.
...
* **Cross-context / cross-plane policy.** Whether this declared `baseRelation` may cross Contexts/planes at all; if yes, what Bridge ids/CL thresholds must be cited and what loss notes are required ([[00_F.09 - Alignment & Bridge across Contexts|F.9]] / [[03_C.03.03 - KindBridge & CL^k - Cross-context Mapping of Kinds|C.3.3]]).

This mirrors [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]: a SlotKind without ValueKind/RefMode is underspecified; a baseRelation without its vocabulary entry is equally underspecified.

### [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4.4.1 - Perspective/voice discipline (dependent-view vs base-view)
...
### [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4.5 - Lexical discipline
...
**Carve-outs (pattern-defined primitives).** This red-flag rule does **not** ban uses where “anchoring” is already a *pattern-defined primitive* elsewhere in the spec, such as [[00_E.10 - Unified Lexical Rules for FPF|E.10]] MG-DA token-to-EntityOfConcern anchoring or [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence anchors. It still acts as a review trigger: confirm you are using the reserved sense, not smuggling a basedness meaning.

**Naming guard for baseRelation tokens.** Do not mint new baseRelation tokens whose head noun is a metaphor (`Anchor*`, `Ground*`, `Attach*`). If you are tempted to do so, you either (i) have not named the relation kind yet, or (ii) are introducing a source alias that must map onto an existing, more specific relation token through [[00_F.18 - Local-First Unification Naming Protocol|F.18]].
...
### [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4.6 - Base-change operation lexicon

As [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] distinguishes slot operations by whether they change a stored reference, resolve a reference, or replace a value, [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]] distinguishes **base declaration changes** by which component changes and what semantics are affected.
...
8. **changeBaseRelation** — not an edit-in-place. Changing `baseRelation` changes meaning; mint a new declaration and relate them via an explicit continuity relation ([[00_F.13 - Lexical Continuity & Deprecation|F.13]] discipline), rather than silently rewriting the kind.

**Relation to [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] slot operations (non-normative mapping).** These are *semantic change classes*; an implementation may realise them using [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] slot operations on the SWBD record fields. Example: a **rebase** may be implemented as a `retarget` of `baseRef` on a *new* SWBD edition. The point of [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]] is that “we retargeted a ref” is not the semantic story; “we rebased the declaration” is.

**Relation to [[00_E.18 - Transformation Flow Structure|E.18]] assurance ops (informative).** On `U.Transfer`, the allowed ops (`ConstrainTo/CalibrateTo/CiteEvidence/AttributeTo`) can be viewed as Context-approved specialisations of `declareBase/rescope/rebase/refreshWitnesses` for specific declared `baseRelation` readings, with stricter declared constraints and lintability.

### [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4.7 - Disambiguation guide for selecting a baseRelation
...
*Note.* The `viewedVia` / `retargetedAlong` operators are defined by the [[00_A.06 - Signature Stack & Boundary Discipline|A.6]].3/A.6.4 viewing/retargeting patterns; this table only classifies them as “relative-to-base” cases.

### [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4.7a - Support wording selection test
...
A support phrase is governed by [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]] only when all of the following can be stated:
...
| --- | --- |
| a source, model, diagram, publication, or view describes, constrains, or exposes something | source-description, specification-use, and publication patterns ([[00_A.07 - Strict Distinction (Clarity Lattice)]], [[00_A.06.03 - U.EpistemicViewing - EntityOfConcern-preserving morphism]], [[06_A.06.02 - U.EffectFreeEpistemicMorphing - Effect-free morphisms of epistemes]], [[05_C.02.03 - Unified Formality Characteristic F]], [[03_E.17 - Multi-View Publication Kit]], local source rules) |
| an observation, test, proof, role, or carrier bears on a claim | evidence patterns ([[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph]], [[04_A.02.04 - U.EvidenceRole]], [[00_G.06 - Evidence Graph & Provenance Ledger]]) |
| a claim is acceptable in an assurance or trust calculus | assurance patterns ([[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)]]) plus evidence ontology named by value when evidence is live |
| a causal, intervention, counterfactual, off-policy, or simulation-only use is admissible | causal-use pattern ([[00_C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability]]) |
| a mathematical lens, mapping, or model makes a use admissible or exposes preserved/lost structure | mathematical-lens patterns ([[00_C.29 - Mathematical Lens Use]], [[00_C.26 - Quantum-Like Modeling Lens]], [[00_F.09 - Alignment & Bridge across Contexts]]) |
| a metric, score, threshold, benchmark, or characteristic warrants a comparison | measurement/characteristic/comparison patterns ([[00_C.16 - Measurement & Metrics Characterization (MM-CHR)]], [[00_C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles]], [[00_G.09 - Parity and Benchmark Harness]]) |
| one thing helps work, enables an action, supplies a resource, or makes operation easier | work/resource/action patterns ([[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)]], [[04_A.15.04 - Work-Relevant Source Restoration]], [[10_A.06.A - U.ActionInvitationPrecisionRestoration - Affordance and Action-Invitation Precision Restoration (ACT-INV)]], [[00_C.11 - Decision Theory (Decsn-CAL)]]) or ordinary Plain help |
| one file, section, packet, companion, entry cue, or expanded case helps a reader find, inspect, or compare another item | publication, entry, or navigation patterns ([[03_E.17 - Multi-View Publication Kit]], [[00_E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline]], [[00_I.02 - Expanded Entry Disambiguation Cases]]) or ordinary orientation |

This test prevents support wording from becoming either a source-relation bucket or a basis bucket. [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]] governs only the support cases that are genuinely base-relative.

## [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:5 - Archetypal Grounding

### [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:5.1 - System archetype: calibration to a standard
...
### [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:5.2 - Episteme archetype: claim admissibility via evidence relations
...
### [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:5.3 - Structural archetype: constructive grounding of a model edge
...
## [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:6 - Bias-Annotation
...
## [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:7 - Conformance Checklist

A carrier (pattern, spec, schema, code carrier, or publication) conforms to [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]] iff:
...
2. **CC‑BD‑2 — Dependent and base are explicit and typed.**
   Every based declaration SHALL make both `dependent` and `base` explicit, and SHALL be SlotKind/ValueKind/RefMode disciplined per [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]].
...
15. **CC‑BD‑15 — Meaning lane separation.**
    Semantic meaning assignment SHALL be modeled via SenseCell/ConceptSet lane constructs ([[00_E.10 - Unified Lexical Rules for FPF|E.10]] D.CTX), not via SWBD. SWBD SHALL be used only for non-semantic base-dependence (admissibility, calibration, attribution, policy gating, constructive grounding, viewing/retargeting specialisations).
...
## [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:8 - Common Anti-Patterns and How to Avoid Them
...
## [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:9 - Consequences
...
A team has adopted [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]] if, for any decision-relevant “relative-to” statement, it can produce a resolvable tuple
...
## [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:10 - Rationale
...
Without explicit change classes, prose collapses distinct edits (rebase vs retime vs rescope vs witness refresh) and recreates the same ambiguity [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] removed at the slot layer.

## [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:11 - SoTA-Echoing
...
## [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:12 - Relations
...
**Builds on [[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]] `U.RelationSlotDiscipline`.**
SWBD introduces a structured record with slots; those slots must be SlotKind/ValueKind/RefKind disciplined, and its change classes must not be confused with slot-edit operations ([[13_A.06.05 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n-ary relations (with slot-operation lexicon)|A.6.5]]) or name-binding terminology ([[00_E.10 - Unified Lexical Rules for FPF|E.10]] / L‑BIND).

**Constrains [[00_A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence admissibility links.**
...
**Aligns with [[04_A.02.04 - U.EvidenceRole|A.2.4]] `U.EvidenceRole`.**
...
**Aligns with [[00_A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] constructive grounding (`tv:groundedBy`).**
...
**Coordinates with [[01_C.02.01 - U.Episteme - Epistemes and their slot relation|C.2.1]] grounding holons.**
...
**Coordinates with [[00_A.06.03 - U.EpistemicViewing - EntityOfConcern-preserving morphism|A.6.3]]–[[08_A.06.04 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] viewing/retargeting.**
...
**Coordinates with [[06_A.02.06 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] and `Γ_time`.**
...
**Feeds [[00_E.10 - Unified Lexical Rules for FPF|E.10]] / [[00_F.18 - Local-First Unification Naming Protocol|F.18]] lexical governance.**
...
**Constrains support wording in [[00_A.06 - Signature Stack & Boundary Discipline|A.6]].P/E.10.**
...
## [[14_A.06.06 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:End
