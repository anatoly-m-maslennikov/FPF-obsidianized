...
**Depends on.** [[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **Lexical Discipline for “Context” (D.CTX)**; [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] **Strict Distinction**; [[00_A.11 - Ontological Parsimony (C-5)|A.11]] **Ontological Parsimony**; [[00_A.08 - Universal Core Principle (C-1)|A.8]] **Universal Core**.
**Coordinates with.** [[00_F.01 - Domain-Family Landscape Survey|F.1]] **Contexts (Contexts)**; [[00_F.02 - Term Harvesting & Normalisation|F.2]] **Harvest**; [[00_F.03 - Intra-Context Sense Clustering|F.3]] **SenseCells**; [[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] **Role Description**; [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] **Naming Discipline**; [[00_F.07 - Concept-Set Table|F.7]] **Concept‑Set Table**; [[00_F.09 - Alignment & Bridge across Contexts|F.9]] **Alignment & Bridge**.
**Aliases (informative).** *Mint‑vs‑Reuse gate*; *Naming governor*.

## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:1 - Intent & applicability
...
The lattice enforces **locality of meaning** (Contexts), **senseFamily separation** ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]), and **parsimony** ([[00_A.11 - Ontological Parsimony (C-5)|A.11]]) while remaining didactically simple.
...
## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:2 - Problem frame
...
3. **Hidden bridges.** Cross‑context sameness is implied by look‑alike words rather than declared ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]).
4. **Kernel sprawl.** New **U.Types** appear to plaster over local vocabulary gaps.

## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:3 - Forces
...
## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:4 - Minimal vocabulary (used in this pattern only)

* **Context** — `U.BoundedContext` (per D.CTX).
* **SenseCell** — address of a **local sense** produced by [[00_F.03 - Intra-Context Sense Clustering|F.3]] (one context × one clustered sense).
* **Concept‑Set row** — a **licensed Cross‑context reading** ([[00_F.07 - Concept-Set Table|F.7]]) of cells in one senseFamily with a declared **Row Scope** and **Row CL(min)**.
* **senseFamily** — as defined in **[[00_F.00.01 - Contextual Lexicon Principles|F.0.1]]**; here used as the **typed discriminator for rows** restricted to {Role | Status | Measurement | Type‑structure | Method | Execution}.
* **Role Description** — a **Role or Status** template anchored to a **single SenseCell** ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]).
* **Alias** — an **additional label** for an existing FPF label (within [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]), no new semantics.
...
## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:5 - The decision lattice (conceptual, notation‑free)
...
### [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:5.1 - Q0 — What is the **senseFamily** of your need?

* If **uncertain**, return to F.1/F.3: stabilise the Context(s) and the local sense.
* If **mixed**, split the need: one decision **per senseFamily** ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]).

### [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:5.2 - Q1 — Is there a **single Context** whose SenseCell already expresses it?

* **Yes →** **Reuse** the **SenseCell**’s label **inside that Context**.

  * If you need assignable behaviour or deontics on that sense: **define a Role Description** **anchored to that SenseCell** ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]).
* **No →** go to Q2.
...
### [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:5.3 - Q2 — Do you need to **read across Contexts** (same senseFamily)?

* **No →** stay within one context; if your desire is merely a nicer label, consider an **Alias** (Q3).
* **Yes →** check [[00_F.07 - Concept-Set Table|F.7]] for a **Concept‑Set row** covering your cells **in this senseFamily** with adequate **Row Scope** and **Row CL(min)**.

  * **Found & sufficient →** **Reuse the row’s FPF label** at that scope.
  * **Not found or insufficient →** either (a) **publish a contrast** (teach difference), or (b) propose a **new row** but only after [[00_F.09 - Alignment & Bridge across Contexts|F.9]] Bridges exist at **τ(scope)**.

> *Example (manager).* You want one label for the **actor** in workflow and provenance prose. [[00_F.07 - Concept-Set Table|F.7]] has a **Naming‑only** row mapping *BPMN Participant* ↔ *PROV Agent* at CL = 2. **Reuse** “actor” **at Naming‑only** scope; do **not** infer identity in models.

### [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:5.4 - Q3 — Is this **only a wording preference** for an existing FPF label?

* **Yes →** add an **Alias** in [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] (Tech register and/or Plain register), no semantics changed.
* **No →** go to Q4.
...
### [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:5.5 - Q4 — Does your need recur across Contexts in a way **not captured** by current rows, **with Bridges** already available at the required CL?

* **Yes →** propose a **new Concept‑Set row** ([[00_F.07 - Concept-Set Table|F.7]]): small (2–4 Contexts), **one senseFamily**, declare **Row Scope** and **Row CL(min)**, include a **counter‑example** if any Bridge has loss notes.
* **No →** go to Q5.

> *Example (engineer).* You repeatedly compare **runtime occurrence** in PROV with **PLC task runs**. [[00_F.09 - Alignment & Bridge across Contexts|F.9]] Bridges exist at CL = 2. Propose **row “execution-occurrence”** at **assignment/enactment-eligibility** scope (not Type-structure).

### [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:5.6 - Q5 — Are you describing a **kernel‑level notion** missing from the catalogue, **not** reducible to existing rows or Role Descriptions, and **present across ≥ 3 domain families** ([[00_A.08 - Universal Core Principle (C-1)|A.8]])?
...
## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:6 - Scope thresholds (default τ) — **how much sameness** you’re allowed to claim
...
| **Assignment-eligibility** | Safe to reference the row’s target as the **thing a `U.RoleAssignment` may point to** (e.g., a run, a value). | **2** | [[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)]] Role Description, acceptance narratives. |
...
## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:7 - Micro‑examples (didactic triad)

### [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:7.1 - For engineers — “Do we need a new **Execution** label?”
...
* **Row?** [[00_F.07 - Concept-Set Table|F.7]] has **execution-occurrence** at **assignment/enactment-eligibility**, CL = 2.
...
### [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:7.2 - For managers — “Can we call them all **actors**?”
...
### [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:7.3 - For researchers — “New **U.Type** for ‘Work Scope’?”

* **Need.** Kernel notion capturing **feasible performance region** across systems.
* **Test [[00_A.08 - Universal Core Principle (C-1)|A.8]].** Appears in **control** (reachable sets), **services** (operating envelope), **measurement** (confidence bands): **≥ 3 families?**
...
## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:8 - Invariants (normative, lightweight)
...
4. **Role Description anchoring.** Role Descriptions are **single-Context**, **single-cell** anchors ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]).
5. **Alias modesty.** Aliases **never** change semantics and live under [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]].
6. **Kernel restraint.** New **U.Types** are **rare**; [[00_A.08 - Universal Core Principle (C-1)|A.8]] **(≥ 3 families)** is mandatory, and duplication with existing U.Types must be ruled out.

## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:8.1 - Mint/Reuse discipline for **policy-ids** (normative addendum)
...
**MintDecisionRef.** A **resolvable reference** to the decision record that introduced (minted) a policy-id into a declared namespace/registry. For **normative** policy-ids this is typically a **DRR id** ([[00_E.09 - Design-Rationale Record (DRR) Method|E.9]]) or an equivalent change decision record. For **purely local, non-exported** policy-ids it MAY be a Gate `DecisionLog` entry ([[00_A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]) if that local-only scope is explicit.
...
## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:9 - Quick reference (one‑glance map)
...
| A nicer label for the same FPF concept      | **Alias in [[00_F.05 - Naming Discipline for U.Types & Roles]]**               | Style only; zero semantics.                      |
| A brand‑new primitive concept               | **New U.Type (rare)**          | Only if cross‑family, irreducible, kernel‑level. |

## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:10 - Anti‑patterns & remedies

| #         | Anti‑pattern               | Symptom                                                                            | Why it harms thinking                              | Remedy (conceptual move)                                                                                                                         |
| --------- | -------------------------- | ---------------------------------------------------------------------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| **AP‑1**  | **Row‑less sameness**      | Declaring “these mean the same” across Contexts without citing a **Concept‑Set row**. | Imports meaning implicitly; no CL guard.           | If Cross‑context reuse is desired, **reuse an existing row** at a declared **scope** ([[00_F.07 - Concept-Set Table]]), else **publish the contrast** and apply [[00_F.09 - Alignment & Bridge across Contexts]] Bridges only when the bridge relation is live. |
...
| **AP‑3**  | **Alias with payload**     | Introducing an Alias that subtly changes intent or senseFamily.                          | Hides semantics behind wording; confuses senseFamilies.   | Aliases ([[00_F.05 - Naming Discipline for U.Types & Roles]]) are **style only**. If semantics change, choose **row reuse** or **Role Description** instead.                                         |
| **AP-4**  | **Role-Description-to-row anchoring** | Role Description points to a **row** rather than a **single SenseCell**.       | Masks locality; **assignments** become cross-context by stealth. | Role Descriptions **must anchor to one SenseCell** ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)]]). Use rows only in prose or aggregated views.                                                |
| **AP‑5**  | **Kernel inflation**       | Proposing a new **U.Type** because a convenient label is missing.                  | Duplicates the kernel; violates parsimony.         | Apply [[00_A.08 - Universal Core Principle (C-1)]]: require **≥ 3 domain families** and **irreducibility**; otherwise **Alias** or **row**.                                               |
| **AP‑6**  | **senseFamily mixing**           | One name that conflates Role, Status, Measurement, or Type‑structure.              | Collapses [[00_A.07 - Strict Distinction (Clarity Lattice)]] Strict Distinction.                  | **Split by senseFamily** first (Q0). Decide **per senseFamily**.                                                                                             |
| **AP‑7**  | **Bridge‑by‑string**       | Treating identical lexical forms as equivalent senses across Contexts.                | Homonym trap; ignores local sense.                 | Equivalence only via **[[00_F.09 - Alignment & Bridge across Contexts]] Bridge** + **row**; never by string.                                                                                  |
| **AP‑8**  | **Row without loss notes** | Publishing a row where Bridges indicate mismatches, but row text is silent.        | Readers assume full equivalence.                   | Include **counter‑example** and **loss sketch** in the row’s narrative ([[00_F.07 - Concept-Set Table]]).                                                                    |
...
## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:11 - Reasoning primitives (judgement schemas, notation‑free)
...
## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:12 - Extended worked examples

### [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:12.1 - **Execution, observation, and acceptance** (engineers)
...
### [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:12.2 - **Actor across workflow and provenance** (managers)
...
### [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:12.3 - **Accuracy across metrology and data quality** (researchers)
...
### [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:12.4 **Subtype relation across OWL and a curated taxonomy** (formalists)
...
**Reasoning.** [[00_F.07 - Concept-Set Table|F.7]] row “subtype‑order” exists at **Type‑structure scope** with CL = 3 (only after verifying acyclicity & anti‑symmetry in `Taxonomy_X`). If the curated taxonomy contains cycles, **downgrade** to Naming‑only or reject the row.

**Outcome.** When CL≥3, you may **reuse row** for structural proofs; else teach differences.

## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:13 - Relations (with other patterns)

* **Builds on:** [[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] (D.CTX) **Context ≡ U.BoundedContext**; [[00_F.01 - Domain-Family Landscape Survey|F.1]] Contexts; [[00_F.02 - Term Harvesting & Normalisation|F.2]] Harvest; [[00_F.03 - Intra-Context Sense Clustering|F.3]] SenseCells.
* **Constrains:**

  * **[[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] Role Description:** **one SenseCell per Role Description**; no row anchoring.
  * **[[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] Naming:** Aliases are style‑only; no semantics movement.
  * **[[00_F.07 - Concept-Set Table|F.7]] Concept‑Set:** rows must declare **Scope** & **Row CL(min)** and carry **loss notes**.
  * **[[00_F.09 - Alignment & Bridge across Contexts|F.9]] Bridges:** any row proposal presupposes Bridges at or above τ(scope).
* **Used by.** All patterns (Part C) whenever new labels are contemplated.

## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:14 - Migration notes (conceptual)
...
4. **Alias proliferation.** Collapse redundant Aliases under a single [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] entry; keep both registers (Tech/Plain).
...
## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:15 - Acceptance tests (SCR/RSCR — concept‑level)

### [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:15.1 - Static conformance (SCR)
...
* **SCR‑F8‑S04 (Alias modesty).** Aliases introduced in [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] do **not** claim new semantics or change senseFamily.
* **SCR‑F8‑S05 (Kernel restraint).** Any new U.Type proposal includes **≥ 3 domain families** of evidence and an **irreducibility** note.

### [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:15.2 - Regression (RSCR)
...
## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:16 - Didactic distillation (90‑second teaching script)
...
## [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:End
