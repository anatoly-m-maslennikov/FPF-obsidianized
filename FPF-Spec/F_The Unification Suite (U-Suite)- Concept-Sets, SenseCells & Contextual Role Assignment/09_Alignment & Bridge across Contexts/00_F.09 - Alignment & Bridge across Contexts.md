...
**Builds on:** [[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] (Context discipline: Context = U.BoundedContext); **[[00_F.00.01 - Contextual Lexicon Principles|F.0.1]] (senseFamily & StatusModality guard; Bridge-only crossing)**; [[00_F.01 - Domain-Family Landscape Survey|F.1]] (Contexts fixed); F.2/F.3 (Cells exist); [[00_F.07 - Concept-Set Table|F.7]] (rows depend on Bridges); [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] (thresholds and reuse choice).

**Coordinates with:** [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] **Trust & Assurance Calculus** (uses CL penalties); **[[05_A.06.01 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] U.Mechanism** (Transport clause for cross-context use; penalties feed **R/R_eff** only; **F/G** invariant); Part C patterns (apply Bridges in formal claims); [[17_A.06.09 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR-XCTX)|A.6.9]] (RPR-XCTX for repairing umbrella “same/equivalent/align/map” prose into explicit Bridge Cards).
**Aliases (informative).** *Context-to-Context translator*; *Sense bridge*.

## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:1 - Intent & applicability
...
## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:3 - Forces
...
## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:4 - Core idea (didactic)
...
## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:5 - Minimal vocabulary (this pattern only)

* **Context** — shorthand for **U.BoundedContext** (per [[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]).
* **SenseCell** - the pair *(Context, Local-Sense)* from [[00_F.03 - Intra-Context Sense Clustering|F.3]].
...
* **senseFamily** — the semantic category (Role, Status, Measurement, Type-structure, Method, Execution…) per [[00_F.00.01 - Contextual Lexicon Principles|F.0.1]] (normative Part F guard).

## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:6 - Bridge kinds (senseFamily-aware)
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:6.1 - Substitution Bridges (sense-preserving)
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:6.2 - Interpretation Bridges (cross-senseFamily, explanatory)
...
## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:7 - CL scale and scope thresholds
...
* **Penalty use (informative):** [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] may convert **CL** into an assurance penalty when a cross-context claim is made.

## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:8 - The Bridge Card (compact sketch)
...
5. Ask whether asking, measuring, exporting, rendering, or bridging changes the represented state itself. If yes, coordinate with [[01_C.26.01 - Probe-Coupled Boundary Interaction|C.26.1]].
6. Ask whether coordinated work or live state is not exported faithfully enough for the intended use by any one report or bridge. If yes, coordinate with [[02_C.26.02 - Enacted Distributed State Evidence|C.26.2]].
7. Ask whether the crossing is a state representation with declared source-loss mode or reduced recoverability. If yes, coordinate with CSC/RT and the [[00_C.26 - Quantum-Like Modeling Lens|C.26]] coarsening support section.
...
- a [[01_C.26.01 - Probe-Coupled Boundary Interaction|C.26.1]] note when the export/probe changes represented state;
- a [[02_C.26.02 - Enacted Distributed State Evidence|C.26.2]] note when coordinated state has no faithful-enough export for the intended use;
- a CSC, RT, or [[00_C.26 - Quantum-Like Modeling Lens|C.26]] coarsening handoff when the exported representation intentionally carries reduced detail, reduced recoverability, or narrower use.

## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:9 - Invariants (normative)
...
9. **Kernel restraint.** Bridges **cannot** be used to promote ad-hoc sameness into a new **U.Type**; [[00_A.11 - Ontological Parsimony (C-5)|A.11]] applies.
10. **Non-inheritance of Contexts.** Bridges **do not** imply -is-a- between Contexts ([[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]).
...
## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:10 - Micro-examples (illustrative, one-liners)
...
## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:11 - Anti-patterns & remedies
...
| **AP-9** | **Bridge as new U.Type** | Using a Bridge to justify minting a new universal Type. | Re-globalises meaning; breaks [[00_A.11 - Ontological Parsimony (C-5)]] parsimony. | Keep Types context-local; where reuse is needed, use **rows** + Bridges, not new primitives. |
...
## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:12 - Worked examples (didactic)

### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:12.1 - Service acceptance (design) vs executions & observations (run)
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:12.2 - Behavioural role vs access role
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:12.3 - Equivalence of subtype notions for structural rows
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:12.4 - Accuracy (metrology) vs accuracy (data-quality)
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:12.5 - Setpoint (control) vs target (service)
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:12.6 - Role substitution & CL gating (RoleAssignment/enactment scope)
...
#### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:12.6.1 - Directional substitution (role-oriented shorthand)
...
* The reverse direction **does not** follow unless declared ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]:13.7).

#### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:12.6.2 - CL > gating policy (didactic default)
...
*Notes.* The **substitution scope** is defined in **[[00_F.09 - Alignment & Bridge across Contexts|F.9]]:13.2-13.3** (Role-Assignment/Enactment-eligible substitution requires **CL >= 2**; Naming-only is **CL >= 1**).
CL penalties feed assurance (R) per **[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]**; safety-critical policies may require **CL >= 2** by default (D.2).

#### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:12.6.3 - Typical bridges (worked patterns)
...
#### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:12.6.4 - Bridge invariants (role-relevant reminders)
...
* **No silent inversion.** Direction is explicit; substitution does not reverse unless declared ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]:13.7).

## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:13 - Reasoning primitives (judgement schemas)
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:13.1 - Bridge declaration
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:13.2 - Substitution scope (senseFamily-preserving)
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:13.3 - Naming-only scope
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:13.4 - Prohibition by kind
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:13.5 - Interpretation embargo
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:13.6 - Weakest-link rule for rows
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:13.7 - Direction guard
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:13.8 - SenseFamily purity
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:13.9 - Loss accumulation
...
## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:14 - Relations

**Builds on:** [[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] (Context discipline: Context = U.BoundedContext); **[[00_F.00.01 - Contextual Lexicon Principles|F.0.1]] (senseFamily guard; Bridge-only crossing)**; [[00_F.01 - Domain-Family Landscape Survey|F.1]] (Contexts fixed); F.2/F.3 (Cells exist); [[00_F.07 - Concept-Set Table|F.7]] (rows depend on Bridges); [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] (thresholds and reuse choice).

**Coordinates with:** [[01_F.09.01 - Bridge Stance Overlay|F.9.1]] for stance overlays that remain subordinate to bridge cards; [[01_E.17.01 - U.ViewpointBundleLibrary - Reusable Viewpoint Bundles|E.17.1]] when viewpoint bundles need explicit cross-family correspondence; [[02_C.16.Q - Quality-Term Precision Restoration|C.16.Q]] / [[00_C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]] when evaluative endpoints or bundle-shaped quality families cite bridge cards without absorbing bridge semantics.

**Constrains:**

* **[[00_F.07 - Concept-Set Table|F.7]] Concept-Set Table:** each cross-context row must name supporting **Bridges**; row scope <= the weakest supporting Bridge.
* **[[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] Mint or Reuse:** reuse choices reference **CL** and **kind**; no reuse without a Bridge.
* **Part C patterns:** formal claims that span Contexts cite Bridges and respect senseFamily/StatusModality & CL constraints.
* **[[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] Trust & Assurance Calculus:** may interpret **CL** as a penalty factor in Cross-context reasoning.

## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:15 - Migration notes (conceptual)
...
4. **senseFamily refactor.** If a Cell’s senseFamily was mis-typed, fix the senseFamily first in [[00_F.03 - Intra-Context Sense Clustering|F.3]], then revisit Bridges; **Interpretation** is safer than forced substitution.
...
## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:16 - Acceptance tests (SCR/RSCR — concept-level)

### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:16.1 - Static conformance (SCR)

* **SCR-F9-S01 (Well-typed).** Every Bridge names **two SenseCells**, each bound to a **Context** from [[00_F.01 - Domain-Family Landscape Survey|F.1]], and states **senseFamily**, **kind**, **dir** (if needed), **CL**, **Loss**, and **scope**.
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:16.2 - Regression (RSCR)
...
## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:17 - Didactic distillation (90-second script)
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:17.1 - Bridge stance overlay compatibility
A bridge card may carry a [[01_F.09.01 - Bridge Stance Overlay|F.9.1]] Bridge Stance Overlay such as `localRename`, `operationalizes`, `partialAnalogy`, `projection`, or `nonEquivalent`. The overlay is a local interpretive annotation and **does not replace** the underlying bridge kind, direction, `CL`, or loss notes.

## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:18 - Archetypal Grounding

### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:18.1 - Tell
...
## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:19 - Bias-Annotation

Lenses tested: **Gov**, **Arch**, **Onto/Epist**, **Prag**, **Did**. Scope: **Universal** for cross-context correspondence and reuse.

* **Gov bias:** [[00_F.09 - Alignment & Bridge across Contexts|F.9]] raises the declaration bar by requiring explicit Bridge Cards.
...
* **Onto/Epist bias:** [[00_F.09 - Alignment & Bridge across Contexts|F.9]] is local-first and resists global meaning claims.
...
## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:20 - Conformance Checklist (CC-[[00_F.09 - Alignment & Bridge across Contexts|F.9]])

A Bridge publication conforms to [[00_F.09 - Alignment & Bridge across Contexts|F.9]] iff:
...
6. **CC-F.9-6 - Overlay non-collapse.**
   If a [[01_F.09.01 - Bridge Stance Overlay|F.9.1]] Bridge Stance Overlay is used, it remains an annotation and does not replace bridge kind, direction, `CL`, or Loss Notes.
...
## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:21 - Consequences

**Benefits.**
[[00_F.09 - Alignment & Bridge across Contexts|F.9]] lets FPF compare, translate, and partially reuse ideas across Contexts without collapsing them into one vocabulary. It gives downstream rows, claims, and assurance reasoning an explicit Bridge Card record instead of relying on prose intuition.
...
## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:22 - Rationale

The core move of [[00_F.09 - Alignment & Bridge across Contexts|F.9]] is simple: cross-context work is unavoidable, but silent sameness is unacceptable. A Bridge therefore does two jobs at once:
...
## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:24 - Bridge Card Publication Discipline

### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:24.1 - Minimal bridge-card declaration
...
If any of these fields is absent, later readers are forced back into inference by prose similarity, which is exactly what [[00_F.09 - Alignment & Bridge across Contexts|F.9]] is supposed to block.

### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:24.2 - One-pair default rule
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:24.3 - Revision over silent drift
...
## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:25 - Bundle and Endpoint Interaction Law

### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:25.1 - Viewpoint and bundle interaction
Viewpoint bundles, quality bundles, and other endpoint bundles may cite Bridges, but they do not absorb bridge semantics. [[00_F.09 - Alignment & Bridge across Contexts|F.9]] remains the pattern for cross-context alignment, while the citing bundle keeps its own ontology.

### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:25.2 - Quality-family interaction
...
### [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:25.3 - Overlay interaction rule
A [[01_F.09.01 - Bridge Stance Overlay|F.9.1]] stance overlay may help readers interpret a bridge, but the bridge card remains primary. If the overlay overstates the bridge kind, direction, `CL`, or Loss Notes, the card wins and the overlay should be narrowed or removed.

## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:26 - Review Matrix and Migration Tests
...
## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:14a - [[00_C.29 - Mathematical Lens Use|C.29]] mathematical-lens use relation

> When meaning, substitution, sense cells, direction, `CL`, or Bridge-supported use crosses context, write the [[00_F.09 - Alignment & Bridge across Contexts|F.9]] Bridge Card first. Add the applicable [[00_C.29 - Mathematical Lens Use|C.29]] output only for mathematical-lens use: candidate mathematical object, `LensMappingMode`, preserved and lost structure, exposed invariants or distinctions, lens-use admissibility value, admissible and non-admissible use, and stop condition. Do not duplicate Bridge semantics inside MathLensUse. A Bridge may make a mathematical lens interpretable across contexts without making it substitution-safe.

## [[00_F.09 - Alignment & Bridge across Contexts|F.9]]:End
