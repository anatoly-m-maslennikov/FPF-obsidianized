...
**Depends on.** [[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **Lexical Discipline for “Context” (D.CTX)**; **[[04_E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]] EntityOfConcern, Description Episteme, and Specification-Use**; [[00_F.01 - Domain-Family Landscape Survey|F.1]] **Domain‑Family Landscape Survey**; [[00_F.02 - Term Harvesting & Normalisation|F.2]] **Term Harvesting & Normalisation**; [[00_F.03 - Intra-Context Sense Clustering|F.3]] **Intra‑Context Sense Clustering**; [[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] **Role Description Definition**; [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] **Strict Distinction**; [[00_A.11 - Ontological Parsimony (C-5)|A.11]] **Ontological Parsimony**; [[00_F.00.01 - Contextual Lexicon Principles|F.0.1]] **context‑local Lexicon Principle (RLP)**.
**Coordinates with.** [[00_F.07 - Concept-Set Table|F.7]] **Concept‑Set Table**; [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] **Mint or Reuse?**; [[00_F.09 - Alignment & Bridge across Contexts|F.9]] **Alignment & Bridge**; [[00_F.13 - Lexical Continuity & Deprecation|F.13]] **Term Registry & Deprecation**.
**Aliases (informative).** *Context‑true naming*; *Two‑register labels*.

## [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:1 - Intent & applicability

**Intent.** Provide a **small, normative code of naming** so that **U.Types** (Cross‑context categories) and **Role Descriptions** (context‑local Roles or Statuses) are labelled **clearly, locally faithful, and globally stable**, without importing tooling, workflows, or editorial process. Names are **consequences of meaning** fixed earlier ([[00_F.01 - Domain-Family Landscape Survey|F.1]]–[[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]), not badges invented to “stabilise” drifting words.

**Applicability.** Use **whenever** you (a) mint or revise a **U.Type** name from a **Concept‑Set row** ([[00_F.07 - Concept-Set Table|F.7]]), or (b) assign labels to a **Role Description** ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]). This pattern governs **what a good name must be**, not *how a team produces it*.

**Non‑goals.** No registries, reviews, or hand‑offs. No style‑police for punctuation beyond conceptual clarity. No bridging or synonym decisions across Contexts ([[00_F.09 - Alignment & Bridge across Contexts|F.9]] does that).

## [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:2 - Problem frame
...
## [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:3 - Minimal vocabulary (this pattern only)
...
* **SenseCell** — the **(Context × Local-Sense)** address cited by a Role Description card ([[00_F.03 - Intra-Context Sense Clustering|F.3]]–[[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]).
* **Concept‑Set row** — the Cross‑context table row that supports minting a **U.Type** name ([[00_F.07 - Concept-Set Table|F.7]]).

## [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:4 - Core idea (didactic)
...
## [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:5 - Normative rules — Role Descriptions (context‑local labels)
...
## [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:6 - Normative rules — U.Types (Cross‑context labels)

Let **U** be a U.Type minted from a **Concept‑Set row** ([[00_F.07 - Concept-Set Table|F.7]]) satisfying [[00_A.08 - Universal Core Principle (C-1)|A.8]] (≥3 domain families) AND MinInterFamilyDistance ≥ δ_family (from F1‑Card).
...
**R‑UT‑3 (No senseFamily mixing in names).** Do **not** name a U.Type with deontic or behavioural language (*“PermittedService”*, *“ResponsibleAgent”*). **Role, Status, Method, and Execution** belong to **Role Descriptions ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]])** or local senses; U.Types are *what‑it‑is* kinds, not *what‑it‑does* or *what‑is‑allowed*.
...
**R‑UT‑5 (No Context tags in names).** U.Types are **Context‑agnostic**; never append “(BPMN)”/“(PROV)”. Provenance for the row lives in [[00_F.07 - Concept-Set Table|F.7]], not in the name.

**R‑UT‑6 (Alias only for pedagogy).** Allow **Plain aliases** for teaching; **Tech label** is unique and stable. Synonym management belongs to **[[00_F.13 - Lexical Continuity & Deprecation|F.13]]**; do not invent alternates ad hoc.
...
## [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:7 - Twin rules
...
**One twin, one context.** At most one plain twin per Context; register in **[[02_E.10.P - Conceptual Prefixes policy & registry|E.10.P]]**.

## [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:8 - Invariants (normative, lightweight)
...
**INV‑F5‑5 (Minimality).** Labels **MUST** reflect the **minimal generality** supported by invariants ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] for Role Description, [[00_F.07 - Concept-Set Table|F.7]] for U.Types).
...
## [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:9 - Reasoning primitives (judgement schemas, notation‑free)
...
## [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:10 - Micro‑examples (illustrative)
...
## [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:11 - Anti‑patterns & remedies
...
| **A9**  | **Synonym spray**           | Multiple Tech labels for one U.Type                                | Fragmentation; alias drift                                         | One **Tech** label; further lexical forms live in the **registry** ([[00_F.13 - Lexical Continuity & Deprecation]]). (R‑UT‑6, INV‑F5‑1)                       |
...
| **A13** | **Bracketed twins**         | `Participant/Agent`, `Service/SLO` as single label                 | Two senses slipped into one card                                   | Pick **one** label per concept; the other lives as alias ([[00_F.13 - Lexical Continuity & Deprecation]]) or as a different card. (INV‑F5‑1, R‑UT‑6)          |
...
## [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:12 - Worked examples

> Each example shows the **reasoning move** that leads to the label; no procedures, no tooling.

### [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:12.1 - Role Description labels (context-local)
...
* **Why.** The *record of work* is a **U.Type** elsewhere ([[01_A.15.01 - U.Work|A.15.1]]); Role Description in this Context carries **badges** of runs. ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] stance split; R-RD-3)

### [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:12.2 - U.Type labels (from Concept‑Set rows)
...
### [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:12.3 - Mixed scenario (service acceptance over execution traces)
...
## [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:13 - Migration notes (conceptual)
...
8. **Deprecation path.** If a label must change, publish the **new Tech + Plain**, keep the old as an **alias** in the registry ([[00_F.13 - Lexical Continuity & Deprecation|F.13]]), and leave the reasoning trail in the Concept‑Set row that forced the rename. (R‑UT‑6, [[00_F.13 - Lexical Continuity & Deprecation|F.13]] linkage)

## [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:14 - Acceptance tests (SCR/RSCR — concept‑level)

### [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:14.1 - Static conformance (SCR)
...
### [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:14.2 - Regression checks (RSCR)
...
* **RSCR-F5-E02 (Edition churn).** When a Context updates, Role Description labels remain stable unless the **sense** changed; if sense changed, **split** the card and keep aliases in [[00_F.13 - Lexical Continuity & Deprecation|F.13]].
...
## [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:15 - Didactic distillation (60‑second recap)
...
## [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]:End
