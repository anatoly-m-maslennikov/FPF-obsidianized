...
**Depends on.** [[00_F.01 - Domain-Family Landscape Survey|F.1]] **context of meaning**; [[00_F.02 - Term Harvesting & Normalisation|F.2]] **Harvesting**; [[00_F.03 - Intra-Context Sense Clustering|F.3]] **Local Sense Clustering**; [[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] **Role Description**; [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] **Naming Discipline**; [[00_F.07 - Concept-Set Table|F.7]] **Concept‑Set Table**; [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] **Mint‑or‑Reuse**.
**Coordinates with.** [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] **Status Windows & Mapping**; [[00_F.11 - Method Quartet Harmonisation|F.11]] **Method Quartet Harmonisation**; [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]] **Service Acceptance Binding**; [[00_F.13 - Lexical Continuity & Deprecation|F.13]] **Lexical Continuity**.
**Aliases (informative).** *Role and Status economy*; *Explosion guard*.

## [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:1 - Intent & applicability

**Intent.** Prevent the uncontrolled growth of **Roles** and **Statuses** by privileging **reuse**, **bundling**, **explicit separation‑of‑duties (SoD)**, and **applicability windows** over minting new names. Keep the vocabulary **small, crisp, and composable** while remaining faithful to local meanings fixed by Contexts ([[00_F.01 - Domain-Family Landscape Survey|F.1]]) and SenseCells ([[00_F.03 - Intra-Context Sense Clustering|F.3]]).

**Applicability.** Whenever a new Role or Status is proposed, a team merges two lines of work, or a domain shifts its jargon. Use this pattern before adding rows to the Concept‑Set Table ([[00_F.07 - Concept-Set Table|F.7]]) or new Role Descriptions ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]).
...
## [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:2 - Problem frame
...
Explosion harms didactics and increases alignment cost ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]).

## [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:3 - Forces
...
## [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:4 - Minimal vocabulary (this pattern only)

* **Role Description** ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]): a **Role** (behavioural mask) or **Status** (epistemic/deontic standing) tied to a **SenseCell**.
* **Concept‑Set row** ([[00_F.07 - Concept-Set Table|F.7]]): a Cross‑context **intent** (“what we count as one thing”) aligned by SenseCells.
...
* **Window** ([[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]): an **claim scope** (time stance, holon level, run segment) that delimits when a Role or Status holds.

## [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:5 - Core idea (didactic)
...
## [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:6 - Solution — the control cabinet (conceptual, notation‑free)

### [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:6.1 - Reuse by row (first lever)

* **Move.** If a proposal matches the **intension** of an existing row ([[00_F.07 - Concept-Set Table|F.7]]), adopt its Role Description or add a local SenseCell **inside that row**.
* **Pay‑off.** Names don’t proliferate; Cross‑context tables stay thin.

**Example (services).** *Service‑availability‑compliance* already exists as a row. New labels *SLO‑Met* / *Uptime‑OK* **reuse** that row; SOSA/SSN Observations later feed it ([[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]).

### [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:6.2 - Bundle instead of hybrid (second lever)
...
### [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:6.3 - Separate by SoD, don’t evade (third lever)
...
### [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:6.4 - Window the Status (fourth lever)
...
### [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:6.5 - Factor modifiers as facets, not names
...
## [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:7 - Invariants (normative)

1. **context‑locality.** Each Role Description remains tied to a **SenseCell** in a **single Context** ([[00_F.03 - Intra-Context Sense Clustering|F.3]], [[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]).
2. **Row preference.** New Role Descriptions **SHOULD** map to an existing row; new rows ([[00_F.07 - Concept-Set Table|F.7]]) require [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] justification.
3. **No hybrid Roles.** If two Roles are conceptually distinct, they **must not** be fused into one to bypass SoD. Use **Bundle + SoD**.
4. **Windowed statuses.** Status proliferation across time/scale **MUST** be expressed as **windows** of a single Status family ([[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]).
5. **Bundle clarity.** A Bundle **names only composition**; it does not inherit or redefine member semantics.
6. **Minimal modifier naming.** Adding a modifier to a label **MUST** pass [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] tests; prefer facets/windows over new Role or Status names.
7. **Concept‑first.** No invariant relies on organization charts or access policies; **semantics precede governance**.

## [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:8 - Reasoning primitives (judgement schemas)
...
## [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:9 - Micro‑examples (engineer, manager, and researcher lenses)

### [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:9.1 - Enactment (change control)
...
### [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:9.2 - Services (SLO evaluation)
...
### [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:9.3 - KD‑CAL (evidence)

* **Proposal.** *Pre‑validated* between *Verified* and *Validated*.
* **Moves.** Use **Status chain** within one family: `Verified → Validated`; represent uncertainty as **confidence** ([[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]), not another Status.
* **Result.** Clean ladder; no extra label.

### [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:9.4 - Sys‑CAL (plant ops)
...
### [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:9.5 - Method quartet (reviews)

* **Proposal.** *Senior‑Reviewer* to bypass `{Author ⟂ Reviewer}`.
* **Moves.** Keep SoD; if seniority matters, introduce **Assurance Level** facet ([[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]) on the **review decision**, not a new Role.
* **Result.** Separation preserved; trust expressed as a Status property, not a Role type.

## [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:10 - Anti‑patterns & remedies
...
| **A2**  | **Modifier‑as‑type**                     | *Night‑Operator*, *Remote‑Operator*, *On‑call‑Reviewer*.                                     | Name proliferation for circumstantial qualifiers.                 | Keep **Role = Operator/Reviewer**; express *night/remote/on‑call* as **facets or windows** ([[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)]]).                                |
| **A3**  | **Window‑as‑type**                       | *Compliant*, *At‑Risk*, *Grace*, *Breached* as separate Status types.                        | Paints temporal phases as different essences; breaks comparisons. | One **Status family** (e.g., **Compliance**) with **windows**: *evaluation / active / grace / archival* ([[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)]]).                   |
| **A4**  | **Row drift**                            | New Concept‑Set row for *Uptime‑OK* when **Service‑Availability‑Compliance** already exists. | Splits one intent across rows; Cross‑context tables get wide.        | **Reuse the row** ([[00_F.07 - Concept-Set Table]]) if intent matches; add local **SenseCell** if needed.                                                     |
...
| **A6**  | **cross-context fusion**                    | One Role Description mixes *Execution (IEC)* with *Duty (ODRL)* semantics.                       | Violates locality; meanings leak across Contexts.                    | Keep each **Role Description** tied to a **SenseCell** ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)]]). cross-context reasoning uses **Bridge** ([[00_F.09 - Alignment & Bridge across Contexts]]).         |
| **A7**  | **Synonym carousel**                     | *Validator* vs *Verifier* vs *Checker* minted separately in the same Context.                   | Cognitive noise; ambiguous separation.                            | Choose **one label** via [[00_F.05 - Naming Discipline for U.Types & Roles]]; keep others as **aliases** in **Lexical Continuity** ([[00_F.13 - Lexical Continuity & Deprecation]]), not new templates.                     |
| **A8**  | **Org-chart mirroring**                  | Roles cloned from a company chart (*Squad-Lead*, *Tribe-Lead*) as generic **Role Descriptions**. | Organisation-specific names masquerade as semantics.              | Map local titles to **Bundles** of generic Roles (e.g., `{Planner, Coordinator}`), or treat as **aliases** ([[00_F.13 - Lexical Continuity & Deprecation]]). |
...
| **A13** | **Contextless Role Description**            | Role Description without a SenseCell anchor.                                                     | Floating meaning; later bridges cannot be made explicit.          | Tie every **Role Description** to a **SenseCell** ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)]]). If none fits, use [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)]] to decide: **new row** or **rename/reuse**.        |
...
## [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:11 - Worked examples

### [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:11.1 - Enactment + Services + KD‑CAL — “SLO compliance without label sprawl”
...
* **Moves ([[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]).** Keep **Compliance** as one **Status family**; define **windows**: *evaluation* (prediction against forecast), *active* (actuals vs target), *grace* (tolerated breach). **Waiver** becomes a **deontic Status** in ODRL Context, not part of Compliance.
...
### [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:11.2 - Method‑CAL + Enactment — “Reviews with SoD and Bundle”
...
### [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:11.3 - Sys‑CAL + LCA‑CAL + Services — “Operations without role fragments”
...
### [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:11.4 - KD‑CAL + Kind-CAL — “Evidence ladder without new labels”
...
## [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:12 - Relations (with other patterns)

* **Builds on:** [[00_F.01 - Domain-Family Landscape Survey|F.1]] (Contexts), [[00_F.02 - Term Harvesting & Normalisation|F.2]] (Harvesting), [[00_F.03 - Intra-Context Sense Clustering|F.3]] (Local Clustering), [[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] (Role Description), [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] (Naming).
* **Constrains:**

  * **[[00_F.07 - Concept-Set Table|F.7]] (Concept‑Set Table):** prefer **row reuse**; new rows require [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] justification.
  * **[[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] (Mint‑or‑Reuse):** apply **four levers** (reuse, bundle, SoD, window) before minting.
  * **[[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] (Status Windows & Mapping):** encode temporal/scale variation as **windows**, not new Status types.
  * **[[00_F.12 - Service Acceptance-Work Evidence Link|F.12]] (Service Acceptance Binding):** bind acceptance to the **Compliance** Status family; avoid ad‑hoc status labels.
  * **[[00_F.13 - Lexical Continuity & Deprecation|F.13]] (Lexical Continuity):** prior names become **aliases**; do not carry forward inflated vocabularies as new types.
* **Used by.** FPF patterns to keep Role and Status vocabularies tight.

## [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:13 - Migration notes (conceptual playbook)

1. **Map to rows.** For each existing Role or Status, identify its **Concept‑Set row**; if two names share an intent, **collapse** to one row (keep other names as **aliases**, [[00_F.13 - Lexical Continuity & Deprecation|F.13]]).
...
5. **Re‑use before minting.** When encountering a gap, scan rows for a near‑match; only if intent genuinely differs, open a **new row** ([[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]).
6. **Preserve continuity.** Keep historic labels as **aliases** under the consolidated template ([[00_F.13 - Lexical Continuity & Deprecation|F.13]]); do not rewrite past texts.
...
## [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:14 - Acceptance tests (SCR/RSCR — concept‑level)

### [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:14.1 - SCR — Static conformance

* **SCR‑F14‑S01 (Row reuse).** Every newly proposed Role Description either **references an existing row** or includes a clear **[[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] justification** for a new row.
...
### [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:14.2 - RSCR — Regression (evolution)

* **RSCR‑F14‑E01 (Vocabulary slope).** Over a given interval, the count of distinct Role or Status templates **does not increase** unless matched by **row justifications** ([[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]).
...
* **RSCR‑F14‑E04 (Alias discipline).** When labels change, prior names are recorded as **aliases** ([[00_F.13 - Lexical Continuity & Deprecation|F.13]]); no silent type multiplication.

## [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:15 - Didactic distillation (90‑second script)
...
## [[00_F.14 - Anti-Explosion Control (Roles & Statuses)|F.14]]:End
