...
**Builds on:** [[00_F.01 - Domain-Family Landscape Survey|F.1]] **context of meaning (U.BoundedContext)**; [[00_F.02 - Term Harvesting & Normalisation|F.2]] **Term Harvesting**; [[00_F.03 - Intra-Context Sense Clustering|F.3]] **Intra‑Context Sense Clustering**; [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] **Naming Discipline**; F.7/F.9 **Bridges & CL**; [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] **Status Families & Windows**; [[00_F.11 - Method Quartet Harmonisation|F.11]] **Method Quartet Harmonisation**; [[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]] **U.PromiseContent**.
...
## [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:1 - Intent & applicability
...
## [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:2 - Problem frame
...
## [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:3 - Forces
...
## [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:4 - Core idea (didactic)
...
4. **Window** — the explicit period in which the judgement is made ([[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]).
...
## [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:5 - Minimal vocabulary (this pattern only)
...
* **Window.** A time envelope (calendar month, batch, incident interval, shift) per [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]].
...
## [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:6 - The binding, as five mental rules (notation‑free)
...
**R5 — Window rule.**
Every verdict is **time‑bounded**. Changing the Window can change the result; **no retroactivity** from new clauses or specs (cf. [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]).

## [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:7 - Clause templates (conceptual schemata)
...
## [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:8 - Invariants (normative)

1. **DesignRunTag split.** Clauses live on the **design stance**; judgements live on the **run stance** about **Work** ([[00_F.11 - Method Quartet Harmonisation|F.11]]).
...
9. **No detached pass.** When the ClauseCell is a `U.PromiseContent` (service promise clause), `Satisfied` about a Work is admissible only if that Work also **delivers** the promised outcome spec for the clause ([[03_A.02.03 - U.PromiseContent (Promise Content)|A.2.3]]:8.1 `fulfilsPromiseContent`). This keeps acceptance on the **same delivery evidence base** (Work facts + Δ anchors + Observations) and prevents “pass verdict separate from delivery”.

## [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:9 - Micro‑examples (didactic, multi‑domain)

### [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:9.1 - SaaS uptime (services + sensing)
...
### [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:9.2 - Furnace band (industrial control)
...
### [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:9.3 - Incident MTTR (services + enactment)
...
## [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:10 - Anti‑patterns & remedies
...
| **A4**  | **Global words**                | *Availability*, *latency* used without a Context.                                               | Collides senses across disciplines.             | Speak with **Context prefixes** ([[00_F.01 - Domain-Family Landscape Survey]]). Cross‑context reuse demands a **Bridge** ([[00_F.09 - Alignment & Bridge across Contexts]]).                                                                         |
...
| **A11** | **Status on “the service”**     | Tagging an abstract umbrella like *“the service”* as “Satisfied”.                            | Loses the entityOfConcern of the judgement.           | Attach to **ClauseCell\@Window about WorkCell**; the service concept remains a promise vocabulary ([[03_A.02.03 - U.PromiseContent (Promise Content)]]).                                               |
| **A12** | **Bridge‑by‑name**              | Equating *Activity* ≡ *Process* because both say “process”.                                  | Assumes global meaning.                         | Use **[[00_F.09 - Alignment & Bridge across Contexts]] Bridge** with **kind/CL/Loss**; or keep them distinct.                                                                                         |

## [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:11 - Extended worked examples
...
### [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:11.1 - CDN latency across regions (services + sensing + types)
...
### [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:11.2 - Stroke care: door‑to‑needle (method + enactment + status)

* **Contexts.** *clinical guideline canon* (Clause), *PROV‑O* (Work), *SOSA/SSN* (Measure), *[[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]* (status windows).
...
### [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:11.3 - Cold‑chain warehouse (control + sensing + deontics)
...
### [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:11.4 - SaaS incident MTTR (services + enactment)
...
## [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:12 - Reasoning primitives (judgement schemas, notation‑free)
...
7. **CL aggregation (assurance hint)**
   ...
   *Reading:* The weakest Bridge governs the assurance level communicated with the verdict (advisory to [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] calculus).
...
## [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:13 - Relations (with other patterns)

* **Builds on:**
  **[[00_F.01 - Domain-Family Landscape Survey|F.1]]** (Contexts): keeps all meanings **local**.
  **[[00_F.02 - Term Harvesting & Normalisation|F.2]]–[[00_F.03 - Intra-Context Sense Clustering|F.3]]**: provide the **SenseCells** that become Clause/Work/Measure anchors.
  **[[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]**: ensures labels for Clause/Work/Measure and Windows are didactically clear.
  **[[00_F.07 - Concept-Set Table|F.7]]–[[00_F.09 - Alignment & Bridge across Contexts|F.9]]**: supply **Bridge** kinds / **CL** and loss semantics.
  **[[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]**: defines **Status** families and **Window** constructs.
  **[[00_F.11 - Method Quartet Harmonisation|F.11]]**: protects **Method**, **MethodDescription**, **Work**, and **Actuation** distinctions.
...
* **Constrains:**
  Later reporting and assurance rules ([[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]) must **not** collapse CL/Loss; they report them alongside status.

## [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:14 - Migration notes (conceptual)
...
## [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:15 - Acceptance tests (SCR/RSCR — concept‑level)

### [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:15.1 - Static conformance (SCR)
...
### [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:15.2 - Regression checks (RSCR)
...
### [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:15.3 Didactic distillation (60‑second recap)
...
## [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]:End
