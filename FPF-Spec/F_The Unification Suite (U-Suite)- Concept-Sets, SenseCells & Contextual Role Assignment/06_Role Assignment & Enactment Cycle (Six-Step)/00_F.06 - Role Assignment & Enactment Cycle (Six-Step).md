...
**Depends on.** [[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **Lexical Discipline for “Context” (D.CTX)**; [[00_F.01 - Domain-Family Landscape Survey|F.1]] **Domain‑Family Landscape Survey**; [[00_F.02 - Term Harvesting & Normalisation|F.2]] **Term Harvesting & Normalisation**; [[00_F.03 - Intra-Context Sense Clustering|F.3]] **Intra‑Context Sense Clustering**; [[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] **Role Description**; [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] **Naming Discipline**.
**Coordinates with.** [[00_F.07 - Concept-Set Table|F.7]] **Concept‑Set Table**; [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] **Mint or Reuse?**; [[00_F.09 - Alignment & Bridge across Contexts|F.9]] **Alignment & Bridge**; [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] **Epistemic Status Mapping**; [[01_A.02.01 - U.RoleAssignment- Contextual Role Assignment|A.2.1]] **U.RoleAssignment**; [[00_A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]].\* **Role–Method–Work alignment**; KD‑CAL (observations, results).
**Aliases (informative).** *Assign-and-verify mental loop*; *six-step role cycle*.

## [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:1 - Intent & applicability

**Intent.** Provide a **minimal set of reasoning moves** that turn a **Role Description** ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]), anchored in a **SenseCell**, into a **sound claim** about a concrete **holder**—either a **Role assignment** or a **Status assertion**—with **local meaning** (within one context) and a **clear path to evidence** (KD‑CAL). These are **mental moves**, not workflows or tools.
...
**Non‑goals.** No process charts, no registries, no governance roles. No Cross‑context equivalences (that is [[00_F.09 - Alignment & Bridge across Contexts|F.9]]). No operational runbooks—only conceptual judgements.

## [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:2 - Problem frame

Without disciplined Role Assignment & Enactment reasoning:

1. **Sense‑family slippage.** Behavioural **Roles** and deontic/epistemic **Statuses** get mixed (keep them on distinct **senseFamilies**, per [[00_F.00.01 - Contextual Lexicon Principles|F.0.1]]).
...
## [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:3 - Forces
...
| **Design vs run**           | Keep **stance** coherent: design-time bindings are judged by design-time descriptions and their carriers; if you need run-time verification, express it as a **run-Status** or **run-Role** Template without confusing **stances** ([[00_A.07 - Strict Distinction (Clarity Lattice)]]). |
| **Fact vs promise**         | Evidence (KD‑CAL) vs deontic expectations (service, policy) must not collapse.       |

## [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:4 - Minimal vocabulary (this pattern only)

* **Context** — shorthand for **U.BoundedContext** (per [[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]).
* **SenseCell σ** — **address** **⟨Context C × Local‑Sense ℓ⟩** per [[00_F.03 - Intra-Context Sense Clustering|F.3]]. (Informative: we write simply **σ**; it already contains **C**.)
* **Role Description** — a **Role** or **Status** card anchored in a SenseCell ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]).
...
* **Eligibility** — conditions on the Holder that *must* hold to apply the Template ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] invariants).
...
## [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:5 - Pre‑conditions (lightweight)

1. The **Context** is in your [[00_F.01 - Domain-Family Landscape Survey|F.1]] cut; **Context ≡ U.BoundedContext**.
2. The **Template** references a **SenseCell** in that Context ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]).
3. The **Holder** is identified (by type or instance) without relying on Cross‑context mappings.

## [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:6 - The six moves (judgement schemas, notation‑free)
...
### [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:6.1 - M1 - Locate — *Fix the Context and the Template*
...
### [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:6.2 - M2 - Stance — *Respect DesignRunTag*
...
### [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:6.3 - M3 - Qualify — *Check Holder eligibility*

**Form.**
`Holder H ∧ eligibility(τ) holds in C ⊢ eligible(H, τ @ C)`

**Reading.** Given the Template’s eligibility predicates ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]), the Holder qualifies to be bound/assessed **in this Context**.
**Note.** Typical predicates: **type membership**, **capability present**, **scope fit**; all context‑local.

### [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:6.4 - M4 - Bind/Assert — *Make the Role Assignment / Status claim*
...
### [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:6.5 - M5 - Evidence — *Shape what would make it true/false*
...
### [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:6.6 - M6 - Conclude — *Issue a defensible verdict with confidence*

**Form.**
`evidence E fits Σ(C) ∧ invariants(τ) hold ⊢ holds(κ) with confidence γ ∈ [0,1]`

**Reading.** If observed facts match the expected evidence shape and Template invariants stand, the assignment/status claim **holds** with some confidence (cf. [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).
...
### [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:6.7 - Autonomy admission (Green‑Gate) and ledger
...
## [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:7 - Core invariants (normative)

1. **Locality.** Every judgement is **about one context**. No Cross‑context equivalence is presumed or implied (that is [[00_F.09 - Alignment & Bridge across Contexts|F.9]]’s remit).
2. **Strict splits.** (**a**) **senseFamily split:** **Role** ≠ **Status** (per [[00_F.00.01 - Contextual Lexicon Principles|F.0.1]]); (**b**) **stance split:** **design** ≠ **run** ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]). Each judgement names its **senseFamily** and **stance**.
...
6. **Name discipline.** Labels used in judgements follow [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] (Tech/Plain registers; no Context tags inside names).

## [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:8 - Reasoning aides (didactic)
...
## [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:9 - Anti‑patterns & remedies

| #         | Anti‑pattern                   | Symptom                                                                                     | Why it harms reasoning                                                     | Remedy (conceptual move)                                                                                                     |
| --------- | ------------------------------ | ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| **AP-1**  | **cross-context Binding**      | A single binding/assertion mixes two Contexts (status/role in C₁ justified by semantics in C₂) without an explicit Bridge. | Violates **Locality**; smuggles a Bridge (kind/CL/Loss) into the claim, making it hard to replay and easy to treat as free substitution. | Re-formulate strictly in one context. If cross-context support is essential, apply **[[00_F.09 - Alignment & Bridge across Contexts]] Bridge** and keep the assignment/status claim local. |
| **AP‑2**  | **Role and Status Conflation**     | “Operator” modeled as a deontic grant; “SLO met” modeled as a Role.                         | Collapses **behavioural mask** and **epistemic/deontic state** ([[00_A.07 - Strict Distinction (Clarity Lattice)]]).      | Re‑type the Template: **Role** for “can/does”, **Status** for “is/has (as a claim)”. Use **M4** accordingly.                 |
...
| **AP‑5**  | **Global Label Illusion**      | Using bare labels (“process”, “agent”, “role”) as if universal.                             | Hides the Context; fuels homonym errors.                                      | Always recover **M1 Locate**: `address(τ)=⟨Context, SenseCell⟩`. Use [[00_F.05 - Naming Discipline for U.Types & Roles]] naming discipline (Tech/Plain registers).              |
...
| **AP-8**  | **Premature Bridge**           | A Bridge is introduced as a shortcut (“align first, then claim”), instead of stating the assignment/status locally and adding a Bridge only when actually needed. | Makes the verdict hostage to an uncertain translation; Bridge Loss and CL penalties leak into the claim and can unnecessarily lower confidence. | Keep the assignment/status claim local; if needed, create an **[[00_F.09 - Alignment & Bridge across Contexts]] Bridge** with loss notes and CL penalty. |
...
| **AP‑12** | **Spec‑in‑Name**               | Cramming constraints into the label (“24x7‑Operator‑With‑Pager”).                           | Names become brittle; invariants become invisible.                         | Keep the label minimal ([[00_F.05 - Naming Discipline for U.Types & Roles]]); move constraints into **eligibility**/**invariants**.                                          |
| **AP‑13** | **Non‑Local Evidence Shape**   | Evidence shape mentions constructs from another Context.                                       | Hidden Cross‑context import.                                                  | Rewrite Σ using only this Context’s vocabulary; if impossible, use **[[00_F.09 - Alignment & Bridge across Contexts]]** Bridge and keep Σ local.                             |

## [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:10 - Worked examples

> Each example is a **context-local** assignment/status reasoning trace using **M1…M6**. cross-context relations, if any, are noted as *optional* bridges ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]) but not relied upon.

### [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:10.1 - Service availability status (ITIL + KD‑CAL)
...
*Optional bridge.* If uptime sensing vocabulary is expressed in **SOSA/SSN**, an **[[00_F.09 - Alignment & Bridge across Contexts|F.9]] Bridge** may map ITIL’s “availability metric” to **ObservableProperty(availability)** with a declared CL penalty; the assignment/status claim itself remains ITIL-local.

### [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:10.2 - Behavioural operator role (IEC 61131‑3 + Enactment)
...
*Trip-wire.* Do **not** restate this as “PROV Activity” without **[[00_F.09 - Alignment & Bridge across Contexts|F.9]]**; keep the assignment/status claim IEC-local.

### [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:10.3 - Dataset accuracy status (ISO/IEC 25024 + KD‑CAL)
...
### [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:10.4 - Access vs behavioural: two claims, two Contexts
...
## [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:11 - Relations (with other patterns)

**Builds on:**
[[00_F.01 - Domain-Family Landscape Survey|F.1]] **Domain‑Family Landscape Survey** (Contexts fixed); [[00_F.02 - Term Harvesting & Normalisation|F.2]] **Term Harvesting** (local terms); [[00_F.03 - Intra-Context Sense Clustering|F.3]] **Intra‑Context Clustering** (SenseCells); [[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] **Role Description** (invariants, stance); [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] **Naming Discipline** (labels).

**Constrains:**
**[[00_F.07 - Concept-Set Table|F.7]]** (Concept-Set Table): rows reference **SenseCells**; Role Description cards **point to** those rows but never **create** cross-context identity.
**[[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] Mint or Reuse?** Uses outcomes of **Role or Status** claims to decide: a new **Role or Status** label only when existing Templates cannot express the claim with eligibility/Window adjustments.
**[[00_F.09 - Alignment & Bridge across Contexts|F.9]]** (Alignment & Bridge): any relation across Contexts is **declared there**; Role Description cards remain context-local.
**[[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] Epistemic Status Mapping.** Consumes **M6** confidences γ and Σ‑adequacy to roll up assurance.
...
## [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:12 - Migration notes (conceptual)
...
3. **Name revisions.** Renaming per [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] does not alter **address(τ)=⟨Context, SenseCell⟩**; claims reference the address, not lexical form.
4. **Bridge introduction.** Adding an **[[00_F.09 - Alignment & Bridge across Contexts|F.9]] Bridge** never upgrades an existing Role or Status claim; at most it enables *separate* translations with declared loss.
...
## [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:13 - Acceptance tests (SCR/RSCR — concept‑level)

### [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:13.1 - Static conformance (SCR)

* **SCR-F6-S01 (Local address).** Every assignment/status claim states `address(τ)=σ` where `σ` is a **SenseCell** (per [[00_F.03 - Intra-Context Sense Clustering|F.3]]); no bare labels.
...
* **SCR‑F6‑S07 (Locality guard).** No Cross‑context terms appear inside a claim; any reference to other Contexts is flagged as **[[00_F.09 - Alignment & Bridge across Contexts|F.9]] Bridge (informative)**, not used to justify the claim.

### [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:13.2 - Regression (RSCR)

* **RSCR‑F6‑E01 (Edition stability).** Adding a new edition/Context does not mutate existing claims’ Contexts or Windows.
* **RSCR‑F6‑E02 (Name stability).** Changing labels per [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] leaves addresses and conclusions invariant.
* **RSCR‑F6‑E03 (Bridge neutrality).** Introducing or revising an **[[00_F.09 - Alignment & Bridge across Contexts|F.9]] Bridge** does not auto‑flip claim truth values; at most it enables explicit translations with loss notes.
...
## [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:14 - Didactic distillation (60‑second recap)
...
## [[00_F.06 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]:End
