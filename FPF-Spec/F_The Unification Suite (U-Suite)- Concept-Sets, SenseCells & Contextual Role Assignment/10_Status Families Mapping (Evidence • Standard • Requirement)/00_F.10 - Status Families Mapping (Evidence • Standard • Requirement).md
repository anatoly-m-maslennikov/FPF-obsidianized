...
**Builds on:** [[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **D.CTX** (Context ≡ `U.BoundedContext`); [[00_F.01 - Domain-Family Landscape Survey|F.1]] (Contexts), [[00_F.02 - Term Harvesting & Normalisation|F.2]] (Seeds), [[00_F.03 - Intra-Context Sense Clustering|F.3]] (Local‑Senses → SenseCells), [[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] (Role Description **Status** templates), [[00_F.09 - Alignment & Bridge across Contexts|F.9]] (Bridges).
**Coordinates with.** [[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] **Trust & Assurance Calculus** (interprets CL penalties); Part C patterns: **KD‑CAL** (measurement semantics), **Norm‑CAL** (deontic logic), **Method‑CAL** (DesignRunTag).

## [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:1 - Intent & applicability
...
Each status meaning is **local to a Context** (`U.BoundedContext`). Cross‑context relationships appear **only** via **Bridges** ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]) with a declared **kind** and **CL** (congruence level).
...
## [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:2 - Problem frame
...
## [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:3 - Forces
...
## [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:4 - Core idea (didactic)

**Three families, two modalities, one habit.**
Treat every status word as a **SenseCell with a declared StatusModality** and **inside one Context**. When you must relate statuses across Contexts, **declare a Bridge** ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]) that says *what kind of relation*, *which CL value applies*, *which way (if narrower/broader)*, and *what is lost*. Prefer **explanation** Bridges; permit **substitution** only when kind/CL allow it.

**Reading an Episteme.** For every `U.Episteme`, read the `EntityOfConcernSlot`, `ClaimGraphSlot`, `ReferenceSchemeSlot`, `ViewpointSlot`, and carrier references under [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] and MVPK. **Statuses classify the Episteme;** enactment remains with `U.System` and `U.Work`. (Formal identity rules: see **KD‑CAL**.)

## [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:5 - Minimal vocabulary (this pattern only)
...
* **StatusModality.** The mode of a StatusCell: **epistemic** or **deontic**. Use this term instead of the bare word *modality* per [[00_E.10 - Unified Lexical Rules for FPF|E.10]] LEX rules.
...
* **Bridge ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]).** The only admissible way to relate StatusCells across Contexts; declares **kind** (≈, ⊑, ⊒, ⋂, ⊥, or an Interpretation arrow), **CL**, and **Loss**; **substitution is modality‑preserving**.

> **StatusModality guard.** EvidenceStatus is **epistemic**; StandardStatus & RequirementStatus are **deontic**. **Role Description Status** templates ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]) bind to these **StatusModalities**; **no mixing**. The bare token *modality* is against E.10/LEX); this pattern uses **StatusModality**.

## [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:6 - The spine: three local ladders (Context‑native, small and renameable)
...
### [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:6.1 - EvidenceStatus (epistemic statusModality)
...
### [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:6.2 - StandardStatus (deontic/curatorial statusModality)
...
### [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:6.3 - RequirementStatus (deontic/compliance statusModality)
...
### [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:6.4 - Contextual Citation Operators (pointer)

**Citation operators (context‑scoped).** Authors MAY use the **typed edges** `supports`, `refutes`, `dependsOn`, `supersedes` **inside a single Context** when expressing how an `Evidence`/`Standard` status applies. **Formal semantics are governed by B.3.2 (Evidence & Validation Logic).** Cross‑Context use requires a declared **Bridge** ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]) and carries CL/Loss penalties.

## [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:7 - Solution — how meanings connect (conceptual, notation‑free)
...
Example: **Measured availability (SOSA)** →ᴍᴇᵃ **SLO clause (ITIL)** with **CL=2**, Loss: sampling window & clock skew. This supports **explanation**; **substitution** (“Satisfied”) requires **same StatusModality**, a stricter Bridge kind ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]) **and** a declared evaluation rule (from the Service pattern), not from [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]].

**S‑4. Keep DesignRunTag honest.**
**StandardStatus** is design‑stance; **EvidenceStatus** is run‑signal; **RequirementStatus** spans both. Use **Interpretation Bridges** ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]) for design↔run readings, not equivalence.

**S‑5. Prefer explanation over substitution.**
If a Bridge cannot reach **CL≥2** on the **same senseFamily**, do **not** substitute. Use **Naming‑only** rows or **explanations**; keep Role Descriptions ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]) out of harm’s way.

## [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:8 - Invariants (normative, lightweight)

1. **Modality purity.** A StatusCell’s **StatusModality** is explicit and **must not change** during reasoning; cross‑modality claims require an **Interpretation Bridge** ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]).
...
## [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:9 - Micro‑illustrations (snapshots, not procedures)

* **Metrology → Service.**
  *Observed uptime (SOSA)* with Window “July” plus Bridge **→ᴍᴇᵃ** to *SLO clause (ITIL)* yields: **we can explain** why “Satisfied” might hold **if** the Service pattern’s evaluation rule says so. [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] itself **does not** declare “Satisfied”.
...
## [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:10 - Anti‑patterns & remedies
...
## [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:11 - Worked examples

> Each example names **Contexts**, shows **StatusCells** on their native ladders, and draws **only the Bridges that [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] allows**.

### [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:11.1 - Service acceptance from run‑time evidence
...
### [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:11.2 - Safety controller: design approval vs run‑time duty
...
### [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:11.3 - ML model: validation vs fairness requirement
...
### [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:11.4 - Medical device log: refutation
...
## [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:12 - Reasoning primitives (judgement schemas, notation‑free)
...
8. **Substitution permission (guarded)**
   ...
   *Reading:* Substitution is allowed only above a **project‑declared threshold θ** (see [[00_F.07 - Concept-Set Table|F.7]]) and aligned Windows.
...
10. **Observation→Requirement clause (SOSA, Work outcomes)**
   ...
   *Reading:* Observations inform clause evaluation within a Window; they never become RequirementStatus. Use [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]] for the verdict pipeline.
...
## [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:13 - Relations

**Builds on:**
[[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **D.CTX** (Context discipline); [[00_F.01 - Domain-Family Landscape Survey|F.1]] (Contexts in view); [[00_F.02 - Term Harvesting & Normalisation|F.2]]–[[00_F.03 - Intra-Context Sense Clustering|F.3]] (Seeds→Local‑Senses→SenseCells); [[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] (Role Description **Status** template with statusModality/target/window slots); [[00_F.07 - Concept-Set Table|F.7]] (Bridge taxonomy & CL semantics); [[00_F.09 - Alignment & Bridge across Contexts|F.9]] (Bridge artefact).

**Constrains:**

* **[[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] (Role Description Status):** a Role Description Status **must** select a **StatusFamily**, **StatusModality**, **target kind**, and **Window**.
* **[[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] (Naming):** status labels reused across Contexts **must** be marked as **Context‑scoped**; global synonyms forbidden.
...
**Used by.**
Service Acceptance ([[00_F.12 - Service Acceptance-Work Evidence Link|F.12]]), Assurance roll‑ups ([[00_B.03 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]), any cross‑domain conformance narrative.

## [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:14 - Migration notes (conceptual)
...
3. **Threshold tuning.** The project sets **θ** (minimum CL for substitution). Lowering θ widens reuse but increases risk; document the choice in [[00_F.07 - Concept-Set Table|F.7]] terms.
...
## [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:15 - Acceptance tests (SCR/RSCR — concept‑level)

### [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:15.1 - Static conformance (SCR)
...
### [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:15.2 - Regression (RSCR)
...
## [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:16 - Didactic distillation (90‑second script)
...
## [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]:End
