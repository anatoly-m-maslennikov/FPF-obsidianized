...
**Depends on.** [[00_F.01 - Domain-Family Landscape Survey|F.1]] **Domain‑Family Landscape Survey**; [[00_F.02 - Term Harvesting & Normalisation|F.2]] **Term Harvesting & Normalisation**; [[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **Lexical Discipline for “Context” (D.CTX)**; [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] **Strict Distinction**; [[00_A.11 - Ontological Parsimony (C-5)|A.11]] **Ontological Parsimony**.
**Coordinates with.** [[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] **Role Description**; [[00_F.07 - Concept-Set Table|F.7]] **Concept‑Set Table**; [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] **Mint or Reuse Decision**; [[00_F.09 - Alignment & Bridge across Contexts|F.9]] **Alignment & Bridge Across Contexts**.
**Aliases (informative).** *context‑local clustering*; *Sense consolidation*.

## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:1 - Intent & applicability

**Intent.** Consolidate the **context‑local lexical units** from [[00_F.02 - Term Harvesting & Normalisation|F.2]] into a **small set of Local‑Senses** that actually operate in that **one context (U.BoundedContext)**. Each Local‑Sense receives a crisp, didactic label pair (Tech/Plain) and a short sense statement. The result is an **addressable basis** for later uses (Role Assignment, tables, bridges) that is **still strictly context‑local**.

**Applicability.** Apply **after** [[00_F.02 - Term Harvesting & Normalisation|F.2]] for any Context that will feed naming (F.4/F.5), decision gates ([[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]), Cross‑context bridges ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]), or exemplars in Part C. Use again whenever the canon (edition) **shifts usage** enough to split or merge senses **within the same context**.
...
## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:2 - Problem Frame

context‑local units (LNF + labels + gloss) from [[00_F.02 - Term Harvesting & Normalisation|F.2]] often **over‑ or under‑differentiate** meaning:
...
[[00_F.03 - Intra-Context Sense Clustering|F.3]] repairs this **inside the Context** by clustering “same sense” and distinguishing “different sense”, with **parsimony**.

## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:3 - Forces
...
## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:4 - Core idea (didactic)
...
## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:5 - Minimal vocabulary (this pattern only)

* **Context** — short for `U.BoundedContext` (per D.CTX).
* **Unit** — a context‑local lexical unit from [[00_F.02 - Term Harvesting & Normalisation|F.2]] (LNF + Tech/Plain + gloss).
...
## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:6 - Solution — how to think the clustering (notation‑free)
...
## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:7 - Outputs (conceptual, not clerical)

[[00_F.03 - Intra-Context Sense Clustering|F.3]] yields, **per Context**:
...
1. A **small set of Local‑Senses**, each with:
   ...
   * **Inside list** (informative): which Units from [[00_F.02 - Term Harvesting & Normalisation|F.2]] it consolidates.
...
## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:8 - Invariants (normative, lightweight)
...
## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:9 - Self‑checks (mental probes)
...
## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:10 - Anti‑patterns & remedies
...
| **A2**  | **Cross‑context creep**       | Folding BPMN *process* with PROV *activity* while clustering **inside** BPMN. | Imports foreign usage; violates locality.               | Constrain attention to **one context**; postpone Cross‑context talk to [[00_F.09 - Alignment & Bridge across Contexts]].                                             |
...
| **A8**  | **Edition blend**          | Mixing 2011 and 2020 usage under one Local‑Sense.                             | Hidden shifts; brittle bridges later.                   | If usage changed with edition, treat as **different Contexts** ([[00_F.01 - Domain-Family Landscape Survey]]) or distinct Local‑Senses with **edition note**. |
...
| **A10** | **Temporal fudge**         | Treating a design‑time sense as if it were run‑time (or vice versa).          | Category errors at enactment.                           | Respect the Context’s **time stance**; keep senses aligned to *design* or *run* as declared in [[00_F.01 - Domain-Family Landscape Survey]].                  |

## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:11 - Local‑Sense Cards (one‑glance form)
...
* **Inside** — which [[00_F.02 - Term Harvesting & Normalisation|F.2]] Units it consolidates (names only)
* **Counter‑example** — a short use that must **not** be included

## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:12 - Worked examples (all **intra‑Context**)

### [[00_F.03 - Intra-Context Sense Clustering|F.3]]:12.1 - BPMN 2.0 (workflow Context)
...
### [[00_F.03 - Intra-Context Sense Clustering|F.3]]:12.2 - PROV‑O (provenance Context)
...
### [[00_F.03 - Intra-Context Sense Clustering|F.3]]:12.3 - ITIL 4 (services Context)
...
### [[00_F.03 - Intra-Context Sense Clustering|F.3]]:12.4 - SOSA/SSN (sensing Context)
...
### [[00_F.03 - Intra-Context Sense Clustering|F.3]]:12.5 - OWL 2 (types Context)
...
### [[00_F.03 - Intra-Context Sense Clustering|F.3]]:12.6 - IEC 61131‑3 (control‑runtime Context)
...
## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:13 - Reasoning primitives (judgement schemas, notation‑free)
...
## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:14 - Relations

**Builds on:**
[[00_F.01 - Domain-Family Landscape Survey|F.1]] **Domain‑Family Landscape Survey** (Contexts fixed); [[00_F.02 - Term Harvesting & Normalisation|F.2]] **Term Harvesting** (Units ready); [[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **D.CTX** (Context discipline); [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] **Strict Distinction**.

**Constrains:**

* **[[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] Role Description.** Role Descriptions **cite SenseCells**; they do **not** invent senses.
* **[[00_F.07 - Concept-Set Table|F.7]] Concept‑Set Table.** Rows are built from **SenseCells** (later Cross‑context assembly); intra‑Context clarity here prevents row bloat.
* **[[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] Mint or Reuse Decision.** Decisions compare proposed names to **existing SenseCells** to avoid type inflation.
* **[[00_F.09 - Alignment & Bridge across Contexts|F.9]] Alignment & Bridge.** Bridges connect **SenseCell ↔ SenseCell** across Contexts; [[00_F.03 - Intra-Context Sense Clustering|F.3]] provides the stable endpoints.

**Is used by.**
Part C Extention Patterns to ground examples and invariants in **Context‑true** language.

## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:15 - Migration notes (conceptual)
...
3. **Edition change → new Context.** When a new edition **reframes** usage, treat it as a **separate Context** ([[00_F.01 - Domain-Family Landscape Survey|F.1]]) and re‑cluster there.
...
6. **Bridge temptation.** Record tensions to bridge **elsewhere**; [[00_F.03 - Intra-Context Sense Clustering|F.3]] never resolves Cross‑context relations.

## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:16 - Acceptance tests (SCR/RSCR — concept‑level)

### [[00_F.03 - Intra-Context Sense Clustering|F.3]]:16.1 - Static conformance (SCR)
...
### [[00_F.03 - Intra-Context Sense Clustering|F.3]]:16.2 - Regression (RSCR)
...
## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:17 - Didactic close (60‑second recap)

> **Within one context,** collect how the canon actually **uses** a head, not how we wish it did. **Merge** aliases that never lead to different conclusions; **split** uses that do. Give each consolidated use a crisp **Tech** label in the Context’s idiom and a faithful **Plain** label. The pair *(Context, Local-Sense)* is your **SenseCell**—the address later cited by Role Descriptions, tables, and bridges. No Cross‑context mergers here; that job belongs to [[00_F.09 - Alignment & Bridge across Contexts|F.9]]. Keep senses few, boundaries sharp, and labels honest.

## [[00_F.03 - Intra-Context Sense Clustering|F.3]]:End
