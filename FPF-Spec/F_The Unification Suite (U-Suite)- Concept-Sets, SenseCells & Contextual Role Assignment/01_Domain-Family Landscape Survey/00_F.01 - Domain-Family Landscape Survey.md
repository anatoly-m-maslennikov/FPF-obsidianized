...
**Depends on.** [[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **Lexical Discipline for “Context” (D.CTX)**; **[[00_F.00.01 - Contextual Lexicon Principles|F.0.1]] Contextual Lexicon Principles**; [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] **Strict Distinction (Clarity Lattice)**; [[00_A.11 - Ontological Parsimony (C-5)|A.11]] **Ontological Parsimony**.
**Coordinates with.** [[00_F.02 - Term Harvesting & Normalisation|F.2]] **Term Harvesting & Normalisation**; [[00_F.03 - Intra-Context Sense Clustering|F.3]] **Intra‑Context Sense Clustering**; [[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] **Role Description**; [[00_F.09 - Alignment & Bridge across Contexts|F.9]] **Alignment & Bridge Across Contexts**; **[[00_G.0 - Frame Standard and Comparability Governance - CG-Spec|G.0]]–[[00_G.01 - CG-Frame-Ready Generator|G.1]]** *(Scope/entityOfConcern handoff)*.  *(Bridges live only in [[00_F.09 - Alignment & Bridge across Contexts|F.9]].)*

**Aliases (informative).** *Contexts‑first survey*; *Context cut*.

## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:1 - Intent & applicability
...
## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:2 - Problem frame
...
## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:3 - Forces
...
## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:4 - Core idea (didactic)

**Think in Contexts, not in words.**
A *Context of meaning* is a **U.BoundedContext** (per D.CTX) that encloses a coherent vocabulary and its rules from a **specific, citable canon** (standard, BoK, seminal paper, textbook tradition). You **name and reason** *inside the Context*. When you must step between Contexts, you will **declare a bridge later** ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]) with explicit losses or mismatches.

## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:5 - Minimal vocabulary (this pattern only)
...
* **SenseCell** *(appears downstream)*. A **(Context × Local‑Sense)** address; [[00_F.03 - Intra-Context Sense Clustering|F.3]] will mint these after clustering. Mentioned here only to keep the destination in view.

## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:6 - Solution — the Contexts‑first survey (conceptual, notation‑free)
...
If two Contexts seem “close”, **resist** collapsing. Note the tension and leave any bridge claim to **[[00_F.09 - Alignment & Bridge across Contexts|F.9]] Alignment & Bridge**.

## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:7 - What to record (conceptual, not clerical)
...
## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:8 - Invariants (normative, lightweight)

1. **Context ≡ U.BoundedContext.** In this pattern, *Context* always means **U.BoundedContext** (per [[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]).
...
5. **No bridging here.** No equivalence or mapping is asserted between Contexts in [[00_F.01 - Domain-Family Landscape Survey|F.1]]. (Bridges live in **[[00_F.09 - Alignment & Bridge across Contexts|F.9]]**.)
...
## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:9 - Self‑checks (mental, not procedural)
...
* **The bridge itch.** Feel the temptation to say “these are the same”? Good. **Write the itch down** and refuse to scratch it here. That’s [[00_F.09 - Alignment & Bridge across Contexts|F.9]]’s job.
* **The memory rule.** If your entire survey cannot be recalled **without opening a document**, it is too large.

## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:10 - Micro‑examples (illustrative only)
...
> With only these Contexts fixed, later steps become almost mechanical: [[00_F.02 - Term Harvesting & Normalisation|F.2]] harvests terms **inside** each Context; [[00_F.03 - Intra-Context Sense Clustering|F.3]] clusters **within** each Context; [[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] names roles or statuses pointing to **SenseCells**; [[00_F.09 - Alignment & Bridge across Contexts|F.9]] draws the bridges you refused to draw here.

## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:11 - Anti‑patterns & remedies
...
| **A5**  | **Bridge‑by‑stealth**      | Phrases like “these are basically the same” inside the survey.                       | Hides losses; imports meaning across Contexts without scrutiny.   | **No bridging here**; write the *itch to bridge* down as an **[[00_F.09 - Alignment & Bridge across Contexts]]** bridge question (§6 Step 6, §8‑5).                           |
...
| **A9**  | **Context inheritance**    | Arranging Contexts in is‑a hierarchies (“PROV is‑a BPMN”).                              | Suggests meaning flows by inheritance; erases locality.        | **No is‑a among Contexts**; relations between Contexts live in **[[00_F.09 - Alignment & Bridge across Contexts]] bridges** (§8‑5).                                        |
...
| **A11** | **Family‑based inference** | Treating Domain‑family membership as implying similarity/equivalence. | Smuggles semantics via shelf labels; breaks locality. | **Domain family is informative only**; locality and any Cross‑context relation must be explicit ([[00_F.09 - Alignment & Bridge across Contexts]]). |

## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:12 - Worked examples
...
### [[00_F.01 - Domain-Family Landscape Survey|F.1]]:12.1 Enactment (`U.RoleAssignment` + `U.RoleEnactment`) with sensing & execution (service acceptance)
...
* You resist equating **PROV Activity** with **BPMN workflow**; later **[[00_F.09 - Alignment & Bridge across Contexts|F.9]]** may relate them with explicit loss.

### [[00_F.01 - Domain-Family Landscape Survey|F.1]]:12.2 Method quartet with types & measurement (model state graph)
...
* You avoid treating **FCA “concept”** as a **U.Type**; later [[00_F.09 - Alignment & Bridge across Contexts|F.9]] can bridge OWL classes to FCA concepts with cautions.

### [[00_F.01 - Domain-Family Landscape Survey|F.1]]:12.3 Control & actuation with services (operational SLOs in plants)
...
## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:13 - Reasoning primitives (judgement schemas, notation‑free)
...
7. **Bridge embargo**
   ...
   *Reading:* [[00_F.01 - Domain-Family Landscape Survey|F.1]] never asserts equivalence across Contexts; postponement is principled, not procrastination.
...
## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:14 - F1‑Card example (informative)
```
F1-Card v2025‑Q3:
...
## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:15 - Relations (with other patterns)

**Builds on:**
[[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **Lexical Discipline for “Context” (D.CTX)** — ensures *Context* ≡ *U.BoundedContext* and reserves “Problem Frame” for narrative use.
[[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] **Strict Distinction** — guards EntityOfConcern/Description-episteme/publication-carrier and DesignRunTag splits while you cut Contexts.
[[00_A.11 - Ontological Parsimony (C-5)|A.11]] **Ontological Parsimony** — motivates the small cut.

**Constrains:**
**[[00_F.02 - Term Harvesting & Normalisation|F.2]]** (Term Harvesting): harvest **inside** Contexts named here; every occurrence carries a Context name.
**[[00_F.03 - Intra-Context Sense Clustering|F.3]]** (Intra‑Context Sense Clustering): cluster **per Context**; no Cross‑context sense claims.
**[[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]** (Role Descriptions): any role template or status template must cite a **SenseCell** that lives in a Context from this cut.
**[[00_F.09 - Alignment & Bridge across Contexts|F.9]]** (Alignment & Bridge): only [[00_F.09 - Alignment & Bridge across Contexts|F.9]] may relate Contexts; never [[00_F.01 - Domain-Family Landscape Survey|F.1]]–[[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]].
...
## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:16 - Migration notes (conceptual)
...
5. **Bridges discovered later.** Do not back‑port bridges into [[00_F.01 - Domain-Family Landscape Survey|F.1]]; leave the Cards untouched and record the mapping in **[[00_F.09 - Alignment & Bridge across Contexts|F.9]]**.
...
## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:17 - Acceptance tests (SCR/RSCR — concept‑level)

### [[00_F.01 - Domain-Family Landscape Survey|F.1]]:17.1 - Static conformance checks (SCR)
...
* **SCR‑F1‑S03 (Locality pledge).** Nowhere in [[00_F.01 - Domain-Family Landscape Survey|F.1]] are Cross‑context equivalences or merges asserted.
...
* **SCR‑F1‑S07 (Family neutrality).** No claim, classification, or relation in [[00_F.01 - Domain-Family Landscape Survey|F.1]] relies on Domain‑family membership; families appear only as shelf labels on cards.
...
### [[00_F.01 - Domain-Family Landscape Survey|F.1]]:17.2 - Regression checks (RSCR)
...
## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:18 - Didactic distillation (90‑second teaching script)

> “Before you name anything, **fix the context of meaning**. A *Context* is a **U.BoundedContext** tied to a specific canon—*BPMN 2.0*, *PROV‑O*, *ITIL 4*, *SOSA/SSN*, *IEC 61131‑3*, *OWL 2*. Words are **local to Contexts**: *process (BPMN)* is a workflow graph, *activity (PROV)* is a run‑time occurrence, *service (ITIL)* is a promise vocabulary. Cut the landscape so each unification line sees **at least three domain families**, with **one‑screen Cards** per Context (scope gist, time stance, trip‑wires). **Do not bridge** Contexts here—just write down the itch to bridge as an **[[00_F.09 - Alignment & Bridge across Contexts|F.9]]** bridge question. Keep the cut **small enough to remember**. With Contexts fixed, harvesting ([[00_F.02 - Term Harvesting & Normalisation|F.2]]), local clustering ([[00_F.03 - Intra-Context Sense Clustering|F.3]]), role template or status templates ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]), and explicit Cross‑context bridges ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]) become straightforward—and you avoid naming ghosts that come from words floating without walls.”

## [[00_F.01 - Domain-Family Landscape Survey|F.1]]:End
