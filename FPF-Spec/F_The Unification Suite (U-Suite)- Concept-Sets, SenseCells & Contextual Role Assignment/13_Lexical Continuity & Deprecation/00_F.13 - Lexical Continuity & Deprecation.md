...
**Builds on:** [[00_F.01 - Domain-Family Landscape Survey|F.1]] **context of meaning**; [[00_F.02 - Term Harvesting & Normalisation|F.2]] **Term Harvesting**; [[00_F.03 - Intra-Context Sense Clustering|F.3]] **Intra‑Context Clustering (SenseCell)**; [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] **Naming Discipline**; [[00_F.07 - Concept-Set Table|F.7]] **Concept‑Set (row) construction**; [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] **Mint‑or‑Reuse decision**; [[00_F.09 - Alignment & Bridge across Contexts|F.9]] **Bridges**; [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] **Status windows**.
...
## [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:1 - Intent & applicability
...
## [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:2 - Problem frame
...
## [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:3 - Forces
...
## [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:4 - Core idea (didactic)
...
> A label (including aliases) always belongs to **one context** or to **one Concept‑Set row**. Cross‑context similarity is handled by **Bridges** ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]), never by shared names.

## [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:5 - Minimal vocabulary (this pattern only)

* **Legacy label** — a previously used label in the same Context (or same Concept‑Set row / Role Description).
* **Preferred label** — the current **[[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]‑conformant** label for that item.
...
## [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:6 - Solution — Continuity, not “registries”
...
### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:6.1 - Continuity relations (normative meanings)

1. **`renames(label_old → label_new)`** — *wording improved, sense unchanged*.
   *Use when:* Same **SenseCell** / same **Concept‑Set row** / same **Role Description**; only the lexical form changed to satisfy [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] (morphology, disambiguation, plain/tech harmony).
...
3. **`splits(label_old ⇒ {label_A, label_B})`** — *one label covered multiple senses; now separated*.
   *Use when:* Your **SenseCell** was really two local senses; [[00_F.03 - Intra-Context Sense Clustering|F.3]] has **split** them; or a **Concept‑Set row** is refactored into two rows.
...
4. **`merges({label_A, label_B} ⇒ label_new)`** — *two labels now recognized as one sense*.
   *Use when:* [[00_F.03 - Intra-Context Sense Clustering|F.3]] shows **same SenseCell**; or two Concept‑Set rows collapse after [[00_F.09 - Alignment & Bridge across Contexts|F.9]] raised CL sufficiently.
...
> **Important:** All five relations are **context‑local** (SenseCell level) or **row‑local** (Concept‑Set). **Never** use them to “alias” across Contexts. If a change crosses Contexts, it is not a rename; it requires a **Bridge** ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]) and often a **split/merge of rows** ([[00_F.07 - Concept-Set Table|F.7]]).

## [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:7 - Invariants (normative)
...
5. **Prefer present for writing.** In normative writing, use the **current preferred label** ([[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]). Aliases are for **reading comprehension**.
6. **Bridge discipline.** If a label shift would require crossing Contexts to “explain”, it is **not a rename**; use **[[00_F.09 - Alignment & Bridge across Contexts|F.9]] Bridge** and, if needed, refactor the **Concept‑Set row(s)**.
7. **Epoch honesty.** When declaring continuity, attach a **succinct epoch note** (“pre‑2023 usage”) if it aids readers.

## [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:8 - Self‑checks (mental, not procedural)
...
## [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:9 - Micro‑examples (illustrative)

### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:9.1 - Pure rename inside a Context (ITIL → clearer plain label)
...
**Why:** [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] morphology & expansion; SenseCell unchanged (same clause semantics).
**Effect:** Old guidance remains readable; new writing spells out the term.

### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:9.2 - Alias for a common legacy synonym (Sys‑CAL)
...
### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:9.3 - Split of a muddled local sense (Enactment)
...
### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:9.4 - Merge after clustering raised confidence (Kind-CAL row)

Two Concept‑Set rows **{“DBaaS”, “Database‑Service”}** converge after [[00_F.03 - Intra-Context Sense Clustering|F.3]] within the same context profile and [[00_F.09 - Alignment & Bridge across Contexts|F.9]] raised CL.
**Relation:** `merges({"DBaaS","Database‑Service"} ⇒ "Database‑Service")`.
**Effect:** “DBaaS” becomes a legacy alias with an epoch note.

### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:9.5 - Not a rename: Cross‑context temptation (forbidden)
...
**Action:** **No continuity relation.** Keep labels; if needed, declare a **Bridge** ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]) explaining design→run mapping with CL/Loss.

## [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:10 - Anti‑patterns & remedies

| #       | Anti‑pattern               | Symptom in texts                                                      | Why it harms thinking                                          | Remedy (conceptual move)                                                                                                         |
| ------- | -------------------------- | --------------------------------------------------------------------- | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| **A1**  | **Cross‑context rename**      | “Let’s rename *process (BPMN)* to *activity (PROV)*.”                 | Erases Context boundaries; hides loss; violates locality.         | **Do not rename across Contexts.** Keep both labels; if you must relate them, declare a **Bridge** ([[00_F.09 - Alignment & Bridge across Contexts]]) with CL/loss.              |
...
| **A4**  | **Paint‑over rename**      | Rename used where sense actually changed.                             | Confuses continuity with revision; hides splits.               | Use **`splits`** (or **`merges`**), not `renames`. If Contexts diverge, adjust **rows** ([[00_F.07 - Concept-Set Table]]) and **Bridges** ([[00_F.09 - Alignment & Bridge across Contexts]]).                |
...
| **A8**  | **Row drift via rename**   | Concept‑Set row is relabeled while its membership silently changes.   | Hides that the set changed; breaks Cross‑context alignment.       | First **split/merge rows** ([[00_F.07 - Concept-Set Table]]) as needed; only then `renames` the row **if** its intension stayed.                             |
| **A9**  | **Bridge‑by‑alias**        | Using an alias to hint two Contexts are “the same.”                      | Smuggles translation without CL/loss.                          | **No Cross‑context aliasing.** If similarity matters, **Bridge** explicitly ([[00_F.09 - Alignment & Bridge across Contexts]]) and keep labels separate.                         |
| **A10** | **Acronym absolutism**     | Treating acronyms as preferred labels everywhere (“SLO” in any Context). | Obscures Context‑specific senses; hurts didactics.                | Prefer **expanded** labels as preferred ([[00_F.05 - Naming Discipline for U.Types & Roles]]); keep acronym as context‑local **alias** only where historically dominant.            |
| **A11** | **Temporal fudge**         | Rename used to imply design↔run shift (“execution ≈ process”).        | Conflates time stances; erases important dualities.            | Keep **DesignRunTag** explicit on labels or glosses; if mapping is needed, do so in **[[00_F.09 - Alignment & Bridge across Contexts]]**.                                       |
...
## [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:11 - Extended examples

### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:11.1 - KD‑CAL × Services — *metric target* labels over time
...
* **Why:** Same local sense; clearer morphology for [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]]; SOSA/SSN labels untouched.
...
### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:11.2 - Sys‑CAL × LCA‑CAL — separating *execution* vs *actuation*
...
### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:11.3 - Kind-CAL × Method‑CAL — false merge avoided
...
### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:11.4 - Enactment × KD‑CAL — retiring a misleading metaphor
...
### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:11.5 - Concept‑Set row refactor after rising CL

* **Rows:** `{“DBaaS”, “Database‑Service”}` representing service notions across several Contexts.
* **[[00_F.03 - Intra-Context Sense Clustering|F.3]] + [[00_F.09 - Alignment & Bridge across Contexts|F.9]] outcome:** High CL; evidence of same Cross‑context alignment.
...
## [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:12 - Reasoning primitives (judgement schemas, notation‑free)
...
### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:12.1 - Same‑sense & same‑place
...
### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:12.2 -Local alias
...
### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:12.3 - Split detection
...
### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:12.4 - Merge admission

`ContextOf(ℓA)=ContextOf(ℓB) ∧ senseOf(ℓA)=senseOf(ℓB) ⊢ merges({ℓA,ℓB} ⇒ ℓN)`
*Reading:* Once [[00_F.03 - Intra-Context Sense Clustering|F.3]] shows identity of sense **within** a Context, merging labels into one preferred label is safe.

### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:12.5 - Retirement
...
### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:12.6 - Cross‑context guard

`ContextOf(ℓ₁) ≠ ContextOf(ℓ₂) ⊢ ¬mayRename(ℓ₁→ℓ₂)`
*Reading:* Different Contexts forbid rename/alias; any relation goes to **Bridge** ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]).

### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:12.7 - Writing discipline

`thing t ⊢ writeWithPreferred(t) = pref(t)`
*Reading:* Normative prose uses the **current** preferred label; aliases are for reading.

### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:12.8 - Reading resolution
...
### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:12.9 - Alias budget

`aliasesFor(thing, register=r) = A ⊢ |A| ≤ 1`
*Reading:* Keep at most one legacy alias per register (Tech/Plain) for any one thing.

### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:12.10 - Row‑level continuity

`rowOf(ℓA)=rowOf(ℓB)=R ∧ intension(R) stable ⊢ mayRenameRow(R,ℓB)`
*Reading:* A row label can change if the **row’s membership/intension** did not change; otherwise refactor rows first ([[00_F.07 - Concept-Set Table|F.7]]).

## [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:13 - Relations

**Builds on:**
[[00_F.01 - Domain-Family Landscape Survey|F.1]] **context of meaning** (keeps locality), [[00_F.02 - Term Harvesting & Normalisation|F.2]] **Harvesting** (provides attested strings), [[00_F.03 - Intra-Context Sense Clustering|F.3]] **Clustering** (establishes SenseCells), [[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] **Naming Discipline** (supplies preferred labels), [[00_F.07 - Concept-Set Table|F.7]] **Concept‑Set rows**, [[00_F.08 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] **Mint‑or‑Reuse**, [[00_F.09 - Alignment & Bridge across Contexts|F.9]] **Bridges**, [[00_F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] **Status windows**, [[00_F.11 - Method Quartet Harmonisation|F.11]] **Method harmonisation**, [[00_F.12 - Service Acceptance-Work Evidence Link|F.12]] **Service acceptance**.

**Constrains:**

* **[[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] (Naming):** may select preferred labels **only** after applying these continuity relations.
* **[[00_F.07 - Concept-Set Table|F.7]] (Rows):** row relabels require row **intension** stability; otherwise use **split/merge rows**.
* **[[00_F.09 - Alignment & Bridge across Contexts|F.9]] (Bridges):** Cross‑context changes must **not** be expressed as renames/aliases.

**Used by.**
All Part C patterns when editions shift; all examples and tutorials when teaching with legacy terminology.

## [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:14 - Migration notes (conceptual playbook)
...
2. **Keep it inside the Context.** If your explanation crosses Contexts, stop—this is **Bridge** territory ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]), not a rename.
3. **Prefer clarity over fashion.** Rename only when the new label **removes a real ambiguity** ([[00_F.05 - Naming Discipline for U.Types & Roles|F.5]] criteria), not to chase style.
...
6. **Rows before names.** If a rename request coincides with a shift in what the row covers, **refactor rows** ([[00_F.07 - Concept-Set Table|F.7]]) first, then choose labels.
...
## [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:15 - Acceptance tests (SCR/RSCR — concept‑level)

### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:15.1 - Static conformance (SCR)
...
* **SCR‑F13‑S05 (Row integrity).** A row rename occurs only when the row’s **intension** is stable; if membership changed, a **row split/merge** is documented ([[00_F.07 - Concept-Set Table|F.7]]).
* **SCR‑F13‑S06 (Bridge discipline).** No alias/rename is used to imply Cross‑context sameness; any such relation belongs under **[[00_F.09 - Alignment & Bridge across Contexts|F.9]]**.

### [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:15.2 - Regression (RSCR)
...
## [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:16 - Didactic distillation (60‑second script)
...
## [[00_F.13 - Lexical Continuity & Deprecation|F.13]]:End
