...
**Depends on.** [[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **Lexical Discipline for “Context” (D.CTX)**; **[[00_F.00.01 - Contextual Lexicon Principles|F.0.1]] Contextual Lexicon Principles** (Source - Local Meaning - Bridge‑Only Crossing); [[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]] **Strict Distinction**; [[00_A.11 - Ontological Parsimony (C-5)|A.11]] **Ontological Parsimony**.
**Coordinates with.** [[00_F.01 - Domain-Family Landscape Survey|F.1]] **Context Map via Context Cards**; [[00_F.03 - Intra-Context Sense Clustering|F.3]] **Intra‑Context Sense Clustering**; [[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] **Role Description**; [[00_F.09 - Alignment & Bridge across Contexts|F.9]] **Alignment & Bridge Across Contexts**.
**Aliases (informative).** *context‑local harvesting*; *Local normalisation*.

## [[00_F.02 - Term Harvesting & Normalisation|F.2]]:1 - Intent & applicability
...
**Applicability.** Use whenever a unification line (from [[00_F.01 - Domain-Family Landscape Survey|F.1]]) needs **actual words** to be referenced by patterns in Part C (Extention patterns) or by Role Descriptions ([[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]). Re‑enter [[00_F.02 - Term Harvesting & Normalisation|F.2]] when a canon/edition changes or when a new Context is admitted in [[00_F.01 - Domain-Family Landscape Survey|F.1]].

**Non‑goals.** No global labels; no Cross‑context equivalence; no workflow or role descriptions; no storage/API talk. [[00_F.02 - Term Harvesting & Normalisation|F.2]] specifies **how to think**, not how to “run a pipeline”.

## [[00_F.02 - Term Harvesting & Normalisation|F.2]]:2 - Problem Frame

Even with Contexts fixed ([[00_F.01 - Domain-Family Landscape Survey|F.1]]), three mistakes recur:
...
[[00_F.02 - Term Harvesting & Normalisation|F.2]] prevents these by **localising** meaning and **naming** strictly **inside** each Context.

## [[00_F.02 - Term Harvesting & Normalisation|F.2]]:3 - Forces
...
| **Speed vs safety**        | Move fast to enable F.3/F.4 vs avoid any Cross‑context conclusion in [[00_F.02 - Term Harvesting & Normalisation]].           |

## [[00_F.02 - Term Harvesting & Normalisation|F.2]]:4 - Core idea (didactic)

**Harvest *inside* each Context; name *in that Context’s idiom*; do not cross Contexts.**
For every Context (a **U.BoundedContext** from [[00_F.01 - Domain-Family Landscape Survey|F.1]]), you gather **attested phrases** as *thought‑cues*, choose a **Local Normal Form (LNF)** that matches the Context’s idiom, attach a **two‑register label** (Tech/Plain), and write a **one‑sentence gloss**. That’s all. You do **not** claim sameness with any other Context; you do **not** embed behaviour or deontics; you do **not** mint U.Types here. These *local lexical units* will become **Local‑Senses** in [[00_F.03 - Intra-Context Sense Clustering|F.3]] and later addressable **SenseCells** (Context × Local‑Sense).

## [[00_F.02 - Term Harvesting & Normalisation|F.2]]:5 - Minimal vocabulary (this pattern only)

* **Context** — Tech‑register alias for **U.BoundedContext** (per [[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]).
...
* **Local lexical unit** — The quintet *(Context, LNF, Tech, Plain, Gloss)*. This is [[00_F.02 - Term Harvesting & Normalisation|F.2]]’s only outcome.
* **Homonymy (signal)** — Awareness that the **same string** has **different local lexical units** across Contexts (no relation asserted).
* **SenseCell** *(appears downstream)* — Address **(Context × Local‑Sense)** minted in [[00_F.03 - Intra-Context Sense Clustering|F.3]]; mentioned here so you know what you’re preparing.

> *Everything above is a way of thinking. None of it implies a database, statuses, or roles.*

## [[00_F.02 - Term Harvesting & Normalisation|F.2]]:6 - Solution — three mental moves (notation‑free)

### [[00_F.02 - Term Harvesting & Normalisation|F.2]]:6.1 - Move A — **Localise the word**

**Question to ask.** *“In which Context am I hearing this word?”*
**Action (mental).** Point to a specific **Context** (from [[00_F.01 - Domain-Family Landscape Survey|F.1]]). Grab 1–2 **attested phrases** that are representative **in this Context**.
...
### [[00_F.02 - Term Harvesting & Normalisation|F.2]]:6.2 -Move B — **Name it in the Context’s idiom**
...
• The **Gloss** must fit on a single line; put details in [[00_F.03 - Intra-Context Sense Clustering|F.3]].

### [[00_F.02 - Term Harvesting & Normalisation|F.2]]:6.3 - Move C — **Fence it off**
...
**Outcome.** A clean, **context‑local** lexical unit that will be safe to cluster in [[00_F.03 - Intra-Context Sense Clustering|F.3]] and safe to bridge (or not) in [[00_F.09 - Alignment & Bridge across Contexts|F.9]].

## [[00_F.02 - Term Harvesting & Normalisation|F.2]]:7 - Guard‑rails (normative, lightweight)

1. **context‑locality.** Every local lexical unit **MUST** cite a Context (U.BoundedContext from [[00_F.01 - Domain-Family Landscape Survey|F.1]]).
...
5. **EntityOfConcern / Description / specification-use hygiene ([[00_A.07 - Strict Distinction (Clarity Lattice)|A.7]]).** **MUST NOT** include behaviour equations, deontic rules, measurement math, or type axioms; those belong to patterns.
6. **No Cross‑context claims.** **MUST NOT** assert equivalence, subsumption, or similarity with terms in other Contexts ([[00_F.09 - Alignment & Bridge across Contexts|F.9]] only).
7. **Edition honesty.** If the Context’s canon has multiple editions with shifting usage, treat them as distinct Contexts in [[00_F.01 - Domain-Family Landscape Survey|F.1]] before harvesting.
8. **Parsimony.** Prefer **few, telling** lexical units over long tails; keep head terms that will power F.3/F.4/F.9.

## [[00_F.02 - Term Harvesting & Normalisation|F.2]]:8 - Micro‑examples (illustrative, context‑local)
...
## [[00_F.02 - Term Harvesting & Normalisation|F.2]]:9 - Didactic heuristics (informative)
...
## [[00_F.02 - Term Harvesting & Normalisation|F.2]]:10 - Anti‑patterns & remedies

| #       | Anti‑pattern                 | Symptom (in thought or prose)                                        | Why harmful                                                  | Remedy (conceptual move)                                                                                |
| ------- | ---------------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- |
| **A1**  | **Global normal form**       | One “canonical” label reused across Contexts.                           | Erases local meaning; invites stealth bridges.               | Keep **LNF per Context**; any Cross‑context relation belongs to **[[00_F.09 - Alignment & Bridge across Contexts]]** only.                                 |
...
| **A5**  | **Premature structure**      | Embedding behaviour, deontics, units, or type axioms into the gloss. | EntityOfConcern / Description / specification-use mixing (violates [[00_A.07 - Strict Distinction (Clarity Lattice)]]); biases later patterns.          | Gloss **usage**, not calculus; structural content belongs to Extention Patterns in Part C.                   |
| **A6**  | **Cross‑context folding**       | “BPMN workflow ≈ PROV activity” written inside [[00_F.02 - Term Harvesting & Normalisation]].                   | Hidden bridge; unpriced losses.                              | No Cross‑context claims in [[00_F.02 - Term Harvesting & Normalisation]]; write the **itch to bridge** for **[[00_F.09 - Alignment & Bridge across Contexts]]**.                                  |
| **A7**  | **Edition blur**             | “BPMN” without year/profile; mixing excerpts across editions.        | Silent sense shift; unrepeatable reasoning.                  | Treat distinct editions as **distinct Contexts** in [[00_F.01 - Domain-Family Landscape Survey]], then harvest.                                     |
| **A8**  | **Vendor‑dialect elevation** | Treating a DSL/keyword list as “the domain”.                         | Projectionism; narrow idiom dominates.                       | If needed, model the DSL as **one context among others**; keep heterogeneity from [[00_F.01 - Domain-Family Landscape Survey]].                     |
...
| **A11** | **Temporal fudge**           | Using run‑time words in design Contexts (or vice versa).                | Category drift; later contradictions.                        | Respect the Context’s **DesignRunTag** from its Card ([[00_F.01 - Domain-Family Landscape Survey]] §7.2).                                      |
...
## [[00_F.02 - Term Harvesting & Normalisation|F.2]]:11 - Worked examples (context‑local only)

> Each line is a **local lexical unit** *(Context, LNF, Tech, Plain, Gloss)*.
> No Cross‑context relation is implied. Later clustering ([[00_F.03 - Intra-Context Sense Clustering|F.3]]) and bridges ([[00_F.09 - Alignment & Bridge across Contexts|F.9]]) may connect them.

### [[00_F.02 - Term Harvesting & Normalisation|F.2]]:11.1 Enactment + sensing
...
### [[00_F.02 - Term Harvesting & Normalisation|F.2]]:11.2 Sys‑CAL / LCA‑CAL + services
...
### [[00_F.02 - Term Harvesting & Normalisation|F.2]]:11.3 Kind-CAL + Method‑CAL + KD‑CAL
...
## [[00_F.02 - Term Harvesting & Normalisation|F.2]]:12 - Reasoning primitives (judgement schemas, notation‑free)

> Read each as a **permitted mental move** over the outcomes of [[00_F.02 - Term Harvesting & Normalisation|F.2]].
...
8. **No Cross‑context conclusion**
   ...
   *[[00_F.02 - Term Harvesting & Normalisation|F.2]] never asserts Cross‑context equivalence.*

9. **Ready‑for‑[[00_F.03 - Intra-Context Sense Clustering|F.3]] signal**
   `lexicalOnly(u) ∧ minimal(u) ∧ didactic(u) ⊢ readyF3(u)`
   *A unit is suitable input for **intra‑Context clustering** in [[00_F.03 - Intra-Context Sense Clustering|F.3]].*

## [[00_F.02 - Term Harvesting & Normalisation|F.2]]:13 - Relations

**Builds on:**
**[[00_F.01 - Domain-Family Landscape Survey|F.1]]** (Contexts fixed; heterogeneity/parsimony in place).
**[[03_E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] D.CTX** (Context ≡ U.BoundedContext; “Problem Frame” reserved for narrative).
**[[00_F.00.01 - Contextual Lexicon Principles|F.0.1]]** (Source - Local Meaning - Bridge‑Only Crossing).

**Constrains:**
**[[00_F.03 - Intra-Context Sense Clustering|F.3]]** (Intra‑Context Sense Clustering): operates **only** on units **from one Context**; produces Local‑Senses and addressable **SenseCells**.
**[[00_F.04 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]** (Role Description Definition): may **cite SenseCells**, not raw strings.
**[[00_F.09 - Alignment & Bridge across Contexts|F.9]]** (Alignment & Bridge): consumes **homonymy signals**; declares explicit Cross‑context mappings with loss policies.

**Used by.**
Extention patterns in Part C when referencing domain idioms (labels stay **context‑local**).

## [[00_F.02 - Term Harvesting & Normalisation|F.2]]:14 - Migration notes (conceptual)

1. **New edition appears.** Add a Context in [[00_F.01 - Domain-Family Landscape Survey|F.1]]; harvest afresh in [[00_F.02 - Term Harvesting & Normalisation|F.2]] using that Context; do not overwrite earlier units.
...
3. **Ambiguity inside a Context.** If use splits, **mint two units** with distinct glosses; [[00_F.03 - Intra-Context Sense Clustering|F.3]] will sort their relation (same/different Local‑Sense).
4. **Language split.** Treat each language canon as its **own Context**; resist cross‑language merges in [[00_F.02 - Term Harvesting & Normalisation|F.2]].
...
## [[00_F.02 - Term Harvesting & Normalisation|F.2]]:15 - Acceptance tests (SCR/RSCR — concept‑level)

### [[00_F.02 - Term Harvesting & Normalisation|F.2]]:15.1 - Static conformance (SCR)

* **SCR‑F2‑S01 (context‑locality).** Every unit cites a Context from [[00_F.01 - Domain-Family Landscape Survey|F.1]].
...
* **SCR‑F2‑S05 (No Cross‑context claims).** Nowhere does [[00_F.02 - Term Harvesting & Normalisation|F.2]] assert equivalence/similarity/subsumption across Contexts.
...
### [[00_F.02 - Term Harvesting & Normalisation|F.2]]:15.2 - Regression (RSCR)
...
* **RSCR‑F2‑E03 (Language split).** Adding a second language yields a second Context; no bilingual collapse in [[00_F.02 - Term Harvesting & Normalisation|F.2]].
* **RSCR‑F2‑E04 (No stealth bridges).** After updates, [[00_F.02 - Term Harvesting & Normalisation|F.2]] still contains **zero** Cross‑context identity claims; any mapping appears only in [[00_F.09 - Alignment & Bridge across Contexts|F.9]].
* **RSCR‑F2‑E05 (Head‑term focus).** Periodic check shows the unit set remains small and oriented to F.3/F.4/F.9 needs.

## [[00_F.02 - Term Harvesting & Normalisation|F.2]]:16 - Didactic distillation (60‑second script)

> “In [[00_F.02 - Term Harvesting & Normalisation|F.2]] you **harvest inside Contexts**. For each Context, pick the canon’s own phrasing, choose a **Local Normal Form** in that idiom, add **Tech** and **Plain** labels, and write a one‑sentence **Gloss** that matches how that Context talks. Stop there. No bridging, no behaviour, no equations. If the same string appears in another Context, treat it as a **different unit**. These units feed [[00_F.03 - Intra-Context Sense Clustering|F.3]], where you’ll sort senses **within** a Context, and [[00_F.09 - Alignment & Bridge across Contexts|F.9]], where you’ll relate Contexts explicitly. This keeps meaning local, names faithful, and later reasoning clean.”

## [[00_F.02 - Term Harvesting & Normalisation|F.2]]:End
