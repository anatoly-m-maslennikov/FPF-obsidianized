...
RSCR tests, Worked‑Examples, and a `TaskMap@Context` (`TaskMap`; handoff record consumed by [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]).

**Primary hooks.** [[G.Core - Part G Core Invariants|G.Core]] (Part‑G invariants + RSCR trigger catalogue + Default Governing Definition Index), [[G.1 - CG‑Frame‑Ready Generator|G.1]] (CG‑FrameContext), [[G.2 - SoTA Harvester & Synthesis|G.2]] (SoTA Synthesis Pack), [[G.3 - CHR Authoring for a CG‑Frame- Characteristics, Scales, Levels, Coordinates|G.3]] (CHR Pack), [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]] (CG‑Spec legality gate), [[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]] (CN‑Spec), [[A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC‑KERNEL)|A.18]] (CSLC), [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] (provenance anchors), [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (trust / freshness / decay), [[E.18 - Transformation Flow Structure|E.18]] + [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] + [[F.9 - Alignment & Bridge across Contexts|F.9]]/[[F.17 - Unified Term Sheet (UTS)|F.17]]/[[E.17 - Multi‑View Publication Kit|E.17]] (GateCrossing / CrossingBundle harnesses), [[G.6 - Evidence Graph & Provenance Ledger|G.6]] (EvidenceGraph / PathId / PathSliceId; wired via Extensions), [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] (Selector & Dispatch), [[G.10 - SoTA Pack Shipping|G.10]] (shipping), [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] (refresh orchestration), plus Contexts/UTS/LEX disciplines already fixed elsewhere in the spec.

**Non‑duplication note.** Universal Part‑G invariants (no shadow specs, crossing visibility, tri‑state guard, penalties→`R_eff`‑only, set‑return semantics, P2W split, typed RSCR causes, Default Governing Definition Index, shipping boundary) are governed in [[G.Core - Part G Core Invariants|G.Core]] and are pulled into [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] only through the `G.Core linkage` manifest in **[[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4.1** (and via explicit delegations in CC).

## [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:1 - Problem frame
...
This pattern provides the design‑time authoring kit and the publication surface for CAL artifacts, while delegating Part‑G‑wide invariants to [[G.Core - Part G Core Invariants|G.Core]] and CN-Spec and CG-Spec legality to `CG‑Spec`/`CN‑Spec`.

## [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:2 - Problem
...
## [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:3 - Forces
...
## [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4 - Solution — CAL authoring kit and publication surface

### [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4.1 - [[G.Core - Part G Core Invariants|G.Core]] linkage (normative)

**Builds on:** [[G.Core - Part G Core Invariants|G.Core]] (Part‑G core invariants; citation/delegation hub)

**GCoreLinkageManifest (normative).** Canonical shape, Nil‑elision, and the Expansion rule are defined in [[G.Core - Part G Core Invariants|G.Core]].
...
By the [[G.Core - Part G Core Invariants|G.Core]] Expansion rule, the effective conformance ids / trigger kinds / pin obligations for [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] are the expansions of the referenced profiles/sets/pin‑sets plus the explicit deltas above.

Notes (normative intent, delegated semantics):

* The semantics of tri‑state outcomes, penalty routing, set‑return discipline, crossing visibility, P2W split, typed RSCR causes, and the Default Governing Definition Index are governed in [[G.Core - Part G Core Invariants|G.Core]] and are not redefined here.
* EvidenceGraph/Path pins (when used) are declared only via **`G.4:Ext.EvidenceGraphWiring`** in **[[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4.5** (so `G.Core linkage` stays minimal and does not “pull in” [[G.6 - Evidence Graph & Provenance Ledger|G.6]] by default).
* Method‑specific pins (e.g., QD descriptor/distance/insert policy pins; open‑ended transfer rules pins) MUST appear only in **Extensions** blocks (see **[[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4.5**) and MUST NOT introduce competing defaults.

### [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4.2 - `CAL Pack@CG-Frame` surface (kit governed by this pattern)
...
* `CAL.Charter@Context` — scope anchor for this CAL pack:
  ...
  * emits `TaskMap@Context` (`TaskMap`) as the canonical handoff record to [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] (task→gates/flows/evidence pins).
...
* **Optional** `CAL.NQD[]` — QD/OEE‑related calculus surfaces when declared:
  ...
  * semantics are governed by method‑specific governing definitions (e.g., [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]]) and not redefined by CAL.
...
* **No refresh governing-definition assignment**: CAL does not govern refresh orchestration; it only publishes pins/payload for refresh (governing definition: [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]).
...
CAL.EvidenceProfile :=
  ⟨ evidenceProfileId(UTS), lanes(F/G/R), anchors([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]])[],
    freshnessPolicyPins[]?, penaltyPolicyPins[]?, ΓFoldRef.edition? ⟩
```

### [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4.3 - CAL authoring chassis C1–C9 (kit governed by this pattern)
...
* emits a `TaskMap@Context` (`TaskMap`) that binds each declared `TaskSignature` (or task family) to:
  ...
  * and any required policy pins/edition pins for reproducibility.
  This is the canonical “handoff manifest” consumed by [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] (thresholds remain only inside `CAL.Acceptance`).
...
* `UnknownHandling` as a tri‑state choice (via [[G.Core - Part G Core Invariants|G.Core]] semantics),
...
* `GateCrossingId[]` / `CrossingBundleId[]` **iff** the clause relies on cross-context, cross-plane, or cross-edition imports (no “silent reuse”).
  Missing required crossing artefacts is a conformance failure and blocks publication of the affected clause/flow (GateCrossing harness: [[E.18 - Transformation Flow Structure|E.18]]/[[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]/[[F.9 - Alignment & Bridge across Contexts|F.9]]/[[F.17 - Unified Term Sheet (UTS)|F.17]]/[[E.17 - Multi‑View Publication Kit|E.17]]; crossing invariants: [[G.Core - Part G Core Invariants|G.Core]]).
...
* provenance anchor references ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]‑style carriers/anchors, cited by id),
* lane tags (`F/G/R`) for each evidence contribution (no implicit lane mixing; penalties route only to `R_eff` as governed by [[G.Core - Part G Core Invariants|G.Core]]),
...
* **does not** redefine QD/OEE semantics (those remain governed by method‑specific patterns such as [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]] / [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] and are wired via `Extensions`).
...
Any cross-context, cross-plane, or cross-edition import required by CAL publication is handled through GateCrossing/CrossingBundle discipline (as governed by [[G.Core - Part G Core Invariants|G.Core]]), and CAL publication is blocked if required crossing artifacts are missing.
...
* emit RSCR‑relevant trigger payload pins (editions/policies/UTS ids/paths) for refresh orchestration (governing definition: [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]).

### [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4.4 - Interfaces (minimal I/O surface)
...
### [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4.5 - Extensions (pattern‑scoped; non‑core)

[[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] supports method‑family and discipline‑specific calculus variations exclusively via pattern‑scoped extensions.
...
- **GoverningPatternId:** [[G.6 - Evidence Graph & Provenance Ledger|G.6]]
- **Uses:** `{[[G.6 - Evidence Graph & Provenance Ledger|G.6]]}`
- **⊑/⊑⁺:** `∅`
...
- **Uses:** `{[[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]}`
- **⊑/⊑⁺:** `∅`
- **RequiredPins/EditionPins/PolicyPins (minimum):**
...
## [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:5 - Archetypal Grounding
...
Downstream, [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] consumes only the handoff manifest: clause ids, operator ids, and evidence profile ids (no embedded thresholds).
...
## [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:6 - Bias-Annotation
...
## [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:7 - Conformance Checklist (normative)

| ConformanceId     | Statement                                                                                                                                                                                                                                                                                                      |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **CC‑G4‑CoreRef** | Conformance with [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] requires satisfying the effective [[G.Core - Part G Core Invariants|G.Core]] obligations referenced by the `GCoreLinkageManifest` in **[[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:4.1** (profiles, pin sets, consumed defaults, and trigger kinds).                                                                                                              |
...
| **CC‑G4‑10**      | `TaskMap@Context` (`TaskMap`) is present and provides [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] with acceptance clause ids (`AcceptanceClauseId[]`; selector gates), operator/flow ids, and evidence profile ids required for explainability and audit; selector implementations must not embed thresholds or duplicate acceptance semantics.    |
| **CC‑G4‑11**      | Any method/discipline specifics are placed under `G.4:4.5 Extensions` as `GPatternExtension` blocks (stable `PatternScopeId`, explicit governing definition, pins, and RSCR triggers); no extension introduces competing defaults or replaces [[G.Core - Part G Core Invariants|G.Core]] invariants. |
| **CC‑G4‑12**      | `CAL Pack@CG-Frame` includes public-id continuity records for public ids: deprecations, edition bumps, and lexical-continuity notes. It exposes refresh payload pins, including editions, policies, UTS ids, and, when present, `PathId` and `PathSliceId`, sufficient for [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] to plan RSCR without inferring semantics from prose. |
| **CC‑G4‑13**      | When `G.4:Ext.NQD` is present, `CAL.NQD[]` is present and is wired only via the declared governing pattern ([[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]): at minimum it pins `DescriptorMapRef.edition`, `DistanceDefRef.edition`, and `InsertionPolicyRef`, and it treats archive/illumination summaries as report‑only unless explicitly promoted by a CAL acceptance clause/policy. |
| **CC‑G4‑14** | CAL does not mint new universal types to encode “strategy/policy”. Strategy is expressed as authored flows + acceptance clauses + policy/task pins (and downstream registry/composition in [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]); any specialization is introduced only via `GPatternExtension` wiring blocks or cited governing patterns.  |

## [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:8 - Common Anti-Patterns and How to Avoid Them
...
## [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:9 - Consequences
...
* Downstream patterns ([[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.8 - SoS‑LOG Bundles & Maturity Ladders|G.8]], [[G.9 - Parity and Benchmark Harness|G.9]], [[G.10 - SoTA Pack Shipping|G.10]], [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]) can reference stable ids/pins without redefining acceptance or operator semantics.
...
## [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:10 - Rationale
...
* [[G.Core - Part G Core Invariants|G.Core]] governs Part‑G invariants and trigger/default discipline,
* [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] governs the CAL kit: authoring objects, publication surface, and handoff manifest.
...
## [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:11 - SoTA-Echoing
...
All of these remain method‑specific semantics and therefore belong in `Extensions` blocks (or their governing patterns), while [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] keeps the calculus kit stable and auditable.

## [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:12 - Relations

**Builds on:** [[G.Core - Part G Core Invariants|G.Core]] (and the pattern template discipline in [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]).

**Uses:** [[G.1 - CG‑Frame‑Ready Generator|G.1]] (CG‑FrameContext), [[G.2 - SoTA Harvester & Synthesis|G.2]] (SoTA Synthesis Pack), [[G.3 - CHR Authoring for a CG‑Frame- Characteristics, Scales, Levels, Coordinates|G.3]] (CHR Pack), [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]] (CG‑Spec legality gate), [[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]] (CN‑Spec), [[A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC‑KERNEL)|A.18]] (CSLC), [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] (provenance anchors), [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (trust/freshness/decay), [[E.18 - Transformation Flow Structure|E.18]] + [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] + [[F.9 - Alignment & Bridge across Contexts|F.9]]/[[F.17 - Unified Term Sheet (UTS)|F.17]]/[[E.17 - Multi‑View Publication Kit|E.17]] (GateCrossing harness).

**Uses (via Extensions):** [[G.6 - Evidence Graph & Provenance Ledger|G.6]] (EvidenceGraph/Path citation; when `G.4:Ext.EvidenceGraphWiring` is present), [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]] (NQD), [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] (E/E‑LOG), [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]] (SoS‑LOG).

**Used by:** [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] (selector/dispatcher), [[G.8 - SoS‑LOG Bundles & Maturity Ladders|G.8]] (SoS‑LOG bundles), [[G.9 - Parity and Benchmark Harness|G.9]] (parity), [[G.10 - SoTA Pack Shipping|G.10]] (shipping), [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] (refresh orchestration).
**Publishes to:** UTS (public ids and public-id continuity records), RSCR (tests and trigger emissions), [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] (handoff manifest), and, as cited payload, shipped packs governed by [[G.10 - SoTA Pack Shipping|G.10]].
...
## [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]:End
