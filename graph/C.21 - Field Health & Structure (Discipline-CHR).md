...
*This pattern supplies the CHR vocabulary of health for disciplines. [[C.20 - Composition of U.Discipline (Discipline‑CAL)|C.20]] composes the discipline; [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]] declares discipline-health characteristics and admissible readings; Part G may publish SoTA palettes or time-series views; Bridges keep cross-context meaning honest; penalties touch **R** only.*

 **Status & placement.** Part C (Kernel Extention Specifications) → Cluster C.I (Core CHRs/CALs).
  **Depends on:** **MM-CHR** ([[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]]), **KD-CAL** ([[C.2 - Epistemic holon composition (KD-CAL)|C.2]]), **USM/Scope** ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]), **Trust & Assurance** ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]), **[[E.10 - Unified Lexical Rules for FPF|E.10]] (LEX‑BUNDLE)**.
  **Coordinates with:** **[[C.20 - Composition of U.Discipline (Discipline‑CAL)|C.20]] Discipline‑CAL** (what a `U.Discipline` is), **[[G.2 - SoTA Harvester & Synthesis|G.2]]** (SoTA palette), **[[G.12 - DHC Dashboards (Discipline‑Health time‑series; admissible telemetry; generation‑first)|G.12]]** (dashboard), **[[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]** (CG‑Spec registry).

## [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:1 - Problem Frame

FPF treats *disciplines* as first-class holons (see **[[C.20 - Composition of U.Discipline (Discipline‑CAL)|C.20]]**): they aggregate epistemes, practices, standards, institutions, and observed Work. Teams routinely say “the field is fragmented,” “standards are converging,” or “replication is improving,” but these claims are rarely **typed** (scale/unit/polarity) or **replayable** (evidence lanes, freshness, scope). [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]] supplies the CHR vocabulary: named Characteristics with CSLC typing, so discipline-health claims can be compared admissibly (CG-Spec) and monitored through time ([[G.12 - DHC Dashboards (Discipline‑Health time‑series; admissible telemetry; generation‑first)|G.12]]) when a project needs that use. Each published value declares ReferencePlane ∈ {world|concept|episteme} and DisciplineId (U.Discipline@UTS); cross-plane use applies CL^plane in Assurance (penalty to R_eff only).

## [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:2 - Problem
...
## [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:3 - Forces
...
## [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:4 - Solution — **Discipline Health Characterisation (DHC)**

### [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:4.0 - Ontology quick sheet (normative, clarifying)
...
Define a **portable minimal set** of CHR **slots**. Each slot is **CHR-typed** (Characteristic, Scale/Unit/Polarity per **[[A.17 - Canonical “Characteristic” (A.CHR‑NORM)|A.17]]–[[A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC‑KERNEL)|A.18]]**), **Context-local**, and guarded by **USM** (Claim scope **G**), **freshness windows**, and **evidence lanes** (TA/VA/LA).  Contexts may extend the set; conforming extensions do not alter scale types illegally.

**“Health” is a vector** of CHR‑typed coordinates; **no single scalar** is implied. Lawful scalarization lives in **Acceptance** ([[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]) under an explicit **CG‑Spec ScaleComplianceProfile (SCP)** and **Γ‑fold** rules, and is never embedded in CHR.

### [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:4.1 - Core Characteristics (kernel-portable names)
...
3. **AlignmentDensity** *(ratio; polarity ↑; ReferencePlane=episteme; CG‑Spec‑bound)*
   Density of **Substitution Bridges** (same **senseFamily**, CL≥2) between major `U.Tradition`s **per 100 DHC‑SenseCells** ([[G.2 - SoTA Harvester & Synthesis|G.2]] F‑hooks) in the SoTA palette.  Substitution rule:  free substitution permitted at **CL=3**; at **CL=2** substitute only with extra‑guard (count in reporting, but this is not «free substitution») Units: `bridges_per_100_cells`. Cross‑Context use requires Bridge+CL; penalties → **R_eff** only.

 4. **DisruptionBalance** *(interval; polarity = target band; ReferencePlane=episteme; CG‑Spec‑bound)*
  Relative share of **disruptive vs consolidating** works within **Γ_time** using a **registered CD‑index class** (editioned; cite **method id** in UTS). **Default plane:** *episteme*. Publish the **target band** via **Acceptance ([[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]])**; not in CHR.
...
### [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:4.1a - Engineering-grade and semio-substitution extension slots
...
### [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:4.2 - Guard Macros (normative)
...
### [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:4.3 - Legality Matrix (extract)
...
## [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:5 - Interfaces & Data Paths

* **Inputs.** `U.Discipline` from **[[C.20 - Composition of U.Discipline (Discipline‑CAL)|C.20]]** (composition), SoTA **Palette**/**BridgeMatrix** from **[[G.2 - SoTA Harvester & Synthesis|G.2]]** (**DHC‑SenseCells** included), EvidenceProfiles from **G.4/G.6**.
* **Outputs.** Per‑Context **DHC rows** (these six slots), **UTS** Name Cards with twin labels (E.5/F.17–[[F.18 - Local‑First Unification Naming Protocol|F.18]]), **Registry/RSCR hooks** on method edition changes; feeds **[[G.12 - DHC Dashboards (Discipline‑Health time‑series; admissible telemetry; generation‑first)|G.12]]** (time‑series).
* **Cross-Context reuse.** Only via **[[F.9 - Alignment & Bridge across Contexts|F.9]] Bridges** with **CL** and **loss notes**; **Φ(CL)** penalties applied to **R** (never F/G).

## [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:6 - Archetypal Grounding (three fields)

### [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:6.1 - Computer Vision (Benchmarks 2015→)
...
### [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:6.2 - Biomedicine (Gene–Disease Associations)
...
### [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:6.3 - Software Performance Engineering (SPE)
...
### [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:6.4 - Decision‑Making (2015→)
...
### [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:6.5 - Evolutionary Architecture (software)
...
## [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:7 - Characteristic reading and publication path

1. **Declare Context & TargetSlice.** (USM) Name editions, Standards, env params, `Γ_time`.
2. **Collect evidence.** Bind sources via **[[G.6 - Evidence Graph & Provenance Ledger|G.6]] EvidenceGraph**; tag lanes and freshness.
3. **Compute DHC slots.** Enforce **Legality Matrix** and Guard Macros.
4. **Bridge (if needed).** Map via **[[F.9 - Alignment & Bridge across Contexts|F.9]]**; attach **CL** and **loss notes**; apply **R** penalties.
...
6. **Publication view.** Feed [[G.12 - DHC Dashboards (Discipline‑Health time‑series; admissible telemetry; generation‑first)|G.12]] with time-series and guard-bands (disruption, diversity) when a dashboard or trend publication is live.

## [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:8 - Bias-Annotation (E-cluster lenses)
...
## [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:9 - Conformance Checklist
...
| **CC-C.21-2 Freshness.** | Published values carry a `Γ_time` selector and freshness window; stale rows route to `{degrade|abstain}` in [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] Acceptance. | Prevents stale cumulative history from masquerading as current health. |
...
| **CC-C.21-12 Lexical firewall.** | Core narrative follows [[E.5.1 - DevOps Lexical Firewall|E.5.1]] and does not use tool/vendor tokens as discipline-health kinds. | Prevents vendor or tool labels from becoming characteristics. |
...
| **CC-C.21-16 Role kit, informative.** | Standard roles from [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] may be used: `DisciplineStewardRole`, `DHCMethodAuthorRole`, `DHCSeriesPublisherRole`; values still declare design/run stance and `ReferencePlane`. | Roles do not become evidence or authority. |
...
## [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:10 - Consequences
...
**Risks avoided.** “Phlogiston disciplines” (charisma-driven fields) surface as unhealthy in DHC readings; **No-Free-Lunch** preserved by [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] (selector returns sets, not universal scalars).

## [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:11 - SoTA-Echoing and rationale

| SoTA/practice anchor | What it changes in [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]] | Adoption stance | Boundary of non-overread |
|---|---|---|---|
| Open Science Collaboration (2015), Munafò et al. (2017), and current reproducibility/metascience practice on replication, transparency, claim granularity, and freshness. | `ReproducibilityRate`, `EvidenceGranularity`, freshness windows, and evidence-lane tagging are live discipline-health characteristics rather than one global credibility score. | Adopt and adapt: use reproducibility as one typed characteristic with scope, window, and evidence lanes. | [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]] does not certify any single claim as true; claim evidence remains under [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]], [[G.6 - Evidence Graph & Provenance Ledger|G.6]], or the evidence pattern governing the claim. |
...
The practical consequence is that [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]] reads discipline health through typed characteristics. It can feed dashboards or time-series publications, but the dashboard is only a publication view over DHC readings; it is not the discipline-health ontology and not project authority.

## [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:12 - Relations

* **Builds on:** **[[A.17 - Canonical “Characteristic” (A.CHR‑NORM)|A.17]]–[[A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC‑KERNEL)|A.18]]** (Characteristic/CSLC), **[[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]** (USM scopes), **[[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]** (assurance lanes), **[[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]]** (MM-CHR templates).
* **Coordinates with:** **[[C.20 - Composition of U.Discipline (Discipline‑CAL)|C.20]]** (what a `U.Discipline` *is*), **[[G.2 - SoTA Harvester & Synthesis|G.2]]** (SoTA palette and BridgeMatrix), **[[G.12 - DHC Dashboards (Discipline‑Health time‑series; admissible telemetry; generation‑first)|G.12]]** (Dashboard operationalization), **[[G.9 - Parity and Benchmark Harness|G.9]]** (parity harness for fair comparisons).
* **Constrains:** **[[G.10 - SoTA Pack Shipping|G.10]]** (pack ships DHC rows + method ids), **[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]** (refresh windows/decay), **[[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]** (selector may reference DHC only via admissible predicates; no cross‑ordinal scalarisation). **Coordinates:** **[[F.9 - Alignment & Bridge across Contexts|F.9]]** (Bridges for cross‑Tradition comparisons).

## [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:13 - Annex — Author’s quick template (copy-paste)
...
## [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]:End
