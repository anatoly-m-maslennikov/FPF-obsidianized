...
**Stage:** release‑time composition and publication; edition‑aware; **GateCrossing‑gated** via [[E.18 - Transformation Flow Structure|E.18]] CrossingBundle (and the relevant GateCrossing harness patterns).
**Builds on:** [[G.Core - Part G Core Invariants|G.Core]] (Part‑G core invariants and delegation); upstream pack/kit governing definitions as cited publications or records (not redefined here).
...
## [[G.10 - SoTA Pack Shipping|G.10]]:1 - Problem frame — Shipping without smuggling semantics
...
[[G.10 - SoTA Pack Shipping|G.10]] fixes the pack boundary: it defines the **single, normative shipping surface** for Part‑G outputs — **`SoTA‑Pack(Core)`** — and a minimal choreography for making shipped artefacts **selector‑ready** and **audit‑citable**, while delegating all Part‑G‑wide invariants to [[G.Core - Part G Core Invariants|G.Core]] (citation/delegation, not restatement).

## [[G.10 - SoTA Pack Shipping|G.10]]:2 - Problem — Why naive shipping breaks reuse, legality, and refresh
...
## [[G.10 - SoTA Pack Shipping|G.10]]:3 - Forces
...
## [[G.10 - SoTA Pack Shipping|G.10]]:4 - Solution — `SoTA‑Pack(Core)` as the shipping object and publication kit

[[G.10 - SoTA Pack Shipping|G.10]] defines a **pack-governed** shipping surface: a notation‑independent object that **cites** all upstream artefacts by stable ids/refs and exposes the minimum pins required to (a) consume the result via selection, (b) audit it via path citations and crossing bundles, and (c) refresh it via typed RSCR triggers.

### [[G.10 - SoTA Pack Shipping|G.10]]:4.1 - [[G.Core - Part G Core Invariants|G.Core]] linkage (normative)

**Builds on:** [[G.Core - Part G Core Invariants|G.Core]] (Part‑G core invariants; Default Governing Definition Index citation)

**GCoreLinkageManifest ([[G.10 - SoTA Pack Shipping|G.10]])** *(normative; expands per `G.Core:4.2`; `Nil‑elision` applies)*
...
* `CorePinsRequired` *(pattern delta; pin names only; id‑valued unless noted)* := {
  ...
  *(Optional pins from `CrossingVisibilityPins` MAY be strengthened to unconditional by listing them above; [[G.10 - SoTA Pack Shipping|G.10]] typically strengthens `UTSRowId[]` and path/crossing bundles when the pack is publicly shipped.)*
...
### [[G.10 - SoTA Pack Shipping|G.10]]:4.2 - `SoTA‑Pack(Core)` object model (normative; notation‑independent)
...
⟨
  ...
  CNSpecRef := ⟨[[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]] ref, CNSpecRef.edition⟩,
  CGSpecRef := ⟨[[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]] ref,  CGSpecRef.edition⟩,
  ...
  SoTAHarvestPackId?          // e.g., [[G.2 - SoTA Harvester & Synthesis|G.2]] output id
  CHRPackId?                  // [[G.3 - CHR Authoring for a CG‑Frame- Characteristics, Scales, Levels, Coordinates|G.3]] output id
  CALPackId?                  // [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] output id
  EvidenceGraphId?            // [[G.6 - Evidence Graph & Provenance Ledger|G.6]] output id
  BridgeMatrixId?             // G.2/G.7 cited id
  BridgeCalibrationTableId?   // [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] output id
  SoSLOGBundleId?             // [[G.8 - SoS‑LOG Bundles & Maturity Ladders|G.8]] output id
  ParityReportId?             // [[G.9 - Parity and Benchmark Harness|G.9]] output id
  DashboardSliceId?           // [[G.12 - DHC Dashboards (Discipline‑Health time‑series; admissible telemetry; generation‑first)|G.12]] output id (optional)
  InteropSurfaceId?           // [[G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]] output id (optional)
...
### [[G.10 - SoTA Pack Shipping|G.10]]:4.2.1 - Portfolio roster (normative; pack-governed; governing-definition delegating)
...
it MUST NOT redefine selection / selected-set semantics (governed by [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]) or parity semantics (governed by [[G.9 - Parity and Benchmark Harness|G.9]]).
...
The `selectorOutcomeKind`, `setResultFamily`, `handoffKind`, `sourceSetFamily`, `sourceSetComposition`, `derivedViewKind`, `basePaletteRef`, `lensId`, and `shortlistId` fields in this roster are payload metadata fields or refs inside the shipped publication form. They do not define publication face kinds, publication form kinds, interop publication form kinds, or carrier kinds, and they do not let [[G.10 - SoTA Pack Shipping|G.10]] re-govern [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], or [[G.2 - SoTA Harvester & Synthesis|G.2]] semantics.

**Interpretation constraints (normative by delegation).** Any universal invariants governing (i) CN/CG spec-ref governing-definition assignment, (ii) crossing visibility and penalty routing, (iii) tri‑state guards, (iv) set‑return semantics, (v) P2W split, (vi) defaults, and (vii) RSCR trigger typing are **not restated here** and are enforced via [[G.Core - Part G Core Invariants|G.Core]] conformance (see `CC‑G10‑CoreRef`).

### [[G.10 - SoTA Pack Shipping|G.10]]:4.3 - Shipping choreography (normative; governing-definition delegating)

[[G.10 - SoTA Pack Shipping|G.10]] prescribes a minimal, governing-definition delegating sequence for composing a shipped pack:
...
4. **S‑4 — Anchor and publish path citations.** Ensure [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] anchors exist and publish/record `PathId/PathSliceId` citations required for downstream explainability (e.g., `C.23/H4`) and maturity rung changes.
...
8. **S‑8 — Optional: ingest interop surface.** If [[G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]] interop is in use, ingest/cite `InteropSurface@Context` as annotation-only notes, pinning external index editions; do not redefine interop semantics.

### [[G.10 - SoTA Pack Shipping|G.10]]:4.4 - Interfaces & hooks (selector‑ and audit‑facing)

| ID         | Interface (conceptual)     | Consumes                                                          | Produces                                                |
| ---------- | -------------------------- | ----------------------------------------------------------------- | ------------------------------------------------------- |
| **[[G.10 - SoTA Pack Shipping|G.10]]‑1** | `Compose_SoTA_Pack`        | G.* outputs, ComparatorSet, Bridges, editions, SCR/DRR deltas     | `SoTA‑Pack(Core)` (UTS row + surfaces) + `AuditPins` (+ `MOOManifestId?`) (+ `PortfolioRosterId?`) |
| **[[G.10 - SoTA Pack Shipping|G.10]]‑2** | `Publish_UTS`              | `PackId(UTS)`, `UTSRowId[]`, deprecation/edition‑bump notes       | UTS rows/Name Cards for the pack and shipped heads (incl. twins when required) |
| **[[G.10 - SoTA Pack Shipping|G.10]]‑3** | `Expose_CrossingHooks`     | GateCrossings, lanes/planes/contexts                              | **CrossingBundle** (**[[E.18 - Transformation Flow Structure|E.18]]:CrossingBundle**) per GateCrossing; **fail** on missing/non‑conformant bundles |
| **[[G.10 - SoTA Pack Shipping|G.10]]‑4** | `Pack_MOO`                 | referenced mechanism/policy/edition ids                           | `MOOManifestId` (ids only; governing-definition delegating) |
| **[[G.10 - SoTA Pack Shipping|G.10]]‑5** | `Emit_TelemetryPins`       | Illumination/archive/OEE events                                   | PathSlice‑keyed telemetry: `policy‑id`, `…Ref.edition` (+ QD/OEE pins when applicable) |
| **[[G.10 - SoTA Pack Shipping|G.10]]‑6** | `Publish_PathCitations`    | [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] anchors, PathIds                                             | PathId/PathSlice citations for `C.23/H4` & rung changes |
| **[[G.10 - SoTA Pack Shipping|G.10]]‑7** | `Ingest_InteropSurface?`   | (optional) `G.13 InteropSurface@Context`                          | Annotated pack notes citing external‑index editions     |

*Surfaces remain **conceptual** per **[[E.5.2 - Notational Independence|E.5.2]]**; RO‑Crate/ORKG/OpenAlex mappings belong to **Annex/Interop** and do not affect Core conformance.*
...
### [[G.10 - SoTA Pack Shipping|G.10]]:4.5 - Consequence of governing-definition assignment (normative boundary statement)

[[G.10 - SoTA Pack Shipping|G.10]] is the **one governing definition** of “shipping” in Part G *(by delegation to `CC‑GCORE‑SKP‑1`)*.
Other `G.x` patterns may produce artefacts that are shipped, but they must not embed shipping obligations; they cite [[G.10 - SoTA Pack Shipping|G.10]] shipping surfaces instead.

### [[G.10 - SoTA Pack Shipping|G.10]]:4.6 - Extensions (pattern‑scoped; non‑core)
...
**Uses:** `{[[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.8 - SoS‑LOG Bundles & Maturity Ladders|G.8]], [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]}`
**⊑/⊑⁺:** `∅`
...
**RSCRTriggerSetIds:** `∅` *(covered by [[G.10 - SoTA Pack Shipping|G.10]] core linkage via `GCoreTriggerSetId.RefreshOrchestration`)*
...
**Uses:** `{[[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]}`
**⊑/⊑⁺:** `∅`
**RequiredPins/EditionPins/PolicyPins (minimum):**
...
### [[G.10 - SoTA Pack Shipping|G.10]]:4.7 - Published surfaces must ship kind, source, derivation, lens, and shortlist token
...
### [[G.10 - SoTA Pack Shipping|G.10]]:4.7.1 - Worked publication slice
...
## [[G.10 - SoTA Pack Shipping|G.10]]:5 - Consequences
...
* If upstream governing definitions fail to provide citable ids/pins, [[G.10 - SoTA Pack Shipping|G.10]] cannot paper over the gap; shipping will block or ship a visibly incomplete pack (depending on policy‑bound failure behaviour, governed by cited definitions).

## [[G.10 - SoTA Pack Shipping|G.10]]:6 - Bias‑Annotation (informative)

Lenses tested: **Gov**, **Arch**, **Onto/Epist**, **Prag**, **Did**.

* **Format bias (Arch/Prag).** A popular export format is tempting to treat as “the pack”.
  *Mitigation:* keep Core surfaces conceptual ([[E.5.2 - Notational Independence|E.5.2]]); move serialisation recipes to Annex/Interop; keep conformance on semantics.
...
## [[G.10 - SoTA Pack Shipping|G.10]]:7 - Archetypal grounding (informative; post‑2015 method families)
...
## [[G.10 - SoTA Pack Shipping|G.10]]:8 - Conformance checklist (CC‑G10)
...
| --- | ----------- | -------------------------------- |
| **CC‑G10‑CoreRef** | The pattern satisfies the **effective** [[G.Core - Part G Core Invariants|G.Core]] obligations declared by `G.10:4.1` (after profile/set/pin‑set expansion under `Nil‑elision`). | Check that the linkage manifest is present and that the expanded obligations are not contradicted. |
...
## [[G.10 - SoTA Pack Shipping|G.10]]:8.1 - Anti‑patterns and remedies

* **AP‑1 Format‑as‑governing‑specification.** Remedy: keep Core surfaces conceptual ([[E.5.2 - Notational Independence|E.5.2]]); move serialisation to Annex/Interop; enforce `CC‑G10.1`.
...
* **AP‑7 Interop ingestion rewrites semantics.** Remedy: ingest interop as cited notes only; semantics remain in [[G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]] (`G.10‑6`, `G.10:Ext.InteropCitation`).
...
## [[G.10 - SoTA Pack Shipping|G.10]]:8.2 - SoTA‑Echoing (post‑2015, for orientation)

* **Research‑object packaging & provenance.** Post‑2015 practice increasingly treats “release artefacts” as *packages with explicit provenance, versions, and minimal replay pins* (e.g., modern research‑object and RO‑Crate‑class approaches). [[G.10 - SoTA Pack Shipping|G.10]] mirrors the “package‑as‑citation‑surface” idea while keeping semantics governing-definition delegated.
...
## [[G.10 - SoTA Pack Shipping|G.10]]:9 - Relations

**Builds on:** [[G.Core - Part G Core Invariants|G.Core]]; consumes/cites artefacts governed by cited patterns from [[G.2 - SoTA Harvester & Synthesis|G.2]] (harvest pack), [[G.3 - CHR Authoring for a CG‑Frame- Characteristics, Scales, Levels, Coordinates|G.3]] (CHR pack), [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] (CAL pack), [[G.6 - Evidence Graph & Provenance Ledger|G.6]] (EvidenceGraph), [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] (bridge calibration), [[G.8 - SoS‑LOG Bundles & Maturity Ladders|G.8]] (SoS‑LOG bundle), [[G.9 - Parity and Benchmark Harness|G.9]] (parity report), optional [[G.12 - DHC Dashboards (Discipline‑Health time‑series; admissible telemetry; generation‑first)|G.12]] (dashboard slice), optional [[G.13 - External Interop Hooks for SoTA Discipline Packs (conceptual)|G.13]] (interop surface).
**Publishes to / used by:** UTS (pack identity), selector‑facing consumers (via [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]), audit and assurance publications (SCR/RSCR), refresh orchestration ([[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]).
**Constrains:** tooling exports are downstream; serialisation and repository integration are explicitly non‑normative here.

## [[G.10 - SoTA Pack Shipping|G.10]]:End
