...
**Builds on:** [[F.1 - Domain‑Family Landscape Survey|F.1]] **context of meaning**; [[F.2 - Term Harvesting & Normalisation|F.2]] **Term Harvesting**; [[F.3 - Intra‑Context Sense Clustering|F.3]] **Intra‑Context Clustering (SenseCell)**; [[F.5 - Naming Discipline for U.Types & Roles|F.5]] **Naming Discipline**; [[F.7 - Concept‑Set Table|F.7]] **Concept‑Set (row) construction**; [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] **Mint‑or‑Reuse decision**; [[F.9 - Alignment & Bridge across Contexts|F.9]] **Bridges**; [[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] **Status windows**.
...
## [[F.13 - Lexical Continuity & Deprecation|F.13]]:1 - Intent & applicability
...
## [[F.13 - Lexical Continuity & Deprecation|F.13]]:2 - Problem frame
...
## [[F.13 - Lexical Continuity & Deprecation|F.13]]:3 - Forces
...
## [[F.13 - Lexical Continuity & Deprecation|F.13]]:4 - Core idea (didactic)
...
> A label (including aliases) always belongs to **one context** or to **one Concept‑Set row**. Cross‑context similarity is handled by **Bridges** ([[F.9 - Alignment & Bridge across Contexts|F.9]]), never by shared names.

## [[F.13 - Lexical Continuity & Deprecation|F.13]]:5 - Minimal vocabulary (this pattern only)

* **Legacy label** — a previously used label in the same Context (or same Concept‑Set row / Role Description).
* **Preferred label** — the current **[[F.5 - Naming Discipline for U.Types & Roles|F.5]]‑conformant** label for that item.
...
## [[F.13 - Lexical Continuity & Deprecation|F.13]]:6 - Solution — Continuity, not “registries”
...
### [[F.13 - Lexical Continuity & Deprecation|F.13]]:6.1 - Continuity relations (normative meanings)

1. **`renames(label_old → label_new)`** — *wording improved, sense unchanged*.
   *Use when:* Same **SenseCell** / same **Concept‑Set row** / same **Role Description**; only the lexical form changed to satisfy [[F.5 - Naming Discipline for U.Types & Roles|F.5]] (morphology, disambiguation, plain/tech harmony).
...
3. **`splits(label_old ⇒ {label_A, label_B})`** — *one label covered multiple senses; now separated*.
   *Use when:* Your **SenseCell** was really two local senses; [[F.3 - Intra‑Context Sense Clustering|F.3]] has **split** them; or a **Concept‑Set row** is refactored into two rows.
...
4. **`merges({label_A, label_B} ⇒ label_new)`** — *two labels now recognized as one sense*.
   *Use when:* [[F.3 - Intra‑Context Sense Clustering|F.3]] shows **same SenseCell**; or two Concept‑Set rows collapse after [[F.9 - Alignment & Bridge across Contexts|F.9]] raised CL sufficiently.
...
> **Important:** All five relations are **context‑local** (SenseCell level) or **row‑local** (Concept‑Set). **Never** use them to “alias” across Contexts. If a change crosses Contexts, it is not a rename; it requires a **Bridge** ([[F.9 - Alignment & Bridge across Contexts|F.9]]) and often a **split/merge of rows** ([[F.7 - Concept‑Set Table|F.7]]).

## [[F.13 - Lexical Continuity & Deprecation|F.13]]:7 - Invariants (normative)
...
5. **Prefer present for writing.** In normative writing, use the **current preferred label** ([[F.5 - Naming Discipline for U.Types & Roles|F.5]]). Aliases are for **reading comprehension**.
6. **Bridge discipline.** If a label shift would require crossing Contexts to “explain”, it is **not a rename**; use **[[F.9 - Alignment & Bridge across Contexts|F.9]] Bridge** and, if needed, refactor the **Concept‑Set row(s)**.
7. **Epoch honesty.** When declaring continuity, attach a **succinct epoch note** (“pre‑2023 usage”) if it aids readers.

## [[F.13 - Lexical Continuity & Deprecation|F.13]]:8 - Self‑checks (mental, not procedural)
...
## [[F.13 - Lexical Continuity & Deprecation|F.13]]:9 - Micro‑examples (illustrative)

### [[F.13 - Lexical Continuity & Deprecation|F.13]]:9.1 - Pure rename inside a Context (ITIL → clearer plain label)
...
**Why:** [[F.5 - Naming Discipline for U.Types & Roles|F.5]] morphology & expansion; SenseCell unchanged (same clause semantics).
**Effect:** Old guidance remains readable; new writing spells out the term.

### [[F.13 - Lexical Continuity & Deprecation|F.13]]:9.2 - Alias for a common legacy synonym (Sys‑CAL)
...
### [[F.13 - Lexical Continuity & Deprecation|F.13]]:9.3 - Split of a muddled local sense (Enactment)
...
### [[F.13 - Lexical Continuity & Deprecation|F.13]]:9.4 - Merge after clustering raised confidence (Kind-CAL row)

Two Concept‑Set rows **{“DBaaS”, “Database‑Service”}** converge after [[F.3 - Intra‑Context Sense Clustering|F.3]] within the same context profile and [[F.9 - Alignment & Bridge across Contexts|F.9]] raised CL.
**Relation:** `merges({"DBaaS","Database‑Service"} ⇒ "Database‑Service")`.
**Effect:** “DBaaS” becomes a legacy alias with an epoch note.

### [[F.13 - Lexical Continuity & Deprecation|F.13]]:9.5 - Not a rename: Cross‑context temptation (forbidden)
...
**Action:** **No continuity relation.** Keep labels; if needed, declare a **Bridge** ([[F.9 - Alignment & Bridge across Contexts|F.9]]) explaining design→run mapping with CL/Loss.

## [[F.13 - Lexical Continuity & Deprecation|F.13]]:10 - Anti‑patterns & remedies

| #       | Anti‑pattern               | Symptom in texts                                                      | Why it harms thinking                                          | Remedy (conceptual move)                                                                                                         |
| ------- | -------------------------- | --------------------------------------------------------------------- | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| **A1**  | **Cross‑context rename**      | “Let’s rename *process (BPMN)* to *activity (PROV)*.”                 | Erases Context boundaries; hides loss; violates locality.         | **Do not rename across Contexts.** Keep both labels; if you must relate them, declare a **Bridge** ([[F.9 - Alignment & Bridge across Contexts|F.9]]) with CL/loss.              |
...
| **A4**  | **Paint‑over rename**      | Rename used where sense actually changed.                             | Confuses continuity with revision; hides splits.               | Use **`splits`** (or **`merges`**), not `renames`. If Contexts diverge, adjust **rows** ([[F.7 - Concept‑Set Table|F.7]]) and **Bridges** ([[F.9 - Alignment & Bridge across Contexts|F.9]]).                |
...
| **A8**  | **Row drift via rename**   | Concept‑Set row is relabeled while its membership silently changes.   | Hides that the set changed; breaks Cross‑context alignment.       | First **split/merge rows** ([[F.7 - Concept‑Set Table|F.7]]) as needed; only then `renames` the row **if** its intension stayed.                             |
| **A9**  | **Bridge‑by‑alias**        | Using an alias to hint two Contexts are “the same.”                      | Smuggles translation without CL/loss.                          | **No Cross‑context aliasing.** If similarity matters, **Bridge** explicitly ([[F.9 - Alignment & Bridge across Contexts|F.9]]) and keep labels separate.                         |
| **A10** | **Acronym absolutism**     | Treating acronyms as preferred labels everywhere (“SLO” in any Context). | Obscures Context‑specific senses; hurts didactics.                | Prefer **expanded** labels as preferred ([[F.5 - Naming Discipline for U.Types & Roles|F.5]]); keep acronym as context‑local **alias** only where historically dominant.            |
| **A11** | **Temporal fudge**         | Rename used to imply design↔run shift (“execution ≈ process”).        | Conflates time stances; erases important dualities.            | Keep **DesignRunTag** explicit on labels or glosses; if mapping is needed, do so in **[[F.9 - Alignment & Bridge across Contexts|F.9]]**.                                       |
...
## [[F.13 - Lexical Continuity & Deprecation|F.13]]:11 - Extended examples

### [[F.13 - Lexical Continuity & Deprecation|F.13]]:11.1 - KD‑CAL × Services — *metric target* labels over time
...
* **Why:** Same local sense; clearer morphology for [[F.5 - Naming Discipline for U.Types & Roles|F.5]]; SOSA/SSN labels untouched.
...
### [[F.13 - Lexical Continuity & Deprecation|F.13]]:11.2 - Sys‑CAL × LCA‑CAL — separating *execution* vs *actuation*
...
### [[F.13 - Lexical Continuity & Deprecation|F.13]]:11.3 - Kind-CAL × Method‑CAL — false merge avoided
...
### [[F.13 - Lexical Continuity & Deprecation|F.13]]:11.4 - Enactment × KD‑CAL — retiring a misleading metaphor
...
### [[F.13 - Lexical Continuity & Deprecation|F.13]]:11.5 - Concept‑Set row refactor after rising CL

* **Rows:** `{“DBaaS”, “Database‑Service”}` representing service notions across several Contexts.
* **[[F.3 - Intra‑Context Sense Clustering|F.3]] + [[F.9 - Alignment & Bridge across Contexts|F.9]] outcome:** High CL; evidence of same Cross‑context alignment.
...
## [[F.13 - Lexical Continuity & Deprecation|F.13]]:12 - Reasoning primitives (judgement schemas, notation‑free)
...
### [[F.13 - Lexical Continuity & Deprecation|F.13]]:12.1 - Same‑sense & same‑place
...
### [[F.13 - Lexical Continuity & Deprecation|F.13]]:12.2 -Local alias
...
### [[F.13 - Lexical Continuity & Deprecation|F.13]]:12.3 - Split detection
...
### [[F.13 - Lexical Continuity & Deprecation|F.13]]:12.4 - Merge admission

`ContextOf(ℓA)=ContextOf(ℓB) ∧ senseOf(ℓA)=senseOf(ℓB) ⊢ merges({ℓA,ℓB} ⇒ ℓN)`
*Reading:* Once [[F.3 - Intra‑Context Sense Clustering|F.3]] shows identity of sense **within** a Context, merging labels into one preferred label is safe.

### [[F.13 - Lexical Continuity & Deprecation|F.13]]:12.5 - Retirement
...
### [[F.13 - Lexical Continuity & Deprecation|F.13]]:12.6 - Cross‑context guard

`ContextOf(ℓ₁) ≠ ContextOf(ℓ₂) ⊢ ¬mayRename(ℓ₁→ℓ₂)`
*Reading:* Different Contexts forbid rename/alias; any relation goes to **Bridge** ([[F.9 - Alignment & Bridge across Contexts|F.9]]).

### [[F.13 - Lexical Continuity & Deprecation|F.13]]:12.7 - Writing discipline

`thing t ⊢ writeWithPreferred(t) = pref(t)`
*Reading:* Normative prose uses the **current** preferred label; aliases are for reading.

### [[F.13 - Lexical Continuity & Deprecation|F.13]]:12.8 - Reading resolution
...
### [[F.13 - Lexical Continuity & Deprecation|F.13]]:12.9 - Alias budget

`aliasesFor(thing, register=r) = A ⊢ |A| ≤ 1`
*Reading:* Keep at most one legacy alias per register (Tech/Plain) for any one thing.

### [[F.13 - Lexical Continuity & Deprecation|F.13]]:12.10 - Row‑level continuity

`rowOf(ℓA)=rowOf(ℓB)=R ∧ intension(R) stable ⊢ mayRenameRow(R,ℓB)`
*Reading:* A row label can change if the **row’s membership/intension** did not change; otherwise refactor rows first ([[F.7 - Concept‑Set Table|F.7]]).

## [[F.13 - Lexical Continuity & Deprecation|F.13]]:13 - Relations

**Builds on:**
[[F.1 - Domain‑Family Landscape Survey|F.1]] **context of meaning** (keeps locality), [[F.2 - Term Harvesting & Normalisation|F.2]] **Harvesting** (provides attested strings), [[F.3 - Intra‑Context Sense Clustering|F.3]] **Clustering** (establishes SenseCells), [[F.5 - Naming Discipline for U.Types & Roles|F.5]] **Naming Discipline** (supplies preferred labels), [[F.7 - Concept‑Set Table|F.7]] **Concept‑Set rows**, [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] **Mint‑or‑Reuse**, [[F.9 - Alignment & Bridge across Contexts|F.9]] **Bridges**, [[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] **Status windows**, [[F.11 - Method Quartet Harmonisation|F.11]] **Method harmonisation**, [[F.12 - Service Acceptance-Work Evidence Link|F.12]] **Service acceptance**.

**Constrains:**

* **[[F.5 - Naming Discipline for U.Types & Roles|F.5]] (Naming):** may select preferred labels **only** after applying these continuity relations.
* **[[F.7 - Concept‑Set Table|F.7]] (Rows):** row relabels require row **intension** stability; otherwise use **split/merge rows**.
* **[[F.9 - Alignment & Bridge across Contexts|F.9]] (Bridges):** Cross‑context changes must **not** be expressed as renames/aliases.

**Used by.**
All Part C patterns when editions shift; all examples and tutorials when teaching with legacy terminology.

## [[F.13 - Lexical Continuity & Deprecation|F.13]]:14 - Migration notes (conceptual playbook)
...
2. **Keep it inside the Context.** If your explanation crosses Contexts, stop—this is **Bridge** territory ([[F.9 - Alignment & Bridge across Contexts|F.9]]), not a rename.
3. **Prefer clarity over fashion.** Rename only when the new label **removes a real ambiguity** ([[F.5 - Naming Discipline for U.Types & Roles|F.5]] criteria), not to chase style.
...
6. **Rows before names.** If a rename request coincides with a shift in what the row covers, **refactor rows** ([[F.7 - Concept‑Set Table|F.7]]) first, then choose labels.
...
## [[F.13 - Lexical Continuity & Deprecation|F.13]]:15 - Acceptance tests (SCR/RSCR — concept‑level)

### [[F.13 - Lexical Continuity & Deprecation|F.13]]:15.1 - Static conformance (SCR)
...
* **SCR‑F13‑S05 (Row integrity).** A row rename occurs only when the row’s **intension** is stable; if membership changed, a **row split/merge** is documented ([[F.7 - Concept‑Set Table|F.7]]).
* **SCR‑F13‑S06 (Bridge discipline).** No alias/rename is used to imply Cross‑context sameness; any such relation belongs under **[[F.9 - Alignment & Bridge across Contexts|F.9]]**.

### [[F.13 - Lexical Continuity & Deprecation|F.13]]:15.2 - Regression (RSCR)
...
## [[F.13 - Lexical Continuity & Deprecation|F.13]]:16 - Didactic distillation (60‑second script)
...
## [[F.13 - Lexical Continuity & Deprecation|F.13]]:End
