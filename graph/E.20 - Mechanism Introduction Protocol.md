...
## [[E.20 - Mechanism Introduction Protocol|E.20]]:1 - Problem frame
...
**Use this when.** Use [[E.20 - Mechanism Introduction Protocol|E.20]] when a proposed FPF change introduces or revises mechanism meaning, suite denotation, suite closure, suite obligations, suite pins, suite protocol semantics, planned-baseline pins, wiring semantics, governing-definition assignment, or what a citeable token denotes.
...
**Generative side.** [[E.20 - Mechanism Introduction Protocol|E.20]] preserves open-ended action by allowing new mechanism definitions, suite variants, wiring, and citeable tokens to enter FPF with a named governing definition; the discipline prevents semantic drift so new work can be added rather than merely blocked.
...
**What this buys.** [[E.20 - Mechanism Introduction Protocol|E.20]] gives the reader one current authoring move: assign the change to the right governing definition and keep mechanism, suite, planning, wiring, and lexical continuity distinct.
...
## [[E.20 - Mechanism Introduction Protocol|E.20]]:2 - Problem
...
## [[E.20 - Mechanism Introduction Protocol|E.20]]:3 - Forces
...
## [[E.20 - Mechanism Introduction Protocol|E.20]]:4 - Solution — the Mechanism Introduction Protocol (MIP)

### [[E.20 - Mechanism Introduction Protocol|E.20]]:4.0 - Terminology note (disambiguation)
...
* **Local wording or alias-docking only:** wording clarifies an already-governed mechanism relation, or [[F.18 - Local‑First Unification Naming Protocol|F.18]] alias docking preserves citeability of an old token without changing what the token denotes.
...
### [[E.20 - Mechanism Introduction Protocol|E.20]]:4.0.1 - Mint vs reuse
...
**Reuses:**
* `U.Mechanism` definition cards and `MechanismDefinitionRef`, suite descriptions (`MechSuiteDescription` and specializations), WorkPlanning plan items (`SlotFillingsPlanItem` and specializations), alias docking ([[F.18 - Local‑First Unification Naming Protocol|F.18]]), RSCR triggers ([[G.Core - Part G Core Invariants|G.Core]]), and PQG profiles ([[E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]]).

### [[E.20 - Mechanism Introduction Protocol|E.20]]:4.1 - Step 1: Classify the introduction
...
2. **New mechanism definition within an existing [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] mechanism kind** (new `MechanismDefinitionRef`, new canonical card).
...
8. **Deprecation / supersession / retirement** (marking mechanisms/suites/plan items as deprecated, declaring successors, and preserving citeability; apply [[E.20 - Mechanism Introduction Protocol|E.20]]:4.9.1).

**Mechanism kind boundary.** A MIP-run may introduce a new `MechanismDefinitionRef`. It does not introduce a new [[E.18 - Transformation Flow Structure|E.18]] transformation-flow locus kind or transformation-flow structure unless [[E.18 - Transformation Flow Structure|E.18]] is explicitly updated, and it does not introduce a new [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]] `U.Kind` unless [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]] and [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] discipline is the current governing question.

**[[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] compatibility.** MIP assigns mechanism meaning to A.6.1-governed mechanism definitions: operation algebra, law set, admissibility conditions, `SlotIndex`, per-operation `SlotSpec`s with required input and output `SlotKind`s, transport or bridge regime, applicability, audit, and monotone realization relation when declared. Suites, planned-baseline records, and Part-G wiring modules may cite or bind that meaning; they do not supply or redefine it.

**New mechanism-family criterion.** Treat a change as a new mechanism family, species, or archetypal grounding only when the existing mechanism-governing pattern cannot express the operation algebra, law set, admissibility conditions, `SlotIndex`, per-operation `SlotSpec`s, required input/output `SlotKind`s, transport boundary, audit semantics, or monotone realization relation when declared without changing its kind invariants. Otherwise classify the change as a new mechanism definition or `MechanismDefinitionRef` within an existing [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] mechanism kind.
...
* the change class(es) from [[E.20 - Mechanism Introduction Protocol|E.20]]:4.1 when needed to disambiguate the assignment,
...
* the best-known Delta-Class (`Δ-0` to `Δ-3`) and impact radius estimate ([[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]]) when the run is plausibly `Δ-2` or `Δ-3`,
* intended RSCR trigger types when a refresh or regression-wiring claim is present, and
* the PQG ([[E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]]) profile set when the run crosses an E.19-governed review boundary.
...
| Suite membership, obligations, spec pins, and suite protocols | **Suite-governing pattern** | [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]] or `A.6.7.<FamilyKey>` | SHALL NOT carry mechanism semantics, acceptance thresholds, gate criteria, DecisionLogs, or publication tails into the suite. |
| Planned baseline pins (planned slot fillings, edition-pinned refs, explicit time selector) | **WorkPlanning governing pattern** | [[A.15.3 - SlotFillingsPlanItem|A.15.3]] plus suite-specific specialization when needed | SHALL NOT embed launch values, witnesses, or gate decisions in planning. |
| SoTA method, comparator, or generator **definitions**, including provenance and evaluation semantics | **SoTA-pack governing pattern** | [[G.2 - SoTA Harvester & Synthesis|G.2]] (SoTA synthesis packs) | SHALL NOT rephrase SoTA evolution as kernel semantics. |
...
| Token renames and drift management | **Lexical governing pattern** | [[F.18 - Local‑First Unification Naming Protocol|F.18]] (alias docking) plus registers per E.10/F.17 | SHALL NOT silently rewrite tokens or break citations. |
| Change-cause taxonomy and regression triggers | **RSCR governing pattern** | [[G.Core - Part G Core Invariants|G.Core]] | SHALL NOT invent ad hoc “reason kinds” scattered in patterns. |
...
### [[E.20 - Mechanism Introduction Protocol|E.20]]:4.3 - Step 3: Card-first canonicalization (eliminate dangling refs)
...
### [[E.20 - Mechanism Introduction Protocol|E.20]]:4.4 - Step 4: Mechanism semantics completion (what “done” means)

**Definition-of-done note (delegated).** MIP uses two completion checkpoints for mechanism cards:

* **Stub done** - a citeability stub for a `MechanismDefinitionRef`: a resolvable canonical target created only to prevent dangling references ([[E.20 - Mechanism Introduction Protocol|E.20]]:4.3), not semantic completion.

 A stub **SHALL** (i) exist at the mechanism-governing pattern's canonical location, (ii) reserve and publish the stable `MechanismDefinitionRef` (and its lexical/register entries), (iii) set `MechanismDefinitionHeader.status = draft`, and (iv) carry an explicit DRR pointer for completing semantics. A stub **SHALL** also list the *[[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]* conformance checklist item IDs it does **not** yet satisfy (without duplicating that checklist here). A stub may preserve citeability for suite or protocol enumeration, but it does not authorize suite closure, gate checks, planned baselines, wiring consumption, reuse, or import unless the fields required for that use are present and marked current.

* **Introduced done** - a mechanism card that can be relied upon as a `U.Mechanism` definition. "Introduced done" is defined by *[[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]* conformance: the card **SHALL** satisfy the applicable *[[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]:7 Conformance Checklist* items (**CC-UM.\***), with the baseline items designated by *[[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]* (e.g., **CC-UM.0** and **CC-UM.1**) being the minimum requirement.

The list below is **informative** only (semantic orientation); the normative structure and “done” criteria are delegated to *[[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]*’s CC items to avoid drift between this protocol and the canonical mechanism definition.
...
* **Specialisation discipline (when `⊑/⊑⁺` is declared):** explicit parent+morphism kind; SlotKind invariance; monotone ValueKind narrowing; no new mandatory inputs to inherited operations (per [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]:4.2.1 / CC‑UM.8).
...
If the mechanism introduces new slot kinds shared across a family/suite, apply [[E.20 - Mechanism Introduction Protocol|E.20]]:4.5.

### [[E.20 - Mechanism Introduction Protocol|E.20]]:4.5 - Step 5: Suite-scoped slot-token lexicon discipline (prevent slot drift)

If the mechanism belongs to a suite or family where multiple member mechanisms share slot vocabulary:

1. The suite-governing pattern SHALL provide a **suite-scoped slot-token lexicon** referencing [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] SlotSpecs (or update it if already present) in the suite-governing pattern's canonical location ([[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]] / `A.6.7.<FamilyKey>`), or as a dedicated lexicon card explicitly referenced from there. The lexicon cites and organizes SlotKind tokens; it does not create new SlotKind semantics.

2. Mechanism cards SHALL cite slot kinds from that lexicon (rather than minting local near-duplicates).
3. New slot kinds SHALL be introduced into the lexicon first, then referenced by member mechanisms. If any citeable `SlotKind` tokens are minted/renamed, apply [[E.20 - Mechanism Introduction Protocol|E.20]]:4.9.
...
### [[E.20 - Mechanism Introduction Protocol|E.20]]:4.6 - Step 6: Suite integration (if the mechanism is a suite member)
...
**Kernel stability rule (recommended).** If the suite is a kernel suite, and the change adds a new required stage, prefer creating a **suite variant** rather than mutating the kernel membership. If mutation is unavoidable, pair it with terminology continuity ([[E.20 - Mechanism Introduction Protocol|E.20]]:4.9) and RSCR triggers ([[E.20 - Mechanism Introduction Protocol|E.20]]:4.10).

### [[E.20 - Mechanism Introduction Protocol|E.20]]:4.7 - Step 7: Planned baseline & P2W planning-to-work boundary (if planning changes)
...
### [[E.20 - Mechanism Introduction Protocol|E.20]]:4.8 - Step 8: Wiring & SoTA updates (keep method evolution out of kernel)

If the introduction involves methods, comparators, selectors, or other SoTA-sensitive choices:

1. Put method/comparator family semantics in **SoTA packs** ([[G.2 - SoTA Harvester & Synthesis|G.2]]) and reference them by edition-pinned refs.
2. Pin the chosen SoTA refs for a baseline in WorkPlanning plan items ([[E.20 - Mechanism Introduction Protocol|E.20]]:4.7); wiring consumes pins rather than silently overriding them.
...
5. If a SoTA update changes a mechanism's signature/laws, that semantic change SHALL be performed in the mechanism-governing pattern, under the [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] mechanism-definition template; the change SHALL emit RSCR triggers ([[E.20 - Mechanism Introduction Protocol|E.20]]:4.10).

### [[E.20 - Mechanism Introduction Protocol|E.20]]:4.9 - Step 9: Terminology continuity (alias docking)

If the introduction renames any public token or changes canonical naming:

1. Use lexical alias docking ([[F.18 - Local‑First Unification Naming Protocol|F.18]]) so old tokens remain citeable.
...
### [[E.20 - Mechanism Introduction Protocol|E.20]]:4.9.1 - Deprecation / supersession / retirement (preserve citeability)

If the change class includes deprecation/supersession/retirement ([[E.20 - Mechanism Introduction Protocol|E.20]]:4.1 #8), the MIP-run SHALL preserve reference continuity while making the status change explicit:
...
2. **Keep the public token citeable.** The deprecated token (`MechanismDefinitionRef`, suite token, plan-item token, etc.) SHALL remain citeable. If a successor token/name is introduced, the old token SHALL be alias-docked per [[F.18 - Local‑First Unification Naming Protocol|F.18]] ([[E.20 - Mechanism Introduction Protocol|E.20]]:4.9).
...
4. **Assign downstream updates to governing definitions.** Any needed suite denotation, closure, obligation, pin, protocol-semantic, WorkPlanning-pin, or wiring-semantic change SHALL be performed in its respective governing definition ([[E.20 - Mechanism Introduction Protocol|E.20]]:4.2), preferably by introducing a suite variant rather than silently swapping kernel membership.
5. **Emit RSCR triggers.** Deprecation/supersession SHALL emit typed RSCR triggers and extend the regression envelope ([[E.20 - Mechanism Introduction Protocol|E.20]]:4.10), including checks for dangling refs and alias coverage.

### [[E.20 - Mechanism Introduction Protocol|E.20]]:4.10 - Step 10: RSCR triggers + regression envelope
...
**Guard (normative).** Trigger kind identifiers (e.g., `RSCRTriggerKindId`) SHALL be selected from the RSCR trigger catalogue governed by [[G.Core - Part G Core Invariants|G.Core]]. A MIP-run SHALL NOT mint ad hoc trigger kinds (“reason kinds”) scattered in arbitrary patterns/modules.
...
### [[E.20 - Mechanism Introduction Protocol|E.20]]:4.11 - Step 11: Apply PQG profiles ([[E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]]) and close the run

Every MIP-run SHALL be reviewed using PQG ([[E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]]) with:
...
## [[E.20 - Mechanism Introduction Protocol|E.20]]:6 - Bias-Annotation
...
## [[E.20 - Mechanism Introduction Protocol|E.20]]:7 - Conformance Checklist (normative)
...
**Conformance groups.** Ordinary [[E.20 - Mechanism Introduction Protocol|E.20]] use starts with trigger triage and stops at the current governing locus when no denotation or mechanism-meaning change is present. Manifest-core items apply only when a MIP-run is actually triggered. Publication/assurance items apply only when citeability, card stubs, alias docking, RSCR, PQG, or deprecation continuity is part of the current claim. Crossing, launch, and work-enactment checks are not governed by [[E.20 - Mechanism Introduction Protocol|E.20]]; if those claims become present, use the gate, planning, or work loci and keep [[E.20 - Mechanism Introduction Protocol|E.20]] to governing-definition assignment.

| ID | Requirement | Purpose |
|---|---|---|
| **CC-E20-0 (MIP trigger triage).** | Every proposed mechanism, suite, planned-baseline, wiring, governing-definition, or citeable-token edit is classified as `MIP not triggered`, `local wording or alias-docking only`, or `MIP-run manifest required` before [[E.20 - Mechanism Introduction Protocol|E.20]] is cited to start a MIP-run. | Prevents pure currentness cleanup from becoming a false runtime gate or expanded authoring event. |
...
| **CC‑E20‑6 (Kernel stability handled).** | If a kernel suite would gain a new required stage, the change **SHOULD** be expressed as a suite variant; if mutation occurs, it **SHALL** include continuity measures (alias docking and explicit delta). | Minimizes [[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]] impact radius of kernel edits. |
...
| **CC‑E20‑10 (PQG coverage).** | Every MIP-run **SHALL** be reviewed under PQG ([[E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]]) with PCP‑BASE and the triggered profiles implied by the change. | Normalizes review and refresh. |
| **CC‑E20‑11 (Deprecation preserves citeability).** | Any deprecation/supersession/retirement action **SHALL** preserve citeability of the deprecated token (alias docking if renamed), keep the canonical mechanism card, suite description, plan item, or wiring module resolvable, and declare a successor pointer or “no successor” explicitly ([[E.20 - Mechanism Introduction Protocol|E.20]]:4.9.1). | Prevents broken citations and orphaned semantics during evolution. |

## [[E.20 - Mechanism Introduction Protocol|E.20]]:8 - Common Anti-Patterns and How to Avoid Them
...
| **Kernel churn by convenience** | New required stage is added directly to kernel suite membership. | Expands the [[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]] impact radius; destabilizes citations. | Prefer suite variant; if not possible, pair with alias docking and explicit deltas. |
| **Token drift by silent rename** | “Just rename UNM to ...” without aliasing. | Breaks citations and downstream reasoning. | Use [[F.18 - Local‑First Unification Naming Protocol|F.18]] alias docking; update registers explicitly. |
| **MIP as gate surrogate** | A MIP-run manifest is treated as a runtime pass/fail result or gate passage. | Governing-definition assignment is being mistaken for project execution or gate decision. | Keep MIP as authoring-side governing-definition assignment; use [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] for gate decisions and [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] for work or enactment claims. |
...
## [[E.20 - Mechanism Introduction Protocol|E.20]]:9 - Consequences
...
## [[E.20 - Mechanism Introduction Protocol|E.20]]:10 - Rationale
...
## [[E.20 - Mechanism Introduction Protocol|E.20]]:12 - Relations

**Builds on:**
* **[[E.8 - FPF Authoring Conventions & Style Guide|E.8]]** (pattern structure and normative authoring discipline)
* **[[E.10 - Unified Lexical Rules for FPF|E.10]] / [[F.17 - Unified Term Sheet (UTS)|F.17]]–[[F.18 - Local‑First Unification Naming Protocol|F.18]]** (lexical registers, twin labels, alias docking)
* **[[E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]]** (PQG/PCP profile-based review)
* **[[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]]** (evolution discipline; DRR/edition thinking)

**Coordinates with:**
* **[[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]** (`U.Mechanism` definition template governance)
* **[[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]** (`MechSuiteDescription` integrity)
* **[[A.15.3 - SlotFillingsPlanItem|A.15.3]]** (`SlotFillingsPlanItem` and planned baseline seam)
* **[[E.18 - Transformation Flow Structure|E.18]]** (`TransformationFlowStructure` values that cite planned baselines)
* **[[G.Core - Part G Core Invariants|G.Core]]** (RSCR trigger catalogue)
* **[[G.2 - SoTA Harvester & Synthesis|G.2]]** (SoTA synthesis packs)
...
## [[E.20 - Mechanism Introduction Protocol|E.20]]:End
