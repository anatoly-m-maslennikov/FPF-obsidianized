...
**At a glance.** Use [[E.17 - Multi‑View Publication Kit|E.17]] when one source-backed episteme, episteme-lane view, morphism, or functional relation needs several readable publication faces for different readers without changing the underlying claim.
...
> **General publication-face form:** one MVPK face is a `U.View` emitted over one source `U.Episteme` or episteme-lane `U.View`, under one publication `U.Viewpoint`, one `U.PublicationScope`, declared pins where needed, one face kind, and one bounded publication use. The face adds no claim by readable form. Evidence use, authority use, gate use, work use, release use, and engineering-justification use require the neighboring FPF pattern governing that claim and typed project-side value/reference that carry that downstream use, such as a `GateDecision`, evidence path, work occurrence, status source, or authority-reference relation.
> **USM binding (overview):** `PublicationScope` is a **USM‑class** object that parameterizes MVPK; see §5.0.
> **Episteme lane.** MVPK treats each face as a `U.View` in the sense of [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] and [[E.17.0 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]] (species `U.EpistemeView`). For any MVPK face, the source is a named `U.Episteme` or episteme-lane `U.View`; the face declares a publication `U.Viewpoint` (`PublicationVPId`) drawn from a `U.ViewpointBundle` ([[E.17.1 - U.ViewpointBundleLibrary - Reusable Viewpoint Bundles|E.17.1]] and [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]). In the morphism profile, every `Emit_s(f)` has `EntityOfConcernSlot` and `DescriptionContext` target `f : U.Morphism`. In a non-morphism publication, the face names the source episteme named by value, episteme-lane view, EntityOfConcern, or claim relation that the face publishes, and no functorial composition claim is present unless the corresponding FPF pattern supplies it. Slot discipline (`ViewSlot` and `ViewRef`) is inherited from [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] and [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] and is not redefined in MVPK.

## [[E.17 - Multi‑View Publication Kit|E.17]]:1 - Intent
...
## [[E.17 - Multi‑View Publication Kit|E.17]]:2 - Problem frame
...
## [[E.17 - Multi‑View Publication Kit|E.17]]:4 - Forces
...
| **Publication-face independence ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]])** | Publication preserves EntityOfConcern, Description-episteme, and specification-use boundaries ↔ authors expect rich presentations. |
...
## [[E.17 - Multi‑View Publication Kit|E.17]]:5 - Solution — the **MVPK Kit**

### [[E.17 - Multi‑View Publication Kit|E.17]]:5.0 - USM scope binding (normative)
* **PublicationScope (USM).** `U.PublicationScope` is defined in **USM** ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] §6.5) analogously to `U.WorkScope` and `U.ClaimScope` as a **set-valued scope object** over `U.ContextSlice`. In MVPK, every emitted `U.View` declares a `U.PublicationScope` that bounds where that face has bounded publication use.
...
### [[E.17 - Multi‑View Publication Kit|E.17]]:5.1 - Terminology (normative)

* **View** (`U.View`): an episteme-lane view (`U.EpistemeView` in the sense of [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] and [[E.17.0 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]]) produced under a publication viewpoint. In MVPK each face (`PlainView`, `TechCard`, `InteropCard`, `AssuranceLane`) is such a `U.View`. In the morphism profile its `EntityOfConcernSlot` and `DescriptionContext` target is a `U.Morphism`; in a non-morphism publication, the target is the source episteme named by value, episteme-lane view, EntityOfConcern, or claim relation named by the source.
...
* **Publication vs presentation vs rendering vs representation (guard):**
    * **Publication** = typed projection from existing source epistemes or episteme-lane views into a `U.View` governed by a `publication face/form` or `interop publication form` `publication-face kind` via species of `U.EpistemicViewing` ([[A.6.3 - U.EpistemicViewing - EntityOfConcern-preserving morphism|A.6.3]]). In the morphism profile, the source epistemes are Description epistemes about a morphism, possibly under specification use when [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]], [[C.2.3 - Unified Formality Characteristic F|C.2.3]], [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]], [[E.10 - Unified Lexical Rules for FPF|E.10]], or another neighboring pattern governing the claiming gate grants that use. [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] supplies the EntityOfConcern and Description-episteme boundary; publication expression does not turn that boundary into a three-member strict-distinction ontology.
    ...
    * **Rendering** = display layout of a carrier, purely graphical formatting; **`U.Work` on carriers** ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]]), not a `publication-face kind`.
    * **Representation** = episteme↔referent relation ([[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]], [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]] through [[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]]); **not** a publication operation and not a `publication-face kind` operation. Use **publication** and **view** here; treat presentation and rendering as **`U.Work` on carriers** ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]]).
...
* **No‑mechanism equivalence:** MVPK **is not** a mechanism; any operational activity, such as build, render, or upload work, is **separate `U.Work` by a system on carriers** ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]]; see **Rule 5 — No Γ-leakage** in §6).
* **ViewpointSpec (`U.Viewpoint`)** — a typed specification that declares stakeholders, concerns, conformance rules, declared **Publication Characteristics**, and pinning requirements per profile. The index set `Σ` consists of identifiers of `U.Viewpoint` instances, typically drawn from `U.ViewpointBundle` species ([[E.17.1 - U.ViewpointBundleLibrary - Reusable Viewpoint Bundles|E.17.1]] or [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]) (see §5.3).
...
### [[E.17 - Multi‑View Publication Kit|E.17]]:5.1a - Episteme-publication lane binding  *(normative)*
...
**Interface and protocol proximity.** When interface, protocol, schema, boundary, or API wording appears beside a functional-flow description, keep the operational boundary, interface, or protocol claim with its own project claim set and typed project-side value named by value and reference, governed by the relevant FPF pattern such as [[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]], [[A.6.C - Contract Unpacking for Boundaries|A.6.C]], or [[E.18 - Transformation Flow Structure|E.18]]. Do not absorb it into the functional-flow publication by layout proximity.

**Retargeting.** If the publication changes the EntityOfConcern or retargeting target from an already described component, process, material `U.Entity`, or source claim into a functional, control, or flow architecture claim, this is not a same-entity publication-use change. Use [[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]], `OntologicalReframing`, or [[E.18 - Transformation Flow Structure|E.18]] as applicable.
...
- work-relevant source restoration under [[A.15.4 - Work-Relevant Source Restoration|A.15.4]];
- project `U.Method`, `U.WorkPlan`, or work-result record under [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]];
- evidence and provenance path under [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]];
- engineering-justification record under [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]];
- constraint or gate decision under [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] or [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]];
- supervisory or control architecture record under [[B.2.5 - Supervisor-Subholon Feedback Loop|B.2.5]];
- carrier, export, OCR, or front-end record under [[A.7 - Strict Distinction (Clarity Lattice)|A.7]];
- same-entity textual relation under [[A.6.3.CR - ConservativeRetextualization- EntityOfConcern-Preserving Textual Re-Expression|A.6.3.CR]];
- representation relation under [[A.6.3.RT - Representation-Scheme Transition- EntityOfConcern-Preserving Representation-Scheme Transition|A.6.3.RT]];
- reduced-use-rendering relation under [[A.6.3.CSC - Controlled Semantic Coarsening|A.6.3.CSC]].
...
MVPK inherits the [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] distinction between `U.Episteme`, `U.EpistemePublication`, publication form, `U.View`, carrier, and authority-reference relation. MVPK does not introduce a generic semio kind and does not let a publication face act as `governingPatternRef`, `authoritySourceRef`, or the source claim for a claim.
...
### [[E.17 - Multi‑View Publication Kit|E.17]]:5.1b - Shared source-relation and bounded-use vocabulary
...
| `independent-verification-present` | A separate check makes the local claim or use available independently of the publication-facing unit only through a named governing pattern and record named by value, such as an [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence path, [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] assurance claim, [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] `GateDecision`, [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] constraint profile, [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] `U.WorkPlan`, [[A.15.1 - U.Work|A.15.1]] dated `U.Work` occurrence, or [[F.9 - Alignment & Bridge across Contexts|F.9]] Bridge Card. |
...
| `reliance use` | The publication-facing unit is used as the source relation for an engineering claim or effect that changes a next work or reliance move, such as method choice, work plan, performed-work claim, release, gate, approval, role or status, evidence, assurance, or external-impact move. |
...
| `operative claim` | A claim whose acceptance would change the next bounded work or reliance move, the typed project-side value named by value and reference to recover, or the cross-context use of the publication-facing unit. Explanatory prose, examples, and source-finding cues are not operative claims unless they are used that way. |
...
### [[E.17 - Multi‑View Publication Kit|E.17]]:5.1d - Compact boundary aid for the present claim or effect
...
1. If one local head is the only unstable part, apply [[E.17.AUD.LHR - PublicationUnit Stability Discipline and Local Head Restoration - repair the overloaded local lexical head before the publication unit inherits it|E.17.AUD.LHR]] or [[C.2.P - Epistemic Precision Restoration|C.2.P]] and stop when the repaired sentence names the local kind, relation, and bounded use.
2. If the bounded `PublicationUnit` or its primary EntityOfConcern interpretation is unstable, apply [[E.17.AUD - PublicationUnit Stability Discipline - keep one publication unit stable enough to read honestly|E.17.AUD]] or [[E.17.AUD.OOTD - PublicationUnit Stability Discipline and PublicationUnit Primary EntityOfConcern Discipline - publication-unit stability over one primary EntityOfConcern|E.17.AUD.OOTD]] before using [[E.17.ID.CR - ComparativeReviewUnit - bounded comparison over comparative review units|E.17.ID.CR]] or [[E.17.EFP - ExplanationFaithfulnessProfile - explanation-use discipline over existing MVPK faces|E.17.EFP]].
3. If the unit is stable and the present problem is comparison overread, apply [[E.17.ID.CR - ComparativeReviewUnit - bounded comparison over comparative review units|E.17.ID.CR]]; use [[F.9 - Alignment & Bridge across Contexts|F.9]], [[C.11 - Decision Theory (Decsn-CAL)|C.11]], [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]], or [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] only when equivalence, recommendation, selection, decision, gate, or release claim is actually being made.
4. If the unit is stable and the present problem is explanation overread, apply [[E.17.EFP - ExplanationFaithfulnessProfile - explanation-use discipline over existing MVPK faces|E.17.EFP]]; use [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]], [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]], [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], or [[A.15.4 - Work-Relevant Source Restoration|A.15.4]] only when evidence, engineering-justification, gate, release, work, or reliance claim is actually being made.
5. If the present problem is a durable reusable name, UTS row, Core-facing term, or cross-context naming relation, apply [[F.18 - Local‑First Unification Naming Protocol|F.18]]; otherwise keep the lighter local repair pattern.

| Present claim or effect question | Apply or recover |
| --- | --- |
| Is the publication-facing unit being used to guide a work or reliance move by appearance, while the acting user still needs the typed project-side value named by value and reference before proceeding? | [[A.15.4 - Work-Relevant Source Restoration|A.15.4]] for the restoration step; the recovered value can then be [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]], [[A.15.1 - U.Work|A.15.1]], [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]], [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]], [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]], [[A.2.9 - U.SpeechAct (Communicative Work Object)|A.2.9]], [[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]], or another typed project-side value named by value and reference. If that exact value is already the question under repair, use it directly. |
| Is the publication-facing unit being used as evidence, provenance, attestation, currentness, freshness, or claim-bound evidence relation? | [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence, provenance, or currentness path for the claim named by value. |
| Is the publication-facing unit being used as engineering justification, assurance, confidence, readiness, or limitations relation? | [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] assurance or engineering-justification claim with evidence, limits, and decay explicit. |
| Is the publication-facing unit being used as gate passage, constraint validity, adjudication, or release decision source? | [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] or [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] project records, including gate profile, constraint profile, decision record, log reference, scope, window, replay reference and freshness reference. |
...
| Did the EntityOfConcern, target, ontology frame, or claim or relation record named by value change? | [[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]], `OntologicalReframing`, or the retargeting or reframing pattern named by value. |
| Is the publication-facing unit being used as bridge, substitution, equivalence, "same", "equivalent", "align", or "map" wording, or cross-context comparison relation? | Use Part F and [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] for repairing "same", "equivalent", "align", or "map" wording into explicit bridge work; use [[F.9 - Alignment & Bridge across Contexts|F.9]] or [[F.9.1 - Bridge Stance Overlay|F.9.1]] for Bridge Cards, bridge kind, direction, `CL`, loss notes, bounded use, and stance overlays. Comparison alone is not a bridge. |
| Is the question under repair carrier, export, OCR, screen, front-end behavior, or work on carriers? | [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] and the exact carrier relation, front-end relation, or work-on-carrier record. |

**Evidence-path boundary.** An [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence, provenance, attestation, freshness, or currentness path carries only the claim named by value it instantiates. It does not approve or authorize work, pass a gate, perform work, supply release permission, or raise assurance or engineering-justification use unless the typed project-side value named by value and reference that carries that downstream claim is also instantiated, such as [[A.15.4 - Work-Relevant Source Restoration|A.15.4]], [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]], [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]], [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], or [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]].
...
### [[E.17 - Multi‑View Publication Kit|E.17]]:5.1e - Local review fields are not FPF kinds
...
### [[E.17 - Multi‑View Publication Kit|E.17]]:5.1f - Shared anti-overread invariants for publication-facing units
...
**Measure and display overread.** A number, score, percentage, color, rank, confidence value, similarity value, dashboard state, or measurement display is orientation only until the measurement source, aggregation rule, time window, population or scope, calibration or evidence path, and intended use are governed by the typed project-side value named by value and reference. Evidence-like use applies [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]; assurance-like use applies [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]; gate-like use applies [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] or [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]; work or reliance use applies [[A.15.4 - Work-Relevant Source Restoration|A.15.4]] and then the recovered typed project-side value named by value and reference; bridge or substitution use applies [[F.9 - Alignment & Bridge across Contexts|F.9]] or [[F.9.1 - Bridge Stance Overlay|F.9.1]].
...
**Same publication-facing unit, multiple interpretations.** A green release dashboard can be one MVPK face for source-finding, an [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] currentness path or evidence path when the evidence query is recoverable, an [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] gate-decision view when the `GateDecisionRef` is recoverable, or an unsupported release cue when those sources are missing. A generated comparative explanation can be an [[E.17.EFP - ExplanationFaithfulnessProfile - explanation-use discipline over existing MVPK faces|E.17.EFP]] explanation-use case, an [[E.17.ID.CR - ComparativeReviewUnit - bounded comparison over comparative review units|E.17.ID.CR]] comparison case, a [[A.6.3.CR - ConservativeRetextualization- EntityOfConcern-Preserving Textual Re-Expression|A.6.3.CR]] generated-summary case, or source-finding only; it is never all of those under one shared evidence-relation class or bounded-use value by fluency alone.
...
- **Generated explanation with source links.** A generated text explains a method and cites sources. The explanation rendering is not source replacement. Source links carry only the pinned operative claims they actually carry. If work or reliance is present, use [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] for the evidence path named by value or keep the rendering as reader help; if the rendering is deliberately reduced-use, use [[A.6.3.CSC - Controlled Semantic Coarsening|A.6.3.CSC]].
- **Comparison table.** A table compares two methods and places one first. Ordering is not selection. The comparator or sorting relation, source references, shared review frame, and unsupported downstream claim remain visible. Choice or decision needs [[C.11 - Decision Theory (Decsn-CAL)|C.11]]; equivalence or bridge relation needs [[F.9 - Alignment & Bridge across Contexts|F.9]] or [[F.9.1 - Bridge Stance Overlay|F.9.1]].
- **Unrecovered source wording.** A draft uses source-object wording, undeclared interpretive-view shorthand, or generic unit wording without naming the FPF kind. Recover the FPF kind stack instead of minting source-relation pseudo-kinds or undeclared interpretive-view pseudo-kinds. Use `PublicationUnit` only when a bounded reader-inspected unit inside a publication is present; otherwise use the exact episteme, view, publication, carrier relation, section of a named non-pattern FPF publication form whose reader-help function and reference are recoverable, [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]] relation claim, or typed project-side value named by value and reference.
- **Translated tutorial.** A translated tutorial can improve reader access to an FPF pattern. It is a derivative rendering, not the original source. Operative claims need source mapping for reliance, translated heads can need [[E.17.AUD.LHR - PublicationUnit Stability Discipline and Local Head Restoration - repair the overloaded local lexical head before the publication unit inherits it|E.17.AUD.LHR]] or [[C.2.P - Epistemic Precision Restoration|C.2.P]], and [[F.18 - Local‑First Unification Naming Protocol|F.18]] is present only when durable naming, UTS, Core-facing, or cross-context naming work is intended.
...
- A comparison table appears to select option B. Block the selection interpretation when no [[C.11 - Decision Theory (Decsn-CAL)|C.11]] `ChoiceResult`, decision record, or visible selection relation exists. Useful publication use remains: use the table as a bounded comparison under [[E.17.ID.CR - ComparativeReviewUnit - bounded comparison over comparative review units|E.17.ID.CR]], or apply [[C.11 - Decision Theory (Decsn-CAL)|C.11]] when selection is intended.
...
- A generated explanation appears to prove a causal relation. Block the evidence or assurance interpretation when source pins and evidence path are absent or insufficient. Useful publication use remains: use the explanation as reader help or source-finding, then apply [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] or [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] only for the evidence named by value or engineering-justification claim they govern.

- [[C.2.P - Epistemic Precision Restoration|C.2.P]] prevents the wrong object from being treated as source, the wrong relation from being treated as source relation, and a loose phrase from being treated as an FPF kind.
- [[E.17.AUD - PublicationUnit Stability Discipline - keep one publication unit stable enough to read honestly|E.17.AUD]] and [[E.17.AUD.OOTD - PublicationUnit Stability Discipline and PublicationUnit Primary EntityOfConcern Discipline - publication-unit stability over one primary EntityOfConcern|E.17.AUD.OOTD]] prevent action on a publication unit whose primary entity of concern, carried publication move, or outside boundary shifted silently.
- [[E.17.ID.CR - ComparativeReviewUnit - bounded comparison over comparative review units|E.17.ID.CR]] prevents a comparison unit from being used as decision, equivalence, bridge, evidence, or release source relation.
- [[E.17.EFP - ExplanationFaithfulnessProfile - explanation-use discipline over existing MVPK faces|E.17.EFP]] prevents fluent explanation from laundering unsupported claims into reliance, assurance, gate, or evidence use.
- [[E.17 - Multi‑View Publication Kit|E.17]] MVPK prevents a readable publication face from being treated as evidence, gate, work, authority, or release source relation by display quality.
- [[F.18 - Local‑First Unification Naming Protocol|F.18]] prevents a local name from becoming global identity without context, kind, lineage, and bridge or cross-context naming relation.

**Anti-escalation examples.** Do not apply a neighboring pattern when its claim being made is absent:

- Do not apply [[F.18 - Local‑First Unification Naming Protocol|F.18]] when a one-off local phrase repair restores the local kind, relation, and bounded use without minting a durable reusable name.
- Do not apply [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] when the publication-facing unit is not being used for reliance, evidence, provenance, currentness, or claim-bound evidence relation.
- Do not apply [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] when a dashboard tile is merely status orientation and no `GateDecisionRef` or gate profile is present.
- Do not apply [[F.9 - Alignment & Bridge across Contexts|F.9]] when a comparison does not claim sameness, substitution, bridge relation, or cross-context equivalence.
- Do not apply [[E.17.EFP - ExplanationFaithfulnessProfile - explanation-use discipline over existing MVPK faces|E.17.EFP]] when the text is only a same-entity rewrite or representation change governed by [[A.6.3.CR - ConservativeRetextualization- EntityOfConcern-Preserving Textual Re-Expression|A.6.3.CR]] or [[A.6.3.RT - Representation-Scheme Transition- EntityOfConcern-Preserving Representation-Scheme Transition|A.6.3.RT]].
...
### [[E.17 - Multi‑View Publication Kit|E.17]]:5.2 - Declared `publication-face kind` values at Part E
...
`AssuranceLane` can expose evidence bindings, evidence-carrier references, pins, and presence bits. It is not a [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] assurance claim, readiness or confidence verdict, engineering-justification record, or evidence-sufficiency result. When a published face is used to raise or lower assurance, readiness, confidence, limitation, or engineering-justification result, the governing source relation is [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]; the lane only helps recover the cited evidence bindings.
...
### [[E.17 - Multi‑View Publication Kit|E.17]]:5.3 - The kit (constructs)
...
### [[E.17 - Multi‑View Publication Kit|E.17]]:5.4 - EntityOfConcern-side input and output vs publication (normative convention)
...
### [[E.17 - Multi‑View Publication Kit|E.17]]:5.5 - Pin & Publication characteristics (normative; never "axes")
...
* **CharacteristicSpace** (`U.CharacteristicSpace`): a CHR-typed product of slots used by dynamics and measurement theories ([[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]]).
...
* **E17-PC-2 (Lexical discipline - no geometry).** Faces and PCs avoid "axis", "dimension", or geometric metaphors; use **Characteristic**, **slot**, **CharacteristicSpace** where applicable (**[[E.10 - Unified Lexical Rules for FPF|E.10]]**; see also [[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]]).
...
* **E17-PC-Ext-3 (Twin-register names).** Supply **Tech** and **Plain** twins; avoid tokens that collide with [[E.10 - Unified Lexical Rules for FPF|E.10]] bans; do not coin "...Space" names for publication forms.
...
3. Update the relevant **GateChecks** or **GateProfiles** ([[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], including GateCrossing checks and crossing-visibility checks from [[E.18 - Transformation Flow Structure|E.18]], [[F.9 - Alignment & Bridge across Contexts|F.9]], and relevant Part G bridge or crossing wiring) to warn or block on invariant violations; never weaken functorial invariants.
...
### [[E.17 - Multi‑View Publication Kit|E.17]]:5.6 - Author ergonomics (non‑normative)
...
## [[E.17 - Multi‑View Publication Kit|E.17]]:6 - Rules and Invariants (normative)
...
3. **Description-source and specification-use sourcing and EpistemicViewing compatibility ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]] and [[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]], [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]–[[A.6.3 - U.EpistemicViewing - EntityOfConcern-preserving morphism|A.6.3]], [[E.17.0 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]]).**
    ...
    * (b) Each `Emit_s(-)` is realised as a species of `U.EpistemicViewing` ([[A.6.3 - U.EpistemicViewing - EntityOfConcern-preserving morphism|A.6.3]]) over those Description epistemes: entityOfConcern-preserving, effect-free, and conservative in the sense of [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]] and [[A.6.3 - U.EpistemicViewing - EntityOfConcern-preserving morphism|A.6.3]]. Publication adds no new commitments beyond what is present in the referenced Description episteme.
...
5. **No Γ‑leakage (publication independence).**
    Publication morphisms carry **no** Γ\_method, Γ\_time, or Γ_work semantics. Any build, render, or upload activity is **separate `U.Work` by a system on carriers** ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]]).
...
6. **Carrier provenance.**
    Every emitted view records its **SCR/RSCR ids** on first occurrence ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]] §5.6).
...
8. **Cross‑Context and ReferencePlane bridging.**
    * If a view crosses contexts or reference planes, it cites the **Bridge + CL policy ids** ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]] §5.8, bridge crossing). Such crossings are explicit on `TechCard` and `AssuranceLane`.
...
## [[E.17 - Multi‑View Publication Kit|E.17]]:7 - Structure & participants
```
                 Σ_viewpoints
                      │
            ┌─────────┴─────────┐
...
## [[E.17 - Multi‑View Publication Kit|E.17]]:8 - Examples (SoTA-aligned Local Tests)
...
4. **Functional-description publication (`PlainView` + `TechCard`).**
    A principle scheme or functional diagram can publish a readable relation from signature or principle episteme content to method-family selection, selected method, `U.WorkPlan`, performed `U.Work`, work-result record, and result measurement. The MVPK faces can help a team inspect that relation and prepare a work plan, but they do not turn the diagram, table, screen, or export into work occurrence, gate passage, evidence, engineering justification, or supervisory or control architecture. For those uses, the team first recovers the existing typed project-side value named by value and reference that carries the claim when available: work-relevant source restoration under [[A.15.4 - Work-Relevant Source Restoration|A.15.4]], project `U.Method`, `U.WorkPlan`, and dated `U.Work` occurrence under [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] and [[A.15.1 - U.Work|A.15.1]], evidence or provenance path under [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]], engineering-justification record under [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], gate or constraint decision under [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] or [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], or supervisory or control architecture record under [[B.2.5 - Supervisor-Subholon Feedback Loop|B.2.5]]. If no existing source carries the needed claim, create only a prospective repair request, future decision request, prospective work-plan entry, or explicit source-gap note; do not backdate evidence, gate passage, work occurrence, release permission, engineering justification, or assurance for the earlier claim.

## [[E.17 - Multi‑View Publication Kit|E.17]]:9 - Conformance checklist (normative)
...
### [[E.17 - Multi‑View Publication Kit|E.17]]:9.1 - Core ordinary checks
...
| **CC-MVPK-5 (Carrier reference)** | First mention includes **SCR/RSCR** ids when carrier work or rendering work is present. | SCR ids visible on the card when used. |

### [[E.17 - Multi‑View Publication Kit|E.17]]:9.2 - Conditional checks
...
## [[E.17 - Multi‑View Publication Kit|E.17]]:10 - Anti‑patterns (with fixes)
...
## [[E.17 - Multi‑View Publication Kit|E.17]]:11 - Consequences
...
## [[E.17 - Multi‑View Publication Kit|E.17]]:12 - SoTA Alignment: Adopted And Adapted Invariants And Rejected Shortcuts
...
## [[E.17 - Multi‑View Publication Kit|E.17]]:13 - Relations

* **Builds on:** [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] and [[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]] for carrier and front-end discipline plus the EntityOfConcern and Description-episteme boundary and specification-use gates; [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]-[[A.6.3 - U.EpistemicViewing - EntityOfConcern-preserving morphism|A.6.3]] for episteme morphisms, `U.EffectFreeEpistemicMorphing`, and `U.EpistemicViewing`; [[E.17.0 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]] for `U.MultiViewDescribing`; [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] and [[E.10 - Unified Lexical Rules for FPF|E.10]] for authoring and publication-language discipline; Part F and Part G for bridge, terminology, characteristic, and pin discipline.
...
## [[E.17 - Multi‑View Publication Kit|E.17]]:15 - Manager’s one‑page review (copy‑paste)
...
## [[E.17 - Multi‑View Publication Kit|E.17]]:End
