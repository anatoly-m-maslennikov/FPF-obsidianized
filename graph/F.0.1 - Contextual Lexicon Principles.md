...
**Builds on:** [[A.1.1 - U.BoundedContext Semantic Frame|A.1.1]] `U.BoundedContext` (formal frame); [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] *Strict Distinction* (C‑6); [[A.8 - Universal Core Principle (C‑1)|A.8]] *Universal Core* (C‑1); [[A.11 - Ontological Parsimony (C‑5)|A.11]] *Ontological Parsimony* (C‑5); [[A.4 - Temporal Duality & Open‑Ended Evolution Principle|A.4]] *Temporal Duality* (C‑7); **[[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] D.CTX** (lexical discipline for “Context”).
**Coordinates with.** **[[F.1 - Domain‑Family Landscape Survey|F.1]]** (Context Map via Context Cards), **[[F.2 - Term Harvesting & Normalisation|F.2]]** (local term capture), **[[F.3 - Intra‑Context Sense Clustering|F.3]]** (intra‑Context clustering), **[[F.7 - Concept‑Set Table|F.7]]** (Concept‑Set Table), **[[F.9 - Alignment & Bridge across Contexts|F.9]]** (Alignment & Bridge), **[[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]** (Trust & Assurance; CL penalties).

> **Didactic note.** In the Tech register, **Context ≡ `U.BoundedContext`** (per [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]). We use “Context of meaning” as a **metaphor only**; *Context* remains the normative short form for `U.BoundedContext`. The word **anchor** is not used in FPF. The word *plane* is reserved to **CHR:ReferencePlane** only.

**Terminology guard (normative, Part F).** The **row classifier** is **senseFamily**: {Role | Status | Measurement | Type‑structure | Method | Execution}. **Characteristic** (MM‑CHR) names measurable aspects only ([[A.17 - Canonical “Characteristic” (A.CHR‑NORM)|A.17]]–[[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]]) and MUST NOT be used for row typing in Part F. Avoid the generic word **facet** in Part F; when unavoidable, reference **[[C.3.5 - KindAT - Intentional Abstraction Facet for Kinds (K0…K3)|C.3.5]] KindAT (informative facet)** or **Compose‑CAL `U.Facet`** explicitly. Only **CHR:ReferencePlane** is permitted (no bare “plane”); use **EntityOfConcern / Description episteme / specification-use boundary** for entity-description-specification-use discipline; use **stance** for design vs run.

## [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:1 - Problem Frame
...
## [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:2 - The Three Principles (normative)

### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:2.1 - P‑S - **Source Localisation Principle** — *Speak with the Context.*
...
* A finite **Context Map** (see **[[F.1 - Domain‑Family Landscape Survey|F.1]]**) is chosen **before** naming work starts.
...
### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:2.2 - P‑L - **Local Meaning Principle** — *Meaning lives inside the Context.*
...
* **Intra‑Context clustering** of raw mentions precedes any Cross‑context act (see **[[F.3 - Intra‑Context Sense Clustering|F.3]]**).
...
### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:2.3 - P‑B - **Explicit Bridge Principle** — *across Contexts, only with a bridge.*

**Rule.** Any relation between terms from **different** Contexts **MUST** be stated as an **Alignment Bridge** (see **[[F.9 - Alignment & Bridge across Contexts|F.9]]**): a named mapping between **SenseCell⟨-⟩** items with a declared **relation kind** (e.g., *overlaps*, *broader‑than*, *near‑equivalent*) and a **Congruence Level (CL)** for trust calculus ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).
...
* Concept‑Sets ([[F.7 - Concept‑Set Table|F.7]]) are built **from bridged cells**, not from lexical strings.
* When the prose wording uses umbrella sameness/alignment tokens (“same/equivalent/align/map/…”), treat it as an RPR trigger and repair it via **[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] (RPR‑XCTX)** before granting any naming or substitution licence.
...
## [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:3 - Minimal Conceptual Objects (conceptual, notationally neutral)

> These conceptual objects are **thought‑objects**; they specify **what must exist conceptually**, not how it is stored.

### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:3.1 - **Context Card** (for each `U.BoundedContext`)

A terse descriptor used in the **Context Map** ([[F.1 - Domain‑Family Landscape Survey|F.1]]):
...
### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:3.2 - **SenseCell** (unit of local meaning, inside one context)
...
> **Where it comes from.** [[F.2 - Term Harvesting & Normalisation|F.2]] describes how SenseCells can be *derived* from local term evidence; [[F.0.1 - Contextual Lexicon Principles|F.0.1]] only **requires** that local meaning be expressible as a SenseCell.

### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:3.3 - **Alignment Bridge** (between SenseCells from different Contexts)

* `left: SenseCell⟨-@A⟩`, `right: SenseCell⟨-@B⟩`
* `relation` (e.g., *equivalent‑under‑assumptions*, *overlaps*, *broader‑than*)
* `CL` (Congruence Level; feeds [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] Trust & Assurance)
* `loss/fit` (explicit statement of what is lost or assumed)

## [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:4 - Invariants (normative)
...
5. **I‑5 - No implicit Cross‑context identity.** Cross‑context relations exist **only** as [[F.9 - Alignment & Bridge across Contexts|F.9]] Bridges with `relation` and `CL`.
6. **I‑6 - Parsimony & heterogeneity hook.** The Context Map is **finite**, **heterogeneous** (≥ 3 families per unification line), and **parsimonious** ([[F.1 - Domain‑Family Landscape Survey|F.1]]).

## [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:5 - Reasoning Primitives (judgement schemata; pure, side‑effect‑free)
...
* **Bridge‑to‑Concept‑Set hint** *(for [[F.7 - Concept‑Set Table|F.7]])*
  `Bridge(x@A, y@B, rel≈equiv, CL≥k) ⊢ candidate_same_row(x, y)`
  *Reading:* High-CL, near‑equivalence bridges can *nominate* cells for one Concept‑Set row (final decision in [[F.7 - Concept‑Set Table|F.7]]).

## [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:6 - Didactic Metaphor (informative)
...
## [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:7 - Placement & Flow

**[[F.0.1 - Contextual Lexicon Principles|F.0.1]]** is the **front door** of Part F. It enables:
**[[F.1 - Domain‑Family Landscape Survey|F.1]]** (choosing Contexts with Context Cards) → **[[F.2 - Term Harvesting & Normalisation|F.2]]** (deriving SenseCells inside each Context) → **[[F.3 - Intra‑Context Sense Clustering|F.3]]** (stabilising local senses) → **[[F.7 - Concept‑Set Table|F.7]]** (building Concept‑Set rows) → **[[F.9 - Alignment & Bridge across Contexts|F.9]]** (stating Bridges).

## [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:8 - Anti‑patterns & remedies
...
| **A5**  | **Context as type**              | Declaring “PROV‑O is‑a BPMN”                               | Implies inherited meanings between Contexts            | Contexts aren’t types; **no is‑a on Contexts** ([[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]). Use Bridges only                       |
...
| **A7**  | **Row from strings**             | Concept‑Set rows built from lexical forms                  | Homonyms/synonyms contaminate rows                  | Build rows from **SenseCells**; add only cells connected by acceptable Bridges ([[F.7 - Concept‑Set Table|F.7]])      |
...
## [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:9 - Compact worked examples
...
### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:9.1 Enactment × Provenance — *process* vs *activity*
...
### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:9.2 - Control × PLC runtime — *actuation* vs *control output*
...
* **Concept‑Set hint**: *Candidate same‑row* ([[F.7 - Concept‑Set Table|F.7]]) with note: “merge permitted at **CL≥2** threshold.”

### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:9.3 Measurement × Service — *observation* vs *service metric*
...
### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:9.4 Type reasoning — *subclass‑of* (OWL) vs *is‑a (plain)*
...
### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:9.5 Deontics × Access — *permission* vs *role (RBAC)*
...
## [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:10 - Extended reasoning moves (pure judgement schemata)
...
### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:10.1 - Context‑qualified use

`Context(C) ∧ mentions(C, s) ⊢ uses(s@C)`
*If s is used under Context C, we treat it as the local term s\@C.*

### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:10.2 - Sense formation (local)

`uses(t@C) ∧ gloss_C(t) ⊢ SenseCell⟨t@C⟩`
*A Context‑true gloss yields a SenseCell inside C.*

### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:10.3 - Admissible Bridge (creation predicate)
...
### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:10.4 - Bridge composition (minimum CL and relation loss)
...
### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:10.5 - Non‑identity by stance
...
### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:10.6 - Row viability (Concept‑Set candidacy)
...
### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:10.7 - Contradiction sieve
...
### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:10.8 - Non‑bridge implication ban

`name(x) = name(y) ∧ A≠B ⊢ ¬Bridge(x@A, y@B, _, _)`
*String equality across Contexts never implies a Bridge.*

## [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:11 - SCR/RSCR acceptance checks (conceptual)
...
### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:11.1 - SCR — Static conformance
...
### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:11.2 - RSCR — Regression & evolution
...
### [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:11.3 - Publish‑ready summary

An artefact is **ready** with respect to [[F.0.1 - Contextual Lexicon Principles|F.0.1]] when:
...
## [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:12 - Quick reference (didactic)
...
## [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:End
