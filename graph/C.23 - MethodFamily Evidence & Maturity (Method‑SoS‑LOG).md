...
**Builds on.** **[[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]** (MethodFamily registry/selector), **[[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]** (Acceptance & EvidenceProfiles), **[[C.22 - Problem Typing & TaskSignature Assignment (Problem-CHR)|C.22]]** (TaskSignature S2), **[[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]] NQD‑CAL** (QD/illumination), **[[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] E/E‑LOG** (emitters/policies), **[[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]** (Assurance lanes & `R_eff`), **[[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]** (Evidence Graph Ref), **[[E.10 - Unified Lexical Rules for FPF|E.10]]** (LEX), **[[E.18 - Transformation Flow Structure|E.18]]** (GateCrossing / CrossingBundle visibility). **Coordinates with.** **[[G.6 - Evidence Graph & Provenance Ledger|G.6]]** (EvidenceGraph), **[[G.8 - SoS‑LOG Bundles & Maturity Ladders|G.8]]** (LOG bundling), **[[G.9 - Parity and Benchmark Harness|G.9]]** (Parity), **[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]** (Refresh).

## [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]:1 - Problem frame

Families of methods compete inside a CG‑Frame. The selector ([[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]) must **admit, degrade, or abstain** per family **without** universal scores, using **typed** problem descriptors and **auditable** evidence. Maturity of a family (how far it has travelled from “clever idea” to “run‑safe”) must be **visible to LOG** rules yet **separate from thresholds** (which live only in **AcceptanceClauses**, [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]).

## [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]:2 - Problem
...
## [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]:3 - Forces
...
## [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]:4 - Solution — **Method‑SoS‑LOG**: deductive shells over Eligibility & Evidence

### [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]:4.1 - Objects & heads (LEX/I‑D‑S)
...
### [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]:4.2 - Rule schema (normative)
...
(c) all relevant **CAL.AcceptanceClauses** ([[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]) evaluate to true under lawful CHR comparisons,
...
**Note (CAL vs LOG).** CAL‑level **`degrade.order`** (fall‑back to order‑only comparisons) is governed by **[[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]**/**CG‑Spec** and is **not** a LOG mode. **SoS‑LOG never overrides CAL outcomes**; a LOG branch **only narrows** `Scope(G)` or **execution mode** (e.g., `sandbox`, `probe‑only`), it **does not** alter CHR scales or admissible orders.
...
**R5 — Proof hooks.** Every branch **MUST** cite **Evidence Graph Ref** ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]), lane tags (TA/VA/LA), freshness windows, and (if bridged) **Bridge ids + loss notes**; the decision is **SCR‑visible**. When **[[G.6 - Evidence Graph & Provenance Ledger|G.6]] EvidenceGraph** is present, also **publish EvidenceGraph path id(s)** for the branch (admit/degrade/abstain). **No self‑evidence**.
...
### [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]:4.3 - Maturity ladder (poset, not a scalar; Description, not Spec)

Publish a **`MethodFamily.MaturityCardDescription@Context`** (UTS enum ids; **Scale kind = ordinal**; **ReferencePlane declared**). Do **not** embed thresholds here; any **maturity floors** used for admission are authored as **[[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] AcceptanceClause** and referenced by id from R1.
...
**M2.** Transitions **MUST** be justified by **EvidenceGraph** paths (once [[G.6 - Evidence Graph & Provenance Ledger|G.6]] is available) and UTS publication; missing anchors ⇒ no advance.
...
> *Rationale note.* Treating maturity as a **poset** aligns with [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]’s lane calculus and avoids **scalarisation across ordinal/ratio** scales; assurance penalties remain on **R**, never **F/G**.

### [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]:4.4 - Unknowns & Shift classes (tri‑state discipline)

**U1. (LEX).** Enumerations for `Degrade(mode)` and Maturity rungs **MUST** be declared as **closed value sets** and **registered at UTS** (twin labels). **Lexical SD** (**[[E.10 - Unified Lexical Rules for FPF|E.10]]**) applies.
...
**U3.** `ShiftClass` semantics follow **[[C.22 - Problem Typing & TaskSignature Assignment (Problem-CHR)|C.22]]**. If `ShiftClass ∈ {covariate‑shift, concept‑drift, adversarial}` or `unknown`, default outcome is `Degrade(scope‑narrow)` unless a CAL.AcceptanceClause explicitly guards the regime.

### [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]:4.5 - Publication & wiring
...
**W3. Strategy token.** Do **not** mint a `U.Type` “Strategy”; strategy remains a **composition** inside [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] (`Compose`) under **E/E‑LOG**.
**W4.** Selector ([[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]) **consumes** these rules; results appear in the **Dispatcher Report** with reasons in/out and cited anchors/bridges.

## [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]:5 - Archetypal Grounding (Tell–Show–Show)
...
*Didactic anchor.* This mirrors [[C.22 - Problem Typing & TaskSignature Assignment (Problem-CHR)|C.22]]’s typed‑signature discipline and CHR legality (no ordinal means; unit alignment for **ratio**).

> Contemporary ecosystem examples of these families (post‑2015) are organised in **DifferentialEquations.jl**, which exposes multiple solver **families** under one call surface—precisely the pattern [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] expects. ([Journal of Open Research Software][17])
...
*Didactic anchor.* Selector returns a **Pareto set** (no cross‑ordinal weighting), as required by [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]].
...
## [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]:6 - Bias‑Annotation
...
## [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]:7 - Conformance Checklist (normative)

| ID           | Requirement                                                                                                                                                                                | Purpose                                       |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------- |
| **CC‑C23.1** | Each `MethodFamily` **SHALL** publish a `MaturityCard` with rung justification via **[[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]** anchors (lanes, freshness windows) and (if bridged) **Bridge ids** with **CL** and loss notes. | Makes maturity **auditable** and lane‑typed.  |
...
| **CC‑C23.6** | **No thresholds** in CHR or Maturity; thresholds **live only** in **AcceptanceClauses** ([[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]).                                                                                             | Separation of concerns.                       |
...
| **CC‑C23.8** | Publish to **UTS** with twin labels; run **GateCrossing visibility checks** on cited crossings: **CrossingBundle** attestation (**E.18/F.9/F.17/E.17/A.21** where live), **LanePurity**, and **Lexical SD** (**[[E.10 - Unified Lexical Rules for FPF|E.10]]**) under GateChecks/GateProfile (**[[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]**). | Publication & crossing visibility hygiene. |
...
| **CC‑C23.10** | **RSCR tests** cover negative/refusal paths (illegal CHR ops; CG‑Spec gate fail; Bridge missing; **Φ table/policy‑id missing**; **Lexical SD violations ([[E.10 - Unified Lexical Rules for FPF|E.10]])**); ensure **branch coverage** (Admit/Degrade/Abstain, unknown). | |
...
## [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]:8 - Consequences
...
## [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]:9 - Rationale

The ladder and LOG shells align with FPF’s **Assurance calculus**: **F** (form) is governed by artefact kind, **G** (scope) by USM slices, and **R** (reliability) accumulates via WLNK then **Φ(CL)** penalties. Treating maturity as **evidence‑typed rungs**—rather than a “score”—avoids illegal arithmetic and lets **DesignRunTag** values remain separate via `DesignRunTag` discipline ([[A.4 - Temporal Duality & Open‑Ended Evolution Principle|A.4]]) and explicit GateCrossings. This mirrors contemporary **science‑of‑science** insights: replication, benchmarking, and field health indicators are the **currency** of maturity, not anecdote.  ([Science][19])

## [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]:10 - Relations

**Builds on:** **[[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]** (selector consumes these rules), **[[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]** (Acceptance & EvidenceProfiles), **[[C.22 - Problem Typing & TaskSignature Assignment (Problem-CHR)|C.22]]** (S2 typing), **[[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]] NQD‑CAL**, **[[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] E/E‑LOG**, **[[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]** (Assurance tuple & WLNK).
**Publishes to:** **UTS** (MaturityCards, rule ids), **SCR/RSCR** (branch coverage; parity hooks).
**Constrains:** **[[G.8 - SoS‑LOG Bundles & Maturity Ladders|G.8]]** (LOG Bundling must cite MaturityCards), **[[G.9 - Parity and Benchmark Harness|G.9]]** (parity harness draws baselines per rung), **[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]** (refresh windows per rung & decay), **[[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]** (Open‑Ended Family mode for GeneratorFamily).
...
## [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]:End
