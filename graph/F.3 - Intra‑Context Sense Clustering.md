...
**Depends on.** [[F.1 - Domain‑Family Landscape Survey|F.1]] **Domain‑Family Landscape Survey**; [[F.2 - Term Harvesting & Normalisation|F.2]] **Term Harvesting & Normalisation**; [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **Lexical Discipline for “Context” (D.CTX)**; [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] **Strict Distinction**; [[A.11 - Ontological Parsimony (C‑5)|A.11]] **Ontological Parsimony**.
**Coordinates with.** [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] **Role Description**; [[F.7 - Concept‑Set Table|F.7]] **Concept‑Set Table**; [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] **Mint or Reuse Decision**; [[F.9 - Alignment & Bridge across Contexts|F.9]] **Alignment & Bridge Across Contexts**.
**Aliases (informative).** *context‑local clustering*; *Sense consolidation*.

## [[F.3 - Intra‑Context Sense Clustering|F.3]]:1 - Intent & applicability

**Intent.** Consolidate the **context‑local lexical units** from [[F.2 - Term Harvesting & Normalisation|F.2]] into a **small set of Local‑Senses** that actually operate in that **one context (U.BoundedContext)**. Each Local‑Sense receives a crisp, didactic label pair (Tech/Plain) and a short sense statement. The result is an **addressable basis** for later uses (Role Assignment, tables, bridges) that is **still strictly context‑local**.

**Applicability.** Apply **after** [[F.2 - Term Harvesting & Normalisation|F.2]] for any Context that will feed naming (F.4/F.5), decision gates ([[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]), Cross‑context bridges ([[F.9 - Alignment & Bridge across Contexts|F.9]]), or exemplars in Part C. Use again whenever the canon (edition) **shifts usage** enough to split or merge senses **within the same context**.
...
## [[F.3 - Intra‑Context Sense Clustering|F.3]]:2 - Problem Frame

context‑local units (LNF + labels + gloss) from [[F.2 - Term Harvesting & Normalisation|F.2]] often **over‑ or under‑differentiate** meaning:
...
[[F.3 - Intra‑Context Sense Clustering|F.3]] repairs this **inside the Context** by clustering “same sense” and distinguishing “different sense”, with **parsimony**.

## [[F.3 - Intra‑Context Sense Clustering|F.3]]:3 - Forces
...
## [[F.3 - Intra‑Context Sense Clustering|F.3]]:4 - Core idea (didactic)
...
## [[F.3 - Intra‑Context Sense Clustering|F.3]]:5 - Minimal vocabulary (this pattern only)

* **Context** — short for `U.BoundedContext` (per D.CTX).
* **Unit** — a context‑local lexical unit from [[F.2 - Term Harvesting & Normalisation|F.2]] (LNF + Tech/Plain + gloss).
...
## [[F.3 - Intra‑Context Sense Clustering|F.3]]:6 - Solution — how to think the clustering (notation‑free)
...
## [[F.3 - Intra‑Context Sense Clustering|F.3]]:7 - Outputs (conceptual, not clerical)

[[F.3 - Intra‑Context Sense Clustering|F.3]] yields, **per Context**:
...
1. A **small set of Local‑Senses**, each with:
   ...
   * **Inside list** (informative): which Units from [[F.2 - Term Harvesting & Normalisation|F.2]] it consolidates.
...
## [[F.3 - Intra‑Context Sense Clustering|F.3]]:8 - Invariants (normative, lightweight)
...
## [[F.3 - Intra‑Context Sense Clustering|F.3]]:9 - Self‑checks (mental probes)
...
## [[F.3 - Intra‑Context Sense Clustering|F.3]]:10 - Anti‑patterns & remedies
...
| **A2**  | **Cross‑context creep**       | Folding BPMN *process* with PROV *activity* while clustering **inside** BPMN. | Imports foreign usage; violates locality.               | Constrain attention to **one context**; postpone Cross‑context talk to [[F.9 - Alignment & Bridge across Contexts|F.9]].                                             |
...
| **A8**  | **Edition blend**          | Mixing 2011 and 2020 usage under one Local‑Sense.                             | Hidden shifts; brittle bridges later.                   | If usage changed with edition, treat as **different Contexts** ([[F.1 - Domain‑Family Landscape Survey|F.1]]) or distinct Local‑Senses with **edition note**. |
...
| **A10** | **Temporal fudge**         | Treating a design‑time sense as if it were run‑time (or vice versa).          | Category errors at enactment.                           | Respect the Context’s **time stance**; keep senses aligned to *design* or *run* as declared in [[F.1 - Domain‑Family Landscape Survey|F.1]].                  |

## [[F.3 - Intra‑Context Sense Clustering|F.3]]:11 - Local‑Sense Cards (one‑glance form)
...
* **Inside** — which [[F.2 - Term Harvesting & Normalisation|F.2]] Units it consolidates (names only)
* **Counter‑example** — a short use that must **not** be included

## [[F.3 - Intra‑Context Sense Clustering|F.3]]:12 - Worked examples (all **intra‑Context**)

### [[F.3 - Intra‑Context Sense Clustering|F.3]]:12.1 - BPMN 2.0 (workflow Context)
...
### [[F.3 - Intra‑Context Sense Clustering|F.3]]:12.2 - PROV‑O (provenance Context)
...
### [[F.3 - Intra‑Context Sense Clustering|F.3]]:12.3 - ITIL 4 (services Context)
...
### [[F.3 - Intra‑Context Sense Clustering|F.3]]:12.4 - SOSA/SSN (sensing Context)
...
### [[F.3 - Intra‑Context Sense Clustering|F.3]]:12.5 - OWL 2 (types Context)
...
### [[F.3 - Intra‑Context Sense Clustering|F.3]]:12.6 - IEC 61131‑3 (control‑runtime Context)
...
## [[F.3 - Intra‑Context Sense Clustering|F.3]]:13 - Reasoning primitives (judgement schemas, notation‑free)
...
## [[F.3 - Intra‑Context Sense Clustering|F.3]]:14 - Relations

**Builds on:**
[[F.1 - Domain‑Family Landscape Survey|F.1]] **Domain‑Family Landscape Survey** (Contexts fixed); [[F.2 - Term Harvesting & Normalisation|F.2]] **Term Harvesting** (Units ready); [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **D.CTX** (Context discipline); [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] **Strict Distinction**.

**Constrains:**

* **[[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] Role Description.** Role Descriptions **cite SenseCells**; they do **not** invent senses.
* **[[F.7 - Concept‑Set Table|F.7]] Concept‑Set Table.** Rows are built from **SenseCells** (later Cross‑context assembly); intra‑Context clarity here prevents row bloat.
* **[[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] Mint or Reuse Decision.** Decisions compare proposed names to **existing SenseCells** to avoid type inflation.
* **[[F.9 - Alignment & Bridge across Contexts|F.9]] Alignment & Bridge.** Bridges connect **SenseCell ↔ SenseCell** across Contexts; [[F.3 - Intra‑Context Sense Clustering|F.3]] provides the stable endpoints.

**Is used by.**
Part C Extention Patterns to ground examples and invariants in **Context‑true** language.

## [[F.3 - Intra‑Context Sense Clustering|F.3]]:15 - Migration notes (conceptual)
...
3. **Edition change → new Context.** When a new edition **reframes** usage, treat it as a **separate Context** ([[F.1 - Domain‑Family Landscape Survey|F.1]]) and re‑cluster there.
...
6. **Bridge temptation.** Record tensions to bridge **elsewhere**; [[F.3 - Intra‑Context Sense Clustering|F.3]] never resolves Cross‑context relations.

## [[F.3 - Intra‑Context Sense Clustering|F.3]]:16 - Acceptance tests (SCR/RSCR — concept‑level)

### [[F.3 - Intra‑Context Sense Clustering|F.3]]:16.1 - Static conformance (SCR)
...
### [[F.3 - Intra‑Context Sense Clustering|F.3]]:16.2 - Regression (RSCR)
...
## [[F.3 - Intra‑Context Sense Clustering|F.3]]:17 - Didactic close (60‑second recap)

> **Within one context,** collect how the canon actually **uses** a head, not how we wish it did. **Merge** aliases that never lead to different conclusions; **split** uses that do. Give each consolidated use a crisp **Tech** label in the Context’s idiom and a faithful **Plain** label. The pair *(Context, Local-Sense)* is your **SenseCell**—the address later cited by Role Descriptions, tables, and bridges. No Cross‑context mergers here; that job belongs to [[F.9 - Alignment & Bridge across Contexts|F.9]]. Keep senses few, boundaries sharp, and labels honest.

## [[F.3 - Intra‑Context Sense Clustering|F.3]]:End
