...
**Builds on:** [[F.1 - Domain‑Family Landscape Survey|F.1]]–[[F.3 - Intra‑Context Sense Clustering|F.3]] (Contexts → seeds → local senses), [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] (Role Characterisation), [[F.5 - Naming Discipline for U.Types & Roles|F.5]] (Naming), [[F.7 - Concept‑Set Table|F.7]] (Concept‑Set table), [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] (Mint/Reuse decision), [[F.9 - Alignment & Bridge across Contexts|F.9]] (Bridges), [[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]–[[F.12 - Service Acceptance-Work Evidence Link|F.12]] (Status & method/service bindings), [[F.15 - SCR-RSCR Harness for Unification|F.15]] (SCR/RSCR).
**Coordinates with:** [[A.1.1 - U.BoundedContext Semantic Frame|A.1.1]] `U.BoundedContext`, [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] **Strict Distinction**, [[A.8 - Universal Core Principle (C‑1)|A.8]] **Heterogeneity**, [[A.11 - Ontological Parsimony (C‑5)|A.11]] **Ontological Parsimony**, [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] **Role–Method–Work Alignment**.
...
## [[F.17 - Unified Term Sheet (UTS)|F.17]]:1 - Intent & Applicability

**Intent.** Provide a **single, normative table**—the **Unified Term Sheet (UTS)**—that distils the output of [[F.1 - Domain‑Family Landscape Survey|F.1]]–[[F.12 - Service Acceptance-Work Evidence Link|F.12]] into **human‑readable rows**. Each row expresses **one Concept‑Set** unified into **one FPF U.Type** with its **Tech/Plain names** and **cross‑context senses**. The UTS is the *front‑door view* that authors, engineers, and managers use; it replaces scattered notes and eliminates guesswork.

**Applicability.** Produce a UTS **per FPF pattern thread** (e.g., *Role Assignment & Enactment*, *Method quartet*, *Trust & Evidence*). Use it:

* to **name** U.Types and their **Tech/Plain** labels ([[F.5 - Naming Discipline for U.Types & Roles|F.5]]),
* to **teach** the mapping from familiar canons to unified concepts,
* to **audit** coverage and heterogeneity ([[A.8 - Universal Core Principle (C‑1)|A.8]]), and
* to **feed** examples in Parts A/C without re‑explaining terminology.

**Why now.** Earlier F‑patterns define *how to think*. **[[F.17 - Unified Term Sheet (UTS)|F.17]]** defines *what you publish* so others can think with you.

## [[F.17 - Unified Term Sheet (UTS)|F.17]]:2 - Problem Frame
...
3. **Coverage is opaque.** No quick check that coverage spans **≥ 3 domain families** across the sheet ([[A.8 - Universal Core Principle (C‑1)|A.8]]).
...
## [[F.17 - Unified Term Sheet (UTS)|F.17]]:3 - Forces
...
| **Parsimony vs. recall**          | Each row is one concept; the UTS as a whole demonstrates heterogeneity across ≥ 3 domain families ([[A.8 - Universal Core Principle (C‑1)|A.8]]). Rows may cite fewer Contexts when the concept truly appears in fewer.                   |
| **Locality vs. comparability**    | Senses are **Context‑scoped** ([[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]). Cross‑context relations are shown only as **explicit bridges** ([[F.9 - Alignment & Bridge across Contexts|F.9]]) with **CL**. |
| **General usability**             | Sheet must be **legible on paper** and **memorisable** (block structure, stable row order).                         |

## [[F.17 - Unified Term Sheet (UTS)|F.17]]:4 - Core Idea

**A UTS is a Concept‑Set table with names.**
Each **row** = one **Concept‑Set** unified into one **FPF U.Type** (the “what we mean”).
Each **column family** shows **how this concept appears** in the chosen **contexts of meaning** ([[F.1 - Domain‑Family Landscape Survey|F.1]]).
...
## [[F.17 - Unified Term Sheet (UTS)|F.17]]:5 - Minimal Vocabulary (for this pattern)

* **UTS (Unified Term Sheet).** The published, human‑readable table per thread.
* **Context.** Alias in Tech register for **`U.BoundedContext`** ([[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]). Normative unit of meaning; every SenseCell is scoped to a Context _(name + edition)._
* **Bounded‑Context Column (BCC).** A didactic column used **only in Layout A**; one column per **Context (`U.BoundedContext`)** from the [[F.1 - Domain‑Family Landscape Survey|F.1]] cut; **not a model element**; the **header includes the Context name + edition**.
...
* **Bridge / CL.** Explicit cross‑context mapping ([[F.9 - Alignment & Bridge across Contexts|F.9]]) with Congruence Level and Loss note.
* **Plain Twin (LEX).** The LEX record pairing the **Unified Tech name** with its **Unified Plain name** for a U.Type; governed by **PTG** and referenced by `Twin‑Map Id (LEX)` ([[E.10 - Unified Lexical Rules for FPF|E.10]] LEX‑BUNDLE).
* **Block Plan.** Didactic grouping of rows to keep the sheet memorizable.
* **Unified Tech name / Unified Plain name.** Dual‑register names chosen per [[F.5 - Naming Discipline for U.Types & Roles|F.5]]; the **Tech name is the neutral, unified term** for the U.Type, not a borrowed Context name.

> **Discipline.** “Context” always means **`U.BoundedContext`** ([[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]). No global words.

## [[F.17 - Unified Term Sheet (UTS)|F.17]]:6 - Row Schema (normative)
...
| **Unified Tech name**     | Short technical name used in spec prose ([[F.5 - Naming Discipline for U.Types & Roles|F.5]]).                                                        |
| **Unified Plain name**    | Everyday name for non‑specialists ([[F.5 - Naming Discipline for U.Types & Roles|F.5]]).                                                              |
| **Plain‑Twin Governance (PTG) (optional)** | Stance for the Unified Plain twin: {**Strict**, **Guarded**, **Provisional**}; use when additional discipline of the Plain twin is required ([[E.10 - Unified Lexical Rules for FPF|E.10]] LEX‑BUNDLE). |
...
| **Bridges (CL/Loss)**     | For any cross‑context relation, record the [[F.9 - Alignment & Bridge across Contexts|F.9]] Bridge with **CL** and a 2–6‑word loss note; if identity, mark **CL=3 (identity)**. |
...
**Constraint.** “SenseCells (by **Context**)” **MUST** cite **at least three** distinct **Contexts** overall across the sheet for the thread ([[A.8 - Universal Core Principle (C‑1)|A.8]]). A single row may show fewer if the concept truly appears in fewer contexts; coverage is a property of the whole UTS.

**Discipline:** Every SenseCell **must** cite the **Context name + edition** (e.g., _“BPMN 2.0 (2011): Activity instance”_).

### [[F.17 - Unified Term Sheet (UTS)|F.17]]:6.1 - NQD Fields (normative; when applicable)
...
### [[F.17 - Unified Term Sheet (UTS)|F.17]]:6.2 - Autonomy fields (when applicable)
...
## [[F.17 - Unified Term Sheet (UTS)|F.17]]:7 - Block Plan (didactic grouping)
...
## [[F.17 - Unified Term Sheet (UTS)|F.17]]:8 - Column Families (two canonical layouts)

### [[F.17 - Unified Term Sheet (UTS)|F.17]]:8.1 - Layout A — Kernel‑first (U.Type as rows)
...
### [[F.17 - Unified Term Sheet (UTS)|F.17]]:8.2 - Layout B — Base‑concept pivot (discipline columns)
...
## [[F.17 - Unified Term Sheet (UTS)|F.17]]:9 - Invariants (normative constraints)

1. **Locality.** Every SenseCell is **Context‑scoped** ([[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]). No global synonyms.
2. **Bridges only via [[F.9 - Alignment & Bridge across Contexts|F.9]].** Cross‑context equivalence appears **only** as an explicit Bridge with **CL**. Any row citing > 1 **Context** must state at least one Bridge.
3. **Heterogeneity.** Across the UTS, coverage must involve **≥ 3 domain families** ([[F.1 - Domain‑Family Landscape Survey|F.1]] Step 2; [[A.8 - Universal Core Principle (C‑1)|A.8]]).
...
6. **Dual register.** Every row has **Tech** and **Plain** labels per [[F.5 - Naming Discipline for U.Types & Roles|F.5]].
...
8. **Unified naming neutrality.** The **Unified Tech name** is the neutral FPF choice per [[F.5 - Naming Discipline for U.Types & Roles|F.5]]; it is **not** lifted wholesale from any single Context unless the Concept‑Set justification ([[F.7 - Concept‑Set Table|F.7]]) shows identity.
...
## [[F.17 - Unified Term Sheet (UTS)|F.17]]:10 - How to Compile (conceptual moves, not a workflow)

**M1 - Fix contexts ([[F.1 - Domain‑Family Landscape Survey|F.1]]).** Declare the **12 (±)** contexts for this thread.
**M2 - Harvest & cluster ([[F.2 - Term Harvesting & Normalisation|F.2]]–[[F.3 - Intra‑Context Sense Clustering|F.3]]).** Identify candidate senses per Context; cluster *within* Contexts; mint **SenseCells**.
**M3 - Form Concept‑Sets ([[F.7 - Concept‑Set Table|F.7]]).** For each “the‑same‑thing” across Contexts, create one **CSR**; attach SenseCells.
**M4 - Name ([[F.5 - Naming Discipline for U.Types & Roles|F.5]]).** Choose **Tech/Plain** labels; assert the **FPF U.Type** (or propose a new one via [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]).
**M5 - Bridge ([[F.9 - Alignment & Bridge across Contexts|F.9]]).** Where Cross‑context relations are not exact, assert Bridges with **CL** and a short **Loss** note.
**M6 - Place rows into blocks (§7).** Keep the sheet memorisable.
**M7 - Write one‑line `FPF Description` and the `Rationale`.**
**M8 - Run acceptance harness ([[F.15 - SCR-RSCR Harness for Unification|F.15]]).** Apply the UTS checks in §11.

> **Note.** These are **thought moves**. No tooling is implied or required.

## [[F.17 - Unified Term Sheet (UTS)|F.17]]:11 - Acceptance Harness (SCR/RSCR) for a UTS

### [[F.17 - Unified Term Sheet (UTS)|F.17]]:11.1 - Static Conformance Rules (SCR‑UTS)
...
* **SCR‑UTS‑05 (Bridge honesty).** All cross‑context sameness claims are expressed via an [[F.9 - Alignment & Bridge across Contexts|F.9]] **Bridge** with a **CL** score; if identity, mark **CL=3** and note “identity/no loss” rather than omitting the bridge.
...
* **SCR‑UTS‑09 (Unified naming).** Each row’s **Unified Tech name** complies with [[F.5 - Naming Discipline for U.Types & Roles|F.5]] rules (dual register, minimal generality, morphology); it is not a mere alias of one Context unless justified by an [[F.9 - Alignment & Bridge across Contexts|F.9]] Bridge with **CL=3**.
...
### [[F.17 - Unified Term Sheet (UTS)|F.17]]:11.2 - Regression Rules (RSCR‑UTS)
...
* **RSCR‑UTS‑B (Name stability).** Tech labels change only with a documented [[F.5 - Naming Discipline for U.Types & Roles|F.5]] decision; Plain labels may evolve didactically if the Tech name stays.
...
* **RSCR‑UTS‑E (Plain discipline).** No per‑column Plain text appears in BCC/DC columns; any additional Plain aliases are tracked in Annex with **tv:** entries and counted against the alias budget ([[F.13 - Lexical Continuity & Deprecation|F.13]]).

## [[F.17 - Unified Term Sheet (UTS)|F.17]]:12 - Canonical Heading Templates (fill with your Contexts/Discipline columns)
...
## | Block | FPF U.Type | Unified Tech name | Unified Plain name | Plain‑Twin Governance (PTG) | Twin‑Map Id (LEX) | FPF Description
  | BCC‑1 (Context name, edition) | BCC‑2 (Context name, edition) | BCC‑3 (Context name, edition) | … (more BCCs from the [[F.1 - Domain‑Family Landscape Survey|F.1]] cut)
...
_(Use the actual Contexts from your [[F.1 - Domain‑Family Landscape Survey|F.1]] cut; always include the edition.)_
...
## [[F.17 - Unified Term Sheet (UTS)|F.17]]:13 - Didactic Aids
...
## [[F.17 - Unified Term Sheet (UTS)|F.17]]:14 - Micro Examples (one line each, illustrative)
...
## [[F.17 - Unified Term Sheet (UTS)|F.17]]:15 - Relations

* **Builds on:** [[F.1 - Domain‑Family Landscape Survey|F.1]]–[[F.3 - Intra‑Context Sense Clustering|F.3]] (contexts & local senses), [[F.7 - Concept‑Set Table|F.7]] (Concept‑Set), [[F.5 - Naming Discipline for U.Types & Roles|F.5]] (names), [[F.9 - Alignment & Bridge across Contexts|F.9]] (Bridges).
* **Feed:** Part A and Part C definitions/examples (row ids used as cross‑refs); teaching bundles ([[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]).
* **Constrained by.** [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] **Strict Distinction**, [[A.11 - Ontological Parsimony (C‑5)|A.11]] **Parsimony**, **[[E.10 - Unified Lexical Rules for FPF|E.10]] §6 Twin‑Register Discipline** (Tech/Plain), **[[E.10.P - Conceptual Prefixes policy & registry|E.10.P]] (prefix registry: tv: / ut:)**, [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **Context discipline**.

## [[F.17 - Unified Term Sheet (UTS)|F.17]]:16 - Migration Notes
...
* **Name evolution.** Update **Plain** labels freely for pedagogy; change **Tech** labels only via [[F.5 - Naming Discipline for U.Types & Roles|F.5]] with clear S‑rules.

## [[F.17 - Unified Term Sheet (UTS)|F.17]]:17 - FAQ (authoring hygiene)
...
*A.* Only via **[[F.9 - Alignment & Bridge across Contexts|F.9]] Bridge** with **CL=3**. Identity must be argued, not assumed by spelling.

**Q3. Where do state‑graphs ([[A.2.5 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]) show up?**
...
## [[F.17 - Unified Term Sheet (UTS)|F.17]]:18 - 90‑Second Teaching Script
...
## [[F.17 - Unified Term Sheet (UTS)|F.17]]:End
