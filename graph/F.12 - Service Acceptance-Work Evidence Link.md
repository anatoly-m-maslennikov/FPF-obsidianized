...
**Builds on:** [[F.1 - Domain‑Family Landscape Survey|F.1]] **context of meaning (U.BoundedContext)**; [[F.2 - Term Harvesting & Normalisation|F.2]] **Term Harvesting**; [[F.3 - Intra‑Context Sense Clustering|F.3]] **Intra‑Context Sense Clustering**; [[F.5 - Naming Discipline for U.Types & Roles|F.5]] **Naming Discipline**; F.7/F.9 **Bridges & CL**; [[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] **Status Families & Windows**; [[F.11 - Method Quartet Harmonisation|F.11]] **Method Quartet Harmonisation**; [[A.2.3 - U.PromiseContent (Promise Content)|A.2.3]] **U.PromiseContent**.
...
## [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:1 - Intent & applicability
...
## [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:2 - Problem frame
...
## [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:3 - Forces
...
## [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:4 - Core idea (didactic)
...
4. **Window** — the explicit period in which the judgement is made ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]).
...
## [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:5 - Minimal vocabulary (this pattern only)
...
* **Window.** A time envelope (calendar month, batch, incident interval, shift) per [[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]].
...
## [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:6 - The binding, as five mental rules (notation‑free)
...
**R5 — Window rule.**
Every verdict is **time‑bounded**. Changing the Window can change the result; **no retroactivity** from new clauses or specs (cf. [[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]).

## [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:7 - Clause templates (conceptual schemata)
...
## [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:8 - Invariants (normative)

1. **DesignRunTag split.** Clauses live on the **design stance**; judgements live on the **run stance** about **Work** ([[F.11 - Method Quartet Harmonisation|F.11]]).
...
9. **No detached pass.** When the ClauseCell is a `U.PromiseContent` (service promise clause), `Satisfied` about a Work is admissible only if that Work also **delivers** the promised outcome spec for the clause ([[A.2.3 - U.PromiseContent (Promise Content)|A.2.3]]:8.1 `fulfilsPromiseContent`). This keeps acceptance on the **same delivery evidence base** (Work facts + Δ anchors + Observations) and prevents “pass verdict separate from delivery”.

## [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:9 - Micro‑examples (didactic, multi‑domain)

### [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:9.1 - SaaS uptime (services + sensing)
...
### [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:9.2 - Furnace band (industrial control)
...
### [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:9.3 - Incident MTTR (services + enactment)
...
## [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:10 - Anti‑patterns & remedies
...
| **A4**  | **Global words**                | *Availability*, *latency* used without a Context.                                               | Collides senses across disciplines.             | Speak with **Context prefixes** ([[F.1 - Domain‑Family Landscape Survey|F.1]]). Cross‑context reuse demands a **Bridge** ([[F.9 - Alignment & Bridge across Contexts|F.9]]).                                                                         |
...
| **A11** | **Status on “the service”**     | Tagging an abstract umbrella like *“the service”* as “Satisfied”.                            | Loses the entityOfConcern of the judgement.           | Attach to **ClauseCell\@Window about WorkCell**; the service concept remains a promise vocabulary ([[A.2.3 - U.PromiseContent (Promise Content)|A.2.3]]).                                               |
| **A12** | **Bridge‑by‑name**              | Equating *Activity* ≡ *Process* because both say “process”.                                  | Assumes global meaning.                         | Use **[[F.9 - Alignment & Bridge across Contexts|F.9]] Bridge** with **kind/CL/Loss**; or keep them distinct.                                                                                         |

## [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:11 - Extended worked examples
...
### [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:11.1 - CDN latency across regions (services + sensing + types)
...
### [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:11.2 - Stroke care: door‑to‑needle (method + enactment + status)

* **Contexts.** *clinical guideline canon* (Clause), *PROV‑O* (Work), *SOSA/SSN* (Measure), *[[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]* (status windows).
...
### [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:11.3 - Cold‑chain warehouse (control + sensing + deontics)
...
### [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:11.4 - SaaS incident MTTR (services + enactment)
...
## [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:12 - Reasoning primitives (judgement schemas, notation‑free)
...
7. **CL aggregation (assurance hint)**
   ...
   *Reading:* The weakest Bridge governs the assurance level communicated with the verdict (advisory to [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] calculus).
...
## [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:13 - Relations (with other patterns)

* **Builds on:**
  **[[F.1 - Domain‑Family Landscape Survey|F.1]]** (Contexts): keeps all meanings **local**.
  **[[F.2 - Term Harvesting & Normalisation|F.2]]–[[F.3 - Intra‑Context Sense Clustering|F.3]]**: provide the **SenseCells** that become Clause/Work/Measure anchors.
  **[[F.5 - Naming Discipline for U.Types & Roles|F.5]]**: ensures labels for Clause/Work/Measure and Windows are didactically clear.
  **[[F.7 - Concept‑Set Table|F.7]]–[[F.9 - Alignment & Bridge across Contexts|F.9]]**: supply **Bridge** kinds / **CL** and loss semantics.
  **[[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]**: defines **Status** families and **Window** constructs.
  **[[F.11 - Method Quartet Harmonisation|F.11]]**: protects **Method**, **MethodDescription**, **Work**, and **Actuation** distinctions.
...
* **Constrains:**
  Later reporting and assurance rules ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]) must **not** collapse CL/Loss; they report them alongside status.

## [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:14 - Migration notes (conceptual)
...
## [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:15 - Acceptance tests (SCR/RSCR — concept‑level)

### [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:15.1 - Static conformance (SCR)
...
### [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:15.2 - Regression checks (RSCR)
...
### [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:15.3 Didactic distillation (60‑second recap)
...
## [[F.12 - Service Acceptance-Work Evidence Link|F.12]]:End
