...
**Tag.** Architectural pattern (foundational Standard; constrains [[G.1 - CG‑Frame‑Ready Generator|G.1]]–[[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]])
...
**Non-duplication note.** Universal Part‑G invariants are governed by [[G.Core - Part G Core Invariants|G.Core]] and are satisfied here **only via delegation** (`CC‑G0‑CoreRef` → `CC‑GCORE‑*`). Single‑governing definition CN/CG spec-ref discipline is enforced via `CC‑GCORE‑CN‑CG‑1` (no shadow specs; no competing defaults).

## [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:1 - Problem frame

A team defines or evolves a `CG‑Frame` (e.g., a frame for creativity measurement, decision quality, architecture trade‑offs, or selected-set publication). Downstream mechanisms ([[G.1 - CG‑Frame‑Ready Generator|G.1]]–[[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] and beyond) must compare, aggregate, and publish CHR‑typed observations in ways that are:
...
## [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:2 - Problem
...
## [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:3 - Forces
...
## [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:4 - Solution — CG‑Spec as the design-time legality gate
...
### [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:4.1 - [[G.Core - Part G Core Invariants|G.Core]] linkage (normative)

**Builds on:** [[G.Core - Part G Core Invariants|G.Core]] (Part-G core invariants; governing-pattern citation)
...
### [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:4.2 - CG‑Spec object model (normative)
...
### [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:4.3 - CG‑Spec conceptual model (normative)
...
⟨
  ...
  CNSpecRef := ⟨[[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]] ref, CNSpecRef.edition⟩,          // CN‑Spec is the governance card (one governing definition)

  Characteristics := [CHR.Characteristic.id…],          // pointers only; authored in [[G.3 - CHR Authoring for a CG‑Frame- Characteristics, Scales, Levels, Coordinates|G.3]] CHR pack
  ...
  // Semantics (tri-state, penalties→R_eff-only, crossing visibility, set-return) are governed by [[G.Core - Part G Core Invariants|G.Core]].
  CL‑Routing := ⟨policy_id, map Bridge.CL → penalty_spec⟩,
  Φ := ⟨phi_policy_id, phi_table_ref?, psi_policy_id?, phi_plane_policy_id?⟩,

  AcceptanceStubs := [AcceptanceStubId…],     // templates only; thresholds remain governed by CAL ([[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]])
...
* `MinimalEvidenceEntry` MUST declare: lane requirements, evidence carriers, freshness window (if any), and explicit failure behavior wiring. The semantics of `{pass|degrade|abstain}` and `degrade(mode=…)` are delegated to [[G.Core - Part G Core Invariants|G.Core]].

### [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:4.4 - Interfaces (normative)

| Interface          | Consumes                             | Produces / constrains                                                      |
| ------------------ | ------------------------------------ | -------------------------------------------------------------------------- |
| **[[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]‑1 Charter**  | CG‑Frame brief, USM scope signals    | `CG‑Spec.Scope`, `entityOfConcern`, `ReferenceMap`                         |
| **[[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]‑2 SCP**      | CHR pack refs ([[G.3 - CHR Authoring for a CG‑Frame- Characteristics, Scales, Levels, Coordinates|G.3]]), legality proofs | `CG‑Spec.SCP` + bindings to lawful operators/aggregators                   |
| **[[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]‑3 Evidence** | SoTA inputs ([[G.2 - SoTA Harvester & Synthesis|G.2]]), carriers ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]])   | `CG‑Spec.MinimalEvidence`, `Γ‑fold` segment pins, `CL‑Routing`, `Φ` ids    |
| **[[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]‑4 Publish**  | All above                            | Versioned `CG‑Spec@UTS` plus Name Cards, public-id continuity records, and RSCR tests and trigger kinds  |
| **[[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]‑5 Expose_CrossingHooks** | `CG‑Spec` + crossing/plane/policy pins | GateCrossing inputs for `GateChecks` (`E.18/A.21`): plane checks, lane purity, lexical SD pins |
| **→ [[G.1 - CG‑Frame‑Ready Generator|G.1]]**          | `CG‑Spec`                            | Generator guardrails (Comparator/SCP/MinEv pins); degrade/abstain wiring   |
| **→ [[G.2 - SoTA Harvester & Synthesis|G.2]]**          | `CG‑Spec`                            | Harvesting inclusion/exclusion and crossing policy constraints             |
| **→ [[G.3 - CHR Authoring for a CG‑Frame- Characteristics, Scales, Levels, Coordinates|G.3]]**          | `CG‑Spec`                            | Required CHR characteristics/scales/operators to exist                     |
| **→ [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]**          | `CG‑Spec`                            | Acceptance templates; evidence minima; Γ‑fold override proof hooks         |
| **→ [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]**          | `CG‑Spec`                            | Eligibility gates and explainability pins (Path/UTS/policy ids)            |
| **→ [[G.6 - Evidence Graph & Provenance Ledger|G.6]]**          | `CG‑Spec`                            | EvidenceGraph/SCR pinning surface (policy ids + Path/PathSlice discipline) |

### [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:4.5 - CG‑Spec authoring chassis (informative)
...
4. **Declare MinimalEvidence.** For each characteristic: required lanes/carriers, freshness window, crossing allowances (if any), and explicit failure behavior wiring (tri-state semantics delegated to [[G.Core - Part G Core Invariants|G.Core]]).
...
### [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:4.6 - Extensions (pattern-scoped; non-core)
...
* **GoverningPatternId:** [[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]]
* **Uses:** `{[[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]]}`
* **⊑/⊑⁺:** `∅`
...
* **GPatternExtensionKind:** `InteropSpecific`
* **GoverningPatternId:** [[F.9 - Alignment & Bridge across Contexts|F.9]]
* **Uses:** `{[[F.9 - Alignment & Bridge across Contexts|F.9]], [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]}`
* **⊑/⊑⁺:** `∅`
* **RequiredPins/EditionPins/PolicyPins (minimum):**
...
* **Notes (wiring-only):** Crossing semantics and penalty routing are delegated to [[G.Core - Part G Core Invariants|G.Core]]; this module only lists the required pins used by `CG‑Spec` entries.
...
* **Notes (wiring-only):** Any SoTA palette/tradition semantics are governed by [[G.2 - SoTA Harvester & Synthesis|G.2]]. [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]] only requires that `CG‑Spec` entries cite the needed SoTA artefacts for auditability.
...
* **Uses:** `{[[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]}`
* **⊑/⊑⁺:** `∅`
* **RequiredPins/EditionPins/PolicyPins (minimum):**
...
## [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:5 - Archetypal Grounding — Tell–Show–Show; System / Episteme

### [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:5.1 - Archetype 1: System comparability under mixed evidence and unit constraints
...
* declares explicit failure behavior wiring (tri-state semantics delegated to [[G.Core - Part G Core Invariants|G.Core]]),
...
### [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:5.2 - Archetype 2: Epistemic comparability for selected-set publication across traditions
...
* makes `AcceptanceStubs` explicit as templates while locating thresholds in CAL ([[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]),
* ensures RSCR triggers are emitted when comparator or policy pins change.

## [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:6 - Bias-Annotation
...
## [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:7 - Conformance Checklist (normative)

| ConformanceId | Statement |
| --- | --- |
| **CC‑G0‑CoreRef** | [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]] is conformant only if the applicable core obligations listed in `G.0:4.1` are satisfied (delegation to `CC‑GCORE‑*`; no shadow specs, no competing defaults, typed RSCR triggers, explicit pins). |
...
| CC‑G0‑07 | If crossing penalties are used, `CL‑Routing` and `Φ` policy ids are explicit and auditable (policy ids are exposed as pins/refs) **and are required pins for downstream SCR publication on penalised claims** (see [[G.6 - Evidence Graph & Provenance Ledger|G.6]]). |
| CC‑G0‑08 | `AcceptanceStubs` in `CG‑Spec` are templates only; any context-local thresholds/acceptance policies are governed by CAL acceptance artefacts ([[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]) and are cited, not duplicated. |
...
| **CC‑G0‑CSLC** | Scale/unit/polarity legality MUST be proven before any aggregation; illegal arithmetic on ordinal/nominal values is nonconformant. (Governed by the relevant legality patterns; [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]] only binds and cites.) |

## [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:8 - Common Anti-Patterns and How to Avoid Them
...
## [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:9 - Consequences
...
## [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:10 - Rationale

`CG‑Spec` centralises frame-level comparability constraints so that:

* CHR authorship ([[G.3 - CHR Authoring for a CG‑Frame- Characteristics, Scales, Levels, Coordinates|G.3]]) remains about *measurement meaning* rather than implicit thresholds,
* CAL ([[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]) governs context-local acceptance/threshold policies and proof ledgers,
* selectors and dispatchers ([[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]) remain policy-governed and auditable rather than encoding hidden legality assumptions,
* refresh ([[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]) can treat legality edits and pin changes as explicit causes with canonical trigger ids.

## [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:11 - SoTA‑Echoing
...
## [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:12 - Relations

**Builds on:** [[G.Core - Part G Core Invariants|G.Core]], `A.19 (CN‑Spec)`, `A.10 (evidence carriers)`, `A.17–[[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]] / [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]] (MM‑CHR legality)`, `A.18 (CSLC)`, `B.3 (trust / Γ‑fold family)`, `F.* (contexts, bridges, CL, UTS)`, `E.10 (lexical rules)`, `E.5.* (notation independence discipline)`.
**Used by:** [[G.1 - CG‑Frame‑Ready Generator|G.1]] (generator guards), [[G.2 - SoTA Harvester & Synthesis|G.2]] (harvesting constraints), [[G.3 - CHR Authoring for a CG‑Frame- Characteristics, Scales, Levels, Coordinates|G.3]] (required CHR), [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] (acceptance templates / proof hooks), [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] (eligibility gates), [[G.6 - Evidence Graph & Provenance Ledger|G.6]] (evidence/pin surfaces), and downstream parity/shipping/refresh where `CG‑Spec` is pinned.
**Publishes to:** `UTS` (Name Cards + editioned `CG‑Spec` segments).

## [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]:End
