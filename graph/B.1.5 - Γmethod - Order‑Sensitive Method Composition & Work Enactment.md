...
> **► decided‑by: [[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] Advanced Mereology**
**[[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] compliance —** Methods compose over **SerialStepOf/ParallelFactorOf** on **MethodDescription/Method** graphs (order, not parthood); stuff‑like inputs are modelled via **PortionOf** on resources and accounted in **Γ_work**; method/version history uses **PhaseOf**; mapping quality is handled via **CL** ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).
...
## [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:1 - Problem frame

* **Strict Distinction ([[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]])** separates **what a holon is** (structure), **how steps are ordered** (order), **how it unfolds** (time), **what it spends** (work/resources), and **what it values** (objectives).
* **Method, MethodDescription, and Work.**

  * **Method** is the **timeless semantic “way of doing”** (a context‑scoped capability; [[A.3.1 - U.Method- Context-Defined Way of Doing|A.3.1]]): it specifies admissible preconditions, effects, and bounds, independent of any particular run.
  * **MethodDescription** is a **design‑time description** of a Method (knowledge on a carrier). It may be an **imperative step‑graph** (this pattern’s focus) or another admissible description form (functional/logical/dynamics/solver, etc.; [[A.3.2 - U.MethodDescription- Description Episteme for a Way of Doing|A.3.2]]:4.2).
  * **Work** is the **dated run‑time occurrence** that enacts a pinned MethodDescription under a `U.RoleAssignment`, records concrete **slot fillings** (parameters/carriers), and books the **resource ledger** ([[A.15.1 - U.Work|A.15.1]]).
...
* **[[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] (Role–Method–Work Alignment)** supplies the **typed ordered relations** we need: **SerialStepOf** (strict precedence) and **ParallelFactorOf** (order‑concurrent branches with a join).
* **[[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]] (Γ\_ctx/Γ\_time)** already handles **non‑commutativity** (order matters) and **temporal slicing**; **[[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]] (Γ\_work)** handles **resource spending** and **efficiency**.
  **Γ\_method** sits **between** them: it composes methods **by order and capability** and **delegates** resource accounting to **Γ\_work**.

## [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:2 - Problem
...
5. **Synergy by arithmetic.** Throughput or quality jumps caused by **proper joins** or **coordination** are misreported as simple sums or averages—violating WLNK and obscuring when a **Meta‑Holon Transition ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]])** should be declared.

## [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:3 - Forces
...
| **Parsimony vs. separation of concerns** | Keep Γ small ↔ Keep **work** and **assurance** in their own lanes (Γ\_work, [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).                       |

## [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:4 - Solution

### [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:4.1 - Terms (didactic recap)

* **U.MethodDescription** — a design‑time description of a `U.Method` ([[A.3.2 - U.MethodDescription- Description Episteme for a Way of Doing|A.3.2]]): typically an imperative **step‑graph** with **SerialStepOf/ParallelFactorOf**, step **capability types**, **pre/post‑conditions**, and required **external interactions**. (Other admissible description forms exist; [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]] focuses on the step‑graph case.)
* **U.Method** — the timeless semantic “way of doing” (capability) described by ≥1 MethodDescription and enacted as `U.Work` ([[A.3.1 - U.Method- Context-Defined Way of Doing|A.3.1]], [[A.15.1 - U.Work|A.15.1]]).
* **U.Work** — the run‑time, dated enactment occurrence: `performedBy → U.RoleAssignment`, `isExecutionOf → U.MethodDescription` (edition‑pinned), plus concrete slot fillings and resource ledger ([[A.15.1 - U.Work|A.15.1]]).
...
### [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:4.2 - The operator family (two companion flavours)
...
1. **Planning (design‑time) — compose specifications**
   ...
   * **Domain.** `D_spec` contains step specifications linked by **SerialStepOf** / **ParallelFactorOf** (**[[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]**).
   * **Result.** A single **U.MethodDescription** whose **MIC** is computed from step interfaces using the **Promote / Forward / Encapsulate** quartet (cf. BIC in [[B.1.2 - System‑specific Aggregation Γ-sys|B.1.2]]). The resulting MethodDescription **SHALL** declare the `U.Method` it describes ([[A.3.2 - U.MethodDescription- Description Episteme for a Way of Doing|A.3.2]]); in the step‑graph case this is the semantic serial/parallel composition of the described `StepMethod`s ([[A.3.1 - U.Method- Context-Defined Way of Doing|A.3.1]]:9).
...
### [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:4.3 - Core aggregation rules (design‑time composition)
...
3. **MIC synthesis (boundary behaviour).**
   For each external interaction of a step, decide **Promote / Forward / Encapsulate** into the composite **MIC**. This inherits the clarity of BIC ([[B.1.2 - System‑specific Aggregation Γ-sys|B.1.2]]) for methods.
...
4. **Assurance hooks (without computing assurance).**
   Record where **[[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] assurance** will later hang: (i) the **cutset** steps that bound reliability/quality, (ii) the **integration edges** whose **CL** will penalise poor fit (mappings, fragile joins), and (iii) the **envelope** (G) intended for the method’s validity.
...
### [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:4.4 - Core aggregation rules (run‑time enactment)

When executing **Γ\_method^run(M\_spec, RA, Fill)**:

1. **Role–Method–Spec alignment ([[A.2 - Role Taxonomy|A.2]] / [[A.3 - Transformer Constitution (Quartet)|A.3]] / [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]).**
   Confirm that `RA.role` is eligible to enact the `U.Method` described by `M_spec` (or a declared equivalent/refinement in the same context), and that the Work’s `performedBy` and `executedWithin` anchors can be satisfied ([[A.15.1 - U.Work|A.15.1]]). If this fails, you may still record an attempted run, but it is **not** a conformant “execution of `M_spec`”.
...
> * **WLNK:** quality/throughput of the composite is bounded by the **critical path** steps (identified for later [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] assurance).
> * **MONO:** strengthening a step (better pre/post, stronger type, improved adapter) **cannot** make the composite worse.

### [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:4.5 - Didactic contrasts (to prevent common confusions)
...
* **Step vs part vs specialization.**
  ...
  – Use **ComponentOf/PartOf** for structural wholes ([[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]).
  – Use **`≤ₘ` refinement / equivalence / substitution** for Method specialisation ([[A.3.1 - U.Method- Context-Defined Way of Doing|A.3.1]]).
  – Use **Kind‑CAL (`⊑`)** for kind/subkind.

* **Method vs Phase.**
  Method composition is **order**; **PhaseOf** (Γ\_time) is **temporal progression** of the **same carrier**. If a phase boundary also introduces **closure/supervision/context rebase**, that is **MHT** ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]), not mere phasing.

* **MethodDescription vs Work.**
  Keep **planning** artefacts (MethodDescription) separate from **run‑time occurrences** (Work). `Γ_method^plan` produces MethodDescriptions; `Γ_method^run` produces Work that cites an edition‑pinned MethodDescription and records effective slot fillings and ledgers ([[A.15.1 - U.Work|A.15.1]]).

## [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:5 - Archetypal grounding (worked, didactic)

### [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:5.1 - System archetype — **Assemble‑Paint‑Test** as one Method
...
* **Assurance hooks ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).**
...
### [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:5.2 - Episteme archetype — **Evidence‑Synthesis‑Publish** as one Method
...
## [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:6 - Method Interface Standard (MIC) — template & examples

### [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:6.1 - MIC template (normative content)
...
### [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:6.2 - MIC excerpts (didactic)
...
## [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:7 - Proof obligations (normative)
...
4. **PO‑PLAN‑CL‑POINTS.** Identify integration edges whose **CL** will matter for [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]; record intended sources of mapping evidence.
...
5. **PO‑RUN‑ASSURANCE.** Provide the observed values for the cutset steps and the actual **CL** of integration mappings to feed [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] assurance.

## [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:8 - Conformance Checklist (normative)
...
| **CC‑B1.5.7** | If joining branches produces apparent super‑additivity beyond WLNK not explainable within Γ\_method, an **MHT** **SHALL** be considered and recorded per [[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]. | Prevent “synergy by arithmetic”.    |

## [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:9 - Anti‑patterns & repairs
...
| **Emergence by Join**  | Throughput leaps past WLNK with no story                      | Either (i) prove within Γ\_method (cutset/CL/order) or (ii) declare **MHT** ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]). |

## [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:10 - Consequences

**Benefits**

* **Didactic clarity.** Readers see **what** is being composed (order & capability) vs **what** is spent (Γ\_work) vs **what** is assured ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).
...
## [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:11 - Rationale (informative)
...
* **Strict Distinction.** The split between **Method** (semantic), **MethodDescription** (spec), **Work** (occurrence), **Γ\_method** (order/type checks), **Γ\_work** (resource ledgers), and **assurance** implements [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]], preventing category errors (semantics vs execution vs claims).
* **Mereology alignment.** Using **SerialStepOf / ParallelFactorOf** ([[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]) keeps method composition orthogonal to structural composition (**ComponentOf**) and temporal phasing (**PhaseOf**).
* **Assurance readiness.** Identifying cutsets and mapping CL points during planning makes [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] application straightforward and auditable.
* **Interfaces matter.** MIC prevents accidental coupling and makes integration points auditable.
* **Separation of concerns.** Γ\_method composes behaviour; Γ\_work accounts resources; [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] assesses quality—keeping algebraic reasoning sound.

## [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:12 - Relations

* **Builds on:** [[A.12 - External Transformer & Reflexive Split|A.12]] (Transformer Role), [[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] (Mereology Extension), [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] (Strict Distinction); [[B.1.1 - Dependency Graph & Proofs|B.1.1]] (Proof Kit), [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]] (Γ\_ctx/Γ\_time).
* **Coordinates with:** [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]] (Γ\_work) for resource accounting; [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (Assurance) for WLNK cutsets and CL penalties.
* **Triggers/Complements:** [[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]] (MHT) when new closure/supervision or context re‑base appears at method level.
...
## [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]:End
