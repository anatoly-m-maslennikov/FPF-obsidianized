...
## [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:1 - Problem frame

Use [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] when an object version is to be improved or judged, but the evaluation that says what "better" means is not yet available, not yet explicit, or not yet adequate for the object.

[[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]] says how a `CharacteristicSpace` is structured: declared characteristics, declared scales, slots, value sets, declared coordinate groups, and no hidden normalization or aggregation. [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] says how to make such a `CharacteristicSpace` for the evaluated object, so that an evaluation can later evaluate that object and [[E.23 - Quality Improvement Loop Method|E.23]] can run an improvement loop without inventing values.
...
**Not this pattern when.** If a suitable evaluation already exists, cite it and use [[E.22 - Improvement-Oriented Quality Evaluation Question Framing|E.22]] for question framing or [[E.23 - Quality Improvement Loop Method|E.23]] for repeated improvement. Use [[A.17 - Canonical “Characteristic” (A.CHR‑NORM)|A.17]], [[A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC‑KERNEL)|A.18]], and [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]] when the live problem is one characteristic, one scale, or measurement legality. Use [[C.16.P - Characteristic and Scale Precision Restoration|C.16.P]] first when candidate coordinate wording still hides whether the use under repair is a characteristic, scale, coordinate, score, metric label, quality-term repair, or governing-pattern relation. Use [[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]] when the live problem is the structure of `CharacteristicSpace` itself. Use [[C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]] when the evaluated EntityOfConcern is a composite engineering quality family that already fits Q-Bundle form. Use [[F.18 - Local‑First Unification Naming Protocol|F.18]] when the live problem is durable naming. Use [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]], [[E.9.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]], or [[E.2.DA - FPF Pillar-Adequacy Evaluation CharacteristicSpace|E.2.DA]] when the evaluated EntityOfConcern is respectively one FPF pattern version, one `DRR`, or one FPF-level Pillar-adequacy evaluated EntityOfConcern.
...
**What this buys.** [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] gives improvement work a way to create the missing evaluation before the loop starts. It keeps [[E.23 - Quality Improvement Loop Method|E.23]] universal and simple: [[E.23 - Quality Improvement Loop Method|E.23]] changes the object and asks an evaluation to re-evaluate it; [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] helps build that evaluation when none is yet adequate.
...
## [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:2 - Problem
...
## [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:3 - Forces
...
| **Measurement legality vs ordinal judgment** | Some coordinates are measured through [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]]; others are evidence-backed ordinal content values. The evaluation must say which is which. |
...
## [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:4 - Solution
...
### [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:4.1 - Local names and kind settlement
...
| `E23StartCondition` | Minimum condition for using this evaluation inside [[E.23 - Quality Improvement Loop Method|E.23]]. | Not the improvement loop itself. |
...
### [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:4.2 - Construction moves
...
11. **Start [[E.23 - Quality Improvement Loop Method|E.23]] only after evaluation values exist.** A repeated improvement loop can start only when the evaluated object version, evidence basis, result form, and evaluation are recoverable enough for re-evaluation.

### [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:4.3 - Evaluation specification minimum

[[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] does not prescribe a publication or record form. It states which evaluation characteristic-space elements must be recoverable before an evaluation characteristic space is reusable for judgement or improvement. The selected publication or record form may be an FPF pattern, local engineering standard, rubric, table, review form, model card section, protocol note, or project rule, but that form is not governed here. The evaluation characteristic-space specification must make these items recoverable by value:
...
This minimum is a content requirement, not a file-format requirement. For an FPF pattern publication form, [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] still governs the authoring form. [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] only states what the evaluation must make recoverable so that [[E.22 - Improvement-Oriented Quality Evaluation Question Framing|E.22]] can frame an improvement-oriented quality evaluation and [[E.23 - Quality Improvement Loop Method|E.23]] can run a repeated improvement loop.
...
### [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:4.4 - Discriminating-case test
...
### [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:4.5 - Scale-set improvement
...
Use [[E.23 - Quality Improvement Loop Method|E.23]] for the repeated improvement method over the scale set when the improvement aim is live. The evaluation for that meta-level improvement may be:

- this pattern's conformance checklist for whether the scale set is constructible and usable;
- [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] when the evaluation characteristic-space specification is itself an FPF pattern version;
- [[E.9.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]] when the decision record selecting the scale set is the `DRR` decision-adequacy object being evaluated;
- [[E.2.DA - FPF Pillar-Adequacy Evaluation CharacteristicSpace|E.2.DA]] when the scale set changes FPF-level Pillar adequacy;
- [[F.18 - Local‑First Unification Naming Protocol|F.18]] when the live problem is name choice for the scale-set heads;
- [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]], [[A.17 - Canonical “Characteristic” (A.CHR‑NORM)|A.17]], [[A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC‑KERNEL)|A.18]], or [[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]] when the live problem is measurement or characteristic-space legality.
...
## [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:5 - Worked slices

**Show, FPF pattern quality.** The evaluated object kind is one FPF pattern version. The existing evaluation is [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]], so [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] stays closed unless [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] itself is being redesigned. [[E.23 - Quality Improvement Loop Method|E.23]] may improve the pattern version under [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]].

**Show, DRR adequacy.** The evaluated object kind is one `DRR` version for a declared campaign-decision use. The existing evaluation is [[E.9.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]]. If a campaign needs a different DRR adequacy coordinate, [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] can test whether that coordinate belongs inside [[E.9.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]], another evaluation, or no current FPF pattern.

**Show, FPF Pillar adequacy.** The evaluated object is FPF as a corpus or release candidate. [[E.2 - The Eleven Pillars|E.2]] gives the Pillars; [[E.2.DA - FPF Pillar-Adequacy Evaluation CharacteristicSpace|E.2.DA]] is the evaluation. [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] explains why [[E.2.DA - FPF Pillar-Adequacy Evaluation CharacteristicSpace|E.2.DA]] needs evaluated object, use, eligibility, coordinates, evidence loci, stop meanings, and neighbour governing relations rather than a Pillar essay.

**Show, name improvement.** The evaluated object is a durable term candidate. [[F.18 - Local‑First Unification Naming Protocol|F.18]] already supplies a grouped lexical quality vector: `SemanticFidelity`, `CognitiveErgonomics`, `MorphologicalActionFit`, and `AliasRisk`, plus NQD discipline over candidate names. [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] treats [[F.18 - Local‑First Unification Naming Protocol|F.18]] as an existing local evaluation for naming, not as a reason to build another one.

**Show, no evaluation yet.** A team says "make this onboarding method better" but cannot say better for whom, by what values, or with what stop. [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] opens before [[E.23 - Quality Improvement Loop Method|E.23]]: it names evaluated object kind, user, use, contrast cases, candidate characteristics, scales, floors, missingness, protected trade-offs, and neighbour governing relations. Only then can [[E.22 - Improvement-Oriented Quality Evaluation Question Framing|E.22]] frame an improvement-oriented quality evaluation and [[E.23 - Quality Improvement Loop Method|E.23]] improve the method.

## [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:6 - Conformance checklist
...
| `CC-A19ECS-9` | If made reusable beyond one local use, the evaluation characteristic-space specification SHALL make the minimum items in `A.19.ECS:4.3` recoverable by value. If the selected publication form is an FPF pattern, [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] also applies to that publication form. | Prevents underspecified evaluations. |
...
| `CC-A19ECS-11` | The evaluation characteristic-space specification SHALL state declarative governing-neighbour relations named by value for evidence, assurance, gate, work, decision, publication, naming, measurement, Q-Bundle, OEE/NQD, and mathematical-lens claims when those claims are being made. It SHALL NOT call neighbouring patterns receivers, owners, hosts, homes, routes, or exits, and SHALL NOT turn reference apparatus or architecture-placement reasoning into the coordinate Solution. | Prevents an evaluation from becoming a second ontology or route catalogue. |
...
## [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:7 - Common anti-patterns
...
## [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:8 - Relations

| Pattern | Relation |
|---|---|
| [[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]] | Defines `CharacteristicSpace`. [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] gives the method for constructing one evaluation `CharacteristicSpace` for an evaluated object. |
| [[A.17 - Canonical “Characteristic” (A.CHR‑NORM)|A.17]], [[A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC‑KERNEL)|A.18]], [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]] | Govern characteristics, scales, scale values, coordinates, measures, units, and measurement legality. [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] uses them by reference for each slot. |
| [[C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]] | Governs Q-Bundle normal form for composite engineering quality families. [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] may select or repair the characteristic-space part before a Q-Bundle endpoint is used. |
| [[E.22 - Improvement-Oriented Quality Evaluation Question Framing|E.22]] | Frames one improvement-oriented quality-evaluation question after an evaluation is declared. [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] constructs the missing or inadequate evaluation. |
| [[E.23 - Quality Improvement Loop Method|E.23]] | Governs repeated improvement after evaluated object version and evaluation are declared. [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] provides the evaluation when it is missing or underdesigned. |
| [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] | Existing evaluation for one FPF pattern version. [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] explains the construction shape but does not replace [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]]. |
| [[E.9.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]] | Existing evaluation for one `DRR` decision-adequacy claim. [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] does not replace it. |
| [[E.2.DA - FPF Pillar-Adequacy Evaluation CharacteristicSpace|E.2.DA]] | Existing evaluation for FPF-level Pillar adequacy. [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] explains why it must publish evaluated object, coordinates, values, evidence loci, status, and stop meanings. |
| [[F.18 - Local‑First Unification Naming Protocol|F.18]] | Existing naming discipline with a grouped lexical quality vector. Use [[F.18 - Local‑First Unification Naming Protocol|F.18]] for durable term and name improvement. |
| [[C.16.P - Characteristic and Scale Precision Restoration|C.16.P]], [[C.16.Q - Quality-Term Precision Restoration|C.16.Q]], [[E.10 - Unified Lexical Rules for FPF|E.10]], [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]], [[C.2.P - Epistemic Precision Restoration|C.2.P]] | Repair overloaded characteristic/scale/score, quality, lexical, relation, and source-use wording before it becomes a coordinate or status value. |
| [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.9 - Parity and Benchmark Harness|G.9]], [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] | Govern OEE/NQD novelty, diversity, archive, pool, selected-set, parity, and refresh semantics. An evaluation may supply `Q` values, but it does not govern the rest of OEE/NQD. |
| [[C.29 - Mathematical Lens Use|C.29]] | Governs mathematical-lens use when a mathematical structure is used to define or justify coordinates. |
| [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]], [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]], [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] | Govern evidence, assurance, local CV, gates, and work when an evaluation result is reused for those claims. |

## [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:9 - Consequences

A conforming [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] result lets [[E.22 - Improvement-Oriented Quality Evaluation Question Framing|E.22]] ask a useful improvement-oriented quality-evaluation question and lets [[E.23 - Quality Improvement Loop Method|E.23]] run a repeated improvement loop without inventing values during the loop. It also gives object-specific evaluation patterns such as [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]], [[E.9.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]], [[E.2.DA - FPF Pillar-Adequacy Evaluation CharacteristicSpace|E.2.DA]], and [[F.18 - Local‑First Unification Naming Protocol|F.18]] a common construction shape: evaluated object kind, use, contrast cases, coordinates, value meanings, evidence basis, result-row shape, calibration points, coordinate-specific payloads, protected trade-offs, status meanings, and local stop or reopen condition.
...
| Improvement pressure can damage the intended value when the evaluation is a weak proxy. | Current proxy-risk anchors: `Goodhart's Law in Reinforcement Learning` (ICLR 2024) and current catastrophic-Goodhart reward-misspecification work (NeurIPS 2024); retained lineage: Goodhart taxonomy. | [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] requires evidence rules, missingness rules, protected trade-offs, and lowering/reopen conditions before a loop can treat a value as improved. | It is not an anti-measurement rule; it makes the measurement or ordinal evaluation explicit enough to be challenged. |
| OEE/NQD separates the quality side from novelty, diversity, archive, pool, and selected-set semantics. | Current QD and OEE/NQD neighbour basis: QD uses quality pressure with novelty/diversity and archive/front practice, while current FPF [[C.17 - Characterising Generative Novelty & Value (Creativity‑CHR)|C.17]], [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.9 - Parity and Benchmark Harness|G.9]], and [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] keep archive, pool, selected-set, parity, and refresh semantics named by value. | The evaluation may supply `Q` values, while novelty, diversity, archive, front, pool, selected-set publication, parity, and refresh remain with neighboring pattern governing the claims. | [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] does not govern OEE/NQD generation, selection, archive, parity, or refresh. |

## [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:11 - Rationale
...
Placing this method under [[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]] keeps the ontology clean. [[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]] governs the structure of `CharacteristicSpace`; [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] governs the construction method for evaluations of declared EntityOfConcern kinds and uses. [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] governs the selected characteristics, scales, coordinate construction, and evaluation-use boundaries of the evaluation characteristic space, not its publication or record form. An FPF pattern is only one possible publication form when the evaluation belongs in FPF; a local rubric, standard, table, or project rule is enough when the use is local. [[E.23 - Quality Improvement Loop Method|E.23]] stays a universal loop method because it does not need to know how every domain chooses its scales. Domain and FPF-specific evaluations such as [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]], [[E.9.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]], [[E.2.DA - FPF Pillar-Adequacy Evaluation CharacteristicSpace|E.2.DA]], and [[F.18 - Local‑First Unification Naming Protocol|F.18]] keep coordinate choices inside those evaluations.

## [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]:End
