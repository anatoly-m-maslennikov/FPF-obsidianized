...
**Primary hooks.** see **§12 Relations** (notably [[G.Core - Part G Core Invariants|G.Core]], [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]], [[G.2 - SoTA Harvester & Synthesis|G.2]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.10 - SoTA Pack Shipping|G.10]], [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]])
**Working‑model first (informative).** prefer working models and didactic micro‑examples; escalate to formal harnesses only when risk warrants (per [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]).
**Non‑duplication note.** Universal Part‑G invariants (tri‑state guard, set-return, penalties→`R_eff`‑only, crossing visibility, typed RSCR triggers, Default Governing Definition Index, P2W split, linkage discipline, shipping boundary) are governed in [[G.Core - Part G Core Invariants|G.Core]] and are **only cited** here.
...
**Neighboring FPF patterns.** Use [[G.2 - SoTA Harvester & Synthesis|G.2]] for the local SoTA set, [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] for governed set-return selection, [[G.10 - SoTA Pack Shipping|G.10]] for shipping surfaces, [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] for refresh wiring, and [[F.17 - Unified Term Sheet (UTS)|F.17]] when the result must also land on a human-facing UTS surface.

**Common wrong neighboring-pattern changes.** If the real entry load is only a one-off governed comparison or shortlist, use [[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]], [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]], or [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]; if the real entry load is project alignment rather than kit authoring, use [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]; if tooling choice is being treated as the first kit candidate, keep the case here only after the chassis and its bindings are explicit.

## [[G.1 - CG‑Frame‑Ready Generator|G.1]]:1 - Problem frame
...
* harvesting semantics are governed by [[G.2 - SoTA Harvester & Synthesis|G.2]],
* selection/dispatch semantics are governed by [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]],
* CHR/CAL payload semantics are governed by [[G.3 - CHR Authoring for a CG‑Frame- Characteristics, Scales, Levels, Coordinates|G.3]] / [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]],
* shipping semantics are governed by [[G.10 - SoTA Pack Shipping|G.10]],
* refresh orchestration governing definition is [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]].

## [[G.1 - CG‑Frame‑Ready Generator|G.1]]:2 - Problem
...
## [[G.1 - CG‑Frame‑Ready Generator|G.1]]:3 - Forces
...
* **Didactic clarity vs. normative minimalism:** authors need a concrete scaffold, but universal invariants must not be duplicated outside [[G.Core - Part G Core Invariants|G.Core]].

## [[G.1 - CG‑Frame‑Ready Generator|G.1]]:4 - Solution

### [[G.1 - CG‑Frame‑Ready Generator|G.1]]:4.1 - [[G.Core - Part G Core Invariants|G.Core]] linkage (normative)

```
// Canonical form: see [[G.Core - Part G Core Invariants|G.Core]] (Nil‑elision + Expansion rule for profiles/sets/pin‑sets).
...
**Citation rule (normative):** `CC‑GCORE‑*`, `RSCRTriggerKindId.*`, and `DefaultId.*` semantics are governed by their canonical definitions: primarily [[G.Core - Part G Core Invariants|G.Core]], and for the defaults above the definitions listed in `G.Core.DefaultGoverningDefinitionIndex`. [[G.1 - CG‑Frame‑Ready Generator|G.1]] MUST NOT restate or redefine those semantics.

### [[G.1 - CG‑Frame‑Ready Generator|G.1]]:4.2 - Six‑module generator chassis (normative)
...
`CGKitId` identifies the versioned **kit manifest** (`CG‑Kit@CG‑Frame`) that lists the six card ids and the minimal wiring pins needed to treat the chassis as a reusable unit (this is **not** a shipping pack; shipping remains governed by [[G.10 - SoTA Pack Shipping|G.10]]).
...
* explicit provenance anchors for the set (via [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]), and any published UTS stubs/rows when applicable

**Governing pattern:** harvesting discipline and SoTA-pack payload are governed by [[G.2 - SoTA Harvester & Synthesis|G.2]].
In [[G.1 - CG‑Frame‑Ready Generator|G.1]], M2 is a *slot in the chassis* and a wiring surface; it does not redefine the harvesting method.
...
**Guardrails (via [[G.Core - Part G Core Invariants|G.Core]]):**

* tri‑state eligibility handling, penalties routing, crossing visibility, and set‑return constraints are not defined here; they are enforced via [[G.Core - Part G Core Invariants|G.Core]] conformance.

**Governing pattern for method payload:** method‑specific emitter semantics are governed by `Extensions` (e.g., [[C.17 - Characterising Generative Novelty & Value (Creativity‑CHR)|C.17]], [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]]).
...
* optional **front metadata or archive metadata** needed for reproducibility when used: ε‑front parameters and/or archive snapshot hooks, with governing-definition assignment through [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] / [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]] / [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] (no local semantics in [[G.1 - CG‑Frame‑Ready Generator|G.1]])

**Governing pattern:** selection/dispatch semantics are governed by [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]].
M4 MUST preserve *set‑return semantics* (as governed by [[G.Core - Part G Core Invariants|G.Core]]) and MUST NOT hard‑code a forced singleton outcome.
...
* an index of referenced CG‑Frame artefacts ready for reuse:
  ...
  * CHR/CAL/LOG bundles (by their ids; semantics governed by [[G.3 - CHR Authoring for a CG‑Frame- Characteristics, Scales, Levels, Coordinates|G.3]], [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]], [[G.8 - SoS‑LOG Bundles & Maturity Ladders|G.8]])
  ...
  * RSCR test identifiers (e.g., from [[F.15 - SCR-RSCR Harness for Unification|F.15]]) and worked examples (where applicable)

**Boundary:** M5 is a **kit/library surface**, not shipping. If a shipped pack is needed, governing-definition assignment is [[G.10 - SoTA Pack Shipping|G.10]].
...
**Boundary:** orchestration semantics are governed by [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]].
M6 prepares *refresh‑readiness metadata* and wiring stubs; it does not define scheduling/priority heuristics.

### [[G.1 - CG‑Frame‑Ready Generator|G.1]]:4.3 - Minimal I/O surface (normative)

| Module | Consumes                                                                    | Produces                                                                               |
| ------ | --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| M1     | CG‑Frame brief + `entityOfConcern` + `CNSpecRef/CGSpecRef` (edition‑pinned) | `CG‑FrameContext` + context pins                                                       |
| M2     | discovery inputs + inclusion criteria *(via [[G.2 - SoTA Harvester & Synthesis|G.2]])*                           | `SoTA_SetId` (+ provenance anchors; optional UTS stubs/rows)                           |
...
| M6     | telemetry inputs + freshness/decay policy pins + RSCR tests                 | `CGKitId` + `RefreshReadinessCardId` (wiring to [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]; no orchestration governance)    |

### [[G.1 - CG‑Frame‑Ready Generator|G.1]]:4.4 - Extensions (pattern‑scoped; non‑core)
...
* `SoTAPaletteDescriptionId`
* `SoTA_SetId`
* `ClaimSheetId[]` / `BridgeMatrixId` *(as referenced by the chosen [[G.2 - SoTA Harvester & Synthesis|G.2]] pack form)*
* `CNSpecRef.edition`, `CGSpecRef.edition` *(already required via `GCorePinSetId.PartG.AuthoringMinimal`)*
**RSCRTriggerSetIds:** `{GCoreTriggerSetId.SoTAHarvestSynthesis}`
**Notes (wiring‑only):** harvesting semantics (living review funnels, inclusion policy families, SoS indicator families, etc.) are defined by [[G.2 - SoTA Harvester & Synthesis|G.2]] and are not duplicated in [[G.1 - CG‑Frame‑Ready Generator|G.1]].
...
**GoverningPatternId:** [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]
**Uses:** `{[[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]}`
**⊑/⊑⁺:** `∅`
...
* `AcceptanceClauseId[]` *(as referenced from [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] outputs)*
...
**Notes (wiring‑only):** [[G.1 - CG‑Frame‑Ready Generator|G.1]] does not redefine selection: it binds M4’s output surface to the [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] selector/dispatcher kernel.
...
**GoverningPatternId:** [[C.17 - Characterising Generative Novelty & Value (Creativity‑CHR)|C.17]]
**Uses:** `{[[C.17 - Characterising Generative Novelty & Value (Creativity‑CHR)|C.17]], [[G.3 - CHR Authoring for a CG‑Frame- Characteristics, Scales, Levels, Coordinates|G.3]]}`
**⊑/⊑⁺:** `∅`
**RequiredPins/EditionPins/PolicyPins (minimum):**

* `CHRPackId?` *(if creativity characteristics are published/typed)*
* edition/policy pins required by the chosen creativity characteristic set (governed by [[C.17 - Characterising Generative Novelty & Value (Creativity‑CHR)|C.17]])

**Notes (wiring‑only):** [[G.1 - CG‑Frame‑Ready Generator|G.1]] only records which creativity characteristics are used for M3/M4 wiring; legality/typing lives in the CHR governing definitions.
...
* `EmitterPolicyRef` *(policy‑id/ref; points to the exploration governance governing definition, e.g., [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] when E/E‑LOG is used)*

**RSCRTriggerKindIds:** `{RSCRTriggerKindId.EditionPinChange, RSCRTriggerKindId.PolicyPinChange, RSCRTriggerKindId.TelemetryDelta, RSCRTriggerKindId.FreshnessOrDecayEvent}`

**Notes (wiring‑only):** QD/QD‑adjacent algorithm families and their parameterisations belong to `C.18 and C.19`; [[G.1 - CG‑Frame‑Ready Generator|G.1]] only fixes the pins needed to make the VariantPool and Shortlist reproducible.
...
**GPatternExtensionKind:** `GeneratorSpecific`
**GoverningPatternId:** [[G.2 - SoTA Harvester & Synthesis|G.2]] *(family semantics are governed by SoTA cards; this block only wires pins; selector-side wiring is governed by [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]].)*
**Uses:** `{[[G.2 - SoTA Harvester & Synthesis|G.2]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]}`
**⊑/⊑⁺:** `∅`
**RequiredPins/EditionPins/PolicyPins (minimum):**
...
**Notes (wiring‑only):** this block enables declared sets of `{Environment, MethodFamily}` pairs without redefining generator semantics in [[G.1 - CG‑Frame‑Ready Generator|G.1]]; it should cite/align with the selector‑side wiring in `G.5:Ext.OpenEndedFamilyWiring`.
...
**GPatternExtensionKind:** `GeneratorSpecific`
**GoverningPatternId:** [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]
**Uses:** `{[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]}`
**⊑/⊑⁺:** `∅`
**RequiredPins/EditionPins/PolicyPins (minimum):**
...
**Notes (wiring‑only):** M6 declares readiness and wiring; orchestration semantics (queueing, prioritisation, cadence) are governed by [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]].
...
**GPatternExtensionKind:** `GeneratorSpecific`
**GoverningPatternId:** [[G.10 - SoTA Pack Shipping|G.10]]
...
**Notes (wiring‑only):** this block does not define shipping; it only records the minimum wiring from the chassis/library index to [[G.10 - SoTA Pack Shipping|G.10]] when shipping is performed.

## [[G.1 - CG‑Frame‑Ready Generator|G.1]]:5 - Archetypal Grounding — Tell–Show–Show (informative)
...
* **M2:** build `SoTA_SetId` via [[G.2 - SoTA Harvester & Synthesis|G.2]] using a living‑review style funnel (e.g., PRISMA‑like trace + update cadence) and publish UTS stubs for reusable constructs.
...
* **M4:** produce `ShortlistId` as a selected-set / shortlist surface via [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], with acceptance predicates sourced from [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]].
* **M5:** publish a `CGFrameLibraryId` indexing the chosen CHR/CAL/LOG bundles and UTS rows; register RSCR tests.
* **M6:** declare refresh readiness (telemetry pins + canonical RSCR trigger kinds) and wire to [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]].
...
* **M2:** harvest SoTA models and safety constraints as a reconstructible set (governed by [[G.2 - SoTA Harvester & Synthesis|G.2]]).
...
## [[G.1 - CG‑Frame‑Ready Generator|G.1]]:6 - Bias‑Annotation (informative)

* **Recency bias:** “newest paper wins” (mitigate with explicit inclusion criteria and update cadence in [[G.2 - SoTA Harvester & Synthesis|G.2]] wiring).
...
* **Scalarisation bias:** collapsing selected sets or partial orders into a single score (mitigate by set‑return discipline pinned through [[G.Core - Part G Core Invariants|G.Core]]).
* **Hidden‑crossing bias:** implicit reuse across contexts (mitigate by explicit crossing pins and Bridge‑only routing via [[G.Core - Part G Core Invariants|G.Core]]).

## [[G.1 - CG‑Frame‑Ready Generator|G.1]]:7 - Conformance Checklist (normative)

| ConformanceId     | Statement   |
| ----------------- | ----------- |
| **CC‑G1‑CoreRef** | The pattern MUST satisfy the **effective** `CoreConformanceIds` implied by `G.1:4.1` (`GCoreConformanceProfileId` expansion + deltas), per [[G.Core - Part G Core Invariants|G.Core]] expansion rules.   |
...
| CC‑G1‑03          | `M2` MUST be wired to [[G.2 - SoTA Harvester & Synthesis|G.2]] (or explicitly cite the [[G.2 - SoTA Harvester & Synthesis|G.2]] artefacts governed by cited patterns) and MUST be reconstructible as a scoped set, including `SoTAPaletteDescriptionId` + `SoTA_SetId` (not free‑floating prose). Provenance MUST be anchored via [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] for the emitted set.  |
| CC‑G1‑04          | `M3` MUST record emitter provenance as a wiring surface, including `EmitterPolicyRef` (policy‑id/ref), edition pins, and provenance anchors (via [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]). Any method‑specific fields MUST be introduced only via `GPatternExtension` blocks.   |
| CC‑G1‑05          | `M4` MUST be wired to [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] (or explicitly cite [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] artefacts governed by cited patterns) and MUST preserve set-result outcomes. `SCRId` MUST be present (or explicitly cited to the governing definition surface) so assurance is id‑addressable; `DRRId` SHOULD be present when a decision‑rationale artefact is minted.   |
...
| CC‑G1‑07          | `M6` MUST publish `CGKitId` and expose refresh‑readiness wiring: canonical `RSCRTriggerKindId[]` applicability + minimal payload pins (including `SlotFillingsPlanItemRef[]` when applicable) and RSCR test ids; orchestration semantics MUST be cited to [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]].  |
| CC‑G1‑08          | Any method/discipline/generator specificity in [[G.1 - CG‑Frame‑Ready Generator|G.1]] MUST be located in `G.1:4.4` as `GPatternExtension` blocks with `PatternScopeId`, `GPatternExtensionKind`, and `GoverningPatternId` (or `governing pattern not yet selected` only for Phase-3 seeds). If QD/illumination or Open‑Ended generator families are declared, the corresponding extension blocks MUST be present and MUST carry the edition and policy pins required by the governing pattern. |

## [[G.1 - CG‑Frame‑Ready Generator|G.1]]:8 - Common Anti‑Patterns and How to Avoid Them (informative)
...
* **Anti-pattern: “Packaging implies shipping governance.”**
  *Avoid:* treat M5 as a library index; treat M6 as readiness wiring; ship only via [[G.10 - SoTA Pack Shipping|G.10]].

## [[G.1 - CG‑Frame‑Ready Generator|G.1]]:9 - Consequences (informative)
...
## [[G.1 - CG‑Frame‑Ready Generator|G.1]]:10 - Rationale (informative)
...
* **Why “kit/index” rather than “pack”?** A CG‑Frame authoring effort must stay modular; shipping is a separate governing boundary ([[G.10 - SoTA Pack Shipping|G.10]]).
...
## [[G.1 - CG‑Frame‑Ready Generator|G.1]]:11 - SoTA‑Echoing (informative)

This chassis is designed to stay compatible with modern (post‑2015) practice without confusing “SoTA” with “currently popular”:

* **Evidence synthesis:** living systematic review protocols (e.g., PRISMA‑style traceability and update cadence) map naturally to M2 wiring governed by [[G.2 - SoTA Harvester & Synthesis|G.2]].
* **Quality‑Diversity and archives:** modern QD families (MAP‑Elites‑class, CMA‑ME‑class, and related archive‑based exploration) fit as M3/M4 extensions ([[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]/[[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]]) because they require explicit descriptor/distance/insertion pins and preserve set‑valued outcomes.
...
* **Set‑valued decision outputs:** modern multi‑objective and set‑valued evaluation practices align with the [[G.Core - Part G Core Invariants|G.Core]] set‑return discipline, preventing hidden scalarisation.
...
## [[G.1 - CG‑Frame‑Ready Generator|G.1]]:12 - Relations

**Builds on:** [[G.Core - Part G Core Invariants|G.Core]], [[E.8 - FPF Authoring Conventions & Style Guide|E.8]], [[E.10 - Unified Lexical Rules for FPF|E.10]], [[E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]].
**Uses:** `A.10 (Provenance Anchors)`, `A.15.3 (SlotFillingsPlanItem)`, `A.19 (CN‑Spec)`, `G.0 (CG‑Spec)`, `G.2 (SoTA Synthesis Pack)`, `G.3 (CHR Pack@CG‑Frame)`, `G.4 (CAL Pack@CG‑Frame)`, `G.5 (Selector & Dispatch)`, `G.10 (Shipping)`, `G.11 (Refresh Orchestration)`, and (via Extensions) `C.17, [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], and C.19`.
...
## [[G.1 - CG‑Frame‑Ready Generator|G.1]]:End
