...
> **Status:** Stable
> **Normativity:** Normative
> **Placement:** Part A → [[A.6 - Signature Stack & Boundary Discipline|A.6]] (Precision restoration / stack discipline)
> **Builds on:** [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]] (RPR recipe), [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] (slot discipline), [[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]] (L/A/D/E claim classification), [[A.2.3 - U.PromiseContent (Promise Content)|A.2.3]] (`U.PromiseContent`), [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]] (`U.Commitment`), [[A.2.9 - U.SpeechAct (Communicative Work Object)|A.2.9]] (`U.SpeechAct`), [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] (`U.Work`), [[E.10 - Unified Lexical Rules for FPF|E.10]] (LEX, incl. L‑SERV, LEX‑BUNDLE & PTG stances), [[F.17 - Unified Term Sheet (UTS)|F.17]] (UTS — Unified Term Sheet), [[F.18 - Local‑First Unification Naming Protocol|F.18]] (Name Cards / NQD‑front; promise ≠ utterance ≠ commitment).
> **Coordinates with:** [[A.6.C - Contract Unpacking for Boundaries|A.6.C]] (agreement/specification/contract shorthand unpacking into promise content, commitment, work/evidence, and interface/boundary claims), [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] (EntityOfConcern / Description episteme / carrier), G.* evidence discipline (EvidenceGraph / SCR), Context/Bridge policy for cross‑Context reuse, [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] (Mint/Reuse), [[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]] (LEX‑AUTH when refactoring existing prose at scale).
...
> **Impact radius:** Any normative prose that uses the “service” cluster (`service`, `service provider`, `server`); LEX rules (L‑SERV / LEX‑BUNDLE); UTS blocks ([[F.17 - Unified Term Sheet (UTS)|F.17]]); boundary/agreement/specification patterns that already talk about services (esp. [[A.6.C - Contract Unpacking for Boundaries|A.6.C]]); any automated repair/lint pipeline used for bulk refactors ([[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]] / LEX‑AUTH).
 **Mint vs reuse:** Mints the `serviceSituation(…)` QRR lens id and the facet headphrase set defined in §4.3. Reuses `U.PromiseContent`, `U.Commitment`, `U.SpeechAct`, `U.System`, `U.Work`, `U.MethodDescription`, and the [[A.6 - Signature Stack & Boundary Discipline|A.6]].P/QRR recipe.
...
**Non‑goal (modularity guard).** This pattern does **not** redefine the semantics or field structure of the promise‑content object (the **promise content**). That kernel meaning is defined in **[[A.2.3 - U.PromiseContent (Promise Content)|A.2.3]] (`U.PromiseContent`)**. [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]] is a precision‑restoration + lexicon discipline that (i) forces facet‑typed head phrases and (ii) provides an optional QRR lens to bind already‑defined kinds without collapsing them. Agreement/specification/contract shorthand unpacking is handled by **[[A.6.C - Contract Unpacking for Boundaries|A.6.C]]**, which applies this pattern when that shorthand contains the service cluster.

## [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:1 - Problem frame
...
## [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:2 - Problem
...
Result: readers can’t apply [[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]] routing, and engineers are incentivized to preserve ambiguity (“service” as a convenient metonym) because it avoids committing to a model.

## [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:3 - Forces
...
## [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:4 - Solution

### [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:4.0 — UTS + LEX preparation (mandatory for authoring/repair)

“Service” is a **polysemy cluster**, not a single token. Therefore, before applying the rewrite rules below to normative prose, the author/editor SHALL create or update a **thread‑local UTS block** ([[F.17 - Unified Term Sheet (UTS)|F.17]]) and its paired **LEX‑BUNDLE entries** ([[E.10 - Unified Lexical Rules for FPF|E.10]]) for the **service cluster** (Tech/Plain twins and PTG stance).
...
**Context selection (universality guard).** The UTS block MUST cite **ContextName@Edition** in each SenseCell ([[F.17 - Unified Term Sheet (UTS)|F.17]]), and the cited contexts SHOULD span at least **three** distinct “service traditions” reflected in this pattern’s SoTA‑Echoing set (e.g., ITSM/service management, EA/modelling, speech‑act/coordination, microservices/SRE practice). This prevents a “FPF‑only” meaning loop and keeps facet names portable.
...
* A conforming **normative Tech** text SHALL treat the bare word **service** (unqualified) as **PTG=Guarded** ([[E.10 - Unified Lexical Rules for FPF|E.10]]): it is valid only under this pattern’s rewrite rules and only as part of a qualified head phrase.
* If a new facet head phrase must be introduced, it SHALL be treated as a **LexicalAct** with an explicit **Mint/Reuse** decision ([[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]), and its **CandidateSet + rationale** SHOULD be recorded via a Name Card ([[F.18 - Local‑First Unification Naming Protocol|F.18]] / NQD‑front) to avoid “clever” but unstable vocabulary.
...
### [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:4.1 — Trigger rule
...
### [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:4.2 — Stable lens: the Service Situation Bundle
...
* `promiseContentRef : PromiseContentRef`
  *Promise content* — the `U.PromiseContent` referent ([[A.2.3 - U.PromiseContent (Promise Content)|A.2.3]]). **Plain head:** *promise content* / *service offering clause* / *service promise clause*.
* `promisedOutcomeSpecRef? : OutcomeSpecRef`
  The **promised outcome template** described by the clause (`U.OutcomeSpec`, [[A.7 - Strict Distinction (Clarity Lattice)|A.7]]:5.10). It may constrain:
...
* `providerAssignmentRef? : RoleAssignmentRef`
  The concrete **role enactor assignment** that holds `providerRoleRef` in the relevant Context/window ([[E.10 - Unified Lexical Rules for FPF|E.10]] / [[A.2.1 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]). This is what everyday talk calls “the service provider” (team/shop/vendor/system).
...
* `accessSpecRef? : MethodDescriptionRef`
  The **service access spec**, a request-facing interface description (API signature, OpenAPI, endpoint interface description, intake SOP, desk procedure). This is typically `promiseContentRef.accessSpec` ([[A.2.3 - U.PromiseContent (Promise Content)|A.2.3]]) and is a `U.MethodDescription`.
...
* `deliveryWorkRef? : WorkRef`
  ...
  **Invariant: SERV‑INV‑3 (Outcome anchoring).**
  ...
  (i) explicitly assert `deliversPromisedOutcome(deliveryWorkRef, promisedOutcomeSpecRef)` ([[A.2.3 - U.PromiseContent (Promise Content)|A.2.3]]:8.1), or
  ...
  **Invariant: SERV‑INV‑4 (Unit-of-delivery measurability).**
  If `promiseContentRef.unitOfDelivery` is present, then its `countingRule` is stated (per [[A.7 - Strict Distinction (Clarity Lattice)|A.7]]:5.10.3, with [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] defaults available) and the cited Work carries the measurements required by that rule (duration, quantity, cases, kWh, etc).
...
Qualifier slots (as needed per [[A.6 - Signature Stack & Boundary Discipline|A.6]].P/A.6.B):

* `scope? : ClaimScope`
* `Γ_time?` (explicit Γ_time selector per [[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]; time windows are explicit when the surrounding passage is time‑sensitive)
* `viewpoint? : ViewpointRef`
* `referenceScheme? / representationScheme?` (only when needed)

**Guidance (didactic).** In normative prose, prefer facet‑explicit predicates: if a predicate targets a specific facet (addressability, deontic force, actuals, mechanism), apply it to the corresponding slot rather than to an untyped “service” noun phrase. (Enforced by CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑3/4/6/9.)
...
* The **promisor / commitment‑holder** is the **provider principal** (or its role assignment) unless the Context explicitly models a system as an agent with standing. *(See CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑8.)*
...
When a `serviceSituation(…)` binds a principal/role assignment to systems (access point / delivery system), the relation kinds are explicit (prefer [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]] base relations when available). **Implicit “system implies provider” readings are invalid.**
* Mechanism/process claims target `deliverySystemRef` and/or `deliveryMethodRef` (and sometimes `accessSpecRef` if the claim is strictly about interface signature), not `promiseContentRef`. *(See CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑9.)*
...
### [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:4.3 — Facet headwords (mandatory lexical rule)
...
(Normative enforcement is via CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑1 and CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑8.)

### [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:4.4 — Addressability rule (the “can you call it?” test)
...
### [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:4.4b — Method/mechanism rule (the “how does it work?” test)
...
A conforming text **SHALL NOT** attach mechanism/process predicates to the **promise content**; the clause may constrain outcomes or acceptance criteria, but mechanism claims belong to design descriptions or method descriptions. *(See CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑9.)*

### [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:4.5 — Deontic rule (the “must/shall” test)
...
**No hidden agency rule (normative):** A conforming text **SHALL NOT** use an access object (e.g., endpoint/access point) as the grammatical subject of an RFC‑keyword sentence. It **SHALL** use the accountable principal (or role assignment) as subject and then state the operational condition on the access point as a predicate/evidence claim. *(See CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑4 and CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑8.)*

### [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:4.6 — Speech‑act rule (the performative verb test)
...
### [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:4.7 — Runtime/telemetry rule (the “actuals” test)
...
### [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:4.8 — Change‑class lexicon (service‑specific narrations)

When the draft describes “service changes”, narrate changes using stable change classes ([[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]]), specialized to the serviceSituation lens:
...
### [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:4.9 — Disambiguation guide (rewrite/selection)
...
## [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:5 - Archetypal grounding
...
## [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:6 - Bias-Annotation
...
## [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:7 - Conformance Checklist (CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]])

0. **CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑0 — UTS/LEX block exists for the service cluster.**
   Any document that applies this pattern (or that introduces normative “service” language) SHALL publish:
   (a) a local **UTS block** ([[F.17 - Unified Term Sheet (UTS)|F.17]]), and
   (b) paired **LEX‑BUNDLE entries** ([[E.10 - Unified Lexical Rules for FPF|E.10]]) for the Tech/Plain twins and PTG stances used here.
   ...
   Any newly introduced facet head phrase SHALL have an explicit Mint/Reuse decision ([[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]) and SHOULD have a Name Card rationale ([[F.18 - Local‑First Unification Naming Protocol|F.18]]).

1. **CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑1 — Unqualified “service” (and cluster stand-ins) is unsupported in normative prose.**
...
2. **CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑2 — `U.PromiseContent` is referred to as a “promise content” in prose.**
...
3. **CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑3 — Addressability implies `accessPointRef` (system), not clause.**
...
4. **CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑4 — Deontic language requires a commitment.**
...
5. **CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑5 — Performative verbs require a speech act.**
...
6. **CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑6 — Actuals require work + evidence.**
...
7. **CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑7 — Bundle lens is used when multiple facets are in play.**
...
8. **CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑8 — Commitments and promises have an accountable principal.**
...
9. **CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑9 — “How it works” claims belong under method or system, not under the clause.**
...
## [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:8 - Common Anti-Patterns and How to Avoid Them
...
## [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:9 - Consequences

* **Pros:**

  * Removes the incentive to keep “service” conveniently vague.
  * Enables [[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]] routing: clause (L), commitment (D), acts/work/evidence (E), mechanisms/interfaces (A/L depending on placement).
...
1) Unqualified head‑noun “service” occurrences in normative prose are **0** (CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑1).
2) Every deontic (“must/shall/guarantee/SLA”) sentence about service delivery references a **service commitment** / `U.Commitment` (CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑4).
3) Every runtime/telemetry “service is down/slow/…” claim is routed to **work + evidence** and, when relevant, distinguishes access‑point symptoms from delivery‑system symptoms (CC‑[[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]‑6 + §4.7).

## [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:10 - Rationale
...
This aligns with [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]]’s requirement to replace umbrella tokens with explicit kind+slots forms and to provide rewrite guides and guardrails.

## [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:11 - SoTA-Echoing
...
## [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:12 - Relations

* **Specialises:** [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]] (RPR) for the lexical/semantic ambiguity cluster around “service”.
...
* **Requires (authoring discipline):** a local UTS block ([[F.17 - Unified Term Sheet (UTS)|F.17]]) and published Tech/Plain twins ([[E.10 - Unified Lexical Rules for FPF|E.10]]) for the service/provider/server cluster; this is the “anti‑FPF‑only loop” guard.
* **Coordinates with:** [[A.6.C - Contract Unpacking for Boundaries|A.6.C]] (agreement/specification/contract shorthand unpacking). When that shorthand includes *service* tokens, apply RPR‑SERV first to select **promise content** vs **commitment** vs **access point/system** vs **work/evidence**, then unpack the resulting atomic statements with [[A.6.C - Contract Unpacking for Boundaries|A.6.C]] and classify them with [[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]] (L/A/D/E).

## [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:12a - Service/cell analogy correction and quantum-like route
...
### [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:12a.1 - Service/cell analogy correction
...
### [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:12a.2 - QL route after service-facet unpacking

Use QL wording only after the service facets have been unpacked and an ordinary [[A.6 - Signature Stack & Boundary Discipline|A.6]], [[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]], [[F.9 - Alignment & Bridge across Contexts|F.9]], [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]], [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]], or [[C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]] governing pattern still leaves a residual probe, export, coarsening, or viability-envelope support load.

QL route action path:

1. If an API read/export is later used as passive state reading while it changes or thins the state, route that remainder through [[C.26.1 - Probe-Coupled Boundary Interaction|C.26.1]].
2. If a service situation is being kept viable by boundary/exchange/adaptation work, route that remainder through [[C.26.3 - Viability-Envelope Boundary Regulation|C.26.3]].
3. If a coarsened representation of the service state is used, route admissible use and non-admissible downstream use through CSC/RT and the [[C.26 - Quantum-Like Modeling Lens|C.26]] coarsening support section only when a QL cue remains.
...
- a [[C.26.1 - Probe-Coupled Boundary Interaction|C.26.1]] note only for passive-read or export mistakes caused by the service interaction;
- a [[C.26.3 - Viability-Envelope Boundary Regulation|C.26.3]] note only when service viability is the actual envelope-regulation problem.

## [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]:End
