...
**Placement.** Part A, cluster A.IV “Signature Stack & Boundary Discipline”; adjacent to [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] `U.RelationSlotDiscipline`.

**Depends on.**
– [[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]] `U.Signature` (universal signature carrier).
– [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] `U.RelationSlotDiscipline` (SlotKind/ValueKind/RefKind stratification + slot-operation lexicon).
– [[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] (Scope discipline; explicit `Γ_time`; implicit “latest/current” is forbidden).
– [[A.2.4 - U.EvidenceRole|A.2.4]] `U.EvidenceRole` (timespan + evidence-role discipline for decision-relevant witness sets).
- [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] (Strict Distinction; EntityOfConcern vs Description-episteme and specification-use cases vs publication face, form, unit, carrier, and rendering lanes).
– [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] (pattern authoring order & SoTA discipline).
– [[E.10 - Unified Lexical Rules for FPF|E.10]] (LEX‑BUNDLE discipline; D.CTX lexical guardrails).

**Coordinates with.**
– [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] Evidence–Provenance DAG discipline (`verifiedBy`, `validatedBy`).
– [[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] per-edge constructive grounding (`tv:groundedBy`) and `validationMode` discipline.
– [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] `U.EpistemeSlotRelation` grounding slots (`GroundingHolonSlot`, `EntityOfConcernSlot`).
– [[A.6.3 - U.EpistemicViewing - EntityOfConcern-preserving morphism|A.6.3]] `U.EpistemicViewing` (`EntityOfConcernRef`-preserving view operators; base-relative “how” without retargeting).
– [[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] `U.EpistemicRetargeting` (base-change along `KindBridge`; retargeting lexicon and continuity rules).
– [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]] `U.KindBridge` & `CL^k` (explicit repair/translation when endpoint kinds or Contexts differ; no silent re-typing).
– [[E.18 - Transformation Flow Structure|E.18]] assurance-operations on `U.Transfer` (`CalibrateTo`, `CiteEvidence`, `AttributeTo`, `ConstrainTo`, …).
– [[F.9 - Alignment & Bridge across Contexts|F.9]] Bridges & CL (cross-context and cross-plane base declarations cite Bridge ids + CL policy).
– [[F.15 - SCR-RSCR Harness for Unification|F.15]] F‑Suite validation harness (SCR/RSCR pins and refresh governance).
- [[F.18 - Local‑First Unification Naming Protocol|F.18]] naming governance (Tech/Plain twins and publication-lane naming boundaries).

**Aliases (informative; discouraged for normative prose).**
– “anchoring / anchor” (source umbrella colloquial; a red-flag token for *under-described dependence*). **Prohibited in Tech register** as a meaning-surrogate; treat it as a defect to be rewritten into an explicit `baseRelation(dependent, base)` form. Allowed only when referring to a **pattern-defined primitive** that already reserves the word (e.g., [[E.10 - Unified Lexical Rules for FPF|E.10]] MG‑DA *Domain Anchoring*; evidence/pin “anchors” where the term is explicitly reserved), or inside quoted source text that is immediately followed by a conformant rewrite.
...
It **reuses** [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] SlotSpec discipline (SlotKind/ValueKind/RefMode), [[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] Scope discipline (`U.Scope`, explicit `Γ_time` when time matters), and [[A.2.4 - U.EvidenceRole|A.2.4]] witness semantics (`U.EvidenceRole`) as the enforcement substrate.

## [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:1 - Problem frame
...
> **Deconfliction note (lexical).** This pattern is about *base-dependence in content* (“X is usable relative to B”). It is not about [[E.10 - Unified Lexical Rules for FPF|E.10]]’s **Domain Anchoring** (MG-DA), where “anchoring” is a *lexical* primitive for binding token morphology to the term's selected EntityOfConcern. When you see `anchor*` in a basedness sentence, treat it as a defect unless an explicit baseRelation token is present.
>
> **Deconfliction note (context/meaning).** This pattern is also not a license to reintroduce “anchor” as a surrogate for **Context**, **SenseCell**, or “where meaning lives”. Any such use is an *anchor‑relapse* and SHALL be rewritten into explicit Context/SenseCell/ConceptSet lane constructs ([[E.10 - Unified Lexical Rules for FPF|E.10]] D.CTX), not into SWBD.
>
...
## [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:2 - Problem
...
| **Compatibility with [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]** | Base declarations introduce slots and edits; they must remain SlotKind/ValueKind/RefKind disciplined and must not collapse slot edits with semantic re-declarations. |
...
## [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4 - Solution — The `U.ScopedWitnessedBaseDeclaration` object and its lexicon

### [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4.1 - Canonical object

**Definition.** A **`U.ScopedWitnessedBaseDeclaration`** (SWBD) is a first-class base-declaration record *shape* (a signature in the [[A.6 - Signature Stack & Boundary Discipline|A.6]].0/A.6.5 sense): it reifies “dependent is usable relative to base via baseRelation, under scope/time, witnessed by pins”.
...
**Notation.** `SlotContent(VK, refMode)` is a compact shorthand for “a slot whose SlotSpec declares `ValueKind=VK` and `refMode ∈ {ByValue | RefKind}` ([[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]])”.

**SlotSpec note.** `VK_*` / `RK_*` / `refMode_*` above are **not** “anything goes”: they are fixed by the chosen `BaseRelationSlot` vocabulary entry and must be declared as SlotSpecs ([[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]). In other words, SWBD is a reusable *shape*, but each Context’s declared `baseRelation` vocabulary entry makes it a concrete, typed signature.

**Instance/prose notation note (convention).** In the prose and examples below, the slot fillers are written as `dependent`, `base`, `baseRelation`, `scope`, `Γ_time`, `witnesses` (no `*Slot` suffix). The `*Slot` suffix is reserved for SlotKinds/positions only ([[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] / [[E.10 - Unified Lexical Rules for FPF|E.10]]).
...
* **WF‑BD‑6 (No silent cross-context reuse).** An SWBD that relates dependent and base across Contexts/planes (or requires scope translation) is admissible only if it cites the Bridge ids + CL policy that make the reuse admissible ([[F.9 - Alignment & Bridge across Contexts|F.9]]). No informal “it is the same entity anyway” prose is an admissible substitute.

This is the discipline-level analogue of [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]’s move: disambiguation is achieved by making the missing structural component explicit and non-optional in decision-relevant contexts.

### [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4.2 - Underlying mathematical lens
...
### [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4.3 - Slot discipline for SWBD

Any signature that introduces SWBD (or SWBD-like relations) SHALL define SlotSpecs per [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]: every position declares **SlotKind / ValueKind / RefKind-or-ByValue**.
...
**Slot vs filler guard.** `*Slot` names the **position** (SlotKind), not a container relation. In prose, say “fills `BaseSlot` with …” or “uses `baseRef` …” rather than calling the base itself “a BaseSlot”. (`Slot` suffix is structural; [[E.10 - Unified Lexical Rules for FPF|E.10]].)

**Slot-level invariants (derived from WF‑BD‑1..4; testable).**
* **Invariant (SlotSpec completeness).** In any SWBD signature, the SlotSpec for `DependentSlot` and `BaseSlot` declares admissible `ValueKind` and `refMode` explicitly ([[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]). If those types cannot be stated, the `baseRelation` vocabulary entry is not yet defined.
* **Invariant (Relation tokenness).** `BaseRelationSlot` holds a declared `U.NameToken` that resolves to a vocabulary entry with a known signature and relation-specific constraints ([[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]] + [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]). It is not free text and is not typed as a publication-lane object (`publication-side object*`).
...
* **Token discipline (LEX).** The token SHALL satisfy [[E.10 - Unified Lexical Rules for FPF|E.10]] token-class morphology for relations/verbs; it SHALL NOT use metaphor heads (`Anchor*`, `Ground*`, `Attach*`) as a meaning-surrogate. If a source synonym must be kept for continuity, record it through [[F.18 - Local‑First Unification Naming Protocol|F.18]] alias discipline while keeping the relation-specific token specific.
...
* **Cross-context / cross-plane policy.** Whether this declared `baseRelation` may cross Contexts/planes at all; if yes, what Bridge ids/CL thresholds must be cited and what loss notes are required ([[F.9 - Alignment & Bridge across Contexts|F.9]] / [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]).

This mirrors [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]: a SlotKind without ValueKind/RefMode is underspecified; a baseRelation without its vocabulary entry is equally underspecified.

### [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4.4.1 - Perspective/voice discipline (dependent-view vs base-view)
...
### [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4.5 - Lexical discipline
...
**Carve-outs (pattern-defined primitives).** This red-flag rule does **not** ban uses where “anchoring” is already a *pattern-defined primitive* elsewhere in the spec, such as [[E.10 - Unified Lexical Rules for FPF|E.10]] MG-DA token-to-EntityOfConcern anchoring or [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence anchors. It still acts as a review trigger: confirm you are using the reserved sense, not smuggling a basedness meaning.

**Naming guard for baseRelation tokens.** Do not mint new baseRelation tokens whose head noun is a metaphor (`Anchor*`, `Ground*`, `Attach*`). If you are tempted to do so, you either (i) have not named the relation kind yet, or (ii) are introducing a source alias that must map onto an existing, more specific relation token through [[F.18 - Local‑First Unification Naming Protocol|F.18]].
...
### [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4.6 - Base-change operation lexicon

As [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] distinguishes slot operations by whether they change a stored reference, resolve a reference, or replace a value, [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]] distinguishes **base declaration changes** by which component changes and what semantics are affected.
...
8. **changeBaseRelation** — not an edit-in-place. Changing `baseRelation` changes meaning; mint a new declaration and relate them via an explicit continuity relation ([[F.13 - Lexical Continuity & Deprecation|F.13]] discipline), rather than silently rewriting the kind.

**Relation to [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] slot operations (non-normative mapping).** These are *semantic change classes*; an implementation may realise them using [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] slot operations on the SWBD record fields. Example: a **rebase** may be implemented as a `retarget` of `baseRef` on a *new* SWBD edition. The point of [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]] is that “we retargeted a ref” is not the semantic story; “we rebased the declaration” is.

**Relation to [[E.18 - Transformation Flow Structure|E.18]] assurance ops (informative).** On `U.Transfer`, the allowed ops (`ConstrainTo/CalibrateTo/CiteEvidence/AttributeTo`) can be viewed as Context-approved specialisations of `declareBase/rescope/rebase/refreshWitnesses` for specific declared `baseRelation` readings, with stricter declared constraints and lintability.

### [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4.7 - Disambiguation guide for selecting a baseRelation
...
*Note.* The `viewedVia` / `retargetedAlong` operators are defined by the [[A.6 - Signature Stack & Boundary Discipline|A.6]].3/A.6.4 viewing/retargeting patterns; this table only classifies them as “relative-to-base” cases.

### [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:4.7a - Support wording selection test
...
A support phrase is governed by [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]] only when all of the following can be stated:
...
| --- | --- |
| a source, model, diagram, publication, or view describes, constrains, or exposes something | source-description, specification-use, and publication patterns ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]], [[A.6.3 - U.EpistemicViewing - EntityOfConcern-preserving morphism|A.6.3]], [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]], [[C.2.3 - Unified Formality Characteristic F|C.2.3]], [[E.17 - Multi‑View Publication Kit|E.17]], local source rules) |
| an observation, test, proof, role, or carrier bears on a claim | evidence patterns ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]], [[A.2.4 - U.EvidenceRole|A.2.4]], [[G.6 - Evidence Graph & Provenance Ledger|G.6]]) |
| a claim is acceptable in an assurance or trust calculus | assurance patterns ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]) plus evidence ontology named by value when evidence is live |
| a causal, intervention, counterfactual, off-policy, or simulation-only use is admissible | causal-use pattern ([[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]]) |
| a mathematical lens, mapping, or model makes a use admissible or exposes preserved/lost structure | mathematical-lens patterns ([[C.29 - Mathematical Lens Use|C.29]], [[C.26 - Quantum-Like Modeling Lens|C.26]], [[F.9 - Alignment & Bridge across Contexts|F.9]]) |
| a metric, score, threshold, benchmark, or characteristic warrants a comparison | measurement/characteristic/comparison patterns ([[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]], [[C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]], [[G.9 - Parity and Benchmark Harness|G.9]]) |
| one thing helps work, enables an action, supplies a resource, or makes operation easier | work/resource/action patterns ([[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]], [[A.15.4 - Work-Relevant Source Restoration|A.15.4]], [[A.6.A - U.ActionInvitationPrecisionRestoration - Affordance and Action-Invitation Precision Restoration (ACT-INV)|A.6.A]], [[C.11 - Decision Theory (Decsn-CAL)|C.11]]) or ordinary Plain help |
| one file, section, packet, companion, entry cue, or expanded case helps a reader find, inspect, or compare another item | publication, entry, or navigation patterns ([[E.17 - Multi‑View Publication Kit|E.17]], [[E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline|E.11]], [[I.2 - Expanded Entry Disambiguation Cases|I.2]]) or ordinary orientation |

This test prevents support wording from becoming either a source-relation bucket or a basis bucket. [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]] governs only the support cases that are genuinely base-relative.

## [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:5 - Archetypal Grounding

### [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:5.1 - System archetype: calibration to a standard
...
### [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:5.2 - Episteme archetype: claim admissibility via evidence relations
...
### [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:5.3 - Structural archetype: constructive grounding of a model edge
...
## [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:6 - Bias-Annotation
...
## [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:7 - Conformance Checklist

A carrier (pattern, spec, schema, code carrier, or publication) conforms to [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]] iff:
...
2. **CC‑BD‑2 — Dependent and base are explicit and typed.**
   Every based declaration SHALL make both `dependent` and `base` explicit, and SHALL be SlotKind/ValueKind/RefMode disciplined per [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]].
...
15. **CC‑BD‑15 — Meaning lane separation.**
    Semantic meaning assignment SHALL be modeled via SenseCell/ConceptSet lane constructs ([[E.10 - Unified Lexical Rules for FPF|E.10]] D.CTX), not via SWBD. SWBD SHALL be used only for non-semantic base-dependence (admissibility, calibration, attribution, policy gating, constructive grounding, viewing/retargeting specialisations).
...
## [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:8 - Common Anti-Patterns and How to Avoid Them
...
## [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:9 - Consequences
...
A team has adopted [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]] if, for any decision-relevant “relative-to” statement, it can produce a resolvable tuple
...
## [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:10 - Rationale
...
Without explicit change classes, prose collapses distinct edits (rebase vs retime vs rescope vs witness refresh) and recreates the same ambiguity [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] removed at the slot layer.

## [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:11 - SoTA-Echoing
...
## [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:12 - Relations
...
[[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]] is the RPR specialisation for “basedness / relative‑to” claims: it makes the relation kind explicit via `baseRelation`, qualifies it with scope/`Γ_time`/witnesses, and standardises evolution via a base‑change lexicon plus lexical red‑flags (`anchor*`).

**Builds on [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] `U.RelationSlotDiscipline`.**
SWBD introduces a structured record with slots; those slots must be SlotKind/ValueKind/RefKind disciplined, and its change classes must not be confused with slot-edit operations ([[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]) or name-binding terminology ([[E.10 - Unified Lexical Rules for FPF|E.10]] / L‑BIND).

**Constrains [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] evidence admissibility links.**
...
**Aligns with [[A.2.4 - U.EvidenceRole|A.2.4]] `U.EvidenceRole`.**
...
**Aligns with [[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] constructive grounding (`tv:groundedBy`).**
...
**Coordinates with [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] grounding holons.**
...
**Coordinates with [[A.6.3 - U.EpistemicViewing - EntityOfConcern-preserving morphism|A.6.3]]–[[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] viewing/retargeting.**
...
**Coordinates with [[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] and `Γ_time`.**
...
**Feeds [[E.10 - Unified Lexical Rules for FPF|E.10]] / [[F.18 - Local‑First Unification Naming Protocol|F.18]] lexical governance.**
...
**Constrains support wording in [[A.6 - Signature Stack & Boundary Discipline|A.6]].P/E.10.**
...
## [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]:End
