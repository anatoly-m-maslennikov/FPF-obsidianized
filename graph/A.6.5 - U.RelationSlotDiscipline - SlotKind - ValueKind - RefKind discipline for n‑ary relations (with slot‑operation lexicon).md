...
**Placement.** Part A, cluster A.IV “Signature Stack & Boundary Discipline”; directly under [[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]] `U.Signature` and alongside [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]–[[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]].
**Depends on.**
– [[A.1 - U.Holon, U.System, and U.Episteme|A.1]] `U.Holon` (holonic carrier model).
– [[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]] `U.Signature` (universal morphism/relationship signatures).
– [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] (Strict Distinction; EntityOfConcern / Description / specification-use vs Surface).
– [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] (pattern authoring order & SoTA discipline).
– [[E.10 - Unified Lexical Rules for FPF|E.10]] (LEX‑BUNDLE: Tech/Plain registers, naming guards).

**Coordinates with.**
– [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] `U.EpistemeSlotRelation` (episteme slots: EntityOfConcern, GroundingHolon, ClaimGraph, Viewpoint, View, ReferenceScheme).
– [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]].* Kind‑CAL (Kinds, KindSignature, KindBridge).
– [[F.18 - Local‑First Unification Naming Protocol|F.18]] (name governance; twin‑register discipline).

## [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:1 - Problem frame

FPF relies heavily on **n‑ary relations and morphisms**:

* episteme component layouts (`U.EpistemeKind` in [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]),
...
This produces subtle bugs (elaborated in [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:2):

* misuse of “Subject/Object” as SlotKind‑like names for very different ValueKinds (explicitly banned for episteme Tech names by [[E.10 - Unified Lexical Rules for FPF|E.10]]),
...
## [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:2 - Problem (symptoms in FPF)
...
5. **Substitution rules not localisable.**
   ...
   * “this Bridge only changes RefKind, not ValueKind”.
   ...
   This blocks clean use of [[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]] `U.Signature` as a shared calculus for method/role/episteme signatures.

6. **Episteme‑specific slots not standardised.**
   For epistemes, the positions “what is this about?”, “in which holon is it grounded?”, “what ClaimGraph is inside?” re‑appear across patterns. Without a shared slot discipline, each pattern names these ad‑hoc, breaking the ability to state **universal laws** over episteme morphisms ([[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]–[[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]]).
...
The result: **local convenience, global incoherence** — exactly what [[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]] and [[E.10 - Unified Lexical Rules for FPF|E.10]] are supposed to prevent.

## [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:3 - Forces
...
* **F2 - Cross‑disciplinary reuse.**
  ...
  * episteme structures ([[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]),
  * systems/roles/methods (A/B),
  * services and APIs (including method/service interfaces and ports),
  * cells in tables and databases,
  * guards, bridges, and flows in [[E.18 - Transformation Flow Structure|E.18]],
  * and publication operations ([[E.17 - Multi‑View Publication Kit|E.17]]).
...
* **F4 - EntityOfConcern / Description / specification-use discipline.**
  Strict distinction ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]], [[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]]) already separates **entities of concern**, their **Description epistemes**, and **specification uses**. The same discipline is needed inside relations:
...
* **F5 - Didactic primacy and naming discipline.**
  [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] and [[E.10 - Unified Lexical Rules for FPF|E.10]] demand patterns that are:
...
## [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4 - Solution — SlotKind / ValueKind / RefKind triple (plus a slot‑operation lexicon)

### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4.1 - Three layers for every argument position
...
In **normative** text, the canonical word is **slot**, and the canonical carrier is a **SlotSpec** triple ([[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]]).
...
2. **ValueKind (kind of slot filler).**
   *Which kinds of things may fill this position in principle (at the intensional level).*
   ...
   – ValueKind is a **Kind** ([[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]].\*) or another kernel‑level type; it is **not** a slot and never carries `*Slot`/`*Ref` suffixes.
...
3. **RefKind (how we store / refer).**
   *What reference/identifier we actually store in episteme when we fill this slot.*
   ...
   – RefKind is **about references, not values**; it usually points to an editioned referent ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]], [[F.15 - SCR-RSCR Harness for Unification|F.15]]) and carries the `.edition` field when pinning a phase.
...
### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4.2 - Naming discipline: `*Slot` and `*Ref`

This pattern introduces the following **lexical constraints**, aligned with [[E.10 - Unified Lexical Rules for FPF|E.10]]:
...
3. **ValueKind names carry no `*Slot`/`*Ref`.**
   ...
   * ValueKinds are named using standard FPF conventions (A/E/F, [[E.10 - Unified Lexical Rules for FPF|E.10]]), without `*Slot`/`*Ref`.
...
These rules become part of the **LEX‑BUNDLE guards** and are enforced by [[F.18 - Local‑First Unification Naming Protocol|F.18]] / name‑acceptance harnesses.

### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4.3 - Integration with `U.Signature` ([[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]])
...
### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4.4 - Typed substitution discipline
...
2. **ValueKind compatibility.**
   The new content **MUST** be of the same ValueKind (or a declared subkind) as `SlotSpec_i.value`; Kind‑CAL governs this (`⊑` in [[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]]–[[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]]). If a Context uses EntityOfConcernClass species constraints ([[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]]), those act as additional guards but do **not** change the SlotKind.

3. **RefKind correctness.**
   If `refMode=RefKind`, the stored field is of that RefKind; substitutions operate on references, not on underlying values. Edition pinning is handled as usual by `.edition` fields in F‑patterns ([[F.15 - SCR-RSCR Harness for Unification|F.15]], etc.).

4. **By‑value vs by‑ref awareness.**
   Substitutions at by‑value slots (e.g. `ClaimGraphSlot`) are **content edits** to the episteme or relation instance; they may affect formality F or assurance lanes. Substitutions at ref slots are **retargetings** of entityOfConcern or context, and their legality is governed by [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]–[[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] and Bridge/CL rules. Tooling SHOULD surface this difference explicitly in authoring surfaces (e.g. separate “Ref” vs “embedded content” columns).
...
### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4.5 - Slot operation lexicon (binding / filling / assignment / retargeting / mutation)
...
#### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4.5.1 - Four‑way separation: Identifier vs Slot vs Slot‑content vs Referent
...
This separation is the anchor for all “binding time” talk in [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4.6.

#### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4.5.2 - Canonical verbs (Tech register) for slot operations *(normative)*
...
8. **mutate / modify** (Referent internal change; content unchanged).
   ...
   **FPF note:** In edition-disciplined contexts, prefer to describe intentional change as **revise**, issue a **re-edition**, and **retarget**, rather than “mutate”, because the Core treats editioned referents as stable per edition ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]], [[F.15 - SCR-RSCR Harness for Unification|F.15]]).
...
#### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4.5.3 - Canonical nouns *(normative)*
...
#### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4.5.4 - Operator naming guidance for slot‑writing operators *(normative, but intentionally lightweight)*
...
Recommended patterns (examples only; Contexts may adopt their own naming style via [[F.18 - Local‑First Unification Naming Protocol|F.18]]):
...
This rule is a lexical enforcement of [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4.4 (typed substitution discipline): the name should tell the reader whether the operator is a **retargeting** (ref change) or a **content edit** (by‑value change).

### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4.6 - Binding time and “early vs late binding” *(normative framing, informative examples)*

In cross‑domain slot talk, “early binding / late binding” is meaningful only if the author states **which link is being fixed when**. Under [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4.5, there are three distinct “times” that writers must not conflate:
...
This preserves the [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] stratification and prevents importing accidental semantics from a specific programming language.

## [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:5 - Archetypal Grounding (Tell‑Show‑Show)

Following [[E.7 - Archetypal Grounding Principle|E.7]], we ground the pattern in a **System** example and an **Episteme** example.

### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:5.1 - System example — authentication pipeline signature
...
### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:5.2 - Episteme example — model evaluation result
...
The SlotKind/ValueKind/RefKind discipline makes these moves **local and explicit**: the pattern describes which moves are allowed where, and [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]–[[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] then constrain how episteme morphisms may change ClaimGraphs and references.

### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:5.3 - Didactic micro‑examples — substitution by SlotKind / ValueKind / RefKind *(informative)*

The following short examples are intended for a didactic guide or for cross‑references from [[A.6 - Signature Stack & Boundary Discipline|A.6]].0/A.6.x/C.2.1. In all of them:
...
#### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:5.3.1 - Mechanics — stool on a test rig
...
#### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:5.3.2 - Microservices — switching the authentication provider
...
#### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:5.3.3 - Data/ML — swapping dataset or target characteristic
...
#### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:5.3.4 - Operational practice — the same runbook in different operating centres
...
#### [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:5.3.5 - SLO/SLA requirements — changing the target characteristic vs changing the threshold
...
## [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:6 - Bias‑Annotation
...
* **Episteme‑first entityOfConcern.**
  The examples and cross‑references prioritise episteme use‑cases ([[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]], [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]–[[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]]) where entityOfConcern and retargeting are subtle. System‑only usages (e.g. method signatures) are absolutely allowed but not the driving case; they inherit the same discipline without additional obligations.

## [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:7 - Conformance Checklist (normative)

**CC‑[[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]‑1 - SlotSpec for every parameter.**
...
**CC‑[[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]‑2 - `*Slot` discipline.**
Any Tech name ending with `…Slot` **MUST** denote a SlotKind; SlotKinds **MUST NOT** be used as ValueKinds or RefKinds.

**CC‑[[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]‑3 - `*Ref` discipline.**
...
**CC‑[[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]‑4 - ValueKind purity.**
...
**CC‑[[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]‑5 - Episteme core SlotKinds.**
For episteme kinds (`U.EpistemeKind`), the following SlotKinds **SHALL** be used (or their documented refinements) in [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] / C.2.x:
...
**CC‑[[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]‑6 - No “Role” as SlotKind head.**
...
**CC‑[[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]‑7 - Substitution checks.**
...
**CC‑[[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]‑8 - Cross‑pattern consistency.**
...
**CC‑[[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]‑9 - Migration of legacy `…Ref`/`…Slot` usage.**
...
**CC‑[[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]‑10 - Pattern integration.**
New or revised patterns in Part A/B/C/E that introduce n‑ary relations, morphisms, or signatures **SHALL** reference [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] in their Relations section and attest that they follow SlotKind/ValueKind/RefKind discipline.

**CC‑[[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]‑11 - Slot‑content terminology.**
...
**CC‑[[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]‑12 - Slot‑operation verb discipline.**
Any normative description of a change “to a slot” **MUST** specify which operation class it is (initialize vs assign/set vs retarget vs by‑value edit vs resolve vs mutate/revise), using the canonical verbs in [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4.5.2 or explicitly mapping local terms to them.

**CC‑[[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]‑13 - Binding‑time clarity.**
...
## [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:8 - Consequences
...
* **Safer substitutions.**
  Substitution, retargeting, and viewing laws ([[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]–[[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]]) can be stated in terms of *which SlotKinds* they read/write and *which ValueKinds* they preserve or retarget, without accidentally collapsing into “just replace the thing”.
...
* **Stricter lexical rules.**
  Some legacy names will need migration (`EpistemicObject`, ad‑hoc `…Ref` types). Mitigation: migration notes in [[F.18 - Local‑First Unification Naming Protocol|F.18]] and dedicated anti‑pattern sections; transitional aliases allowed but marked deprecated.

* **Learning curve.**
  Authors must learn to think “SlotKind/ValueKind/RefKind” *and* distinguish “retarget vs edit vs resolve” before writing `id` or `subject`. Mitigation: Tell‑Show‑Show examples and a didactic micro‑guide on slot operations referenced from [[A.6 - Signature Stack & Boundary Discipline|A.6]].0/C.2.1/E.17.0.

## [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:9 - Rationale
...
In practice, “Ref” types tend to be quietly used as if they were values, eroding the EntityOfConcern / Description / specification-use split and making edition discipline invisible. By insisting that ValueKind is always the conceptual kind (“what sort of thing is this about?”) and RefKind is always the reference/identifier kind (“how do we point at it in Episteme?”), the pattern aligns with [[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]]’s intension/description/specification discipline and with modern resource‑aware logics that keep values and resources distinct.

**Why add a slot‑operation lexicon.**
The triple only buys safety if authors and tools can see it at a glance **and** can narrate changes without collapsing layers. [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4.5 makes the common “put something in a slot” moves explicit: initialization vs assignment vs retargeting vs by‑value editing vs resolution. This directly reduces ambiguity in episteme morphism descriptions ([[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]–[[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]]) and prevents accidental imports from a specific PL’s terminology.

**Why standardise episteme SlotKinds.**
entityOfConcern and grounding recur across epistemes; standard SlotKinds (`EntityOfConcernSlot`, `GroundingHolonSlot`, `ClaimGraphSlot`, etc.) let [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]–[[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] and [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] talk about substitutions and retargetings once, instead of re‑defining “what this is about” in every pattern.
...
## [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:10 - SoTA‑Echoing (post‑2015 practice alignment)

**Purpose.** To situate SlotKind/ValueKind/RefKind discipline with respect to contemporary typed and relational approaches, without importing any external calculus into the Core. All items are used as conceptual comparators; concrete reuse in a `U.BoundedContext` would happen only via explicit Bridges ([[F.9 - Alignment & Bridge across Contexts|F.9]]) with declared CL penalties.
...
3. **Relational models of types‑and‑effects (adapt).**
   ...
   **Adapted:** [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] stratifies *slot / value / reference* instead of *value / effect*, and applies the discipline not only to programs but also to epistemes, roles, methods, and services. ([15])
...
4. **Optics / lenses as disciplined projections (echo).**
   ...
   **Echoed:** SlotKind plays the role of the focus coordinate; ValueKind is the focus type; RefKind determines whether the focus is stored by value or via a handle. This perspective informs later use of SlotKind discipline in EpistemicViewing ([[A.6.3 - U.EpistemicViewing - EntityOfConcern-preserving morphism|A.6.3]]) and multi‑view publication ([[E.17 - Multi‑View Publication Kit|E.17]]). ([16])

**Cross‑Context reuse and Bridges.** When a `U.BoundedContext` chooses to adopt a concrete row‑typing discipline, relational logic, or optics library, it **SHALL** do so via explicit Bridges ([[F.9 - Alignment & Bridge across Contexts|F.9]]) with CL and (for plane crossings) `Φ(CL)`/`Φ_plane` policy‑ids, keeping numerical policies and notations Context‑local. [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] only constrains the **slot discipline** that such Bridges must respect.

## [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:11 - Relations (with other patterns)

**Specialises [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]] Relational Precision Restoration (RPR).**
[[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] is the RPR specialisation for “n‑ary relation as slots”: it restores hidden arity by making participant positions explicit as SlotKinds, and stabilises change semantics via the slot‑operation lexicon + lexical guards.

**Builds on [[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]] `U.Signature`.**
...
**Constrains [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] `U.EpistemeSlotRelation`.**
Fixes core episteme SlotKinds (`EntityOfConcernSlot`, `GroundingHolonSlot`, `ClaimGraphSlot`, `ViewpointSlot`, `ViewSlot`, `ReferenceSchemeSlot`) and their ValueKinds/`ByValue` vs Ref discipline. [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] and its extensions SHALL use these SlotKinds (or documented refinements) so that episteme morphisms can be expressed uniformly over slots.

**Supports [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]–[[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] (episteme morphisms and viewing).**
EntityOfConcern‑preserving vs entityOfConcern‑retargeting morphisms can now be stated as constraints on which SlotKinds’ ValueKinds/RefKinds they may change. Retargeting becomes “retargeting at `EntityOfConcernSlot` under a Kind‑Bridge” rather than an ad‑hoc parameter tweak. The operation lexicon in [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4.5 makes “retarget vs edit vs resolve” explicit in these morphism descriptions.

**Coordinates with [[B.5 - Canonical Reasoning Cycle|B.5]].* (RoleEnactment).**
Role/assignment relations may declare SlotKinds such as `HolderHolonSlot`, `RoleSlot`, `ContextSlot`, `WindowSlot` with clear ValueKinds/RefKinds, instead of overloading “role” for both holonic roles and relation positions. This keeps `U.Role` semantics ([[A.2 - Role Taxonomy|A.2]], [[F.6 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]) separate from slot discipline.

**Coordinates with [[E.17 - Multi‑View Publication Kit|E.17]] `U.MultiViewDescribing`.**
...
**Feeds [[F.18 - Local‑First Unification Naming Protocol|F.18]] (LEX‑BUNDLE) and [[E.10 - Unified Lexical Rules for FPF|E.10]] (LEX).**
Provides lexical guards for `*Slot` and `*Ref`, and (via [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:4.5) for operation verbs:
...
**Used by [[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]] `CharacteristicSpace` and measurement patterns.**
Characteristic‑space slots already behave as positions with attached kinds; slot discipline in [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] gives a uniform story for how such slots appear inside relation signatures, episteme cards, and service definitions, and how substitution over those slots is checked.
...
## [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:End
