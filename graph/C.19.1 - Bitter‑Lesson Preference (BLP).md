...
**Builds on.** [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] (E/E‑LOG), [[C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]] (Agent‑Tools‑CAL; **ATC‑2**), [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (Assurance), [[E.3 - Principle Taxonomy & Precedence Model|E.3]] (Precedence), [[E.5 - Four Guard‑Rails of FPF|E.5]] (Guard‑Rails).
**Coordinates with.** [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] (Selector), [[G.8 - SoS‑LOG Bundles & Maturity Ladders|G.8]] (SoS‑LOG Bundles), [[G.9 - Parity and Benchmark Harness|G.9]] (Parity), [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] (Refresh‑Telemetry), [[A.0 - Onboarding Glossary (NQD & E-E‑LOG)|A.0]] (On‑Ramp).
**Keywords.** general‑method preference; scale‑amenability; **BLP‑waiver**; iso‑scale parity; **Scale‑Audit**; slope vector; **α/δ tolerances**.

## [[C.19.1 - Bitter‑Lesson Preference (BLP)|C.19.1]]:1 - Problem frame
...
## [[C.19.1 - Bitter‑Lesson Preference (BLP)|C.19.1]]:2 - Policy clauses (normative; synchronized with Core)
...
(d) **Resources**: publish **Resrc‑CAL** accounts (time/energy/FLOPs) and assurance deltas ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).
...
Record **Heuristic Debt** only when an admitted heuristic functions as reusable method-family policy, selector-facing method preference, durable override of a general scale-amenable alternative, DRR-backed scale waiver, or project-side method choice that claims scale advantage or BLP override. Ordinary local bounded tactics that make no reusable-method, scale-advantage, selector-facing, or override claim may remain local and bounded without Heuristic Debt publication. `BLP.HeuristicDebtEntry` is a [[C.19.1 - Bitter‑Lesson Preference (BLP)|C.19.1]]-local or [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]-linked policy/debt entry; it is not a universal `U.*` record kind unless separately admitted through [[F.18 - Local‑First Unification Naming Protocol|F.18]], [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]], and [[E.9 - Design‑Rationale Record (DRR) Method|E.9]]. For a live debt entry, record scope, responsible role, expiry/review window, and a de-hardening plan; track in **CalibrationLedger/BCT** and cite in SCR.
...
BLP is constitutional (instantiates **P‑10/P‑11/P‑7/P‑1**), but **does not supersede Guard‑Rails ([[E.5 - Four Guard‑Rails of FPF|E.5]]) or precedence rulings ([[E.3 - Principle Taxonomy & Precedence Model|E.3]])**. Where **NQD/E/E‑LOG** promotes illumination into dominance, **BLP adopts that lens** for the audited window.

**BLP‑7 — Publication discipline.**
Scale‑Audit artefacts **SHALL** be exported to **[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]** with edition pins, CI level, α/δ, ComparatorSet, and **BLP.Policy@Context** reference so downstream selectors can reuse evidence without re‑running audits.

## [[C.19.1 - Bitter‑Lesson Preference (BLP)|C.19.1]]:3 - Conformance Checklist (CC‑BLP)
...
8. Audit artefacts exported to **[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]** with **BLP.Policy@Context** id.
...
## [[C.19.1 - Bitter‑Lesson Preference (BLP)|C.19.1]]:5 - Archetypal grounding (post-2015; informative)
...
## [[C.19.1 - Bitter‑Lesson Preference (BLP)|C.19.1]]:6 - Payload — exports

`BLP.Policy@Context` (UTS row; editioned):
`⟨PreferenceDefault, α/δ tolerances + CI, Scale‑Audit recipe ([[G.9 - Parity and Benchmark Harness|G.9]] link; DoE), WaiverRegister{reason, responsibleRoleRef, expiry}, E/E‑LOG lens policy‑ids, ATC.PolicyRef? (agentic), G.11.TelemetryPins⟩`.

**UTS row template (conceptual; pencil‑ready).**
`BLP.Policy@Context := PreferenceDefault=(prefer‑general|neutral), α/δ=(α=…, δ=…, CI=…), Scale‑Audit=(parity=[[G.9 - Parity and Benchmark Harness|G.9]]; sweep=S={…}; DoE=factorial|LHD; kneeTest=policy‑τ), WaiverRegister=[{reason=…, responsibleRoleRef=…, expiry=…}], E/E‑LOG=(policyIds=…), ATC.PolicyRef=(…), TelemetryPins=(edition=…, seeds=…, comparatorSet=…)`.

## [[C.19.1 - Bitter‑Lesson Preference (BLP)|C.19.1]]:7 - Relations

**Depends on:** **G.5/G.9** (selector/parity), **[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]** (refresh telemetry), **C.5** (Resrc‑CAL), **[[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]** (NQD‑CAL), **[[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]]** (E/E‑LOG), **F.7/F.9** (Bridges, CL/Φ/Ψ). **Constrained by:** **[[E.5 - Four Guard‑Rails of FPF|E.5]]** Guard‑Rails and **[[E.3 - Principle Taxonomy & Precedence Model|E.3]]** precedence.

### [[C.19.1 - Bitter‑Lesson Preference (BLP)|C.19.1]]:7.1 - [[C.29 - Mathematical Lens Use|C.29]] mathematical-lens use relation

When a mathematical lens is chosen over a general, scale-amenable method because it is elegant, specialized, or theoretically prestigious, [[C.19.1 - Bitter‑Lesson Preference (BLP)|C.19.1]] governs the scale-advantage and method-preference claim. A [[C.29 - Mathematical Lens Use|C.29]] application may state `CandidateMathObject`, `LensMappingMode`, `PreservedStructure`, `LostStructure`, `LensUseAdmissibilityValue`, `admissibleUse`, `nonAdmissibleUse`, and `StopCondition`; it does not supply BLP compatibility, scale dominance, or waiver evidence.

If scale advantage is live, cite a `Scale-Audit` or `BLP-waiver`. If scale advantage is not live, keep the mathematical lens local and bounded by its [[C.29 - Mathematical Lens Use|C.29]] stop condition.

> *Memory hook.* **Prefer what scales; explain when you don’t.**

When [[E.23 - Quality Improvement Loop Method|E.23]] selects between a Ralph-like general adaptive loop, a specialized object-family cycle, or a mixed operation-family set, [[C.19.1 - Bitter‑Lesson Preference (BLP)|C.19.1]] governs the BLP comparison and waiver discipline. The local [[E.23 - Quality Improvement Loop Method|E.23]] cost and risk prompt `token_or_compute_cost + tool_cost + adaptation_attempt_cost + human_supervision_cost + rework_cost - avoided_loss_value` is not a scalar quality score; it is a practical accepted-work cost account for deciding whether the next pass, added operation, or method-family switch is BLP-compatible. Repeated automation alone does not satisfy BLP; the record must still name the object under improvement, object-under-improvement evaluation, protected trade-offs, bounded cost and risk posture, and stop or switch condition.

## [[C.19.1 - Bitter‑Lesson Preference (BLP)|C.19.1]]:End
