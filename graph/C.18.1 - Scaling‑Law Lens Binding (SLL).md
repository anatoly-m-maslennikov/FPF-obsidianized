...
Make **generation/selection** scale‑savvy: at the level of **conceptual descriptors**, declare (a) **which monotone knobs** we would scale, (b) the **ScaleWindow** over which we claim behaviour, and (c) the **elasticity class** we observed—**without** imposing numeric fits or vendor tools at Core level. This surfaces knees early and keeps comparisons lawful and fair across families. (Parity is handled by **[[G.9 - Parity and Benchmark Harness|G.9]]**; illumination remains a **report-only telemetry** unless a CAL policy promotes it.)

**Builds on.** [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]] (MM‑CHR), [[C.17 - Characterising Generative Novelty & Value (Creativity‑CHR)|C.17]] (Creativity‑CHR), [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]] (NQD‑CAL); advisory: C.5 (Resrc‑CAL).
**Coordinates with.** [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] (E/E‑LOG), [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] (Selector & Registry), [[G.9 - Parity and Benchmark Harness|G.9]] (Parity Harness), [[G.10 - SoTA Pack Shipping|G.10]] (Shipping), [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] (Refresh‑Telemetry), [[C.24 - Agentic Tool-Use and Call Planning (C.Agent-Tools-CAL)|C.24]] (Agent‑Tools‑CAL).
...
## [[C.18.1 - Scaling‑Law Lens Binding (SLL)|C.18.1]]:1 - Problem frame
...
## [[C.18.1 - Scaling‑Law Lens Binding (SLL)|C.18.1]]:2 - Problem

Omitting **Scale Variables** and the comparison window causes: (i) **unfair parity** (compute/data/FoA mismatched), (ii) **illumination/coverage report-metric  creep** into dominance by default, (iii) late detection of knees and budget waste. **[[G.9 - Parity and Benchmark Harness|G.9]]** already forbids scalarising mixed scales and mandates equal **FreshnessWindows**/**pinned editions**; SLL complements this with **ScaleWindow** & elasticity.

## [[C.18.1 - Scaling‑Law Lens Binding (SLL)|C.18.1]]:3 - Forces
...
## [[C.18.1 - Scaling‑Law Lens Binding (SLL)|C.18.1]]:4 - Solution — *binding lens for generator/selector profiles* (normative)

### [[C.18.1 - Scaling‑Law Lens Binding (SLL)|C.18.1]]:4.1 - Types (aliases; ΔKernel = 0).
`SLL.Profile` is an **annotation** on a `MethodFamily/Generator` or a `Selector` profile; **no new U.Types** are minted (LEX discipline).

### [[C.18.1 - Scaling‑Law Lens Binding (SLL)|C.18.1]]:4.2 - Fields (conceptual descriptors).
...
### [[C.18.1 - Scaling‑Law Lens Binding (SLL)|C.18.1]]:4.3 - Norms (SLL).
...
* **SLL‑3 (Parity).** Where `S` is declared, comparisons **SHALL** ensure **iso‑scale parity** and lawful **UNM/NormalizationMethod‑based mapping** across heterogeneous knobs (e.g., FLOPs↔tokens) **before** comparing outcomes; **FreshnessWindows/editions** must be equal/pinned per **[[G.9 - Parity and Benchmark Harness|G.9]]**. Record **seeds/replicates**, ComparatorSet, and policy‑ids in telemetry/SCR.
...
* **SLL‑6 (Telemetry invariants).** Probes **SHALL** export seeds/replicates, edition pins, policy‑ids, and Resrc‑CAL units to **[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]**.

### [[C.18.1 - Scaling‑Law Lens Binding (SLL)|C.18.1]]:4.4 - Method — minimal SoTA probe recipe (notation‑agnostic; informative).
...
## [[C.18.1 - Scaling‑Law Lens Binding (SLL)|C.18.1]]:5 - Interfaces — minimal I/O (conceptual)

**[[G.9 - Parity and Benchmark Harness|G.9]] Plan/Run Parity** consumes `S`/ScaleWindow to align budgets, **pin editions**, and perform **UNM/NormalizationMethod‑based mapping**; **[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]** carries **policy‑id**, **PathSliceId**, seeds/replicates, CI level, and edition pins per parity CC.

## [[C.18.1 - Scaling‑Law Lens Binding (SLL)|C.18.1]]:6 - Conformance Checklist (CC‑SLL)
...
## [[C.18.1 - Scaling‑Law Lens Binding (SLL)|C.18.1]]:7 - Anti‑patterns & remedies

Hidden budget mismatches; averaging ordinals across families; **illumination in dominance by default**; unpinned editions; slope claims without **replicates/CI**; training/inference phase mixing → **cure** with **[[G.9 - Parity and Benchmark Harness|G.9]]** parity (equal windows/editions; normalize‑then‑compare; return sets), phase‑label the claim, and record slope uncertainty per Scale‑Audit discipline.

## [[C.18.1 - Scaling‑Law Lens Binding (SLL)|C.18.1]]:8 - Archetypal grounding (post‑2015; informative)
...
## [[C.18.1 - Scaling‑Law Lens Binding (SLL)|C.18.1]]:9 - Payload — exports
...
## [[C.18.1 - Scaling‑Law Lens Binding (SLL)|C.18.1]]:10 - Relations
...
- Non-admissible use: more scale is not linear improvement, and a scale word does not create a [[C.27 - Temporal Claim Adequacy- State Readings, Temporal Trends, and Intervention-Sensitive Temporal Change|C.27]] rate-change claim by itself.
- Exit: if comparison or benchmark use is live, cite [[G.9 - Parity and Benchmark Harness|G.9]] for parity; if the statement is only a linear effort fantasy, name the scale variable and scale window or downgrade.

**Builds on:** C.16/17/18. **Coordinates with:** [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] (lenses/policies), **[[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]** (set‑returning selector), **[[G.9 - Parity and Benchmark Harness|G.9]]** (parity; **ParetoOnly** default; UNM/NormalizationMethod‑based mapping), **[[G.10 - SoTA Pack Shipping|G.10]]** (shipping).

> *Pedagogical cue.* **Say what you would scale, probe it twice, and use the slope‑class to steer.**

## [[C.18.1 - Scaling‑Law Lens Binding (SLL)|C.18.1]]:12a - [[C.29 - Mathematical Lens Use|C.29]] mathematical-lens use relation

> [[C.18.1 - Scaling‑Law Lens Binding (SLL)|C.18.1]] supplies scale-window and scaling-law evidence for [[C.29 - Mathematical Lens Use|C.29]] when a mathematical lens claims scale behavior, universality, knees, exponents, coarse-graining validity, or diminishing returns. [[C.29 - Mathematical Lens Use|C.29]] cannot treat mathematical compression as scalable without an SLL or BLP-compatible scale-window account where scale is load-bearing. If no scale claim is live, [[C.29 - Mathematical Lens Use|C.29]] uses a local stop condition rather than opening scale-law work.

## [[C.18.1 - Scaling‑Law Lens Binding (SLL)|C.18.1]]:End
