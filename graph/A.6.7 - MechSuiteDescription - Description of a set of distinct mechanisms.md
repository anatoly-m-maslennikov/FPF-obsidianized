...
**Placement.** Part A → cluster A.IV ([[A.6 - Signature Stack & Boundary Discipline|A.6]]), immediately after [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]].

**Builds on.** [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] (pattern template discipline), [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] (`U.Mechanism.Intension` canonical form), [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] (slot/ref discipline), [[E.10 - Unified Lexical Rules for FPF|E.10]] (lexical + ontological rules; strict distinction; minimal specificity; kind suffixes), [[E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]] (conformance checks), [[E.18 - Transformation Flow Structure|E.18]] (transformation-flow structure and P2W carry-through discipline; crossing visibility), [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] (OperationalGate(profile) and gate-level decisions).

**Used by.** Any framework area that needs a stable “universal kernel” shared across multiple mechanisms (notably the universalization of Part G patterns, including but not limited to [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]), and any “mechanism stack” whose correctness is defined by **shared legality + transport + audit obligations** rather than by a single shared `BaseType`.
...
## [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:1 - Problem frame

In FPF, a **mechanism** is a node-level `U.Mechanism.Intension` with explicit SlotSpecs inside operator signatures, and a declared LawSet/guards/transport/audit ([[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]], [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]). Many architectures, however, require **a stable bundle of multiple different mechanisms** that are intended to be used together under shared legality and crossing discipline (e.g., a characterization chain, a legality-gated selection pipeline, or a universal Part‑G kernel that multiple G.* patterns must reuse).
...
Additionally, FPF reserves “Pack” for publication/shipping bundling (e.g., [[G.10 - SoTA Pack Shipping|G.10]]); using “Pack” to mean “container of mechanisms” creates ontological collisions and downstream confusion.

## [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:2 - Problem
...
5. preserve strict distinction among:
   ...
   * a publication bundle (`Pack`, e.g., [[G.10 - SoTA Pack Shipping|G.10]]).

## [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:3 - Forces
...
2. **Minimal specificity + kind suffix discipline ([[E.10 - Unified Lexical Rules for FPF|E.10]]).**
...
4. **Transport and crossing visibility discipline.**
   Cross-context and cross-plane steps must be visible and bridge-only; penalties must route to `R/R_eff` only; suites must not embed CL/Φ/Ψ/Φ_plane tables. Visibility is mediated via [[E.18 - Transformation Flow Structure|E.18]] / P2W (crossing bundles + UTS/Path pins), not by “implicit semantics”.
...
## [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:4 - Solution

Introduce a new Kernel description token:

### [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:4.1 `MechSuiteDescription` (data model)
...
* **No “Pack” meaning.**
  A suite MUST NOT be named or treated as a publication pack. `Pack` remains reserved for publication/shipping bundling (e.g., [[G.10 - SoTA Pack Shipping|G.10]]).
...
### [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:4.2 SuiteObligations (canonical obligation vocabulary)
 ...
 * Well-formedness constraint: suite obligations do not introduce any additional graph edge kind beyond [[E.18 - Transformation Flow Structure|E.18]] `U.Transfer` and do not embed CL/Φ/Ψ/Φ_plane tables. Any transport-related obligation is expressed only as referenced pins/anchors whose realization is mediated by [[E.18 - Transformation Flow Structure|E.18]] / gate surfaces.
...
9. **`crossing_visibility_required`.**
   Well-formedness constraint: any GateCrossing relevant to suite use publishes a `CrossingBundle` ([[E.18 - Transformation Flow Structure|E.18]]) and can be cited as an audit anchor.
...
### [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:4.3 SuiteSpecPins
...
### [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:4.4 SuiteProtocols
...
### [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:4.5 SuiteAuditObligations
...
### [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:4.6 Examples (tell–show–show discipline)
...
* The suite embeds Φ/CL tables or invents an additional graph edge kind beyond [[E.18 - Transformation Flow Structure|E.18]] `U.Transfer`.

All violate the separation between mechanism/suite descriptions and gate-level operational control.

## [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:5 - Archetypal Grounding
...
* It remains descriptive: it does not execute, it does not contain run-time outputs, and it does not replace the [[E.18 - Transformation Flow Structure|E.18]] subgraph that actually connects nodes by `Uses` and manages crossings.

## [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:6 - Bias-Annotation
...
## [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:7 - Conformance Checklist

A `MechSuiteDescription` is conformant iff all applicable items hold:

**CC‑[[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]‑1 (Correct level).** The suite’s `mechanisms` enumerate **distinct** `U.Mechanism.Intension` members. The suite is not encoded as `MechFamilyDescription`.

**CC‑[[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]‑2 (Description token, not `U.*`).** The suite token is a Description token and MUST NOT be introduced under `U.*`. Its name ends with `…Description`.

**CC‑[[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]‑3 (No execution semantics).** The suite MUST NOT define mechanism blocks (`OperationAlgebra`, `LawSet`, etc.) and MUST NOT be used as a mechanism node.

**CC‑[[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]‑4 (No gate decisions).** The suite MUST NOT define `GateDecision`, MUST NOT publish `DecisionLog`, and MUST preserve gate/mechanism separation.

**CC‑[[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]‑5 (Spec pins, not duplication).** If the suite is legality-gated for numeric comparison/aggregation/scoring, it MUST require `CG‑Spec` citation pins (and SHOULD require `CN‑Spec` pins where applicable). It MUST NOT duplicate spec content as “local CG‑Spec”.

**CC‑[[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]‑5a (CN+CG pins for legality-gated characterization).** If the suite is legality-gated for characterization, it MUST require both `CNSpecRef` and `CGSpecRef` as pins (references), consistent with [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:4.3.

**CC‑[[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]‑6 (Transport discipline preserved).** The suite MUST NOT introduce transport exceptions. Any crossing obligations must remain Bridge-only and must route penalties to `R/R_eff` only.

**CC‑[[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]‑7 (Tri-state guard discipline when used).** If the suite declares admissibility/eligibility semantics, it MUST use `GuardDecision := {pass|degrade|abstain}` and MUST NOT coerce unknown to pass.

**CC‑[[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]‑8 (No thresholds in core).** The suite MUST NOT publish acceptance thresholds or “passing scores”. Thresholds must remain in acceptance clauses / task signatures / gate profiles.

**CC‑[[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]‑9 (Crossing visibility anchors).** If suite use depends on crossings (context or plane/kind, entry into `U.WorkEnactment` (LaunchGate), or edition-key changes), the suite MUST require crossing visibility anchors (BridgeId/channel, ReferencePlane, CL mode, policy-id pins, UTS/Path pins) as audit obligations, without embedding the tables.

**CC‑[[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]‑10 (Suite id present).** The suite MUST declare `mech_suite_id: MechSuiteId` so that downstream planning/audit can cite it stably.

**CC‑[[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]‑11 (Two-bridge discipline preserved).** If suite obligations claim cross-kind/EntityOfConcern validity, they MUST require explicit `CL^k` handling (two-bridge rule) and MUST NOT allow implicit EntityOfConcern changes.

**CC‑[[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]‑12 (Implementation export hygiene when cited).** If the suite cites realizations/implementations, the citations MUST preserve export/import discipline (LOG/CHR: no Γ export; CAL: exactly one Γ; imports acyclic).

**CC‑[[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]‑13 (No Pack conflation).** The suite MUST NOT be introduced, named, or used as a publication/shipping `Pack`.

**CC‑[[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]‑14 (Protocol closure & explicitness).** If `suite_protocols` is present, every `ProtocolStep.mechanism` MUST be a member of `mechanisms` (WF‑MS‑2) and the protocol MUST NOT rely on implicit mechanism steps or implicit crossings.

**CC‑[[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]‑15 (P2W split preserved when applicable).** If the suite requires a planned-baseline pin, that baseline MUST be a `WorkPlanning` plan item and MUST NOT contain launch values or `FinalizeLaunchValues` witnesses; such witnesses remain `U.WorkEnactment`-only.

## [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:8 - Common Anti-Patterns and How to Avoid Them
...
5. **Anti-pattern: “Implicit calls”.**
   ...
   **Fix:** protocols enumerate steps and required pins; [[E.18 - Transformation Flow Structure|E.18]] `Uses` edges remain explicit.

## [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:9 - Consequences
...
## [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:10 - Rationale
...
## [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:11 - SoTA-Echoing
...
## [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:12 - Relations

* **Relates to [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]:** suite members are `U.Mechanism.Intension`; the suite does not replace the mechanism definition.
* **Relates to [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]:** suites must not weaken slot/ref discipline; any suite protocol assumes member mechanisms follow [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] invariants (SlotKind stability, correct refMode, no semantic meaning in SlotIndex).
* **Relates to [[E.18 - Transformation Flow Structure|E.18]] / P2W:** suite protocols describe intended composition; actual composition and crossings are expressed in [[E.18 - Transformation Flow Structure|E.18]] subgraphs and P2W flow.
* **Relates to [[E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]]:** suite-level conformance is a conceptual review checklist; suites require pins/anchors rather than procedural validation.
* **Relates to [[G.10 - SoTA Pack Shipping|G.10]]:** suites are not packs; publication/shipping is handled via [[G.10 - SoTA Pack Shipping|G.10]] and MVPK faces.

## [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]]:End
