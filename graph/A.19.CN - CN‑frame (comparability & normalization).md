...
> **Scope.** This CN‑frame Algebra & Normalization Discipline **extends [[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]]** by fixing the **governance Standard** for CN‑frames, defining a **conformance checklist** and **regression harness**, and providing **didactic one‑pagers** and **anti‑patterns** so teams can introduce CN‑frames without tool lock‑in. The mandatory pattern structure and authoring discipline from **Part E** (Style Guide, Tell‑Show‑Show, checklists, DRR, guard‑rails) are applied throughout.
>
> **Governing-pattern boundary (cite, don’t duplicate).** [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]] governs the **CN-frame governance card, registry, bridges, and checklist/harness** (`CN-Spec`, registry, bridges, checklist/harness). It does **not** govern any CHR-mechanism **intensions**, term cards, or method taxonomies. Those are governed by the corresponding mechanism-governing patterns: **[[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]**, **[[A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]**, **[[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]**, **[[A.19.ULSAM - Unified Lawful Scale Aggregation Mechanism (ULSAM)|A.19.ULSAM]]**, **[[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]**, and **[[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]**. Evidence/backing is governed by **[[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]]**; legality gates are governed by **[[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]**. Therefore [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]] specifies *where the references live*, *what must be citeable for audit*, and *how governance changes trigger regression* — not mechanism semantics.
>
> **Reader guide (fast navigation).**
> - “What does `NormalizationMethodId/…InstanceId/≡_UNM/NormalizationFix` mean?” → **[[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]**.
> - “What is an Indicator / `IndicatorChoicePolicy` and why NCV ≠ Indicator?” → **[[A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]**.
> - “Why can we trust a normalization / where does calibration or evidence live?” → **[[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]] (MM‑CHR)**.
> - “What is admissible to compare or aggregate, and what is `MinimalEvidence`?” -> **[[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]] (CG-Spec)**.

## [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:1 - Context

[[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]] established a substrate‑neutral picture:

* a **CN‑frame** = *(Context‑local)* **CharacteristicSpace (CS)** + **chart** (coordinate patch + units) + a referenced **Normalization mechanism (UNM)** pinned from `CN‑Spec.normalization`. Any semantics of admissibility, invariants, and `≡_UNM` is governed by the [[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]] governing pattern (see **[[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]**);
...
**Terminology guard.** *CN‑frame* is the **lens** (I); *CN‑Spec* is the **governance card** (S) that fixes admissible charts/normalization *references*/comparability/Γ‑fold for that lens **in one `U.BoundedContext`**; *CN‑Description* is the didactic surface (D) with worked examples and anti‑patterns. Mechanism‑level term cards (e.g., `NormalizationMethod`, `NormalizationMethodInstance`, `NCV`, `≡_UNM`, `IndicatorChoicePolicy`) are governed by the corresponding **[[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]].<MechId>** patterns and are only **cited** here.

**Lexical guard (map/Map, by reference).** Follow the lexical discipline governed by **[[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]**: avoid introducing new normalization tokens that use “map/Map/mapping” (because `…Map` is a Part‑G method‑type kind). In normalization contexts prefer **normalize / transform / re‑parameterize**. Legacy tokens (including retired κ‑notation) are handled via **alias docking** ([[F.18 - Local‑First Unification Naming Protocol|F.18]]); [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]] applies this rule and does not redefine it.

[[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]] makes this *operational and auditable*.

## [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:2 - Problem
...
4. **Un‑bridgeable reports.** Cross‑team roll‑ups average **incongruent** CN‑frames, violating the **weakest‑link (WLNK)** discipline from Γ and [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]].

## [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:3 - Forces
...
## [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:4 - Solution — **The CN‑Spec** (CN‑Spec) + **Registry** + **Bridges**

### [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:4.1 - The **CN‑Spec** (comparability & normalization specification per CN‑frame, in one `U.BoundedContext`)
...
CN‑Spec {
  ...
  cs_basis          : [{
    ...
    characteristic  : <U.Characteristic>,          // per [[A.17 - Canonical “Characteristic” (A.CHR‑NORM)|A.17]] / [[A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC‑KERNEL)|A.18]]
  ...
  normalization     : {
   UNM_id?,                                      // reference to the UNM mechanism; canonical Intension: [[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]
   methods: [NormalizationMethodId],              // A.19.UNM-governed tokens; semantics are governed by [[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]
   instances?: [NormalizationMethodInstanceId],   // A.19.UNM-governed tokens; evidence/backing lives in [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]]
   method_descriptions: [NormalizationMethodDescriptionRef], // refs only; semantics and corpus live with [[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]
   admissible_reparameterizations,                // A.19.UNM-governed declarations (opaque here; see [[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]])
   invariants,                                    // A.19.UNM-governed invariant tokens (opaque here; see [[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]])
   fix?: <NormalizationFixSpec>                   // A.19.UNM-governed fix spec (opaque here; see [[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]])
   }
  comparability     : { mode ∈ {coordinatewise, normalization-based}, minimal_evidence }  // `minimal_evidence` is a gate reference (default: CG‑Spec.MinimalEvidence; see [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]] and [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]])
  indicator_policy? : { IndicatorChoicePolicyRef, scope, edition }  // policy ref only; semantics governed by [[A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]
  acceptance        : { checklist_for_admission, window, evidence_anchors } // gates RSG state checks
  aggregation       : { Γ_fold, WLNK/COMM/LOC/MONO choices, time_policy }   // fold tokens only; semantics governed by [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] and [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]] (and the folding mechanism card, if cited)
...
**Governing-pattern assignment note.** CN-Spec stores only the **governance references and declarations**. The semantics and term cards for `NormalizationMethod*`, `≡_UNM`, `NCV`, `IndicatorChoicePolicy`, and any other CHR-mechanism vocabulary are governed by the corresponding mechanism-governing patterns such as **[[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]** and **[[A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]**; evidence backing lives in **[[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]]**. (Kernel reminder: per **A19‑CS‑5**, `U.CharacteristicSpace` carries no hidden normalizations or aggregations.) In [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] terms, `UNM_id` points to a canonical **`U.Mechanism.Intension`** card; the CN‑Spec **references** that mechanism and does **not** introduce implicit **Transport**.

**L‑CN‑Spec‑NORM‑IDs (by reference).** When CN‑Spec (or its audit trail) needs stable normalization tokens, use **NormalizationMethodId**/**NormalizationMethodInstanceId** as specified by [[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]. Avoid generic “map” nouns and retired κ‑notation (see the **[[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]** lexical guard); preserve retired tokens only via **[[F.18 - Local‑First Unification Naming Protocol|F.18]] alias docking**. If you introduce reference‑typed fields, obey **[[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]** (`*Ref` reserved for reference fields; `*Slot` reserved for SlotKinds).

### [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:4.2 - **CN‑frame Registry** (per Context)
...
### [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:4.3 - **Bridges** (across contexts)

Cross‑context reuse occurs **only** via explicit **Alignment Bridges** ([[F.9 - Alignment & Bridge across Contexts|F.9]]) between CN‑Specs:

```
Bridge CN‑frameA@Context1  →  CN‑frameB@Context2
  channel: {Scope|Kind}                 // [[F.9 - Alignment & Bridge across Contexts|F.9]] (and [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] Transport)
  planes: ReferencePlane(src,tgt)       // [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] (must be recorded)
  CL: {3|2|1|0}
  CL_plane?: {3|2|1|0}                  // only when planes differ
  kept_characteristics: [… ]
  lost_characteristics: [… ]
  transform: {pullback | pushforward | re-scaling | re-binning | … }  // illustrative; use the operator vocabulary governed by [[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]] and [[F.9 - Alignment & Bridge across Contexts|F.9]]
  extra_guards: {additional evidence, review role, or waiver speech act}
```

**CL policy (reference).** **CL levels and the penalty Φ(CL) are defined in [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]** (CL is **ordinal**; do not average). In [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] terms, any cross‑context (or cross‑plane) reuse is declared **only** via a mechanism’s **Transport** clause: **name the BridgeId and channel** (`Scope|Kind`) and **record** `ReferencePlane(src,tgt)`; if planes differ, declare the `CL^plane` regime. **Transport is declarative** (it does not introduce a `U.Transfer` edge and does not restate CL ladders or Φ tables). When both scope and *entityOfConcern* change, apply the **two‑bridge rule** (Scope bridge + **KindBridge (CL^k)**). Penalties from scope/kind/plane **route to `R/R_eff` only** (never to **F/G**). This CN‑Spec may **add operational guards** per level (e.g., “extra reviewer at CL=1”, “waiver at CL=2”), but it **does not redefine** the scale or Φ. For episteme‑specific frames, see also **[[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]**.

## [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:5 - Conformance Checklist (normative)
...
**CC‑A19.D1‑7 (Aggregation discipline).** `aggregation.Γ_fold` **MUST** specify WLNK/COMM/LOC/MONO choices and the **time policy** (e.g., average of rates vs integral of counts). **No free‑hand averages.** The legality/semantics of folding is governed by **[[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]** and **[[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]** (and, when a folding mechanism is cited, by its mechanism-governing pattern); CN‑Spec only stores the governance pins.

**CC‑A19.D1‑8 (Bridge‑only reuse).** Cross‑context consumption **MUST** cite a **Bridge** with: (i) `channel ∈ {Scope|Kind}`, (ii) recorded `ReferencePlane(src,tgt)`, (iii) CL (and `CL^plane` when planes differ), and (iv) **loss notes**; coordinate‑by‑name without a Bridge **fails**. If the data participate in **gating/assurance**, apply **Φ(CL) per [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]**; this CN‑Spec does not restate Φ.
...
**CC‑A19.D1‑10 (Maintenance, deprecation, and DRR).** Every CN-Spec **MUST** carry a **source-maintenance role assignment**, a **deprecation plan**, and links to **DRR** entries for rationale and changes (Part [[E.9 - Design‑Rationale Record (DRR) Method|E.9]]).

**CC‑A19.D1‑11 (Anchors & lanes for comparability).** Any **admission** into a CN‑frame that is later **used for comparison/aggregation** **SHALL** cite the corresponding **[[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] EvidenceRole** anchors for each characteristic, with **assuranceUse lane** tags {TA, VA, LA} and **validity windows** (where applicable), so that the **SCR** can report lane‑separated contributions and freshness ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]). Absence of anchors for a required characteristic renders items **incomparable**.

**CC‑A19.D1‑12 (Notation independence).** CN‑Spec content **MUST NOT** depend on a tool or file format; semantics precede notation ([[E.5.2 - Notational Independence|E.5.2]] Notational Independence).
...
## [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:6 - Consequences (informative)
...
## [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:7 - Rationale (informative)

The CN‑Spec aligns [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]] with **Part E**: it packages Tell‑Show‑Show, Conformance Checklists, and DRR‑backed change, while honouring **DevOps Lexical Firewall**, **Unidirectional Dependency**, and **Notational Independence** so that semantics never depend on tooling.  It also operationalises [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] **Trust & Assurance** by making CL penalties and WLNK folds first‑class.

## [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:8 - Archetypal Grounding *(Tell‑Show‑Show)*

> **Same slots, three arenas; no tooling implied.** The examples below use plain-language normalization descriptions as placeholders; any normative use must cite A.19.UNM-governed ids/refs ([[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]) and evidence pins ([[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]]), not invent new terminology here.

### [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:8.1 - **Industrial line** — *Weld‑quality CN‑frame* (`AssemblyLine_2026`)
...
### [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:8.2 - **Software/SRE line** — *Latency CN‑frame* (`SRE_Prod_Cluster_EU_2026`)
...
### [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:8.3 - **Clinical/episteme line** — *Trial‑outcome CN‑frame* (`Cardio_2026`)
...
### [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:8.4 - Worked mini-schemas (entity/relational mixtures across CN‑frames, informative)
...
#### [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:8.4.2 - Assurance CN‑frame — Evidence freshness & mapped comparisons
...
* A **NormalizationMethodInstance** reference records that an admitted comparison/assertion relied on a declared normalization instance. The admissibility conditions, monotonicity constraints and evidence semantics are governed by **[[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]** and **[[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]]**.
...
| Table                | Key Columns (essential)                                                                                                               |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| **NORMALIZATION\_METHOD** | `NORMALIZATION_METHOD_ID` (PK); `KIND` (token; see [[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]); `DESCRIPTION_REF` |
| **NORMALIZATION\_INSTANCE** | `NORMALIZATION_INSTANCE_ID` (PK); `NORMALIZATION_METHOD_ID` (FK); `SRC_CHAR_ID`; `TGT_CHAR_ID`; `FORMULA_SPEC|LUT_REF` (illustrative); `VALIDITY_WINDOW` (illustrative); `EVIDENCE_REF` (pin/ref; see [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]]) |
...
In this stub, **NORMALIZATION\_INSTANCE** records a mapping instance that has to be accounted for when reconstructing an assertion or comparison. The exact meaning of `FORMULA_SPEC`/`VALIDITY_WINDOW`/evidence pins is governed by the UNM and evidence patterns ([[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]] / [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]]); the point here is that the instance is **referenceable** so audits can follow it. The Bridge table enumerates official Bridges between contexts (for example, bridging a “Ready” state in an engineering context to “Ready” in an operations context, with CL indicating how fully comparable they are). An ASSURANCE\_EVENT log could record when a penalty was applied due to a low-CL Bridge or when an assertion was refreshed or invalidated due to new evidence or time lapse.

#### [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:8.4.3 Alignment CN‑frame — Design-time reuse of states across Contexts
...
## [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:9 - Anti‑patterns (and the fix)
...
| **Normalization smuggling**     | Quiet “per‑unit” normalisation mid‑stream | Trend reversal               | Declare UNM normalization references (`NormalizationMethodId` / `NormalizationMethodInstanceId`) + named invariants (see [[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]])        |
...
## [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:10 - Didactic quick cards (one‑liners teams reuse)
...
11. **Use method/instance IDs; avoid generic “map” nouns.** Prefer `NormalizationMethodId`/`NormalizationMethodInstanceId` (see the **[[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]** lexical guard).

## [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:11 - SCR / RSCR Harness (acceptance & regression)

> **These are concept‑level checks; notation‑agnostic.**

### [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:11.1 - **SCR — Acceptance (first introduction)**
...
* **SCR‑A19.4‑S02 (Normalization clarity).** `normalization` cites the UNM mechanism (`UNM_id?`) and provides the normalization references required by the [[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]] governing pattern (methods / invariants / fix, and instances when used). If instances are referenced in assurance logs, their evidence/backing and validity constraints are handled by the governing evidence pattern ([[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]]), not by [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]].
...
* **SCR‑A19.4‑S06 (entityOfConcern & anchors surfaced).** For each CN‑Spec characteristic used in the worked example, cite the corresponding CHR Characteristic name and the evidence anchor(s) ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]) that make the reading observable in this Context.

### [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:11.2 - **RSCR — Regression (on change)**
...
## [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:12 - Interaction summary (wiring to the rest of the kernel)

* **[[A.2 - Role Taxonomy|A.2]] / [[A.2.5 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]] (Roles / RSG).** RSG **checklists** quote **CN‑Spec.acceptance**; enactment gates rely on **admitted** CN‑frame data.
* **[[B.1 - Universal Algebra of Aggregation (Γ)|B.1]] (Γ‑algebra).** CN‑Spec’s `Γ_fold` instantiates Γ\_ctx/Γ\_time/WLNK/MONO choices explicitly.
* **[[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (Assurance).** Bridge CL enters the **R** term; WLNK protects safety roll‑ups.
* **C.6 / C.7 (LOG‑CAL / CHR‑CAL).** Units, scales, and measurement templates come from CHR; proofs about folds live in LOG‑CAL.

## [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]]:13 - Minimal CN‑Spec template (copy/paste, informational)

**Template note (refs-only).** This template shows *slot placement* for governance. Token semantics for normalization belong to the [[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]] governing pattern ([[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]]); indicatorization semantics belong to the indicatorization governing pattern (e.g., [[A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]]); evidence/backing semantics belong to [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]]; legality/evidence gates belong to [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]].
...
Normalization:
  ...
  invariants: [<property>… ]           # what ≡_UNM preserves (token semantics: see [[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]])
  fix?: <NormalizationFixSpec>          # canonical representative of the ≡_UNM class (token semantics: see [[A.19.UNM - Unified Normalization Mechanism (UNM)|A.19.UNM]])
Indicators (optional):
  policy_ref: <IndicatorChoicePolicyRef>
  resulting_indicators: [<IndicatorId>… ] // selection is policy‑defined; NCVs alone do not make an Indicator (see [[A.19.UINDM - Unified Indicatorization Mechanism (UINDM)|A.19.UINDM]])
Comparability:
  mode: coordinatewise | normalization-based
  minimal_evidence: <what must be observed to compare>  # legality/evidence gate surface (see [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]] and [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]])
Aggregation:
