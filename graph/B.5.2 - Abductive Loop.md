...
**Builds on.**
`B.5 Canonical Reasoning Cycle`, `B.5.1 Exploration`, `B.5.2.0 U.AbductivePrompt`, [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]], [[B.3.3 - Assurance Subtypes & Levels|B.3.3]].

**Coordinates with.**
`B.4.1 Observe-Notice-Stabilize-Route` for pre-abductive routing, [[A.16 - Language-State Move Coordination|A.16]] for admissible language-state moves, [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]] for lexical repair before hypothesis publication, and [[C.16.Q - Quality-Term Precision Restoration|C.16.Q]] / [[A.6.A - U.ActionInvitationPrecisionRestoration - Affordance and Action-Invitation Precision Restoration (ACT-INV)|A.6.A]] when the initiating publication face or cue is evaluative or action-inviting rather than explanatory.

## [[B.5.2 - Abductive Loop|B.5.2]]:1 - Problem frame
...
## [[B.5.2 - Abductive Loop|B.5.2]]:2 - Problem
...
## [[B.5.2 - Abductive Loop|B.5.2]]:3 - Forces
...
## [[B.5.2 - Abductive Loop|B.5.2]]:4 - Solution - Structured abductive micro-cycle

[[B.5.2 - Abductive Loop|B.5.2]] defines abduction as a typed, iterative micro-cycle that begins from an admissible `U.AbductivePrompt`, expands a candidate set, filters that set by explicit plausibility criteria, and publishes one selected conjecture as a new `U.Episteme` with `AssuranceLevel:L0`.

### [[B.5.2 - Abductive Loop|B.5.2]]:4.1 - Nature of abduction in FPF
...
### [[B.5.2 - Abductive Loop|B.5.2]]:4.2 - Four-step micro-cycle
...
### [[B.5.2 - Abductive Loop|B.5.2]]:4.3 - Entry discipline via `U.AbductivePrompt`

`AnomalyStatement` remains a canonical prompt species, but it is not the only one. [[B.5.2 - Abductive Loop|B.5.2]] also accepts the broader prompt species governed by [[B.5.2.0 - U.AbductivePrompt|B.5.2.0]], such as `ProblemCuePrompt`, `OpportunityCuePrompt`, and `ProbeCuePrompt`. This broadens entry without dissolving type discipline.

### [[B.5.2 - Abductive Loop|B.5.2]]:4.4 - Plausibility filters
...
## [[B.5.2 - Abductive Loop|B.5.2]]:5 - Archetypal Grounding
...
## [[B.5.2 - Abductive Loop|B.5.2]]:6 - Bias-Annotation

This pattern biases authors toward visible candidate plurality, explicit plausibility criteria, and persistent prompt provenance. That bias is intentional. [[B.5.2 - Abductive Loop|B.5.2]] would rather keep early conjectures slightly over-exposed than let their origin and selection grounds disappear.

## [[B.5.2 - Abductive Loop|B.5.2]]:7 - Conformance Checklist

- `CC-[[B.5 - Canonical Reasoning Cycle|B.5]].2-1` Every abductive run **SHALL** begin from a declared `U.AbductivePrompt`; arbitrary prose fragments are not sufficient prompt-entry forms.
- `CC-[[B.5 - Canonical Reasoning Cycle|B.5]].2-2` A conforming abductive run **SHALL** record at least one rival candidate alongside any selected prime hypothesis, unless the author explicitly justifies why no rival candidate was available.
- `CC-[[B.5 - Canonical Reasoning Cycle|B.5]].2-3` Selection of a prime hypothesis **SHALL** cite at least two explicit plausibility filters.
- `CC-[[B.5 - Canonical Reasoning Cycle|B.5]].2-4` The selected prime hypothesis **SHALL** be published as a new `U.Episteme` with `AssuranceLevel:L0`.
- `CC-[[B.5 - Canonical Reasoning Cycle|B.5]].2-5` The prime hypothesis record **SHALL** preserve a link to the initiating prompt and to the filtering rationale that justified selection.
- `CC-[[B.5 - Canonical Reasoning Cycle|B.5]].2-6` A hypothesis that cannot support any downstream deduction, probe design, or evidence path **SHALL NOT** be presented as a conforming abductive result.

## [[B.5.2 - Abductive Loop|B.5.2]]:8 - Common Anti-Patterns and How to Avoid Them

| Anti-pattern | What it looks like | How FPF prevents it |
|---|---|---|
| **Authority candidate** | One favored conjecture is advanced immediately, with no rival set and no explicit filtering. | `CC-[[B.5 - Canonical Reasoning Cycle|B.5]].2-2` and `CC-[[B.5 - Canonical Reasoning Cycle|B.5]].2-3` require candidate plurality and visible plausibility grounds. |
| **Untestable grand conjecture** | The candidate sounds deep or comprehensive, but it creates no admissible next step for checking, probing, or deduction. | `CC-[[B.5 - Canonical Reasoning Cycle|B.5]].2-6` rejects prime hypotheses that cannot open a downstream path. |
| **Prompt amnesia** | A later reader can see the conjecture but not the initiating anomaly, opportunity, or probe pressure. | `CC-[[B.5 - Canonical Reasoning Cycle|B.5]].2-1` and `CC-[[B.5 - Canonical Reasoning Cycle|B.5]].2-5` keep prompt provenance attached. |
...
## [[B.5.2 - Abductive Loop|B.5.2]]:9 - Consequences
...
## [[B.5.2 - Abductive Loop|B.5.2]]:10 - Rationale

The Canonical Reasoning Cycle needs a disciplined beginning that is neither over-formalized nor mystical. [[B.5.2 - Abductive Loop|B.5.2]] supplies that beginning. It keeps hypothesis generation explicit, connects it to typed prompt publications, and prepares the output for later assurance work without pretending that early plausibility is already evidence.

## [[B.5.2 - Abductive Loop|B.5.2]]:11 - SoTA-Echoing
...
## [[B.5.2 - Abductive Loop|B.5.2]]:12 - Relations

- **Is the first reasoning phase within:** `B.5 Canonical Reasoning Cycle`.
- **Typically operates during:** `B.5.1 Exploration`.
- **Consumes:** `U.AbductivePrompt` publications from [[B.5.2.0 - U.AbductivePrompt|B.5.2.0]], often reached through [[B.4.1 - Observe -- Notice -- Stabilize -- Route|B.4.1]] and [[A.16 - Language-State Move Coordination|A.16]].
...
### [[B.5.2 - Abductive Loop|B.5.2]]:12.1 - Prompt-entry broadening via `U.AbductivePrompt`

Older wording that makes `AnomalyStatement` the exclusive entry form is superseded. [[B.5.2 - Abductive Loop|B.5.2]] accepts `U.AbductivePrompt`, where `AnomalyStatement` remains one canonical species alongside cue-derived prompt species such as `ProblemCuePrompt`, `OpportunityCuePrompt`, and `ProbeCuePrompt`.

## [[B.5.2 - Abductive Loop|B.5.2]]:13 - Prompt, Candidate, and Hypothesis Package Discipline
...
### [[B.5.2 - Abductive Loop|B.5.2]]:13.1 - Prompt package
...
If the initiating publication is still primarily evaluative, action-inviting, or lexically overloaded, it should first be repaired by the relevant [[A.6 - Signature Stack & Boundary Discipline|A.6]] family before it is treated as a stable abductive prompt. [[B.5.2 - Abductive Loop|B.5.2]] assumes typed entry, not raw lexical ambiguity.

### [[B.5.2 - Abductive Loop|B.5.2]]:13.2 - Candidate-set note
...
### [[B.5.2 - Abductive Loop|B.5.2]]:13.3 - Prime-hypothesis record
...
This is how [[B.5.2 - Abductive Loop|B.5.2]] stays connected to the rest of the reasoning cycle. The abductive loop does not merely emit an idea; it emits a conjecture with explicit handoff terms.

## [[B.5.2 - Abductive Loop|B.5.2]]:14 - Admissible Transitions, Abort Paths, and Reopening
...
### [[B.5.2 - Abductive Loop|B.5.2]]:14.1 - Relation to [[B.4.1 - Observe -- Notice -- Stabilize -- Route|B.4.1]] and [[A.16 - Language-State Move Coordination|A.16]]

[[B.4.1 - Observe -- Notice -- Stabilize -- Route|B.4.1]] and [[A.16 - Language-State Move Coordination|A.16]] often supply the pre-abductive seam. They help preserve, stabilize, and route upstream publications before they are fit for explicit conjecture. [[B.5.2 - Abductive Loop|B.5.2]] begins only once the current publication is ready to function as an abductive prompt. This boundary matters because it prevents two opposite errors:
...
### [[B.5.2 - Abductive Loop|B.5.2]]:14.2 - Abort, defer, and split cases
...
### [[B.5.2 - Abductive Loop|B.5.2]]:14.3 - Reopening and rival reinstatement

A prime hypothesis may later lose support under deduction, probe results, or new evidence. When that happens, [[B.5.2 - Abductive Loop|B.5.2]] prefers explicit reopening to silent replacement.
...
### [[B.5.2 - Abductive Loop|B.5.2]]:14.4 - Scope discipline during iteration

Abductive drift often comes from silent scope expansion. A conjecture first framed for one target slice quietly becomes a universal explanation. [[B.5.2 - Abductive Loop|B.5.2]] therefore expects scope discipline to remain explicit during iteration. If a candidate requires a broader or narrower scope than the prompt originally declared, that scope move should be stated rather than smuggled in under the rhetoric of a "better explanation."

## [[B.5.2 - Abductive Loop|B.5.2]]:15 - Worked Examples

### [[B.5.2 - Abductive Loop|B.5.2]]:15.1 - Service degradation diagnosis
...
### [[B.5.2 - Abductive Loop|B.5.2]]:15.2 - Opportunity-driven materials inquiry
...
### [[B.5.2 - Abductive Loop|B.5.2]]:15.3 - Probe-driven theory repair
...
The selected prime hypothesis is the mediating construct, but the scope-restriction candidate remains stored as a live rival because it could still outperform if later deductions fail. This example illustrates why [[B.5.2 - Abductive Loop|B.5.2]] tracks the rival set rather than only the currently favored conjecture.

## [[B.5.2 - Abductive Loop|B.5.2]]:16 - Authoring and Review Guidance

### [[B.5.2 - Abductive Loop|B.5.2]]:16.1 - For authors
...
If those answers cannot be given, the publication is probably not yet at [[B.5.2 - Abductive Loop|B.5.2]] and should return to prompt-shaping or lexical repair.

### [[B.5.2 - Abductive Loop|B.5.2]]:16.2 - For reviewers
...
### [[B.5.2 - Abductive Loop|B.5.2]]:16.3 - For integrators and assurance leads

Integrators should remember that `L0` is still early assurance. [[B.5.2 - Abductive Loop|B.5.2]] supplies disciplined conjectures, not corroborated claims. Its value is that it exposes where deduction, method design, and evidence acquisition should now concentrate. Assurance leads therefore should preserve the prompt link and the filter rationale rather than flattening the conjecture into a decontextualized work item.

## [[B.5.2 - Abductive Loop|B.5.2]]:17 - Migration and Boundary Notes

### [[B.5.2 - Abductive Loop|B.5.2]]:17.1 - Migration from anomaly monopoly
...
### [[B.5.2 - Abductive Loop|B.5.2]]:17.2 - Migration from inspiration rhetoric
...
### [[B.5.2 - Abductive Loop|B.5.2]]:17.3 - Boundary to deduction and evidence

[[B.5.2 - Abductive Loop|B.5.2]] ends when one conjecture is published as a prime `L0` hypothesis or when the run is explicitly aborted, deferred, or split. Deduction, evidence acquisition, and later assurance do not belong to the abductive loop itself, even though the loop must prepare a clean handoff to them.

## [[B.5.2 - Abductive Loop|B.5.2]]:End
