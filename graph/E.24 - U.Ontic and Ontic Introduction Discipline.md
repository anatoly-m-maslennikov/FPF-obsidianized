...
## [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:0 - Use This When
...
- If the issue is only one wording-use repair row, use [[E.10 - Unified Lexical Rules for FPF|E.10]] and [[E.10.ARCH - Wording-Use Ontological Precision Restoration Architecture|E.10.ARCH]].
- If the issue is only a new or revised mechanism meaning, use [[E.20 - Mechanism Introduction Protocol|E.20]].
- If the issue is only durable naming, use [[F.18 - Local‑First Unification Naming Protocol|F.18]].
- If the issue is only a pattern publication-form or section-order matter, use [[E.8 - FPF Authoring Conventions & Style Guide|E.8]].

## [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:1 - Problem Frame

Some FPF governed objects are small enough to define with one relation or one record. Others require a durable ontic. `U.Episteme` is the central example: it needs identity criteria, typed slots, slot-filling discipline, filled assignments, card and publication species, description boundary, publication-form boundary, relation to `U.Signature`, and dependent episteme-morphism and publication patterns. [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] works because it makes the small ontic slot relation explicit.
...
[[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] governs that ontic-introduction decision.

## [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:2 - Problem
...
## [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:3 - Forces
...
| Naming vs ontology | [[F.18 - Local‑First Unification Naming Protocol|F.18]] can make a name better, but naming cannot decide the kind, slot relation, species, and dependent-pattern duties by itself. |

## [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:4 - Solution

This pattern selects `U.Ontic` as the FPF kind for an ontic. `U.Ontic` is the `EntityOfConcern` of [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]: a connected ontology fragment whose stable identity, slots, admissible slot values, neighboring ontology units, dependent pattern obligations, and non-use boundary must be held together before FPF can use that fragment safely in action-facing patterns.
...
The `U.Ontic` decision is selected because the repeated semantic-area, ontic-slot-relation, ontological-neighborhood, and dependent-pattern braid is now itself a governed object in FPF. Without a named kind, the same architecture unit would be re-described as a semantic area, pattern nest, ontology family, local frame, slot relation, or description and publication arrangement in different places, recreating the duplicate-ontology problem [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] is meant to prevent. With `U.Ontic`, DRRs and patterns can cite one kind for the ontology-architecture unit while still keeping each filled value under its own governing pattern.

The cost is kernel growth and metamodel risk. [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] contains that cost by making `U.Ontic` narrow. A local use frame, source label, project-side expression, recurring table, pattern nest, or draft ToC row is not a `U.Ontic` merely because it looks ontology-shaped. It becomes a `U.Ontic` only when the [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] decision names stable identity, an ontic slot relation, selected semantic area, selected ontological neighborhood, dependent pattern obligations, existing-pattern reuse, and non-use boundary by value.
...
- If the name has independent `EntityOfConcern` identity, stable identity criteria, a governing pattern, admissible use boundaries, and dependent-pattern reliance, it may remain or become a `U.*` kind after an [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] decision.
...
Role participation therefore uses slot discipline without demoting `U.Role` to a local field and without turning every relation participant into a role. A system acting as an engineer is still the same `U.System` filling the holon-typed holder slot in an assignment such as `System#EngineerRole:EngineeringContext@Window`; the role name does not create a new system kind. A performed `U.Work` then points to that assignment through `performedBy -> U.RoleAssignment` and to the enacted way of doing through `enactsMethod -> U.Method`. If a source word such as "engineer", "reviewer", or "operator" is under repair, recover the holder, `U.Role`, bounded context, assignment relation, temporal window, work, method, and method-description values through [[A.2 - Role Taxonomy|A.2]], [[A.2.1 - U.RoleAssignment- Contextual Role Assignment|A.2.1]], and [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] rather than minting a participation type.

Specialized participation words still require care. A role-like, method-like, mechanism-like, source-like, temporal, or publication-like word may name either a slot position or a governed filler. [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] does not settle every subject ontology by itself. It requires the author to keep the slot position and the filled value distinct, then use the governing pattern for the filled value or run a separate [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] decision when the participation family itself appears to need a durable ontic. Ordinary phrases such as "the television plays the role of transformed object" are not enough to create `U.Role`: the television fills `transformedEntityOrStructure` in the `U.Transformation` core; it is not thereby assigned a contextual functional mask.
...
3. **Filled value assignment or ordinary-use core.** Give a compact filled instance or first-use frame only when users need one concrete application shape. It fills the type-level slots; it is not a second ontology and not a competing slot relation. Under [[C.29 - Mathematical Lens Use|C.29]], that filled assignment may be viewed as a tuple when tuple reasoning is current.
4. **Description episteme and publication.** Claims about the ontic, its slots, its slot fillers, or relations among those values use [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]; a pattern section, table, diagram, publication, card, or view may describe the ontic, but it is not the ontic.
...
Not every ontic needs every named layer. Add a named signature or kind level only when dependent patterns must rely on slot constraints across species or morphisms. Add a named filled-value assignment only when patterns need a publication-form-agnostic filled value; use [[C.29 - Mathematical Lens Use|C.29]] tuple-view language only when tuple reasoning is current. Add named card, view, or publication species only when holonic working forms, views, or publication forms are themselves governed by the ontic. Name attached or non-slot components only when common adjacent structures must stay recoverable while remaining outside the ontic identity.

Keep annotation proportional. [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] requires source-ontology recovery only for wording that can change the ontic, slot, filled value, governing pattern, admissible use, or dependent-pattern obligation. If a domain sentence already preserves those values, do not replace it with a typed paraphrase merely to show that an ontic exists.
...
Precision restoration uses the same discipline without turning it into lexical style. First recover the ad hoc ontic implied by the source situation: which meaning area, candidate object of concern, slots, neighboring patterns, and typed values are being compressed by the wording or source-side situation. Then repair toward the normative FPF ontic and linked typed values when such an ontic exists. If no normative ontic exists, use the direct governing patterns, keep the frame local, or open an [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] ontic-introduction decision.

[[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] is the governing description pattern for `U.Ontic`. In that sense it is the ontic-of-ontics pattern: it describes the `U.Ontic` EoC, its slot relation, and its decision discipline. That self-application is allowed only under the same checks it imposes on other ontics; it is not a license for every local ontology-shaped bundle to become a `U.*` kind.

[[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] is compatible with modular ontology and ontology-design-pattern practice: modular ontology libraries and ontology design patterns show why reusable small ontology structures matter, and recent process-modeling work shows that implicit process patterns must be made explicit for reuse. [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] is narrower and more FPF-specific: it selects when FPF should introduce a durable action-facing ontic, rather than importing an external microtheory or treating every reusable repair table as ontology.

If the choice between "write an ontic" and "keep the existing pattern constellation" needs reusable scoring, build a separate evaluation `CharacteristicSpace` through [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]. The evaluated object is then the FPF pattern-set architecture alternative, not the ontic itself: current constellation, local frame, or durable ontic. Candidate architecture characteristics include cross-pattern coupling, subject cohesion, explicit `onticSlotRelation` and `SlotSpec` discipline, invariant recoverability, duplicate-ontology resistance, dependent-pattern thinness, change-impact locality, first-use burden, and FPF ecology fit. [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] uses these as diagnostic pressure for the introduction decision; it does not itself become the full architecture-characteristic evaluation pattern.

Do not grow [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] by adding full publication-section rules, adequacy scales, wording-use restoration rules, or a general architecture-characteristic theory. [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] keeps only the object split and the ontic-introduction decision needed before dependent patterns rely on a durable ontic.

Use the current split this way:

- use [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] for `U.Ontic` identity, type-level `onticSlotRelation`, semantic area, ontological neighborhood, dependent-pattern obligations, and non-use boundary;
- use [[E.24.CD - Ontic Candidate Detection|E.24.CD]] when the current problem is detection of an ontic candidate, hidden-form classification, or sufficiency rationale for deciding whether a recurring construct is a durable ontic, a local use frame, direct governing-pattern use, or source wording only;
- use [[E.24.PUB - Ontic Description and Publication Discipline|E.24.PUB]] when the current problem is the distinction among the ontic, the ontic-description episteme, the publication of that description, and publication forms such as cards, records, tables, schemas, diagrams, views, or source packets;
- use [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] only when the contested question is how to construct an evaluation `CharacteristicSpace` for comparing pattern-set architecture alternatives, such as current constellation, local frame, or durable ontic.

This split keeps [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] ontic-first. Candidate detection, publication discipline, and contested evaluation remain neighboring governed objects rather than sections that make [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] a general discovery, documentation, or scoring pattern.


Introduce or rely on a durable FPF ontic only after the ontic-introduction decision satisfies four checks.

### [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:4.1 - Check 1: Existing Governing Pattern Check

Name the current claim under decision and ask whether an existing pattern already carries it.

Use direct governing patterns first. If the case is method semantics, use [[A.3.1 - U.Method- Context-Defined Way of Doing|A.3.1]]; if it is method description, use [[A.3.2 - U.MethodDescription- Description Episteme for a Way of Doing|A.3.2]]; if it is mechanism meaning, use [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] and [[E.20 - Mechanism Introduction Protocol|E.20]]; if it is work planning or dated work, use [[A.15.2 - U.WorkPlan|A.15.2]] or [[A.15.1 - U.Work|A.15.1]]; if it is evidence, gate, source, assurance, decision, release, or publication use, use that governing pattern.
...
### [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:4.2 - Check 2: Stable Identity Test
...
### [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:4.3 - Check 3: Typed Slot Relation Test
...
OnticIntroductionDecisionCard:
  ...
  filledAssignmentExample: pump-station backup architecture change, with [[A.3.4 - U.Transformation- Bounded Change Under Conditions|A.3.4]] transformation core, [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] method and work chain, [[C.27.TA - Temporal Aspect- Time Windows, Rhythm, Cadence, and Currentness|C.27.TA]] two-release-cycle recovery timing, and evidence references plus result references only where those claims are being made.
  publicationBoundary: [[A.3.4 - U.Transformation- Bounded Change Under Conditions|A.3.4]] describes the ontic and its slot relation; a table, card, diagram, or transformation-flow view may publish that description but is not the transformation.
...
For [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] itself, this record is already decided: `ProposedOnticName = Ontic`, `OnticAsEntityOfConcern = connected ontology fragment under FPF settlement`, and `UKindDecision.verdict = selected U-kind` with `selectedUKindName = U.Ontic`. Other proposed ontics must still fill the record by value; they do not inherit the `U.*` decision from [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]].

The slot relation must use [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] slot discipline and must not define a second slot discipline. A role-like, method-like, mechanism-like, source-like, publication-like, temporal, or architecture-like slot-position label is not a kind decision. It becomes a kind decision only when the governing pattern names that filled value by value and admits that kind.

### [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:4.4 - Check 4: Placement and Dependent-Pattern Obligation
...
- names that pass [[F.18 - Local‑First Unification Naming Protocol|F.18]];
- evaluation pattern for the resulting pattern or DRR, usually [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] for a pattern and [[E.9.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]] for the DRR.
...
### [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:4.5 - Local Use Frame Decision
...
## [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:5 - Archetypal Grounding
...
### [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:5.1 - Episteme as Durable Ontic

`U.Episteme` passes [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]. It has stable identity, a normative `U.EpistemeSlotRelation`, required slots, optional slots, filled assignments, card and publication species, a description boundary, a publication-form boundary, and dependent patterns in [[C.2 - Epistemic holon composition (KD-CAL)|C.2]], A.6.2-A.6.4, and [[E.17 - Multi‑View Publication Kit|E.17]]. [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] is therefore the right form: a subject pattern with a small typed ontic slot relation and dependent-pattern obligations.

### [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:5.2 - Multi-Pattern Subject Matter as an Ontic-Candidate Archetype

A project phrase such as "algorithm", "process", "solver", "workflow", "system", "quality", "time", "source", or "architecture" can point to one recognizable subject that is spread across several FPF values and patterns. The point of this archetype is not that all such subjects are one kind. The point is that [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] must decide the status of the cross-pattern subject before patterns rely on it.

In this archetype, "process" and "workflow" are source-side labels until recovered. They may point to method, method description, work plan, dated work, transformation-flow structure, evidence relation, source relation, gate relation, result relation, publication relation, or another governed value. They become durable FPF ontology only through the same [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] decision as any other proposed ontic; otherwise keep them quote-only, reduced-use, or mapped to the governing patterns that already carry the claim.

The [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] move is:
...
The same test applies to any draft-only locus. If the label has no current governing text, do not cite it as ontology. Either cite current governing patterns, keep the label as investigation context, or open an [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] ontic-introduction decision.

### [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:5.4 - System-Like Head Concepts

`system`, `episteme`, `architecture`, `method`, `mechanism`, `temporal claim`, `dynamics`, and `change` can each appear as a broad head for many dependent FPF patterns. That breadth is not itself enough to create a durable FPF ontic. Apply [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] before treating a broad head as current governing ontology: name the primary `EntityOfConcern`, stable identity, `onticSlotRelation`, selected `semanticArea`, selected `ontologicalNeighborhood`, dependent patterns, description boundary, and publication boundary. If those rows are missing, use the current governing patterns that already carry the claim and do not cite the broad head as if it supplied current slot discipline.

### [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:5.5 - Mature Comparator Discharge

[[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] is mature only when its selected mature-pattern ingredients are present in the body, not only in a separate planning or evaluation note.

| Comparator | Selected mature ingredient | Current [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] locus | Lowering condition |
| --- | --- | --- | --- |
| [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] | stable identity plus small typed slot relation for a durable ontic | `E.24:4.2`, `E.24:4.3`, `E.24:5.1` | Lower if [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] asks for fields but no longer asks what preserves or changes identity. |
| [[E.20 - Mechanism Introduction Protocol|E.20]] | introduction discipline for one governed subject family | `E.24:4.1`, `E.24:4.4`, `E.24:8` | Lower if mechanism-specific doctrine is copied here instead of left with [[E.20 - Mechanism Introduction Protocol|E.20]], [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]], and related patterns. |
| [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] | publication-form and section-order boundary | `E.24:0`, `E.24:4.4`, `E.24:6`, `E.24:8` | Lower if [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] starts regulating pattern format instead of the ontic-introduction decision. |
...
| [[F.18 - Local‑First Unification Naming Protocol|F.18]] | durable naming after ontology is settled | `E.24:4.4`, `E.24:6`, `E.24:7` | Lower if a new name substitutes for identity, slot, and dependent-pattern settlement. |

### [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:5.6 - Bias-Annotation
...
### [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:5.7 - Rationale
...
This prevents a separate ontology for every participation name while preserving the real action-facing gain of the role patterns. "Engineer", "reviewer", "evidence reviewer", and "operator" do not become new system kinds merely because they appear in project language. They are recovered, when the case requires it, as role values and assignment relations under [[A.2 - Role Taxonomy|A.2]], [[A.2.1 - U.RoleAssignment- Contextual Role Assignment|A.2.1]], and [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]. Conversely, arbitrary relation participants such as a transformed television, an evidence target, an input, an output, a base, or a dependent are slot fillers or relation participants under their governing patterns, not `U.Role` values merely because ordinary language can say they "play a role."

Without [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]], FPF ontology development oscillates between two bad moves. One move invents a new umbrella name and leaves the mixed ontology intact. The other refuses the new name but still leaves several patterns carrying duplicated local slot doctrine. [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] gives a bounded authoring decision: use an existing governing pattern, introduce a durable ontic, keep a local use frame local, or keep the source label quote-only or reduced-use.
...
### [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:5.8 - SoTA-Echoing and Currentness

[[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] does not claim to replace ontology engineering, OWL-style formal ontology, or UFO-style foundational ontology. Its governing reason is the current FPF need for action-facing ontology compactness, plus a narrow SoTA echo:

| Source family | Current lesson for [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] | FPF decision |
| --- | --- | --- |
...
Use external sources when one ontic or subject matter itself depends on a source tradition. Put that source decision in the DRR and in the governing pattern for that subject matter. Do not make [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] carry a borrowed external theory of every durable ontic.

### [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:5.9 - Currentness and Lowering Logic

Treat [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] as current for ontic-introduction decisions only while the current FPF slot, precision-restoration, naming, and pattern-quality apparatus remain the governing source set. Lower [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]'s current authority for a case when one of these changes governs that case:
...
| `CC-E24-8` | The selected name passes [[F.18 - Local‑First Unification Naming Protocol|F.18]]; the name does not hide a second ontology or one umbrella for several kinds. |
| `CC-E24-8a` | Durable `U.*` names, reusable SlotKind heads, species or record-form names, public ids, Core-facing heads, and cross-context labels use [[F.18 - Local‑First Unification Naming Protocol|F.18]]; `F.17 UTS` and Name Card material is opened only when that name becomes public, Core-facing, or cross-context, and never replaces [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] SlotSpec discipline. |
| `CC-E24-9` | Pattern-quality and DRR-adequacy checks stay in [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] and [[E.9.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]]; they are not copied as user-facing ontic or subject-matter content. |
...
| `CC-E24-14` | Candidate detection, publication-form discipline, and contested evaluation are sent to [[E.24.CD - Ontic Candidate Detection|E.24.CD]], [[E.24.PUB - Ontic Description and Publication Discipline|E.24.PUB]], and [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] respectively; [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] itself stays centered on `U.Ontic` identity, slot relation, semantic area, ontological neighborhood, and dependent-pattern obligations. |


## [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:7 - Common Anti-Patterns

| Anti-pattern | Symptom | Repair |
| --- | --- | --- |
| Shadow-kind by repetition | The same local record appears in several patterns and starts being cited as an object. | Apply [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]; either write a durable ontic pattern or rename it as a local use frame. |
...
## [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:8 - Relations

- **Builds on:** [[E.8 - FPF Authoring Conventions & Style Guide|E.8]], [[E.9 - Design‑Rationale Record (DRR) Method|E.9]], [[E.9.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]], [[E.10 - Unified Lexical Rules for FPF|E.10]], [[E.10.ARCH - Wording-Use Ontological Precision Restoration Architecture|E.10.ARCH]], [[E.20 - Mechanism Introduction Protocol|E.20]], [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]], [[F.18 - Local‑First Unification Naming Protocol|F.18]], [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]], [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]], [[E.24.CD - Ontic Candidate Detection|E.24.CD]], and [[E.24.PUB - Ontic Description and Publication Discipline|E.24.PUB]].
- **Coordinates with:** governing patterns that describe durable ontics or their filled values, especially [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] for epistemes, [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] and [[E.20 - Mechanism Introduction Protocol|E.20]] for mechanisms, [[A.3.1 - U.Method- Context-Defined Way of Doing|A.3.1]] and [[A.3.2 - U.MethodDescription- Description Episteme for a Way of Doing|A.3.2]] for method and method description, [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] for role-method-work alignment, and precision-restoration patterns such as [[C.2.P - Epistemic Precision Restoration|C.2.P]], [[C.2.P.DR - Declarative Representation Precision Restoration|C.2.P.DR]], and [[C.30.STRAT - Stratification Wording Precision Restoration|C.30.STRAT]].
...
## [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:9 - Consequences
...
## [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:End
