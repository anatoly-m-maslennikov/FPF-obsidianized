...
## [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:1 - Problem frame
...
* **F** (Formality) describes *how the claim is expressed* and how strongly it supports verification workflows ([[C.2.3 - Unified Formality Characteristic F|C.2.3]]).
* **G** (Claim scope) describes *where the claim is asserted to apply* as a set-like object ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]).
* **R** (Reliability) describes *how strongly the claim is warranted* by linked evidence under that scope.
* **CL / CL^k / CL^plane** (Congruence Levels) describe *lossy transport* when claims are reused across contexts, kinds, or planes ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]).
...
* Scope growth is smuggled in through phrasing instead of explicit scope operators ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]).
* Cross-context reuse occurs without explicit bridges and without routing congruence loss into R.

This pattern makes **R** explicit in KD‑CAL and fixes the **triad discipline** required by Kind‑CAL ([[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]) and the Trust & Assurance calculus ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).

## [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:2 - Problem
...
## [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:3 - Forces
...
## [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:4 - Solution

### [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:4.1 - Canonical triad relation
...
* `F(c)` is Formality ([[C.2.3 - Unified Formality Characteristic F|C.2.3]]), treated as an **ordinal**.
* `G(c)` is Claim scope ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]), treated as a **set-like scope object**.
* `R_eff(c)` is Effective reliability for `c`, treated as a **ratio-scale** scalar in `[0,1]` (or an **ordinal proxy** at **[M‑0/M‑1]**; see §4.5.A).
  `R_eff` is computed **pathwise** (DEF‑C2.2‑3): when more than one admissible justification path exists, publish multiple path records (PathId rows) and cite which PathId(s) a guard/decision consumed (see §4.8.A / [[G.6 - Evidence Graph & Provenance Ledger|G.6]]). Any collapse to a single scalar is an explicitly declared Γ‑policy (no implicit averaging).
...
* When the claim is published on the Working‑Model surface, the author also declares `validationMode ∈ {postulate, inferential, axiomatic}` ([[E.14 - Human‑Centric Working‑Model|E.14]] / [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).

**Mode-to-lane hint (informative).** `validationMode` sets the *default expectation* for which assurance lane carries the initial support load ([[B.3.3 - Assurance Subtypes & Levels|B.3.3]] or [[B.3.5 - Working-Model Relations & Grounding (CT2R-LOG)|B.3.5]]).
...
**Profile note (informative; fold compatibility).** Some profiles treat empirical `R` as N/A for strictly **axiomatic** lines and use a tagged proxy `R_proxy := F` (`line=formal`) for folding, as an explicit proxy rather than an implicit “F⇒R” rule ([[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]).

`⟨F,G,R⟩` is an **assurance tuple**, not a `U.CharacteristicSpace`; do not draw “trajectories” in `⟨F,G,R⟩`.

### [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:4.2 - What Reliability R means in KD‑CAL
...
### [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:4.3 - Pathwise weakest-link propagation (series vs parallel)
...
This matches the KD‑CAL propagation rule ([[C.2 - Epistemic holon composition (KD-CAL)|C.2]]:4.3) and the Trust & Assurance skeleton ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]): weakest-link on the spine, penalize only by the worst (lowest) congruence encountered on the path (no averaging).
...
Independence is recorded as an explicit note (e.g., separate rigs/datasets/proof lines), per CC‑[[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]‑10 and the KD‑CAL composition rule ([[C.2 - Epistemic holon composition (KD-CAL)|C.2]]:4.3).
If the “multiple paths” actually cover **different** scope slices, do not use `max` to hide weaker slices; instead publish distinct `G_path` (SpanUnion‑style coverage) and keep per‑path `R_eff` traceable ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] / [[C.2 - Epistemic holon composition (KD-CAL)|C.2]]:4.3).
...
### [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:4.4 - Congruence penalties route to R only (no silent widening)
...
**No implicit translation.** Cross‑Context reuse is conformant only via an explicit Bridge (declared CL + loss note) and an explicit `translate(Bridge,·)`; see **CC‑[[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]‑4**.
...
`translate` is the USM operator ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]). It may drop unmappable slices and may include refit-like normalization; **this is not a penalty**. Any further narrowing is an explicit Δ‑move (ΔG−) under [[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]. Congruence loss (CL/CL^k/CL^plane) still routes to **R only**.
...
### [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:4.4.A - Worked micro-example: `translate(G)` + penalty ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]:12.2)
...
### [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:4.5 - Effective reliability under transport (policy-defined, monotone, bounded)
...
* `CL` be the congruence level of a scope bridge ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).
* `CL^k` be the congruence level of a kind bridge ([[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]).
* `CL^plane` be the congruence level of a plane transport bridge ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] / plane patterns).
...
This generalises the [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] skeleton to multiple congruence ladders (scope vs kind vs plane) without introducing new penalty characteristics. If a dimension is not present on the path, its penalty term is treated as neutral (`0` in the subtractive default).

**Provisional marking.**
Default admissibility thresholds for reuse are set by Bridge calibration profiles (e.g., [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]). Typically, `CL=1` requires an explicit waiver to proceed and `CL=0` is inadmissible; this pattern only specifies that such thresholds gate transport before any numeric penalty is meaningful.

### [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:4.5.A - Math-by-level gating ([[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:4.3)
...
### [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:4.6 - Evidence lanes are not new characteristics

KD‑CAL does not add new global coordinates beyond F–G–R. Instead, it requires that reliability be *explainable* via **assurance lanes** ([[B.3.3 - Assurance Subtypes & Levels|B.3.3]]):
...
### [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:4.7 - Scope operations are kind-safe (and use the ClaimScope algebra)
...
* (cross‑Context) there exists a declared **KindBridge** relating `K1` and `K2` with an explicit `CL^k` ([[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]).

This constraint prevents “type-by-scope” anti-patterns where scope manipulation is used to hide type mismatch.

### [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:4.8 - Minimal authoring recipe

A minimal, conforming KD‑CAL authoring flow for reliability is:

1. **Fix the typed claim.** State the claim as a typed proposition about a EntityOfConcern (Kind‑CAL, [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]).
2. **Declare claim scope.** Write `G` explicitly using [[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] operators; avoid scope-by-wording.
...
4. **Bind evidence.** Attach evidence stubs and lane tags (TA/VA/LA) and validity windows / decay policy where applicable ([[B.3.3 - Assurance Subtypes & Levels|B.3.3]], [[B.3.4 - Evidence Decay & Epistemic Debt|B.3.4]]).
...
7. **Declare bridges on reuse.** If you reuse across contexts/kinds/planes/notations, declare the bridge(s) (including NotationBridge where applicable) and their CLs.
   Cross‑Context reuse is conformant only when an explicit Bridge is declared; CL admissibility rules apply (waiver or forbid) before any numeric penalty is meaningful (see **CC‑[[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]‑4**).
...
### [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:4.8.A - Authoring template: Path summary row (copy/paste)

When publishing `R_eff` for a claim, authors SHOULD include a compact, claim-local **path summary**. This is intentionally shaped so it can be turned into tooling later (EvidenceGraph/PathId in [[G.6 - Evidence Graph & Provenance Ledger|G.6]]) without introducing new Core types or face-kinds.
...
## [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:5 - Archetypal Grounding

Informative; non-binding.

### [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:5.1 - System illustration
...
* `F(c1)=F5` because the controller and constraints are expressed as a machine-checkable model plus executable test harness ([[C.2.3 - Unified Formality Characteristic F|C.2.3]]).
* `G(c1)` is the declared operating envelope ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]) as a product set in `(μ, m, speed, tire)` space.
* Evidence:
...
If telemetry is reused from the track context to the road context, a scope bridge is declared with `CL=2`. Using the default monotone penalty table ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]), the LA contribution is reduced, and the derived `R_eff(c1)` drops accordingly. The claim’s envelope `G(c1)` does not change; only the warrant for transporting the evidence does.

### [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:5.2 - Episteme illustration
...
## [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:6 - Bias-Annotation
...
* **Prag bias:** Teams may Goodhart R by narrowing scope or selecting easy tests. This pattern mitigates by requiring explicit scope declaration and by making scope changes first-class ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]).
...
## [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:7 - Conformance Checklist

Normative.

| ID                                            | Requirement                                                                                                                                                                                                                 | Purpose                                                                       |
| --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| **CC‑[[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]‑1 (Triad publication).**           | Authors of a KD‑CAL location **SHALL** publish `⟨F,G,R_eff⟩` as a bundle for a specific claim, rather than publishing `R` alone.                                                                                            | Prevents decontextualised confidence scores.                                  |
| **CC‑[[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]‑2 (R-only penalty routing).**      | A conforming implementation of KD‑CAL transport **SHALL** satisfy **INV‑C2.2‑1**.                                                                                                                                           | Ensures bridges reduce warrant without silently mutating expression or scope. |
| **CC‑[[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]‑3 (Weakest-link fold).**           | A conforming implementation of KD‑CAL reliability propagation **SHALL** use **DEF‑C2.2‑3** as the default for required supports, unless an alternative Γ‑fold is explicitly declared and remains monotone and conservative. | Prevents confidence laundering through aggregation.                           |
| **CC‑[[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]‑4 (Bridge visibility for reuse).** | Authors **SHALL** declare explicit bridges with CL values for any cross-context, cross-kind, or cross-plane reuse that affects `R_eff`.                                                                                     | Makes transport loss auditable and machine-checkable.                         |
| **CC‑[[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]‑5 (Penalty policy visibility).**   | Authors or tooling **SHALL** reference the active policy identifiers used for `Φ`, `Ψ`, `Φ_plane` **and** the penalty aggregation rule `Π` (if not the default) when computing `R_eff`.                                   | Ensures repeatability and prevents hidden policy drift.                       |
| **CC‑[[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]‑6 (Type before scope).**           | Authors and validators **SHALL** enforce **WFC‑C2.2‑1** for scope composition operations.                                                                                                                                   | Prevents ill-typed scope algebra from creating incoherent reliability claims. |
| **CC‑[[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]‑7 (Evidence binding).**            | Authors **SHALL** bind any asserted `R_eff` to evidence references that enable TA/VA/LA inspection, consistent with the assurance lane discipline ([[B.3.3 - Assurance Subtypes & Levels|B.3.3]]) and evidence decay discipline ([[B.3.4 - Evidence Decay & Epistemic Debt|B.3.4]]).                            | Keeps R grounded and updateable.                                              |
| **CC‑[[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]‑8 (No ordinal arithmetic).**       | Validators **SHALL** reject any computation that treats `F` or `CL` as if they were ratio-scale numbers (e.g., averaging, subtraction), except where explicitly permitted as a policy-defined penalty function on `R`. Validators **SHALL** also reject arithmetic over `R_eff` when it is published as an **ordinal proxy** ([M‑0/M‑1]). | Enforces CSLC legality and prevents silent scalarisation.                     |
| **CC‑[[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]‑9 (Stance carriers declared).**    | Authors **SHALL** declare `U.BoundedContext K`, `S ∈ {design, run}`, and (where applicable) `ReferencePlane` and `validationMode`, and **SHALL NOT** merge design- and run-time assurance into one score.                 | Prevents DesignRunTag chimera and makes interpretation auditable.              |
| **CC‑[[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]‑10 (Parallel requires independence).** | Authors **SHALL** treat `max`-composition of support paths as admissible **only** when an explicit independence justification is recorded; otherwise supports are treated as one entangled line and remain weakest-link. | Prevents confidence inflation by double-counting correlated evidence.         |

## [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:8 - Common Anti-Patterns and How to Avoid Them
...
| **Scope laundering**       | The claim’s applicability grows by wording changes while `G` is unchanged.                    | It silently widens scope, making comparisons meaningless.        | Use [[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] operators and treat scope changes as explicit revisions.                                       |
...
## [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:9 - Consequences
...
## [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:10 - Rationale
...
## [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:12 - Relations

**Builds on:** [[C.2 - Epistemic holon composition (KD-CAL)|C.2]] (KD‑CAL overview), [[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] (Claim scope and operators), [[C.2.3 - Unified Formality Characteristic F|C.2.3]] (Formality F), [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (Trust & Assurance calculus), [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]] (Γ‑fold patterns), [[B.3.3 - Assurance Subtypes & Levels|B.3.3]] (assurance lanes), [[B.3.4 - Evidence Decay & Epistemic Debt|B.3.4]] (refresh/decay), [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]] (Kind‑CAL and kind bridges), [[F.9 - Alignment & Bridge across Contexts|F.9]] (Bridges & CL), [[G.6 - Evidence Graph & Provenance Ledger|G.6]] (EvidenceGraph PathId discipline), [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] (Bridge calibration / admissibility thresholds).
**Coordinates with:** [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]] (MM‑CHR evidence discipline), [[E.14 - Human‑Centric Working‑Model|E.14]] (working-model assertions), E.18/F.9/F.17/E.17/A.21 where crossing bundles and gate checks are live, [[C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]] (Q‑Bundle, for avoiding confusion between epistemic reliability and system reliability).
**Used by:** [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]] (cross-kind reuse discipline), guard macro bundles in [[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]] and [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]], and any acceptance/gating logic that consumes `R_eff` while preserving `F` and `G`.
**Clarifies:** The KD‑CAL meaning of reliability implicit in [[C.2 - Epistemic holon composition (KD-CAL)|C.2]]:4.1 and the transport clauses referenced across [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] and [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]].

## [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:End
