...
**Builds on.**
* **[[E.17.0 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]] — `U.MultiViewDescribing`.** Supplies the generic notion of `U.Viewpoint`, `U.View`, and `ViewFamily` over an `EntityOfConcernClass ⊑ U.Entity` (here: `EntityOfConcernClass = U.Holon`).
* **[[E.17.1 - U.ViewpointBundleLibrary - Reusable Viewpoint Bundles|E.17.1]] — `U.ViewpointBundleLibrary`.** Provides the generic `U.ViewpointBundle`/`ViewFamilyId` structure; TEVB is a concrete bundle (`VF.TEVB.ENG`) in the core library.
* **[[A.1 - U.Holon, U.System, and U.Episteme|A.1]] — Holon.** Holon kinds `U.System` and `U.Episteme` as the typical engineering entities of concern.
* **[[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]–[[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] — Episteme morphisms.** `U.EffectFreeEpistemicMorphing`, `U.EpistemicViewing`, `U.EpistemicRetargeting` as the generic morphism classes behind engineering views.
* **[[A.7 - Strict Distinction (Clarity Lattice)|A.7]] and [[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]] - Strict Distinction: EntityOfConcern, Description episteme, and Description episteme admitted for specification use.** TEVB uses DescriptionContext; engineering descriptions and specifications under TEVB are Description epistemes and specification-use cases with explicit `ViewpointRef`.
* **[[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] — `U.EpistemeSlotRelation`.** Provides `EntityOfConcernSlot`, `ViewpointSlot`, `ViewSlot` and the slot discipline ([[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]) used by TEVB-aligned Description epistemes and specification-use Description epistemes.

**Used by.**
* **[[E.18 - Transformation Flow Structure|E.18]]:5.12 — transformation-flow viewpoint-family map.** As a canonical consumer, [[E.18 - Transformation Flow Structure|E.18]] binds its engineering transformation-flow families (Functional, Procedural, Role-Enactor or Device-Structure, Module-Interface) to TEVB viewpoints `VP.Functional`, `VP.Procedural`, `VP.RoleEnactor`, `VP.ModuleInterface`.
* **[[E.17 - Multi‑View Publication Kit|E.17]] (MVPK).** Publication of engineering morphisms uses TEVB engineering viewpoints on the Description-episteme and specification-use side and separate publication-side viewpoints over publication faces and forms.
...
3. **EngineeringVPId vs publication-side viewpoint id.** `VP.*` in this pattern are **EngineeringVPId** values ([[E.18 - Transformation Flow Structure|E.18]]:5.12). MVPK publication uses separate publication-side viewpoint ids, linked to TEVB viewpoints only through correspondences.
4. **No new role coordinates in EntityOfConcern and Description-episteme boundary and specification-use discipline.** TEVB references stakeholder groups via `U.RoleEnactor` families but does not introduce `U.Role` as a coordinate in Description episteme or specification-use case signatures ([[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]]). Role semantics remain confined to RoleEnactment patterns ([[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]], F-R family).
5. **EntityOfConcern retention.** In ordinary TEVB use, `DescriptionContext.EntityOfConcernRef` remains the holon selected by `EntityOfConcernClassSpec`. Capability, Method, procedure/control, role-enactor structure, structural architecture, module, interface, and allocation terms are viewpoint concern/content inside the Description episteme unless the text explicitly opens [[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] retargeting with a KindBridge and species-extension rule.
6. **No extra viewpoints inside TEVB.** TEVB defines a **fixed core set** of four engineering viewpoints. Other labels such as “Assurance‑Oriented”, “Interop‑Oriented”, “Information/Data‑Oriented”, “Operational/Deployment”, “Mission/Context” may appear only as **lexical aliases** in [[E.18 - Transformation Flow Structure|E.18]]:5.12 (e.g. as `ViewFamilyId` / `AliasInViewFamilies` values for transformation-flow species). They do not extend `TEVB.EngBundle.viewpoints` and are not additional `U.Viewpoint` kinds in this bundle; when SoTA or local practice demands explicit assurance, information, or mission viewpoints, provide them as **separate `U.ViewpointBundle` species** imported alongside TEVB rather than by mutating `VF.TEVB.ENG`.
...
## [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:1 - Problem frame  *(informative)*
...
In FPF, [[E.17.0 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]] and [[E.17.1 - U.ViewpointBundleLibrary - Reusable Viewpoint Bundles|E.17.1]] give the **generic machinery**:
...
[[E.18 - Transformation Flow Structure|E.18]] already uses a **canonical engineering viewpoint-family map** with names like “Functional”, “Procedural”, “Role-Enactor (Device-Structure)”, “Module-Interface”. Without a formal bundle tying these together, those names drift and the mapping between [[E.18 - Transformation Flow Structure|E.18]], MVPK, EntityOfConcern, Description-episteme boundary, and specification use becomes fragile.
...
## [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:2 - Problem  *(informative)*
...
3. **Viewpoints conflated with publication faces/forms and files.** “Functional view” is used both for the underlying viewpoint and for a concrete document or dashboard; MVPK faces/forms, [[E.18 - Transformation Flow Structure|E.18]] transformation-flow families, and EntityOfConcern and Description-episteme and specification-use distinctions become entangled.
4. **Role leakage into EntityOfConcern and Description-episteme boundary and specification-use discipline.** Engineering views that are about role-enactors are written directly in terms of `U.Role`, blurring the boundary between RoleEnactment ([[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]) and description and specification-use lanes, and breaking [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] and [[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]].
...
TEVB makes engineering viewpoint families **first‑class reusable bundles** and pins them to an explicit `EntityOfConcernClass` (engineering holons) so that [[E.18 - Transformation Flow Structure|E.18]], MVPK and discipline-packs can align on the same vocabulary.

## [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:3 - Forces  *(informative)*
...
## [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:4 - Solution — TEVB as a core `U.ViewpointBundle` for holons  *(normative)*

### [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:4.1 - TEVB bundle identity
...
The selection `{VP.Functional, VP.Procedural, VP.RoleEnactor, VP.ModuleInterface}` is the current **NQD-frontier** for engineering holon viewpoints in Part G: it realises a Function-Behaviour-Structure-plus-Role (`F-B-S+R`) cut that is non-dominated against candidate families including explicit information or data, assurance or safety, and mission or context viewpoints under the N, U, C, and D characteristics ([[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]). Part G records the SoTA candidate set and rejected alternatives; TEVB only fixes the **core four** where each `VP.* : U.Viewpoint` is defined below. These four are the **only** viewpoints in the core TEVB bundle.

  > **Note.** Other ViewFamilyId values used in the [[E.18 - Transformation Flow Structure|E.18]] transformation-flow viewpoint-family map (e.g., *Assurance‑Oriented*, *Interoperability‑Oriented*, *Information/Data‑Oriented*, *Operational/Deployment*, *Mission/Context*) remain **lexical families only** for transformation-flow species ([[E.18 - Transformation Flow Structure|E.18]]:5.12). They do not add viewpoints to TEVB; they are orthogonal to TEVB’s `viewpoints` set.

### [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:4.2 - TEVB engineering viewpoints
...
* `AllowedEpistemeKinds : FinSet(U.EpistemeKindRef)` — Description-episteme and specification-use kinds admissible under this viewpoint (all obeying EntityOfConcern and Description-episteme boundary, specification use, and [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] slot disciplines);
...
#### [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:4.2.1 - `VP.Functional` - functional behavior and capability viewpoint
...
* **ConformanceRules (examples).**
  ...
  * Functional ports/signatures are separated from module interfaces unless an [[A.6.M - Module Relation Repair|A.6.M]] module-interface claim is current.
  * When functional views participate in retargeting patterns, they satisfy the relevant [[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] retargeting constraints; concrete consumer patterns such as [[E.18 - Transformation Flow Structure|E.18]] may impose additional rules.
...
#### [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:4.2.2 - `VP.Procedural` — process & control viewpoint
...
* **AllowedEpistemeKinds (shape).**
  ...
  * `MethodDescription`, `MethodSpec` for operational procedures ([[A.3.1 - U.Method- Context-Defined Way of Doing|A.3.1]]–[[A.3.2 - U.MethodDescription- Description Episteme for a Way of Doing|A.3.2]]).
  ...
  * `WorkflowDescription`, `WorkflowSpec` (business processes, orchestration logic).
  ...
  These epistemes:
  * respect the **order discipline** (Γ_method, Γ_ctx) and [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] (Role–Method–Work alignment);
  * carry E.10.D2-conformant DescriptionContext with `ViewpointRef = VP.Procedural`.

* **ConformanceRules (examples).**
  * Pre/post‑conditions at step boundaries are explicit and type‑checked ([[A.3 - Transformer Constitution (Quartet)|A.3]].1/A.3.2, Γ_method).
  * No embedding of Work or calendars inside procedural descriptions ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]] and [[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]]).
  * Failure modes and recovery actions are declared and traceable to safety analyses ([[F.15 - SCR-RSCR Harness for Unification|F.15]] harnesses where relevant).
...
#### [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:4.2.3 - `VP.RoleEnactor` - role, bearer, and device-structure viewpoint
...
* **AllowedEpistemeKinds (shape).**
  ...
  * `RoleEnactmentDescription` for mappings `Holder#Role:Context` under [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]].
  * `TransformerBearerDescription` for `U.System bearing TransformerRole@Context` around an [[A.3.4 - U.Transformation- Bounded Change Under Conditions|A.3.4]] transformation.
...
* **ConformanceRules (examples).**
  * Role vs method vs work vs capability separation is upheld through [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] and [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]].
...
#### [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:4.2.4 - `VP.ModuleInterface` - module and interface viewpoint
...
* **ConformanceRules (examples).**
  * Interfaces are typed and explicitly bound to standards or signature declarations where applicable ([[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]], [[A.6.M - Module Relation Repair|A.6.M]], [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]).
  * Functional ports are not treated as module interfaces unless the module-interface or substitutability claim is current.
  * No inlining of methods, work, or functional behavior into module structure; use [[A.3 - Transformer Constitution (Quartet)|A.3]].4/A.6.F/A.15 for those claims.
...
## [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:5 - Archetypal grounding  *(informative)*
...
Each `VP.*` viewpoint is a `U.Viewpoint` as in [[E.17.0 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]], with:
...
Each of these is a **family of Description epistemes and specification-use cases** with `DescriptionContext = ⟨EntityOfConcernRef(Plant_X or ControlStack_X), BoundedContextRef, ViewpointRef=VP.*⟩` and TEVB ensures that [[E.18 - Transformation Flow Structure|E.18]] and MVPK can rely on this common structure.

## [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:6 - Conformance checklist  *(normative)*
...
Each `VP.*` viewpoint MUST be a well‑formed `U.Viewpoint` per [[E.17.0 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]]:
...
* `BoundedContextRef` pointing to the engineering context ([[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]).

Capability, Method, procedure/control, role-structure, structural-architecture, module, interface, and allocation terms in those descriptions are viewpoint concern/content unless the text explicitly declares an [[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] retargeting, KindBridge, and species-extension rule that changes `EntityOfConcernRef`.
...
TEVB-aligned descriptions and specification-use cases MAY reference `U.RoleEnactor` families in `StakeholderFamilies` but SHALL NOT add `Role` or `RoleEnactor` as characteristics in Description episteme or specification-use case signatures beyond what [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] and [[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]] already provides. Role semantics stay in RoleEnactment patterns; TEVB just selects concerns.
...
## [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:7 - Rationale & SoTA echoing  *(informative)*

### [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:7.1 - NQD‑grounded choice of the core four
...
Other candidates (e.g. dedicated information, assurance, or mission viewpoints) remain important but either duplicate concerns already captured by TEVB (when specialised to engineering holons) or are better modelled as orthogonal quality bundles ([[C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]]) or non-engineering viewpoint bundles (business and governance viewpoint bundles). TEVB therefore pins only the core four and leaves the rest to specialised families.

### [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:7.2 - Alignment with post‑2015 engineering practice
...
## [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:8 - Relations  *(informative)*

* **Builds on.** [[E.17.0 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]] (`U.MultiViewDescribing`), [[E.17.1 - U.ViewpointBundleLibrary - Reusable Viewpoint Bundles|E.17.1]] (`U.ViewpointBundleLibrary`), [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] and [[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]] (EntityOfConcern and Description-episteme boundary plus specification use), [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] (EpistemeSlotRelation), A.6.2-A.6.4 (episteme morphisms).
* **Constrains.** [[E.18 - Transformation Flow Structure|E.18]]:5.12 (transformation-flow viewpoint-family map), engineering description and specification-use patterns, MVPK engineering publication guidance.
* **Coordinates with.** MVPK publication face, publication form, publication unit, and publication carrier discipline; F-R family (Role, RoleDescription, RoleSpec); [[F.18 - Local‑First Unification Naming Protocol|F.18]] (naming discipline for ViewFamilyId, ViewpointId, EngineeringVPId, and publication-side viewpoint ids).
...
## [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]:End
