...
**Depends on.** [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **Lexical Discipline for “Context” (D.CTX)**; [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] **Strict Distinction**; [[A.11 - Ontological Parsimony (C‑5)|A.11]] **Ontological Parsimony**; [[A.8 - Universal Core Principle (C‑1)|A.8]] **Universal Core**.
**Coordinates with.** [[F.1 - Domain‑Family Landscape Survey|F.1]] **Contexts (Contexts)**; [[F.2 - Term Harvesting & Normalisation|F.2]] **Harvest**; [[F.3 - Intra‑Context Sense Clustering|F.3]] **SenseCells**; [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] **Role Description**; [[F.5 - Naming Discipline for U.Types & Roles|F.5]] **Naming Discipline**; [[F.7 - Concept‑Set Table|F.7]] **Concept‑Set Table**; [[F.9 - Alignment & Bridge across Contexts|F.9]] **Alignment & Bridge**.
**Aliases (informative).** *Mint‑vs‑Reuse gate*; *Naming governor*.

## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:1 - Intent & applicability
...
The lattice enforces **locality of meaning** (Contexts), **senseFamily separation** ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]]), and **parsimony** ([[A.11 - Ontological Parsimony (C‑5)|A.11]]) while remaining didactically simple.
...
## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:2 - Problem frame
...
3. **Hidden bridges.** Cross‑context sameness is implied by look‑alike words rather than declared ([[F.9 - Alignment & Bridge across Contexts|F.9]]).
4. **Kernel sprawl.** New **U.Types** appear to plaster over local vocabulary gaps.

## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:3 - Forces
...
## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:4 - Minimal vocabulary (used in this pattern only)

* **Context** — `U.BoundedContext` (per D.CTX).
* **SenseCell** — address of a **local sense** produced by [[F.3 - Intra‑Context Sense Clustering|F.3]] (one context × one clustered sense).
* **Concept‑Set row** — a **licensed Cross‑context reading** ([[F.7 - Concept‑Set Table|F.7]]) of cells in one senseFamily with a declared **Row Scope** and **Row CL(min)**.
* **senseFamily** — as defined in **[[F.0.1 - Contextual Lexicon Principles|F.0.1]]**; here used as the **typed discriminator for rows** restricted to {Role | Status | Measurement | Type‑structure | Method | Execution}.
* **Role Description** — a **Role or Status** template anchored to a **single SenseCell** ([[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]).
* **Alias** — an **additional label** for an existing FPF label (within [[F.5 - Naming Discipline for U.Types & Roles|F.5]]), no new semantics.
...
## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:5 - The decision lattice (conceptual, notation‑free)
...
### [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:5.1 - Q0 — What is the **senseFamily** of your need?

* If **uncertain**, return to F.1/F.3: stabilise the Context(s) and the local sense.
* If **mixed**, split the need: one decision **per senseFamily** ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]]).

### [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:5.2 - Q1 — Is there a **single Context** whose SenseCell already expresses it?

* **Yes →** **Reuse** the **SenseCell**’s label **inside that Context**.

  * If you need assignable behaviour or deontics on that sense: **define a Role Description** **anchored to that SenseCell** ([[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]).
* **No →** go to Q2.
...
### [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:5.3 - Q2 — Do you need to **read across Contexts** (same senseFamily)?

* **No →** stay within one context; if your desire is merely a nicer label, consider an **Alias** (Q3).
* **Yes →** check [[F.7 - Concept‑Set Table|F.7]] for a **Concept‑Set row** covering your cells **in this senseFamily** with adequate **Row Scope** and **Row CL(min)**.

  * **Found & sufficient →** **Reuse the row’s FPF label** at that scope.
  * **Not found or insufficient →** either (a) **publish a contrast** (teach difference), or (b) propose a **new row** but only after [[F.9 - Alignment & Bridge across Contexts|F.9]] Bridges exist at **τ(scope)**.

> *Example (manager).* You want one label for the **actor** in workflow and provenance prose. [[F.7 - Concept‑Set Table|F.7]] has a **Naming‑only** row mapping *BPMN Participant* ↔ *PROV Agent* at CL = 2. **Reuse** “actor” **at Naming‑only** scope; do **not** infer identity in models.

### [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:5.4 - Q3 — Is this **only a wording preference** for an existing FPF label?

* **Yes →** add an **Alias** in [[F.5 - Naming Discipline for U.Types & Roles|F.5]] (Tech register and/or Plain register), no semantics changed.
* **No →** go to Q4.
...
### [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:5.5 - Q4 — Does your need recur across Contexts in a way **not captured** by current rows, **with Bridges** already available at the required CL?

* **Yes →** propose a **new Concept‑Set row** ([[F.7 - Concept‑Set Table|F.7]]): small (2–4 Contexts), **one senseFamily**, declare **Row Scope** and **Row CL(min)**, include a **counter‑example** if any Bridge has loss notes.
* **No →** go to Q5.

> *Example (engineer).* You repeatedly compare **runtime occurrence** in PROV with **PLC task runs**. [[F.9 - Alignment & Bridge across Contexts|F.9]] Bridges exist at CL = 2. Propose **row “execution-occurrence”** at **assignment/enactment-eligibility** scope (not Type-structure).

### [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:5.6 - Q5 — Are you describing a **kernel‑level notion** missing from the catalogue, **not** reducible to existing rows or Role Descriptions, and **present across ≥ 3 domain families** ([[A.8 - Universal Core Principle (C‑1)|A.8]])?
...
## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:6 - Scope thresholds (default τ) — **how much sameness** you’re allowed to claim
...
| **Assignment-eligibility** | Safe to reference the row’s target as the **thing a `U.RoleAssignment` may point to** (e.g., a run, a value). | **2** | [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] Role Description, acceptance narratives. |
...
## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:7 - Micro‑examples (didactic triad)

### [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:7.1 - For engineers — “Do we need a new **Execution** label?”
...
* **Row?** [[F.7 - Concept‑Set Table|F.7]] has **execution-occurrence** at **assignment/enactment-eligibility**, CL = 2.
...
### [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:7.2 - For managers — “Can we call them all **actors**?”
...
### [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:7.3 - For researchers — “New **U.Type** for ‘Work Scope’?”

* **Need.** Kernel notion capturing **feasible performance region** across systems.
* **Test [[A.8 - Universal Core Principle (C‑1)|A.8]].** Appears in **control** (reachable sets), **services** (operating envelope), **measurement** (confidence bands): **≥ 3 families?**
...
## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:8 - Invariants (normative, lightweight)
...
4. **Role Description anchoring.** Role Descriptions are **single-Context**, **single-cell** anchors ([[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]).
5. **Alias modesty.** Aliases **never** change semantics and live under [[F.5 - Naming Discipline for U.Types & Roles|F.5]].
6. **Kernel restraint.** New **U.Types** are **rare**; [[A.8 - Universal Core Principle (C‑1)|A.8]] **(≥ 3 families)** is mandatory, and duplication with existing U.Types must be ruled out.

## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:8.1 - Mint/Reuse discipline for **policy-ids** (normative addendum)
...
**MintDecisionRef.** A **resolvable reference** to the decision record that introduced (minted) a policy-id into a declared namespace/registry. For **normative** policy-ids this is typically a **DRR id** ([[E.9 - Design‑Rationale Record (DRR) Method|E.9]]) or an equivalent change decision record. For **purely local, non-exported** policy-ids it MAY be a Gate `DecisionLog` entry ([[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]) if that local-only scope is explicit.
...
## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:9 - Quick reference (one‑glance map)
...
| A nicer label for the same FPF concept      | **Alias in [[F.5 - Naming Discipline for U.Types & Roles|F.5]]**               | Style only; zero semantics.                      |
| A brand‑new primitive concept               | **New U.Type (rare)**          | Only if cross‑family, irreducible, kernel‑level. |

## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:10 - Anti‑patterns & remedies

| #         | Anti‑pattern               | Symptom                                                                            | Why it harms thinking                              | Remedy (conceptual move)                                                                                                                         |
| --------- | -------------------------- | ---------------------------------------------------------------------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| **AP‑1**  | **Row‑less sameness**      | Declaring “these mean the same” across Contexts without citing a **Concept‑Set row**. | Imports meaning implicitly; no CL guard.           | If Cross‑context reuse is desired, **reuse an existing row** at a declared **scope** ([[F.7 - Concept‑Set Table|F.7]]), else **publish the contrast** and apply [[F.9 - Alignment & Bridge across Contexts|F.9]] Bridges only when the bridge relation is live. |
...
| **AP‑3**  | **Alias with payload**     | Introducing an Alias that subtly changes intent or senseFamily.                          | Hides semantics behind wording; confuses senseFamilies.   | Aliases ([[F.5 - Naming Discipline for U.Types & Roles|F.5]]) are **style only**. If semantics change, choose **row reuse** or **Role Description** instead.                                         |
| **AP-4**  | **Role-Description-to-row anchoring** | Role Description points to a **row** rather than a **single SenseCell**.       | Masks locality; **assignments** become cross-context by stealth. | Role Descriptions **must anchor to one SenseCell** ([[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]). Use rows only in prose or aggregated views.                                                |
| **AP‑5**  | **Kernel inflation**       | Proposing a new **U.Type** because a convenient label is missing.                  | Duplicates the kernel; violates parsimony.         | Apply [[A.8 - Universal Core Principle (C‑1)|A.8]]: require **≥ 3 domain families** and **irreducibility**; otherwise **Alias** or **row**.                                               |
| **AP‑6**  | **senseFamily mixing**           | One name that conflates Role, Status, Measurement, or Type‑structure.              | Collapses [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] Strict Distinction.                  | **Split by senseFamily** first (Q0). Decide **per senseFamily**.                                                                                             |
| **AP‑7**  | **Bridge‑by‑string**       | Treating identical lexical forms as equivalent senses across Contexts.                | Homonym trap; ignores local sense.                 | Equivalence only via **[[F.9 - Alignment & Bridge across Contexts|F.9]] Bridge** + **row**; never by string.                                                                                  |
| **AP‑8**  | **Row without loss notes** | Publishing a row where Bridges indicate mismatches, but row text is silent.        | Readers assume full equivalence.                   | Include **counter‑example** and **loss sketch** in the row’s narrative ([[F.7 - Concept‑Set Table|F.7]]).                                                                    |
...
## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:11 - Reasoning primitives (judgement schemas, notation‑free)
...
## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:12 - Extended worked examples

### [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:12.1 - **Execution, observation, and acceptance** (engineers)
...
### [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:12.2 - **Actor across workflow and provenance** (managers)
...
### [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:12.3 - **Accuracy across metrology and data quality** (researchers)
...
### [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:12.4 **Subtype relation across OWL and a curated taxonomy** (formalists)
...
**Reasoning.** [[F.7 - Concept‑Set Table|F.7]] row “subtype‑order” exists at **Type‑structure scope** with CL = 3 (only after verifying acyclicity & anti‑symmetry in `Taxonomy_X`). If the curated taxonomy contains cycles, **downgrade** to Naming‑only or reject the row.

**Outcome.** When CL≥3, you may **reuse row** for structural proofs; else teach differences.

## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:13 - Relations (with other patterns)

* **Builds on:** [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] (D.CTX) **Context ≡ U.BoundedContext**; [[F.1 - Domain‑Family Landscape Survey|F.1]] Contexts; [[F.2 - Term Harvesting & Normalisation|F.2]] Harvest; [[F.3 - Intra‑Context Sense Clustering|F.3]] SenseCells.
* **Constrains:**

  * **[[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] Role Description:** **one SenseCell per Role Description**; no row anchoring.
  * **[[F.5 - Naming Discipline for U.Types & Roles|F.5]] Naming:** Aliases are style‑only; no semantics movement.
  * **[[F.7 - Concept‑Set Table|F.7]] Concept‑Set:** rows must declare **Scope** & **Row CL(min)** and carry **loss notes**.
  * **[[F.9 - Alignment & Bridge across Contexts|F.9]] Bridges:** any row proposal presupposes Bridges at or above τ(scope).
* **Used by.** All patterns (Part C) whenever new labels are contemplated.

## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:14 - Migration notes (conceptual)
...
4. **Alias proliferation.** Collapse redundant Aliases under a single [[F.5 - Naming Discipline for U.Types & Roles|F.5]] entry; keep both registers (Tech/Plain).
...
## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:15 - Acceptance tests (SCR/RSCR — concept‑level)

### [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:15.1 - Static conformance (SCR)
...
* **SCR‑F8‑S04 (Alias modesty).** Aliases introduced in [[F.5 - Naming Discipline for U.Types & Roles|F.5]] do **not** claim new semantics or change senseFamily.
* **SCR‑F8‑S05 (Kernel restraint).** Any new U.Type proposal includes **≥ 3 domain families** of evidence and an **irreducibility** note.

### [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:15.2 - Regression (RSCR)
...
## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:16 - Didactic distillation (90‑second teaching script)
...
## [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:End
