...
**Depends on.** [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **Lexical Discipline for “Context” (D.CTX)**; **[[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]] EntityOfConcern, Description Episteme, and Specification-Use Discipline**; [[F.1 - Domain‑Family Landscape Survey|F.1]] **Domain‑Family Landscape Survey**; [[F.2 - Term Harvesting & Normalisation|F.2]] **Term Harvesting**; [[F.3 - Intra‑Context Sense Clustering|F.3]] **Intra‑Context Sense Clustering**; [[A.2.1 - U.RoleAssignment- Contextual Role Assignment|A.2.1]] **`U.RoleAssignment`**; [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] **Strict Distinction**; [[A.11 - Ontological Parsimony (C‑5)|A.11]] **Ontological Parsimony**.
**Coordinates with.** [[F.5 - Naming Discipline for U.Types & Roles|F.5]] **Naming Discipline for U.Types & Roles**; [[F.7 - Concept‑Set Table|F.7]] **Concept‑Set Table**; [[F.9 - Alignment & Bridge across Contexts|F.9]] **Alignment & Bridge Across Contexts**; [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] **Trust & Assurance Calculus** (for later status evaluation).
**Aliases (informative).** *Mask/Badge card*; *role card* (plain only).

## [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:1 - Intent & applicability
...
Each template is **grounded in a SenseCell** `⟨Context, Local‑Sense⟩` from [[F.3 - Intra‑Context Sense Clustering|F.3]] and declares **minimal invariants** that later **assignments** must satisfy. No Cross‑context meaning is imported here.
...
## [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:2 - Problem frame
...
## [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:3 - Forces

| Force                         | Tension to resolve                                                                                                           |
| ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| **Behaviour vs knowledge**    | A role changes how the holder **acts**; a status changes how the holder is **treated/assessed**. Keep **EntityOfConcern, Description episteme, and specification-use positions** distinct ([[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]]; [[A.7 - Strict Distinction (Clarity Lattice)|A.7]]). |
| **Locality vs reuse**         | We want reusable templates, yet meanings are **context‑local** ([[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]], [[F.1 - Domain‑Family Landscape Survey|F.1]]).                                                   |
...
## [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:4 - Minimal vocabulary (this pattern only)

* **Context** — **U.BoundedContext** (per [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]).
* **Local‑Sense** — a consolidated sense in a Context ([[F.3 - Intra‑Context Sense Clustering|F.3]]).
...
## [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:5 - Core idea (didactic)
...
## [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:6 - The Role Description Card (one‑screen sketch)
...
* **Label pair:** **Tech** (idiomatic) - **Plain** (didactic)  *(naming discipline in [[F.5 - Naming Discipline for U.Types & Roles|F.5]])*
...
* **Time stance:** **design** / **run** aligned to the Context ([[F.1 - Domain‑Family Landscape Survey|F.1]]).
...
* **RSG gate:** mark which **states are enactable under autonomy** (cf. [[A.2.5 - U.RoleStateGraph- The Named State Space of a Role|A.2.5]]); link to `AutonomyBudgetDeclRef`.
...
## [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:7 - Normative invariants (template discipline)
...
3. **Time honesty.** The card’s stance (**DesignRunTag**) **MUST** match the Context’s stance ([[F.1 - Domain‑Family Landscape Survey|F.1]]).
4. **Minimality.** Invariants **SHOULD** be the **fewest that decide** the assignment; avoid procedural sequences.
5. **No Cross‑context smuggling.** A single card **MUST NOT** import foreign semantics; if two Contexts are needed, the relation is handled later in **[[F.9 - Alignment & Bridge across Contexts|F.9]]**.
6. **Label fidelity.** **Tech** label **MUST** be idiomatic to the Context; **Plain** label **MUST** not widen the sense ([[F.3 - Intra‑Context Sense Clustering|F.3]]).
...
## [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:8 - Reasoning primitives (judgement schemas, notation‑free)
...
7. **Bridge embargo**
   ...
   *Reading:* No Cross‑context equivalence is asserted here; use **[[F.9 - Alignment & Bridge across Contexts|F.9]]** later.

## [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:9 - Worked examples (Context‑true)

> Illustrative cards only; names are **tech/plain labels**, not final U.Type IDs ([[F.5 - Naming Discipline for U.Types & Roles|F.5]] will govern naming).

### [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:9.1 - **Role Template:** *participant (workflow actor)* — Context: **BPMN 2.0 (2011)**
...
### [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:9.2 - **Status Template:** *access‑role membership* — Context: **NIST RBAC (2004)**
...
### [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:9.3 - **Status Template:** *incident (service disruption)* — Context: **ITIL 4 (2020)**
...
### [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:9.4 - **Role Template:** *task runner (control runtime)* — Context: **IEC 61131‑3**
...
* **Typical interactions:** **Execution** ([[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] family), **Actuation** (Sys‑CAL).
* **Common misreads:** ≠ **workflow task**; ≠ **algorithm** (design).

## [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:10 - Anti‑patterns & remedies
...
| **A2**  | **Pan‑Context template**   | One card cites several canons implicitly (“BPMN/PROV process”).           | Imports meaning across Contexts; hides losses.              | Keep **one SenseCell per card**. If Cross‑context relation is needed, apply **[[F.9 - Alignment & Bridge across Contexts|F.9]] Bridge**.                     |
| **A3**  | **Silent time flip**    | Card defined in a **design** Context asserts run‑time facts (or vice versa). | Violates [[F.1 - Domain‑Family Landscape Survey|F.1]] time stance; produces category errors.      | Align **Time stance** to the Context; relocate run‑facts to status/evidence lines or to another Context.              |
...
| **A7**  | **Global label**        | Tech label chosen for cross‑discipline appeal (“Actor”) not Context idiom.   | Loses local meaning; harms [[F.3 - Intra‑Context Sense Clustering|F.3]] clustering.               | Use **Context‑idiomatic Tech label**; provide a Plain label for teaching ([[F.5 - Naming Discipline for U.Types & Roles|F.5]] governs labels).                     |
...
| **A11** | **Tooling creep**       | Mentions manifests, pipelines, editors.                                   | Violates [[E.5 - Four Guard‑Rails of FPF|E.5]] guard‑rails; notational dependency.         | Remove all process/tool talk; keep card **concept‑only**.                                                       |
| **A12** | **Bridge‑by‑label**     | Using identical labels to imply Cross‑context sameness.                      | Stealth equivalence; no loss policy.                     | Labels do not bridge. Any Cross‑context claim goes to **[[F.9 - Alignment & Bridge across Contexts|F.9]]** with a declared CL policy.                           |

## [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:11 - Concept‑level operators (refinement & compatibility)
...
### [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:11.1 - Same‑Context equivalence
...
### [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:11.2 - Refinement (strictness order)
...
### [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:11.3 - Incompatibility (mutual exclusion)
...
### [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:11.4 - Co‑wearability / co‑bearability
...
### [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:11.5 - Time‑stance alignment

**Form.**
`stance(T)=design ⊢ inv(T) may not assert run‑facts`
`stance(T)=run ⊢ inv(T) may not assert design‑commitments`

**Reading.** Invariants must **respect** the Context’s DesignRunTag ([[F.1 - Domain‑Family Landscape Survey|F.1]]).

### [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:11.6 - Binding/Assertion admissibility
...
## [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:11.7 - Cross‑context embargo (inside [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]])

**Form.**
`sense(T₁)=⟨C,−⟩, sense(T₂)=⟨C′,−⟩, C≠C′ ⊢ no‑relation(T₁,T₂) here`

**Reading.** **[[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]** never asserts Cross‑context relations. If a relation is desired, it becomes a **Bridge** in **[[F.9 - Alignment & Bridge across Contexts|F.9]]**.

## [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:12 - Relations (where this card sits)

**Builds on:**
[[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **D.CTX** (Context ≡ U.BoundedContext); [[F.1 - Domain‑Family Landscape Survey|F.1]] (Contexts cut); [[F.2 - Term Harvesting & Normalisation|F.2]] (harvested terms); [[F.3 - Intra‑Context Sense Clustering|F.3]] (Local‑Sense → **SenseCell**); [[A.2.1 - U.RoleAssignment- Contextual Role Assignment|A.2.1]] **`U.RoleAssignment`**; [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] **Strict Distinction**.

**Constrains:**
**[[F.5 - Naming Discipline for U.Types & Roles|F.5]]** (Naming): pairs **Tech/Plain** must reflect the **Context idiom** and avoid Cross‑context overreach.
...
**[[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]** (Mint or Reuse?): prefer **refinement (⪯)** over new cards; split cards rather than mixing senseFamilies.
...
**Is used by.**
[[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] family (Role–Method–Work alignment) to interpret **System‑in‑Role** and **Work**; Part B evidence/status checks to interpret **evaluation invariants**.

## [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:13 - Migration notes (conceptual playbook)
...
4. **Rename safely (labels only).** If [[F.5 - Naming Discipline for U.Types & Roles|F.5]] revises labels, change **Tech/Plain** wording; **SenseCell** and **invariants** remain untouched.
...
6. **Bridge discovery.** Do **not** inject Cross‑context text into cards; record the relation as an **[[F.9 - Alignment & Bridge across Contexts|F.9]] Bridge** (with CL policy), leaving the cards as they are.

## [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:14 - Acceptance tests (SCR/RSCR — concept‑level)

### [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:14.1 - Static conformance (SCR)
...
### [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:14.2 - Regression (RSCR)
...
* **RSCR-F4-E04 (Bridge discipline).** After adding an **[[F.9 - Alignment & Bridge across Contexts|F.9]] Bridge**, Role Description cards remain **unchanged**; cross-context meanings do not seep back into cards.
* **RSCR‑F4‑E05 (Label updates).** Label changes per **[[F.5 - Naming Discipline for U.Types & Roles|F.5]]** preserve SenseCell and invariants; tests treat them as **renames**, not semantic edits.

## [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:15 - Didactic distillation (60‑second close)
...
## [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]:End
