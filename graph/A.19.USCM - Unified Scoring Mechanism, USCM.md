...
> **Modified:** 2026‑01‑20
>
> **Governing-pattern note, Phase‑3 canonicalization:** this pattern governs the canonical `U.Mechanism.Intension` for `USCM.IntensionRef` (CHR suite stage `score`). Mechanism-intension semantics of characterisation mechanisms live in explicitly designated governing patterns ([[E.20 - Mechanism Introduction Protocol|E.20]]).
> [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] governs the **template** of `U.Mechanism.Intension`; this pattern governs the **USCM-specific** slots, operations, laws, admissibility, applicability, transport, plane, and audit obligations for that template.
>
> **Canonicalization hook, ID‑continuity‑safe:** any other appearances of the USCM intension (e.g., a legacy grounding stub in [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] or suite prose in [[A.19.CHR - CHRMechanismSuite|A.19.CHR]]) SHALL be reduced to a **Tell + Cite** stub pointing to **`A.19.USCM:4.1`**, while preserving the original section headings and their public `PatternId:SectionPath` IDs for continuity (alias‑dock legacy tokens rather than deleting them).
...
## [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:0 - At a glance — didactic, informative
...
* **P2W seam:** concrete edition/policy pin bindings (including `ScoringMethodDescriptionRef@edition(…)` when USCM is used) are chosen in planned baseline plan items ([[A.15.3 - SlotFillingsPlanItem|A.15.3]] + `A.19.CHR:4.7.2`); executions only record effective refs/pins in `Audit`.
...
## [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:1 - Problem frame
...
## [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:2 - Problem
...
## [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:3 - Forces
...
## [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:4 - Solution
...
Method semantics (“how to score”) remain out of suite core: they belong in SoTA packs ([[G.2 - SoTA Harvester & Synthesis|G.2]]) and wiring‑only extension modules (`GPatternExtension` blocks), while USCM remains the stable conceptual mechanism boundary.

### [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:4.1 - Mechanism.Intension
...
* **SignatureManifest (optional; importability):** if a USCM publication is intended to be imported/reused, it SHOULD publish a `SignatureManifest` ([[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]] / [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]; `CC‑[[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]]‑18`, `CC‑UM.1`) consistent with `IntensionHeader`/`Imports`, explicitly exposing the stable SlotKind surface (including `ScoringMethodDescriptionSlot`) and any declared scalarization commitment.
...
* **LawSet** (minimum; legality‑first, no hidden scalarization):
  ...
  1. **SCP+CSLC legality:** any numeric transform used to produce `ScoreProfileSlot` MUST be admissible under `CGSpecSlot.SCP` and CSLC‑lawful (cites [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]] + [[A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC‑KERNEL)|A.18]]).
  2. **ScoringMethod is explicit (no hidden defaults):** `Score` MUST cite `ScoringMethodDescriptionSlot` (edition‑pinned via P2W when reproducibility matters; see `A.19.CHR:4.7.2`). If a score is issued, the scoring method **𝒢** (Coordinate→Score) MUST be disclosed as required by [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]] (bounded codomain; monotonicity consistent with template polarity). USCM MUST NOT rely on an implicit “default scoring method”.
...
### [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:4.2 - Interpretation notes — informative
...
## [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:5 - Archetypal Grounding — informative

### [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:5.1 - Tell
...
### [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:5.2 - Show — U.System
...
### [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:5.3 - Show — U.Episteme
...
## [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:6 - Bias-Annotation — informative
...
## [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:7 - Conformance Checklist
...
4. **ScoringMethod is explicit and auditable.** `Score` cites `ScoringMethodDescriptionSlot` (edition‑pinned when reproducibility matters). No implicit “default scoring method” is assumed. The disclosed method respects polarity/monotonicity discipline (cf. [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]]).
...
## [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:8 - Common Anti‑Patterns and How to Avoid Them
...
## [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:9 - Consequences
...
## [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:10 - Rationale
...
## [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:11 - SoTA-Echoing
...
**Pack note, Phase‑3:** this pattern does not currently cite a USCM-specific [[G.2 - SoTA Harvester & Synthesis|G.2]] SoTA pack or ClaimSheet. If such a pack is introduced, `ScoringMethodDescriptionSlot` SHOULD be wired to `ScoringMethodDescriptionRef(ed=...)` entries defined in that pack’s ClaimSheets, keeping the USCM mechanism semantics unchanged.

### [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:11.1 - SoTA alignment map
...
## [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:12 - Relations

* **Builds on**

  * [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] / `CC‑UM.*` (mechanism intension shape and authoring checks).
  * `A.19.CHR:4.2.1` (CHR SlotKind lexicon).
  * [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]] (CG‑Spec, specifically `SCP` and `MinimalEvidence`).
  * [[A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC‑KERNEL)|A.18]] (CSLC legality discipline).
  * [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]] (ScoringMethod disclosure; polarity/monotonicity discipline for score mappings).
  * [[A.15.3 - SlotFillingsPlanItem|A.15.3]] + `A.19.CHR:4.7.2` (P2W planned baseline seam for edition/policy pin bindings; cited as seam, not duplicated in Intension).
  * [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]] (CN‑Spec, specifically `comparability` routing and normalization‑based comparability expectations).
* **Used by**

  * [[A.19.CHR - CHRMechanismSuite|A.19.CHR]] (suite membership and suite protocols; USCM is the `score` stage).
  * Downstream CHR stages that require score measures as inputs (e.g., `CPM`, `SelectorMechanism`).
  * [[E.18 - Transformation Flow Structure|E.18]] when USCM instances are used as nodes in a selected `TransformationFlowStructure`; the selected `ScoringMethodDescriptionRef@edition(…)` and other pins live in planned baselines (P2W), while executions surface effective refs/pins via `Audit`.
* **Coordinates with**
  ...
  * [[G.2 - SoTA Harvester & Synthesis|G.2]] and `GPatternExtension` wiring modules for post‑2015 method families, without mutating the USCM kernel.
...
## [[A.19.USCM - Unified Scoring Mechanism, USCM|A.19.USCM]]:End
