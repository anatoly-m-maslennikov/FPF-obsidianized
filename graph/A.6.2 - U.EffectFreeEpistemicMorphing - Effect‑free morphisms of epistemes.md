...
**One‑line summary.** `U.EffectFreeEpistemicMorphing` (EFEM) is the universal class of **effect-free, law-constrained morphisms between epistemes**. An EFEM morphism rewrites episteme components (ClaimGraph, `entityOfConcernRef`, optional `groundingHolonRef`, `viewpointRef`, `referenceScheme`, and—where [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]+ is in use—`representationSchemeRef` and related slots, plus meta) in a **conservative, functorial, reproducible** way, with an explicit mode for what happens to the **EntityOfConcernSlot** (`EntityOfConcernChangeMode ∈ {preserve, retarget}`) as defined by `C.2.1 U.EpistemeSlotRelation`.

**Placement.** After **[[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] `U.Mechanism`** and before any specialisations (`A.6.3 U.EpistemicViewing`, `A.6.4 U.EpistemicRetargeting`).

**Builds on.**
[[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]] `U.Signature` (subject/vocabulary/laws/applicability); [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] `U.Mechanism`; [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] `U.RelationSlotDiscipline`; [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] `U.Episteme — Epistemes and their slot relation`; [[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]] (EntityOfConcern and Description-episteme boundary and specification use/refinement gates); [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]].* (Kind‑CAL / KindBridge for EntityOfConcern classes).

**Used by.**
[[A.6.3 - U.EpistemicViewing - EntityOfConcern-preserving morphism|A.6.3]] `U.EpistemicViewing`; [[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] `U.EpistemicRetargeting`; [[E.17.0 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]] `U.MultiViewDescribing`; [[E.17 - Multi‑View Publication Kit|E.17]] (MVPK); [[E.18 - Transformation Flow Structure|E.18]] (structural reinterpretation over transformation-flow structure).

**EntityOfConcern change-mode discipline.** EFEM uses `EntityOfConcernChangeMode` for the preserve/retarget characteristic over [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]'s EntityOfConcernSlot / entityOfConcernRef family. Earlier source-side spellings must be normalized to the EntityOfConcern family before conformant use and do not define a second EntityOfConcern ontology.

## [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:1 - Problem frame
...
All of these are **episteme→episteme** transforms: they change what is written in an episteme, but they **do not themselves measure, execute, or actuate**. They are neither Work ([[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]) nor Mechanisms in the [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] sense; they are “pure morphisms over epistemes”.

Without a universal pattern for such morphisms:

* every family (KD‑CAL, [[E.18 - Transformation Flow Structure|E.18]], MVPK, discipline packs) reinvent their own notion of “projection”, “reinterpretation”, or “refinement”;
...
* cross‑family reasoning (e.g. “this [[E.18 - Transformation Flow Structure|E.18]] structural reinterpretation is a retargeting, not a view”) becomes brittle and ad‑hoc.

## [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:2 - Problem

Concretely, without EFEM:

1. **No single place for “effect‑free” discipline.**
   The distinction *“episteme‑only change”* vs *“Work in the world”* is already important ([[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] separates episteme components from Work and from publication forms, renderings, and carriers), but the laws for “episteme‑only” operations are scattered or implicit.
...
3. **No functorial backbone.**
   MVPK, KD‑CAL, and [[E.18 - Transformation Flow Structure|E.18]] all implicitly assume that episteme transforms **compose** and respect identities, but the conditions for this (purity, conservativity, idempotence, scope) are not formulated once and reused. Different parts of the spec repeat subtly different sets of laws.
...
## [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:3 - Forces
...
* `U.EpistemicViewing` ([[A.6.3 - U.EpistemicViewing - EntityOfConcern-preserving morphism|A.6.3]]) — EFEM with `EntityOfConcernChangeMode = preserve`.
* `U.EpistemicRetargeting` ([[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]]) — EFEM with `EntityOfConcernChangeMode = retarget`, tied to KindBridges/ReferencePlanes.

### [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:4.2 - Signature Block ([[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]] alignment)

As a `U.Signature`, EFEM publishes the following **SubjectBlock** and the standard four‑row block (“SubjectBlock / Vocabulary / Laws / Applicability”) from [[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]], specialised to episteme→episteme morphisms.
...
* **Types**
  * `U.Episteme` (as holon; realised via species `U.EpistemeCard`, `U.EpistemeView`, `U.EpistemePublication` under [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]).
  * `U.EpistemeKind` (episteme n‑ary relation signature; slots per [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] / [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]).
  * `U.SubjectRef` (source-migration wiring name only; for Description epistemes, including Description epistemes admitted for specification use, it decodes to `DescriptionContext = ⟨EntityOfConcernRef, BoundedContextRef, ViewpointRef⟩` per [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] §6.1 / [[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]]). It does not define another EntityOfConcern family.
...
* **Operators (arrow algebra)**
  ...
  * `entityOfConcernChangeMode(f) : U.EntityOfConcernChangeMode`  // EFEM‑level characteristic from [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]].

Each operator that takes epistemes as arguments obeys **SlotSpec discipline** from [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]: in particular, laws below are phrased in terms of the **named SlotKinds** (`EntityOfConcernSlot`, `GroundingHolonSlot`, `ClaimGraphSlot`, `ViewpointSlot`, `ReferenceSchemeSlot`, `ViewSlot`, and—when the [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]+ extension is used—`RepresentationSchemeSlot`) and their associated ValueKind/RefKind; we never speak of “field 1/2/3”.

**Laws row** and **Applicability** are given by P0–P5 and the Scope clause below.

### [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:4.3 - Laws P0–P5 (normative)
...
#### [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:4.3.1 - P0 — Typed signature & component profile ([[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]‑grounded)

For any EFEM morphism `f : X→Y`:

1. **Typed epistemes.** `X` and `Y` are epistemes of declared kinds `K_X, K_Y : U.EpistemeKind`, each with a SlotKind signature as per [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] and [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] (at least `EntityOfConcernSlot`, `ClaimGraphSlot`, `ViewpointSlot?`, `RepresentationSchemeSlot?`, `ReferenceSchemeSlot?`; `GroundingHolonSlot?`, `ViewSlot?` where relevant).
...
2. **Component projection.** For each episteme `E`, EFEM laws may refer to:
   ...
   * `representationSchemeRef?(E) : U.RepresentationSchemeRef` — only for episteme kinds that use the [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]+ `RepresentationSchemeSlot`;
...
3. **Declared `EntityOfConcernChangeMode`.**
   ...
   * if `entityOfConcernChangeMode(f) = retarget`, then `entityOfConcernRef(Y) ≠ entityOfConcernRef(X)` in general and the record names a **KindBridge** between the two EntityOfConcern values ([[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] / [[F.9 - Alignment & Bridge across Contexts|F.9]]).

4. **SubjectRef source-migration discipline.**
   For Description epistemes, including Description epistemes admitted for specification use (`…Description` / `…Spec`), `subjectRef(E)` is a `DescriptionContext = ⟨EntityOfConcernRef, BoundedContextRef, ViewpointRef⟩` ([[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]]). EFEM species state how source-migration `subjectRef` transforms in terms of these components (usually: preserve or explicitly adjust `ViewpointRef` while preserving `EntityOfConcernRef` and `BoundedContextRef`).

#### [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:4.4.2 - P1 — Purity (no external effects)

EFEM morphisms are **pure functions on epistemes**:
* Applying `f : X→Y` **does not**:
  * change any `U.System` or `U.Holon` state;
  * execute Work (`U.WorkEnactment`) or run a `U.Mechanism` ([[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]) with operational guards;
...
#### [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:4.3.3 - P2 — Conservativity (no new EntityOfConcern commitments)
...
Where `entityOfConcernChangeMode(f) = retarget`, conservativity is understood **relative to a declared invariant** of the KindBridge ([[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]]): e.g., conservation of energy for a Fourier transform, or preservation of functional behaviour for a structural reinterpretation.

#### [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:4.3.4 - P3 — Functoriality (identity, composition, correspondence)
...
3. **Correspondence‑aware composition.**
   When EFEM changes `RepresentationScheme` or `ReferenceScheme`, a **CorrespondenceModel** (as in [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] §6 and [[E.17 - Multi‑View Publication Kit|E.17]]) may be needed to witness commutativity: composition respects these correspondences up to declared isomorphism/oplax naturality (witness epistemes may be recorded in `meta`).

#### [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:4.3.5 - P4 — Idempotence & determinism (on fixed configuration)
...
#### [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:4.3.6 - P5 — Applicability, scope & compatibility
...
* **Viewpoint discipline.**
  For Description epistemes, including Description epistemes admitted for specification use, EFEM specifies which `U.Viewpoint`s ([[E.17.0 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]]) are admissible and how it interacts with `U.MultiViewDescribing` families (e.g., “works only on engineering viewpoints from TEVB” or “viewpoint‑agnostic normalisation”).
...
Cross‑Context or cross‑plane use (changing `U.BoundedContext` or `ReferencePlane`) is **not part of EFEM**; it is handled by Bridges (Part F) and [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] transport, which then feed new epistemes into EFEM.

## [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:5 - Archetypal Grounding (Tell–Show–Show)
...
### [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:5.1 - Typed specification-use refinement `Specify_DescEp_SpecDesc` (species of EFEM)
...
This matches [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] and [[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]]: EntityOfConcern-to-Description (`Describe_EoC_DescEp`) is the strict-boundary describing step and is not itself an episteme→episteme morphism; `Specify_DescEp_SpecDesc` is an optional EFEM species over a Description episteme after a specification use/refinement gate is present. EFEM supplies the episteme→episteme laws for that refinement; it does not make Specification a third peer in [[A.7 - Strict Distinction (Clarity Lattice)|A.7]].

### [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:5.2 - Internal normalisation of a View (species of EFEM, `entityOfConcernChangeMode = preserve`)
...
### [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:5.3 - Retargeting sketch (bridge‑backed, `entityOfConcernChangeMode = retarget`)

*Context.* [[E.18 - Transformation Flow Structure|E.18]] structural reinterpretation maps a physical layout view into a functional behaviour view, changing the EntityOfConcern from “physical module assembly” to “functional graph” along a KindBridge.
...
The details belong to [[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] and [[E.18 - Transformation Flow Structure|E.18]]; EFEM provides the generic discipline.

### [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:5.4 - Worked SlotSpec example (engineering SystemDescription episteme kind)
*(informative)*
...
This table is an instance of [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] `U.RelationSlotDiscipline`: each row is a SlotSpec triple ⟨SlotKind, ValueKind, refMode/RefKind⟩; no additional kernel types are introduced, and [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]’s constraints on `EntityOfConcernSlot`/`GroundingHolonSlot` are preserved.
...
* `Specify_DescEp_SpecDesc_Sys : SystemDescription → SystemSpec` — a `EntityOfConcernChangeMode = preserve` species that:
  ...
  * satisfies P2 by only adding claims that are logical consequences of the original description plus the fixed `EntityOfConcern` ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]] and [[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]]);
  * satisfies CC‑[[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]‑5 by explicitly declaring its slot profile and change mode.
...
In later [[A.6 - Signature Stack & Boundary Discipline|A.6]].3/A.6.4/E.17.\* patterns, concrete EpistemeKinds (for specific engineering description and specification-useification idioms) are expected to provide SlotSpecs of this general shape and to state explicitly, per CC‑[[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]‑5 / CC‑EFEM.\*, which slots their EFEM species read and write.

## [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:6 - Bias & Defaults (informative)
...
* **SlotKinds, not “fields”.** Laws talk about `EntityOfConcernSlot`, `GroundingHolonSlot`, etc., and their ValueKind/RefKind, as per [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] and [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]; they never use unnamed tuple positions or “unnamed slot positions”. This keeps EFEM aligned with the slot discipline used for methods, roles, services, and other n‑ary relations.

* **Local‑first semantics.** EFEM is **Context‑local**; crossings of Context or ReferencePlane are always delegated to Bridges / [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] transport (with CL penalties to `R/R_eff` only). No “implicit cross‑Context EFEM” is permitted.

* **EntityOfConcern and Description-episteme boundary and specification-use/refinement respect.** EFEM never collapses EntityOfConcern with Description epistemes or with specification-use refinements: EntityOfConcern-to-Description and optional specification-use refinement operations are typed explicitly. The former remains the [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] describing boundary; the latter is an EFEM species only when it is an episteme→episteme refinement admitted by an exact specification use/refinement gate.

## [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:7 - Conformance Checklist (normative)

| ID                                                  | Requirement                                                                                                                                                                                                                                                                                                                                                                                           |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **CC‑EFEM.1 (Typed episteme objects).**             | Every morphism advertised as `U.EffectFreeEpistemicMorphing` SHALL have domain and codomain epistemes whose kinds (`U.EpistemeKind`) publish SlotKinds/ValueKinds/RefKinds according to [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] and [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] (at least `EntityOfConcernSlot` and `ClaimGraphSlot`; other slots as declared).                                                                                                               |
| **CC‑EFEM.2 (Declared EntityOfConcernChangeMode).** | Each EFEM **species** SHALL declare the `EntityOfConcernChangeMode` characteristic `entityOfConcernChangeMode : U.Morphism → {preserve, retarget}` as per [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]. For every instance `f`, `entityOfConcernChangeMode(f)` MUST be either `preserve` (⇒ `entityOfConcernRef` unchanged) or `retarget` (⇒ a KindBridge and invariant are explicitly named; see [[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] / [[F.9 - Alignment & Bridge across Contexts|F.9]]).                                                                                         |
...
| **CC‑EFEM.7 (EntityOfConcern and Description-episteme boundary, specification use/refinement, and `subjectRef` discipline).**      | For any episteme that is a `…Description`/`…Spec` ([[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]]), EFEM laws SHALL be phrased in terms of `DescriptionContext = ⟨EntityOfConcernRef, BoundedContextRef, ViewpointRef⟩` and MUST respect the EntityOfConcern and Description-episteme boundary, specification use/refinement gates, and DescriptionContext invariants (including DESCCTX-13 Viewpoint-locality as defined in [[E.10 - Unified Lexical Rules for FPF|E.10]].D2/C.2.1): `Describe_EoC_DescEp` lives in [[A.7 - Strict Distinction (Clarity Lattice)|A.7]]; `Specify_DescEp_SpecDesc` MAY be species of EFEM only as an episteme→episteme refinement and MUST preserve the `EntityOfConcern`. |
| **CC‑EFEM.8 (Slot‑level read/write declaration).**  | Any EFEM species that defines morphisms between epistemes SHALL also satisfy [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] checkpoint CC‑[[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]‑5: it MUST state whether it is a species of `U.EffectFreeEpistemicMorphing`/`U.EpistemicViewing`/`U.EpistemicRetargeting`, declare its `entityOfConcernChangeMode`, name which SlotKinds it reads and writes, and state its behaviour on `entityOfConcernRef`, `groundingHolonRef`, `viewpointRef`, and `referenceScheme`. |

## [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:8 - SoTA‑Echoing (informative, lineage)
...
## [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:9 - Consequences

* **Single place for episteme‑to‑episteme laws.**
  All effect-free transforms of knowledge epistemes, across KD‑CAL, MVPK, [[E.18 - Transformation Flow Structure|E.18]], discipline packs, can now be defined as species of EFEM, instead of each family re‑inventing its own law set.
...
* **Stable backbone for Viewing & Retargeting.**
  [[A.6.3 - U.EpistemicViewing - EntityOfConcern-preserving morphism|A.6.3]] and [[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] do not need to repeat P0–P5; they specialise EFEM with additional constraints (preserve/retarget). Other patterns (e.g. MultiViewDescribing, MVPK, [[E.18 - Transformation Flow Structure|E.18]] structural reinterpretation) can depend on EFEM as a stable base.

* **Slot‑level clarity.**
  By formulating EFEM laws in terms of SlotKinds/ValueKinds/RefKinds ([[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]) and the EpistemeSlotRelation ([[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]), it becomes much harder for Episteme to confuse “EntityOfConcern”, “slot in a relation”, and “reference to that entity”.

* **Better didactics.**
  The old “semantic triangle” becomes a didactic projection of EFEM over the EpistemeSlotRelation: EFEM + [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] explain precisely what the triangle was trying to gesture at (symbol, concept, referent), while correctly foregrounding operations, viewpoints, grounding holons, and reference schemes.

## [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:10 - Rationale

**Why a separate EFEM pattern ([[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]) instead of folding into [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] or [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]?**

* [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] governs **Mechanisms** (operations with AdmissibilityConditions, Γ_time, transport and Bridges)—too operational for the pure episteme transforms we want here.
* [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] fixes the **ontology of epistemes** (slots, components, ReferencePlane), but does not talk about morphisms. EFEM is explicitly a **morphism‑level** pattern over that ontology.

This split mirrors how Signature ([[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]]) separates “what is declared” from “how it is realised”: [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] says what an episteme is; [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]] says what an admissible episteme-to-episteme transform is.
...
FPF already committed to a single SlotKind/ValueKind/RefKind discipline ([[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]) across relations, methods, roles, and now epistemes. Re‑using that discipline here:
...
## [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:11 - Relations

* **Specialises / is specialised by.**

  * Builds on [[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]] `U.Signature` and [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] `U.Mechanism` for the uniform SubjectBlock/vocabulary/laws/applicability structure.
  * Specialised by [[A.6.3 - U.EpistemicViewing - EntityOfConcern-preserving morphism|A.6.3]] `U.EpistemicViewing` (entityOfConcern‑preserving EFEM) and [[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] `U.EpistemicRetargeting` (entityOfConcern-retargeting EFEM).

* **Constrained by.**
  [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] `U.RelationSlotDiscipline` (SlotKind/ValueKind/RefKind); [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] `U.EpistemeSlotRelation` (episteme components, ReferencePlane); [[E.10.D2 - EntityOfConcern, Description Episteme, and Specification-Use Discipline|E.10.D2]] (EntityOfConcern and Description-episteme boundary and specification use/refinement gates); Part F (Bridges, CL, ReferencePlane crossings); [[E.10 - Unified Lexical Rules for FPF|E.10]] (LEX‑BUNDLE naming rules, especially on `…Slot` / `…Ref` and ban on Subject/Object in episteme tech names).

* **Consumed by.**
  [[E.17.0 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]] `U.MultiViewDescribing` (families of Description epistemes, including Description epistemes admitted for specification use, under Viewpoints); [[E.17 - Multi‑View Publication Kit|E.17]] (MVPK — publication as species of Viewing/EFEM); [[E.18 - Transformation Flow Structure|E.18]] (structural reinterpretation and other transformation-flow relations over epistemes); KD‑CAL/LOG‑CAL rules that reason about episteme transforms categorically.

## [[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]:End
