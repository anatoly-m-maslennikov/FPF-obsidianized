...
## [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:1 - Problem frame

A library is not a theory.

`Γ_epist` ([[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]) can reliably aggregate and audit evidence, but aggregation alone does not create a supervising core. A MET names the point where a `Transformer` re‑identifies a portfolio as *one* higher‑order episteme with an explicit boundary, objective, and supervisory principles.
...
## [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:2 - Problem
...
## [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:3 - Forces
...
## [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:4 - Solution

A Meta‑Epistemic Transition is modeled as a **Meta‑Holon Transition ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]])** specialized to knowledge epistemes or publications (typically starting from a `Γ_epist` portfolio and ending in a new `U.Episteme` holon).

### [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:4.1 - Definition (normative)
...
* A MET is an act of **creation**, not passive drift. Therefore the `emergesAs` relation **MUST** be attributed to an explicit external `Transformer` ([[A.12 - External Transformer & Reflexive Split|A.12]]) that performed the synthesis.
* A MET declaration **MUST** be supported by a **Promotion Record** ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]:5.1) containing explicit evidence for the B‑O‑S‑C triggers (B.2.1), interpreted for epistemes as below. The record still carries the parent schema fields (`eventType`, `identityStance`, and the explicit `preConfig/postHolon` deltas); do not “compress” MET into a narrative paragraph.
* If the synthesis introduces new primitives/terms (i.e., it reframes the vocabulary rather than only summarising), the Promotion Record **SHOULD** treat the event as a `ContextReframe` (or, where the local taxonomy permits paired types, `Fusion + ContextReframe`) and **MUST** satisfy `MHT‑CTX‑MAP`: include the context mapping summary (`triggers.X?`) and record the new `boundedContext` plus its CL baseline in `postHolon.boundedContext` ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]:5.1, [[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]:5.2).
* Post‑MET trust/assurance for the new meta‑episteme **MUST** be evaluated as a claim about a *new holon*, not silently inherited from the constituents: satisfy `MHT‑ASS‑REBAS` and apply congruence penalties when composing evidence across constituents (see [[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]:5.2 and [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).

### [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:4.2 - The B-O-S-C triggers for epistemes
...
In practice, many METs also involve **X (context rebase)** when vocabulary or definitions change. When that happens, the Promotion Record **MUST** carry `triggers.X?` and satisfy `MHT‑CTX‑MAP` ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]:5.2).

### [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:4.3 - Didactic note for managers (informative)
...
### [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:4.4 - Common anti-patterns and how to avoid them (informative)
...
| **“Forced marriage” of ideas**         | Conflicting epistemes are merged into an incoherent hybrid.                                                               | A MET is not a mechanical merge. The `Transformer` must supply a supervisory principle that reconciles or contextualises the constituents, and the trust model ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]) penalises incoherent integration via congruence penalties. |
...
## [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:5 - Archetypal Grounding

### [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:5.1 - System vignette (Tell–Show–Show)
...
### [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:5.2 - Episteme vignette (cross-domain table)
...
## [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:6 - Bias-Annotation
...
## [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:7 - Conformance Checklist
...
* **CC-B2.3.2 (Trigger mandate):** The `Transformer` **MUST** provide a **Promotion Record** ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]) containing evidence for all four epistemic B‑O‑S‑C triggers.
...
* **CC-B2.3.6 (Context reframe mapping):** If the MET introduces new primitives/terms or changes definitions, the Promotion Record **MUST** satisfy `MHT‑CTX‑MAP` ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]:5.2): list concept/unit/terminology mappings with CL levels and record the new `boundedContext` and its CL baseline.

## [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:8 - Consequences
...
## [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:9 - Rationale
...
## [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:10 - SoTA-Echoing
...
## [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:11 - Relations

* **Is a specialization of:** `B.2 Meta-Holon Transition (MHT)`.
* **Builds on:** `B.2.1 BOSC Triggers` and the [[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]] Promotion Record.
* **Is complemented by:** `B.2.2 MST (Sys)` (system emergence) and `B.2.4 MFT` (capability emergence).
* **Is performed by:** An external `Transformer` ([[A.12 - External Transformer & Reflexive Split|A.12]]) executing an abductive synthesis (see [[B.5.2 - Abductive Loop|B.5.2]] for abductive moves).
* **Produces:** A new `U.Episteme` whose trust/assurance is governed by `B.3 Trust & Assurance Calculus`.

## [[B.2.3 - Meta-Epistemic Transition (MET)|B.2.3]]:End
