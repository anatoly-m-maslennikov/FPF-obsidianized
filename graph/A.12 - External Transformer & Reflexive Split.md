...
## [[A.12 - External Transformer & Reflexive Split|A.12]]:1 - Intent & Context
...
*   **[[A.3 - Transformer Constitution (Quartet)|A.3]] (Transformer Constitution):** Which defines the core quartet of action: the `Agent` (who acts), the `MethodDescription` (the recipe), the `Method` (the capability), and the `Work` (the event).
*   **[[A.2 - Role Taxonomy|A.2]] (Contextual Role Assignment):** Which provides the universal syntax `Holder#Role:Context` for defining agents.
...
## [[A.12 - External Transformer & Reflexive Split|A.12]]:2 - Problem
...
2.  **Audit Dead-Ends:** An auditor tracing a change finds that the system is its own justification. There is no external evidence, no log from an independent actor, and therefore, no way to verify the integrity of the transformation. This is a direct violation of **Evidence Graph Referring ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]])**.
...
## [[A.12 - External Transformer & Reflexive Split|A.12]]:3 - Forces
...
## [[A.12 - External Transformer & Reflexive Split|A.12]]:4. Solution
...
## [[A.12 - External Transformer & Reflexive Split|A.12]]:4.1 - The Principle of the External Transformer
...
### [[A.12 - External Transformer & Reflexive Split|A.12]]:4.1.1 - Reflexivity vs cross‑reference (normative note)
...
## [[A.12 - External Transformer & Reflexive Split|A.12]]:4.2 - The Reflexive Split Pattern
...
## [[A.12 - External Transformer & Reflexive Split|A.12]]:5 - Archetypal Grounding
...
## [[A.12 - External Transformer & Reflexive Split|A.12]]:6 - Conformance Checklist
...
| **CC-A12.4 (Episteme Carrier as Target)** | When a `U.Episteme` is modified, the `Target` of the transformation **MUST** be its **symbol carrier** (`U.System`), not the `U.Episteme` itself. | Reinforces **Strict Distinction ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]])**. Knowledge doesn't change by magic; a physical agent must act on its physical representation. |
| **CC-A12.5 (No Self-Evidence)** | The Agent that performs a transformation **cannot** be the sole source of evidence for the success or properties of that transformation. Evidence **MUST** be anchored via an independent `Observer`. | Prevents conflicts of interest in assurance. The `Transformer` does the work; a separate `Observer` (another RoleAssignment) validates it. This aligns with **[[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] (Evidence Graph Referring)**. |

## [[A.12 - External Transformer & Reflexive Split|A.12]]:7 - Consequences
...
| **Unlocks Deeper Analysis:** Once an internal control loop is made explicit, it can be formally analyzed for stability, performance, and failure modes using tools like the Supervisor-Subsystem Feedback Loop pattern ([[B.2.5 - Supervisor-Subholon Feedback Loop|B.2.5]]). | - |

## [[A.12 - External Transformer & Reflexive Split|A.12]]:8 - Rationale
...
*   **Physics (Constructor Theory):** As discussed in [[A.3 - Transformer Constitution (Quartet)|A.3]], Constructor Theory recasts physics in terms of what transformations are possible. A transformation is always performed by a "constructor" (our `Transformer`) on a substrate. The theory does not contain "self-constructing" substrates. FPF's externalist stance is fully aligned with this physical worldview.
...
## [[A.12 - External Transformer & Reflexive Split|A.12]]:9 - Relations
...
## [[A.12 - External Transformer & Reflexive Split|A.12]]:End
