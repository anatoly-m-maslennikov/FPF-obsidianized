...
**Primary hooks.** [[G.Core - Part G Core Invariants|G.Core]] (RSCR trigger catalogue + alias docking + Default Governing Definition Index), [[G.6 - Evidence Graph & Provenance Ledger|G.6]] (EvidenceGraph; `PathId`/`PathSliceId`), [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] (Bridge Sentinels; CL/Φ/plane policy pins), [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] (set-returning selection/dispatch), [[G.8 - SoS‑LOG Bundles & Maturity Ladders|G.8]] (SoS-LOGBundle telemetry hooks), [[G.9 - Parity and Benchmark Harness|G.9]] (parity reruns), [[G.10 - SoTA Pack Shipping|G.10]] (shipping hooks and pack-level telemetry pins), [[G.12 - DHC Dashboards (Discipline‑Health time‑series; admissible telemetry; generation‑first)|G.12]] (dashboard telemetry pins), [[B.3.4 - Evidence Decay & Epistemic Debt|B.3.4]] (freshness/decay), [[E.18 - Transformation Flow Structure|E.18]] (GateCrossing/CrossingBundle visibility), optional `C.18 and C.19` (QD/E–E policy pins), [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]] (SoS-LOG branches / maturity ladders), [[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] (causal-use support records whose SoTA-sensitive fields can change downstream causal-use results).

**Non-duplication note (Phase-2).**
This pattern **does not** (i) define the meaning of RSCR trigger kinds, (ii) introduce “shadow specs” for CN/CG legality, (iii) redefine tri-state guards / penalties / set-return semantics, (iv) re-govern shipping or harvesting, or (v) mint new `RSCRTriggerKindId` / default governing definitions (design-time changes live in [[G.Core - Part G Core Invariants|G.Core]] and are recorded via DRR, [[E.9 - Design‑Rationale Record (DRR) Method|E.9]]).
All such universal norms are **cited via [[G.Core - Part G Core Invariants|G.Core]]** and enforced through **delegation** in this pattern’s conformance checklist.

## [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:1 - Problem frame — Keeping shipped SoTA current without global rebuilds
...
[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] is the **Part G governing definition** of the **refresh orchestration kit**: it turns typed refresh causes into **scoped plans** and **auditable execution reports**, while delegating all cause semantics and universal invariants to [[G.Core - Part G Core Invariants|G.Core]].

## [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:2 - Problem — Why naive refresh breaks comparability and legality
...
## [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:3 - Forces — Minimal recomputation under strict invariants
...
* **Modularity vs. orchestration power.** [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] must coordinate harvesting/parity/shipping without re-implementing them or importing discipline-specific method semantics into core.
...
## [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:4 - Solution — RSCR-driven refresh as a P2W-scoped orchestration kit

### [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:4.1 - [[G.Core - Part G Core Invariants|G.Core]] linkage (normative)

**GCoreLinkageManifest (normative; canonical shape per [[G.Core - Part G Core Invariants|G.Core]]; Nil‑elision permitted).**
...
By the [[G.Core - Part G Core Invariants|G.Core]] **Expansion rule**, the **effective** conformance ids / trigger‑kinds / pin‑obligations for [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] are the manifest expansions (profiles/sets/pin‑sets) plus the explicit deltas above.

**LegacyTriggerAliasIds (visible; labels only).** `{[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:T0…T7}` (docked via `TriggerAliasMapRef`; aliases are never semantic authorities).

### [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:4.2 - Refresh orchestration kit (pattern-governed; conceptual artefacts)

[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] defines a minimal kit of *authoring-plane* artefacts that make refresh explicit and auditable.
...
> *Note (normative by delegation).* ID continuity and alias discipline are governed by [[G.Core - Part G Core Invariants|G.Core]] (do not restate as local rules here).

### [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:4.3 - Orchestration semantics (conceptual; delegating to governing definitions)

[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] turns typed causes into scoped actions without governing the semantics of those actions.

**4.3.1 Ingestion.**
Consume RSCR triggers from:

* telemetry hooks (e.g., [[G.8 - SoS‑LOG Bundles & Maturity Ladders|G.8]], [[G.10 - SoTA Pack Shipping|G.10]], [[G.12 - DHC Dashboards (Discipline‑Health time‑series; admissible telemetry; generation‑first)|G.12]]),
* freshness/decay events ([[B.3.4 - Evidence Decay & Epistemic Debt|B.3.4]]),
...
* cited evidence paths ([[G.6 - Evidence Graph & Provenance Ledger|G.6]] `PathId/PathSliceId`),
* declared crossings ([[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] sentinels; `CrossingBundle` visibility),
...
* `RerunHarvest` (delegates to [[G.2 - SoTA Harvester & Synthesis|G.2]]/[[G.1 - CG‑Frame‑Ready Generator|G.1]]/governing definition; if used)
* `RerunParity` (delegates to [[G.9 - Parity and Benchmark Harness|G.9]])
* `RecomputeSelectionOrSetPublication` (delegates to [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]])
* `RebindBridgeOrCrossing` (delegates to [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] and visibility harnesses)
* `UpdateEvidenceBindings` (delegates to [[G.6 - Evidence Graph & Provenance Ledger|G.6]])
* `ReshipPack` (delegates to [[G.10 - SoTA Pack Shipping|G.10]])
* `UpdateBundle` (delegates to [[G.8 - SoS‑LOG Bundles & Maturity Ladders|G.8]])
* `UpdateDashboardSlice` (delegates to [[G.12 - DHC Dashboards (Discipline‑Health time‑series; admissible telemetry; generation‑first)|G.12]])
...
Gating outcomes (admit / degrade / abstain) follow [[G.Core - Part G Core Invariants|G.Core]] tri-state semantics and are recorded via policy ids and cited evidence paths, rather than as local bespoke statuses.

### [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:4.3a - Causal-use refresh sentinels

When a shipped pack, parity report, evidence path, dashboard slice, fairness audit, policy evaluation, or selector output consumes [[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]], refresh planning includes causal-use sentinel causes when they can change supported use, unsupported use, support verdict, or downstream assurance:

| Causal-use sentinel | Typical affected [[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] result or related claim | Refresh payload pins |
| --- | --- | --- |
...
These sentinels do not mint new `RSCRTriggerKindId` values. They are domain-facing payload distinctions carried under the canonical trigger kinds governed by [[G.Core - Part G Core Invariants|G.Core]], usually evidence-publication edit, edition-pin change, policy-pin change, telemetry delta, freshness/decay event, or tokenization/name change.

### [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:4.4 - Extensions (pattern-scoped; non-core)

Discipline-specific refresh strategies and generator-specific wiring live as `GPatternExtension` blocks. Scheduling, ordering, priority, and budget policy for the refresh queue are not a separate extension semantics: [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] governs the required policy pins on `RefreshQueue` and `RefreshPlan@Context`, while [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] keeps WorkPlanning separate from executed Work.

#### [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:Ext.LegacyTriggers
...
**Uses:** `{[[G.Core - Part G Core Invariants|G.Core]]}` (cites `G.Core.TriggerAliasMap.G11`)
**⊑/⊑⁺:** `∅`
...
#### [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:Ext.DecayAndDebt
...
**Notes (wiring-only):** Any budget/priority logic remains policy-bound; [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] only wires decay events to refresh planning.

#### [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:Ext.QDRefreshWiring

**PatternScopeId:** `G.11:Ext.QDRefreshWiring`
**GPatternExtensionId:** `QDRefreshWiring`
**GPatternExtensionKind:** `MethodSpecific`
**GoverningPatternId:** [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]] (QD semantics; descriptor/distance/insertion)
**Uses:** `{[[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.8 - SoS‑LOG Bundles & Maturity Ladders|G.8]]}`
**⊑/⊑⁺:** `∅`
**RequiredPins / EditionPins / PolicyPins (minimum):**
...
**Notes (wiring-only):** [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] does not restate QD semantics; it ensures pins are present so reruns are comparable.

#### [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:Ext.OEERefreshWiring

**PatternScopeId:** `G.11:Ext.OEERefreshWiring`
**GPatternExtensionId:** `OEERefreshWiring`
**GPatternExtensionKind:** `MethodSpecific`
**GoverningPatternId:** [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] (open-ended exploration / E–E logistics)
**Uses:** `{[[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.8 - SoS‑LOG Bundles & Maturity Ladders|G.8]], [[G.9 - Parity and Benchmark Harness|G.9]]}`
**⊑/⊑⁺:** `∅`
**RequiredPins / EditionPins / PolicyPins (minimum):**
...
#### [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:4.4a - Scheduling and priority policy pins
...
[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] therefore treats scheduling as policy-bound refresh planning:
...
* `RSCRTriggerKindId[]` still comes from [[G.Core - Part G Core Invariants|G.Core]]; scheduling policy does not mint trigger kinds.
...
## [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:5 - Archetypal Grounding — System / Episteme (informative; Tell–Show–Show)

**`U.System` illustration — Safety-critical maintenance loop (pump + calibration).**
A centrifugal pump is serviced under a documented procedure (method description). Sensors report vibration drift (telemetry), and a calibration standard is updated (edition bump). [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] does not “rebuild the whole maintenance doctrine”: it emits a refresh plan scoped to the affected inspection slices (paths) and publishes a refresh report with pins to the updated standard edition and the evidence paths. Deprecation notices are issued for obsolete thresholds in the procedure’s acceptance clauses (by governing pattern), preserving ID continuity.

**`U.Episteme` illustration — Living review and benchmark pack (claims + parity).**
A claim sheet behind a shipped SoTA pack changes (new evidence, retraction, or revised measurement definition). Bridges are recalibrated, affecting CL/plane penalties. [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] ingests canonical trigger kinds, computes the minimal closure over affected `PathSliceId`s, schedules targeted parity reruns, then re-ships the pack through the pattern governing shipping semantics while publishing an edition bump log that makes the evolution replayable.

## [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:6 - Bias-Annotation (informative)
...
* **Onto/Epist bias (toward one governing definition semantics).** Risk: teams try to localize trigger meaning for convenience. Mitigation: alias docking is allowed, but semantics stay in [[G.Core - Part G Core Invariants|G.Core]].
...
## [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:7 - Conformance Checklist (normative)

| ID                                                    | Requirement                                                                                                                                                                                                                                                                                                                                     | Purpose / Notes                                                                                                            |
| ----------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| **CC‑G11‑CoreRef**                                    | A conforming [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] artefact **MUST** satisfy the **effective** core conformance set implied by the `GCoreLinkageManifest` in `G.11:4.1` (profile expansion + explicit deltas; delegated to [[G.Core - Part G Core Invariants|G.Core]]).                                                                                                                                       | Phase‑2 bridge clause: [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] is conformant only if the relevant [[G.Core - Part G Core Invariants|G.Core]] invariants and trigger discipline are satisfied. |
...
| **CC-G11.10 (Causal-use refresh sentinels).**          | When a refreshed publication or output consumes [[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]], a conforming `RefreshPlan@Context` **SHALL** include causal-use sentinel payload distinctions when counterfactual realizability, counterfactual-data identification/bounding, target-trial reporting, causal fairness, causal representation validation, off-policy/causal-RL evaluation, or simulation validation can change supported use, unsupported use, support verdict, assurance, parity, or downstream selection. | Keeps moving causal SoTA from silently invalidating shipped causal-use results while preserving [[G.Core - Part G Core Invariants|G.Core]] trigger governance. |

## [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:8 - Common Anti-Patterns and How to Avoid Them (informative)
...
| **Silent crossing during refresh** | Refresh changes context or plane assumptions without crossings       | Violates crossing visibility; penalties become hidden    | Require crossing pins + [[E.18 - Transformation Flow Structure|E.18]] visibility; block publication (CC‑G11.6)             |
...
## [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:9 - Consequences (informative)
...
* **Stable semantics with flexible operations.** Trigger meaning is centralized ([[G.Core - Part G Core Invariants|G.Core]]), while scheduling sophistication can evolve as policy-bound extensions.
* **Clear governing-definition assignment boundaries.** Orchestration coordinates governing definitions; it does not redefine their semantics (shipping remains [[G.10 - SoTA Pack Shipping|G.10]], selection remains [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], etc.).
...
## [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:10 - Rationale (informative)

[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] is intentionally a **thin orchestration governing definition**:

* The refresh loop is powerful enough to coordinate reruns and republishing, but **too thin to become a second spec**. That is why trigger semantics, invariants, and defaults are delegated to [[G.Core - Part G Core Invariants|G.Core]].
...
## [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:11 - SoTA-Echoing — Post‑2015 practices aligned (informative)
...
1. **Continuous refresh is necessary in deployed evaluation pipelines.**
   ...
   Alignment: [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] formalizes triggers as typed causes and forces edition/policy pins for replay.
...
2. **Non-stationarity requires explicit drift/decay handling, not ad-hoc updates.**
   ...
   Alignment: [[B.3.4 - Evidence Decay & Epistemic Debt|B.3.4]] supplies decay semantics; [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] wires decay events into refresh planning and controlled deprecation.
...
5. **Efficient orchestration benefits from bandit/early-stopping scheduling—but it must not become semantics.**
   ...
   Alignment: scheduling is expressed as [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] queue/plan policy pins (`RefreshPriorityPolicyIdRef`, `BudgetDeclRef`) so core semantics remain stable and WorkPlanning stays separate from executed Work.
   Adoption: **Adapt** (useful practice, but quarantined outside core norms).

## [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:12 - Relations

**Builds on:** [[G.Core - Part G Core Invariants|G.Core]] (Part‑G invariants; RSCR trigger catalogue; alias docking; Default Governing Definition Index), [[G.6 - Evidence Graph & Provenance Ledger|G.6]] (EvidenceGraph, `PathId/PathSliceId`), [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] (Bridge sentinels; CL/Φ/plane pins), [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] (selector & set-return), [[G.8 - SoS‑LOG Bundles & Maturity Ladders|G.8]] (bundle telemetry hooks), [[G.9 - Parity and Benchmark Harness|G.9]] (parity), [[G.10 - SoTA Pack Shipping|G.10]] (shipping hooks), [[B.3.4 - Evidence Decay & Epistemic Debt|B.3.4]] (freshness/decay), [[E.18 - Transformation Flow Structure|E.18]] (GateCrossing visibility).
**Coordinates with:** [[G.12 - DHC Dashboards (Discipline‑Health time‑series; admissible telemetry; generation‑first)|G.12]] (dashboard telemetry pins), optional `C.18 and C.19` (QD/E–E pins), [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]] (SoS-LOG branches and maturity ladders), [[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] (causal-use support records, support verdicts, supported-use values, unsupported-use values, and SoTA-sensitive causal-use sentinel payloads), [[F.15 - SCR-RSCR Harness for Unification|F.15]] (RSCR harness publications, when present).
**Publishes to:** UTS (refresh plan/report, deprecations, edition bumps), and to the relevant governing patterns’ publication faces, forms, or units via delegated actions.

## [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]:End
