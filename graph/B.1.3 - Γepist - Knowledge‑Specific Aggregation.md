...
**[[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] compliance —** Use **ConstituentOf** for semantic parts; **PortionOf** only for quantitative splits of texts/data with declared μ (token/byte, etc.); **PhaseOf** for versions/revisions of MethodDescription/documents; no **ComponentOf** here.

> **Plain‑English headline.**
> **Γ\_epist** composes **epistemic holons** (claims, models, datasets, arguments) into a **single episteme** while preserving **provenance**, applying **conservative trust bounds** ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] F/G/R), and penalizing **poor conceptual fit** via **congruence levels (CL)**. It is **not** a physical sum; it is a **semantic and evidential fold**.

## [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:1 - Problem frame

* **Holonic foundation.** In the FPF, a `U.Episteme` is a holon whose identity is **knowledge‑bearing** ([[A.1 - U.Holon, U.System, and U.Episteme|A.1]]). It can be a **statement/claim**, a **model**, a **theory**, a **specification**, a **dataset with semantics**, or a **compiled scholarly artifact**.
* **Strict Distinction ([[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]).** We separate:
...
* **Mereology ([[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]).** For knowledge composition we primarily use **ConstituentOf** (logical/semantic parts), **UsageOf/ReferenceTo** (external reliance), and **MemberOf** for **collections** (anthologies, corpora). We do **not** use **ComponentOf** (physical) in Γ\_epist.
  `PhaseOf` handles temporal versions of the **same** episteme; **RoleBearerOf** is irrelevant here because knowledge **does not play a role**—it is **used** by a holon‑in‑role (Transformer) at run‑time ([[A.12 - External Transformer & Reflexive Split|A.12]]).
* **Assurance ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).** Knowledge carries **F**, **G**, **R** (Formality, ClaimScope, Reliability). Integration edges carry **CL** (congruence level) that penalizes poor fit. Γ\_epist **must** preserve provenance and apply **conservative** bounds: no “truth averaging,” no silent context hops. **Obligations here are mode/assurance‑gated per [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]].**  # [M‑0]
* **Order/time flavours.** Argument sequences may need **Γ\_ctx** (non‑commutative ordering of premises to conclusion). Knowledge evolution uses **Γ\_time** (versioning, deprecation, update). When composition produces **new closure or supervision** (e.g., explanatory theory emerges), we declare **MHT** ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]).

## [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:2 - Problem

Naive aggregation of knowledge holons causes recurring failures:

1. **Trust inflation by averaging.** Averaging confidences of conflicting claims creates a falsely “reliable” whole; violates **WLNK** and **[[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]** conservatism.
2. **Provenance erasure.** Merges that drop sources, methods, or links break **[[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] Evidence Graph Referring** and make results unauditable.
...
5. **Context chimeras.** Combining items across **bounded contexts** (different vocabularies/units/policies) without a **Context Reframe** ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]) silently corrupts claims and inflates **R**.
...
## [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:3 - Forces
...
| **Parsimony vs. Fit**                      | Small rule set ([[A.11 - Ontological Parsimony (C‑5)|A.11]]) ↔ explicit **congruence** penalties and **context** rebasing when needed.                              |

## [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:4 - Solution — **Terms, operator family, invariant Standard, core rules**

### [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:4.1 - Terms (didactic recap)
...
* **Evidence/Provenance Graph** — edges like **evidences**, **derivesFrom**, **usesMethod**, **isMeasuredBy** with anchors ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]).
...
* **Bounded context** — a modelling Standard (vocabulary/units/policy). Crossing it requires **Context Reframe** ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]) or explicit mappings with CL.

> **Didactic reminders.**
> • Knowledge does **not** “act.” Transformers ([[A.12 - External Transformer & Reflexive Split|A.12]]) **use** knowledge.
...
### [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:4.2 - The operator family (companion flavours)

To keep **design vs run** clean ([[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]), Γ\_epist has two companion flavours that share the same algebra but serve different moments:
...
* **Result.** A **context‑anchored** episteme (e.g., published paper/spec) whose **mappings to the context vocabulary** are explicit and carry **CL**; assurance will reference this context baseline ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).

**Relationship to Γ\_ctx / Γ\_time.**
If the knowledge fold explicitly depends on **argument order** (e.g., derivation), the internal fold uses **Γ\_ctx** for the sequence. If a **temporal storyline** (updates, retractions) is important, use **Γ\_time** to slice versions; **Γ\_epist** then composes the **current slice**. If composition yields **new explanatory closure** beyond WLNK/CL, declare **MHT** ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]).

### [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:4.3 - Invariant Standard (how the Quintet applies; **math by level**)
...
### [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:4.4 - Core rules for epistemic aggregation (design‑time synthesis)
...
### [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:4.5 - Core rules for compilation (run‑time context anchoring)

When computing **Γ\_epist^compile(E\_synth, Ctx, T)**:

1. **Context bindings.**  # [M‑1+]
   Map all operative concepts/units/claims into **Ctx**; record mappings and their **CL**. If the rebase changes boundary/objective of the episteme (e.g., from descriptive compendium to explanatory theory with commitments), **declare Context Reframe (MHT)** per [[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]].

2. **Assurance baseline (gated).**
   Recalculate the **assurance tuple** ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]) **in Ctx**: F and R may change with formalization and mapping penalties; G is re‑expressed in Ctx’s scope.
   **Gating:**
...
## [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:5 - Archetypal grounding (worked, didactic)

### [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:5.1 - Episteme — **Meta‑analysis into a guidance statement**
...
### [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:5.2 - Episteme — **Safety case from heterogeneous evidence**
...
* **Γ\_epist^compile (certification context):**
  Context re‑base to regulatory vocabulary; if the re‑base changes objective/boundary (e.g., from internal assurance to public certification), consider **MHT (Context Reframe)** per [[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]].

### [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:5.3 - Contrast (didactic)
...
## [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:6 - Proof obligations (normative)
...
4. **PO‑COMP‑MHT.** If the compilation re‑anchors **boundary**, **objective**, or **identity** (e.g., from compendium to explanatory theory), an **MHT (Context Reframe)** MUST be declared with a Promotion Record ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]).
...
## [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:7 - Conformance Checklist (normative)
...
| **CC‑B1.3.8** | Design‑time **synthesis** and run‑time **compilation** MUST NOT be conflated; use the appropriate flavour.                                                          | Maintain [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] separation.      |

## [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:8 - Anti‑patterns & repairs
...
## [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:9 - Consequences
...
* **Didactic clarity.** Separates **semantic folding** (Γ\_epist) from **order** (Γ\_ctx), **time** (Γ\_time), **spend** (Γ\_work), and **emergence** ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]).
...
## [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:10 - Rationale (informative)
...
## [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:11 - Relations

* **Builds on:** [[A.12 - External Transformer & Reflexive Split|A.12]] (Transformer Role—compilers/editors enact), [[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] (Mereology Extension—ConstituentOf/MemberOf/PhaseOf usage), [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] (Strict Distinction).
* **Coordinates with:** [[B.1.1 - Dependency Graph & Proofs|B.1.1]] (Proof kit), [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]] (Γ\_ctx/Γ\_time inside knowledge folds), [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]] (Γ\_work for compute/collection spend).
* **Triggers/Complements:** [[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]] (MHT) when explanatory closure or context re‑base creates a **new whole** (theory, standard).
* **Feeds:** [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (Assurance) — `F/G/R` and **CL** baselines computed here become inputs to trust calculations.
...
## [[B.1.3 - Γepist - Knowledge‑Specific Aggregation|B.1.3]]:End
