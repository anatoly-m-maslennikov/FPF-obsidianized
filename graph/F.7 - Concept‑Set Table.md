...
**Depends on.** [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **Lexical Discipline for ‘Context’** (Context ≡ `U.BoundedContext`); **[[F.0.1 - Contextual Lexicon Principles|F.0.1]] senseFamily (normative)**; [[F.1 - Domain‑Family Landscape Survey|F.1]] **Domain‑Family Landscape Survey**; [[F.2 - Term Harvesting & Normalisation|F.2]] **Term Harvesting**; [[F.3 - Intra‑Context Sense Clustering|F.3]] **Intra‑Context Sense Clustering** (SenseCells); [[F.5 - Naming Discipline for U.Types & Roles|F.5]] **Naming Discipline**; [[F.9 - Alignment & Bridge across Contexts|F.9]] **Alignment & Bridge Across Contexts**.
**Coordinates with.** [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] **Role Description**; [[F.6 - Role Assignment & Enactment Cycle (Six-Step)|F.6]] **Role Assignment & Enactment Cycle (Six-Step)**; Part C patterns (for examples), **MM‑CHR (for Characteristic)**; [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] (RPR‑XCTX for repairing umbrella cross‑Context sameness/alignment prose before it justifies rows).
**Aliases (informative).** *Concept‑Set table*, *comparison grid*.

## [[F.7 - Concept‑Set Table|F.7]]:1 - Intent & applicability

**Intent.** Provide a **single, didactic page** where each **row** presents **one Concept‑Set**—a *set of SenseCells from different Contexts that we are licensed (by explicit Bridges) to treat as “the same for a stated scope”*. Columns are **Contexts**; cells carry **local labels**. The table **does not invent equivalences**: it **summarises** already declared **[[F.9 - Alignment & Bridge across Contexts|F.9]] Bridges**, exposing *scope, losses, and counter‑examples* at a glance.
...
## [[F.7 - Concept‑Set Table|F.7]]:2 - Problem frame
...
## [[F.7 - Concept‑Set Table|F.7]]:3 - Forces
...
## [[F.7 - Concept‑Set Table|F.7]]:4 - Core idea (didactic)
...
that FPF **treats as one** *for a declared scope* because there exist **[[F.9 - Alignment & Bridge across Contexts|F.9]] Bridges** connecting these SenseCells pairwise (directly or via a short chain) with **congruence level** $\text{CL}$ above a **threshold** suitable for that scope. The **table row** shows:

* **FPF Label** *(Tech/Plain)* — the *didactic, FPF‑level* name chosen per [[F.5 - Naming Discipline for U.Types & Roles|F.5]].
...
## [[F.7 - Concept‑Set Table|F.7]]:5 - Minimal vocabulary (this pattern only)

* **Context** — shorthand for `U.BoundedContext` (per [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]).
* **senseFamily** — **referenced from [[F.0.1 - Contextual Lexicon Principles|F.0.1]]**; not redefined here; used to **type** rows and to require **uniformity** within a row.
* **SenseCell** — a **(Context × Local‑Sense)** address from [[F.3 - Intra‑Context Sense Clustering|F.3]].
* **Bridge ([[F.9 - Alignment & Bridge across Contexts|F.9]])** — an explicit, declarative Cross‑context mapping with a **congruence level** **CL** and **loss note**.
...
## [[F.7 - Concept‑Set Table|F.7]]:6 - The table (conceptual layout)
...
4. **Rationale & counter‑examples** are **obligatory one‑liners**; if you need paragraphs, you need an [[F.9 - Alignment & Bridge across Contexts|F.9]] walkthrough, not a row.
...
## [[F.7 - Concept‑Set Table|F.7]]:7 - Conceptual construction (thought moves, not workflow)

> The table is derived from earlier patterns; it **creates nothing new**.

* **Sourcing.** Candidate cells come **only** from **SenseCells** ([[F.3 - Intra‑Context Sense Clustering|F.3]]).
* **Licensing.** A row exists **iff** the relevant **Bridges ([[F.9 - Alignment & Bridge across Contexts|F.9]])** already justify sameness at the chosen **Row Scope**.
...
## [[F.7 - Concept‑Set Table|F.7]]:8 - Invariants (normative)

1. **Context‑loyal cells.** Every non‑empty cell is a **SenseCell** address; no minted paraphrases.
2. **Bridge sufficiency.** For a *Concept‑Set* row, **every pair** of filled cells is connected by an **[[F.9 - Alignment & Bridge across Contexts|F.9]] Bridge path** whose **bottleneck CL** ≥ **Row CL(min)** printed for the row.
...
## [[F.7 - Concept‑Set Table|F.7]]:9 - Micro‑illustrations (safe patterns)

> Illustrative only; these presume corresponding **[[F.9 - Alignment & Bridge across Contexts|F.9]] Bridges** exist with stated CL and losses.
...
## [[F.7 - Concept‑Set Table|F.7]]:10 - Anti‑patterns & remedies

| #         | Anti‑pattern                | Symptom in a row                                                               | Why it breaks thinking                                               | Remedy (conceptual move)                                                                                          |
| --------- | --------------------------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| **AP‑1**  | **Bridge‑free sameness**    | Cells listed as “same” because their labels look alike; no cited Bridges.      | Violates locality; imports meaning across Contexts by name.             | A row **exists only** if backed by **[[F.9 - Alignment & Bridge across Contexts|F.9]] Bridges**. Otherwise produce a **contrast row**.                         |
...
| **AP‑3**  | **senseFamily mixing**      | One row mixes Role, Status, Measurement, and Type‑structure cells.             | Conflates senseFamily ([[F.0.1 - Contextual Lexicon Principles|F.0.1]]); readers cannot tell “what kind of sameness”. | **Type each row.** If two senseFamilys are needed, **split** into two rows.                                             |
...
| **AP‑9**  | **Duplicate rows by style** | Two rows with the same cell set but different FPF labels.                      | Name inflation; readers assume two distinct concepts.                | Keep **one row** per Concept‑Set per scope. Alternative labels appear as **aliases** in [[F.5 - Naming Discipline for U.Types & Roles|F.5]], not new rows.        |
...
## [[F.7 - Concept‑Set Table|F.7]]:11 - Worked examples

> Each example gives a **row** (compact), then a **reading** explaining scope and limits. All sameness claims presuppose suitable **[[F.9 - Alignment & Bridge across Contexts|F.9]] Bridges** with the stated CL.

### [[F.7 - Concept‑Set Table|F.7]]:11.1 - Behavioural actor across Contexts (naming‑only)
...
### [[F.7 - Concept‑Set Table|F.7]]:11.2 - Execution occurrence (assignment/enactment-eligibility)
...
### [[F.7 - Concept‑Set Table|F.7]]:11.3 - Result value as KD‑metric (measurement)
...
### [[F.7 - Concept‑Set Table|F.7]]:11.4 - Subtype relation (type‑structure)
...
### [[F.7 - Concept‑Set Table|F.7]]:11.5 - Contrast: “role” (access vs behaviour)
...
## [[F.7 - Concept‑Set Table|F.7]]:12 - Reasoning primitives (judgement schemas, notation‑free)

> All judgements are **pure** (no side effects). “Contexts” are `U.BoundedContext`. `SC(C)` denotes a SenseCell in Context `C`. `CL(X↔Y)` is the congruence level of the **best** Bridge path ([[F.9 - Alignment & Bridge across Contexts|F.9]]) between SenseCells `X` and `Y` (bottleneck along that path).

### [[F.7 - Concept‑Set Table|F.7]]:12.1 - Row licensing
...
### [[F.7 - Concept‑Set Table|F.7]]:12.2 - Bottleneck CL for a row
...
### [[F.7 - Concept‑Set Table|F.7]]:12.3 - Scope guard
...
### [[F.7 - Concept‑Set Table|F.7]]:12.4 - Contrast decision
...
### [[F.7 - Concept‑Set Table|F.7]]:12.5 - Row extension guard
...
### [[F.7 - Concept‑Set Table|F.7]]:12.6 - Loss disclosure obligation
...
## [[F.7 - Concept‑Set Table|F.7]]:13 - Relations (with other patterns)

**Builds on:**
[[F.1 - Domain‑Family Landscape Survey|F.1]] **Contexts fixed** → defines the column set; [[F.2 - Term Harvesting & Normalisation|F.2]] **Harvest** → supplies harvested terms; [[F.3 - Intra‑Context Sense Clustering|F.3]] **SenseCells** → provide cell addresses; [[F.5 - Naming Discipline for U.Types & Roles|F.5]] **Naming Discipline** → provides the two‑register **FPF labels**; [[F.9 - Alignment & Bridge across Contexts|F.9]] **Bridges** → justify each row.

**Constrains:**
[[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] **Role Description** — when a template cites an FPF label from the table, it **inherits the Row Scope**; no template may claim semantics beyond the row’s licence.
[[F.6 - Role Assignment & Enactment Cycle (Six-Step)|F.6]] **Role Assignment & Enactment Cycle (Six-Step)** — Move M‑4 (“choose label”) must reference a row if it wants Cross‑context reading.

**Used by.**
Part C patterns for didactic alignment pages; Part B trust calculus ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]) may consume **Row CL(min)** when computing translation penalties.

## [[F.7 - Concept‑Set Table|F.7]]:14 - Migration notes (conceptual)
...
3. **Sense revision inside a Context.** If a SenseCell splits ([[F.3 - Intra‑Context Sense Clustering|F.3]]), decide which child cell (if any) remains in the row; the rest may require **new rows** or a **contrast**.
...
5. **Deprecation.** If a row no longer meets its invariant, mark its FPF label as **retired** in [[F.5 - Naming Discipline for U.Types & Roles|F.5]] and point to successor rows (if any).
6. **Edition churn.** When a Context is superseded ([[F.1 - Domain‑Family Landscape Survey|F.1]]), either keep the cell (if semantics stable) or treat the successor as a **new Context** and re‑evaluate licensability.

## [[F.7 - Concept‑Set Table|F.7]]:15 - Acceptance tests (SCR/RSCR — concept‑level)

### [[F.7 - Concept‑Set Table|F.7]]:15.1 - Static conformance checks (SCR)

* **SCR‑F7‑S01 (Context‑loyal cells).** Every non‑empty cell references an existing **SenseCell** ([[F.3 - Intra‑Context Sense Clustering|F.3]]) in a declared Context ([[F.1 - Domain‑Family Landscape Survey|F.1]]).
...
### [[F.7 - Concept‑Set Table|F.7]]:15.2 - Regression checks (RSCR)

* **RSCR‑F7‑E01 (Bridge drift).** After any Bridge change ([[F.9 - Alignment & Bridge across Contexts|F.9]]), recompute **Row CL(min)**; flag rows whose scope is now overstated.
* **RSCR‑F7‑E02 (Sense split).** After a SenseCell splits ([[F.3 - Intra‑Context Sense Clustering|F.3]]), ensure rows referencing it either pick a child cell or retire.
...
## [[F.7 - Concept‑Set Table|F.7]]:16 - Didactic distillation (60‑second teaching script)
