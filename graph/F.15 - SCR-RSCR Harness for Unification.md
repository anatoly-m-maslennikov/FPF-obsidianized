...
**Builds on:** [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **Lexical Discipline for “Context” (D.CTX)**; [[F.0.1 - Contextual Lexicon Principles|F.0.1]] **Foundational Principles**; [[F.1 - Domain‑Family Landscape Survey|F.1]]–[[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]].
**Coordinates with.** [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] **Trust & Assurance Calculus** (for CL use on Bridges).

## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:1 - Intent & applicability

**Intent.** Provide a **minimal, notation‑free harness of conceptual checks** that tells you whether a unification slice is **sound**: Contexts are fixed and diverse ([[F.1 - Domain‑Family Landscape Survey|F.1]]), terms are harvested and clustered **inside** Contexts ([[F.2 - Term Harvesting & Normalisation|F.2]]–[[F.3 - Intra‑Context Sense Clustering|F.3]]), Role Descriptions **point to one SenseCell** ([[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]), names obey discipline ([[F.5 - Naming Discipline for U.Types & Roles|F.5]]), rows are reused instead of multiplied ([[F.7 - Concept‑Set Table|F.7]]–[[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]), bridges are **explicit and penalised** ([[F.9 - Alignment & Bridge across Contexts|F.9]]), statuses vary by **windows** not type proliferation ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]), and cross‑line bindings ([[F.11 - Method Quartet Harmonisation|F.11]]–[[F.12 - Service Acceptance-Work Evidence Link|F.12]]) respect locality.
...
## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:2 - Problem frame
...
## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:3 - Forces
...
## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:4 - Core idea (didactic)
...
## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:5 - Minimal vocabulary (this pattern only)
...
## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:6 - Objects under check

1. **Contexts** — `U.BoundedContext` cards ([[F.1 - Domain‑Family Landscape Survey|F.1]]).
2. **Local‑Sense** — clustered sense inside one context ([[F.3 - Intra‑Context Sense Clustering|F.3]]).
3. **SenseCell** — *(Context × Local‑Sense)* address (F.3/F.4).
4. **Concept‑Set row** — a cross-context alignment hypothesis with explicit Bridge support ([[F.7 - Concept‑Set Table|F.7]]).
5. **Role Description** — Role or Status definition pointing to **one** SenseCell ([[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]).
6. **Bridge** — explicit Cross‑context relation with CL and loss notes ([[F.9 - Alignment & Bridge across Contexts|F.9]]).
7. **Windows** — temporal/scale views for a Status family ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]).
8. **Aliases** — name continuity commitments ([[F.13 - Lexical Continuity & Deprecation|F.13]]).
9. **Bundles & SoD** — reuse levers that replace hybrid roles ([[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]).

## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:7 - Solution overview — the harness as a lattice of checks
...
## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:8 - SCR — Static conformance rules (S‑Local)

> All S‑Local rules are **Context‑internal** and derive only from [[F.1 - Domain‑Family Landscape Survey|F.1]]–[[F.5 - Naming Discipline for U.Types & Roles|F.5]].

**SCR‑F15‑S1 (Anchored term).**
`Seed σ has context C ⊢ C ∈ Contexts(L)`
*Reading:* Every harvested seed lives in a Context that is **deliberately in view** for your line ([[F.1 - Domain‑Family Landscape Survey|F.1]], [[F.2 - Term Harvesting & Normalisation|F.2]]).

**SCR‑F15‑S2 (Edition trace).**
`Occurrence ω supports σ ⊢ ω carries edition+location`
*Reading:* A Local‑Sense can be mentally reconstructed from attestations ([[F.2 - Term Harvesting & Normalisation|F.2]]).

**SCR‑F15‑S3 (Intra‑Context clustering).**
`Local‑Sense λ clusters {σᵢ} ⊢ ∀i: context(σᵢ)=context(λ)`
*Reading:* No Cross‑context items inside a Local‑Sense ([[F.3 - Intra‑Context Sense Clustering|F.3]]).
...
*Reading:* No global normal form at this stage ([[F.2 - Term Harvesting & Normalisation|F.2]]).

## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:9 - SCR — Static conformance rules (S‑Cross)
...
*Reading:* Every Role Description points to exactly **one** SenseCell; no mixed semantics ([[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]).

**SCR-F15-S8 (Name discipline).**
`Role Description τ ⊢ name(τ) obeys F.5`
*Reading:* Labels follow the agreed morphology, register pairing, and minimal generality ([[F.5 - Naming Discipline for U.Types & Roles|F.5]]).

**SCR‑F15‑S9 (Row sufficiency).**
`Row ρ lists cells {⟨Cᵢ,λᵢ⟩} ⊢ |distinct(Cᵢ)| ≥ 2`
*Reading:* A row is meaningful only if it spans **at least two Contexts** ([[F.7 - Concept‑Set Table|F.7]]).

**SCR‑F15‑S10 (Row purity).**
`Row ρ ⊢ no cell contains Cross‑context clustering`
*Reading:* Each cell is a **single** SenseCell, not a pre‑merged bundle ([[F.7 - Concept‑Set Table|F.7]]).

**SCR‑F15‑S11 (Reuse before mint).**
`Proposed row ρ' overlaps intent(ρ) ⊢ prefer reuse(ρ) ∨ document [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] decision`
*Reading:* Rows are reused by default; new rows require a mint‑or‑reuse rationale ([[F.7 - Concept‑Set Table|F.7]]–[[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]).

**SCR‑F15‑S12 (Bridge is explicit).**
`C₁≠C₂ ∧ relation asserted between λ₁,λ₂ ⊢ Bridge β: ⟨⟨C₁,λ₁⟩ ↔ ⟨C₂,λ₂⟩, kind, CL, loss⟩`
*Reading:* Cross‑context relations appear **only** as Bridges with declared kind (≡, ⊑, ⊒, ⟂), Congruence Level, and loss notes ([[F.9 - Alignment & Bridge across Contexts|F.9]]; [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] for CL semantics).
...
*Reading:* Temporal and scale variation appears as **windows**, not as new types ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]).

**SCR-F15-S15 (SoD preservation).**
`Bundle B = {τ₁,τ₂,…} with SoD(τᵢ ⟂ τⱼ) ⊢ no single **Role Description** fuses τᵢ,τⱼ`
*Reading:* Separation‑of‑Duties is a **normative constraint**, not a label tweak ([[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]).
...
*Reading:* Acceptance compares **anchored** executions and statuses, with any Cross‑context step made explicit ([[F.12 - Service Acceptance-Work Evidence Link|F.12]] + [[F.9 - Alignment & Bridge across Contexts|F.9]]).

> **SCR/RSCR “Twin Harness” tests**

**SCR‑TWIN‑01 - Head term check.** Plain twin preserves/declares the head per **CC‑TWIN‑3**.
**SCR‑TWIN‑02 - Kind check.** Plain twin maps to the same **Kind** as the Tech name ([[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]).
...
4.  **Register.** Create/update entries in **[[E.10.P - Conceptual Prefixes policy & registry|E.10.P]]**; link a **DRR** for each change.
5.  **Lint.** Enable the **Twin Harness** in CI to block new ambiguous twins.

## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:10 - Judgement schemas (core moves)
...
## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:11 - Micro‑witnesses (illustrative)
...
## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:12 - Relations (with other patterns)

**Builds on:** [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] (Context semantics), [[F.1 - Domain‑Family Landscape Survey|F.1]]–[[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]].
**Constrains:** Any addition to [[F.1 - Domain‑Family Landscape Survey|F.1]]–[[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]] is **publish‑ready** only if all relevant **SCR** here evaluate **true** on its slice.
**Feed:** [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] may use Bridge CL and loss notes to adjust assurance.

## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:13 - RSCR — Regression & Stability Rules (R‑Evo)
...
### [[F.15 - SCR-RSCR Harness for Unification|F.15]]:13.1 - Contexts & editions
...
### [[F.15 - SCR-RSCR Harness for Unification|F.15]]:13.2 - Local‑Senses & SenseCells
...
### [[F.15 - SCR-RSCR Harness for Unification|F.15]]:13.3 - Concept‑Set rows

**RSCR‑F15‑E5 (Row identity).**
`Row ρ@t0 with cells {⟨Cᵢ,λᵢ⟩} → ρ@t1 with {⟨Cᵢ,λᵢ'⟩} ⊢ ρ “same” iff intent(λᵢ')≈intent(λᵢ) ∀i`
*Reading:* A row is the **same** row only if each cell still means the same Local-Sense in its Context. Otherwise, mint a **new row** and retire the old ([[F.7 - Concept‑Set Table|F.7]]–[[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]).
...
### [[F.15 - SCR-RSCR Harness for Unification|F.15]]:13.4 - Role Descriptions (Role and Status)

**RSCR-F15-E7 (Single-cell continuity).**
`Role Description τ@t0 → τ@t1 ⊢ anchor(τ@t1)=one SenseCell ∧ (sameCell ∨ justifiedSwitch)`
*Reading:* A **Role Description** keeps pointing to **one** SenseCell; switching cells requires a **one-sentence** rationale tied to the row you reuse ([[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]], [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]).

**RSCR-F15-E8 (Alias-then-rename).**
`name(τ@t0) → name(τ@t1) ⊢ create alias(name@t0→name@t1) unless semantics changed`
*Reading:* If only the **name** improves, create an **Alias** ([[F.13 - Lexical Continuity & Deprecation|F.13]]). If semantics change, **mint a new Role Description** instead.

### [[F.15 - SCR-RSCR Harness for Unification|F.15]]:13.5 - Bridges
...
### [[F.15 - SCR-RSCR Harness for Unification|F.15]]:13.6 - Status windows & SoD

**RSCR‑F15‑E11 (Window stability).**
`Status family Σ windows@t0 → @t1 ⊢ window set changes only if variance‑of‑meaning is shown`
*Reading:* Add or remove windows **only** when meaning genuinely varies across time/scale—never for convenience ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]).

**RSCR‑F15‑E12 (SoD invariance).**
`SoD(τᵢ ⟂ τⱼ) @t0 → @t1 ⊢ SoD preserved; no new Role Description conflates τᵢ,τⱼ`
*Reading:* Separation‑of‑Duties remains in force through changes ([[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]).

## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:14 - Anti‑patterns the harness catches (and the fix)
...
## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:15 - Worked “dry‑runs” (composite slices)

> Each dry‑run shows **how the checks fire** when something evolves. These are **thinking rehearsals**, not procedures.

### [[F.15 - SCR-RSCR Harness for Unification|F.15]]:15.1 - New edition of ITIL (services) arrives
...
**Pay‑off.** History is preserved; reuse remains safe; acceptance bindings ([[F.12 - Service Acceptance-Work Evidence Link|F.12]]) still compare anchored things.

### [[F.15 - SCR-RSCR Harness for Unification|F.15]]:15.2 - Rename a Role Description without changing meaning

**Slice.** **Role Description** `IncidentStatus`@t0 → `ServiceIncidentStatus`@t1; same SenseCell.

**Fire.**
E8 (**alias‑then‑rename**): create **Alias** `IncidentStatus → ServiceIncidentStatus` ([[F.13 - Lexical Continuity & Deprecation|F.13]]).
S8 (**name discipline**): new name fits the suffix rules ([[F.5 - Naming Discipline for U.Types & Roles|F.5]]).

**Pay‑off.** Readers find both names; semantics untouched.

### [[F.15 - SCR-RSCR Harness for Unification|F.15]]:15.3 - Tighten a Bridge (low-CL overlap → equivalence)
...
### [[F.15 - SCR-RSCR Harness for Unification|F.15]]:15.4 - Window misuse detected
...
**Pay‑off.** No type explosion; the evaluation logic in [[F.12 - Service Acceptance-Work Evidence Link|F.12]] stays uniform.

## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:16 - Migration cues (conceptual)
...
6. **Guard SoD early.** When binding composite responsibilities ([[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]), check SoD before naming.
...
## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:17 - Acceptance summary (“Harness green”)
...
4. **Memory rule still holds**: the active Context set for the line fits in a careful mind without external aids ([[F.1 - Domain‑Family Landscape Survey|F.1]]).

## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:18 - Didactic distillation (90‑second teaching script)
...
## [[F.15 - SCR-RSCR Harness for Unification|F.15]]:End
