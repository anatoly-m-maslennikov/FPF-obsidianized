...
**Status.** Transitional stub (informative). This section defines no dedicated “module” subsystem. Enforceable boundary discipline lives in **[[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]] `U.Signature`** and **[[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] `U.Mechanism`**, with guard‑rails in **[[E.5.3 - Unidirectional Dependency|E.5.3]]** (Unidirectional Dependency) and **[[E.10 - Unified Lexical Rules for FPF|E.10]]** (LEX‑BUNDLE stratification).

## [[A.5 - Open‑Ended Kernel & Extension Layering|A.5]]:1 - Problem frame
...
## [[A.5 - Open‑Ended Kernel & Extension Layering|A.5]]:2 - Problem
...
## [[A.5 - Open‑Ended Kernel & Extension Layering|A.5]]:3 - Forces
...
## [[A.5 - Open‑Ended Kernel & Extension Layering|A.5]]:4 - Solution
...
2. **Boundary packaging via `U.Signature` ([[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]]).** Reusable bundles are published as signatures with an explicit `SignatureManifest` (`imports`, `provides`).
3. **Dependency vs specialisation are separate relations.** `imports` forms a dependency DAG constrained by **[[E.5.3 - Unidirectional Dependency|E.5.3]]**; refinement/extension (`⊑`, `⊑⁺`) is expressed separately (e.g., **[[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] `U.MechMorph`**) and should not be conflated with `imports`.
...
This section is intentionally lightweight: it provides architectural intent and neighboring-pattern pointers only. Any new enforceable modularity constraints belong in the [[A.6 - Signature Stack & Boundary Discipline|A.6]].* boundary patterns (or in E.* guard‑rails), not here.

## [[A.5 - Open‑Ended Kernel & Extension Layering|A.5]]:End
