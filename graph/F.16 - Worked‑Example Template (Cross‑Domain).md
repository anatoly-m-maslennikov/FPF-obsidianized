...
**Builds on:** [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **Lexical Discipline for “Context” (D.CTX)**; [[F.1 - Domain‑Family Landscape Survey|F.1]]–[[F.15 - SCR-RSCR Harness for Unification|F.15]].
**Coordinates with.** [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] **Trust & Assurance Calculus** (CL on Bridges); Part C patterns (Sys‑CAL, KD‑CAL, Kind-CAL, Method‑CAL).

## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:1 - Intent & applicability
...
## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:2 - Problem frame
...
## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:3 - Core idea (didactic)

A robust worked example is a **compact theatre**:

* **Stage** = a declared **unification line** (which threads of Part C are in play).
* **Backdrop** = **Context set** (Contexts from [[F.1 - Domain‑Family Landscape Survey|F.1]]), each with a one‑line Card.
...
* **Moral** = a handful of **harness checks** ([[F.15 - SCR-RSCR Harness for Unification|F.15]]) that the reader can verify mentally.

## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:4 - Minimal vocabulary (this pattern only)

* **Context / Context** — always **U.BoundedContext** ([[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]).
* **Local‑Sense** — a sense clustered within one context ([[F.3 - Intra‑Context Sense Clustering|F.3]]).
...
* **Role Description (τ)** — a Role or Status that **points to one SenseCell** ([[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]–[[F.6 - Role Assignment & Enactment Cycle (Six-Step)|F.6]]).
* **Bridge (β)** — explicit Cross‑context relation with **kind** (≡ / overlaps / broader‑than / narrower‑than), **CL**, and **loss note** ([[F.9 - Alignment & Bridge across Contexts|F.9]]).
* **Window** — a bounded interval (time/scale/phase) tied to a Status ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]).
* **SoD** — Separation-of-Duties constraint among **Roles** ([[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]).

## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:5 - The one‑page **Worked‑Example Canvas**
...
3. **Context set (compact Cards).** 3–6 Contexts from [[F.1 - Domain‑Family Landscape Survey|F.1]] with one‑line scope and, if inherent, **design vs run** stance.
...
**Harness pings ([[F.15 - SCR-RSCR Harness for Unification|F.15]]).** *S-Row-Cross*, **S-RoleDesc-SingleCell**, *E-NoSilentEdition*.
...
## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:6 - Invariants (normative)
...
## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:7 - The row panel (how to show it without notation)
...
## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:8 - Worked micro‑example (didactic)
...
## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:9 - Relations (with other patterns)

**Builds on:**
[[F.1 - Domain‑Family Landscape Survey|F.1]] (Contexts), [[F.2 - Term Harvesting & Normalisation|F.2]]–[[F.3 - Intra‑Context Sense Clustering|F.3]] (terms & senses), [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]–[[F.6 - Role Assignment & Enactment Cycle (Six-Step)|F.6]] (roles), [[F.7 - Concept‑Set Table|F.7]]–[[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] (rows), [[F.9 - Alignment & Bridge across Contexts|F.9]] (bridges), [[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] (windows), [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]] (SoD), [[F.15 - SCR-RSCR Harness for Unification|F.15]] (harness).

**Constrains:**
Any example placed in Part C or Part B **must** render its claim through this canvas (or a faithful reduction), so readers can run [[F.15 - SCR-RSCR Harness for Unification|F.15]] mentally.

## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:10 - Didactic distillation (60‑second script)

> “A good cross‑domain example fits on **one page**. First, name the **claim**. Then show the **Contexts** you’re using. List the **SenseCells** you will actually touch. Draw **one row** that makes them the same **for this claim**. Every Cross‑context nuance you can’t justify in that row becomes a **Bridge** with a **kind**, a **CL**, and a **loss** sentence. Point each **Role Description** to **one** cell. If time/scale matters, state the **window**; if duties matter, state **SoD**. Finish with two or three **harness pings** from [[F.15 - SCR-RSCR Harness for Unification|F.15]]. That’s it—no tooling, no long procedures. The reader can now replay your thought and agree (or disagree) at the right place.”

## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:11 - Anti‑patterns & remedies
...
| **AP‑2**  | **Stealth bridging**       | Phrases like “basically the same” with no Bridge.              | Imports meaning Cross‑context; hides losses.                      | State a **Bridge β** with **kind, CL, loss** (§5‑6, [[F.9 - Alignment & Bridge across Contexts|F.9]]).                 |
...
| **AP‑4**  | **Global words**           | *process, role, service* appear unprefixed.                    | Context‑less words drift mid‑example.                             | Prefix first mention with the **Context** (*process (BPMN)*) (§6‑1, [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]). |
| **AP‑5**  | **Window‑free comparison** | Numbers/targets compared with no stated window.                | Apples‑to‑oranges across time/scale.                           | Declare a **Window** for the Status (§5‑8, [[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]).                         |
| **AP‑6**  | **SoD leakage**            | Duties named but the same actor implicitly holds both.         | Violates Separation‑of‑Duties intent.                          | State **SoD** and keep duties disjoint (§5‑8, [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]).                      |
...
| **AP‑9**  | **CL silence**             | Bridge kind given, no CL or loss note.                         | Reader cannot assess translation risk.                          | Add **CL** and **loss** in every Bridge (§5‑6, [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).                      |
...
## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:12 - Extended worked micro‑examples

> Each example fits the **one‑page canvas** (§5) and makes the **row** and **bridges** do the work.

### [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:12.1 - Type alignment: OWL class vs FCA concept (design‑time only)
...
### [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:12.2 - Role vs permission: SoD in enactment vs access control
...
### [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:12.3 - Method quartet: from MethodDescription to Work with observations
...
## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:13 - Reasoning primitives (judgement schemas)

> These are **mental checks** you can perform on any example page.

1. **Row validity**
   `cells(ρ) = {⟨Cᵢ,Sᵢ⟩} with |Contexts(ρ)| ≥ 2 ⊢ validRow(ρ)`
   *Reading:* A row is valid only if it spans at least two Contexts and all entries are legitimate **SenseCells** (from [[F.3 - Intra‑Context Sense Clustering|F.3]]).
...
9. **Harness ping mapping**
   ...
   *Reading:* Each named harness check ([[F.15 - SCR-RSCR Harness for Unification|F.15]]) has a clear reading in these judgements.

## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:14 - Acceptance tests (SCR/RSCR)

### [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:14.1 - Static conformance (SCR)
...
### [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:14.2 - Regression (RSCR)
...
* **RSCR‑F16‑E02 (Bridge re‑score).** If an upstream **CL** definition changes ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]), affected Bridges on the page show the new CL and, if needed, an updated **loss** sentence.
* **RSCR‑F16‑E03 (Row resilience).** If a SenseCell is split in [[F.3 - Intra‑Context Sense Clustering|F.3]] (sense refinement), the example either keeps the same row using one child sense, or splits into two rows with a short justification.
...
## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:15 - Migration notes (conceptual)
...
3. **Stabilise vocabulary.** If you find yourself rewriting terms mid‑page, you likely forgot a Context; return to [[F.1 - Domain‑Family Landscape Survey|F.1]] and add a Card.
...
6. **Keep the page living.** When upstream rows/bridges evolve ([[F.7 - Concept‑Set Table|F.7]]–[[F.9 - Alignment & Bridge across Contexts|F.9]]), adjust the page *minimally* and call out the change in a margin note (conceptual, not procedural).

## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:16 - Teaching variants (all obey §6 invariants)
...
## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:17 - Didactic checklist (author’s quick scan)
...
## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:18 - Closing distillation (30‑second echo)
...
## [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]:End
