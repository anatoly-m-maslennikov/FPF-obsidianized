...
> **Type:** Architectural (A) — [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]] specialisation (RPR)
> **Status:** Stable
> **Normativity:** Normative
> **Placement:** [[A.6 - Signature Stack & Boundary Discipline|A.6]] cluster; immediately after [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]]
> **Builds on:** [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]] (RPR); [[F.0.1 - Contextual Lexicon Principles|F.0.1]]:2.3 (Explicit Bridge Principle); [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] (Context discipline); E.10.U9 (Alignment/Bridge lexical discipline); [[F.9 - Alignment & Bridge across Contexts|F.9]] (Bridge discipline + reasoning primitives); F.7/F.8 (Concept‑Set rows & weakest‑link); [[F.5 - Naming Discipline for U.Types & Roles|F.5]] (labels); [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] (Strict Distinction: lanes + stance hygiene); [[E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]] (normative precision)
> **Coordinates with:** [[E.17 - Multi‑View Publication Kit|E.17]] (Viewpoints / Views / Correspondences, when the prose is really about views/projections); [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]] (KindBridge, when the claim is about kind/classification transfer); [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]] (Identification/indexing, when the umbrella is really about IDs); Concept‑Set row scope rules; [[E.10 - Unified Lexical Rules for FPF|E.10]] lexical SD (umbrella tokens); [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] penalty conversion (if used)
> **Delta‑Class:** Δ‑3 (new normative pattern; additive; does not change existing kernel semantics)
> **Impact radius:** any document, table row, or boundary statement that asserts cross‑context sameness/compatibility/alignment between SenseCells, or collapses **[[A.7 - Strict Distinction (Clarity Lattice)|A.7]] lanes** / `CHR:ReferencePlane`s under umbrella “same/equivalent/…” wording
> **Mint vs reuse:** reuses `Bridge`, `BridgeKind`, `dir`, `CL`, `Loss`, `scope`; adds [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]‑specific Bridge‑Card qualifiers (`Γ_time`, `facetSpan`) (annotation slots; do not alter the kernel Bridge predicate); does not mint new kernel relations
...
## [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:1 - Problem frame
...
[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] is the RPR specialisation that makes this structure explicit and prevents accidental “global identity” claims when the author’s intent is merely naming convenience or interpretive help.

## [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:2 - Problem
...
## [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:3 - Forces
...
## [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:4 - Solution

Treat every cross‑Context umbrella‑sameness statement as an **RPR trigger** that must be rewritten into an explicit **Bridge claim** ([[F.9 - Alignment & Bridge across Contexts|F.9]]) with declared attributes.

This specialisation follows the [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]] RPR envelope: it (i) defines a **trigger rule**, (ii) fixes the **stable lens** (Bridge Card), (iii) fixes a **minimal claim skeleton**, (iv) provides a **disambiguation guide**, and (v) standardises **change narration** for this class of ambiguity.

### [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:4.0 - Trigger rule (normative)

An occurrence SHALL be treated as an [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] trigger when **either** (i) `CtxA ≠ CtxB`, **or** (ii) the statement collapses **[[A.7 - Strict Distinction (Clarity Lattice)|A.7]] lanes** (`Object | Description | Carrier`) or `CHR:ReferencePlane`s under an umbrella sameness predicate, and the prose (or a table row comment) uses any of the following as if they were a single relation:
...
**Lane/plane‑only caveat.** If `CtxA = CtxB` and the trigger is solely a lane/plane collapse, repair lane/plane typing first ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]] / declared `Φ_plane`). You MAY satisfy this pattern by re‑typing endpoints + adding an explicit non‑licensing marker; do not invent a Bridge unless you actually need an auditable cross‑Context licence record.
...
* In this spec, **Context** means `U.BoundedContext` ([[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] / D.CTX).
* Use **lane** for the [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] split (**Object | Description | Carrier**).
* **CHR:ReferencePlane** is reserved for world/concept/episteme crossings; do **not** use it as a synonym for lane.

0. **Resolve endpoints as SenseCells (and pin editions where relevant).** If the prose wording uses pronominal/metonymic bundles (“the system”, “the model”, “it”, “this class”, “that table”, “the service”), treat this as an endpoint‑identity problem first: enumerate candidates and select the intended `σ@Ctx` endpoints (Candidate‑Set Note, [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]]:4.0b). Also check **lane** and **stance/time tags**: ensure each candidate sits on the intended [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] lane (**Object | Description | Carrier**) and record any time-stance tags on the relevant carriers or source publications (e.g., `DesignRunTag = design | run`) that affect substitution safety. Do not treat `DesignRunTag` as a separate Context; it is a time tag on carriers, source publications, or source epistemes as applicable. If the only crossing is design↔run, express it as an Interpretation Bridge (`kind=⇄ᴅʀ`, `scope=Explanation‑only`) unless you have a separately justified substitution Bridge within a fixed lane. If the triggering token is an identifier/key/code, repair it as a Carrier‑lane identification/indexing claim first ([[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]), and only then decide whether there is also a sense‑level Bridge claim. If the ambiguity is actually a **CHR:ReferencePlane** mix (e.g., “a database column” vs “a real‑world attribute”), treat that as a ReferencePlane issue: restate endpoints on a single `CHR:ReferencePlane`, or handle the crossing through a declared `Φ_plane` policy before attempting any substitution licence. In decision/publication lanes, endpoint ambiguity is fail‑closed: if the intended endpoints cannot be resolved from local cues and `witnessRefs`, keep the sentence as Plain explanatory prose (or an Explanation‑only Bridge) and do not use it to justify cross‑Context reuse, Concept‑Set rows, or substitution.
   * **Modularity note:** if the endpoint token itself is a known umbrella term (e.g., “service”), apply the relevant endpoint‑disambiguation RPR first (e.g., [[A.6.8 - Service Polysemy Unpacking (RPR‑SERV)|A.6.8]] for “service”), then return here for the cross‑context sameness predicate.
   * **View/projection note:** if the prose is primarily about **views/projections/correspondences** rather than sameness licences, coordinate with [[E.17 - Multi‑View Publication Kit|E.17]] (multi‑view describing). You may still need a Bridge for naming/substitution licences, but do not let “is a view of” silently become “is the same as”.
...
This is a pattern specialisation of [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]]: it provides the stable lens, claim skeleton, change‑class lexicon, and a disambiguation guide tailored to cross‑Context “sameness”.

### [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:4.1 - Stable lens

**Stable lens (QRR):** the **Bridge Card** ([[F.9 - Alignment & Bridge across Contexts|F.9]]) used as a qualified relation record for cross‑Context sameness claims.
...
**[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] qualifiers (pattern‑level; Bridge‑Card annotations).** [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] additionally requires:
...
**Kernel restraint reminder.** Bridges translate between local senses; they do **not** justify minting a new global `U.Type` by “sameness”. If the desired outcome is a new shared type or kind, apply the type‑minting discipline ([[A.11 - Ontological Parsimony (C‑5)|A.11]]) and keep Bridges as translators.
...
### [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:4.2 - Explicit claim skeleton

[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] fixes the minimal slot set that must be made explicit whenever a cross‑Context (or cross‑lane / cross‑plane) “same/equivalent/align/map/…” assertion appears.
| Slot                 |               Required | Meaning / constraints                                                                                                                  |
| -------------------- | ---------------------: | -------------------------------------------------------------------------------------------------------------------------------------- |
| `BridgeId`           |          Yes (if cited) | Required whenever the Bridge is referenced from multiple places, used to justify row scope, or used as a licence in decision/publication lanes. Inline cards MAY omit an id for a single‑use didactic gloss. **When present, the id is a registry reference** (per the [[F.9 - Alignment & Bridge across Contexts|F.9]] registry‑reference note): check existence / edition pinning, not signature export. |
| `σA@CtxA`, `σB@CtxB` |                    Yes | Endpoints are **SenseCells** (not strings, not “the systems”).                                                                         |
| `senseFamily`        |                    Yes | Use a named family ([[F.9 - Alignment & Bridge across Contexts|F.9]]). For substitution‑capable Bridges, this MUST be a single family (Role, Status, Measurement, Type-structure, ...). If the correspondence crosses families, use an **Interpretation** kind (`⇄ᴅʀ / →ᴍᴇᵃ / →ᴅᵉᵒ`) and record the channel explicitly (e.g., `Method ⇄ᴅʀ Execution`, `Measurement →ᴍᴇᵃ Requirement/Clause`, `Deontic →ᴅᵉᵒ Execution`), keeping `scope=Explanation‑only`. |
| `kind`               |                    Yes | One of the [[F.9 - Alignment & Bridge across Contexts|F.9]] kinds: `≈ / ⊑ / ⊒ / ⋂ / ⊥ / ⇄ᴅʀ / →ᴍᴇᵃ / →ᴅᵉᵒ`. Use `⊑/⊒` only for defensible inclusion. If you can name a counter‑case that violates inclusion for these endpoints, you do **not** have `⊑/⊒` — use `⋂` or refine endpoints (SenseCell split). |
| `dir`                |                    Yes | Always explicit ([[F.9 - Alignment & Bridge across Contexts|F.9]]). Use `A→B` for any **substitution** claim (Role Assignment & Enactment‑eligible), even when `kind=≈`. Use `A↔B` only to express a symmetric correspondence (or Type‑structure reuse); it does **not** imply bidirectional substitution. **No implicit inversion.** **Inclusion sanity:** when `kind∈{⊑,⊒}`, ensure `dir` matches the intended safe reading (substitution, when allowed, goes **from narrower to broader**); if needed, swap endpoints or declare the inverse Bridge explicitly rather than relying on prose. |
...
| `Loss`               |                    Yes | **Non‑empty Loss Notes** stating what fails to carry (units, scope, granularity, preconditions, stance). `Loss: none` is permitted **only** when `CL=3` and matched invariants are cited; for `kind=⊥`, use `Loss: n/a (incompatibility claim)` ([[F.9 - Alignment & Bridge across Contexts|F.9]]). |
...
| `scope`              |                    Yes | Allowed use ([[F.9 - Alignment & Bridge across Contexts|F.9]]): `Explanation‑only / Naming‑only / Role Assignment & Enactment‑eligible / Type‑structure`. This is a **maximum licence** for how the Bridge may be used in reasoning and tables. Do not confuse it with **Claim scope (G)** from USM ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]), and do not encode “sometimes substitution” by mixing scopes—narrow endpoints instead (see below). |
...
The following constraints are stated as *admissibility conditions* ([[E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]]): they define when a Bridge Card is well‑formed for a claimed licence.
...
* **INV‑XCTX‑KS‑2 (Disjoint embargo).** If `kind=⊥`, then `scope = Explanation‑only`, and the Bridge cannot support Concept‑Set rows or substitution ([[F.9 - Alignment & Bridge across Contexts|F.9]]:13.4).
* **INV‑XCTX‑KS‑3 (Interpretation embargo).** If `kind∈{⇄ᴅʀ, →ᴍᴇᵃ, →ᴅᵉᵒ}`, then `scope = Explanation‑only`, and the Bridge cannot support Concept‑Set rows or substitution ([[F.9 - Alignment & Bridge across Contexts|F.9]]:13.5).
* **INV‑XCTX‑KS‑4 (Role Assignment & Enactment substitution).** If `scope = Role Assignment & Enactment‑eligible`, then `kind∈{≈,⊑,⊒}`, `dir = A→B`, `CL≥2`, the Bridge is senseFamily‑preserving, endpoints are stance‑compatible, Loss notes are non‑empty, and a counter‑example is stated ([[F.9 - Alignment & Bridge across Contexts|F.9]]:13.2, [[F.9 - Alignment & Bridge across Contexts|F.9]]:13.8, [[F.9 - Alignment & Bridge across Contexts|F.9]]:16.1).
* **INV‑XCTX‑KS‑5 (Type‑structure reuse).** If `scope = Type‑structure`, then `senseFamily = Type‑structure`, `kind=≈`, `dir=A↔B`, `CL=3`, and matched invariants are stated (Type‑structure is only supported by near‑identity; see [[F.9 - Alignment & Bridge across Contexts|F.9]]:6.1 and [[F.9 - Alignment & Bridge across Contexts|F.9]]:16.1).
...
### [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:4.3 - Change‑class lexicon

[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] forbids “re‑align / re‑map / now equivalent” as a change description. Changes are narrated using the **[[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]] change classes**; the Bridge‑specific verbs below are narrative shorthands that map to [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]]:4.4 (`declareRelation`, `withdrawRelation`, `retargetParticipant`, `reviseByValue`, `rescope`, `retime`, `refreshWitnesses`).
Authors SHALL NOT use umbrella verbs (“re‑align”, “re‑map”, “now equivalent”, …) as change narration. Narrate changes using the change‑class lexicon below (mapped to [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]]:4.4).
...
4. `retime(BridgeId, Γ_time→Γ_time')` (maps to [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]] `retime(newΓ_time)`; semantic; edition‑fenced in decision/publication lanes)
5. `changeBridgeKind(BridgeId, kind→kind')` (maps to [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]] `changeRelationKind`)
...
9. `reviseFacetSpan(BridgeId, facetSpan→facetSpan')` (maps to [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]] `reviseByValue`; semantic; edition‑fenced in decision/publication lanes)
...
### [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:4.4 - Lexical guardrails and name selection

**Umbrella tokens (red‑flag triggers):** “same”, “identical”, “equivalent”, “align”, “map”, “match”, “correspond(s)”, and close variants.

These are only in‑scope here when used as **cross‑Context predicates** (`CtxA ≠ CtxB`) or when the prose collapses **[[A.7 - Strict Distinction (Clarity Lattice)|A.7]] lanes** / `CHR:ReferencePlane`s under an umbrella sameness predicate. For that case:
...
### [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:4.5 - RPR Disambiguation Guide (XCTX)
...
| “Same ID / same key / 1‑to‑1”      | Identification/indexing claim ([[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]) ± semantic Bridge                                              | Is the claim about **Carrier‑lane equality** (identifier scheme), or about **sense/meaning**? What is the reference scheme? Are collisions/aliases possible? | First: repair as an identification/indexing relation ([[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]]). Then (only if needed): declare a Bridge for meaning with explicit `kind/dir/CL/Loss/scope`. | [[A.6.6 - U.BaseDeclarationDiscipline - Kind-explicit, scoped, witnessed base declaration discipline (with base-change lexicon)|A.6.6]] (Carrier), E (reference scheme), [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] (meaning)     |
| “B is a view or projection of A”      | Explanation‑only or Naming‑only by default; substitution only after explicit guards/refined endpoints | Is this a `U.View` statement, a correspondence statement ([[E.17 - Multi‑View Publication Kit|E.17]]), or a reuse licence? Does projection drop constraints, fields, or stance? | `Bridge(σA,σB): kind=⊑ (if A is narrower), dir A→B (if substitution is intended), Loss states dropped structure/constraints, scope capped unless proven` | [[E.17 - Multi‑View Publication Kit|E.17]] (views), E (witness), A (if substitution proposed)   |
...
### [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:4.6 - Mapping witnesses are not Bridges (normative clarification)

Many projects use “map” to mean an implementation witness: a lookup table, aligner model, transformation function, or ETL step. [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] treats those implementation witnesses as **witnesses**, not as semantics. The Bridge is where you record:
...
### [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:4.7 - Coordination notes (keep [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] modular)

* **Views / projections / correspondences:** if the core intent is multi‑view description (“this diagram is a view of that system”, “these views correspond”), route the modelling discipline to **[[E.17 - Multi‑View Publication Kit|E.17]]** and keep [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] focused on preventing umbrella‑token licence smuggling. [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] may still be used to declare any *naming/substitution* licence between view elements, but it MUST NOT replace [[E.17 - Multi‑View Publication Kit|E.17]]’s correspondence discipline.
* **Kinds / classifications:** if the cross‑context claim is about **kind transfer** (“Class X in A is the same kind as Class Y in B” as a classification move), consider recording the classification channel using **[[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]] KindBridge**. Do not conflate Bridge‑CL with kind‑mapping CL^k.

## [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:5 - Archetypal Grounding

### [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:5.1 - System archetype: identity “sameness” across products
...
### [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:5.2 - Episteme archetype: schema/ontology alignment between knowledge graphs (class-level)
...
A reviewer proposes Type‑structure reuse (“treat them as the same class across graphs”). Under [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]], this triggers a required invariant check:
...
## [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:6 - Bias‑Annotation
...
Consequently, [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] may feel “heavy” in early drafts, but it prevents latent cross‑Context defects that are costly to discover later.

## [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:7 - Conformance Checklist

A document or boundary statement conforms to [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] iff:

* **CC‑[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]‑0 (UTS/LEX trigger coverage).** The local lexicon treats umbrella‑sameness tokens as RPR triggers and points authors to Bridge‑explicit rewrites.
* **CC‑[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]‑1 (No standalone umbrella predicate).** Cross‑Context umbrella tokens SHALL NOT be used as standalone cross‑Context predicates unless either:
...
* **CC‑[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]‑2 (SenseCell endpoints).** Every such claim names endpoints as `σ@Context` (edition‑pinned where relevant), not as strings or system names.
* **CC‑[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]‑3 (Direction explicitness).** `dir` is stated on every Bridge. If `kind` is non‑symmetric, any inverse use without redeclaration is non‑conformant.
* **CC‑[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]‑4 (Licence separation).** If the intent is explanation only, authors SHALL either (a) declare `scope = Explanation‑only` on a Bridge, or (b) use explicit non‑licensing prose (no Bridge licence). If the intent is naming compatibility, authors SHALL declare a Bridge with `scope = Naming‑only`. In all cases, the text SHALL NOT invite substitution unless a substitution‑eligible Bridge exists.
* **CC‑[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]‑5 (Substitution thresholds).** Any statement that implies substitution MUST be backed by a substitution‑eligible Bridge (`kind∈{≈,⊑,⊒}`, `CL≥2`, same `senseFamily`, stance‑compatible), with Loss notes and a counter‑example discipline.
* **CC‑[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]‑6 (Weakest‑link respect).** Any Concept‑Set row or composed claim that depends on multiple Bridges MUST bound its scope and `CL` by the weakest participating Bridge.
* **CC‑[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]‑7 (Loss visibility).** Loss notes are present and **non‑empty**. `Loss: none` is permitted only for `CL=3` with cited invariants; `Loss: n/a` is permitted for `kind=⊥`. Loss must be consistent with the allowed scope.
* **CC‑[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]‑8 (Change narration).** Changes to cross‑Context fit are narrated using the change‑class lexicon (declare/withdraw/adjustCL/rescope/…) rather than umbrella verbs.
* **CC‑[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]‑9 (Kind/scope admissibility).** Any Bridge used to justify cross‑Context sameness satisfies the admissibility constraints INV‑XCTX‑KS‑1 … INV‑XCTX‑KS‑5 (no overlap‑to‑substitution; no disjoint/interpretation rows; substitution is directional; Type‑structure only under `≈` + `CL=3` + invariants).
* **CC‑[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]‑10 (Registry reference hygiene).** If a BridgeId (or policy/edition id) is cited, it is treated as a **registry reference** (existence / edition pinning), not as a semantic symbol exported by signatures.
* **CC‑[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]‑11 (Edition basis).** In decision/publication lanes, any Bridge used to justify Naming‑only / substitution / Type‑structure SHALL state `Γ_time` (edition pins or “as‑of” basis). If `Γ_time` cannot be stated, the claim MUST remain Explanation‑only and MUST NOT justify rows or substitution.
* **CC‑[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]‑12 (Facet honesty).** If the correspondence holds only on a subset of facets, the author SHALL either (a) refine endpoints into the facet SenseCells (preferred) or (b) declare `facetSpan` explicitly, with `Loss` consistent with that facet span. Whole‑cell Bridges MUST NOT be used to smuggle facet‑only correspondences.

## [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:8 - Common Anti‑Patterns and How to Avoid Them
...
## [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:9 - Consequences
...
## [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:10 - Rationale
...
[[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] turns a dangerous linguistic convenience into an explicit, reviewable, evolvable claim.

## [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:11 - SoTA‑Echoing

(informative; post‑2015 alignment)

| SoTA practice                                                            | Primary source (post‑2015)                                              | What [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] echoes                                                   | What [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] adds                                                                                               | Stance                   |
| ------------------------------------------------------------------------ | ----------------------------------------------------------------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------ |
...
## [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:12 - Relations

* **Specialises:** [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]] (Relational Prose Repair) by fixing the relation skeleton for cross‑Context sameness claims.
* **Uses:** [[F.9 - Alignment & Bridge across Contexts|F.9]] Bridge discipline (Bridge Card, `BridgeKind`, `dir`, `CL`, Loss notes, scope licences, weakest‑link).
* **Coordinates with:** [[E.10 - Unified Lexical Rules for FPF|E.10]] lexical discipline (umbrella tokens) and [[F.5 - Naming Discipline for U.Types & Roles|F.5]] label discipline (Tech/Plain labels do not imply bridges).
* **Constrains:** Any cross‑Context Concept‑Set row scope claims via weakest‑link and substitution thresholds.

## [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]]:End
