...
terms:
  ...
  - "U.TransformerRole"
related:
  - "[[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]"
...
**[[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] compliance —** **Γ\_ctx** relies on **SerialStepOf/ParallelFactorOf** (order semantics); **Γ\_time** composes **PhaseOf** slices of the *same* carrier with coverage/no‑overlap; **PortionOf** is orthogonal (quantities within steps), mappings are not parthood.
...
## [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:1 - Problem frame

The universal algebra **Γ** ([[B.1 - Universal Algebra of Aggregation (Γ)|B.1]]) assumes local commutativity and locality for most structures. But many real‑world compositions are **not** order‑indifferent (recipes, proofs that unfold by steps, manufacturing routes), and many composites are **nothing but** a history (asset history, model revisions, experiment runs). For these cases FPF offers two universal flavours:

* **Γ\_ctx** — **procedural composition** (where SerialStepOf / ParallelFactorOf edges are present; see [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]] Γ_method for typing and joins; [[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] governs only mereological edges such as PortionOf/PhaseOf).
**Γ\_time** — *temporal* aggregation for **phase composition of the same carrier** (where `PhaseOf` edges from **[[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]** are present).

Both flavours **inherit WLNK and MONO** from the Quintet ([[B.1 - Universal Algebra of Aggregation (Γ)|B.1]]) and remain compatible with **[[A.12 - External Transformer & Reflexive Split|A.12]]** (Transformer Principle) and **[[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]** (Strict Distinction): they do *order* and *time*, not structure, mapping, or cost.

## [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:2 - Problem
...
## [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:3 - Forces
...
## [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:4 - Solution — **Part 1: What these flavours are, and when to use them**

### [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:4.1 - Two flavours at a glance (edge discipline)
...
> **Strict Distinction ([[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]) reminder.**
...
### [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:4.2 - Operator signatures (normative)
...
* **T:** the transformer that performs the material act of sequencing/combining steps ([[A.12 - External Transformer & Reflexive Split|A.12]]).
...
* **T:** the transformer that composes the timeline ([[A.12 - External Transformer & Reflexive Split|A.12]]).
* **Output H′:** the holon reconstructed over τ (system history, theory revision history, dataset growth, etc.).

### [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:4.3 - Adapted invariants (what replaces COMM/LOC)

Both flavours **keep** IDEM, WLNK, MONO from [[B.1 - Universal Algebra of Aggregation (Γ)|B.1]]. They **replace** COMM/LOC by discipline specific to order and time.
...
### [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:4.4 - Guards that make the folds provable

**For Γ\_ctx**

1. **Edge discipline:** only `SerialStepOf` / `ParallelFactorOf`.
2. **OrderSpec σ:** explicit partial order; joins must have well-typed inputs and outputs (see [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]] for join soundness).
...
### [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:4.5 - Selection checklist (didactic quick guide)
...
### [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:4.6 - Didactic contrasts (one‑liners)
...
## [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:5 - Proof Kit (ready‑to‑reuse obligations for Γ\_ctx / Γ\_time)

This Proof Kit instantiates the generic obligations from **[[B.1.1 - Dependency Graph & Proofs|B.1.1]] §6** for the order/time flavours. Attach these items whenever you apply Γ\_ctx or Γ\_time to a `DependencyGraph D`.

### [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:5.1 - Γ\_ctx obligations
...
### [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:5.2 - Γ\_time obligations

* **TIME‑CARR (Carrier Identity).**
  State explicitly the **carrier holon** whose history is being reconstructed.
  *Claim:* All `PhaseOf` arcs refer to the same carrier; if identity changes, model a Transformer producing a new holon ([[A.12 - External Transformer & Reflexive Split|A.12]]), not another phase.
...
## [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:6 - Archetypal grounding (worked micro‑examples)

Use these as templates; each fits on a page and references the obligations above.

### [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:6.1 - **Γ\_ctx — U.System (manufacturing route)**
...
### [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:6.2 - **Γ\_ctx — U.Episteme (order‑bound argument)**
...
### [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:6.3 - **Γ\_time — U.System (asset history)**
...
* **Routing:** Work/energy footprints per phase via **Γ\_work**; structural upgrades (new rotor) are Transformers ([[A.12 - External Transformer & Reflexive Split|A.12]]), not phases, if identity changes.

### [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:6.4 - **Γ\_time — U.Episteme (paper revisions)**
...
## [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:7 - Conformance Checklist (normative checklist)

| ID            | Requirement                                                                                                                                                                     | Purpose                                       |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------- |
| **CC‑B1.4.1** | **Γ\_ctx** input `D_ctx` SHALL use **only** `SerialStepOf` / `ParallelFactorOf` edges; **Γ\_time** input `D_time` SHALL use **only** `PhaseOf` edges.                           | Keep flavours matched to [[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] edges.          |
...
| **CC‑B1.4.7** | Structural inclusion, mappings, and resource spending SHALL NOT be encoded as order/time edges; route to **Γ\_sys and Γ\_epist**, value-level links, or **Γ\_work** respectively. | Enforce [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] Strict Distinction.              |
| **CC‑B1.4.8** | If coverage breaks or identity changes, the modeller SHALL refactor the graph or declare a **Meta‑Holon Transition** ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]).                                                     | Make emergence explicit.                      |

## [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:8 - Anti‑patterns and their fixes
...
| **History‑as‑structure**             | `v2 ComponentOf v1`                                         | Use `PhaseOf`; if identity actually changed, model a Transformer ([[A.12 - External Transformer & Reflexive Split|A.12]]) producing a new holon.                          |
...
## [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:9 - Consequences
...
## [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:10 - Rationale (informative)

This pattern implements **[[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]’s ordered relations** (`SerialStepOf`, `ParallelFactorOf`) and leverages **[[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]’s `PhaseOf`** for timeline; consistent with **Strict Distinction**: order and time are not structure, and costs are not history. The adapted invariants (NC‑1..3 and T‑1..3) give precise replacements for COMM/LOC where these do not hold, while retaining WLNK and MONO. The result is a small, stable interface that matches how engineers and researchers already argue about procedures and histories, without importing domain‑specific notations into the kernel.

## [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:11 - Relations
...
- [[C.27 - Temporal Claim Adequacy- State Readings, Temporal Trends, and Intervention-Sensitive Temporal Change|C.27]] may flag: an authored temporal claim that turns a temporal slice, phase name, aggregate membership, or temporal ordering into a rate-change adequacy claim.
...
- Exit: if only slice composition is live, stay in [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]; if rate-change adequacy changes admissible use, use [[C.27 - Temporal Claim Adequacy- State Readings, Temporal Trends, and Intervention-Sensitive Temporal Change|C.27]] for that claim and cite the governing FPF pattern for any law, work, causal, or benchmark question.

* **Builds on:** [[B.1 - Universal Algebra of Aggregation (Γ)|B.1]] (Universal Γ), [[B.1.1 - Dependency Graph & Proofs|B.1.1]] (Dependency Graph & Proofs), [[A.12 - External Transformer & Reflexive Split|A.12]] (Transformer), [[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] (Mereology Extension), [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] (Strict Distinction).
* **Specialises into:** **[[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]] Γ\_method** (adds duration, capability typing, join soundness rules).
* **Works alongside:** **[[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]] Γ\_work** (resource accounting per step/phase).
* **Triggers:** **[[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]] Meta‑Holon Transition (MHT): Recognizing Emergence and Re‑identifying Wholes** when re‑ordering or re‑phasing produces genuinely new properties.
* **Feeds:** **[[B.4 - Canonical Evolution Loop|B.4]] Canonical Evolution Loop** (time‑aware cycles that carry explicit costs and order).
...
## [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]]:End
