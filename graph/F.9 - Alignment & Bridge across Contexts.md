...
**Builds on:** [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] (Context discipline: Context = U.BoundedContext); **[[F.0.1 - Contextual Lexicon Principles|F.0.1]] (senseFamily & StatusModality guard; Bridge-only crossing)**; [[F.1 - Domain‑Family Landscape Survey|F.1]] (Contexts fixed); F.2/F.3 (Cells exist); [[F.7 - Concept‑Set Table|F.7]] (rows depend on Bridges); [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] (thresholds and reuse choice).

**Coordinates with:** [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] **Trust & Assurance Calculus** (uses CL penalties); **[[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] U.Mechanism** (Transport clause for cross-context use; penalties feed **R/R_eff** only; **F/G** invariant); Part C patterns (apply Bridges in formal claims); [[A.6.9 - U.CrossContextSamenessDisambiguation - Repairing cross-context “same - equivalent - align” via explicit Bridges (RPR‑XCTX)|A.6.9]] (RPR-XCTX for repairing umbrella “same/equivalent/align/map” prose into explicit Bridge Cards).
**Aliases (informative).** *Context-to-Context translator*; *Sense bridge*.

## [[F.9 - Alignment & Bridge across Contexts|F.9]]:1 - Intent & applicability
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:3 - Forces
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:4 - Core idea (didactic)
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:5 - Minimal vocabulary (this pattern only)

* **Context** — shorthand for **U.BoundedContext** (per [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]).
* **SenseCell** - the pair *(Context, Local-Sense)* from [[F.3 - Intra‑Context Sense Clustering|F.3]].
...
* **senseFamily** — the semantic category (Role, Status, Measurement, Type-structure, Method, Execution…) per [[F.0.1 - Contextual Lexicon Principles|F.0.1]] (normative Part F guard).

## [[F.9 - Alignment & Bridge across Contexts|F.9]]:6 - Bridge kinds (senseFamily-aware)
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:6.1 - Substitution Bridges (sense-preserving)
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:6.2 - Interpretation Bridges (cross-senseFamily, explanatory)
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:7 - CL scale and scope thresholds
...
* **Penalty use (informative):** [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] may convert **CL** into an assurance penalty when a cross-context claim is made.

## [[F.9 - Alignment & Bridge across Contexts|F.9]]:8 - The Bridge Card (compact sketch)
...
5. Ask whether asking, measuring, exporting, rendering, or bridging changes the represented state itself. If yes, coordinate with [[C.26.1 - Probe-Coupled Boundary Interaction|C.26.1]].
6. Ask whether coordinated work or live state is not exported faithfully enough for the intended use by any one report or bridge. If yes, coordinate with [[C.26.2 - Enacted Distributed State Evidence|C.26.2]].
7. Ask whether the crossing is a state representation with declared source-loss mode or reduced recoverability. If yes, coordinate with CSC/RT and the [[C.26 - Quantum-Like Modeling Lens|C.26]] coarsening support section.
...
- a [[C.26.1 - Probe-Coupled Boundary Interaction|C.26.1]] note when the export/probe changes represented state;
- a [[C.26.2 - Enacted Distributed State Evidence|C.26.2]] note when coordinated state has no faithful-enough export for the intended use;
- a CSC, RT, or [[C.26 - Quantum-Like Modeling Lens|C.26]] coarsening handoff when the exported representation intentionally carries reduced detail, reduced recoverability, or narrower use.

## [[F.9 - Alignment & Bridge across Contexts|F.9]]:9 - Invariants (normative)
...
9. **Kernel restraint.** Bridges **cannot** be used to promote ad-hoc sameness into a new **U.Type**; [[A.11 - Ontological Parsimony (C‑5)|A.11]] applies.
10. **Non-inheritance of Contexts.** Bridges **do not** imply -is-a- between Contexts ([[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]).
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:10 - Micro-examples (illustrative, one-liners)
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:11 - Anti-patterns & remedies
...
| **AP-9** | **Bridge as new U.Type** | Using a Bridge to justify minting a new universal Type. | Re-globalises meaning; breaks [[A.11 - Ontological Parsimony (C‑5)|A.11]] parsimony. | Keep Types context-local; where reuse is needed, use **rows** + Bridges, not new primitives. |
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:12 - Worked examples (didactic)

### [[F.9 - Alignment & Bridge across Contexts|F.9]]:12.1 - Service acceptance (design) vs executions & observations (run)
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:12.2 - Behavioural role vs access role
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:12.3 - Equivalence of subtype notions for structural rows
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:12.4 - Accuracy (metrology) vs accuracy (data-quality)
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:12.5 - Setpoint (control) vs target (service)
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:12.6 - Role substitution & CL gating (RoleAssignment/enactment scope)
...
#### [[F.9 - Alignment & Bridge across Contexts|F.9]]:12.6.1 - Directional substitution (role-oriented shorthand)
...
* The reverse direction **does not** follow unless declared ([[F.9 - Alignment & Bridge across Contexts|F.9]]:13.7).

#### [[F.9 - Alignment & Bridge across Contexts|F.9]]:12.6.2 - CL > gating policy (didactic default)
...
*Notes.* The **substitution scope** is defined in **[[F.9 - Alignment & Bridge across Contexts|F.9]]:13.2-13.3** (Role-Assignment/Enactment-eligible substitution requires **CL >= 2**; Naming-only is **CL >= 1**).
CL penalties feed assurance (R) per **[[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]**; safety-critical policies may require **CL >= 2** by default (D.2).

#### [[F.9 - Alignment & Bridge across Contexts|F.9]]:12.6.3 - Typical bridges (worked patterns)
...
#### [[F.9 - Alignment & Bridge across Contexts|F.9]]:12.6.4 - Bridge invariants (role-relevant reminders)
...
* **No silent inversion.** Direction is explicit; substitution does not reverse unless declared ([[F.9 - Alignment & Bridge across Contexts|F.9]]:13.7).

## [[F.9 - Alignment & Bridge across Contexts|F.9]]:13 - Reasoning primitives (judgement schemas)
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:13.1 - Bridge declaration
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:13.2 - Substitution scope (senseFamily-preserving)
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:13.3 - Naming-only scope
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:13.4 - Prohibition by kind
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:13.5 - Interpretation embargo
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:13.6 - Weakest-link rule for rows
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:13.7 - Direction guard
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:13.8 - SenseFamily purity
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:13.9 - Loss accumulation
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:14 - Relations

**Builds on:** [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] (Context discipline: Context = U.BoundedContext); **[[F.0.1 - Contextual Lexicon Principles|F.0.1]] (senseFamily guard; Bridge-only crossing)**; [[F.1 - Domain‑Family Landscape Survey|F.1]] (Contexts fixed); F.2/F.3 (Cells exist); [[F.7 - Concept‑Set Table|F.7]] (rows depend on Bridges); [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] (thresholds and reuse choice).

**Coordinates with:** [[F.9.1 - Bridge Stance Overlay|F.9.1]] for stance overlays that remain subordinate to bridge cards; [[E.17.1 - U.ViewpointBundleLibrary - Reusable Viewpoint Bundles|E.17.1]] when viewpoint bundles need explicit cross-family correspondence; [[C.16.Q - Quality-Term Precision Restoration|C.16.Q]] / [[C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]] when evaluative endpoints or bundle-shaped quality families cite bridge cards without absorbing bridge semantics.

**Constrains:**

* **[[F.7 - Concept‑Set Table|F.7]] Concept-Set Table:** each cross-context row must name supporting **Bridges**; row scope <= the weakest supporting Bridge.
* **[[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] Mint or Reuse:** reuse choices reference **CL** and **kind**; no reuse without a Bridge.
* **Part C patterns:** formal claims that span Contexts cite Bridges and respect senseFamily/StatusModality & CL constraints.
* **[[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] Trust & Assurance Calculus:** may interpret **CL** as a penalty factor in Cross-context reasoning.

## [[F.9 - Alignment & Bridge across Contexts|F.9]]:15 - Migration notes (conceptual)
...
4. **senseFamily refactor.** If a Cell’s senseFamily was mis-typed, fix the senseFamily first in [[F.3 - Intra‑Context Sense Clustering|F.3]], then revisit Bridges; **Interpretation** is safer than forced substitution.
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:16 - Acceptance tests (SCR/RSCR — concept-level)

### [[F.9 - Alignment & Bridge across Contexts|F.9]]:16.1 - Static conformance (SCR)

* **SCR-F9-S01 (Well-typed).** Every Bridge names **two SenseCells**, each bound to a **Context** from [[F.1 - Domain‑Family Landscape Survey|F.1]], and states **senseFamily**, **kind**, **dir** (if needed), **CL**, **Loss**, and **scope**.
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:16.2 - Regression (RSCR)
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:17 - Didactic distillation (90-second script)
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:17.1 - Bridge stance overlay compatibility
A bridge card may carry a [[F.9.1 - Bridge Stance Overlay|F.9.1]] Bridge Stance Overlay such as `localRename`, `operationalizes`, `partialAnalogy`, `projection`, or `nonEquivalent`. The overlay is a local interpretive annotation and **does not replace** the underlying bridge kind, direction, `CL`, or loss notes.

## [[F.9 - Alignment & Bridge across Contexts|F.9]]:18 - Archetypal Grounding

### [[F.9 - Alignment & Bridge across Contexts|F.9]]:18.1 - Tell
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:18.2 - Show (System lane)

A service team may reuse the word *availability* across monitoring, SLO review, and architecture discussion. [[F.9 - Alignment & Bridge across Contexts|F.9]] requires the team to publish Bridge Cards that separate observation, status target, and architectural concern rather than treating the shared label as silent sameness. The benefit is that naming convenience survives while substitution rights stay bounded by `senseFamily`, `CL`, and Loss Notes.

### [[F.9 - Alignment & Bridge across Contexts|F.9]]:18.3 - Show (Episteme lane)

A comparative bundle may say that two traditions both discuss *readiness* or *capability*. Under [[F.9 - Alignment & Bridge across Contexts|F.9]], that statement is only explanatory until the author publishes the two SenseCells, the Bridge kind, direction, `CL`, and the counter-example that marks where the comparison stops. The Bridge then becomes an auditable correspondence rather than a rhetorical shortcut.

### [[F.9 - Alignment & Bridge across Contexts|F.9]]:18.4 - Coarsened cross-context note is not yet a Bridge Card

A service or review bundle may circulate a short cross-context note such as `the vendor bulletin is basically the same readiness signal as our rollback worksheet`. That note may be useful as informal orientation talk, but it is not yet an admissible Bridge Card and not yet a formal [[F.9 - Alignment & Bridge across Contexts|F.9]] `Naming-only` row.
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:19 - Bias-Annotation

Lenses tested: **Gov**, **Arch**, **Onto/Epist**, **Prag**, **Did**. Scope: **Universal** for cross-context correspondence and reuse.

* **Gov bias:** [[F.9 - Alignment & Bridge across Contexts|F.9]] raises the declaration bar by requiring explicit Bridge Cards.
...
* **Onto/Epist bias:** [[F.9 - Alignment & Bridge across Contexts|F.9]] is local-first and resists global meaning claims.
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:20 - Conformance Checklist (CC-[[F.9 - Alignment & Bridge across Contexts|F.9]])

A Bridge publication conforms to [[F.9 - Alignment & Bridge across Contexts|F.9]] iff:
...
6. **CC-F.9-6 - Overlay non-collapse.**
   If a [[F.9.1 - Bridge Stance Overlay|F.9.1]] Bridge Stance Overlay is used, it remains an annotation and does not replace bridge kind, direction, `CL`, or Loss Notes.
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:21 - Consequences

**Benefits.**
[[F.9 - Alignment & Bridge across Contexts|F.9]] lets FPF compare, translate, and partially reuse ideas across Contexts without collapsing them into one vocabulary. It gives downstream rows, claims, and assurance reasoning an explicit Bridge Card record instead of relying on prose intuition.
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:22 - Rationale

The core move of [[F.9 - Alignment & Bridge across Contexts|F.9]] is simple: cross-context work is unavoidable, but silent sameness is unacceptable. A Bridge therefore does two jobs at once:
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:23 - SoTA-Echoing

**SoTA note.** This section does not mint an independent second bridge rule track. It stays truthful only when Bridge kinds, `CL`, Loss Notes, weakest-link scope, the [[A.6.3.CSC - Controlled Semantic Coarsening|A.6.3.CSC]] neighbor boundary, and the review matrix below still tell the same story about admissible cross-context reading.

| Claim need | SoTA practice (post-2015) | Primary source (post-2015) | Alignment with [[F.9 - Alignment & Bridge across Contexts|F.9]] | Adoption status |
| --- | --- | --- | --- | --- |
| Shared labels across contexts are not enough for supported cross-context reuse. | Terminology and ontology practice distinguishes objects, concepts, definitions, designations, and typed relations instead of treating the same string as identity. | ISO 704:2022; ISO 1087:2019; ISO/IEC 21838-2:2021 (BFO). | [[F.9 - Alignment & Bridge across Contexts|F.9]] requires typed SenseCells, bridge kind, direction where needed, `CL`, and Loss Notes rather than string-equals identity. | **Adopt/Adapt.** Adopt explicit term/concept/relation discipline; adapt it into Bridge Cards; reject lexical sameness as reuse support. |
| Viewpoint and context boundaries must stay explicit when descriptions are reused. | Architecture-description practice distinguishes an entity of interest, architecture description, viewpoint, view, model kind, concern, and correspondence. | ISO/IEC/IEEE 42010:2022. | [[F.9 - Alignment & Bridge across Contexts|F.9]] binds every Bridge to declared Contexts and forces downstream rows to obey weakest-link scope instead of outrunning the supporting correspondences. | **Adopt.** Adopt boundary-explicit architecture-description discipline and apply it to FPF cross-context bridge cards. |
| Data/catalog/validation practice separates metadata, validation conditions, and exchange support from substitution authority. | Web-data and semantic-web standards make metadata, provenance, structural constraints, validation, and catalog federation explicit without turning metadata into the data itself. | W3C Data on the Web Best Practices (2017); W3C SHACL (2017); W3C DCAT v3 (2024). | [[F.9 - Alignment & Bridge across Contexts|F.9]] separates explanatory/interpretive bridges from substitution bridges and keeps bridge publication distinct from coarsened notes or catalog-style discovery aids. | **Adapt/Reject.** Adapt explicit metadata and validation practice; reject treating discovery, gloss, or validation support as substitution support. |
| Model-based engineering uses traceable model elements and formal semantics, but tool interoperability is not itself semantic identity. | Current MBSE practice improves precision, traceability, and interoperability through explicit model elements, libraries, APIs, and formal semantics. | OMG SysML v2.0 Language Specification (2025); OMG KerML v1.0 Specification (2025). | [[F.9 - Alignment & Bridge across Contexts|F.9]] uses Bridge Cards as human-readable, reviewable relations whose `CL` and loss fields remain narrower than and do not replace any hidden tool or model interchange claim. | **Adapt.** Adopt traceable relation discipline; reject tool or interchange success as proof of same meaning. |
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:24 - Bridge Card Publication Discipline

### [[F.9 - Alignment & Bridge across Contexts|F.9]]:24.1 - Minimal bridge-card declaration
...
If any of these fields is absent, later readers are forced back into inference by prose similarity, which is exactly what [[F.9 - Alignment & Bridge across Contexts|F.9]] is supposed to block.

### [[F.9 - Alignment & Bridge across Contexts|F.9]]:24.2 - One-pair default rule
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:24.3 - Revision over silent drift
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:25 - Bundle and Endpoint Interaction Law

### [[F.9 - Alignment & Bridge across Contexts|F.9]]:25.1 - Viewpoint and bundle interaction
Viewpoint bundles, quality bundles, and other endpoint bundles may cite Bridges, but they do not absorb bridge semantics. [[F.9 - Alignment & Bridge across Contexts|F.9]] remains the pattern for cross-context alignment, while the citing bundle keeps its own ontology.

### [[F.9 - Alignment & Bridge across Contexts|F.9]]:25.2 - Quality-family interaction
...
### [[F.9 - Alignment & Bridge across Contexts|F.9]]:25.3 - Overlay interaction rule
A [[F.9.1 - Bridge Stance Overlay|F.9.1]] stance overlay may help readers interpret a bridge, but the bridge card remains primary. If the overlay overstates the bridge kind, direction, `CL`, or Loss Notes, the card wins and the overlay should be narrowed or removed.

## [[F.9 - Alignment & Bridge across Contexts|F.9]]:26 - Review Matrix and Migration Tests
...
## [[F.9 - Alignment & Bridge across Contexts|F.9]]:14a - [[C.29 - Mathematical Lens Use|C.29]] mathematical-lens use relation

> When meaning, substitution, sense cells, direction, `CL`, or Bridge-supported use crosses context, write the [[F.9 - Alignment & Bridge across Contexts|F.9]] Bridge Card first. Add the applicable [[C.29 - Mathematical Lens Use|C.29]] output only for mathematical-lens use: candidate mathematical object, `LensMappingMode`, preserved and lost structure, exposed invariants or distinctions, lens-use admissibility value, admissible and non-admissible use, and stop condition. Do not duplicate Bridge semantics inside MathLensUse. A Bridge may make a mathematical lens interpretable across contexts without making it substitution-safe.

## [[F.9 - Alignment & Bridge across Contexts|F.9]]:End
