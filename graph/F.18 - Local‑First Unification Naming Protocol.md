...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:0 - Use this when

Use [[F.18 - Local‑First Unification Naming Protocol|F.18]] when a name must become stable and reusable across one local context, a concept set, a bridge, or a durable FPF/publication vocabulary. Use it when the issue is not merely one overloaded local phrase, but a name that must carry context, kind, local sense, use-domain, bridge relation or [[F.9.1 - Bridge Stance Overlay|F.9.1]] bridge stance when live, and lineage without smuggling global sameness.

**First useful move.** Ask whether the live work is durable naming. If yes, make the name local-first by naming its context, kind, purpose/use-domain, local sense, and bridge relation or [[F.9.1 - Bridge Stance Overlay|F.9.1]] bridge stance when live. If no, use the lighter governing pattern: local phrase repair under [[E.10 - Unified Lexical Rules for FPF|E.10]] or [[C.2.P - Epistemic Precision Restoration|C.2.P]], relation precision under [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]], or the domain pattern that governs the named object.
...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:1 - Context

Names must carry enough signal for everyday use, yet never smuggle in Cross‑context identities, hidden assumptions, or role/metric clutter. [[F.18 - Local‑First Unification Naming Protocol|F.18]] supplies that naming discipline and weaves it through [[F.1 - Domain‑Family Landscape Survey|F.1]]–[[F.17 - Unified Term Sheet (UTS)|F.17]]: Term Harvesting, Sense Clustering, Role Descriptions, Concept‑Sets, Bridges, Lexical Continuity, Anti‑Explosion control, and the Unified Term Sheet (UTS).
...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:2 - Problem
...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:3 - Forces
...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:4 - Solution — The Local‑First Naming Protocol

[[F.18 - Local‑First Unification Naming Protocol|F.18]] defines **eight rules** (R‑rules) and **six practices** (P‑practices). Together they produce **Name Cards** that any reader can interpret **ontologically** without guessing, and that slot cleanly into the rest of Part F.

**Path Card (subset of Name Card).** A **Name Card** whose named FPF value is an **EvidenceGraph Path**: it cites a **PathId** (or **PathSliceId**), **Context**, **ReferencePlane**, **Γ_time**, and any **Bridge id(s) + CL/CL^plane** (with loss notes). Used by **[[G.6 - Evidence Graph & Provenance Ledger|G.6]]** and **[[G.10 - SoTA Pack Shipping|G.10]]** to make justifications portable on UTS.

### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:4.1 - The Eight R‑rules (normative)
...
A minted name must resolve to a Local-Sense inside its Context (the result of [[F.2 - Term Harvesting & Normalisation|F.2]]–[[F.3 - Intra‑Context Sense Clustering|F.3]]). If a name points to a Role Description, state that template and its sense basis. Avoid heavily overloaded label words: when needed, prefer concise two-word Tech labels that hint at the intended sense.

**R5 — Use *Twin Registers* (Unified Tech + Plain).**
Provide two human‑oriented labels on the Name Card, per **[[E.10 - Unified Lexical Rules for FPF|E.10]]** register discipline:
...
### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:4.2 - The Six P‑practices (normative process)
...
When P1 uses **NQD-CAL ([[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]])**, treat the **Quality vector** over candidates as part of the same archive as [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]’s **NQD-frontier**. Recommended components (all **ordinal; no arithmetic means**):
...
* **AliasRisk (A — Lexical overload).**
  ...
  *Scale (ordinal; ↓ better).* `{Safe, Context‑dependent, High‑Risk, Overloaded}` with `Safe ≻ Context‑dependent ≻ High‑Risk ≻ Overloaded`. Avoid adopting **Overloaded** labels unless required by legacy and called out explicitly in notes. When [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]’s **DomainDiversitySignature** is available, AliasRisk MAY be refined into a CHR‑typed characteristic with the same polarity.

Use these components for **Pareto comparison only** (per **[[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]]** ordinal discipline). Do **not** collapse them into a single scalar score; the NQD-front is computed over the **vector of lexical Q-components** together with **Novelty** and **Diversity_P**.
...
**P3 — Mint‑or‑Reuse gate ([[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]).**
...
**P4 — Concept‑Set linkage ([[F.7 - Concept‑Set Table|F.7]]).**
...
**P5 — UTS registration ([[F.17 - Unified Term Sheet (UTS)|F.17]]).**
...
**P6 — Lexical-continuity hygiene ([[F.13 - Lexical Continuity & Deprecation|F.13]]).**
...
### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:4.3 - Guarded-head note for locally risky labels
...
This is especially useful when one head already appears in more than one admissible local reading, such as `projection` in [[A.16 - Language-State Move Coordination|A.16]] move language and in [[F.9.1 - Bridge Stance Overlay|F.9.1]] bridge stance language. The note should therefore name the consumer sites, keep each local reading explicit, and resist any temptation to flatten them into one global head.

### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:4.3a - Durable-head settlement for entry/discoverability vocabulary

When naming work touches the discoverability amendment, `PCP-TERM` plus [[F.18 - Local‑First Unification Naming Protocol|F.18]]
and [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]] must settle the durable heads used here, including
...
| --- | --- | --- | --- |
| `pattern-entry discoverability` | composite entry quality over one entry-recognition stack | [[E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline|E.11]] | broad `discoverability` alone |
| `description recognition signature` | first-contact cue structure of one description-bearing unit | [[A.6.RSIG - Recognition Signatures for Descriptions|A.6.RSIG]] | `discoverability of descriptions` |
| `recognition text` | existing first reading text inside one pattern | [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] | invented `discoverability surface` |
| `FirstEntryPatternComparisonSet` | case-relative [[E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline|E.11]] entry-distribution grouping of plausible candidate patterns, tempting wrong patterns, entry-load reclassifications, and admissible entry stops | [[E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline|E.11]]; use [[I.2 - Expanded Entry Disambiguation Cases|I.2]] for expanded entry-disambiguation cases when compact cues are insufficient | `route`, `semanticArea` as navigation label, `ontologicalNeighborhood` as navigation label |
| `entry lexeme retrieval aid` | lexical and query retrieval aid without alias minting | [[F.17 - Unified Term Sheet (UTS)|F.17]], [[F.18 - Local‑First Unification Naming Protocol|F.18]], and [[E.10 - Unified Lexical Rules for FPF|E.10]], coordinated by [[E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline|E.11]] | `lexical discoverability`, bare search-help wording |
| `ExpandedEntryDisambiguationCase` | bounded case that expands compact first-entry comparison when wrong-pattern risk, repeated failure, retrieval-facing use, or compact guidance insufficiency is live | [[I.2 - Expanded Entry Disambiguation Cases|I.2]] | `workflow`, `scenario script`, `route`, untyped interpretation label |
| `thin echo` | low-detail projection pointer to the cited pattern, cited pattern section, field, relation, or section of a named non-pattern FPF publication form; the pointer makes the publication form, companion function, and reference recoverable, and the referenced section carries the claim | [[E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline|E.11]] | duplicated guidance, parallel blurbs |
...
### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:4.3b - EntityOfConcern family naming settlement

When naming work touches the [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] EntityOfConcern family, use these rows as the local-first settlement. They are Name Card settlements for field and head names; they do not create a new `U.*` kind, relation kind, evidence kind, publication kind, or universal entity ontology.
...
| Name or source wording | [[F.18 - Local‑First Unification Naming Protocol|F.18]] settlement | Must not mean |
| --- | --- | --- |
...
| `EntityOfConcernSlot` | Entity-valued [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] slot. Relation, claim-record, and kind named by value/reference cases enter only through the declared relation/claim/reference discriminator and the governing pattern. | A relation-valued bucket, a generic claim-use bucket, or a way to avoid the relation named by value, evidence, gate, work, decision, assurance, or architecture pattern. |
| `entityOfConcernRef`, `EntityOfConcernRef` | Reference handle for the EntityOfConcern under the slot/ref discipline of [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]. Same-EntityOfConcern cases preserve this reference; retargeting changes it only through [[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] and its invariant/loss boundary. | A free publication-unit field, title, topic, carrier reference, authoring-work reference, or review-object reference. |
...
| `publicationUnitPrimaryEntityOfConcern` | Technical head for the primary entity of concern, non-claim-bearing kind named by value, topic, or subject that one bounded `PublicationUnit` is mainly about while carrying one move and one outside-work boundary. When a claim-bearing episteme or episteme-lane `U.View` is live, recover it through `EntityOfConcernRef`. | A [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] slot by itself, a second EntityOfConcern ontology, a carrier identity, a title, or project-side authority by readable form. |
| `EntityOfInterest`, `EoIClass`, `DescribedEntity*`, `describedEntityRef`, `primary described entity` | Use `EntityOfConcern`, `EntityOfConcernRef`, `EntityOfConcernClass`, `publicationUnitPrimaryEntityOfConcern`, or the local FPF kind named by value. | Live technical vocabulary, second [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]] slot family, alias permission, or permission to keep old semantics beside the selected family. |
| `EntityOfConcernAlignmentCase` | Local table head for the same-by-value, retarget, bridge, relation-record, distinct-entity, and blocked-use split. | A new kernel kind or a substitute for [[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]], F.9/F.17/F.18, [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]], or the pattern governing the relation kind. |

### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:5.1 - Card purpose & mode guard (normative)
...
* **MintNew.** The Card is the **output of an NQD-style lexical search** over a **candidate label set** generated inside
  ...
  AliasRisk}` together with **Novelty (N)** and **Diversity_P** (per [[A.0 - Onboarding Glossary (NQD & E-E‑LOG)|A.0]] / [[C.17 - Characterising Generative Novelty & Value (Creativity‑CHR)|C.17]]–[[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]] / [[B.5.2.1 - Creative Abduction with NQD|B.5.2.1]]).
...
2. **Twin labels** — **Unified Tech** and **Plain** (per [[E.10 - Unified Lexical Rules for FPF|E.10]]).
...
8. **Sense reference** — a Local‑Sense reference (how [[F.2 - Term Harvesting & Normalisation|F.2]]–[[F.3 - Intra‑Context Sense Clustering|F.3]] clustered it).
...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:6 - What *belongs* in the label—and what does not
...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:7 - Worked triad (three short, context‑local examples)
...
### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:7.1 - Industrial operations Context: “Thermal Management - 2026”
...
### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:7.2 - Clinical Context: “Hospital OR - 2026”
...
### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:7.3 - Public service Context: “Civic Services - 2026”
...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:8 - Conformance Checklist (editor aid) — *Part I: naming & cards* (**non‑normative**)
...
Each Name Card carries a **Unified Tech** and a **Plain** label ([[E.10 - Unified Lexical Rules for FPF|E.10]]). Tech appears in **Core** prose; Plain in teaching/examples.
...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:9 - Normative Standard (what must be true)
...
If the public name expresses a **structural relation**, its intended sense **MUST** be backed by *exactly one Constructive trace* in the structural calculus (Compose-CAL) and **SHALL** declare `validationMode=axiomatic` (see [[E.14 - Human‑Centric Working‑Model|E.14]]). If the name expresses an **epistemic relation**, Constructive backing is optional; **declare** `validationMode ∈ {inferential, postulate}` and use **Logical/Mapping** and/or **Empirical Validation** as appropriate. **Do not mix relation kinds** inside a single name. *(Do not use “Tier-1/2”; formality is expressed via F per [[C.2.3 - Unified Formality Characteristic F|C.2.3]].)*
...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:10 - Rationale (why this exists and why these rules)
...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:12 - Examples (worked mini‑cases for engineer‑managers)
...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:13 - Anti‑Patterns & Failure Modes (what to avoid)
...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:14 - Assurance & Conformance (human‑only checks)

### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:14.1 - Author checklist (before requesting review).
...
* [ ] I produced both **Unified Tech** and **Plain** labels (per [[E.10 - Unified Lexical Rules for FPF|E.10]]), with the same sense and pointing to the same **Row ID**.
...
### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:14.2 - Reviewer checklist (at the gate).**
...
### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:14.3 - Lightweight outcomes.**
...
**CC‑F18‑1 (Context first).** Every unified name **SHALL** be grounded in **local senses** that were harvested and clustered **inside named Contexts** (“context of meaning”) prior to unification. No name may be minted from free‑floating glosses or Cross‑context intuition. Use [[F.1 - Domain‑Family Landscape Survey|F.1]]–[[F.3 - Intra‑Context Sense Clustering|F.3]] first.

**CC‑F18‑2 (Bridge‑only sameness).** Claims of sameness across Contexts **SHALL** be expressed only via explicit **Bridges** with a stated congruence and loss note. Mere spelling similarity never licenses a unified name. Use [[F.9 - Alignment & Bridge across Contexts|F.9]].

**CC‑F18‑3 (Twin‑register naming).** Each unified concept **SHALL** carry **two labels**—a **Unified Tech** label (used in Core prose) and a **Plain** label (used for teaching). Labels are chosen per the naming discipline of [[F.5 - Naming Discipline for U.Types & Roles|F.5]] and the register rules of **[[E.10 - Unified Lexical Rules for FPF|E.10]]**.

**CC‑F18‑4 (Neutrality of the Tech label).** The **Unified Tech** label **SHALL NOT** be borrowed wholesale from any single source Context where that borrowing would re‑import that Context’s private distinctions. Prefer a **neutral** term; **Cross‑context reuse occurs only via the UTS row and explicit Bridges**. (See [[F.5 - Naming Discipline for U.Types & Roles|F.5]] “allowed/forbidden forms”.)

**CC‑F18‑5 (One concept per row, one Tech label per row).** A unified concept **SHALL** be captured as **one Concept‑Set row** in the **UTS**. Exactly **one Unified Tech label** is normative for that row; additional deprecated strings are aliases in Annex (budgeted). Use [[F.7 - Concept‑Set Table|F.7]] with [[F.17 - Unified Term Sheet (UTS)|F.17]].

**CC‑F18‑6 (SenseCell citations).** For each unified name, the **SenseCells** (Context × Local‑Sense) that justify it **SHALL** be cited in the UTS row with **Context name + edition**. Omit edition only if the Context has a single stable edition. See [[F.17 - Unified Term Sheet (UTS)|F.17]] §6.

**CC‑F18‑7 (Sheet‑level coverage).** Within a thread’s UTS, the **set of rows** carrying unified names **SHALL** collectively cite **≥ 3 distinct Contexts**, ensuring breadth of evidence. Coverage is a property of the **sheet**, not of every single row. See [[F.17 - Unified Term Sheet (UTS)|F.17]] §6 constraint.

**CC‑F18‑8 (No global words).** In Core text, **“Context” always means `U.BoundedContext`**; **discipline columns** may be used in teaching layouts but **is not** a bearer of meaning. Do not write context‑free claims of sameness. See [[E.10 - Unified Lexical Rules for FPF|E.10]] and [[F.17 - Unified Term Sheet (UTS)|F.17]] §5.

**CC‑F18‑9 (Didactic primacy).** A unified name **SHALL** be teachable on **one page**: its **UTS row** + a short narrative that a careful reader can replay ([[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]] template). If it cannot be taught concisely, the naming attempt is premature.
...
**CC‑F18‑11 (Strict distinction guard).** Names **SHALL** respect **[[A.7 - Strict Distinction (Clarity Lattice)|A.7]] Strict Distinction**: do not collapse **Role ↔ Method ↔ Work** or **Status ↔ Description** into one word. Align with F.11/F.12 where relevant.

**CC‑F18‑12 (Change control via [[F.13 - Lexical Continuity & Deprecation|F.13]]).** Renames, splits, merges, and retirements **SHALL** follow [[F.13 - Lexical Continuity & Deprecation|F.13]]’s lexical continuity rules; the UTS remains the canonical public term sheet for these changes.

**CC-F18-13 (Lexical Pareto discipline).** When a Name Card uses **NQD-CAL ([[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]])** to score label candidates, the **chosen Unified Tech label** **SHALL** lie on the **Pareto frontier** of the lexical Q-tuple `{SemanticFidelity, CognitiveErgonomics, MorphologicalActionFit, AliasRisk}` (per **[[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]]** ordinal discipline and P1’s NQD-front definition), unless an explicit exception is recorded. If authors deliberately select a dominated candidate (e.g., to honour legacy regulation or user muscle memory), the Name Card’s notes **MUST** state the reason for stepping off the frontier.
...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:16 - Anti‑patterns & safe rewrites (normative)
...
| -- | ------------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| A1 | “These terms are the same **because they look alike**.” | Cross‑context **spelling** is not evidence of conceptual identity. | “We assert sameness **via a Bridge**; the loss note explains what survives across Contexts.” ([[F.9 - Alignment & Bridge across Contexts|F.9]])                     |
| A2 | “Use the BPMN word as our Tech label.”                  | Imports BPMN’s local commitments into the unified term.         | “Choose a **neutral** Unified Tech label; cite BPMN as a **SenseCell** in the row.” ([[F.5 - Naming Discipline for U.Types & Roles|F.5]], [[F.17 - Unified Term Sheet (UTS)|F.17]] §6)                  |
...
| A5 | “Contextless glossary.”                                 | Violates local‑first; readers re‑globalise.                     | “Publish a **UTS** per thread; each row lists **Contexts** (Contexts+editions).” ([[F.17 - Unified Term Sheet (UTS)|F.17]])                                |
| A6 | “We’ll fix synonyms later.”                             | Synonym sprawl grows costs.                                     | “Apply **[[E.10 - Unified Lexical Rules for FPF|E.10]]** rules now: one Tech, one Plain; retire extras via **[[F.13 - Lexical Continuity & Deprecation|F.13]]**.”                                        |
| A7 | “One mega‑row for everything service‑like.”             | Bundles distinct concepts; harms teachability.                  | “One **Concept‑Set per idea**; group with a **Block Plan** for pedagogy.” ([[F.17 - Unified Term Sheet (UTS)|F.17]] §7)                                 |

### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:16.1 - Canonical semantic unpacking for “contract” language (normative; used across FPF)
...
A “contract” bundle typically spans the whole [[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]] square; assign each piece explicitly to its [[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]] value:
...
Example 2 (§[[F.18 - Local‑First Unification Naming Protocol|F.18]]:18.2) shows one naming instantiation of this unpacking.

## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:17 - Migration notes (renames, splits, merges, retirements)

**M1 — Start from Contexts, not from the old global word.** When inheriting legacy names, first **re‑harvest** terms inside the chosen Contexts ([[F.2 - Term Harvesting & Normalisation|F.2]]–[[F.3 - Intra‑Context Sense Clustering|F.3]]). Only then decide whether to **reuse** or **mint** ([[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]).

**M2 — Preserve reader muscle memory without duplicating meaning.** Keep the **Plain** label close to familiar speech, but make the **Tech** label precise. If a legacy term is overloaded, publish it as a **deprecated alias** in Annexes, not as a second Tech label. ([[F.13 - Lexical Continuity & Deprecation|F.13]]; Part H “Deprecated Aliases”.)

**M3 — Prefer split‑then‑bridge over vague merges.** If one legacy word covers two distinct concepts, **split** into two rows and add a short **relation note** between them (e.g., “recipe vs run”). Do **not** hide the split under a wide new name. ([[F.7 - Concept‑Set Table|F.7]]; [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]].)

**M4 — Keep identifiers stable; move rows between blocks.** When the **Block Plan** evolves, move rows rather than renumbering; record the move in the row’s **Notes** field. ([[F.17 - Unified Term Sheet (UTS)|F.17]] §16.)

**M5 — Upgrade rationale quality with worked examples.** Every rename or split should be accompanied by a **one‑page example** that shows the new row in action across at least **two Contexts**. ([[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]]; “tell‑show‑show”.)

## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:18 - Worked examples (compact)
...
### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:18.1 - Example 1 — *MethodDescription* vs *Work* (recipe vs run)
...
* **Anti-pattern (for illustration only, non-conformant).**
 ...
 > All four are one **head-term family** (*plane*). Even if Diversity_P over raw strings looks high (four labels), **head-term diversity is 1**, so this set **fails** the [[F.18 - Local‑First Unification Naming Protocol|F.18]] diversity intent. A conformant Card would either: (a) add labels with other heads (e.g., *Layer*, *Track*, *Band*), or (b) explicitly record why other heads are rejected (AliasRisk, domain idiom) and accept low lexical Diversity_P with a rationale.
* **Enactment** (speech‑act nuance).
* **Bridges:** recipe↔run **related**, not identical; loss note “control‑flow vs. execution.”
* **Why it matters:** Managers can schedule **Work** while authors improve the **MethodDescription**—no category errors. The NQD‑front preserves tradition‑diverse, lexically stable options until a reasoned choice is made. (F.11/F.16; [[F.17 - Unified Term Sheet (UTS)|F.17]] rows.)

### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:18.2 - Example 2 — *Service* (promise) vs *SpeechAct* (utterance) vs *Commitment* (deontic)
...
* **Ontology note (informative):**
  `U.SpeechAct` and `U.Commitment` are defined normatively in Part A ([[A.2.9 - U.SpeechAct (Communicative Work Object)|A.2.9]] and [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]] respectively). This [[F.18 - Local‑First Unification Naming Protocol|F.18]] card is a lexical/NQD reference, not the ontology definition site.

* **Bridges:** utterance **institutes** commitment; commitment **binds** promise content; no synonymy claimed.
* **Why it matters:** Status tracking becomes intelligible without pretending that a “service” acts; the NQD‑front yields neutral, cross‑tradition readable labels. ([[F.12 - Service Acceptance-Work Evidence Link|F.12]]; [[F.17 - Unified Term Sheet (UTS)|F.17]] blocks D/R.)

### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:18.3 - Example 3 — *Characteristic* names without process-phase bias
...
* **Why it matters:** Prevents process phase/time from leaking into labels; the NQD-front ensures neutrality and recognizability. (A-series CHR rationale; [[F.17 - Unified Term Sheet (UTS)|F.17]] §4-§6.)

## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:19 - FAQ (authoring hygiene)

**Q1. How many Contexts must a naming proposal cite?**
**A.** The **sheet** for a thread should cite **≥ 3** distinct Contexts overall; an individual row may cite fewer if the concept appears in fewer Contexts. The point is breadth at the **UTS** level, not token‑stuffing rows. ([[F.17 - Unified Term Sheet (UTS)|F.17]] §6 constraint.)

**Q2. Can a Source Context’s term ever become the Tech label?**
**A.** Only if its form is **already neutral** and does **not** smuggle in that Context’s private commitments. When in doubt, pick a fresh neutral Tech label and keep the Source term in **SenseCells**. ([[F.5 - Naming Discipline for U.Types & Roles|F.5]].)

**Q3. Where do we put discipline‑vantage views like “Operations” vs “Research”?**
**A.** Use the **discipline columns** in a teaching layout if helpful, but remember: **discipline columns are not Context columns** and carry no editions. ([[F.17 - Unified Term Sheet (UTS)|F.17]] §5.)

**Q4. How do we keep names stable while the story evolves?**
**A.** Keep **row ids** stable; evolve placement via the **Block Plan** and record moves in **Notes**. Use [[F.13 - Lexical Continuity & Deprecation|F.13]] for renames/splits/merges. ([[F.17 - Unified Term Sheet (UTS)|F.17]] §16; [[F.13 - Lexical Continuity & Deprecation|F.13]].)

**Q5. What if two teams insist on different Tech labels for the same concept?**
**A.** Publish **one** Tech label; treat the other as a **deprecated alias** (Annex). Bridge their local senses on the row. ([[F.13 - Lexical Continuity & Deprecation|F.13]]; Part H.)

## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:20 - 90‑second teaching script (for engineer‑managers)
...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:21 - Acceptance Harness (SCR/RSCR) for [[F.18 - Local‑First Unification Naming Protocol|F.18]]

**Purpose.** Provide auditable, notation‑independent checks that a proposed unified name (and its publication on a UTS line) satisfies the **local‑first** unification discipline. The harness extends the general unification checks in **[[F.15 - SCR-RSCR Harness for Unification|F.15]]** with **naming‑specific** obligations.

### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:21.1 - Static Conformance Rules (SCR‑UNIFY)

| ID                                                                                                                                                                                                       | Requirement (normative “SHALL/SHALL NOT”)                   | Why this exists (conceptual)                                 | Where this is reflected |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------ | ----------------------- |
| **SCR‑U‑01 (Row‑first).** A unified Tech/Plain name **SHALL** be published **only** on a **Concept‑Set row** whose cells are SenseCells drawn from declared Contexts. No free‑floating names.               | Names are *lenses* onto a defended **row**, not vice‑versa. | **[[F.7 - Concept‑Set Table|F.7]]** row‑as‑unit; **[[F.17 - Unified Term Sheet (UTS)|F.17]]** UTS row discipline.            | §[[F.7 - Concept‑Set Table|F.7]]; §[[F.17 - Unified Term Sheet (UTS)|F.17]]             |
| **SCR‑U‑02 (Bridge‑only equivalence).** Cross‑context sameness **SHALL** be claimed **only** via an explicit **Bridge** with a relation kind and **CL** + loss/fit note.                                    | Prevents “string‑match identity”.                           | **[[F.9 - Alignment & Bridge across Contexts|F.9]]** Bridges; **[[F.0.1 - Contextual Lexicon Principles|F.0.1]]** principles.                       | §[[F.9 - Alignment & Bridge across Contexts|F.9]]                    |
| **SCR‑U‑03 (Neutral Tech).** The **Unified Tech** label **SHALL** be **neutral**—not lifted wholesale from any single Context **unless** the row’s Concept‑Set shows exact identity.                        | Avoids importing a local worldview as global.               | **[[F.5 - Naming Discipline for U.Types & Roles|F.5]]** naming neutrality; **[[F.17 - Unified Term Sheet (UTS)|F.17]]** 9.8 naming neutrality.   | §[[F.5 - Naming Discipline for U.Types & Roles|F.5]]; §[[F.17 - Unified Term Sheet (UTS)|F.17]]             |
| **SCR‑U‑04 (Twin registers).** Each row **SHALL** carry **Tech** and **Plain** names with the Part E register discipline; Plain is teacher‑friendly, Tech is morphology‑stable.                          | Satisfies didactic primacy without jargon creep.            | **[[E.10 - Unified Lexical Rules for FPF|E.10]]** registers; **[[F.5 - Naming Discipline for U.Types & Roles|F.5]]** naming rules.                    | §[[E.10 - Unified Lexical Rules for FPF|E.10]]; §[[F.5 - Naming Discipline for U.Types & Roles|F.5]]             |
| **SCR‑U‑05 (No window‑in‑name).** Variations by **time/phase/scale** **SHALL** be handled by **applicability windows** on **Statuses** (or examples), **NOT** by baking modifiers into the unified name. | Prevents type/status explosion by adjectives.               | **F.10/F.12** windows; **[[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]** explosion guard.             | §[[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]; §[[F.12 - Service Acceptance-Work Evidence Link|F.12]]; §[[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]     |
| **SCR‑U‑06 (Heterogeneity).** A UTS block **SHALL** demonstrate **≥ 3** independent domain families across its rows, or an explicit Bias‑Annotation shall scope the claim.                               | Enforces trans‑disciplinary reach or honest scope.          | **[[F.17 - Unified Term Sheet (UTS)|F.17]]** invariants 3; **[[E.8 - FPF Authoring Conventions & Style Guide|E.8]]** Bias‑Annotation.              | §[[F.17 - Unified Term Sheet (UTS)|F.17]]; §[[E.8 - FPF Authoring Conventions & Style Guide|E.8]]             |
...
| **SCR‑U‑08 (One‑breath rationale).** Each row **SHALL** include a **single‑sentence** Unification Rationale that states **why** the cells belong in the same Concept-Set row despite wording differences.             | Keeps the argument visible and auditable.                   | **[[F.17 - Unified Term Sheet (UTS)|F.17]]** invariant 7.                                        | §[[F.17 - Unified Term Sheet (UTS)|F.17]]                   |
| **SCR‑U‑09 (Alias budget).** Per register, deprecated aliases on a unified name **SHALL** be **≤ 1**; additional deprecated labels go to Annex/Glossary.                                                         | Controls lexical drift while preserving continuity.         | **[[F.13 - Lexical Continuity & Deprecation|F.13]]** alias budget rule.                                  | §[[F.13 - Lexical Continuity & Deprecation|F.13]]                   |
| **SCR‑U‑10 (No Cross‑context rename).** A rename **SHALL** occur **within** the same Context or same row; Cross‑context “renames” are **prohibited**—use Bridges.                                                 | Keeps locality intact; forbids silent conflation.           | **[[F.13 - Lexical Continuity & Deprecation|F.13]]** continuity; **[[F.9 - Alignment & Bridge across Contexts|F.9]]** Bridges.                        | §[[F.13 - Lexical Continuity & Deprecation|F.13]]; §[[F.9 - Alignment & Bridge across Contexts|F.9]]             |
| **SCR‑U‑11 (Semantic read‑through).** A unified Tech/Plain name **SHALL** pass a **semantic read‑through**: the Name Card lists **5–10 diverse NQD archetypal situations** and the **NQD‑front notes** record rejected candidate and their **mismatch patterns**. | Prevents labels that mislead across the intended situations; ties lexical choice to demonstrated use. | §[[F.18 - Local‑First Unification Naming Protocol|F.18]] §4.2; §[[F.18 - Local‑First Unification Naming Protocol|F.18]] §5. | §[[F.18 - Local‑First Unification Naming Protocol|F.18]] |

### [[F.18 - Local‑First Unification Naming Protocol|F.18]]:21.2 - Regression Rules (RSCR‑UNIFY)

| ID                                                                                                                                                                       | Regression duty across editions                                        | Effect |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------- | ------ |
| **RSCR‑U‑E01 (Edition drift).** When a source Context updates, re‑validate the row: stable sense ⇒ **rename/alias**; changed sense ⇒ **split/merge** rows; never overwrite. | Preserves truthfulness without erasing history. **[[F.13 - Lexical Continuity & Deprecation|F.13]]** RSCR.         |        |
| **RSCR‑U‑E02 (CL honesty).** Bridges **SHALL NOT** increase their CL (claiming higher-CL sameness) without new witnesses; **SHOULD** reduce CL when editions diverge.     | Guards against optimism bias in equivalence. **[[F.17 - Unified Term Sheet (UTS)|F.17]]** old wording.  |        |
| **RSCR‑U‑E03 (Alias creep).** Periodically prune aliases to the **≤ 1** budget per register.                                                                             | Maintains narratively crisp UTS. **[[F.13 - Lexical Continuity & Deprecation|F.13]]** RSCR‑Alias.                  |        |
| **RSCR‑U‑E04 (Name neutrality check).** If the Unified Tech label is traceable to one context’s idiom, re‑justify neutrality or retitle the row.                            | Keeps the name “ours,” not “theirs.” **[[F.17 - Unified Term Sheet (UTS)|F.17]]** 9.7–9.8.                 |        |
| **RSCR‑U‑E05 (Window misuse).** Reject newly proposed types that are really **windows** on an existing Status/Role.                                                      | Prevents explosion by adjectives. **[[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]** S14/E11 patterns.           |        |

## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:22 - Migration & Deprecation Notes (informative, naming‑specific)

1. **Start from rows, not strings.** When consolidating legacy labels, **build or revisit the Concept‑Set row** first; only then pick the **Unified Tech/Plain** names. This keeps **meaning** primary. **([[F.7 - Concept‑Set Table|F.7]], [[F.17 - Unified Term Sheet (UTS)|F.17]])**
2. **Prefer alias over merge.** If the *sense* is stable but the label misleads, **rename and retain one alias**; if the sense changed, **mint a new row** (no retrofits). **([[F.13 - Lexical Continuity & Deprecation|F.13]])**
...
4. **Keep neutrality visible.** If stakeholders push a brand‑coloured label, document why the chosen **Unified Tech** is **neutral** and include the brand as an **alias** in Glossary/Annex. **([[F.5 - Naming Discipline for U.Types & Roles|F.5]], [[F.17 - Unified Term Sheet (UTS)|F.17]])**
5. **Don’t globalise a Context.** Never move a Context label into the unified name as if it were universal. Use **Bridges** to relate Contexts, with an explicit **loss note**. **([[F.0.1 - Contextual Lexicon Principles|F.0.1]], [[F.9 - Alignment & Bridge across Contexts|F.9]])**

## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:23 - FAQ (authoring hygiene for engineer‑managers)

**Q1. Can we reuse a dominant industry term as the Unified Tech name?**
**A.** Only if the row’s Concept‑Set shows **exact identity** across Contexts; otherwise pick a **neutral** Unified Tech and list the industry label as an **alias** in the Glossary. **([[F.5 - Naming Discipline for U.Types & Roles|F.5]], [[F.17 - Unified Term Sheet (UTS)|F.17]])**

**Q2. Two terms look identical across Contexts—may we skip Bridges?**
**A.** No. **Sameness is argued, not spelled.** Publish a **Bridge** with relation kind and **CL** plus a short **loss/fit** note. **([[F.9 - Alignment & Bridge across Contexts|F.9]], [[F.0.1 - Contextual Lexicon Principles|F.0.1]])**

**Q3. When do we mint a new U.Type vs. add a new row vs. add an alias?**
**A.** Use **[[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] Mint‑or‑Reuse**: if the *concept meaning* changes, **new U.Type**; if cross-context alignment needs a new Context, **new row**; if only the label misleads, **alias/rename**.

**Q4. Our team keeps proposing “qualified roles” (e.g., *Night‑Operator*). What do we do?**
**A.** Keep the **Role** unified and express qualifiers as **windows** on **Statuses** or as **example context**. This follows **[[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]** and **[[F.12 - Service Acceptance-Work Evidence Link|F.12]]**.

**Q5. Can we compress two near‑equivalent rows into one to “simplify the sheet”?**
**A.** Only if the **one-breath rationale** remains true after review and the Bridges preserve equivalence with the same CL, or a higher CL only when new witnesses justify it; otherwise keep **two rows** with explicit differences. **([[F.17 - Unified Term Sheet (UTS)|F.17]], [[F.9 - Alignment & Bridge across Contexts|F.9]])**

## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:24 - Didactic distillation (90‑second script)
...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:24a - Name-card guardrails for set-candidate language
...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:25 - SoTA-Echoing (post-2015 practice alignment)

[[F.18 - Local‑First Unification Naming Protocol|F.18]] is local-first, kind-bound, and context-bound naming discipline. Its SoTA source-use material must not make ordinary naming look mathematically certified. Mathematical and search traditions are admitted only when they make the Name Card more reviewable for the current naming problem.

* **Terminology work, especially ISO 704:2022 and ISO 1087:2019.** This is the direct practice basis for term formation, designation, definition, and concept discipline. [[F.18 - Local‑First Unification Naming Protocol|F.18]] adopts the requirement that a reusable name must keep its kind, context, intended use, and definition recoverable, while rejecting dictionary substitution and global vocabulary rows as sufficient naming work.
...
* **Semantic transparency and morphology in interfaces and code.** These traditions provide source-use material for readable labels and error prevention where comprehension is live. [[F.18 - Local‑First Unification Naming Protocol|F.18]] adapts them into MorphologicalActionFit and local head-term checks, while rejecting readability as proof that the name is ontologically correct.

## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:26 - Relations

**Builds on:**
**[[F.0.1 - Contextual Lexicon Principles|F.0.1]]** Contextual Lexicon Principles (local meaning; bridge‑only Cross‑context claims). **[[F.1 - Domain‑Family Landscape Survey|F.1]]–[[F.3 - Intra‑Context Sense Clustering|F.3]]** Contexts → term harvesting → local sense clustering. **[[F.5 - Naming Discipline for U.Types & Roles|F.5]]** Naming discipline. **[[F.7 - Concept‑Set Table|F.7]]** Concept‑Set construction. **[[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]** Mint‑or‑Reuse decision lattice. **[[F.13 - Lexical Continuity & Deprecation|F.13]]** Lexical continuity (renames/aliases/splits/merges). **[[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]** Anti‑explosion controls (bundles, SoD, windows). **[[F.15 - SCR-RSCR Harness for Unification|F.15]]** SCR/RSCR harness. **[[F.17 - Unified Term Sheet (UTS)|F.17]]** UTS as the public term sheet.
...
## [[F.18 - Local‑First Unification Naming Protocol|F.18]]:End
