...
1. **Purpose.** A conceptual prefix exists **to aid reasoning**, not to name files, serialisations, or APIs. It labels a **role in thought** (e.g., meta‑type, calculus operator, relation family).
 ...
 3. **No tool lock‑in.** A prefix **MUST NOT** imply a particular notation or machine binding (see [[E.5.1 - DevOps Lexical Firewall|E.5.1]]–[[E.5.2 - Notational Independence|E.5.2]]).
 4. **Minting rule.** New prefixes are introduced by a **DRR** ([[E.9 - Design‑Rationale Record (DRR) Method|E.9]]) that demonstrates
...
* `ut:` — **Universal relation family** (e.g., `PartOf` sub‑relations). *Anchor:* [[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] (Mereology) — informative alias vocabulary.
* `tv:` — **Trace & Validation vocabulary** (CT2R‑LOG): `tv:AliasOf`, `tv:groundedBy`. *Anchor:* [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (Trust & Assurance, LOG‑use).
* `ev:` — **Evidence hooks** (bindings/roles). *Anchor:* [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] / [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (Evidence Graph Referring).
* `mero:` — **Mereology trace types** (internal labels: `SumTrace` / `SetTrace` / `SliceTrace`) used **informatively** in examples. *Anchor:* [[B.1 - Universal Algebra of Aggregation (Γ)|B.1]] (Γ‑aggregation).

**Conformance Checklist ([[E.10.P - Conceptual Prefixes policy & registry|E.10.P]]).**
...
**Relations.** Constrains [[E.5.1 - DevOps Lexical Firewall|E.5.1]] (Lexical Firewall) & [[E.5.2 - Notational Independence|E.5.2]] (Notational Independence); Depends on [[E.9 - Design‑Rationale Record (DRR) Method|E.9]] (DRR).

## [[E.10.P - Conceptual Prefixes policy & registry|E.10.P]]:End
