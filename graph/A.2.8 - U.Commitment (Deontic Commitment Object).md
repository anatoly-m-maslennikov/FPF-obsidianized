...
> **Status:** Stable
> **Normativity:** Normative (unless explicitly marked informative)
> **Placement:** Part A → **[[A.2 - Role Taxonomy|A.2]] Roles & Agency Kernel**
> **Refines:** [[A.2 - Role Taxonomy|A.2]] (Role Taxonomy)
> **Builds on:** [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] (authoring template), [[A.2.1 - U.RoleAssignment- Contextual Role Assignment|A.2.1]] (RoleAssignment), [[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] (Scope & `Γ_time`), [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] (EntityOfConcern / Description episteme / carrier), [[A.2.3 - U.PromiseContent (Promise Content)|A.2.3]] (`U.PromiseContent` as promise), [[A.15.1 - U.Work|A.15.1]] (`U.Work`)
> **Purpose (one line):** Provide a minimal, reusable kernel object for deontic commitments (who is accountable, under what modality, in what scope/window, with respect to which referents, with which adjudication hooks), **explicitly separating the commitment object from its utterance descriptions** ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]]), so deontics stop “living” in naming patterns and become stable across [[A.6 - Signature Stack & Boundary Discipline|A.6]] and later governance patterns.

## [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:0 - Terminology: “binding” is overloaded (normative)
...
## [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:1 - Problem frame
...
[[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]] provides L/A/D/E claim-classification discipline, and [[A.6.C - Contract Unpacking for Boundaries|A.6.C]] provides contract-language unpacking, but both benefit from a **kernel-level** object that pins down what a `U.Commitment` is structurally (so “contract/binding” rhetoric does not leak back in as ontology).

## [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:2 - Problem
...
## [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:3 - Forces
...
## [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:4 - Solution
...
* how it is used as the canonical payload for **D-quadrant** claims ([[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]]),
* and what must be stated for a commitment to be considered **auditable**.

### [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:4.1 - Normative definition
...
Per [[A.7 - Strict Distinction (Clarity Lattice)|A.7]], a `U.Commitment` is **not** the text that states it: it is an object that is typically **instituted by** (and recorded via) one or more **speech acts and utterance descriptions** and may be carried by utterance carriers or publication carriers.

### [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:4.2 - Minimal structure (normative)
...
PromiseContentRef ::=
  ObjectIdRef
  // MUST resolve to a `U.PromiseContent` object ([[A.2.3 - U.PromiseContent (Promise Content)|A.2.3]]). (Some chapters may call this a “promise content”.)
...
SpeechActRef ::=
  ObjectIdRef
  // MUST resolve to a `U.SpeechAct` Work occurrence ([[A.2.9 - U.SpeechAct (Communicative Work Object)|A.2.9]]).
...
### [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:4.3 - Interaction rules (normative)
...
5. **Commitment changes are explicit (no silent mutation).**
   When a commitment is updated, narrowed, broadened, superseded, or revoked, the change **SHOULD** be represented as a new `U.Commitment` (new ID) and an instituting `U.SpeechAct` ([[A.2.9 - U.SpeechAct (Communicative Work Object)|A.2.9]]) that references the affected commitment IDs (e.g., via `U.Commitment.source.speechActRef` and a status/supersession claim), rather than editing a published commitment in place without an auditable change record.

### [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:4.4 - Canonical use in boundary claim registers (recommended)

When using the [[A.6 - Signature Stack & Boundary Discipline|A.6]] stack, represent each **D-quadrant** atomic claim as a `U.Commitment` payload with:
...
## [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:5 - Archetypal Grounding (Tell–Show–Show)

### [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:5.1 - Tell (universal rule)
...
### [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:5.2 - Show #1 (system archetype: incident response SLO discipline, post‑2015 SRE practice)
...
### [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:5.3 - Show #2 (episteme archetype: protocol specification with behavioural typing motif)
...
## [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:6 - Bias-Annotation
...
## [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:7 - Conformance Checklist (normative)

1. **CC‑[[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]‑1 (Accountable subject).** A normative `U.Commitment` **MUST** name an accountable `subject` (role assignment, role enactor, or party) and **MUST NOT** use a specification episteme, interface-description episteme, or document-carried episteme as subject.

2. **CC‑[[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]‑2 (Explicit modality).** A normative `U.Commitment` **MUST** specify `modality` as `DeonticModalityToken` (with any RFC-keyword synonyms normalized to it).

3. **CC‑[[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]‑3 (Scope & validity explicit).** A normative `U.Commitment` **MUST** specify `scope` (`U.ClaimScope`) and `validityWindow` (`U.QualificationWindow`), or explicitly cite the context policy that supplies defaults (do not rely on “implied defaults”).

4. **CC‑[[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]‑4 (Referents present and by ID).** `referents` **MUST** be non‑empty. If the bound content exists as claim IDs, the commitment **SHOULD** reference those IDs in `referents` rather than restating their content.

5. **CC‑[[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]‑5 (Auditable commitments have hooks).** If the commitment is intended to be auditable, it **SHALL** include `adjudication.evidenceRefs` referencing the evidence claims (typically `E-*`) that make adjudication possible.
6. **CC‑[[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]‑6 (Evidence separation).** If an `E-*` claim is referenced only for measurement/verification, it **SHALL** appear in `adjudication.evidenceRefs` (not in `referents`).

## [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:8 - Common Anti-Patterns and How to Avoid Them
...
## [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:9 - Consequences

**Benefits**

* Makes deontic statements **first-class and lintable** (subject/modality/scope/referents/hooks).
* Enables clean integration with boundary routing ([[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]]) and contract unpacking ([[A.6.C - Contract Unpacking for Boundaries|A.6.C]]) without embedding ontology in naming patterns.
...
## [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:10 - Rationale
...
## [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:11 - SoTA-Echoing (informative; post‑2015 alignment)
...
## [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:12 - Relations

**Uses / builds on**

* [[A.2.1 - U.RoleAssignment- Contextual Role Assignment|A.2.1]] for identifying accountable roles vs role-enactors (role assignments).
* [[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] for expressing scope and time/window (`U.ClaimScope`, `U.QualificationWindow`).
* [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] for keeping “binding” distinct from “utterance” and from “carriers”.

**Used by**

* [[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]] (Quadrant D) as the canonical payload shape for deontic statements.
* [[A.6.C - Contract Unpacking for Boundaries|A.6.C]] (Contract Unpacking) as the formal anchor for the “Commitment” component of the bundle.
* Part D governance/ethics patterns (future work) for expressing layered, conflicting, multi-authority commitments.

**Coordinates with**

* [[A.2.3 - U.PromiseContent (Promise Content)|A.2.3]] (`U.PromiseContent`): services are promise clauses; commitments bind accountable subjects to those clauses.
* **[[A.2.9 - U.SpeechAct (Communicative Work Object)|A.2.9]] (`U.SpeechAct`)**: `U.Commitment.source.speechActRef` points to the instituting communicative work occurrence when provenance matters.
* [[A.15.1 - U.Work|A.15.1]] (`U.Work`) and evidence patterns: adjudication hooks refer to evidence in work, not to text.

## [[A.2.8 - U.Commitment (Deontic Commitment Object)|A.2.8]]:End
