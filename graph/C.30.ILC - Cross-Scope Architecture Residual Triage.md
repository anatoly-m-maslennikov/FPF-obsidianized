...
> **Type:** Architectural subpattern under [[C.30 - Grounded Architecture and Selected-Structure Adequacy|C.30]]
> **Status:** Stable
> **Normativity:** Normative for FPF pattern, companion-text, and project-description use that claims architecture-specific residuals across declared scopes.

## [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:1 - Problem frame
...
What goes wrong if [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] is missed: a local improvement, control layer, scale label, interface grammar, or evidence reuse is treated as whole-holon architecture adequacy while the residual moves into another declared holon level or declared scope.

What [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] buys in practice: the practitioner can keep useful conflict or frustration language as an entry label while governing the architecture residual itself: affected holon levels or scopes, the selected structure that carries them, residual-bearing locus, and one admissible architecture move or governing-pattern application.
...
The useful architecture intuition is narrower than a new `Frustration` kind: local optimization at one declared holon level or declared scope can create a persistent residual in another declared holon level, declared scope, or level-bearing structure relation. Depending on the claim being made, that residual is governed by [[C.29 - Mathematical Lens Use|C.29]] only when a recoverable multilevel mapping, scale mapping, or coarse-graining mapping is being claimed; by [[C.31.ASAP - Architecture Scale-Amenability Preference|C.31.ASAP]] when architecture scale preference over declared alternatives is being claimed; or by [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] when residual-reducing candidate architecture moves form a candidate set being used. An ordinary conflict between structures is not enough for the RG lens or frustration mathematical lens, but a conflict between structures assigned to different declared holon levels or scale windows may be enough when the mapping, preserved-structure line, and lost-structure line are recoverable. The first [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] output is only the grounded triage record.

Without a pattern, teams either discuss the residual as vague `complexity`, treat it as ordinary stakeholder conflict, jump into measurement, use mathematical frustration language as proof, or jump to candidate generation too early. [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] keeps the first move small: identify whether the residual is architecture-shaping and name the first admissible architecture move or governing-pattern application.
...
## [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:3 - Forces
...
## [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:4 - Solution
...
**Multilevel optimization boundary.** [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] can recognize that local optimization in one declared holon level or declared scope degrades another declared holon level or declared scope. It does not optimize the architecture and does not prove that one global function exists. Use [[C.29 - Mathematical Lens Use|C.29]] with `MLU.Description@MultilevelLearningFrustration` only when the mathematical representation supplies a recoverable mapping between declared levels, scopes, scale windows, or coarse-graining steps and states what structure is preserved and lost. Conflicting structures can enter this lens only when each structure is assigned to a declared holon level, scope, scale window, or coarse-graining step and the mapping shows why the conflict is interlevel. If scale window, RG relation, coarse-graining relation, preserved structure, lost structure, or conflict residual slope becomes an architecture scale-preference claim, use [[C.31.ASAP - Architecture Scale-Amenability Preference|C.31.ASAP]] and keep any mathematical-lens claim in [[C.29 - Mathematical Lens Use|C.29]]. If the practitioner needs to generate, compare, or publish residual-reducing candidate architecture moves, use [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] for the candidate set and [[C.11 - Decision Theory (Decsn-CAL)|C.11]] when a final local choice is being made; [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] stops at the residual and first admissible move. If the case is only a conflict between two selected structures with no declared multilevel mapping or scale mapping, keep it in [[C.30 - Grounded Architecture and Selected-Structure Adequacy|C.30]], [[C.30.ASV - Architecture Structural View Adequacy (ASV)|C.30.ASV]], `D.3`, `D.4`, [[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]], evidence, assurance, or decision patterns as applicable.
...
|---|---|
| measurement or characteristic claim | [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]] or the characteristic pattern that governs the characteristic under evaluation |
| scale window, RG relation, coarse-graining relation, preserved structure, lost structure, or conflict residual slope | [[C.31.ASAP - Architecture Scale-Amenability Preference|C.31.ASAP]] when an architecture scale-preference claim is being made; use [[C.29 - Mathematical Lens Use|C.29]] when mathematical-lens use is being claimed |
| multilevel learning or frustration mathematical-lens use with recoverable level mapping or scale mapping | [[C.29 - Mathematical Lens Use|C.29]] with `MLU.Description@MultilevelLearningFrustration` |
| candidate generation or residual-reducing candidate architecture moves | [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] for the candidate set; [[C.11 - Decision Theory (Decsn-CAL)|C.11]] when final local choice is being made |
| final local choice | [[C.11 - Decision Theory (Decsn-CAL)|C.11]] |
| causal outcome claim | [[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] |
| evidence or assurance | [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]], [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], or [[G.6 - Evidence Graph & Provenance Ledger|G.6]] |
| ethical or stakeholder mediation | `D.3` or `D.4` |

**D.3 and D.4 boundary.** `D.3` and `D.4` handle conflict topology, values, stakeholder mediation, and ethical negotiation. [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] handles architecture-specific recognition: whether the conflict is borne by declared holon levels or declared scopes inside a selected structure: structural views, allocation, interfaces, control rates, work reuse, evidence reuse, scale windows, or coarse-graining loss. It is a triage and architecture-move pattern, not a negotiation pattern.
...
| Local autonomy conflicts with control scope | add control layer; change allocation; apply [[C.30.LCA - Control Structure View Adequacy (LCA)|C.30.LCA]] | Treat autonomy label as causal or safety proof. |
| Evidence reuse hides source loss | add evidence scope; add source-return condition; apply [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] or [[G.6 - Evidence Graph & Provenance Ledger|G.6]] | Treat reused evidence as automatically valid in the wider scope. |
| A scale window changes the residual | apply [[C.31.ASAP - Architecture Scale-Amenability Preference|C.31.ASAP]], with [[C.29 - Mathematical Lens Use|C.29]] when scale-lens use is being made | Treat two observations as a universal scale law. |
| A frustration lens with recoverable level mapping or scale mapping makes candidate moves comparable | use [[C.29 - Mathematical Lens Use|C.29]] for lens adequacy and [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] for the candidate set | Treat an unassigned or same-scope structure conflict as RG mathematics or frustration mathematics, or treat an interlevel residual without recoverable mapping as a global optimizer, proof, or selected architecture. |

**Worked slice A - clean module layout, bad flow.** A product team redraws modules so each component has an explicit responsibility relation or enactor relation, but order-to-cash flow now crosses more work transfers and exceptions rise. [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] names the module structure, selected transformation-flow structure, affected work scope, cross-scope residual, and first move: expose hidden coupling or apply [[C.30.TFS-REL - Architecture Transformation-Flow Structure Relation|C.30.TFS-REL]]. It does not turn the exception count into a modularity measure until [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]] or the characteristic pattern governing the characteristic under evaluation is applied.

**Worked slice B - AI agent control conflict.** A local agent optimizes its local objective and violates a supervisor's allowed-mode constraint. [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] names the agent scope, supervisor scope or control scope, control relation, local optimization claim, residual-bearing locus, and local repair attempted. The first move may be add control layer, change allocation, or apply [[C.30.LCA - Control Structure View Adequacy (LCA)|C.30.LCA]]. Safety, causality, and gate claims use their governing patterns.

**Worked slice C - evidence scope residue.** A reusable certification evidence set removes repeated evidence work for several product variants, but one variant has a hidden environment difference. [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] names the work scope or evidence scope and source-return condition. It applies [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] or [[G.6 - Evidence Graph & Provenance Ledger|G.6]] when an evidence-validity claim is being made.

**Worked slice D - frustration residual before synthesis.** Several decompositions reduce local module work but each creates a different integration, control-rate, or evidence-reuse residual in another declared scope. [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] records the residuals and first architecture moves. If the team needs a residual-reducing candidate palette, [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] stops and applies [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] to the candidate set. If the team claims a multilevel-learning lens or frustration lens, [[C.29 - Mathematical Lens Use|C.29]] carries the lens-use fields and stop condition only after the level mapping, scope mapping, scale-window mapping, or coarse-graining mapping and preserved structure and lost structure are recoverable.

## [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:5 - Archetypal Grounding

| Archetype | Without [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] | With [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] |
|---|---|---|
...
## [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:6 - Bias-Annotation
...
* **Generic-structure-conflict bias.** A conflict between selected structures is treated as interlevel conflict even though no declared holon level, scale window, or coarse-graining relation is recoverable. Repair by keeping the case in [[C.30 - Grounded Architecture and Selected-Structure Adequacy|C.30]], [[C.30.ASV - Architecture Structural View Adequacy (ASV)|C.30.ASV]], or the governing pattern unless the structures are assigned to different declared holon levels or scale windows.
...
* **Global-optimizer bias.** Local optimization in one declared holon level or declared scope is used as if the architecture literally optimizes one global function. Repair by keeping the local optimization claim as a triage input unless [[C.29 - Mathematical Lens Use|C.29]] supplies an admissible mathematical-lens use with recoverable level mapping or scale mapping and the candidate-set or decision pattern carries any synthesis or selection claim.
* **Measurement-first bias.** A residual is measured before its level-bearing structure and scope grounding are declared. Repair by applying [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]] or the characteristic pattern governing the characteristic under evaluation only after triage names the affected characteristic or measurement relation.
...
* **Synthesis-jump bias.** A local residual immediately triggers candidate generation. Repair by identifying the first admissible architecture move before applying [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] to a residual-reducing candidate set.
...
## [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:7 - Conformance Checklist
...
| CC-ILC-9 | If multilevel learning or frustration is used as mathematics, the record names [[C.29 - Mathematical Lens Use|C.29]], `MLU.Description@MultilevelLearningFrustration`, the recoverable level mapping or scale mapping, and preserved structure and lost structure; if residual-reducing candidate moves form a candidate set being used, the record applies [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] to the candidate set. | Preserves the useful multilevel optimization line without importing ontology, proof, or a hidden selector. |

## [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:8 - Common Anti-Patterns and How to Avoid Them
...
| Structure conflict as interlevel conflict | Two structures are in tension, but no declared holon level, scale window, or coarse-graining relation is recoverable. | Use [[C.30 - Grounded Architecture and Selected-Structure Adequacy|C.30]], [[C.30.ASV - Architecture Structural View Adequacy (ASV)|C.30.ASV]], `D.3`, `D.4`, [[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]], evidence, assurance, or decision patterns as applicable; use [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] only when the conflict is across declared levels or scopes. |
| Frustration-as-ontology | `Frustration` is treated as a new FPF kind, psychological state, physics kind, biology kind, assurance score, or proof. | Keep frustration as an entry label; recover `FrustrationResidual`, conflict carriers, residual-bearing locus, and governing patterns for [[C.29 - Mathematical Lens Use|C.29]], evidence, assurance, causal, mediation, or synthesis claims when those claim kinds are being made. |
| Optimization-as-global-proof | A local optimization claim is treated as proof that the whole architecture optimizes one global objective. | Record local and wider-scope claims separately; use [[C.29 - Mathematical Lens Use|C.29]] only for an admissible mathematical lens with recoverable level mapping or scale mapping and use candidate-generation, comparison, or decision patterns for synthesis and selection. |
| Measurement-first conflict | The team starts measuring before declaring what is in conflict. | Run ILC triage first; apply [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]] or the characteristic pattern governing the characteristic under evaluation only when the measured characteristic is under evaluation. |
...
| Stakeholder-only conflict | A structural residual is treated as mediation with no architecture move. | Use `D.3` or `D.4` only when values, stakeholder negotiation, or ethical mediation is being claimed. |
| Hidden candidate generation | The residual immediately spawns many designs. | State the first admissible move; apply [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] to a residual-reducing candidate set only when candidate generation is being claimed. |
...
## [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:9 - Consequences
...
| Vanchurin, Wolf, Katsnelson, and Koonin multilevel learning and frustration line, plus related multilevel optimization and complexity-frustration materials. | Local optimization at one declared holon level, scope, or level-bearing structure relation can create persistent residual in another; frustrated optimization can be a candidate mathematical lens when a level mapping or scale mapping is recoverable. | Adopt and adapt: [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] uses this line for architecture triage only. [[C.29 - Mathematical Lens Use|C.29]] with `MLU.Description@MultilevelLearningFrustration` carries mathematical-lens adequacy with preserved structure and lost structure; no `U.Frustration`, physics or biology ontology transfer, global optimizer proof, causal proof, assurance proof, or stakeholder-mediation takeover. | First recover holon levels or scopes, level-bearing structure, conflict carriers, residual-bearing locus, local repair, and the first architecture move. Apply [[C.29 - Mathematical Lens Use|C.29]] only when the lens mapping is being claimed; apply [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] only when residual-reducing candidate moves form a candidate set being used. |
| Control and cyber-physical systems practice. | Local autonomy, feedback, supervisor relations, and rate separation can create cross-scope conflict. | Reuse through [[C.30.LCA - Control Structure View Adequacy (LCA)|C.30.LCA]], [[B.2.5 - Supervisor-Subholon Feedback Loop|B.2.5]], [[C.27 - Temporal Claim Adequacy- State Readings, Temporal Trends, and Intervention-Sensitive Temporal Change|C.27]], and [[A.3.3 - U.Dynamics- State-Space and Transition-Law Episteme|A.3.3]]; do not let ILC carry control proof. | A control conflict is governed by control-structure or dynamics patterns only when those claims are being made. |
...
## [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:12 - Relations

* Builds on [[C.30 - Grounded Architecture and Selected-Structure Adequacy|C.30]] and [[C.30.ASV - Architecture Structural View Adequacy (ASV)|C.30.ASV]] for grounded architecture, selected-structure, and structural-view adequacy.
* Uses [[A.22 - Structure and Structural Views (STRUCT-CAL)|A.22]] for structure and structural-view discipline.
* Coordinates with [[C.30.TFS-REL - Architecture Transformation-Flow Structure Relation|C.30.TFS-REL]], [[C.30.LCA - Control Structure View Adequacy (LCA)|C.30.LCA]], [[A.6.F - Function and Functional Precision Restoration (RPR-FUNCTION)|A.6.F]], and [[A.6.M - Module Relation Repair|A.6.M]] when the residual concerns flow, control, function, allocation, module, or interface structure.
* Applies [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]] or the characteristic pattern that governs the characteristic under evaluation for measurement or characteristic claims.
* Applies [[C.29 - Mathematical Lens Use|C.29]] with `MLU.Description@MultilevelLearningFrustration` only when multilevel learning or frustration is used as a mathematical lens with recoverable level mapping or scale mapping and preserved structure and lost structure; applies [[C.31.ASAP - Architecture Scale-Amenability Preference|C.31.ASAP]] for architecture scale-preference claims and [[C.29 - Mathematical Lens Use|C.29]] for mathematical-lens claims when scale, RG, coarse-graining, preserved structure, lost structure, or scale-window adequacy is being claimed.
* Applies [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] for candidate generation and residual-reducing candidate architecture moves.
* Applies [[C.11 - Decision Theory (Decsn-CAL)|C.11]] for final local choice, [[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] for causal outcome claims, [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]], [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], or [[G.6 - Evidence Graph & Provenance Ledger|G.6]] for evidence or assurance, and `D.3` or `D.4` for ethical or stakeholder mediation.

Neighboring claims stay with their governing patterns: [[C.30 - Grounded Architecture and Selected-Structure Adequacy|C.30]] for grounded architecture and selected-structure adequacy, [[C.30.ASV - Architecture Structural View Adequacy (ASV)|C.30.ASV]] for structural-view adequacy, [[A.22 - Structure and Structural Views (STRUCT-CAL)|A.22]] for structure and structural-view discipline, [[C.30.TFS-REL - Architecture Transformation-Flow Structure Relation|C.30.TFS-REL]] for architecture-transformation-flow relation, [[C.30.LCA - Control Structure View Adequacy (LCA)|C.30.LCA]] for control-structure view relation, [[A.6.F - Function and Functional Precision Restoration (RPR-FUNCTION)|A.6.F]] for function-use repair, [[A.6.M - Module Relation Repair|A.6.M]] for module-interface repair, [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]] or the local characteristic pattern for the characteristic under evaluation, [[C.29 - Mathematical Lens Use|C.29]] for mathematical-lens use, [[C.31.ASAP - Architecture Scale-Amenability Preference|C.31.ASAP]] for architecture scale-preference, [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] for candidate-set generation, [[C.11 - Decision Theory (Decsn-CAL)|C.11]] for final local choice, [[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] for causal use, [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]], [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], or [[G.6 - Evidence Graph & Provenance Ledger|G.6]] for evidence or assurance, and `D.3` or `D.4` for ethical or stakeholder mediation. [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]] governs only cross-scope architecture residual triage.

## [[C.30.ILC - Cross-Scope Architecture Residual Triage|C.30.ILC]]:End
