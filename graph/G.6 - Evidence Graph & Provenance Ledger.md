...
**Primary hooks.** [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] (evidence anchors/carriers; SCR/RSCR anchoring), [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (assurance lanes and `F/G/R` skeleton), [[F.9 - Alignment & Bridge across Contexts|F.9]] (BridgeCard/CL), [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] (CAL `EvidenceProfiles` + `ProofLedger` linkage), [[G.Core - Part G Core Invariants|G.Core]] (Part‑G invariants, RSCR trigger catalogue, default-governing-definition index), E.18/A.21 (GateCrossing + CrossingBundle checks), [[F.17 - Unified Term Sheet (UTS)|F.17]] (UTS publication), [[F.15 - SCR-RSCR Harness for Unification|F.15]] (RSCR), [[E.10 - Unified Lexical Rules for FPF|E.10]] (LEX), [[E.5 - Four Guard‑Rails of FPF|E.5]].* (notation‑independence discipline).
**Non‑duplication note.** Universal Part‑G invariants (no shadow specs; Bridge‑only crossings; tri‑state discipline; penalties→`R_eff` only; P2W split; typed/id‑based RSCR causes; defaults with one governing definition; Δ‑discipline) are governed by [[G.Core - Part G Core Invariants|G.Core]] and are *cited* via `CC‑GCORE‑*`. This pattern defines only the *EvidenceGraph kit* and its path‑addressable provenance surfaces.

## [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:1 - Problem frame

SoTA claims, operators, and method families are admitted (or gated) using assurance signals derived from diverse artefacts and anchors. FPF already mandates **Evidence Graph Referring** ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]), lane discipline, and the assurance skeleton ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]). What is often still missing in practice is a *first‑class, citable* object that makes the provenance of an admission/decision **addressable**:
...
**Why here (not in [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]])?** [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] governs CAL artefacts (EvidenceProfiles, ProofLedger, acceptance policies). [[G.6 - Evidence Graph & Provenance Ledger|G.6]] packages *cross‑artefact provenance* as a graph and mints *path identities* that downstream surfaces can cite without duplicating CAL tables or re‑inventing legality rules.

## [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:2 - Problem
...
## [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:3 - Forces
...
## [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:4 - Solution — EvidenceGraph (notation‑independent; lane‑aware; path‑addressable)

### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:4.1 - [[G.Core - Part G Core Invariants|G.Core]] linkage (normative)
...
`GCoreLinkageManifest := ⟨
  ...
  CorePinsRequired := {
    ...
    PathId[]/PathSliceId[],      // strengthened (unconditional for [[G.6 - Evidence Graph & Provenance Ledger|G.6]])
    ...
    CAL.ProofLedgerId[]?         // iff Γ-fold is overridden (cite CAL ProofLedger ids; governed by [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]])
  },
  DefaultsConsumed := { DefaultId.GammaFoldForR_eff },
  TriggerAliasMapRef? := G.Core.TriggerAliasMap.G6
⟩`

**Conditional add‑on (tri‑state guard).** If [[G.6 - Evidence Graph & Provenance Ledger|G.6]] is used to publish or consume guard outcomes (e.g., via `G.6:Ext.SoSLOGPathCitationWiring`), additionally require:
`CoreConformanceProfileIds += { GCoreConformanceProfileId.PartG.TriStateGuard }`.

*(Nil‑elision + expansion rule are per `G.Core:4.2`.)*

### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:4.2 - EvidenceGraph (object; surface governed by the kit)

**Definition (object).** An `EvidenceGraph` is a **typed DAG** whose nodes are resolvable to [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] anchors/carriers and evidencing roles, and whose edges represent minimal, normative provenance relations suitable for audit and path citation.

* **Nodes.** Each node is an [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]‑anchored evidence carrier or evidence role (e.g., a proof carrier, a measurement record carrier, a tool‑qualification carrier). Nodes MUST remain grounded in [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] anchors and MUST NOT introduce mereological structure ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] firewall).
...
* **Edges (minimal normative vocabulary).** The pattern admits a small set of provenance edges sufficient for audit:
  ...
  * *(Informative only)* `usedCarrier`, `interpretedBy` MAY appear as authoring aids, but MUST NOT be relied on for conformance checks (their semantics remain non‑normative in [[G.6 - Evidence Graph & Provenance Ledger|G.6]]).
...
* **Externality (no self‑evidence).** Any evidencing `TransformerRole` that would certify the evaluated holon MUST be modelled as external (or model a meta‑holon explicitly); [[G.6 - Evidence Graph & Provenance Ledger|G.6]] does not permit reflexive “self‑evidence” shortcuts.
* **Context and plane attachment.** Nodes and claims carry `BoundedContext` and `ReferencePlane`. Any movement across context/kind/plane/design↔run/edition boundaries is represented via explicit GateCrossing/CrossingBundle artefacts (with crossing pins routed per [[G.Core - Part G Core Invariants|G.Core]]).

### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:4.3 - PathId and PathSliceId (citable justification addresses)
...
*The universal definition of “what kinds of changes force refresh” is governed by [[G.Core - Part G Core Invariants|G.Core]] (typed trigger kinds). [[G.6 - Evidence Graph & Provenance Ledger|G.6]] only makes the slice addressable and pin‑complete.*
...
### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:4.4 - Assurance and legality binding (delegation‑first; no shadow specs)

[[G.6 - Evidence Graph & Provenance Ledger|G.6]] does not redefine [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] or legality rules; it binds evidence paths to existing governing definitions:

* **Assurance skeleton.** Lane separation and the `F/G/R` skeleton are as per [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]. Any statement about penalty routing or default Γ‑fold is delegated to [[G.Core - Part G Core Invariants|G.Core]] and the Default Governing Definition Index (do not restate).
* **CAL linkage.** When a path claims a proof obligation or an override (e.g., an explicit Γ‑fold override), it MUST cite the relevant CAL `ProofLedger` / `EvidenceProfiles` artefacts ([[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]) rather than inventing local semantics.
* **Legality binding.** If a path includes numeric comparisons/aggregations, the legality surface MUST be *cited* via `CG‑Spec` ([[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]) rather than re‑implemented in [[G.6 - Evidence Graph & Provenance Ledger|G.6]] prose.

### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:4.5 - Conceptual interface (notation‑independent surface; informative shapes)

These are conceptual shapes, not tool APIs ([[E.5 - Four Guard‑Rails of FPF|E.5]] discipline).

* `Explain(pathId | pathSliceId)` → returns a citation‑ready explanation bundle: lane split, relevant pins (crossings/policies/editions), freshness binding, and links to contributing anchors ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]) and any CAL evidence/profile refs.
...
### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:4.6 - Extensions (pattern‑scoped; non‑core)
...
* **GoverningPatternId:** [[G.Core - Part G Core Invariants|G.Core]]
* **Uses:** `{[[G.Core - Part G Core Invariants|G.Core]]}` *(cites `G.Core.TriggerAliasMap.G6`; does not redefine meanings)*
* **⊑/⊑⁺:** `∅`
...
* **Uses:** `{[[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]}` *(SoS‑LOG decisions cite paths; optional lens/attribution wiring is governed by [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]]; refresh consumes triggers)*
* **⊑/⊑⁺:** `∅`
* **RequiredPins/EditionPins/PolicyPins (minimum):**

  * `SoSLogRuleId[]` / `BranchId[]` *(as cited labels; semantics governed by [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]])*
  * `FailureBehaviorPolicyId` *(when `degrade(mode=...)` is used)*
  * `PathId[] | PathSliceId[]` (the cited justification addresses)
  * `LensId?` *(when a [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] lens is used for attribution/explainability; id only; semantics governed by [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]])*
...
* **Notes (wiring‑only):** [[G.6 - Evidence Graph & Provenance Ledger|G.6]] does not define LOG semantics; it defines the *path‑citation surface* that LOG must cite.
...
* **GPatternExtensionKind:** `InteropSpecific`
* **GoverningPatternId:** [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]
* **Uses:** `{[[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]], [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]}` *(bridge/sentinel semantics and calibration records are governed by [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]; refresh orchestration is governed by [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]])*
* **⊑/⊑⁺:** `∅`
* **RequiredPins/EditionPins/PolicyPins (minimum; conditional on use):**

  * `BridgeId/BridgeCardId`
  * `RegressionSetId?` / `SentinelId[]?` *(as published by [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]], when sentinel wiring is used)*
...
* **Notes (wiring‑only):** This module requires that bridge/sentinel changes re‑trigger RSCR **path‑locally** for affected `PathId/PathSliceId` scopes, without redefining sentinel semantics (governed by [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]) and without inventing new trigger kinds (governed by [[G.Core - Part G Core Invariants|G.Core]]).
...
* **GoverningPatternId:** [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]] *(QD artefact semantics); uses [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] for exploration/logging/lens wiring as needed*
...
* **RequiredPins/EditionPins/PolicyPins (minimum; conditional on use):**
  ...
  * `EmitterPolicyRef?`
  * `LensId?` *(when a [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] lens is used in selection/telemetry attribution)*
...
## [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:5 - Archetypal Grounding (System / Episteme)
...
## [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:6 - Bias‑Annotation
...
## [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:7 - Conformance Checklist (normative) — **CC‑G6**

| ConformanceId                                     | Requirement                                                                                                                                                                                                                                                                                                                                                                  | Purpose |
| ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| **CC‑G6‑CoreRef**                                 | [[G.6 - Evidence Graph & Provenance Ledger|G.6]] is conformant only if it satisfies the **effective** `CC‑GCORE‑*` set expanded from the `GCoreLinkageManifest` in **§4.1** (explicit crossings & pins, penalties→`R_eff` only, P2W split, typed RSCR trigger kinds, defaults with one governing definition, UTS discipline, Δ‑discipline).                                                                                                 | Cite core invariants |
| **CC‑G6‑1 (Anchor & lanes)**                      | Every citable path MUST resolve to [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] anchors (SCR/RSCR addressable) and MUST declare lane tags (`TA/VA/LA`) on bindings.                                                                                                                                                                                                                                                | Ground auditability |
...
| **CC‑G6‑14 (Legality binding; no shadow specs)**  | If numeric operations are cited/used in a path, legality MUST be pinned/cited via `CG‑Spec` rather than asserted locally, and the path/SCR surface MUST fail fast on illegal arithmetic/typing (e.g., CSLC/scale violations); do not “promote” ordinal to cardinal by convention inside [[G.6 - Evidence Graph & Provenance Ledger|G.6]]. *(Delegation target for “no shadow specs”: `CC‑GCORE‑CN‑CG‑1`.)*                                                                                     | Prevent illicit arithmetic |
...
## [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:7.5 - Interfaces & Hooks (normative)

Each hook below defines: **Trigger → Obligation → Publishes/Consumes → Invariants**.
Where universal invariants apply (crossings, penalties, trigger typing), this section *cites* [[G.Core - Part G Core Invariants|G.Core]] rather than redefining semantics.

### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:7.5.1 - H1 — UTS Name Card for EvidenceGraph Nodes
...
* **Publishes/Consumes.** Publishes: UTS row. Consumes: [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] anchor metadata.
* **Invariants.** UTS publication and any deprecation/aliasing follow the delegated UTS discipline through [[G.Core - Part G Core Invariants|G.Core]] and [[F.17 - Unified Term Sheet (UTS)|F.17]].

### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:7.5.2 - H2 — UTS PathCard (PathId/PathSliceId)
...
* **Invariants.** Crossing visibility and penalty routing are delegated to [[G.Core - Part G Core Invariants|G.Core]] (`CC‑GCORE‑CROSS‑1`, `CC‑GCORE‑PEN‑1`).

### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:7.5.3 - H3 — RSCR Trigger on Evidence‑Impacting Edit (typed; alias‑dockable)

* **Trigger.** Any edit in [[G.6 - Evidence Graph & Provenance Ledger|G.6]] that can change a path’s audit‑relevant surface (evidence structure, crossing pins, penalty policy pins, plane binding, freshness binding, edition/policy pins, or telemetry‑bound fields).
* **Obligation.** Emit RSCR triggers **using canonical `RSCRTriggerKindId`** (from [[G.Core - Part G Core Invariants|G.Core]]) and record affected scope (`PathId/PathSliceId`) plus payload pins required for downstream refresh. If a deprecated `G.6:H3:*` label is recorded, it is recorded as an alias label and docked via `G.Core.TriggerAliasMap.G6`. When `G.6:Ext.BridgeSentinelWiring` is used, include the bridge/sentinel payload pins required by that extension.
...
* **Invariants.** Trigger typing and alias docking are delegated to [[G.Core - Part G Core Invariants|G.Core]] (`CC‑GCORE‑TRIG‑*`). Penalty routing invariants are delegated (`CC‑GCORE‑PEN‑1`).

### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:7.5.4 - H4 — SoS‑LOG Path Citation (selector explainability)
...
* **Invariants.** Tri‑state semantics are governed by [[G.Core - Part G Core Invariants|G.Core]] (`CC‑GCORE‑GUARD‑1`); [[G.6 - Evidence Graph & Provenance Ledger|G.6]] does not add a new decision value.

### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:7.5.5 - H5 — Maturity Rung Transition Justification
...
* **Publishes/Consumes.** Publishes: updated UTS entry for maturity artefacts. Consumes: cited paths and [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] anchors.
* **Invariants.** Any thresholding policy remains governed by CAL/LOG governing definitions; [[G.6 - Evidence Graph & Provenance Ledger|G.6]] provides citation, not policy.

### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:7.5.6 - H6 — Bridge/CL Edge Annotation (GateCrossings)
...
* **Invariants.** Crossing visibility is governed by [[G.Core - Part G Core Invariants|G.Core]] (`CC‑GCORE‑CROSS‑1`); penalties routing is governed by [[G.Core - Part G Core Invariants|G.Core]] (`CC‑GCORE‑PEN‑1`).

### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:7.5.7 - H7 — ReferencePlane penalty policy publication (ids only)
...
### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:7.5.8 - H8 — CrossingBundle exposure ([[E.18 - Transformation Flow Structure|E.18]])

* **Trigger.** [[G.6 - Evidence Graph & Provenance Ledger|G.6]] artefacts are exported for release or consumed by downstream patterns that require GateCrossing checks.
...
* **Invariants.** Crossing discipline and ID continuity are governed by [[G.Core - Part G Core Invariants|G.Core]] (`CC‑GCORE‑CROSS‑1`, `CC‑GCORE‑ID‑*`).

### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:7.5.9 - H9 — SCR surface for assurance provenance

* **Trigger.** A downstream artefact cites a path for audit/selection/maturity.
* **Obligation.** Expose the required provenance fields in SCR views: lane split, context or plane pins, freshness binding, crossing pins (when present), and links to [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] anchors and CAL refs.
...
### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:7.5.10 - H10 — ProofLedger linkage (CAL ↔ [[G.6 - Evidence Graph & Provenance Ledger|G.6]])

* **Trigger.** A proof obligation or evidence role is attached to a claim and is represented in [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] artefacts.
* **Obligation.** Link EvidenceGraph nodes/edges to CAL ProofLedger/EvidenceProfiles entries and to [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] carriers via the minimal provenance edge vocabulary.
* **Publishes/Consumes.** Publishes: CAL proof refs as pins in the path explanation surface. Consumes: CAL artefacts.
* **Invariants.** [[G.6 - Evidence Graph & Provenance Ledger|G.6]] does not redefine CAL proof semantics; it only cites them.

### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:7.5.11 - H11 — Telemetry ingest (selector & probe outcomes)
...
* **Obligation.** Ingest the observation as a run‑time evidence line (anchored in [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]), with explicit lane typing and explicit scope/time binding. If method‑specific telemetry pins are required, they are governed by Extensions (e.g., `G.6:Ext.QD_OEE_TelemetryPins`).
...
### [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:7.5.12 - Minimal conformance (hooks)
...
## [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:8 - Common Anti-Patterns and How to Avoid Them
...
* **Implicit crossings (“same entity, different context”).**
  **Avoid:** represent crossings only via explicit crossing artefacts/pins; treat edition/plane/context changes as explicit crossing‑relevant edits and trigger RSCR (governed by [[G.Core - Part G Core Invariants|G.Core]] crossing discipline).
* **Smuggling legality rules into EvidenceGraph prose.**
  **Avoid:** cite/pin legality surfaces (`CG‑Spec` and CAL artefacts); do not introduce local “mini‑CG” rules in [[G.6 - Evidence Graph & Provenance Ledger|G.6]] (cite `CC‑GCORE‑CN‑CG‑1`).
...
## [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:9 - Consequences
...
## [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:10 - Rationale

[[G.6 - Evidence Graph & Provenance Ledger|G.6]] concretizes the “because‑graph” implicit in [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] into a typed, lane‑aware DAG with stable path addresses. It relies on canonical governing definitions for semantics:

* [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] for anchoring discipline and carrier reality,
* [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] for the assurance skeleton,
* [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] for proof/evidence profile semantics,
* [[G.Core - Part G Core Invariants|G.Core]] for universal crossing, penalty, Default Governing Definition Index, and typed RSCR cause discipline.

This preserves conceptual modularity: [[G.6 - Evidence Graph & Provenance Ledger|G.6]] standardizes *addressable provenance*, not a competing legality or selection mechanism.

## [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:11 - SoTA‑Echoing
...
## [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:12 - Relations

**Builds on:** [[G.Core - Part G Core Invariants|G.Core]], [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] (evidence anchors/carriers; SCR/RSCR), [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] (assurance skeleton), [[F.9 - Alignment & Bridge across Contexts|F.9]] (BridgeCard/CL), [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] (CAL EvidenceProfiles/ProofLedger), `E.18/A.21` (GateCrossing/CrossingBundle checks), [[E.10 - Unified Lexical Rules for FPF|E.10]] (lexical rules), `E.5.*` (notation independence), [[F.17 - Unified Term Sheet (UTS)|F.17]] (UTS), [[F.15 - SCR-RSCR Harness for Unification|F.15]] (RSCR).
**Publishes to:** UTS (Name Cards + PathCards), SCR/RSCR surfaces, downstream selectors/LOG by `PathId` citation, refresh/orchestration as typed triggers (consumed by [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] when used).
**Used by:** [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] (selector explainability and admissibility justifications), [[G.8 - SoS‑LOG Bundles & Maturity Ladders|G.8]] (SoS‑LOG bundles), [[G.9 - Parity and Benchmark Harness|G.9]] (parity harness traces), [[G.10 - SoTA Pack Shipping|G.10]] (shipping pins and audit payload), [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] (refresh orchestration).
**Constrains:** downstream patterns MUST cite paths when evidence is claimed; they MUST treat edits to pinned evidence/crossing/policy/edition/time bindings as refresh‑relevant causes with canonical trigger ids (governed by [[G.Core - Part G Core Invariants|G.Core]]).

## [[G.6 - Evidence Graph & Provenance Ledger|G.6]]:End
