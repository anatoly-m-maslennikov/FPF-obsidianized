...
> **Modified:** 2026‑01‑20
>
> **Governing-pattern note, Phase‑3 canonicalization:** this pattern governs the canonical `U.Mechanism.Intension` for `CPM.IntensionRef` (CHR suite stage `compare`). Mechanism-intension semantics are governed by explicitly designated governing patterns ([[E.20 - Mechanism Introduction Protocol|E.20]]).
> [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] governs the **template** of `U.Mechanism.Intension`; this pattern governs the **CPM-specific constraints** over the SlotKind surface supplied by the suite: operations, laws, admissibility, applicability, transport, plane, and audit obligations for that template. It is not a second schema and does not govern the CHR SlotKind lexicon.
>
> **Canonicalization hook, ID‑continuity‑safe:** any other appearances of the CPM intension (e.g., suite prose in [[A.19.CHR - CHRMechanismSuite|A.19.CHR]]) SHALL be reduced to a **Tell + Cite** stub pointing to **`A.19.CPM:4.1`**, while preserving the original section headings and their public `PatternId:SectionPath` IDs for continuity (alias‑dock legacy tokens rather than deleting them).
...
## [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:0 - At a glance (didactic, informative)
...
* **P2W seam:** concrete `ComparatorSpecRef.edition` and any policy ids are bound **only** in planned baseline plan items ([[A.15.3 - SlotFillingsPlanItem|A.15.3]] + `A.19.CHR:4.7.2`). CPM’s kernel does **not** bind project‑specific pins; executions record the **effective** refs/pins in `Audit`.
* **Reproducible comparisons:** for parity/benchmark style runs that require a stable run package + report surface (editions, windows, parity pins), route packaging through [[G.9 - Parity and Benchmark Harness|G.9]] (Parity / Benchmark Harness). CPM stays kernel‑only.
...
## [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:1 - Problem frame
...
* a **mechanism** (in the `U.Mechanism.Intension` sense, per [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] / slot discipline [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]),
* that is **suite‑conformant** (per CHR obligations and protocol closure in [[A.19.CHR - CHRMechanismSuite|A.19.CHR]]),
...
## [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:2 - Problem
...
## [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:3 - Forces
...
6. **Avoiding “second centers of gravity”:** mechanism semantics must have a governing pattern; otherwise the suite, [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] archetypes, and Part‑G wiring drift apart.

## [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:4 - Solution
...
### [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:4.1 - Mechanism.Intension (canonical; normative)
...
* **Scope note:** this intension is an **instance** authored to the `U.Mechanism.Intension` shape ([[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]). It does **not** publish/telemetry, does **not** publish `GateDecision` nor `DecisionLog` surfaces (gate‑only), and does **not** embed selection. It emits `Audit` pins and a tri‑state guard only (per suite obligations).
  * **P2W separation:** this intension does **not** bind project‑specific pins (editions, policy‑ids, bridge ids, etc.). Binding lives in planned baseline plan items ([[A.15.3 - SlotFillingsPlanItem|A.15.3]] + `A.19.CHR:4.7.2`); executions record effective refs/pins in `Audit`.
...
* **LawSet** (minimum; set‑valued comparison, no hidden scalarization):
  ...
  1. **ComparatorSet gate:** `ComparatorSpecSlot` MUST be an element of `CGSpecSlot.ComparatorSet` (legality gate; cite [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]).
  ...
  3. **CSLC+SCP legality:** any numeric ops implied by the comparator MUST be admissible under `CGSpecSlot.SCP` and CSLC‑lawful (cite [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]] + [[A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC‑KERNEL)|A.18]]).
...
### [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:4.2 - Interpretation notes — informative
...
## [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:5 - Archetypal Grounding — informative

### [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:5.1 - Tell
...
### [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:5.2 - Show (U.System) — comparing two supplier options without faking a total order
...
### [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:5.3 - Show (U.Episteme) — uncertainty‑aware comparison with set‑valued outcomes
...
## [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:6 - Bias-Annotation — informative
...
## [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:7 - Conformance Checklist
...
| :--- | :--- | :--- |
| **CC‑A19CPM‑0** | **Mechanism.Intension completeness.** The publication includes the full intension shape (header/imports/subject/slot index/op algebra/laws/admissibility/applicability/transport/time/plane/audit) and uses tri‑state guards. | SlotIndex is **derived**; see [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] + `CC‑UM.*`. |
...
| **CC‑A19CPM‑12** | **P2W separation.** Editions/policy‑ids are bound only in planned baseline plan items; CPM records effective refs/pins in Audit and does not bind them. | Planned baseline = [[A.15.3 - SlotFillingsPlanItem|A.15.3]] + suite PlanItem. |
...
## [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:8 - Common Anti‑Patterns and How to Avoid Them
...
## [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:9 - Consequences
...
## [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:10 - Rationale
...
## [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:11 - SoTA-Echoing

**SoTA vs popular note.** This section records alignment to post‑2015 evidence‑backed practice. It is **not** a mandate to use fashionable methods; method semantics stay in SoTA packs ([[G.2 - SoTA Harvester & Synthesis|G.2]]) and wiring modules, while this pattern fixes the stable CPM mechanism boundary.

**Pack note (Phase‑3).** If/when a CPM‑specific [[G.2 - SoTA Harvester & Synthesis|G.2]] SoTA pack/ClaimSheet is introduced, prefer citing the pack’s `ClaimSheetId`(s) over raw bibliographic pointers below, keeping CPM’s kernel semantics unchanged.
...
## [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:12 - Relations

**Builds on / cites (non‑exhaustive):**

* [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] (shape of `U.Mechanism.Intension`; specialization discipline)
* [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] (slot discipline; SlotIndex as derived projection)
* [[A.19.CHR - CHRMechanismSuite|A.19.CHR]] (suite membership + obligations + `suite_protocols`; CHR SlotKind lexicon)
* [[A.15.3 - SlotFillingsPlanItem|A.15.3]] + `A.19.CHR:4.7.2` (P2W planned baseline binding; CPM remains refs‑only w.r.t. pin binding)
* [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]] (CN‑Spec comparability routing + acceptance/admission surfaces)
* [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]] (CG‑Spec: `ComparatorSet`, `SCP`, `MinimalEvidence`, CL/ReferencePlane framing)
* [[A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC‑KERNEL)|A.18]] (CSLC scale lawfulness)
* [[E.10 - Unified Lexical Rules for FPF|E.10]] (lexical/ontological authoring rules; kind suffix discipline)
* [[E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]] (checks; authoring discipline)
* [[E.20 - Mechanism Introduction Protocol|E.20]] (governing-pattern discipline)
* [[F.18 - Local‑First Unification Naming Protocol|F.18]] (alias docking; ID continuity)
* [[E.18 - Transformation Flow Structure|E.18]] (project transformation-flow structures consume CPM instances; CPM does not create a parallel “card deck”)
...
* [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] (selection conformance), [[G.9 - Parity and Benchmark Harness|G.9]] (parity / benchmark harness), [[G.10 - SoTA Pack Shipping|G.10]]/PTM (publish/telemetry outside suite closure).

## [[A.19.CPM - Unified Comparison Mechanism (CPM)|A.19.CPM]]:End
