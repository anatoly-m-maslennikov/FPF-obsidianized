...
> **One‑line summary.** Establishes **`U.Kind`** as the **minimal, context‑local intensional carrier** of “what a statement is about,” separates **intent** (KindSignature + its own **F**) from **extent** (*which* instances belong to the kind **in a given Context slice**), and situates **typed reasoning** alongside **USM Scope (G)** and **F–G–R** without conflation. Details of the core objects and operations live in **[[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]]–[[C.3.5 - KindAT - Intentional Abstraction Facet for Kinds (K0…K3)|C.3.5]]**; guard shapes are standardized in **[[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]**.

**Status.** Normative in **Part C**. Identifier **[[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]**. This pattern lays the **architectural invariant** and manager‑level guidance. The **mechanics** are defined by its child patterns.
...
**Depends on.**
— **[[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] USM** (Context slices & Scopes): **`U.ClaimScope` = G**, **`U.WorkScope`**, ∈/∩/**SpanUnion**/translate, **Γ\_time** policy, Bridges + **CL** (scope).
— **[[C.2.2 - Reliability R in the F-G-R triad|C.2.2]] F–G–R**: weakest‑link composition; penalties to **R** for Cross‑context congruence (CL).
— **[[C.2.3 - Unified Formality Characteristic F|C.2.3]] Unified Formality F**: F0…F9 as an **ordinal Characteristic** (expression rigor).

**Sub‑patterns (normative unless noted).**
— **[[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]]** - `U.Kind` & `U.SubkindOf` (partial order).
— **[[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]]** - `KindSignature` (**intent**, with **F**) & `Extension/MemberOf` (**extent** in a slice).
— **[[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]** - **KindBridge** & **`CL^k`** (type‑congruence; penalties route to **R**).
— **[[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]** - **RoleMask** (context‑local adaptation without cloning kinds).
— **[[C.3.5 - KindAT - Intentional Abstraction Facet for Kinds (K0…K3)|C.3.5]]** - **KindAT** (K0…K3, **informative facet**, not a Characteristic).
— **[[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]** - **Typed Guard Macros** (annex): admit/compose, masks, Cross‑context reuse; AT is **forbidden** in guards.
...
— **Mark as legacy** any uses of **‘validity’ as a Characteristic** or **‘operation’ as a Scope‑like Characteristic**; **redirect** to **`U.ClaimScope`** / **`U.WorkScope`** ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]) for applicability. Editors SHOULD add glossary redirects in Part E.

**Editorial note (cut‑over).** Content formerly in [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]] that defined guard shapes, decision trees, and macro anti‑patterns now resides in **[[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]**. Membership **evaluation obligations** live in **[[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]]** with `MemberOf`.

## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:1 - Purpose & Rationale
...
2. A clean **split of characteristics**:
   ...
   – **F** = *how strictly* content is expressed ([[C.2.3 - Unified Formality Characteristic F|C.2.3]]).
...
## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:2 - Context
...
## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:3 - Problem
...
## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:4 - Forces
...
## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:5 - Solution — Architectural Decisions (overview)

**[[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]‑D1 — `U.Kind` is intensional and context‑local.**
Kinds name *what a claim quantifies over*. They form a partial order **`⊑`** (**SubkindOf**). *(See [[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]].)*

**[[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]‑D2 — Separate **intent** and **extent**.**
— **KindSignature(k)**: the intensional content (predicates/invariants/Standards). It carries its **own F** ([[C.2.3 - Unified Formality Characteristic F|C.2.3]]).
— **Extension(k, slice)**/**MemberOf**: which instances belong to `k` **in a given `U.ContextSlice`**. *(See [[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]].)*

**[[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]‑D3 — Kinds do **not** carry Scope.**
**Scope** lives with **claims/capabilities** (USM): a set of **Context slices** where the statement holds. Kinds carry **intent/extent** only. *(USM [[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] + [[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]].)*

**[[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]‑D4 — Typed reuse across Contexts is explicit.**
Use a **KindBridge** with **`CL^k`** (type‑congruence) and loss notes. Its effect is **only via R** penalties; **F/G remain unchanged**. *(See [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]].)*

**[[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]‑D5 — Local adaptation without cloning.**
Use a **RoleMask** to bind a kind to Context‑specific constraints/aliases; promote to a **subkind** if the mask becomes stable and widely reused. *(See [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]].)*

**[[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]‑D6 — An **informative** “abstraction tier” exists for Kinds (AT: K0…K3).**
A facet (not a Characteristic) that helps plan **ΔF/ΔR** and forecast bridge style; **AT never appears in guards**. *(See [[C.3.5 - KindAT - Intentional Abstraction Facet for Kinds (K0…K3)|C.3.5]].)*

**[[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]‑D7 — Guard shapes are standardized and fail‑closed.**
Typed compatibility first (same‑Context **`⊑`** or **KindBridge**), then **Scope coverage** (USM), then **R** penalties and freshness. *(See [[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]].)*
...
## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:6 - Core Concepts (informative summary; authoritative norms live in [[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]]–[[C.3.5 - KindAT - Intentional Abstraction Facet for Kinds (K0…K3)|C.3.5]])

> This section fixes the **Standard** of terms used in [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]] and points to the sub‑patterns for complete mechanics. All “**SHALL/MUST**” statements here are normative.
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:6.1 - `U.Kind` & `U.SubkindOf (⊑)`
...
**Summary of norms** *(authoritative text: **[[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]] K‑01–K‑02**)*.
...
> *Full treatment:* **[[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]]** (definitions, invariants, examples).

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:6.2 - **KindSignature** (intent) & **F**

**Definition.** `KindSignature(k)` is the **intent**: predicates/invariants/Standards that define the kind in the Context. Its expression rigor has an explicit **`U.Formality`** ([[C.2.3 - Unified Formality Characteristic F|C.2.3]]).

**Summary of norms** *(authoritative text: **[[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]] K‑03–K‑04**)*.
— `KindSignature(k)` declares its F ([[C.2.3 - Unified Formality Characteristic F|C.2.3]]). Claim‑level F does **not** auto‑inherit; weakest‑link applies on the claim’s own support paths.
— If a signature change alters membership, treat it as a content change (Contexts may version kinds).

> *Full treatment:* **[[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]]** (signature/intent with F; relation to claims).

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:6.3 - **Extension** & **MemberOf** (extent in a slice)
...
**Summary of norms** *(authoritative text: **[[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]] K‑05–K‑08**)*.
...
> *Full treatment:* **[[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]]** (extent semantics, examples, authoring hints).

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:6.4 - **KindBridge** & **`CL^k`** (type‑congruence)

**Summary of norms** *(authoritative text: **[[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]] KB‑01–KB‑12**)*.
...
> *Full treatment:* **[[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]** (bridge shape, anchors, examples).

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:6.5 - **RoleMask** (adaptation without cloning)
...
**Summary of norms** *(authoritative text: **[[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]] RM‑01–RM‑08**)*.
...
> *Full treatment:* **[[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]** (mask taxonomy, guard discipline, promotion rule).

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:6.6 - **KindAT (K0…K3)** — *informative facet*
...
> *Full treatment:* **[[C.3.5 - KindAT - Intentional Abstraction Facet for Kinds (K0…K3)|C.3.5]]** (manager heuristics, anti‑misuse).

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:6.7 - Quick examples (two‑characteristic awareness)
...
## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7 - How to use typed reasoning

## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.1 How typed reasoning plugs into **F–G–R & USM**

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.1.1 - The basic shape of a typed claim (manager view)
...
1. **entityOfConcern (Kind).** *Which things the statement talks about.*
   ...
   — The **definition** of kind **k** lives in **KindSignature(k)** (with its **F**, [[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]]).
   — **Which items count as “k”** is evaluated in the **TargetSlice** ([[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]]) by a deterministic membership check.

2. **Applicability (Scope, G).** *Where the statement holds.*
   `U.ClaimScope(Claim)` is the **collection of contexts** where the claim is valid (USM [[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]). Guards test: “Scope **covers** the TargetSlice”.

**Discipline.** The guard first checks **typed compatibility** (in the same Context: “is‑a / subkind‑of”; across Contexts: a **KindBridge**, [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]), then **Scope coverage** (USM), then **R** freshness and any bridge congruence penalties. See **[[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]] Guard\_TypedClaim**.

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.1.2 - Composition of typed claims
...
* **same Context:** require **“is‑a / subkind‑of”** to hold (the source kind is a subkind of the expected kind) ([[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]]).
* **Cross‑context:** require a **KindBridge** that maps the source kind to a **local kind that is a subkind of the expected kind** in the target Context ([[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]). If neither holds, the composition is **unsafe**; introduce a subkind, add an adapter (or a RoleMask), or decline.

* **Role‑aware option (same Context):** if the consumer expects a **RoleMask** over the expected kind, you may satisfy the mask’s explicit constraints ([[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]) instead of changing kinds, provided those constraints are observable at gate time.
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.1.3 - F–G–R with typed claims (what changes, what doesn’t)

* **F (Formality).**
  – **Claim‑level F** follows [[C.2.3 - Unified Formality Characteristic F|C.2.3]] (weakest‑link along the claim’s support paths).
  – **KindSignature F** is declared **on the kind** ([[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]]) and influences claims **only** if the claim essentially depends on those predicates (weakest‑link again).
  – **Raising F** can *reveal* hidden assumptions (which may trigger ΔG in the claim), but **does not change G** by itself.

* **G (Scope).**
  – Always **set‑valued over Context slices** (USM [[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]).
...
* **R (Reliability).**
  ...
  – **Cross‑context penalties** split cleanly: a **scope‑bridge penalty** (USM) and a **kind‑bridge penalty** ([[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]). Both **reduce R only**; neither changes **F** or **G**.
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.1.4 - ESG gating with typed claims
...
* **Do not** gate on **AT** ([[C.3.5 - KindAT - Intentional Abstraction Facet for Kinds (K0…K3)|C.3.5]]); it is an informative facet only.
* Use **[[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]] guard macros** to keep guards short and auditable.

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.2 - How typed reasoning plugs into the CAL chain (Lang‑CHR → Role‑CAL)
...
#### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.2.1 - **Lang‑CHR** — stable words first
...
**What it requires.** Authors use controlled narrative ([[C.2.3 - Unified Formality Characteristic F|C.2.3]] **F3**) at minimum: single‑meaning terms, explicit “shall / if / then”, and no drifting synonyms.
...
#### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.2.2 - **Kind‑CAL** (this Part) — name the *entityOfConcern*
...
#### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.2.3 - **Structure‑CAL** — give Kinds usable shape
...
#### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.2.4 - **Compose‑CAL** — compose with typed pre‑checks

**What it supplies.** The **order of checks** you must follow for safe composition:

1. **Typed compatibility**: in the same Context, the producer’s kind **is a subkind of** the consumer’s kind; across Contexts, a **KindBridge** maps the producer’s kind to a local kind that fits, with an acceptable **kind‑bridge congruence level** ([[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]).
...
#### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.2.5 - **CT2R‑LOG** — speak the logic, keep the math honest
...
#### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.2.6 - **Role‑CAL** — adapt without cloning
...
#### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.2.7 - Mini end‑to‑end example (manager‑oriented)
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.3 - Compliance & Regulatory Alignment — profile
...
> **Authoritative obligations and guard macros** (C‑REG‑1…8, Guard_RegAdopt / Guard_RegChange / Guard_RegXContextUse) and worked scenarios live in **[[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]], Annex A (Regulatory adoption profile)**.

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.4 - How typed reasoning plugs into **Assurance Lanes (VA/LA/TA) & Evidence design**
...
> **Authoritative evidence obligations and guard macros** (planning/attachment, VA/LA/TA duties, anti‑patterns) are in **[[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]], Annex B**.

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.5 - How typed reasoning plugs into **ESG and Method–Work gating**
...
#### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.5.1 - Scope & fit
...
* **USM** ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]): `U.ClaimScope` (G) and `U.WorkScope` coverage + `Γ_time`,
* **Kind‑CAL core** ([[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]]–[[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]]): `U.Kind`, `MemberOf(e,k,slice)`, `⊑`,
* **KindBridge** ([[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]) with **`CL^k`** and loss notes,
* **Scope Bridge** (Part B) with **CL** and loss notes,
* **RoleMask** ([[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]) when local adaptations of a kind are used,
* **Formality F** ([[C.2.3 - Unified Formality Characteristic F|C.2.3]]) when transitions gate on rigor,
* **Assurance R** ([[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]) for evidence freshness and penalties Φ/Ψ.

**Guard macros.** The **normative guard shapes** for ESG and Method–Work (**Guard_TypedClaim**, **Guard_TypedJoin**, **Guard_MaskedUse**, **Guard_XContext_Typed**) are specified in **Annex [[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]**. Use those shapes; the present section is a manager‑level overview only.

#### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.5.2 - Inputs & roles (at guard time)
...
#### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.5.3 - Manager’s 7‑step checklist (operational)
...
#### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:7.5.4 - Cross‑references

* **USM / [[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]:** Scope coverage, `Γ_time`, serial **∩**, **SpanUnion**, Bridge+CL.
* **Kind‑CAL / [[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]]–[[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:** `U.Kind`, `⊑`, `MemberOf`, RoleMask, KindBridge + **`CL^k`**.
* **Formality / [[C.2.3 - Unified Formality Characteristic F|C.2.3]]:** `U.Formality` thresholds (when ESG gates on rigor).
* **Assurance / [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]]:** Freshness windows; **Φ(CL)** and **Ψ(`CL^k`)** penalties to **R** (weakest‑link on paths).

This subsection is **normative** for guards in ESG and Method–Work that **use kinds**.

## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:8 - Cross‑context typed reuse & assurance accounting

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:8.1 - The **two‑bridge rule** (mandatory)

When any part of the use crosses Contexts:

1. **Scope Bridge** (USM/Part B) with **CL** → penalty **Φ(CL)** to **R**.
2. **KindBridge** ([[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]) with **`CL^k`** → penalty **Ψ(`CL^k`)** to **R**.

Both bridges carry **loss notes**; neither changes **F** or **G**. See **[[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]] Guard\_XContext\_Typed**.

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:8.2 - Narrowing after mapping (best practice)
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:8.3 - Typical Cross‑context patterns (manager’s catalog)
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:8.4 - Decision pattern (fast path)
...
## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:9 - Authoring guidance (engineers‑managers)

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:9.1 - When to mint a `U.Kind`
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:9.2 - Writing a **KindSignature** (and picking **F**)
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:9.3 - Setting the **AT** facet (K0…K3)
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:9.4 - Writing a typed claim (with USM blocks)
...
* **Guards:** use **[[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]] Guard\_TypedClaim**; if Cross‑context, add **Guard\_XContext\_Typed** (two‑bridge rule).
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:9.5 - Minimal “Kind card” contents (Context catalog)
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:10.1 - Reviewer’s 8‑point checklist
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:10.2 - Integrator’s composition playbook (typed first, then scope)
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:10.3 - Assurance lead: wiring penalties and windows
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:10.4 - Red flags (stop‑the‑line)
...
## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:11 - Worked examples (end‑to‑end)

> *Each example shows the typed pre‑check, Scope composition, penalties to **R**, and the managerial decision. Full guard clauses for these scenarios are in **Annex [[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]**.*

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:11.1 - Cyber‑physical braking policy across labs and plants
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:11.2 - API decision rule with adapter and subkind promotion
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:11.3 - Clinical dosage rule across jurisdictions (bridge + mask)
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:11.4 - ML fairness constraint with typed quantification
...
## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:12 - Anti‑patterns & how to fix them
...
| ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| **“We widened G because we reworded the type.”**                 | Confuses **entityOfConcern** (kind) with **applicability** (scope). Abstract wording ≠ broader scope. | Keep typed pre‑check separate ([[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]] `⊑` or [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]] KindBridge). Widen **G** only via **ΔG+** with support (USM [[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]).                                 |
| **“Kind scope” block attached to a Kind.**                       | Kinds don’t carry Scope; they carry **intent/extent**.                                          | Remove the block. Scope stays on claims/capabilities (USM). If you meant classifier definedness, state it via **K‑07** ([[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]]).                       |
...
| **Mask used as a hidden kind (“just call it the masked kind”).** | Opaque drift; reviewers can’t see constraints.                                                  | Register a **RoleMask** ([[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]). If reused across guards, **promote to subkind** with `⊑`.                                                            |
...
| **Using AT (K0…K3) as a gate/threshold.**                        | AT is an **informative facet**, not a Characteristic; gating on AT recreates a G‑ladder.        | Remove AT from guards. Use it only to **aim ΔF/ΔR** and to set **bridge expectations** ([[C.3.5 - KindAT - Intentional Abstraction Facet for Kinds (K0…K3)|C.3.5]]).                                                       |
| **“Automated execution success proves the type claim.”**                            | Execution success (F5/6) is not proof (F7+); also confuses **R** with **F**.                    | If you need proof‑grade properties, raise **F** for the claim/KindSignature ([[C.2.3 - Unified Formality Characteristic F|C.2.3]]) or restrict the claim. Keep **R** as evidence freshness/coverage. |
| **Hidden “latest” in membership or scope checks.**               | Non‑deterministic evaluation; unverifiable audit trail.                                         | Declare **Γ\_time** explicitly in Scope (USM). Membership must be **deterministic** ([[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]] K‑05/K‑07).                                                |
...
| **Collapsing subkinds silently in a bridge.**                    | Reviewers don’t see lost distinctions → false confidence.                                       | Record **loss notes** on the KindBridge ([[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]] KB‑11); consider **narrowing** mapped Scope or adding an adapter.                                      |

## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:13 - Governance & conformance pull‑ups

> *Contexts adopt Kind‑CAL by meeting the **Context‑level** obligations below. They summarize, not duplicate, the formal requirements in **[[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]]–[[C.3.5 - KindAT - Intentional Abstraction Facet for Kinds (K0…K3)|C.3.5]]** and **[[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]**. Use this as an adoption checklist.*

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:13.1 - Context‑level obligations (must‑haves)

1. **Kinds & order.** Maintain a Context catalog of `U.Kind` with an explicit **partial order** `⊑`.
   – Conformance: **[[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]]** (K‑01/K‑02).

2. **Kind signatures (intent).** For each Kind, publish a **KindSignature** with declared **F** ([[C.2.3 - Unified Formality Characteristic F|C.2.3]]).
   – Conformance: **[[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]]** (K‑03/K‑04).

3. **Deterministic membership.** Ensure `MemberOf(e,k,slice)` is **deterministic**; declare any definedness domain.
   – Conformance: **[[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]]** (K‑05/K‑07).

4. **Typed guards.** When a claim quantifies over kinds, guards SHALL use the **typed guard macros** (or equivalents) from **[[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]**; **Scope coverage** and **typed checks** are separate predicates.
...
5. **Role masks.** If a local projection is needed, register a **RoleMask** (type: constraint/vocabulary/composite); avoid cloning kinds.
   – Conformance: **[[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]** (RM‑01…RM‑06).

6. **Two‑bridge rule for Cross‑context use.**
   – **Scope Bridge** (Part B) with **CL** → Φ(CL) to **R**.
   – **KindBridge** ([[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]) with **`CL^k`** → Ψ(`CL^k`) to **R**.
   – Conformance: **[[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]** (KB‑01…KB‑10).
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:13.2 - ESG / Method‑Work template inserts (normative snippets)
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:13.3 - Audits & levels of adoption (informative)

* **USM‑Typed‑Ready.** Catalog exists; `⊑` declared; guard macros installed.
* **USM‑Typed‑Guarded.** All typed claims use **[[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]** guard shapes; **Γ\_time** explicit; two‑bridge rule enforced.
...
## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:14 - Migration & editorial impact
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:14.1 - Inventory & refactor (steps)
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:14.2 - Edits to other parts (normative redirects, no new math)

* **[[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] (USM).**
...
* **[[C.2.2 - Reliability R in the F-G-R triad|C.2.2]] (F–G–R).**
...
* **[[C.2.3 - Unified Formality Characteristic F|C.2.3]] (F).**
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:14.3 - Alias and record continuity
...
## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:15 - Extended rationale & design notes  \[I]
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:15.1 - Why **no Scope on kinds**

Scope answers **“where the claim holds”** (set of Context slices, USM); kinds answer **“what the claim is about”**. Putting Scope on kinds would either (a) duplicate claim Scope, or (b) smuggle applicability into a classifier. We prevent both by: **intent/extent on kinds** ([[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]]), **Scope on claims/capabilities** (USM).

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:15.2 - Why **two bridges** (Scope vs Kind)
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:15.3 - Why **AT is a facet**
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:15.4 - Why **RoleMask** and not “clone a kind”
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:15.5 - Fit with **Compose‑CAL** and **LOG‑CAL**
...
### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:15.6 - CT2R lens (intuition)
...
## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:15bis - Rationale (Part E form)
...
**Decision ([[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]‑D1…D7).**
...
— ESG guard shapes in **[[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]**; typed pre‑check in Compose‑CAL (§7.2.4).

**Related patterns.**
[[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] (USM), [[C.2.2 - Reliability R in the F-G-R triad|C.2.2]] (F–G–R), [[C.2.3 - Unified Formality Characteristic F|C.2.3]] (F), Part B (Bridges), Role‑CAL, Compose‑CAL, [[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]]–[[C.3.5 - KindAT - Intentional Abstraction Facet for Kinds (K0…K3)|C.3.5]], [[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]].

## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:16 - Quick reference for managers

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:16.1 - 10‑minute start
...
4. Use the **typed guard macro** ([[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]).
5. Route bridge levels to **R** (Φ/Ψ); check freshness.

### [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:16.2 - 30‑day rollout plan
...
## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:17 - Local glossary (reading aid)

> *Canonical definitions live in sub‑patterns; this list is for quick recall while reading [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]].*

* **`U.Kind`** — Minimal intensional “type/kind” object; carries **KindSignature** and **`⊑`** ([[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]].1/C.3.2).
* **`U.SubkindOf (⊑)`** — Partial order on kinds ([[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]]).
* **KindSignature(k)** — Predicate‑like intent that defines the kind; has its own **F** ([[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]]).
* **Extension(k, slice)** — Set of instances of `k` **inside** a `U.ContextSlice` ([[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]]).
* **MemberOf(e, k, slice)** — Boolean membership predicate ([[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]]).
* **RoleMask(k, Context)** — Registered adaptation (constraints/aliases; optional narrowing), no new kind ([[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]).
* **KindBridge** — Cross‑context mapping for kinds (intent/order) with **`CL^k`** and loss notes ([[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]).
* **`CL^k`** — Kind‑congruence level; penalty **Ψ(`CL^k`)** goes to **R** ([[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]).
* **AT (K0…K3)** — Informative facet of a Kind; aids planning/navigation; never used in guards ([[C.3.5 - KindAT - Intentional Abstraction Facet for Kinds (K0…K3)|C.3.5]]).
* **Guard macros** — Typed guard shapes for ESG/composition ([[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]).

> *End of [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]. See **[[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]]–[[C.3.5 - KindAT - Intentional Abstraction Facet for Kinds (K0…K3)|C.3.5]]** and **[[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]** for the referenced mechanics and guard macros.*

## [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]]:End
