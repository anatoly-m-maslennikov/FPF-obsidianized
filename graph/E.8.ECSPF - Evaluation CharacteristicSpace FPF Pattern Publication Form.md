...
## [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]]:1 - Problem frame

Use [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]] when an evaluation `CharacteristicSpace` constructed or repaired under [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] must be published as an FPF pattern. The question is not "what values should this evaluated object be judged by?" but "how do we write the FPF pattern publication form so those values remain usable, reviewable, and bounded?"

[[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] governs the evaluation characteristic-space specification: evaluated object kind, use scope, contrast cases, coordinate set, value meanings, evidence basis, result-row shape, calibration points, coordinate-specific payloads, missingness, protected trade-offs, status meanings, and stop or reopen conditions. [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] governs ordinary FPF authoring form. [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]] governs their intersection: an FPF pattern whose main payload is a reusable evaluation.

**Not this pattern when.** Use [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] when the characteristic-space specification itself is missing or inadequate. Use [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] when the pattern is not an evaluation-characteristic-space pattern. Use [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]], [[E.9.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]], [[E.2.DA - FPF Pillar-Adequacy Evaluation CharacteristicSpace|E.2.DA]], [[F.18 - Local‑First Unification Naming Protocol|F.18]], [[C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]], or a project-local evaluation when one already supplies the value meanings for the evaluated object and use. Use [[E.22 - Improvement-Oriented Quality Evaluation Question Framing|E.22]] to frame one quality evaluation and [[E.23 - Quality Improvement Loop Method|E.23]] to run repeated improvement. Use a local rubric, table, or project rule instead of an FPF pattern when the evaluation is not intended for durable FPF reuse.

**First useful move.** Start from the accepted [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] specification. Name the evaluated object kind, declared use, and first action-guiding evaluation use in the pattern's recognition text before presenting coordinate tables or conformance rows.

**FPF-publication boundary.** If the evaluation is local, temporary, or project-specific, do not publish an FPF pattern. Keep the [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] specification in the local publication form and cite the FPF neighbouring patterns named by value it uses.
...
**What this buys.** [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]] lets FPF publish evaluations as real patterns: practitioner-readable first, exact enough for review, and bounded enough that [[E.22 - Improvement-Oriented Quality Evaluation Question Framing|E.22]] and [[E.23 - Quality Improvement Loop Method|E.23]] can consume them without stealing their values.
...
## [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]]:2 - Problem

[[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] can produce a good evaluation characteristic-space specification without saying how to publish that specification as an FPF pattern. [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] can produce a good generic FPF pattern without saying how a coordinate set, object-kind-fit rule, evidence basis, result-row shape, calibration points, status set, and stop condition should be placed when they are the pattern's main payload.
...
7. **Pattern-quality confusion.** The author uses [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] to judge whether the FPF pattern version is good, but forgets that the new pattern must still publish the evaluation for one evaluated object kind by value.
8. **Quality-carrier leakage.** [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] values, corpus projection, README/ToC/E.11/I.2 alignment, retrieval, cold-reader evidence, monolith parity, landing evidence, or developer/reviewer/executor correspondence for the publication form is written into the evaluation pattern as if it were the evaluated object's method.

## [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]]:3 - Forces
...
| **Generic [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] form vs ECS payload** | The canonical pattern skeleton stays fixed, but the payload has special fields from [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]. |
...
| **Evaluation of object vs evaluation of FPF pattern version** | The evaluation judges its evaluated object; [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] may separately evaluate whether the authored FPF pattern publication form is good enough. |

## [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]]:4 - Solution

When an [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] specification is selected for durable FPF publication, author the evaluation as an [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] pattern with these additional placement rules:
...
6. **Keep outside claims with governing patterns.** `Relations` and compact non-use boundaries name the governing pattern for evidence, assurance, gate, work, decision, naming, measurement, OEE/NQD, mathematical lens, [[E.22 - Improvement-Oriented Quality Evaluation Question Framing|E.22]] quality-evaluation, and improvement-loop claims. They do so declaratively and do not replace the evaluation publication form with reference boilerplate, phrase apparatus, or architecture-placement rationale. If phrase-level apparatus appears, apply [[F.19 - Ontology-First Plain Technical Rewriting|F.19]]; if remaining words still hide precision, apply [[E.10 - Unified Lexical Rules for FPF|E.10]], [[E.10.ARCH - Wording-Use Ontological Precision Restoration Architecture|E.10.ARCH]], [[F.18 - Local‑First Unification Naming Protocol|F.18]], or the governing pattern. If a wording repair changes an FPF-governed phrase in the evaluation specification or publication form, the pre-repair and post-repair evaluated object kind, relation or claim kind, slot or use-position when live, admissible use, and scope must remain recoverable; lexical substitution without that check and governing-pattern reference when another pattern governs the kind under repair, relation, claim, or position is not a repair.
7. **Evaluate the publication form with [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]].** When the FPF pattern publication form is under quality improvement, [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] evaluates the FPF pattern version's quality. The evaluation coordinates inside the pattern continue to judge the evaluated object declared by that evaluation. The [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] result, corpus-projection evidence, README/ToC/E.11/I.2 alignment, retrieval or cold-reader evidence, monolith parity, landing evidence, and developer/reviewer/executor correspondence stay in the quality, review, projection, or release carriers unless the publication form's own `EntityOfConcern` and user move are that evaluation/projection work.

The authoring flow and the quality-improvement flow are different flows. This pattern publishes an evaluation for its declared evaluated object kind. A later [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] evaluation of this pattern is evidence about the publication form as an FPF pattern, not part of the evaluation that the pattern publishes. That evidence may cause edits to recognition text, coordinates, cases, or boundaries, but it remains outside the pattern unless rewritten as user-facing evaluation guidance.

### [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]]:4.1 - Canonical placement table

| [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] section | ECS-specific payload |
|---|---|
...
| `Rationale` | Why this coordinate set and publication-form are selected, including relation to [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] and existing evaluations named by value. |
...
| `Relations` | [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]], [[E.8 - FPF Authoring Conventions & Style Guide|E.8]], [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]], [[E.22 - Improvement-Oriented Quality Evaluation Question Framing|E.22]], [[E.23 - Quality Improvement Loop Method|E.23]], and exact domain or neighbour patterns. |

### [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]]:4.2 - Local names and kind settlement
...
| `ECSPayload` | The by-value [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] specification inside the pattern. | Not an arbitrary table or checklist. |
...
| `PatternVersionQualityEvaluation` | Optional [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] evaluation over the authored pattern publication form. | Not a replacement for the evaluation for one evaluated object kind and not publication-form method content. |

## [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]]:5 - Archetypal Grounding
...
**Show, pattern-quality evaluation.** [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] is an evaluation for one FPF pattern version. Its publication form must still open with the working question "is this pattern good enough for the declared use?" before showing coordinates such as first-move recoverability, boundary fit, and SoTA binding.

**Show, local rubric that should not become an FPF pattern.** A project team defines a temporary rubric for choosing a meeting room. The [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] specification may be adequate locally, but no durable FPF pattern is needed because the evaluated object kind and use do not recur across FPF practice.
...
## [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]]:6 - Bias-Annotation
...
## [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]]:7 - Conformance Checklist

| Check | Requirement | Why |
|---|---|---|
| `CC-E8ECSPF-1` | The pattern publication form SHALL name the [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] evaluation characteristic-space specification or carry its evaluated object kind, use, object-kind-fit rule, coordinate set, value meanings, evidence basis, result-row shape, calibration points, coordinate-specific payloads, missingness, trade-offs, status, and stop condition by value. | Prevents publication-form/content collapse. |
...
| `CC-E8ECSPF-6` | Relations SHALL name governing patterns for evidence, assurance, gate, work, decision, naming, measurement, OEE/NQD, mathematical-lens, [[E.22 - Improvement-Oriented Quality Evaluation Question Framing|E.22]] quality-evaluation, and improvement-loop claims when the publication form makes those claims. They SHALL keep pattern application and relation kind explicit, keep simple relations free of phrase apparatus, and keep ordinary references or architecture-placement reasoning out of publication-form evaluation prose. | Prevents a second ontology or apparatus-overwrapped publication form. |
...
| `CC-E8ECSPF-7` | If the authored publication form is under improvement, [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] SHALL evaluate FPF pattern-version quality separately from the evaluation's evaluated object result. | Keeps pattern quality distinct from evaluated object quality. |
...
| `CC-E8ECSPF-12` | The publication form SHALL keep [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] values, `PatternQualityStatus`, corpus-projection evidence, README/ToC/E.11/I.2 alignment, card/retrieval evidence, cold-reader evidence, monolith parity, landing evidence, developer/reviewer/executor correspondence, and other quality-carrier facts out of the pattern. These facts belong in the [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] result, [[E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]] run record, README/ToC/E.11/I.2, card/retrieval/projection carrier, or release/landing evidence carrier unless the role test shows that the publication form's own `EntityOfConcern` and user move are that evaluation/projection work. | Prevents quality of the publication form from replacing the evaluation published by the pattern. |

## [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]]:8 - Common Anti-Patterns and How to Avoid Them
...
| **Frozen evaluation publication form.** | The evaluated EntityOfConcern kind, use, use of a cited source, source adoption/adaptation/rejection decision, or coordinate meanings change, but the pattern keeps the old values as if still current. | Reopen [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] for the evaluation EntityOfConcern and state whether earlier evaluation results remain comparable, need a bridge, or must be retired. |
...
| **Pattern-quality report as evaluation pattern.** | [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] status, all-`4`/all-`5` posture, corpus projection, retrieval evidence, README/ToC/E.11/I.2 alignment, monolith parity, landing readiness, or role-turn correspondence appears anywhere in the pattern as if it were the evaluation method. | Move that evidence to the quality/review/projection/release carrier and keep the pattern body focused on the evaluation for the declared evaluated object kind. |
| **Apparatus-overwrapped publication form.** | The evaluation relation is written through role, carrier, locus, flow, status, or package words that add no evaluated object kind, coordinate meaning, evidence rule, user move, or flow-role distinction. | Apply [[F.19 - Ontology-First Plain Technical Rewriting|F.19]]; if remaining content still hides a word/head/use, apply [[E.10 - Unified Lexical Rules for FPF|E.10]], [[E.10.ARCH - Wording-Use Ontological Precision Restoration Architecture|E.10.ARCH]], [[F.18 - Local‑First Unification Naming Protocol|F.18]], or the governing pattern. |

## [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]]:9 - Consequences

A conforming [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]] publication form makes an evaluation findable, teachable, and reusable inside FPF. It lets [[E.22 - Improvement-Oriented Quality Evaluation Question Framing|E.22]] frame quality evaluations and [[E.23 - Quality Improvement Loop Method|E.23]] run improvement loops without re-inventing values. It also makes the cost visible: a reusable evaluation-characteristic-space pattern must publish more than a local rubric, because it must prevent wrong-kind use, hidden value drift, neighbour theft, and proxy-for-value substitution.
...
## [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]]:10 - Rationale

The split between [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] and [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]] preserves the FPF distinction between an evaluation characteristic-space specification and its publication form. [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] says what must exist for an evaluation to be adequate. [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]] says how that adequate evaluation is authored as an FPF pattern when FPF publication is selected. This prevents two symmetric mistakes: stuffing FPF pattern-format requirements into a general characteristic-space construction method, and publishing an evaluation-characteristic-space pattern whose coordinate set is not recoverable by value.

## [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]]:11 - SoTA-Echoing
...
| Pattern publication must remain action-guiding. | Pattern-language practice treats a pattern as reusable action guidance for recurring situations, not as a static rubric table. | **Lineage and current FPF reference use.** Pattern-language practice is retained as lineage and problem pressure; current FPF [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] supplies the governing publication-form rules for recognition text, first useful move, worked cases, and relations. | The publication form keeps recognition text and first evaluation use before coordinate tables. | [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]] does not replace [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]; it specializes it for evaluation-characteristic-space patterns. |

## [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]]:12 - Relations

| Pattern | Relation |
|---|---|
| [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] | Governs the canonical FPF authoring form. [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]] specializes that form for evaluation `CharacteristicSpace` pattern publication forms. |
| [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] | Constructs or repairs the evaluation `CharacteristicSpace`. [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]] authors the FPF pattern publication form for the selected specification when FPF reuse is selected. |
| [[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]], [[A.17 - Canonical “Characteristic” (A.CHR‑NORM)|A.17]], [[A.18 - Minimal CSLC in Kernel (Characteristic ⟷ Scale ⟷ Level ⟷ Coordinate) (A.CSLC‑KERNEL)|A.18]], [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]] | Govern `CharacteristicSpace`, characteristic, scale, coordinate, and measurement legality. |
| [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] | Evaluates quality of the authored FPF pattern publication form. It does not replace the evaluation for one evaluated object kind. |
| [[E.22 - Improvement-Oriented Quality Evaluation Question Framing|E.22]] | Frames one quality evaluation using the evaluation published by the publication form. |
| [[E.23 - Quality Improvement Loop Method|E.23]] | Runs repeated improvement using the evaluation published by the publication form. |
| [[E.9.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]], [[E.2.DA - FPF Pillar-Adequacy Evaluation CharacteristicSpace|E.2.DA]], [[F.18 - Local‑First Unification Naming Protocol|F.18]], [[C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]] | Existing or candidate evaluations that may use this authoring specialization when their publication-form is being written or refreshed. |
| [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]], [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]], [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] | Govern evidence, assurance, gate, decision, and work claims when an evaluation result is reused for those purposes. |
| [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.9 - Parity and Benchmark Harness|G.9]], [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] | Govern OEE/NQD archive, novelty, diversity, pool, selected-set, parity, and refresh claims. |
| [[C.29 - Mathematical Lens Use|C.29]] | Governs mathematical-lens use when a mathematical structure defines or justifies coordinate choice. |

## [[E.8.ECSPF - Evaluation CharacteristicSpace FPF Pattern Publication Form|E.8.ECSPF]]:End
