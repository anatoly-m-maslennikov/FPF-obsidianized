...
## [[C.13 - Constructional Mereology (Compose‑CAL)|C.13]]:1 - Intent
...
**Consumed by.** CT2R‑LOG ([[B.3.5 - Working-Model Relations & Grounding (CT2R-LOG)|B.3.5]]) Working‑Model alias logic and any FPF pattern that needs part–whole semantics. Compose‑CAL does **not** import alias definitions; it merely emits traces that others may reference.
...
## [[C.13 - Constructional Mereology (Compose‑CAL)|C.13]]:2 - Problem frame & Problem
...
## [[C.13 - Constructional Mereology (Compose‑CAL)|C.13]]:3 - Forces
...
## [[C.13 - Constructional Mereology (Compose‑CAL)|C.13]]:4 - Solution

### [[C.13 - Constructional Mereology (Compose‑CAL)|C.13]]:4.1 - Solution sketch
...
**Note.** The calculus names an internal backbone **`mero:KernelPartOf`**; the Kernel’s public `ut:PartOf`/**[[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]** catalogue remain unchanged. Publish only via Working‑Model aliases (CT2R‑LOG).
...
### [[C.13 - Constructional Mereology (Compose‑CAL)|C.13]]:4.2 - Normative Standard (high‑level)
...
* **C13-N2.** *Structural grounding stance.* Every **structural** edge **MUST** reference **exactly one** Γₘ trace as its grounding witness **and SHALL declare `validationMode = axiomatic`** (see [[B.3.5 - Working-Model Relations & Grounding (CT2R-LOG)|B.3.5]] / [[E.14 - Human‑Centric Working‑Model|E.14]]). **Structural edges MUST NOT** be published in `postulate` or `inferential` stances.
...
* **C13‑N5.** *Separation of concerns.* Γₘ provides constructions and traces; naming, aliasing and human‑level relation taxonomies are defined outside Compose‑CAL (see [[B.3.5 - Working-Model Relations & Grounding (CT2R-LOG)|B.3.5]] for the CT2R‑LOG handshake).
...
### [[C.13 - Constructional Mereology (Compose‑CAL)|C.13]]:4.3 - Scope, applicability, terms & notation
...
* **PortionOf** ⇢ `slice(entity, facet="material/spatial‑region")` **plus** metrical semantics ([[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]);
* **ConstituentOf** (logical/content) ⇢ `sum` narrative over conceptual parts. *(Material mixtures are **not** `ConstituentOf`; use `PortionOf` or `ComponentOf` per [[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]].)*

## [[C.13 - Constructional Mereology (Compose‑CAL)|C.13]]:5 - Archetypal Grounding *(System / Episteme duo)*
...
### [[C.13 - Constructional Mereology (Compose‑CAL)|C.13]]:5.1 - **System** (structural; constructive grounding)
...
### [[C.13 - Constructional Mereology (Compose‑CAL)|C.13]]:5.2 - **Episteme** (knowledge‑bearing; non‑constructive first)
...
### [[C.13 - Constructional Mereology (Compose‑CAL)|C.13]]:5.3 - Scope justification
...
## [[C.13 - Constructional Mereology (Compose‑CAL)|C.13]]:6 - Bias‑Annotation *(cognitive anti‑patterns and counter‑moves)*

| Bias (name)                       | Symptom                                                                                                         | Counter‑move (conceptual)                                                                                                    | Where to look                               |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- |
| **Constructor‑centrism**          | Treating the trace as “the real thing” and the Working‑Model edge (e.g., **ComponentOf**) as merely decorative. | Re‑affirm **Working‑Model first** (publish in `ut:*Of`), then attach constructive narratives only when assurance demands it. | [[B.3.5 - Working-Model Relations & Grounding (CT2R-LOG)|B.3.5]] (Working‑Model relations & grounding) |
| **Collection ↔ Composition swap** | Using **MemberOf** to stand in for **PartOf**, then inferring structural identity.                              | Keep **set** outputs as *collections*; use **sum** for wholes with extensional identity.                                     | [[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] (Advanced Mereology)                   |
| **Temporal leakage**              | Smuggling sequence/phase into part‑trees.                                                                       | Route order/time to their planes; **no** “temporal slice” constructor in Compose‑CAL.                                        | [[B.1 - Universal Algebra of Aggregation (Γ)|B.1]].\* (Γ\_method and Γ\_time)                |
| **Over‑slicing**                  | Multiplying aspects until identity becomes opaque.                                                              | Declare the **facet** explicitly; stop when aspects no longer aid recognition of the same whole.                             | [[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] (Aspect/Phase distinction)             |
...
| **Axiomatic inflation**           | Demanding constructive traces for epistemic links by default.                                                   | Use *inferential* / *postulate* where appropriate; reserve *axiomatic* for structural identity.                              | [[B.3.5 - Working-Model Relations & Grounding (CT2R-LOG)|B.3.5]] (validation modes)                    |

## [[C.13 - Constructional Mereology (Compose‑CAL)|C.13]]:7 - Conformance Checklist *(normative, calculus‑level)*
...
| **CC‑C13‑11 (Member non‑transitive).** | When publishing `MemberOf`, do not rely on transitive closure across collection‑of‑collections; surface semantics remain non‑transitive per [[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]. | Prevent Member→Component drift. |

> **Author’s note.** Compose‑CAL is a calculus for **constructive** reasoning about structure. Publishing remains in the **Working‑Model** layer (see [[B.3.5 - Working-Model Relations & Grounding (CT2R-LOG)|B.3.5]]); constructive narratives are attached when the team seeks an assurance record with additional constructive evidence, never as a substitute for clear human‑facing relations.

## [[C.13 - Constructional Mereology (Compose‑CAL)|C.13]]:8 - Consequences
...
* **Human–first publication, formal–on‑demand.** Teams keep publishing **Working‑Model** relations (e.g., `ut:ComponentOf`), while **assurance** is attached as needed via a constructive grounding narrative and `tv:groundedBy` (see [[B.3.5 - Working-Model Relations & Grounding (CT2R-LOG)|B.3.5]]).
...
* **Didactic economy.** Authors learn one compact calculus; reviewers gain a predictable place to look for constructive justification when `validationMode = axiomatic` ([[B.3.5 - Working-Model Relations & Grounding (CT2R-LOG)|B.3.5]] alignment).
...
> **One‑line takeaway.** Compose‑CAL gives a minimal, universal *how‑it‑was‑built* story for any structural edge, without disturbing the human‑first publication surface defined in [[B.3.5 - Working-Model Relations & Grounding (CT2R-LOG)|B.3.5]].

## [[C.13 - Constructional Mereology (Compose‑CAL)|C.13]]:9 - Rationale (informative)
...
This separation mirrors long‑standing distinctions between composition, collection, and aspect, while enforcing **parsimony**: no additional constructors are introduced into the Kernel (C‑5). The calculus remains **notation‑agnostic**: its meanings are given in prose and mathematics; any diagrams are illustrative only, in line with the Notational‑Independence guard‑rail ([[E.5 - Four Guard‑Rails of FPF|E.5]]).

**Why constructive grounding lives outside the publication surface.**
FPF privileges **Working‑Model** relations as the canonical form for communication and design. Compose‑CAL supplies the **constructive shoulder** of the **Assurance Layer**: when authors choose `validationMode = axiomatic`, they narrate the whole as a `sum` of parts (with optional `set` and `slice` scaffolding) and point to that narrative via `tv:groundedBy`. This keeps the text readable while preserving a path to an assurance record with additional constructive evidence ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] family, Authoring Template).

**Why order/time are out of scope.**
Correctness‑by‑sequence and temporal coverage are orthogonal to **parthood**. Encoding them as parts breeds contradictions (e.g., “phase‑as‑component”). Compose‑CAL deliberately refuses any “serial/parallel/temporal constructor,” delegating such concerns to `Γ_method` and `Γ_time` and aligning with [[B.1 - Universal Algebra of Aggregation (Γ)|B.1]]’s flavour separation.

## [[C.13 - Constructional Mereology (Compose‑CAL)|C.13]]:10 - Relations

**Builds on**

* **[[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] Advanced Mereology.** Uses its structural catalogue (Component/Portion/Aspect vs Member) as the *target* of constructive narratives; never collapses Member into Part.
* **[[E.5 - Four Guard‑Rails of FPF|E.5]] Guard‑Rails (Notational Independence).** Meanings are given in prose; diagrams are illustrative only.
* **[[E.5 - Four Guard‑Rails of FPF|E.5]] Guard‑Rails (Unidirectional Dependency).** Compose‑CAL depends **downward** only; it never imports alias layers or higher planes.
* **[[E.8 - FPF Authoring Conventions & Style Guide|E.8]] Authoring Conventions.** Conforms to the canonical pattern template (Grounding section for architectural patterns; CC placement).

**Coordinates with**
* **[[B.3.5 - Working-Model Relations & Grounding (CT2R-LOG)|B.3.5]] CT2R‑LOG.** `tv:groundedBy` refers (conceptually) to Compose‑CAL traces when `validationMode = axiomatic`; **Working‑Model** relations remain the publication interface.
* **[[B.1 - Universal Algebra of Aggregation (Γ)|B.1]] flavours.** Keeps order (`Γ_method`) and time (`Γ_time`) outside structure; may co‑appear in narratives when relevant but never as constructors.
...
* **[[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]] (Measurement substrate).** Supplies quantitative hooks when a constructive narrative benefits from explicit counts/ratios (e.g., cardinalities, coverage), while keeping metrics distinct from mereology.
...
## [[C.13 - Constructional Mereology (Compose‑CAL)|C.13]]:End
