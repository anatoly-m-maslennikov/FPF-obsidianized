...
**One-line summary.** `BridgeStanceOverlay` governs one stance annotation over an existing [[F.9 - Alignment & Bridge across Contexts|F.9]] bridge card so authors can say how to read that bridge without changing its kind, direction, `CL`, or loss notes and without treating the overlay as bridge authority or as a cure for missing source-bearing return.
**Primary EntityOfConcern in plain terms.** One overlay annotation attached to an existing [[F.9 - Alignment & Bridge across Contexts|F.9]] bridge card; not the bridge card itself, not a second bridge kind, and not the pattern that governs coarsened renderings.
...
**Start here when.** Your first honest artefact is already an [[F.9 - Alignment & Bridge across Contexts|F.9]] bridge card, and the practical question is how to read that bridge rather than whether a bridge exists at all.
...
**Not this pattern when.** Not this overlay when the case is the bridge card itself under [[F.9 - Alignment & Bridge across Contexts|F.9]], or when a coarsened rendering still needs source-bearing return before any bridge-bearing use is admissible; use `A.6.3.CSC Controlled Semantic Coarsening` for that coarsened-rendering relation.

## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:1 - Problem frame
...
## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:2 - Problem
...
## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:3 - Forces
| Force | Tension |
|---|---|
| **Expressive stance vs bridge discipline** | Add interpretive clarity without introducing a rival bridge-kind system. |
| **Reuse vs inflation** | Make stance annotations reusable across bundles while keeping bridge cards structurally governed by [[F.9 - Alignment & Bridge across Contexts|F.9]]. |
| **Interpretive help vs substitution abuse** | Help readers interpret a bridge without silently licensing substitution beyond what [[F.9 - Alignment & Bridge across Contexts|F.9]] allows. |

## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:4 - Solution
A Bridge Stance Overlay is a local interpretive annotation attached to an existing [[F.9 - Alignment & Bridge across Contexts|F.9]] bridge card. It does not change the underlying bridge kind, direction, `CL`, or loss notes.

### [[F.9.1 - Bridge Stance Overlay|F.9.1]]:4.1 - Starter overlay vocabulary
| Stance | Intended reading | What it does **not** imply |
|---|---|---|
| `localRename` | the target term is near-renaming within the current context boundary; if a cross-context relation is live, the [[F.9 - Alignment & Bridge across Contexts|F.9]] bridge card must exist first | automatic cross-context identity |
| `operationalizes` | the target is a procedural or operational reading aid over the declared bridge | enactment, implementation, execution permission, gate approval, [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] work authority, or type-structure equivalence |
...
### [[F.9.1 - Bridge Stance Overlay|F.9.1]]:4.2 - Boundary rule
...
### [[F.9.1 - Bridge Stance Overlay|F.9.1]]:4.3 - Relation to `CL` and loss
...
## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:5 - Archetypal Grounding
...
## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:6 - Bias-Annotation
Lenses tested: **Gov**, **Arch**, **Onto/Epist**, **Prag**, **Did**. Scope: **Universal** for stance overlays attached to existing [[F.9 - Alignment & Bridge across Contexts|F.9]] Bridge Cards inside FPF.
This pattern favors disciplined cross-school comparison and bridge readability over sweeping synonym claims. The main mitigation is that every stance remains subordinate to the underlying Bridge Card, its direction, `CL`, and loss notes, with explicit handoff to [[A.6.3.CSC - Controlled Semantic Coarsening|A.6.3.CSC]] when the issue under repair is source-bearing return for a coarsened rendering rather than bridge-card reading.

## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:7 - Conformance Checklist
- `CC-[[F.9 - Alignment & Bridge across Contexts|F.9]].1-1` A stance annotation **SHALL NOT** replace the underlying [[F.9 - Alignment & Bridge across Contexts|F.9]] bridge kind.
- `CC-[[F.9 - Alignment & Bridge across Contexts|F.9]].1-2` Stance annotations **SHOULD** be accompanied by explicit loss notes when they materially affect interpretation.
- `CC-[[F.9 - Alignment & Bridge across Contexts|F.9]].1-3` `nonEquivalent` **SHALL** block silent substitution.
- `CC-[[F.9 - Alignment & Bridge across Contexts|F.9]].1-4` A stance annotation **SHALL NOT** claim higher-CL sameness than the bridge card's `CL` and kind allow.
- `CC-[[F.9 - Alignment & Bridge across Contexts|F.9]].1-5` A stance annotation **SHALL NOT** stand in for source-bearing return when a coarsened rendering still needs reopen before any bridge-bearing reading is admissible; that relation belongs to `A.6.3.CSC Controlled Semantic Coarsening`.

## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:8 - Common Anti-Patterns and How to Avoid Them
...
- **Stance inflation.** Do not use the annotation to smuggle in substitution rights that [[F.9 - Alignment & Bridge across Contexts|F.9]] withholds.
...
## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:9 - Consequences
...
## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:10 - Rationale
`U.LanguageStateSpace` and `U.LanguageStateMoveTrajectory` create many legitimate cross-school comparisons. [[F.9.1 - Bridge Stance Overlay|F.9.1]] gives those comparisons a reusable stance vocabulary without fragmenting the underlying [[F.9 - Alignment & Bridge across Contexts|F.9]] bridge discipline.
...
## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:11 - SoTA-Echoing
...
## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:12 - Relations
- Builds on: [[F.9 - Alignment & Bridge across Contexts|F.9]], [[C.2.2a - U.LanguageStateSpace - Language-state chart over U.CharacteristicSpace|C.2.2a]].
- Primary boundary: [[F.9 - Alignment & Bridge across Contexts|F.9]] governs Bridge Card discipline; [[F.9.1 - Bridge Stance Overlay|F.9.1]] governs only stance overlays attached to existing Bridge Cards.
- Coordinates with: [[A.16.0 - U.LanguageStateMoveTrajectory - Optional trajectory-account normal form over the language-state U.CharacteristicSpace|A.16.0]], [[E.17.1 - U.ViewpointBundleLibrary - Reusable Viewpoint Bundles|E.17.1]], [[A.6.P - Relational Precision Restoration (RPR) - Kind‑Explicit Qualified Relation Discipline|A.6.P]], [[A.6.A - U.ActionInvitationPrecisionRestoration - Affordance and Action-Invitation Precision Restoration (ACT-INV)|A.6.A]], [[C.16.Q - Quality-Term Precision Restoration|C.16.Q]], [[C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]], and [[B.4.1 - Observe -- Notice -- Stabilize -- Route|B.4.1]].
- Constrains: local stance annotations on bridge cards used in comparative and tradition bundles.
## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:13 - Worked Bridge-Card Examples

### [[F.9.1 - Bridge Stance Overlay|F.9.1]]:13.1 - `localRename`
...
### [[F.9.1 - Bridge Stance Overlay|F.9.1]]:13.4 - `partialAnalogy` and `nonEquivalent`
A comparative bundle may need to mention an explanatory resemblance across traditions without claiming substitution. In such cases `partialAnalogy` may guide reading when the shared pattern is local and declared. If review concludes that this local resemblance still lacks reuse support, `nonEquivalent` should be preferred so that apparent similarity does not drift into silent replacement. The label blocks equivalence and silent substitution; it does not by itself assert `Disjoint` or `CL=0` unless the underlying [[F.9 - Alignment & Bridge across Contexts|F.9]] bridge card says so.

### [[F.9.1 - Bridge Stance Overlay|F.9.1]]:13.5 - `projection` is not a coarsened-note cure
A team may write a short comparison note saying that one report-only metric is `basically a projection` of a richer operations concept. That sentence does not by itself justify a `projection` overlay. First recover the underlying [[F.9 - Alignment & Bridge across Contexts|F.9]] bridge card, its direction, `CL`, and Loss Notes from the source-bearing episteme or source publication needed for the Bridge Card. Only then may the overlay say how to read that bridge. If readers still need return to the source-bearing episteme or source publication before any bridge-bearing use is admissible, the overlay must wait; it cannot repair the missing bridge card.

## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:14 - Practical use guidance
...
## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:15 - Legacy-note repair and boundary
Legacy comparative notes often contain undeclared stance language such as "roughly the same", "really a projection", or "just an operational version". When such a note is normalized, the first repair step is to recover the underlying bridge card in [[F.9 - Alignment & Bridge across Contexts|F.9]]; only then may a Bridge Stance Overlay be added as an explicit local stance annotation.
...
## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:16 - Overlay Declaration Discipline

A stance overlay is useful only when it stays visibly subordinate to the bridge card it qualifies.

### [[F.9.1 - Bridge Stance Overlay|F.9.1]]:16.1 - Minimal overlay declaration
A usable stance overlay should normally publish:

- the qualified [[F.9 - Alignment & Bridge across Contexts|F.9]] bridge card,
...
### [[F.9.1 - Bridge Stance Overlay|F.9.1]]:16.2 - One primary stance per bridge card
...
### [[F.9.1 - Bridge Stance Overlay|F.9.1]]:16.3 - Overlay locality
...
## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:17 - Interaction with `CL`, Direction, and Loss

### [[F.9.1 - Bridge Stance Overlay|F.9.1]]:17.1 - `CL` remains prior
...
### [[F.9.1 - Bridge Stance Overlay|F.9.1]]:17.2 - Direction-sensitive reading
...
### [[F.9.1 - Bridge Stance Overlay|F.9.1]]:17.3 - Loss emphasis rule
...
## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:18 - Bundle Use and Comparative Reading

### [[F.9.1 - Bridge Stance Overlay|F.9.1]]:18.1 - Bundle-level reuse
...
### [[F.9.1 - Bridge Stance Overlay|F.9.1]]:18.2 - Comparative stance caution
...
### [[F.9.1 - Bridge Stance Overlay|F.9.1]]:18.3 - Boundary to second bridge taxonomy
If authors start grouping bridges primarily by stance label and ignoring bridge kind, direction, `CL`, or loss, they have implicitly created a rival bridge taxonomy. [[F.9.1 - Bridge Stance Overlay|F.9.1]] forbids that drift.

## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:19 - Review Matrix and Migration Tests
...
## [[F.9.1 - Bridge Stance Overlay|F.9.1]]:End
