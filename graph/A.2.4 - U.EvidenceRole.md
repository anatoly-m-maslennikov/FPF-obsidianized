...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:1 - Context and intent
...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:2 - Problem
...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:3 - Forces
...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:4 - Solution — Term and definition
...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:5 - Role family and specialisations
### [[A.2.4 - U.EvidenceRole|A.2.4]]:5.3 - Causal evidence-role specialisations
...
The corresponding `CausalEvidenceSupportBasis` values are governed by [[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]]: `observationalAssociationSupportBasis`, `interventionalActionSupportBasis`, `realizedCounterfactualSampleSupportBasis`, `identifiedCounterfactualEstimateSupportBasis`, and `simulationOnlyCounterfactualOutputBasis`. [[A.2.4 - U.EvidenceRole|A.2.4]] only classifies evidence roles held by epistemes; it does not mint a second causal support-basis value set.

What this does not authorize: [[A.2.4 - U.EvidenceRole|A.2.4]] does not decide the causal-use question, estimand, identification, or counterfactual sampling realizability; it preserves the evidence-role assignment and the authority-reference boundary so [[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] and [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] can judge the causal-use claim without vocabulary laundering.
...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:6 - Clear distinctions (Strict Distinction, litmus tests)
...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:7 - Core invariants (concept level)
...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:8 - Facets and semantics (normative)
...
### [[A.2.4 - U.EvidenceRole|A.2.4]]:8.1 - Claim-scope schema

Every `U.EvidenceRole` definition **within a `U.BoundedContext`** **MUST** declare a claim-scope record. This record ties the role’s meaning to the target claim named by value and its claim scope, and aligns with the typed-claim form used in [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]:
...
| `assuranceUse`  | `TA` / `VA` / `LA`                 | Declares whether the evidence functions as typing, verification, or validation input ([[B.3.3 - Assurance Subtypes & Levels|B.3.3]]).       |
...
### [[A.2.4 - U.EvidenceRole|A.2.4]]:8.2 - Timespan and decay
...
When the relevance window closes (`validUntil` reached), the evidence incurs **Epistemic Debt (ED)**. Per [[B.3.4 - Evidence Decay & Epistemic Debt|B.3.4]], debt must trigger one of three managed actions:
...
### [[A.2.4 - U.EvidenceRole|A.2.4]]:8.3 - Provenance hooks

Each `U.EvidenceRole` **MUST** anchor into the **Evidence–Provenance DAG** ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]):
...
* SCR/RSCR anchors ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]) are mandatory for all carriers.
...
### [[A.2.4 - U.EvidenceRole|A.2.4]]:8.4 - Contribution to assurance

A `U.EvidenceRole` classifies an episteme; its contribution to the target claim’s assurance tuple ⟨F, G, R⟩ is computed in [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] using:
...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:9 - Worked examples

### [[A.2.4 - U.EvidenceRole|A.2.4]]:9.1 - Formal line — *Proof as evidence for a theorem*
...
#### [[A.2.4 - U.EvidenceRole|A.2.4]]:9.2 - Empirical line — *Sensor calibration as evidence for an accuracy claim*
...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:10 - Conformance checklist (normative)
...
**CC-ER-06 (Provenance)**
Every evidence-role assignment **MUST** anchor into the EPV-DAG ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]). For empirical line, `fromWorkSet` must point to external `U.Work`; self-evidence is prohibited.
...
Every binding **SHALL** declare `assuranceUse ∈ {TA | VA | LA}` and, for **empirical** (LA) bindings, expose `timespan/valid_until` and `decayPolicy` so that SCR can report lane‑separated contributions and freshness ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]).

## [[A.2.4 - U.EvidenceRole|A.2.4]]:11 - Anti-patterns and remedies
...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:12 - Operators (conceptual, tooling-agnostic)
...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:13 - Relations

**Builds on:**
[[A.2 - Role Taxonomy|A.2]] `U.Role`, [[A.2.1 - U.RoleAssignment- Contextual Role Assignment|A.2.1]] `U.RoleAssignment` (role as mask, binding as assignment), [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] Evidence Graph Referring (EPV-DAG), [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] Trust & Assurance Calculus.

**Coordinates with:**
[[A.3.2 - U.MethodDescription- Description Episteme for a Way of Doing|A.3.2]] `U.MethodDescription` (protocols, proof obligations), E.6.1 Epistemic Roles via `U.RoleAssignment` (didactic gateway), [[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] when an evidence role is used to support causal-use claims and must stay distinct as observational, interventional, realized counterfactual sample, identified counterfactual estimate, or simulation-only counterfactual output.
...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:14 - Migration notes (quick wins)
...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:15 - Didactic quick cards (engineer-manager ready)
...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:16 - SCR/RSCR audit stubs (assurance scaffolding)
...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:17 - Minimal evidence-role assignment schema (informative)
...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:18 - Memory hooks and acceptance cross-checks (informative)
...
## [[A.2.4 - U.EvidenceRole|A.2.4]]:End
