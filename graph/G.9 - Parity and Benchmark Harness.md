...
## [[G.9 - Parity and Benchmark Harness|G.9]]:0 — Use this when
...
## [[G.9 - Parity and Benchmark Harness|G.9]]:0.1 — What goes wrong if missed
...
## [[G.9 - Parity and Benchmark Harness|G.9]]:0.2 — What this buys
...
- one harness that downstream selection can consume without inventing a [[G.9 - Parity and Benchmark Harness|G.9]]-local CSLC gate or a shadow governance card
...
## [[G.9 - Parity and Benchmark Harness|G.9]]:1 — Intent
...
* executes parity in a way that is consumable by **[[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]** (selected-set outcomes, DRR/SCR evidence trace);
* publishes **ParityReport@Context** suitable for downstream consumption, shipping, and refresh/RSCR wiring.

## [[G.9 - Parity and Benchmark Harness|G.9]]:2 — Problem frame
...
[[G.9 - Parity and Benchmark Harness|G.9]]’s role is to make these recoverable as **pinned and publishable** as a *method of obtaining outputs* (MOO) without introducing new governing spec refs.

## [[G.9 - Parity and Benchmark Harness|G.9]]:3 — Forces
...
## [[G.9 - Parity and Benchmark Harness|G.9]]:4 — Solution

### [[G.9 - Parity and Benchmark Harness|G.9]]:4.0 — [[G.Core - Part G Core Invariants|G.Core]] linkage (normative)

This pattern is **core‑invariant** and therefore binds to **[[G.Core - Part G Core Invariants|G.Core]]** by declaration (not by restating invariants here).

**GCoreLinkageManifest ([[G.9 - Parity and Benchmark Harness|G.9]])** *(normative; expands per `G.Core:4.2`)*
...
### [[G.9 - Parity and Benchmark Harness|G.9]]:4.1 — Objects and publication records
...
`ParityPlan@Context := ⟨
  ...
  PortfolioMode?, DominanceRegime?,         // may be explicit or inherited via DefaultGoverningDefinition (semantics follow [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]])
  ParityContextId,
  ...
  Budgeting?,
  ParityPinSet,
  PlanItemRefs[]?                          // references to [[A.15.3 - SlotFillingsPlanItem|A.15.3]] SlotFillingsPlanItem (planned baseline), when parity depends on planned slot fillings
⟩`

**(2) `ParityPinSet`** *(pin set)*
A declared set of pins required for reproducibility and audit (editions + policy‑ids + UTS/Path pins).
The concrete contents are *pattern-local* ([[G.9 - Parity and Benchmark Harness|G.9]] declares the pin set), but must satisfy the *core pin discipline* via [[G.Core - Part G Core Invariants|G.Core]].
...
A UTS-publishable parity publication record produced by running a `ParityPlan@Context`. By itself it is not a dated `U.Work` occurrence, audit performance, evidence path, assurance result, or gate decision; those claims require [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]/[[A.15.1 - U.Work|A.15.1]], [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]/[[G.6 - Evidence Graph & Provenance Ledger|G.6]], [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], or [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] respectively.
...
* Tech/Plain twins follow [[E.10 - Unified Lexical Rules for FPF|E.10]] rules (no drift‑inducing synonyms in Tech).

### [[G.9 - Parity and Benchmark Harness|G.9]]:4.2 — Parity planning (design‑time / WorkPlanning)

Planning is the act of making the parity run *reproducible by construction*:

1. **Fix the baseline set.** Choose the `BaselineSet` (MethodFamilies, and optionally GeneratorFamilies) to compare; where parity context matters, cite `SoS‑LOGBundleId?` and source-maturity ids by reference (acceptance-gate thresholds remain in [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] Acceptance).
...
9. **Attach planned baselines (when applicable).** If parity depends on planned slot fillings, the plan cites the relevant `SlotFillingsPlanItem` refs ([[A.15.3 - SlotFillingsPlanItem|A.15.3]]) via `PlanItemRefs[]` rather than embedding a competing baseline object (nil‑elision when unused).
10. **Publish crossing pins (when invoked).** Cross-Context or cross-plane/Kind reuse requires explicit Bridge/CL/Φ pins; penalties affect `R_eff` only (invariants pinned through [[G.Core - Part G Core Invariants|G.Core]]).

### [[G.9 - Parity and Benchmark Harness|G.9]]:4.3 — Execution protocol (run‑time / selector‑adjacent)

Execution is **one run** under the pinned plan:

1. **Validate CSLC references and pins.** Validate the cited CSLC comparability and admissibility references, active pins, and witnesses; run eligibility or acceptance checks under the plan’s `TaskSignature (S2)` and refuse or abstain on non-admissible operations (record trace; no “fourth status”). If a live [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] gate consumes this check, cite its `GateDecisionRef`/`DecisionLogRef`; do not create a [[G.9 - Parity and Benchmark Harness|G.9]]-local CSLC gate.
2. **Invoke selection/dispatch.** Apply **[[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]** under the plan’s pinned refs and emit selector outputs in a form consistent with [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]’s `PortfolioMode` and selected-set semantics.
...
### [[G.9 - Parity and Benchmark Harness|G.9]]:4.3a — Worked parity slice
...
### [[G.9 - Parity and Benchmark Harness|G.9]]:4.3b — Conditional causal method rung parity
...
- causalUseSupportRecordRef and causalUseSupportVerdict when a [[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] support record or verdict is being consumed;
- causalFollowUpWindowRef;
- outcomeMeasureRef;
...
What this does not authorize: [[G.9 - Parity and Benchmark Harness|G.9]] does not decide causal identification, causal fairness, or counterfactual sampling realizability; it keeps parity and benchmark harness authority and sends causal-use support to [[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]].

### [[G.9 - Parity and Benchmark Harness|G.9]]:4.9 — Extensions (pattern‑scoped; non‑core)
...
* **Uses:** `{[[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]], [[F.9 - Alignment & Bridge across Contexts|F.9]], [[E.18 - Transformation Flow Structure|E.18]], [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]}`
* **⊑/⊑⁺:** `∅`
* **RequiredPins/EditionPins/PolicyPins (minimum; conditional on use):**
  ...
  * `CrossingBundleId?`
* **RSCRTriggerSetIds:** `{GCoreTriggerSetId.BridgeCalibrationKit}` *(preferred; expands in [[G.Core - Part G Core Invariants|G.Core]])*
...
* **Uses:** `{[[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]], [[G.6 - Evidence Graph & Provenance Ledger|G.6]], [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]}`
* **⊑/⊑⁺:** `∅`
* **RequiredPins/EditionPins/PolicyPins (minimum; conditional on use):**
  * `SoSLogRuleId[]` / `BranchId[]` *(ids as cited labels; semantics come from [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]])*
...
* **GPatternExtensionKind:** `MethodSpecific`
* **GoverningPatternId:** [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]
...
* **Notes (wiring-only):** Declares the pins required to make DHC‑based parity reproducible and RSCR‑refreshable; semantics of DHC lives in [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]].
...
* **Uses:** `{[[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]}`
* **⊑/⊑⁺:** `∅`
* **RequiredPins/EditionPins/PolicyPins (minimum; conditional on use):**
...
* **Notes (wiring-only):** Post‑2015 QD families are referenced here only as wiring + edition/policy pin obligations (semantics come from [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]/[[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]]/[[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]).
...
* **Uses:** `{[[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]}`
* **⊑/⊑⁺:** `∅`
* **RequiredPins/EditionPins/PolicyPins (minimum; conditional on use):**
...
## [[G.9 - Parity and Benchmark Harness|G.9]]:5 — Interfaces (minimal I/O; conceptual)

| Interface                          | Consumes                                                                                                                                         | Produces                                                                                        |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------- |
| **[[G.9 - Parity and Benchmark Harness|G.9]]‑1 `Plan_Parity`**            | `BaselineSet`, `BaselineBindingRef`, `FreshnessWindows`, `Budgeting?`, `EpsilonDominance?`, `CNSpecRef.edition`, `CGSpecRef.edition`, `ComparatorSpecRef.edition`, `SCPRef.edition?`, `MinimalEvidenceRef.edition?`, `UNM_id?`, `NormalizationMethodId[]?`, `NormalizationMethodInstanceId[]?`, `ParityPinSet`, `PlanItemRefs[]?` | `ParityPlan@Context` (UTS entry; edition‑pinned)                                                |
| **[[G.9 - Parity and Benchmark Harness|G.9]]‑2 `Run_Parity`**             | `ParityPlan@Context`, `TaskSignatureRef` (S2), **[[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]‑3 Select**                                                                                  | Selector outputs (selected sets / archives / sets as refs), DRR+SCR pins with `PathId[]`/`PathSliceId?` |
| **[[G.9 - Parity and Benchmark Harness|G.9]]‑3 `Publish_ParityReport`**   | parity-run trace refs + active pins                                                                                                            | `ParityReport@Context` (UTS publication record; emits canonical RSCR ids)                       |
| **[[G.9 - Parity and Benchmark Harness|G.9]]‑4 `Expose_ParityTelemetry`** | Telemetry deltas (archive changes, coverage/regret signals, etc.)                                                                                | Telemetry events carrying `PathSliceId?`, policy‑ids, and edition pins for refresh wiring       |

*Publication records are conceptual here; serialisations belong in shipping and interop publication forms (see [[G.10 - SoTA Pack Shipping|G.10]] and interop annexes), not in [[G.9 - Parity and Benchmark Harness|G.9]].*

## [[G.9 - Parity and Benchmark Harness|G.9]]:6 — Conformance Checklist (CC‑G9)

**CC‑G9‑CoreRef (normative; mandatory).**
[[G.9 - Parity and Benchmark Harness|G.9]] conforms only if it satisfies the **effective** set of `CC‑GCORE‑*` declared in **[[G.9 - Parity and Benchmark Harness|G.9]]:4.0 GCoreLinkageManifest** (including trigger typing, Default Governing Definition Index links, and P2W split).
...
10. **CC‑G9.10 — RSCR parity tests are published (local).**
   Parity publication **SHALL** include RSCR parity tests (via [[F.15 - SCR-RSCR Harness for Unification|F.15]] harness refs) that cover negative/refusal paths relevant to this plan (missing pins, edition drift, missing bridge calibration refs, etc.).

11. **CC‑G9.11 — GateCrossing visibility (delegation point).**
    Delegated to `CC‑GCORE‑CROSS‑1` and the applicable GateCrossing/CrossingBundle harness checks ([[E.18 - Transformation Flow Structure|E.18]], [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], [[F.9 - Alignment & Bridge across Contexts|F.9]], and relevant Part G bridge or crossing wiring). This remains a stable delegation point.

12. **CC‑G9.12 — Tech‑register lexical discipline (local).**
    Tech prose and heads **SHALL** follow [[E.10 - Unified Lexical Rules for FPF|E.10]]: do not introduce drift‑prone primitives (e.g., “metric” as a Tech primitive); reference the source pattern's canonical terms and pinned refs.
...
* **AP‑4 Cross‑Context reuse without visible crossing pins.** Remedy: cite Bridge/reference-plane records and crossing visibility records (delegated to [[G.Core - Part G Core Invariants|G.Core]]).
...
## [[G.9 - Parity and Benchmark Harness|G.9]]:8 — Archetypal grounding (informative; SoTA‑oriented)

**Show‑A — Multi‑tradition parity for decision systems (post‑2015 practice).**
ParityPlan pins a rolling evidence window and comparator refs; ParityReport publishes a selected-set outcome plus the evidence trace. Family labels such as preference-learning comparators, causal decision pipelines, offline-RL evaluation pipelines, and robust BO-style selectors remain illustrative until a [[G.2 - SoTA Harvester & Synthesis|G.2]] SoTA pack or named current source pins the exact family being compared; the parity report still must preserve the selected set or partial order rather than collapse everything into a single scalar.
...
**Show‑C — Open‑ended parity (POET as lineage; current generator-family claims require a named [[G.2 - SoTA Harvester & Synthesis|G.2]] SoTA pack or exact current source).**
...
A team compares an observational predictor, an intervention optimizer, and a counterfactual policy strategy under one "best causal method" headline. [[G.9 - Parity and Benchmark Harness|G.9]] first runs `CausalRungParityScreen`: if rungs, support bases, estimands, or outcome windows differ, the screen returns degraded parity or abstain before a full record is fabricated. When full parity remains plausible, [[G.9 - Parity and Benchmark Harness|G.9]] requires `CausalMethodRungParityRecord`: each method declares `targetCausalUseClaimKind`, target `CausalityLadderRung`, `estimandRef`, interventional-action basis, `CausalEvidenceSupportBasis`, relevant [[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] support record and verdict when consumed, follow-up window, outcome measure, source population and target population basis, and estimation-validity basis. If those fields differ, the parity report names `declaredCausalityLadderBridgeOrLossRef`, transportability or estimation refs where available, and degraded parity or abstain result. The admissible output may be a selected set by comparable rung, not one scalar winner.

## [[G.9 - Parity and Benchmark Harness|G.9]]:9 — Cited Records (what this pattern publishes)
...
* Telemetry pins/events (by id), for refresh wiring ([[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]) and RSCR harnesses ([[F.15 - SCR-RSCR Harness for Unification|F.15]]).

## [[G.9 - Parity and Benchmark Harness|G.9]]:10 — Relations
...
- [[C.27 - Temporal Claim Adequacy- State Readings, Temporal Trends, and Intervention-Sensitive Temporal Change|C.27]] may flag: dynamic parity when a benchmark actually compares rate-change, rhythm change, recovery speed, intervention effect, effort budget, or dynamic outcome.
...
- Non-admissible use: a [[C.29 - Mathematical Lens Use|C.29]] output does not publish a benchmark report, create benchmark superiority, supply selector output, or supply parity-measure admissibility by itself.
- [[C.29 - Mathematical Lens Use|C.29]] application: for an under-lensed or overread parity input, cite the applicable [[C.29 - Mathematical Lens Use|C.29]] output for the stated use: `NoMathLensUseNeeded`, `MathLensUse.LensCandidateNote`, `MathLensUse.OneLine`, `MathLensUse.MiniCard`, `MathLensUse.FullCard`, or `NeighborGoverningPatternNote`. Use the cheap output that changes the next admissible parity move; full-card work is only required when the live parity or benchmark claim needs it.

**Builds on:** [[G.Core - Part G Core Invariants|G.Core]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.6 - Evidence Graph & Provenance Ledger|G.6]], [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]], [[F.15 - SCR-RSCR Harness for Unification|F.15]], [[E.17 - Multi‑View Publication Kit|E.17]], [[E.18 - Transformation Flow Structure|E.18]], [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], [[F.17 - Unified Term Sheet (UTS)|F.17]], [[E.5.2 - Notational Independence|E.5.2]], [[E.10 - Unified Lexical Rules for FPF|E.10]].
**Publishes to:** **UTS** (plan/report ids), **[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]** (refresh wiring), **[[G.10 - SoTA Pack Shipping|G.10]]** (shipping publication form; parity records are cited records).
**Uses:** **[[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]**, **[[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]]**, **[[F.9 - Alignment & Bridge across Contexts|F.9]]**, and [[C.28 - CausalUse-CAL- Causal-Use Questions, Causality-Ladder Rungs, Identification and Realizability|C.28]] when parity compares causal methods or causal-use claims.
**Uses (optional, via Extensions):** **[[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]**, **[[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]] and [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]]** (QD/OEE wiring), **[[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]** (SoS‑LOG narration and failure‑policy pins).

## [[G.9 - Parity and Benchmark Harness|G.9]]:11 — Working reading checks
...
## [[G.9 - Parity and Benchmark Harness|G.9]]:End
