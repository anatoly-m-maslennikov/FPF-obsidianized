...
## [[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]]:1 - Context

FPF patterns are the **canon**: they define the generative rules that other artifacts depend on. Teams need to **change** patterns as the SoTA moves, but ad‑hoc edits lead to drift, low comparability, and brittle downstream updates. We need a **method** that (a) *generates* better alternatives, (b) *selects* them against explicit quality/assurance targets, and (c) *publishes* a machine‑ and human‑checkable **trace** that can be replayed, audited, and re‑run. (Built to cohere with **DRR ([[E.9 - Design‑Rationale Record (DRR) Method|E.9]])**, **LEX‑BUNDLE ([[E.10 - Unified Lexical Rules for FPF|E.10]])**, **Canonical Evolution Loop ([[B.4 - Canonical Evolution Loop|B.4]])**, **NQD/E‑E ([[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]] and [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]])**, **Evidence Graph Referring ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]])**, **Trust ([[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]])**, **F‑Suite validation ([[F.15 - SCR-RSCR Harness for Unification|F.15]])**.)

## [[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]]:2 - Problem
...
## [[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]]:3 - Forces
...
## [[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]]:4 - Solution — A *governed evolutionary* authoring method with a publishable **LEX‑AUTH Trace (LAT)**
...
### [[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]]:4.1 - Method (design‑time choreography)

**Stage A — Frame & Scope (Context, Objectives, Invariants)**

1. **Anchor** the work in a **`U.BoundedContext`** for the spec (e.g., `FPF/Core`), cite governing guard‑rails (**[[E.5 - Four Guard‑Rails of FPF|E.5]].\***), and state **objectives** for the change (e.g., clarity ↑, universality ↑, assurance cost ↓).
...
7. **Bridge across Contexts** explicitly ([[F.9 - Alignment & Bridge across Contexts|F.9]]): any imported definitions/claims declare **CL** and *loss notes*; propose scoped **narrowing** where needed.
8. **Type scopes** with **USM ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]])**: keep **ClaimScope (G)** distinct from **WorkScope**; no “applicability/envelope” smuggling.

**Stage D — Validate & Decide (Assurance, Tests, DRR)**
9. **Run the harness**: update **SCR/RSCR** ([[F.15 - SCR-RSCR Harness for Unification|F.15]]), lint lexical rules ([[E.10 - Unified Lexical Rules for FPF|E.10]]), run **Γ‑consistency** and **RSG/SoD** checks where relevant.
10. **Score** candidates on **Quality & SoTA metrics** (§4.4) and **assurance deltas** (Δ⟨F,G,R⟩).
11. Record a **DRR** ([[E.9 - Design‑Rationale Record (DRR) Method|E.9]]) with *options considered*, *trade‑offs*, chosen candidate, *blast‑radius*.
...
14. Schedule **evidence refresh** windows and an **evolution watchpoint** ([[B.4 - Canonical Evolution Loop|B.4]] loop): when metrics or SoTA inputs decay, reopen Stage B.

### [[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]]:4.2 - The **LEX‑AUTH Trace (LAT)** — what it is and why it matters
...
```
LAT:
  context: FPF/Core, pattern: [[F.15 - SCR-RSCR Harness for Unification|F.15]], semver: x.y+1, delta-class: Δ‑2
...
### [[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]]:4.3 - What counts as “changed the pattern as a whole” — **Delta‑Classes & versioning**
...
### [[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]]:4.4 - Quality & SoTA metrics (selection lenses)
...
* **Lexical discipline** ([[E.10 - Unified Lexical Rules for FPF|E.10]]): 0 violations (DevOps lexicon, process/function conflations).
...
## [[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]]:5 - Conformance Checklist (normative)
...
## [[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]]:6 - Consequences
...
## [[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]]:7 - Rationale & Links (informative)

LEX‑AUTH extends the FPF constitution by **operationalising pattern evolution**: it plugs **[[B.4 - Canonical Evolution Loop|B.4]] Canonical Evolution Loop** into **[[E.9 - Design‑Rationale Record (DRR) Method|E.9]] DRR**, binds **SoTA** via `U.EvidenceRole` and **KD‑CAL**, drives **candidate generation** with **[[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]] NQD‑CAL** under **[[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] E/E‑LOG**, enforces **lexical discipline** via **[[E.10 - Unified Lexical Rules for FPF|E.10]] LEX‑BUNDLE**, and validates with **[[F.15 - SCR-RSCR Harness for Unification|F.15]]** regression harnesses. Cross‑context safety is carried by **[[F.9 - Alignment & Bridge across Contexts|F.9]] Bridges** with **CL penalties** in **[[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] Trust**. The whole remains **notation‑independent** ([[E.5.2 - Notational Independence|E.5.2]]) and stays within the **Core → Tooling → Pedagogy** dependency rule ([[E.5.3 - Unidirectional Dependency|E.5.3]]).

## [[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]]:8 - Operators (authoring deltas you are allowed to apply)
...
## [[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]]:9 - Self‑application Work Log (how this very pattern was authored)
...
**SoTA pack (evidence bound).** Prior FPF kernel commitments to **DRR ([[E.9 - Design‑Rationale Record (DRR) Method|E.9]])**, **[[E.10 - Unified Lexical Rules for FPF|E.10]] LEX‑BUNDLE**, **[[B.4 - Canonical Evolution Loop|B.4]] Evolution**, **[[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]] and [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]]** NQD/E‑E, **[[F.15 - SCR-RSCR Harness for Unification|F.15]]** harness, **[[F.9 - Alignment & Bridge across Contexts|F.9]]** Bridges, **[[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]** Trust; these are treated as the authoritative internal SoTA for the Canon here.
...
## [[E.15 - Lexical Authoring & Evolution Protocol (LEX‑AUTH)|E.15]]:End
