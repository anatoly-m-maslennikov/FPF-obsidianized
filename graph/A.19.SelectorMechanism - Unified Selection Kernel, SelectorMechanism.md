...
> **Status:** Stable
> **Normativity:** Normative (unless explicitly marked informative)
> **Placement:** Part A / CN‑Spec cluster ([[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]]) / CHR mechanism-governing patterns (Phase‑3)
...
> [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] governs the **template** of `U.Mechanism.Intension` and the `U.MechAuthoring` discipline; this pattern governs the **SelectorMechanism-specific** slots, operations, laws, admissibility, applicability, transport, plane, time, and audit obligations for that template.
>
> **ID continuity note.** When migrating away from any legacy “card location”, preserve public anchors: keep the legacy section heading/ID as a **Tell + Cite stub** (or dock aliases via [[F.18 - Local‑First Unification Naming Protocol|F.18]]) rather than deleting or silently renaming it.
>
> **Canonicalization hook (ID‑continuity‑safe):** any other appearances of the SelectorMechanism intension content (e.g., a legacy grounding stub in [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] or suite prose in [[A.19.CHR - CHRMechanismSuite|A.19.CHR]]) SHALL be reduced to a **Tell + Cite** stub pointing to **`A.19.SelectorMechanism:4.1`**, while preserving the original section headings and their public `PatternId:SectionPath` IDs for continuity (alias‑dock legacy tokens rather than deleting them).
...
> * **Alias‑dock, don’t break:** if any legacy tokens exist (e.g., a historical `UNSELM` name token), dock them via [[F.18 - Local‑First Unification Naming Protocol|F.18]] + [[E.10 - Unified Lexical Rules for FPF|E.10]] rules; do not mint a competing head.
...
## [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:0 - At a glance — didactic, informative
...
* **How it evolves:** method semantics and SoTA algorithm families connect via [[G.2 - SoTA Harvester & Synthesis|G.2]] packs and wiring modules; the kernel signature stays stable and teachable.
...
* **P2W seam:** concrete edition/policy pin bindings (e.g., `TaskSignatureRef@edition(…)`, `CGSpecRef@edition(…)`, evidence overrides) are chosen in planned baseline plan items ([[A.15.3 - SlotFillingsPlanItem|A.15.3]] + `A.19.CHR:4.7.2`); executions only record effective refs/pins in `Audit`.
* **Transformation-flow use:** when used as a node type in [[E.18 - Transformation Flow Structure|E.18]], selector instances are chosen in planned baseline plan items (P2W); this pattern governs the intension that those instances cite.
...
## [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:1 - Problem frame
...
## [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:2 - Problem
...
## [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:3 - Forces
...
## [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:4 - Solution
...
Method semantics and SoTA algorithm families do not live inside the kernel: they connect via [[G.2 - SoTA Harvester & Synthesis|G.2]] SoTA packs and wiring modules, and via lawful specializations `⊑/⊑⁺` that obey the specialisation-chain discipline (`A.6.1:4.2.1`).

### [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:4.1 - Mechanism.Intension — normative core

Archetypal Grounding — **Mechanism.Intension** (normative).

* **Scope note:** this intension is an **instance** authored to the `U.Mechanism.Intension` shape governed by [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]. It defines only the mechanism’s semantic surface (slots/ops/laws/guards/audit). It does **not** bind project‑specific pins (P2W), and it does **not** emit GateDecision/GateLog; it emits `Audit` pins and a tri‑state guard only.
...
* **LawSet** (minimum): the selection kernel is set‑returning and policy‑bound
  ...
  5. **No competing defaults:** `PortfolioMode`/dominance defaults (when relevant) MUST be sourced from their declared governing patterns (typically via `TaskSignatureSlot` routing and/or the selector conformance/default rules in [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]), and MUST NOT be re‑declared inside the kernel.
...
### [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:4.2 - Boundary and layering rules
...
3. **Report‑only summaries inside suite closure.** Any scalar summaries, illumination metrics, or auxiliary “why not chosen” telemetry are report‑only unless explicitly promoted by policy, and MUST NOT be used as hidden dominance rules (`A.19.CHR:4.3.3`).
   Publishing and telemetry remain outside suite closure and are handled by established publication forms such as [[G.10 - SoTA Pack Shipping|G.10]] or `PTM`, not as hidden tails inside selection.
...
## [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:5 - Archetypal Grounding — informative

### [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:5.1 - Tell
...
### [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:5.2 - Show, U.System example
...
### [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:5.3 - Show, U.Episteme example
...
## [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:6 - Bias-Annotation — informative
...
## [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:7 - Conformance Checklist

| ID | Requirement |
|----|-------------|
| **CC‑A19SelectorMechanism‑0** | **MechAuthoring discipline:** the canonical SelectorMechanism `Mechanism.Intension` in `A.19.SelectorMechanism:4.1` MUST satisfy [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] `U.MechAuthoring` and the relevant `CC‑UM.*` checks; this pattern does not override the `U.Mechanism.Intension` shape. |
...
| **CC‑A19SelectorMechanism‑9** | **P2W separation:** planned baseline plan items MUST bind editions and policy pins ([[A.15.3 - SlotFillingsPlanItem|A.15.3]] + CHR P2W hook); these bindings MUST NOT be invented as run-time decisions inside the suite protocol. |
...
## [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:8 - Common Anti-Patterns and How to Avoid Them — informative
...
## [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:9 - Consequences
...
## [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:10 - Rationale
...
## [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:11 - SoTA-Echoing

**SoTA vs popular note.** This section records alignment to post‑2015 evidence‑backed practice. It is not a mandate to use fashionable methods; method semantics stay in SoTA packs ([[G.2 - SoTA Harvester & Synthesis|G.2]]) and wiring modules, while this pattern fixes the stable selection boundary.

**Pack note, Phase‑3:** this pattern does not currently cite a SelectorMechanism‑specific [[G.2 - SoTA Harvester & Synthesis|G.2]] pack or ClaimSheet. If and when such packs are introduced, they should connect via `CriteriaSlot` and `TaskSignatureSlot` routing, keeping kernel semantics unchanged.

### [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:11.1 - SoTA alignment map (normative)
...
* **Selected-set-as-output (QD framing):** adopt the *decision framing* (declared selected set as a first-class result) while keeping concrete QD/retained-set algorithms out of the kernel; they belong in [[G.2 - SoTA Harvester & Synthesis|G.2]] packs and wiring modules, preserving evolvability.
* **Archive retained sets (diversity as result):** adapt archive thinking by keeping diversity/illumination signals report‑only unless an explicit CAL/policy promotes them to dominance; this prevents silent scalarization and preserves governing-pattern defaults (typically [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] and CAL).
...
## [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:12 - Relations

* **Builds on**

  * [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] and `CC‑UM.*` for the mechanism intension shape and specialisation-chain discipline.
  * [[A.19.CHR - CHRMechanismSuite|A.19.CHR]] for suite membership, suite protocol closure, SlotKind lexicon, and threshold and default discipline.
  * [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]] for `CG‑Spec` legality and evidence surfaces.
  * [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]] for `CN‑Spec` governance card used as an explicit input.
  * [[C.22 - Problem Typing & TaskSignature Assignment (Problem-CHR)|C.22]] for `TaskSignature` as a policy-routing artifact when used.
  * [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]] for slot discipline (SlotIndex as projection; SlotKind invariance).
  * [[A.15.3 - SlotFillingsPlanItem|A.15.3]] + `A.19.CHR:4.7.2` for the P2W planned baseline seam for edition/policy pin bindings (cited as seam, not duplicated in Intension).
* **Used by**

  * [[A.19.CHR - CHRMechanismSuite|A.19.CHR]] as the canonical `select` stage in CHR pipelines.
  * [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] as the primary conformance and specialization context for selector-based method dispatch and `PortfolioMode` policies.
  * [[E.18 - Transformation Flow Structure|E.18]] when selector instances are used as transformation-flow structure nodes; planned pins live in P2W, effective pins surface via `Audit`.
* **Coordinates with**
  ...
  * [[E.20 - Mechanism Introduction Protocol|E.20]] governing-pattern discipline and [[F.18 - Local‑First Unification Naming Protocol|F.18]] alias docking for Phase‑3 canonicalization and ID continuity.

## [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]]:End
