...
## [[A.13 - The Agential Role & Agency Spectrum|A.13]]:1 - Intent & Context
...
This pattern builds directly upon the foundations laid in the FPF Kernel to provide that definition. [[A.1 - U.Holon, U.System, and U.Episteme|A.1]] established that only a **`U.System`** can be the bearer (`holder`) of behavioral roles.  [[A.2.1 - U.RoleAssignment- Contextual Role Assignment|A.2.1]] defined the universal `U.RoleAssignment` (`Holder#Role:Context`) as the canonical way to assign roles. [[A.3 - Transformer Constitution (Quartet)|A.3]] and [[A.12 - External Transformer & Reflexive Split|A.12]] defined the `TransformerRole` and the principle of the external agent.
...
## [[A.13 - The Agential Role & Agency Spectrum|A.13]]:2 - Problem
...
1.  **Episteme-as-Actor:** Models might incorrectly assign agency to knowledge epistemes or publications (`U.Episteme`), leading to nonsensical claims like "the specification decided to update the system." This is a direct violation of **Strict Distinction ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]])**.
...
3.  **Unfalsifiable Claims:** Without a measurable basis, "agency" becomes a subjective label. A team might call their system an "agent" for marketing purposes, but this claim has no verifiable meaning and cannot be audited, violating **Evidence Graph Referring ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]])**.
...
## [[A.13 - The Agential Role & Agency Spectrum|A.13]]:3 - Forces
...
| **System-only Action vs. Collective Action**| How does agency apply to groups like teams or swarms? This requires a clear link to the rule from [[A.1 - U.Holon, U.System, and U.Episteme|A.1]] that any *acting group* must be modeled as a `U.System`. |

## [[A.13 - The Agential Role & Agency Spectrum|A.13]]:4 - Solution

FPF's solution is threefold: it defines an Agent via `U.RoleAssignment` ([[A.2.1 - U.RoleAssignment- Contextual Role Assignment|A.2.1]]), makes agency measurable with a dedicated Characterization, and provides a didactic summary via a graded scale.

### [[A.13 - The Agential Role & Agency Spectrum|A.13]]:4.1 - The Core Definition: Agent as a Contextual Role Assignment
...
### [[A.13 - The Agential Role & Agency Spectrum|A.13]]:4.2 - The `AgentialRole` and its Specializations
...
### [[A.13 - The Agential Role & Agency Spectrum|A.13]]:4.3 - Measuring Agency: The `Agency-CHR` and the Spectrum
...
The `Agency-CHR` profile is grounded in contemporary research (e.g., Active Inference, Basal Cognition) and includes the following key characteristics. Each is measured for a specific agent in a specific context and must be backed by evidence ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]).
...
#### [[A.13 - The Agential Role & Agency Spectrum|A.13]]:4.3.1 - Context-bounded task-family specialization claims
...
### [[A.13 - The Agential Role & Agency Spectrum|A.13]]:4.4 - The Agency Grade (Didactic Layer)
...
## [[A.13 - The Agential Role & Agency Spectrum|A.13]]:5 - Archetypal Grounding
...
## [[A.13 - The Agential Role & Agency Spectrum|A.13]]:6 - Conformance Checklist
...
| :--- | :--- | :--- |
| **CC-A13.1 (Holder Type)** | The `holder` of a `U.RoleAssignment` with `role: U.AgentialRole` **MUST** be a `U.System`. | Prevents the "episteme-as-actor" category error. Enforces **Strict Distinction ([[A.7 - Strict Distinction (Clarity Lattice)|A.7]])**. |
...
| **CC-A13.3 (CHR Evidence)** | Any claim about an Agent's Agency Grade or autonomy profile **MUST** be substantiated by an auditable `Agency-CHR` profile with Evidence Graph Ref ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]). | Makes claims of agency falsifiable and prevents "agency by marketing." |
...
| **CC-A13.5 (Collective as System)** | To claim agency for a collective (e.g., a team, a swarm), the collective **MUST** first be modeled as a `U.System` with a defined `U.Boundary` and a coordination `U.Method`. | Prevents the error of assigning agency to a mere set or collection (`MemberOf`). Aligns with **[[A.1 - U.Holon, U.System, and U.Episteme|A.1]]** and **[[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]**. |
| **CC-A13.6 (MHT for Emergent Agency)** | If a collection of systems, previously non-agential or at a lower grade, develops a new supervisory structure and crosses a documented `Agency-CHR` threshold, a **Meta-Holon Transition (MHT, [[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]])** **MUST** be declared. | Makes the emergence of collective agency an explicit, auditable event, preventing "magic" emergence. |

## [[A.13 - The Agential Role & Agency Spectrum|A.13]]:7 - Consequences
...
## [[A.13 - The Agential Role & Agency Spectrum|A.13]]:8 - Rationale
...
## [[A.13 - The Agential Role & Agency Spectrum|A.13]]:9 - Relations
...
## [[A.13 - The Agential Role & Agency Spectrum|A.13]]:End
