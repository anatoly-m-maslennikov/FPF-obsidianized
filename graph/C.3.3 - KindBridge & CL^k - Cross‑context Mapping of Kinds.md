...
**Status.** Normative in **Part C**. Identifier **[[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]**.
**Audience.** Engineering managers, architects, assurance leads, editors.

**Depends on.**

* **[[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]] — U.Kind & SubkindOf (Core):** kinds are context-local `U.Kind` records; `⊑` is a partial order; kinds **do not carry Scope**.
* **[[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]] — KindSignature (+F) & Extension/MemberOf:** signature declares its own **F**; membership `MemberOf(e,k,slice)` is **deterministic** per `U.ContextSlice`.
* **[[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] — USM (Context slices & Scopes):** Claim scope (**G**) and Work scope live on claims/capabilities; scope bridging and **CL** penalties are defined there.
* **[[C.2.2 - Reliability R in the F-G-R triad|C.2.2]] — F–G–R:** weakest‑link; penalties land in **R**, not **F/G**.
* **[[C.2.3 - Unified Formality Characteristic F|C.2.3]] — U.Formality (F):** signature rigor.
...
## [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:1 - Purpose & Audience
...
**[[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]** gives managers an explicit, auditable channel for **(2)**, so a team can say, with evidence: *“`Vehicle` in Lab maps to `TransportUnit` in Plant with `CL^k=2`; the EV subkind collapses; here’s what we lost.”* Guards stay deterministic; assurance math stays clean (penalties in **R** only).

## [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:2 - Context
...
## [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:3 - Problem
...
## [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:4 - Forces
...
## [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:5 - Solution — The **KindBridge** object (overview)
...
## [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:6 - Norms & Invariants (normative)

> The following formalize the **KB‑01…KB‑12** rules announced in [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]].

### [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:6.1 - Subject & Scope of a KindBridge
...
**KB‑02 (No Scope).** A KindBridge **MUST NOT** map Claim/Work scope (**G**). Scope translation uses the **USM Bridge + CL** channel ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]], Part B).

**No blended score.** Congruence for Scope (**CL**) and for Kind (**CL^k**) **MUST NOT** be aggregated into a single “interoperability” score in guards; each channel is assessed and penalized **separately**. See **Annex [[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]] §5 (E‑06)**.

### [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:6.2 - Declaration & Shape
...
### [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:6.3 - Order & Monotonicity
...
### [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:6.5 - Loss Notes & Definedness
...
## [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:7 - Interactions (informative)

### [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:7.1 - With USM Scope bridges (two channels)
...
**Discipline:** compute both; **do not** collapse them into one “interoperability score.”
 ...
 See **Annex [[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]] §5 (E‑01)** for the normative evaluation order in guards.

### [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:7.2 - With membership ([[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]])
...
### [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:7.3 - With Role masks ([[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]])
...
### [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:7.4 - With guards (Annex [[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]])

Use the **`Guard_XContext_Typed`** macro (Annex [[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]]), which requires **both bridges** and applies **both penalties** to **R**:
...
## [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:8 - Authoring, Review & Rating Guidance (informative)

### [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:8.1 - Authoring a KindBridge
...
### [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:8.2 - Review playbook (10 minutes)
...
### [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:8.3 - Rating `CL^k` (rules of thumb)
...
## [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:9 - Worked Examples (informative)

### [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:9.1 - Vehicle → TransportUnit (manufacturing)
...
### [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:9.2 - AuthenticatedRequest across services (software)
...
### [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:9.3 - AdultPatient across jurisdictions (clinical)
...
## [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:10 - Anti‑patterns & Remedies (informative)
...
## [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:11 - Conformance Checklist (normative)
...
## [[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]]:End
