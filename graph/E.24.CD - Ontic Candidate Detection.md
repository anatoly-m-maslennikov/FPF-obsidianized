...
## [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:0 - Use This When
...
**What this buys.** The author gets a compact candidate cluster and a sufficiency rationale before opening [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]. This keeps [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] small and keeps candidate discovery from becoming a registry, score form, or warning catalogue.

**Not this pattern when.**

- If the durable ontic is already selected and its identity and slot relation must be governed, use [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]].
- If the current problem is only confusion between an ontic, its description, and publication forms, use [[E.24.PUB - Ontic Description and Publication Discipline|E.24.PUB]].
- If an existing subject pattern already governs the claim, use that pattern directly.
- If the issue is one wording-use repair, use [[E.10 - Unified Lexical Rules for FPF|E.10]], [[E.10.ARCH - Wording-Use Ontological Precision Restoration Architecture|E.10.ARCH]], or the relevant precision-restoration pattern.
- If the contested question is how to compare pattern-set architecture alternatives, construct the evaluation `CharacteristicSpace` through [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]].

## [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:1 - Problem Frame

[[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] introduces or rejects a durable `U.Ontic`. Before that decision, FPF often needs a smaller move: detect whether an apparent subject is actually a candidate ontic or merely a record, local frame, source label, draft locus, or direct use of existing patterns.
...
[[E.24.CD - Ontic Candidate Detection|E.24.CD]] governs that detection move. It prepares an [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] decision; it does not make the durable ontic decision by itself.

## [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:2 - Problem
...
## [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:3 - Forces
...
| First-use affordability vs full architecture analysis | The author needs a quick first move; contested alternatives can use [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] in a separate evaluation frame without making every candidate pass a scoring ritual. |
...
## [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:4 - Solution

Use an `OnticCandidateCluster` as a local detection aid. It is not a `U.*` kind, not a permanent registry entry, and not the ontic. It is a compact description of why the author is considering an [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] decision.
...
- `PossibleSlotRelation` sketches the candidate relation only enough to decide whether [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] should open.
...
If the signals do not cohere around one concern, do not open [[E.24.CD - Ontic Candidate Detection|E.24.CD]] only to collect them. Use the direct governing pattern, [[E.10.ARCH - Wording-Use Ontological Precision Restoration Architecture|E.10.ARCH]], [[E.24.PUB - Ontic Description and Publication Discipline|E.24.PUB]], or a local-use disposition.

### [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:4.2 - Hidden Form Classifications

Classify the detected construct before opening [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:

| Classification | Meaning | Next move |
| --- | --- | --- |
| Durable ontic candidate | The concern appears to need stable identity, a type-level slot relation, semantic area, ontological neighborhood, and dependent-pattern reliance. | Open [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]. |
...
| Publication-form-only case | The visible object is a card, record, table, schema, diagram, view, packet, or source form that publishes or organizes another EoC. | Use [[E.24.PUB - Ontic Description and Publication Discipline|E.24.PUB]] or the relevant publication pattern. |
| Source wording only | The source label compresses several values but should not enter current FPF vocabulary. | Keep quote-only or reduced-use; use [[E.10.ARCH - Wording-Use Ontological Precision Restoration Architecture|E.10.ARCH]] if repair is needed. |
| Evaluation-construction case | The current problem is comparing pattern-set architecture alternatives. | Build the evaluation `CharacteristicSpace` through [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]]. |

### [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:4.3 - Sufficiency Rationale

If the classification is durable ontic candidate, write a short sufficiency rationale before opening [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:
...
### [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:4.4 - Project Data-Structure Recovery
...
Example: an "ArchitectureDecisionRecord" may carry an architecture move, selected structure, decision, evidence, source freshness, gate condition, responsible role assignment, and publication date. That record is not a root `U.ArchitectureDecisionRecord` ontic by appearance. It may be a publication form over values governed by [[C.30 - Grounded Architecture and Selected-Structure Adequacy|C.30]], decision, gate, evidence, source, role-assignment, and [[E.24.PUB - Ontic Description and Publication Discipline|E.24.PUB]] patterns. Only if the relation itself needs stable identity and dependent-pattern reliance does [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] open.

### [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:4.5 - Stop Conditions

Stop [[E.24.CD - Ontic Candidate Detection|E.24.CD]] when one of these dispositions is reached:

- **Open [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]]:** durable ontic candidate is selected for a full ontic-introduction decision.
...
Do not keep [[E.24.CD - Ontic Candidate Detection|E.24.CD]] open as a standing registry of possibilities. Once the disposition is clear, move to the selected governing pattern.

## [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:5 - Archetypal Grounding

### [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:5.1 - Episteme Candidate That Becomes a Durable Ontic

Before [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]], "description", "view", "claim set", and "publication" could be confused. [[E.24.CD - Ontic Candidate Detection|E.24.CD]] would detect stable concern across forms, typed-value spread, slot doctrine, publication-form temptation, and dependent-pattern need. The sufficiency rationale points to a durable ontic: `U.Episteme`, with EntityOfConcernSlot, claim graph, viewpoint, reference scheme, grounding, and publication-form boundaries.

The next move is E.24-style introduction, then the governing pattern [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]. The cards and publications are not the episteme; they describe or publish it.

### [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:5.2 - Problem Card as Stress Case
...
[[E.24.CD - Ontic Candidate Detection|E.24.CD]] asks what is under concern:
...
- If several patterns need a reusable problem ontology with stable identity and slot relation, open [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]].
...
### [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:5.3 - Project Schema With Ontology-Looking Fields
...
The table may be a planning aid, work record, publication form, or schema for project tooling. Its fields reveal possible FPF values, but they do not decide their kinds. [[E.24.CD - Ontic Candidate Detection|E.24.CD]] recovers the project concern and typed values first. If the relation among method, mechanism, work, evidence, result, source, and transformed entity is governed by existing transformation, method, work, mechanism, evidence, source, and publication patterns, keep the table as a publication or local use frame. If dependent patterns need a reusable relation that existing patterns do not provide, open [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]].

### [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:5.4 - Characteristic Space as Candidate

Evaluation work often starts as a score table. The visible table may hide a `U.CharacteristicSpace`: characterized object kind, characteristics, scale bindings, value meanings, coordinate groups, missingness semantics, normalization boundaries, comparability boundaries, and evidence hooks. If the score table is only a local report, use the relevant evaluation pattern. If several patterns rely on that space as a reusable object, the candidate can be selected as `U.CharacteristicSpace` and governed by [[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]].

The table is not the characteristic space. It can publish one filled evaluation over the space.

## [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:6 - Bias-Annotation
...
The mitigation is concrete: recover the recognizable concern, typed values, current governing patterns, and possible slot relation before naming a candidate or opening [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]].

## [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:7 - Conformance Checklist
...
| `CC-E24CD-8` | Publication-form confusion is sent to [[E.24.PUB - Ontic Description and Publication Discipline|E.24.PUB]] rather than solved by declaring the form to be the ontic. |
| `CC-E24CD-9` | Contested comparison of architecture alternatives is sent to [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] rather than built into [[E.24.CD - Ontic Candidate Detection|E.24.CD]]. |
| `CC-E24CD-10` | The stop condition names one next governing pattern or reduced-use disposition. |

## [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:8 - Common Anti-Patterns and How to Avoid Them

| Anti-pattern | Symptom | Repair |
| --- | --- | --- |
| Card-to-kind jump | A useful card is promoted into a `U.*` kind because it has repeated fields. | Recover the EoC and typed values; use [[E.24.PUB - Ontic Description and Publication Discipline|E.24.PUB]] for publication form. |
...
| One-word candidate | A broad word is renamed and treated as settled. | Use [[E.10.ARCH - Wording-Use Ontological Precision Restoration Architecture|E.10.ARCH]] and [[E.24.CD - Ontic Candidate Detection|E.24.CD]] together: recover typed values and slot relation before naming. |
| Registry trap | The author keeps a growing list of possible ontics without disposition. | Stop at one of the [[E.24.CD - Ontic Candidate Detection|E.24.CD]] classifications and move to the next governing pattern. |
| Scoring before identity | A score form compares alternatives before the candidate EoC and slot relation are clear. | First write the sufficiency rationale; use [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] only when evaluation construction is actually current. |
...
## [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:9 - Consequences

Positive consequences:

- [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] stays compact because candidate discovery has its own pattern.
...
- The author must do a short detection pass before opening [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]] for a new durable ontic.
- Some attractive names are lowered to source wording, local frames, or publication forms.
- A contested architecture comparison may require a separate [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] evaluation construction instead of a quick authorial preference.

## [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:10 - Rationale

FPF needs [[E.24.CD - Ontic Candidate Detection|E.24.CD]] because ontic candidates are rarely visible as pure ontology. They show up as forms that people use: project tables, cards, schemas, diagrams, source packets, draft pattern rows, examples, and repeated words. Those forms are important because they reveal project concerns, but they are unreliable as ontology decisions.
...
## [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:11 - SoTA-Echoing

| Source family | Current lesson for [[E.24.CD - Ontic Candidate Detection|E.24.CD]] | FPF decision |
| --- | --- | --- |
...
## [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:12 - Relations

- **Builds on:** [[E.24 - U.Ontic and Ontic Introduction Discipline|E.24]], [[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]], [[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]], [[E.10 - Unified Lexical Rules for FPF|E.10]], [[E.10.ARCH - Wording-Use Ontological Precision Restoration Architecture|E.10.ARCH]], [[F.18 - Local‑First Unification Naming Protocol|F.18]], [[F.19 - Ontology-First Plain Technical Rewriting|F.19]], and [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]].
- **Coordinates with:** [[E.24.PUB - Ontic Description and Publication Discipline|E.24.PUB]] for ontic-description and publication-form boundary, [[A.19 - CharacteristicSpace & Dynamics Hook (A.CHR‑SPACE)|A.19]] for `U.CharacteristicSpace`, [[A.19.ECS - Evaluation CharacteristicSpace Construction|A.19.ECS]] for evaluation-characteristic construction, and the governing subject patterns for values recovered in the candidate cluster.
...
## [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:13 - Footer Marker

## [[E.24.CD - Ontic Candidate Detection|E.24.CD]]:End
