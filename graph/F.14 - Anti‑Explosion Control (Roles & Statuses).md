...
**Depends on.** [[F.1 - Domain‑Family Landscape Survey|F.1]] **context of meaning**; [[F.2 - Term Harvesting & Normalisation|F.2]] **Harvesting**; [[F.3 - Intra‑Context Sense Clustering|F.3]] **Local Sense Clustering**; [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] **Role Description**; [[F.5 - Naming Discipline for U.Types & Roles|F.5]] **Naming Discipline**; [[F.7 - Concept‑Set Table|F.7]] **Concept‑Set Table**; [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] **Mint‑or‑Reuse**.
**Coordinates with.** [[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] **Status Windows & Mapping**; [[F.11 - Method Quartet Harmonisation|F.11]] **Method Quartet Harmonisation**; [[F.12 - Service Acceptance-Work Evidence Link|F.12]] **Service Acceptance Binding**; [[F.13 - Lexical Continuity & Deprecation|F.13]] **Lexical Continuity**.
**Aliases (informative).** *Role and Status economy*; *Explosion guard*.

## [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:1 - Intent & applicability

**Intent.** Prevent the uncontrolled growth of **Roles** and **Statuses** by privileging **reuse**, **bundling**, **explicit separation‑of‑duties (SoD)**, and **applicability windows** over minting new names. Keep the vocabulary **small, crisp, and composable** while remaining faithful to local meanings fixed by Contexts ([[F.1 - Domain‑Family Landscape Survey|F.1]]) and SenseCells ([[F.3 - Intra‑Context Sense Clustering|F.3]]).

**Applicability.** Whenever a new Role or Status is proposed, a team merges two lines of work, or a domain shifts its jargon. Use this pattern before adding rows to the Concept‑Set Table ([[F.7 - Concept‑Set Table|F.7]]) or new Role Descriptions ([[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]).
...
## [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:2 - Problem frame
...
Explosion harms didactics and increases alignment cost ([[F.9 - Alignment & Bridge across Contexts|F.9]]).

## [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:3 - Forces
...
## [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:4 - Minimal vocabulary (this pattern only)

* **Role Description** ([[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]): a **Role** (behavioural mask) or **Status** (epistemic/deontic standing) tied to a **SenseCell**.
* **Concept‑Set row** ([[F.7 - Concept‑Set Table|F.7]]): a Cross‑context **intent** (“what we count as one thing”) aligned by SenseCells.
...
* **Window** ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]): an **claim scope** (time stance, holon level, run segment) that delimits when a Role or Status holds.

## [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:5 - Core idea (didactic)
...
## [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:6 - Solution — the control cabinet (conceptual, notation‑free)

### [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:6.1 - Reuse by row (first lever)

* **Move.** If a proposal matches the **intension** of an existing row ([[F.7 - Concept‑Set Table|F.7]]), adopt its Role Description or add a local SenseCell **inside that row**.
* **Pay‑off.** Names don’t proliferate; Cross‑context tables stay thin.

**Example (services).** *Service‑availability‑compliance* already exists as a row. New labels *SLO‑Met* / *Uptime‑OK* **reuse** that row; SOSA/SSN Observations later feed it ([[F.12 - Service Acceptance-Work Evidence Link|F.12]]).

### [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:6.2 - Bundle instead of hybrid (second lever)
...
### [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:6.3 - Separate by SoD, don’t evade (third lever)
...
### [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:6.4 - Window the Status (fourth lever)
...
### [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:6.5 - Factor modifiers as facets, not names
...
## [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:7 - Invariants (normative)

1. **context‑locality.** Each Role Description remains tied to a **SenseCell** in a **single Context** ([[F.3 - Intra‑Context Sense Clustering|F.3]], [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]).
2. **Row preference.** New Role Descriptions **SHOULD** map to an existing row; new rows ([[F.7 - Concept‑Set Table|F.7]]) require [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] justification.
3. **No hybrid Roles.** If two Roles are conceptually distinct, they **must not** be fused into one to bypass SoD. Use **Bundle + SoD**.
4. **Windowed statuses.** Status proliferation across time/scale **MUST** be expressed as **windows** of a single Status family ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]).
5. **Bundle clarity.** A Bundle **names only composition**; it does not inherit or redefine member semantics.
6. **Minimal modifier naming.** Adding a modifier to a label **MUST** pass [[F.5 - Naming Discipline for U.Types & Roles|F.5]] tests; prefer facets/windows over new Role or Status names.
7. **Concept‑first.** No invariant relies on organization charts or access policies; **semantics precede governance**.

## [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:8 - Reasoning primitives (judgement schemas)
...
## [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:9 - Micro‑examples (engineer, manager, and researcher lenses)

### [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:9.1 - Enactment (change control)
...
### [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:9.2 - Services (SLO evaluation)
...
### [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:9.3 - KD‑CAL (evidence)

* **Proposal.** *Pre‑validated* between *Verified* and *Validated*.
* **Moves.** Use **Status chain** within one family: `Verified → Validated`; represent uncertainty as **confidence** ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]), not another Status.
* **Result.** Clean ladder; no extra label.

### [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:9.4 - Sys‑CAL (plant ops)
...
### [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:9.5 - Method quartet (reviews)

* **Proposal.** *Senior‑Reviewer* to bypass `{Author ⟂ Reviewer}`.
* **Moves.** Keep SoD; if seniority matters, introduce **Assurance Level** facet ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]) on the **review decision**, not a new Role.
* **Result.** Separation preserved; trust expressed as a Status property, not a Role type.

## [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:10 - Anti‑patterns & remedies
...
| **A2**  | **Modifier‑as‑type**                     | *Night‑Operator*, *Remote‑Operator*, *On‑call‑Reviewer*.                                     | Name proliferation for circumstantial qualifiers.                 | Keep **Role = Operator/Reviewer**; express *night/remote/on‑call* as **facets or windows** ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]).                                |
| **A3**  | **Window‑as‑type**                       | *Compliant*, *At‑Risk*, *Grace*, *Breached* as separate Status types.                        | Paints temporal phases as different essences; breaks comparisons. | One **Status family** (e.g., **Compliance**) with **windows**: *evaluation / active / grace / archival* ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]).                   |
| **A4**  | **Row drift**                            | New Concept‑Set row for *Uptime‑OK* when **Service‑Availability‑Compliance** already exists. | Splits one intent across rows; Cross‑context tables get wide.        | **Reuse the row** ([[F.7 - Concept‑Set Table|F.7]]) if intent matches; add local **SenseCell** if needed.                                                     |
...
| **A6**  | **cross-context fusion**                    | One Role Description mixes *Execution (IEC)* with *Duty (ODRL)* semantics.                       | Violates locality; meanings leak across Contexts.                    | Keep each **Role Description** tied to a **SenseCell** ([[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]). cross-context reasoning uses **Bridge** ([[F.9 - Alignment & Bridge across Contexts|F.9]]).         |
| **A7**  | **Synonym carousel**                     | *Validator* vs *Verifier* vs *Checker* minted separately in the same Context.                   | Cognitive noise; ambiguous separation.                            | Choose **one label** via [[F.5 - Naming Discipline for U.Types & Roles|F.5]]; keep others as **aliases** in **Lexical Continuity** ([[F.13 - Lexical Continuity & Deprecation|F.13]]), not new templates.                     |
| **A8**  | **Org-chart mirroring**                  | Roles cloned from a company chart (*Squad-Lead*, *Tribe-Lead*) as generic **Role Descriptions**. | Organisation-specific names masquerade as semantics.              | Map local titles to **Bundles** of generic Roles (e.g., `{Planner, Coordinator}`), or treat as **aliases** ([[F.13 - Lexical Continuity & Deprecation|F.13]]). |
...
| **A13** | **Contextless Role Description**            | Role Description without a SenseCell anchor.                                                     | Floating meaning; later bridges cannot be made explicit.          | Tie every **Role Description** to a **SenseCell** ([[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]). If none fits, use [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] to decide: **new row** or **rename/reuse**.        |
...
## [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:11 - Worked examples

### [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:11.1 - Enactment + Services + KD‑CAL — “SLO compliance without label sprawl”
...
* **Moves ([[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]).** Keep **Compliance** as one **Status family**; define **windows**: *evaluation* (prediction against forecast), *active* (actuals vs target), *grace* (tolerated breach). **Waiver** becomes a **deontic Status** in ODRL Context, not part of Compliance.
...
### [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:11.2 - Method‑CAL + Enactment — “Reviews with SoD and Bundle”
...
### [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:11.3 - Sys‑CAL + LCA‑CAL + Services — “Operations without role fragments”
...
### [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:11.4 - KD‑CAL + Kind-CAL — “Evidence ladder without new labels”
...
## [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:12 - Relations (with other patterns)

* **Builds on:** [[F.1 - Domain‑Family Landscape Survey|F.1]] (Contexts), [[F.2 - Term Harvesting & Normalisation|F.2]] (Harvesting), [[F.3 - Intra‑Context Sense Clustering|F.3]] (Local Clustering), [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] (Role Description), [[F.5 - Naming Discipline for U.Types & Roles|F.5]] (Naming).
* **Constrains:**

  * **[[F.7 - Concept‑Set Table|F.7]] (Concept‑Set Table):** prefer **row reuse**; new rows require [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] justification.
  * **[[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] (Mint‑or‑Reuse):** apply **four levers** (reuse, bundle, SoD, window) before minting.
  * **[[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] (Status Windows & Mapping):** encode temporal/scale variation as **windows**, not new Status types.
  * **[[F.12 - Service Acceptance-Work Evidence Link|F.12]] (Service Acceptance Binding):** bind acceptance to the **Compliance** Status family; avoid ad‑hoc status labels.
  * **[[F.13 - Lexical Continuity & Deprecation|F.13]] (Lexical Continuity):** prior names become **aliases**; do not carry forward inflated vocabularies as new types.
* **Used by.** FPF patterns to keep Role and Status vocabularies tight.

## [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:13 - Migration notes (conceptual playbook)

1. **Map to rows.** For each existing Role or Status, identify its **Concept‑Set row**; if two names share an intent, **collapse** to one row (keep other names as **aliases**, [[F.13 - Lexical Continuity & Deprecation|F.13]]).
...
5. **Re‑use before minting.** When encountering a gap, scan rows for a near‑match; only if intent genuinely differs, open a **new row** ([[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]).
6. **Preserve continuity.** Keep historic labels as **aliases** under the consolidated template ([[F.13 - Lexical Continuity & Deprecation|F.13]]); do not rewrite past texts.
...
## [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:14 - Acceptance tests (SCR/RSCR — concept‑level)

### [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:14.1 - SCR — Static conformance

* **SCR‑F14‑S01 (Row reuse).** Every newly proposed Role Description either **references an existing row** or includes a clear **[[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]] justification** for a new row.
...
### [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:14.2 - RSCR — Regression (evolution)

* **RSCR‑F14‑E01 (Vocabulary slope).** Over a given interval, the count of distinct Role or Status templates **does not increase** unless matched by **row justifications** ([[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]).
...
* **RSCR‑F14‑E04 (Alias discipline).** When labels change, prior names are recorded as **aliases** ([[F.13 - Lexical Continuity & Deprecation|F.13]]); no silent type multiplication.

## [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:15 - Didactic distillation (90‑second script)
...
## [[F.14 - Anti‑Explosion Control (Roles & Statuses)|F.14]]:End
