...
**Depends on.** A‑kernel ([[A.1 - U.Holon, U.System, and U.Episteme|A.1]]–[[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]), **MM‑CHR** ([[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]]) for measurements, **KD‑CAL** for similarity/corpora, **Sys‑CAL** for carriers, **Decsn‑CAL** (objectives; advisory), **Compose‑CAL** (set aggregation; advisory).

**Coordinates with.** **[[B.5.2.1 - Creative Abduction with NQD|B.5.2.1]]** (binding), **[[C.17 - Characterising Generative Novelty & Value (Creativity‑CHR)|C.17]] Creativity‑CHR** (characteristics & scales), **[[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] E/E‑LOG** (policies: emitter selection, explore/exploit).
...
## [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]:1 - Problem frame
Open‑ended search (NQD) equips FPF with illumination‑style generation and Pareto / selected-set selection in multi‑criteria, partially ordered spaces; it feeds [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] without scalarising ordinal or mixed‑scale characteristics.

## [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]:2 - Problem
...
## [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]:3 - Forces
...
## [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]:4 - Solution
...
**Type notes.**
- `U.DescriptorMap (Tech; twin‑labelled Plain) : Hypothesis → ℝ^d` (declares encoder, invariances, version, **CharacteristicSpaceRef**). Publish Tech/Plain per **[[E.10 - Unified Lexical Rules for FPF|E.10]]**; declare `DescriptorMapRef.edition` and `DistanceDefRef.edition`. **Dimensionality rule.** **Require `d≥2` only when QD/illumination surfaces are active**; for non‑QD contexts `d≥1` is lawful.
...
- `U.EmitterPolicyRef` points to a named policy in **[[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] E/E‑LOG**.
...
- `U.IlluminationSummary` is a **telemetry summary** over `Diversity_P` (see [[C.17 - Characterising Generative Novelty & Value (Creativity‑CHR)|C.17]]), not a dominance characteristic.
...
## [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]:5 - Conformance Checklist
...
- **C18‑2** Record `EmitterPolicyRef` (policy id from [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]]) and parameter set.
- **C18‑3** Compute `D = ΔDiversity_P(h | Pool)` under the same DescriptorMap & TimeWindow as the Pool (see [[C.17 - Characterising Generative Novelty & Value (Creativity‑CHR)|C.17]]).
- **C18‑4** Exclude Illumination from dominance unless policy explicitly promotes it.
- **C18‑5** Keep `Use‑Value` separate from assurance scores; do not alter `F/G/R` semantics (see [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], [[C.17 - Characterising Generative Novelty & Value (Creativity‑CHR)|C.17]] §Use‑Value).
...
**Defaults.** Default-governance responsibility is split on purpose: [[G.Core - Part G Core Invariants|G.Core]] and [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] govern `DefaultId.DominanceRegime` and selector-facing default routing, while [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] governs emitter, insertion, and pool-policy defaults. [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]] consumes those defaults and records the active refs instead of restating them locally. Minimum provenance remains: `DescriptorMapRef.edition` and `DistanceDefRef.edition`, `DHCMethodRef.edition`, `EmitterPolicyRef`, `InsertionPolicyRef`, `TimeWindow`, `Seeds`, `DedupThreshold?`; also record `FamilyCoverage/MinInterFamilyDistance`.
...
## [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]:6 - Archetypal Grounding
...
## [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]:7 - Bias‑Annotation
Lexical firewall and notation independence apply; no vendor/tool tokens; ordinal characteristics never averaged; illumination treated as report‑only telemetry unless a policy promotes it. ([[E.5.1 - DevOps Lexical Firewall|E.5.1]], [[E.5.2 - Notational Independence|E.5.2]], [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]])

## [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]:8 - Consequences
...
## [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]:9 - Rationale
...
## [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]:10 - Relations
Builds on: [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]], [[C.2 - Epistemic holon composition (KD-CAL)|C.2]]. Coordinates with: [[B.5.2.1 - Creative Abduction with NQD|B.5.2.1]] (binding), [[C.17 - Characterising Generative Novelty & Value (Creativity‑CHR)|C.17]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.6 - Evidence Graph & Provenance Ledger|G.6]], [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]].

## [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]]:End
