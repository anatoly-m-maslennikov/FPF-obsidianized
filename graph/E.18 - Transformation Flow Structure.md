...
TransformationFlowStructure:
  ...
  mathematicalDescriptionRef?: [[E.18.2 - Transformation Flow Mathematical Description|E.18.2]] only if a graph, algebra, or category expression is being used.
```
...
**Structure ontology.** [[E.18 - Transformation Flow Structure|E.18]] keeps these distinctions primary:
...
| transformation locus | an [[E.18 - Transformation Flow Structure|E.18]] locus, path, path slice, substructure, or valuation used to express, decompose, constrain, or locate a bounded `U.Transformation` | governed by [[A.3.4 - U.Transformation- Bounded Change Under Conditions|A.3.4]] for transformation identity and slots |
...
When a sentence says that a system performs a functional transformation at one point in a flow, [[E.18 - Transformation Flow Structure|E.18]] carries only the selected flow structure, locus, path, slice, crossing, valuation, and pins. The bounded transformation, transformer or candidate bearer, input/output or functional-port boundary, functioning relation, method/algorithm, mechanism, and performed work are recovered through [[A.3.4 - U.Transformation- Bounded Change Under Conditions|A.3.4]], [[A.6.F - Function and Functional Precision Restoration (RPR-FUNCTION)|A.6.F]], [[C.30.ASV - Architecture Structural View Adequacy (ASV)|C.30.ASV]], [[A.6.M - Module Relation Repair|A.6.M]], [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]], and the [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] family as applicable. A computational algorithm may fill `MethodRef?` or `MethodDescriptionRef?`; a physical-world way of transforming may fill `U.Method`; neither is inferred from [[E.18 - Transformation Flow Structure|E.18]] structure membership.

**Not this pattern when.** Use [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] for internal step validity, [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] for gate-decision publication, [[E.20 - Mechanism Introduction Protocol|E.20]] for mechanism-governing-definition placement, [[A.3.4 - U.Transformation- Bounded Change Under Conditions|A.3.4]] for bounded transformation under conditions, [[E.18.2 - Transformation Flow Mathematical Description|E.18.2]] for mathematical descriptions of the selected structure, [[C.27.TA - Temporal Aspect- Time Windows, Rhythm, Cadence, and Currentness|C.27.TA]] for temporal aspects, [[C.27 - Temporal Claim Adequacy- State Readings, Temporal Trends, and Intervention-Sensitive Temporal Change|C.27]] for temporal-claim adequacy or supported-use claims, the [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] family for work planning or performed work, [[E.17 - Multi‑View Publication Kit|E.17]] for publication faces, and [[E.10 - Unified Lexical Rules for FPF|E.10]] for wording-use repair when the current EntityOfConcern is not the selected structure, path, crossing, or flow valuation.
...
**What this buys.** [[E.18 - Transformation Flow Structure|E.18]] keeps selected structure, publication pins, crossings, CV/GF separation, and refresh locality in one current structure pattern without turning every domain-specific path into its own flow doctrine or every mathematical graph description into the selected structure.

## [[E.18 - Transformation Flow Structure|E.18]]:2 - Problem frame

Teams can produce many **well-typed flow valuations** for transformations of the same context holon under `VP.Functional`, for example for a declared `U.Capability` or transformation claim. The holon is the described/context object; the [[E.18 - Transformation Flow Structure|E.18]] `EntityOfConcern` remains the selected `TransformationFlowStructure` over transformations and adjacent governed loci. The P2W reference path is:
`U.Signature(profile=FormalSubstrate) -> U.PrincipleFrame -> U.Mechanism -> U.ContextNormalization (UNM) -> SelectionAndTuning locus (G.5/selector relation) <-> WorkPlanning locus ([[A.15.2 - U.WorkPlan|A.15.2]] U.WorkPlan or plan-item relation) -> U.Work -> EvaluatingAndRefreshing locus (G.11/refresh relation)`
...
MVPK already fixes publication drift at the **single-arrow** scope; [[E.18 - Transformation Flow Structure|E.18]] lifts those **publication and comparability laws** to the **selected transformation-flow structure as a whole**.

## [[E.18 - Transformation Flow Structure|E.18]]:3 - Problem
...
## [[E.18 - Transformation Flow Structure|E.18]]:4 - Forces
...
* **Scopes:** `Gamma_time` (budgets, horizons), `PublicationScope` for faces ([[E.17 - Multi‑View Publication Kit|E.17]]), and **slice ids** for refresh ([[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]).

 **CtxState (PS‑projection; closed slots):** `CtxState = ⟨L, P, E⃗, D⟩` is the **projection of [[E.17 - Multi‑View Publication Kit|E.17]] Publication Scope**.
 **Slot definitions (normative):**
  ...
  • `P := ReferencePlane` — the reference plane/units registry id; **no plane/unit declarations or translations** occur in CV; crossings remain gated ([[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]).
 ...
 **Data-shape location.** [[E.18 - Transformation Flow Structure|E.18]] names the structure and valuation obligations for `PathId`, `PathSliceId`, Gamma pins, and lineage: flow is a valuation over `U.Transfer`, raw transfer preserves `CtxState`, and path or slice evidence is carried through this pattern plus [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]], with [[G.6 - Evidence Graph & Provenance Ledger|G.6]] or [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] for cases involving evidence-path visibility or refresh wiring. These are the current structure loci for path and slice currentness.

 * **Locus kinds:** `Transformation`, `Signature`, `Mechanism`, `WorkPlanning`, `Work`, `Check`, and `StructuralReinterpretation` are the current minimal structure-positioned locus baseline. Domain-specific species are open-world and non-exhaustive, but each species binds to one of the locus kinds or requires an explicit [[E.18 - Transformation Flow Structure|E.18]] update. These are positioned loci in the selected structure, not a local taxonomy of new FPF kinds.
  **Exact identification (no local ontology):**
  — `Transformation` **≡** **[[A.3.4 - U.Transformation- Bounded Change Under Conditions|A.3.4]]** `U.Transformation` when the structure locus, path, path slice, substructure, or flow valuation is being used as a bounded transformation under conditions.
  — `Signature` **≡** **[[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]]** `U.Signature` (universal, law-governed declaration).
  — `Mechanism` **≡** **[[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]]** `U.Mechanism` (law-governed application over a SubjectKind/BaseType), with placement and stabilization relations in [[E.20 - Mechanism Introduction Protocol|E.20]] when current.
  — `WorkPlanning` **≡** **[[A.15.2 - U.WorkPlan|A.15.2]]** `U.WorkPlan` or its current plan-item relation when planning is the governed value.
  — `Work` **≡** **[[A.15.1 - U.Work|A.15.1]]** `U.Work` / `U.WorkEnactment` (dated world-contact; `FinalizeLaunchValues` only here).
  — `Check` **≡** `OperationalGate(profile)` (universal **gate**; [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] governs CV when internal step validity is current, and [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] governs gate profile, check aggregation, decision, and publication minima when gate fit or gate decision is current).
  — `StructuralReinterpretation` **≡** the [[E.18 - Transformation Flow Structure|E.18]] placement of **[[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]]** `U.EpistemicRetargeting` as a structure-positioned locus; it is not a new retargeting kind. **All retargeting semantics** (slot-scoped discipline, `EntityOfConcernSlot`/`GroundingHolonSlot` behaviour, invariants, Bridges, witnesses) come from **[[C.2.1 - U.Episteme - Epistemes and their slot relation|C.2.1]]** and **[[A.6.2 - U.EffectFreeEpistemicMorphing - Effect‑free morphisms of epistemes|A.6.2]]–[[A.6.5 - U.RelationSlotDiscipline - SlotKind - ValueKind - RefKind discipline for n‑ary relations (with slot‑operation lexicon)|A.6.5]]**; [[E.18 - Transformation Flow Structure|E.18]] does **not** introduce a local variant of retargeting.

  `OperationalGate` is the [[E.18 - Transformation Flow Structure|E.18]] check locus with DecisionLog aggregation. A check-locus label names only the current gate or check value that the selected structure positions: [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] governs internal constraint validity when that claim is current, [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] governs gate profile, aggregation, decision, and publication minima when gate fit or gate decision is current, and [[A.3.4 - U.Transformation- Bounded Change Under Conditions|A.3.4]] governs the bounded transformation claim that the check constrains.
  The only extra discipline [[E.18 - Transformation Flow Structure|E.18]] adds for `StructuralReinterpretation` is **structure-local**: CtxState and GateCrossing behaviour are governed by **CC-E18-06-EX** and **CC-E18-11** (projection-preserving w.r.t. `⟨L,P,E⃗,D⟩`, PathSlice-local, and "no plane/unit change without a gate"). `StructuralReinterpretation` is not a gate exception; it carries a recorded witness condition for saying no GateCrossing occurred. If any `CtxState` slot, plane/unit, edition, or design/run boundary changes, the case is a GateCrossing again.
> **MVPK integration (import).** Every locus with an external publication face is published via **MVPK** faces (`PlainView`, `TechCard`, `AssuranceLane`, `InteropCard`) under a declared **PublicationScope** ([[E.17 - Multi‑View Publication Kit|E.17]]). [[E.18 - Transformation Flow Structure|E.18]] **reuses** MVPK's publication laws (pins, declared-order discipline, "no new numeric claims / no I/O re-listing") and only adds structure-scope constraints in S3 and **CC-E18-09/10**; it does **not** define a second, local publication semantics.
...
* where a Bridge is involved: the **BridgeCard** ([[F.9 - Alignment & Bridge across Contexts|F.9]]) and its disclosed fields (`BridgeId`, `bridgeChannel`, **CL** and loss notes; **`CL^k`** when `bridgeChannel=Kind`; **`ReferencePlane(src,tgt)`**);
* where planes differ: **`CL^plane`** and the active **`Φ_plane`** as a **`PolicyIdRef`** (policy-id + resolvable refs; [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:8.1);
* the active penalty policy identifiers **`Φ(CL)`** (and **`Ψ(CL^k)`** if used) as **`PolicyIdRef`** bundles (policy-id + `PolicySpecRef` + `MintDecisionRef?`; [[F.8 - Mint or Reuse- (U.Type vs Concept-Set vs Role Description vs Alias)|F.8]]:8.1);
* any additional pins mandated by the active **GateProfile** / GateChecks ([[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]) for this crossing.
...
### [[E.18 - Transformation Flow Structure|E.18]]:5.2 - S2 - Flows as valuations (paths + state + guards)
* A **Flow** is a **valuation** `nu` over `U.Transfer` relations and cut-sets, paired with an **admissible path** `p = v0 -> ... -> vk` in the selected structure. The valuation maps transfer relations or cut-sets to token/state values under `CtxState` and links publication-event records to a declared `PublicationScopeId`; it is not itself the performed work. The concrete pins and identifiers (`PathId`, `PathSliceId`, Gamma_time on compare and launch faces) are governed here as path and slice publication obligations and by [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] when CV witnesses are current; use [[G.6 - Evidence Graph & Provenance Ledger|G.6]] for evidence-path visibility and [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] for refresh wiring. This reflects the "selected structure != flow" norm (flow = valuation), with gates placed exactly on GateCrossings.
...
* **Eulerian (structural):** define a **function on transfer relations** ("how much/what is associated with each relation under a given regime"), with gate laws. [[E.18 - Transformation Flow Structure|E.18]] deliberately fixes the **Eulerian semantics of flow** at the selected-structure scope: "flow (= valuation) + publication log", while change over time appears as **re-valuation** over a **PathSlice** (the execution/republishing window) under gate rules and the SquareLaw. This yields comparability, reproducibility, and slice-local refresh.

### [[E.18 - Transformation Flow Structure|E.18]]:5.3 - S3 - Publication discipline (faces)

[[E.18 - Transformation Flow Structure|E.18]] **imports [[E.17 - Multi‑View Publication Kit|E.17]]** wholesale **and associates MVPK faces with `PublicationScope` (USM)**.
...
[[E.18 - Transformation Flow Structure|E.18]] **does not re-specify** these laws; it only adds **structure-scope obligations** for faces published over transformation-flow paths:
...
2. **Gate requirement on cited editions.** Any face that references editions of `CG-Spec`, `ComparatorSet`, or `UNM.TransportRegistryPhi` includes **`BridgeCard + UTS row`**; faces without this are treated as **non-consumable downstream**. Bridge and terminology-synchronization checks are governed by [[F.9 - Alignment & Bridge across Contexts|F.9]], [[F.17 - Unified Term Sheet (UTS)|F.17]], [[E.17 - Multi‑View Publication Kit|E.17]], and [[E.18 - Transformation Flow Structure|E.18]]; selection and comparator pressure stays with [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]], [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], or [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] for current selector or comparator cases.
...
4. **Gamma_time on compare and launch faces.** All compare and launch faces on [[E.18 - Transformation Flow Structure|E.18]] paths pin `Gamma_time`; implicit *latest* is not admissible. [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] carries current GateProfile binding and minimum profile semantics; [[E.18 - Transformation Flow Structure|E.18]] paths include the pin. **CHR avoids acceptance thresholds** (*NoThresholdsInCHR*); thresholding and launches are carried by [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], Part G, and `U.Work` for current launch or thresholding cases. Unknowns remain tri-state (`pass|degrade|abstain`) and fold per the active GateProfile ([[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]).

> **Reminder.** MVPK already bans "signature" on faces, I/O re-listing, arithmetic on faces, and unpinned numeric content ([[E.17 - Multi‑View Publication Kit|E.17]] §5.4-5.5). [[E.18 - Transformation Flow Structure|E.18]] **does not weaken or override** those rules; it only constrains how they are used along transformation-flow paths.
...
**Decision stability & idempotency (gate-local).** Gate decisions are stable under a declared equivalence relation over the pins used by [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]; the witness is recorded as `DecisionLog` or `EquivalenceWitnessRef`, with [[G.6 - Evidence Graph & Provenance Ledger|G.6]] or [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] used for cases involving evidence-path visibility or refresh implications. [[E.18 - Transformation Flow Structure|E.18]] **does not** prescribe storage formats, key shapes, or hashing schemes.
...
Treat a step as a **EntityOfConcernRef/kind** transition (including `StructuralReinterpretation` under CC-E18‑06‑EX) **iff** the **UTS row**:
  — satisfies the minimal bridge and terminology-synchronization obligations of [[F.9 - Alignment & Bridge across Contexts|F.9]], [[F.17 - Unified Term Sheet (UTS)|F.17]], [[E.17 - Multi‑View Publication Kit|E.17]], and [[E.18 - Transformation Flow Structure|E.18]] for the current crossing case (identity, `ReferencePlane`, `CL/CL^plane`, edition pins for `CG-Spec`, `ComparatorSet`, `UNM.TransportRegistryPhi`, `ComparatorSetRef`, `BridgeId`, and `Phi-RuleIds`), and
  — is additionally marked as a **KindBridge** per [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]] (`bridgeChannel=Kind`, `CL^k`, mapping or signature‑translation, order‑preservation claims, loss notes, definedness area, determinism).
...
### [[E.18 - Transformation Flow Structure|E.18]]:5.4 - S4 - Assurance‑operations on `U.Transfer` (counterfactual admissibility)
...
### [[E.18 - Transformation Flow Structure|E.18]]:5.5 - S5 - Comparability & aggregation (normalize‑then‑compare; counterfactual form)
...
**Edition-aware set/archive publication records (e.g., QD archives) pin `DescriptorMapRef.edition`, `DistanceDefRef.edition`, and `CharacteristicSpaceRef.edition` when applicable; refresh is slice-local. Comparator, archive, and refresh checks are governed by [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]], [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.9 - Parity and Benchmark Harness|G.9]], and [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] for current archive or refresh cases.**

### [[E.18 - Transformation Flow Structure|E.18]]:5.6 - S6 - Cycle discipline (Selection ↔ Planning)
...
### [[E.18 - Transformation Flow Structure|E.18]]:5.7 - S7 - Selector semantics ([[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]) & parity harness ([[G.9 - Parity and Benchmark Harness|G.9]])
[[E.18 - Transformation Flow Structure|E.18]] keeps set-return, archive preservation, and comparator refs visible along the path. It does not define selector, archive, dominance, or comparator semantics; those remain with [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]], [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.9 - Parity and Benchmark Harness|G.9]], and [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] for current selector or comparator cases.
...
If `PortfolioMode=Archive`, a **QD archive** can be returned; when generation is in scope, pairs `{environment, method}` are managed under declared **EnvironmentValidityRegion** and **TransferRulesRef**; parity records and `PathSliceId` are pinned on publication. Comparator semantics and archive pinning are governed by [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]], [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.9 - Parity and Benchmark Harness|G.9]], and [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] for current archive or comparator cases.

### [[E.18 - Transformation Flow Structure|E.18]]:5.8 - S8 - Guard aggregation assignment and handling (USM §1.2)
...
**GateProfile data shape (cross-reference).** [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] carries the current GateProfile binding and minimum profile semantics. [[E.18 - Transformation Flow Structure|E.18]] names the structure only where crossings need it; fuller profile-matrix material is not a separate current authority unless a current governing pattern explicitly admits it.

**Bridge-aware guards (cross-reference).** USM guards apply bridge-translation semantics (`translate(Bridge, Scope)`) with CL penalties in R-lane; guard vocabulary is governed by **[[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]**, while gate aggregation remains in [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]].

**Error/timeout/unknown (profile-bound).** GateCheck errors/timeouts fold to **`degrade`** under `Lean\|Core` and to **`block`** under `SafetyCritical\|RegulatedX`; `unknown` follows the GateCheck's governing rule (safety-default: `degrade`). The [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] DecisionLog record and equivalence witness carry decision stability; [[E.18 - Transformation Flow Structure|E.18]] does not define storage or key structures.

### [[E.18 - Transformation Flow Structure|E.18]]:5.9 - S9 - Transport & crossings
* Cross-Context or cross-plane transfer relations appear as **GateCrossings** that include a **Bridge** with **CL** policy; **Phi(CL)/Phi_plane** are published; penalties appear **in R only**; **Scope membership** (USM) is unchanged by crossings. **SquareLaw is checked within a single `DesignRunTag`; a `T^D<->T^R` change is modelled as a pair of coordinated gates with `DesignRunTagFrom/To` and the selected [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] work or publication locus for the current case.**
...
### [[E.18 - Transformation Flow Structure|E.18]]:5.10 - S10 - Non‑mechanism boundary
...
### [[E.18 - Transformation Flow Structure|E.18]]:5.11 - S11 - Coordination wording labels (when current)
...
### [[E.18 - Transformation Flow Structure|E.18]]:5.12 - S12 - Viewpoint Families To [[E.18 - Transformation Flow Structure|E.18]] Constructs (neutral, holonic)
**S12 status.** S12 is secondary viewpoint-mapping input for a current viewpoint-family mapping claim. It is not the ordinary [[E.18 - Transformation Flow Structure|E.18]] core for naming a selected structure, flow valuation, path slice, or crossing.

[[E.18 - Transformation Flow Structure|E.18]] does not mint new viewpoint or view kinds. It **imports** the generic multi-view machinery of [[E.17.0 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]] `U.MultiViewDescribing`, bundles from [[E.17.1 - U.ViewpointBundleLibrary - Reusable Viewpoint Bundles|E.17.1]], and the TEVB engineering bundle from [[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]. S12 only describes how these existing `U.Viewpoint` / `U.ViewpointBundle` ids are *used* in transformation-flow structures and in `UTS.ViewpointMap`; intent/concern semantics are governed by E.17.0-E.17.2.

**Two-part use of TEVB and MVPK (ISO 42010 summary, no local re‑definition).**

* **Engineering viewpoints.** For engineering holons, [[E.18 - Transformation Flow Structure|E.18]] assumes a TEVB bundle with `ViewFamilyId = VF.TEVB.ENG`. `EngineeringVPId` is one of `{VP.Functional, VP.Procedural, VP.RoleEnactor, VP.ModuleInterface}`, and TEVB is the normative source for their semantics. [[E.18 - Transformation Flow Structure|E.18]] does not refine these viewpoints.
* **Publication viewpoints.** Publication viewpoints come from MVPK ([[E.17 - Multi‑View Publication Kit|E.17]]); `PublicationVPId` is a `MVPK.ViewpointId` that governs faces under a `PublicationScope`.
* **Architecture relation.** [[E.18 - Transformation Flow Structure|E.18]] can supply the selected transformation-flow structure used by an `ArchitectureOf@Context` claim when `DescriptionContext`, described holon, bounded context, and structure kind are explicit. It does not define architecture itself, and a transformation-flow structure is not the functional architecture by default. Use [[C.30 - Grounded Architecture and Selected-Structure Adequacy|C.30]], [[C.30.ASV - Architecture Structural View Adequacy (ASV)|C.30.ASV]], and the architecture transformation-flow relation pattern when the selected structure is used in an architecture-flow relation. Crossings and penalties follow [[E.18 - Transformation Flow Structure|E.18]] gating rules (S9; CC-E18-11/23) but do not change viewpoint semantics.
* **Separation of roles.** `VP.*` from TEVB are **EngineeringVPId** values only; they are not publication faces. `PublicationVPId` values are defined in MVPK. The mapping between them is entirely via ISO-style correspondences and the `UTS.ViewpointMap`; [[E.18 - Transformation Flow Structure|E.18]] does not define a second notion of viewpoint.

**Context object scope (summary).**

* **Engineering described object.** The engineering entity described by TEVB may be a holon (`U.System` or `U.Episteme`) per TEVB's `EntityOfConcernClassSpec`. That holon is the context object whose transformations, work, interfaces, or viewpoints may be structured. [[E.18 - Transformation Flow Structure|E.18]] does not make that holon its own EoC unless the selected transformation-flow structure itself is under concern.
  Transformation, method, procedure/control, role-enactor structure, structural architecture, module, interface, and allocation terms are viewpoint concern/content about that holon unless a concrete [[E.18 - Transformation Flow Structure|E.18]] use explicitly opens [[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] retargeting with `KindBridge (CL^k)`, retargeting witness, and the applicable species rule.
* **Publication described object.** MVPK can treat the *architecture description* itself as an EntityOfConcern; publication viewpoints for that AD are defined in MVPK, not here. [[E.18 - Transformation Flow Structure|E.18]] only checks that such faces honor MVPK discipline and [[E.18 - Transformation Flow Structure|E.18]] crossing rules when they publish selected transformation-flow material.

**Naming rules (aligned with [[E.17 - Multi‑View Publication Kit|E.17]].0/E.17.1/E.17.2).**
* `ViewFamilyId` is the `U.ViewpointBundle.viewFamilyId` (e.g. `VF.TEVB.ENG` for TEVB); its lexical and ontological discipline is governed by [[E.17.1 - U.ViewpointBundleLibrary - Reusable Viewpoint Bundles|E.17.1]].
* `EngineeringVPId : ViewpointId` is always a `U.ViewpointId` drawn from some bundle (for TEVB, one of `{VP.Functional, VP.Procedural, VP.RoleEnactor, VP.ModuleInterface}`). [[E.18 - Transformation Flow Structure|E.18]] never defines new `VP.*` ids.
* `PublicationVPId : ViewpointId` is a `MVPK.ViewpointId` defined in [[E.17 - Multi‑View Publication Kit|E.17]]; TEVB viewpoints are **never** reused as publication viewpoints (per TEVB guard and MVPK).
...
* Within S12, “viewpoint”, “view” and “correspondence” have exactly the meanings given in [[E.17.0 - U.MultiViewDescribing - Viewpoints, Views & Correspondences|E.17.0]]; “publication face” means an MVPK face (`PlainView`, `TechCard`, `InteropCard`, `AssuranceLane`) under some `PublicationVPId`.
* Faces are **carriers for views**: a face is part of a view only when linked via an ISO‑style `CorrespondenceRef` to an engineering `U.View` under some `EngineeringVPId`; S12 does not add extra conditions beyond [[E.17 - Multi‑View Publication Kit|E.17]].0/E.17.2.
...
**Purpose.** Provide a neutral ([[F.18 - Local‑First Unification Naming Protocol|F.18]]) mapping from TEVB engineering viewpoint families - bundle `VF.TEVB.ENG` with `VP.Functional / VP.Procedural / VP.RoleEnactor / VP.ModuleInterface` - to [[E.18 - Transformation Flow Structure|E.18]] constructs so that the same holon can be described through functional, procedural, role-enactor, or module-interface viewpoints while the [[E.18 - Transformation Flow Structure|E.18]] construct scope remains explicit. S12 does not introduce new `U.Viewpoint` or `U.View` kinds, and it does not claim that all such views share one underlying transformation-flow structure unless the structure, EntityOfConcernRef, and correspondence refs are declared.

**Holon target.** The mapping applies to any holon, with the constraint that only `U.System` enacts `U.Work` (A.3/A.15). Supervisory and structural hierarchies remain distinct ([[B.2.5 - Supervisor-Subholon Feedback Loop|B.2.5]]).

**Viewpoint family to primary [[E.18 - Transformation Flow Structure|E.18]] constructs (TEVB-aligned)**
*All four families referenced below are TEVB engineering viewpoints; the "what ..." clauses are interpretive glosses for how they *use* [[E.18 - Transformation Flow Structure|E.18]] constructs. Formal intent/concerns/allowed episteme kinds remain in TEVB ([[E.17.2 - TEVB - Typical Engineering Viewpoints Bundle|E.17.2]]).*
1) **Function-Oriented View (`EngineeringVPId = VP.Functional`, capability and transformation viewpoint)** - "what transformation is achieved under roles"
    ...
    * **Publication:** MVPK publication faces per [[E.17 - Multi‑View Publication Kit|E.17]]; comparable claims pin to `CG‑Spec/ComparatorSet` editions; crossings are published through `Bridge+UTS` and `CL/CL^plane` (penalties → **R‑lane** only).
    * **Checks:** [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] (CV) inside transformations; [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] (GateFit) at gates; comparator, set-return, and No-Hidden-Scalarization discipline is carried through [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]], [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.9 - Parity and Benchmark Harness|G.9]], and [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] for current selector or comparator cases.
...
2) **Procedure‑Oriented View (`EngineeringVPId = VP.Procedural`, step/time storyboard)** — “what steps occur and when”
    * **FPF constructs:** `U.WorkPlan` ([[A.15.2 - U.WorkPlan|A.15.2]]) for intent/schedule; `U.WorkEnactment` for enactment.
    ...
    * **Holonic note:** Applies to any `U.System` scope (single holon or a supervised sub‑holon cluster); supervisory structure is handled by roles rather than structural mereology ([[B.2.5 - Supervisor-Subholon Feedback Loop|B.2.5]]).
3) **Role‑Enactor / Device‑Structure View (`EngineeringVPId = VP.RoleEnactor`)** — “what carrier/ports/constraints exist; who typically enacts it”
    ...
    * **Publication:** MVPK faces are **typed projections**, not `U.Work` records or execution carriers; faces add **no new numeric claims** ([[E.17 - Multi‑View Publication Kit|E.17]]). Constraints and compatibility appear as CV checks ([[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]]).
    * **Holonic note:** Structural mereology (part/whole of the carrier) is modeled in Part A; [[E.18 - Transformation Flow Structure|E.18]] ties interface/exposure semantics to mathematical-lens expressions and gates only when those are current.
    ...
    * **Role‑label guard.** `TypicalEnactorRoleName` is **pedagogical only** and is not used as a GateFit role; GateFit uses `U.Role` ([[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]).
4) **Module‑Interface View (`EngineeringVPId = VP.ModuleInterface`, physical/logical module structure)** — “what modules exist and how they specify commitments and constraints across interfaces”
    ...
    * **Holonic note:** The same module can appear as a holon in multiple views; supervisory loops ([[B.2.5 - Supervisor-Subholon Feedback Loop|B.2.5]]) remain orthogonal to structural composition.
This is an expandable list of viewpoint families; [[E.18 - Transformation Flow Structure|E.18]] is intentionally viewpoint-neutral. Additional engineering bundles beyond TEVB (safety, mission, information, ...) are introduced as separate `U.ViewpointBundle` species via [[E.17 - Multi‑View Publication Kit|E.17]].1/E.17.2; S12 does not define them.

**View-family label discipline for transformation-flow loci (recognition-only).**
*Scope.* When a viewpoint-family mapping claim is current, a pattern or domain profile may declare `LocusViewFamilyLabels[]` for transformation-flow locus labels so practitioners can recognize familiar engineering wording while the selected structure stays governed by [[E.18 - Transformation Flow Structure|E.18]]. Semantics come from the referenced `U.ViewpointBundle`, [[E.18 - Transformation Flow Structure|E.18]] locus binding, and MVPK correspondences; labels are recognition aids, not loci, viewpoints, publication faces, checks, or work records.
...
4. Twin registers are allowed as Tech and Plain labels per [[E.10 - Unified Lexical Rules for FPF|E.10]]; naming follows [[F.18 - Local‑First Unification Naming Protocol|F.18]] local-first discipline.
...
Publish a UTS block named `ViewpointMap` only when an engineering or publication viewpoint-family mapping claim is made or consumed. Ordinary [[E.18 - Transformation Flow Structure|E.18]] use does not require `UTS.ViewpointMap` when the question under repair is only the selected structure, flow valuation, path slice, or crossing.
...
(iii) Rows that carry edition identifiers also include `BridgeCard + UTS` rows through [[F.9 - Alignment & Bridge across Contexts|F.9]], [[F.17 - Unified Term Sheet (UTS)|F.17]], [[E.17 - Multi‑View Publication Kit|E.17]], and [[E.18 - Transformation Flow Structure|E.18]]; edition-bearing faces that lack such rows are not admissible for downstream consumption.
...
## [[E.18 - Transformation Flow Structure|E.18]]:6 - Archetypal Grounding (Tell–Show–Show; concise)
...
*Show-D (FPF pattern development and use).* A development-flow valuation represents pattern development work: creation, evaluation, projection, publication, and later repair of a pattern. A use-flow valuation represents application of that pattern to its own `EntityOfConcern`. An evaluation or use-found defect can select the smallest development slice for repair. [[E.18 - Transformation Flow Structure|E.18]] keeps the common selected structure visible while separating the developed pattern, the use of the pattern, the evidence found during use, and the edition or slice that is reopened.

**Cross-pattern boundary slice (QD archive).** A QD selector returns an archive. Under [[E.18 - Transformation Flow Structure|E.18]], this is one `PathSlice` in one `TransformationFlowStructure`; selection returns a set/archive, not a hidden scalar. Under [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]], the archive insertion or update step has a current CV class, `CV.Status`, and witness or refusal; no acceptance is inferred. Under [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], a comparability gate or `LaunchGate` can publish a `GateDecision` only when that gate relation is current and consumes the relevant CV result. Under [[E.20 - Mechanism Introduction Protocol|E.20]], if a new selector mechanism-governing definition is introduced, the mechanism-governing definition is the locus for the meaning while suites and wiring only cite or bind it. These are four governed loci, not one prescribed work order.
...
## [[E.18 - Transformation Flow Structure|E.18]]:7 - Conformance — **Unified checklist (normative)**
...
| **CC-E18-02 — Locus kinds bind governed values** | Loci are structure-positioned bindings to governed values. The current minimal locus baseline is `{Transformation, Signature, Mechanism, WorkPlanning, Work, Check, StructuralReinterpretation}`. Domain-specific species are open-world and non-exhaustive; they bind to one of these locus kinds or require an explicit [[E.18 - Transformation Flow Structure|E.18]] update. The baseline is **not** a local ontology: `Transformation -> [[A.3 - Transformer Constitution (Quartet)|A.3]].4`, `Signature -> [[A.6 - Signature Stack & Boundary Discipline|A.6]].0`, `Mechanism -> [[A.6 - Signature Stack & Boundary Discipline|A.6]].1/E.20`, `WorkPlanning -> [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]].2`, `Work -> [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]].1`, `Check -> [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] or A.21`, and `StructuralReinterpretation -> [[A.6.4 - U.EpistemicRetargeting - EntityOfConcern retargeting morphism|A.6.4]] plus E.18/A.20` where CV is current. A morphism expression is a mathematical-lens view when current, not the FPF kind of every locus. | Type registry shows at least the listed locus kinds; additional species map to one of them; checks are realized as `OperationalGate` when a gate/check locus is present (see CC-E18-06-EX/11). **Lint:** registry/table exposes `{species -> {locusKind, governingPattern}}`; missing or mismatched governing pattern fails. |
...
| **CC-E18-06-EX (strictly limited) — Projection retargeting without gate** | A locus of kind **`StructuralReinterpretation`** retargets the **published projection** without invoking `OperationalGate` **only if all hold**: **(a)** `CtxState` is preserved; **(b)** any **EntityOfConcernRef** change has a **KindBridge** (`CL^k`) entry on MVPK/**UTS**; **(c)** a **SquareLaw-retargeting witness** is present (on UTS); **(d)** the operation is **PathSlice-local** (`PathSliceId` pinned); **(e)** **no plane/unit change** occurs (plane/unit changes remain gated); **(f)** **CV.ReinterpretationEquivalence** ([[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]]) is `pass`; **(g)** **NoHiddenScalarization** - if the step concerns a comparable return shape, set/partial-order semantics are preserved and comparators remain ref-only (current comparator and set-return loci). | UTS row includes `bridgeChannel=Kind` and `CL^k`; SquareLaw-retargeting witness present; PathSliceId pinned; CV status recorded; no scalarization detected. |
...
> **Scope note ([[E.18 - Transformation Flow Structure|E.18]] vs neighbor patterns):** Detailed mechanism-scoped checks and publication obligations are governed by the current neighbor patterns named in this pattern's Relations. [[E.18 - Transformation Flow Structure|E.18]] fixes only selected-structure obligations: single `U.Transfer` relation kind, gate crossings, valuation, publication pins, CV/GF boundary, and slice-local refresh.
...
* *Signature locus* - structure-positioned use of **[[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]]** `U.Signature` (universal block). It is a governed value bound into the selected structure, not a local kind and not a `C.3.2 KindSignature`.
* *KindSignature ([[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]])* - definition of a `U.Kind` by intent, extent, and formality; **unrelated** to [[E.18 - Transformation Flow Structure|E.18]] locus kinds; never a `genus`.
* *Species (domain-scoped)* — typed specialisations `speciesOf(kind=...)` that declare `KindDefinition=<current governing pattern id>` (e.g., `kind=Mechanism; KindDefinition=[[A.6 - Signature Stack & Boundary Discipline|A.6]].1`).
...
* **GateCheckKind boundary.** `GateCheckKind` is a publication/check lexeme used inside `GateCheckRef`, not a structure locus kind. No `GateCheckKind` becomes an [[E.18 - Transformation Flow Structure|E.18]] `Check` locus unless an `OperationalGate(profile)` locus is actually present.

* **GateCheckRef shape (publication lexeme governed by [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]).** Where publication faces over a selected structure carry GateChecks, a **GateCheckRef** is a record
  defined by [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]; [[E.18 - Transformation Flow Structure|E.18]] constrains only where such faces carry those refs along transformation-flow paths.
...
* **GateFit (aspect, not an entity).** GateFit names the **aspect** of checks that evaluate **profile‑fit**; there is no separate GateFit entity. “Gate decision under GateFit” means “the gate’s decision computed from GateChecks with `aspect=GateFit`”.
  ...
  This shape is publication-only; it introduces no new execution steps and no arithmetic on faces.  (Couples to [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] or [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] without duplicating their check catalogs.)
* *VALATA (VA/LA/TA)* — value-annotation scheme used on **AssuranceLane**; **carriers** are referenced via **SCR/RSCR**; detailed evidence obligations are governed by [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]] and the named evidence, publication, or crossing pattern for the current case. Included here so evidence pins are self-describing in Part E texts.
...
## [[E.18 - Transformation Flow Structure|E.18]]:8 - Gating Profiles (applied to [[E.18 - Transformation Flow Structure|E.18]])
This table is a selected-structure coverage table for [[E.18 - Transformation Flow Structure|E.18]] crossings and path slices. It is not the source of `GateProfile` semantics. [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] governs gate decision semantics, folds, `DecisionLog` minima, and the GateFit check-catalog boundary.

> Gating is expressed as **publication-gating** per [[E.17 - Multi‑View Publication Kit|E.17]] profiles. The structure model aligns with the **CC items** listed for the chosen profile; broader obligation profiles include all narrower-profile items.
...
**Recommended defaults (non-normative, tie-in to [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] and [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]).** Profiles inherit along a `PathSlice`; local overrides only **add** GateChecks; weakening uses a new `PathSlice` and refresh wiring through the current [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] locus when refresh wiring is current.

## [[E.18 - Transformation Flow Structure|E.18]]:9 - [[E.18 - Transformation Flow Structure|E.18]] LEX Discipline (registration)
Register Tech tokens (ASCII) used by this pattern with twin-labels: `TransformationFlowStructure`, `TransformationFlowValuation`, `StructuralReinterpretation`, `OperationalGate`, `GateProfile`, `GateCheckRef`, **`GateCheckKind`**, `DecisionLog`, `USM.CompareGuard`, `USM.LaunchGuard`, `KindBridge`, `SubflowRef`, `FlowEmbed`, `SentinelId`, `PathSliceId`, `SliceRefresh`, `FinalizeLaunchValues`, `VALATA`. Add an ASCII alias **`CLKind`** <-> Plain `CL^k` (cf. `CLPlane` <-> `CL^plane`). Reference MVPK [[E.17 - Multi‑View Publication Kit|E.17]] naming for faces.
...
## [[E.18 - Transformation Flow Structure|E.18]]:10 - Consequences
...
c) **Tooling alignment:** some incumbent DAG-only orchestrators conflict with budgeted cycles and set-return semantics; adapters project [[E.18 - Transformation Flow Structure|E.18]] semantics to their interop boundary, while [[E.18.2 - Transformation Flow Mathematical Description|E.18.2]] carries the mathematical graph-description relation when that projection matters.

## [[E.18 - Transformation Flow Structure|E.18]]:11 - Rationale

[[E.18 - Transformation Flow Structure|E.18]] states **strict separation of concerns** (selected-structure scope only); **specialized semantics are governed by the patterns named below for those current relations**:
...
* **How comparability works:** UNM is the single governing locus for unit, plane, and transport declarations, and selectors operate **only** on normalized, edition-pinned comparators, returning sets or archives rather than totals. Edition-aware pins and archive semantics are checked through [[A.19.SelectorMechanism - Unified Selection Kernel, SelectorMechanism|A.19.SelectorMechanism]], [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]], [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], [[G.9 - Parity and Benchmark Harness|G.9]], and [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] for current selector or archive cases.
...
## [[E.18 - Transformation Flow Structure|E.18]]:12 - SoTA‑Echoing (post‑2015, multi‑Tradition)

> Each row states the source idea, the FPF invariant [[E.18 - Transformation Flow Structure|E.18]] adopts, the practitioner move it changes, and the shortcut it rejects. Vendor, tool, and literature tokens are informative; the invariant and practitioner move carry the pattern explanatory work.
...
| **Operads, wiring diagrams, and hypergraph categories** (Spivak, *The operad of wiring diagrams*, arXiv [`1305.0297`](https://arxiv.org/abs/1305.0297); Baez & Fong, *A Compositional Framework for Passive Linear Networks*, arXiv [`1504.05625`](https://arxiv.org/abs/1504.05625)). | Typed ports and interface junctions motivate Bridge/CL/Phi pins at crossings; [[E.18 - Transformation Flow Structure|E.18]] adapts the math by requiring publication pins that the math alone does not supply. | When an interface or boundary crossing matters, publish the Bridge, UTS row, CL/CL^plane, and R-lane penalty placement instead of leaving an unpinned junction. | Treating an interface diagram, wiring diagram, or decorated cospan as sufficient crossing evidence. |
...
*Cross-tradition note.* Rows 1-3 (compositional graph practice), rows 4-5 (publication and reproducibility practice), row 6 (controls/robotics), row 7 (evolutionary search), and row 8 (PL/semantics) jointly position [[E.18 - Transformation Flow Structure|E.18]] across multiple traditions per [[E.8 - FPF Authoring Conventions & Style Guide|E.8]], but each row is retained only because it changes a practitioner move or rejected overread.

## [[E.18 - Transformation Flow Structure|E.18]]:13 - Bias‑Annotation (per [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] SG‑bias slot)

* **Acyclic-bias risk.** Tooling accustomed to DAGs may discourage admissible feedback loops; [[E.18 - Transformation Flow Structure|E.18]] explicitly permits loops with budget/sentinel controls (CC-E18-13, -18).
* **Scalarization-bias risk.** Cultural defaults to single-score rankings can suppress Pareto/QD sets; [[E.18 - Transformation Flow Structure|E.18]] keeps declared order relations and return sets visible (CC-E18-10, CC-E18-12).
* **Interop-dominance risk.** File/format ecosystems (CWL/RO-Crate/lineage) can be mistaken for semantic sources; [[E.18 - Transformation Flow Structure|E.18]] places them in **InteropCard** and keeps governing semantics in loci/gates.
* **Over-formalization risk.** Category-theoretic formalisms can obscure operational guard-rails; [[E.18 - Transformation Flow Structure|E.18]] grounds crossings in Bridge/UTS/CL/Phi pins and SquareLaw audits (CC-E18-11, -17).
* **Retrospective rewrite risk.** Global rewrites break replay; [[E.18 - Transformation Flow Structure|E.18]] confines them to edition bumps and slice-local refresh (CC-E18-16).
...
## [[E.18 - Transformation Flow Structure|E.18]]:14 - Relations (explicit pattern-to-pattern relations)

> Relation rows are typed as **builds_on / constrains / coordinates / specializes / publishes_on / requires / provides_checks_for**.

**Foundations**
* **[[E.18 - Transformation Flow Structure|E.18]] -> builds_on -> [[E.17 - Multi‑View Publication Kit|E.17]] MVPK (for publications of selected structure material).** Faces, pins, lanes, functorial publication, Lean/Core/Regulated profiles.
* **[[E.18 - Transformation Flow Structure|E.18]] -> builds_on -> [[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]] U.Signature / [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] U.Mechanism.** Locus kinds and governing-definition content boundaries.
* **[[E.18 - Transformation Flow Structure|E.18]] -> builds_on -> [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] Strict Distinction (EntityOfConcern, Description episteme, Description episteme admitted for specification use, and publication/carrier separation).** No new claims on faces; publication faces project selected structure, crossing, or flow-valuation information without becoming the governed selected structure, Description episteme, specification use, evidence, gate decision, work occurrence, or carrier.

**Flow semantics & checks**
* **[[E.18 - Transformation Flow Structure|E.18]] -> coordinates -> [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] U.Flow (ConstraintValidity scope).** CV checks apply inside transformations; no declaration/translation of planes/units in CV; **error/timeout/unknown folds** follow **CC-E18-22** as the **minimum default** (profiles can be stricter).
  **Terminology discipline ([[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] boundary).** In CV scope, publications use **status/witness** language; **GateDecisionRationale/GateDecisionExplanation** are reserved for gating and do not apply to CV.
* **[[E.18 - Transformation Flow Structure|E.18]] -> coordinates -> [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] GateProfilization (GateFit scope).** **GateFit-scoped GateChecks** are aggregated by `OperationalGate(profile)` with CV=>GF activation; the **enumeration and publication shape** of GateChecks are governed by **[[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]]**. **Equivalently:** a GateFit decision different from `abstain` appears only when aggregated `ConstraintValidity = pass`; otherwise the **GateDecisionExplanation (GateFit-oriented)** does not apply.
* **[[E.18 - Transformation Flow Structure|E.18]] -> uses -> USM.CompareGuard and USM.LaunchGuard.** Guards publish scope and responsible gate; guard failures are handled by the declared gate.
* **[[E.18 - Transformation Flow Structure|E.18]] -> constrains -> F.9/F.17 bridge and terminology-synchronization loci (Bridge+UTS, CL/CL^plane, Phi -> R).** A transition is treated as a **Crossing** iff `Bridge+UTS` and the appropriate `CL/CL^plane` are published; otherwise this crossing explanation does not apply. Where Phi defines penalties, they appear in the R-lane only.
...
**UNM & comparability**
* **[[E.18 - Transformation Flow Structure|E.18]] -> constrains -> UNM declaration and use loci.** `CG-Spec`, `ComparatorSet`, and `UNM.TransportRegistryPhi` declarations are governed by UNM; normalize-then-compare is mandatory.
* **[[E.18 - Transformation Flow Structure|E.18]] -> constrains -> [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]] SelectionAndTuning.** Set-returning, comparator-pinned decisions, no hidden scalarization; `MethodTuning` without launch-value slot filling.
* **[[E.18 - Transformation Flow Structure|E.18]] -> constrains -> [[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]] EvaluatingAndRefreshing.** EditionBumpProposal, two-phase update through the UNM declaration locus, path-local refresh.

**Work boundary**
* **[[E.18 - Transformation Flow Structure|E.18]] -> coordinates with -> [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] U.WorkEnactment (`FinalizeLaunchValuesOnlyInWork`).** Single point of `FinalizeLaunchValues`; `FreshnessUpToDate` hard at LaunchGate; acceptance/telemetry published here.

**Structure & reuse**
* **[[E.18 - Transformation Flow Structure|E.18]] -> provides selected-structure base for transformation-flow families.** Flow patterns such as P2W and EvaluatingAndRefreshing use [[E.18 - Transformation Flow Structure|E.18]] for selected structure, valuation, crossings, guards, MVPK faces, and slice-local refresh. The current ontology is: [[A.3.4 - U.Transformation- Bounded Change Under Conditions|A.3.4]] governs each bounded `U.Transformation`, [[E.18 - Transformation Flow Structure|E.18]] governs the selected compound structure over transformations and adjacent governed loci, and neighbor patterns govern method, work, mechanism, evidence, publication, gate, decision, and refresh claims when those claims are current.
* **[[E.18 - Transformation Flow Structure|E.18]] -> coordinates with -> architecture transformation-flow relation patterns.** When a selected transformation-flow structure is used in an architecture-flow relation, the architecture transformation-flow relation pattern records the relation between `TransformationFlowStructure` and `ArchitectureOf@Context`; [[E.18 - Transformation Flow Structure|E.18]] keeps selected structure, crossing, and flow-valuation discipline.
* **[[E.18 - Transformation Flow Structure|E.18]] -> publishes_on -> [[E.17 - Multi‑View Publication Kit|E.17]] MVPK views** (`PlainView`, `TechCard`, `InteropCard`, `AssuranceLane`) for every transfer/locus where publication occurs; Lean mode applies only as per profile.

## [[E.18 - Transformation Flow Structure|E.18]]:15 - Conformance Use Checks
...
Relation boundary: [[E.18 - Transformation Flow Structure|E.18]] governs selected transformation-flow structures for structures of atomic `U.Transformation` values and their structure-positioned slot-filler loci. It does not define a second change ontology, a work sequence, a method, a mechanism, a mathematical graph expression, or a publication record. When a selected-structure use raises bounded-transformation, dynamics-episteme, temporal-aspect, temporal-claim adequacy, work planning, performed work, evidence, assurance, gate, decision, architecture, structural-view, mechanism, selector, comparison, refresh, publication, or wording-use claims, name the governing pattern for that relation before relying on the structure.

When a selected structure locus, selected path, path slice, substructure, or flow valuation expresses, decomposes, or constrains one bounded transformation relation, use [[A.3.4 - U.Transformation- Bounded Change Under Conditions|A.3.4]] for the atomic `U.Transformation` claim and use [[E.18 - Transformation Flow Structure|E.18]] for the selected structure, containing locus, pins, locus kind, crossing, publication, comparability, and refresh discipline. [[E.18 - Transformation Flow Structure|E.18]] locus kinds do not automatically fill neighboring slots: `Transformation` points to [[A.3.4 - U.Transformation- Bounded Change Under Conditions|A.3.4]], `Signature` points to [[A.6.0 - U.Signature - Universal, law‑governed declaration for a SubjectKind on a BaseType|A.6.0]], `Mechanism` points to [[A.6.1 - U.Mechanism- Law-Governed Operation Algebra over a Subject Kind|A.6.1]] and [[E.20 - Mechanism Introduction Protocol|E.20]], `WorkPlanning` and `Work` point to the [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] work family, and `Check` points to [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]] or [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]] according to the current claim.

## [[E.18 - Transformation Flow Structure|E.18]]:15a - [[E.18.1 - Principles-to-Work Carry-Through|E.18.1]] P2W Child-Pattern Relation

[[E.18.1 - Principles-to-Work Carry-Through|E.18.1]] is a child pattern for principles-to-work carry-through. It inherits this pattern's selected structure, path, flow-valuation, transfer, crossing, and gate minimum, then adds the local P2W relation from accepted problem-side output to the next FPF kind named by value, relation, record, or application. Pilot examples for one specialization belong in the selected child pattern that uses them; [[E.18 - Transformation Flow Structure|E.18]] keeps only the selected-structure law and this short child-pattern relation.

## [[E.18 - Transformation Flow Structure|E.18]]:End
