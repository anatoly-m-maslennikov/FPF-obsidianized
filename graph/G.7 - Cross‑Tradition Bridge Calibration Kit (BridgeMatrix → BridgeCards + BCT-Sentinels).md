...
**Stage.** design‑time (calibration + publication) + run‑time (sentinel‑driven telemetry emission; orchestration governed by **[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]**)
**Primary output.** A bridge calibration kit that turns **[[G.2 - SoTA Harvester & Synthesis|G.2]]**’s BridgeMatrix rows into **[[F.9 - Alignment & Bridge across Contexts|F.9]]** `BridgeCard`s and publishes: a `BridgeCalibrationTable (BCT)` + `CalibrationLedger` + `RegressionSet` + `SentinelSet`, plus UTS‑visible crossing rows and RSCR‑ready sentinel triggers scoped to `PathSliceId` / `PatternScopeId`.
**Primary hooks.** [[G.Core - Part G Core Invariants|G.Core]] (Part‑G invariants + RSCR trigger catalogue + Default Governing Definition Index), **[[G.2 - SoTA Harvester & Synthesis|G.2]]** (BridgeMatrix), **[[F.9 - Alignment & Bridge across Contexts|F.9]]** (BridgeCard + CL/CL^k), **F.3/F.7** (SenseCell anchoring; row bottleneck discipline), **E.18/A.21** (GateCrossing + CrossingBundle checks), **[[G.6 - Evidence Graph & Provenance Ledger|G.6]]** (PathId/PathSliceId citation surface), **[[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]** (downstream consumer for eligibility/selection), **[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]** (refresh orchestration consumer), **[[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]]** (assurance lanes + penalty policies), **[[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]** (DHC accounts such as AlignmentDensity), **[[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]] and [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]]** (QD/OEE pins when relevant), **[[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]** (SoS‑LOG clauses as explainability gates for cross‑Tradition choices), **[[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]** (Acceptance hooks/thresholds when bridges are used as selector gates), **[[E.10 - Unified Lexical Rules for FPF|E.10]]** (LEX / strict distinction discipline).
**Working‑Model first.** Prefer a minimal, auditable calibration procedure and worked micro‑cases; escalate to heavier harnesses only where risk warrants (per **[[E.8 - FPF Authoring Conventions & Style Guide|E.8]]**).
**Non‑duplication note.** Universal Part‑G invariants (no shadow specs; Bridge‑only crossings; penalty routing to `R_eff` only; P2W split; typed/id‑based RSCR causes; defaults with one governing definition; Δ‑discipline) are governed by [[G.Core - Part G Core Invariants|G.Core]] and are *cited* via `CC‑GCORE‑*`. This pattern defines only the *bridge calibration kit* and its surfaces.

## [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:1 - Problem frame

SoTA synthesis (**[[G.2 - SoTA Harvester & Synthesis|G.2]]**) can legitimately preserve pluralism by exporting a **BridgeMatrix**: a Tradition×Tradition inventory of “comparable constructs” with preliminary notes (candidate correspondences, likely losses, tentative levels). Downstream patterns (CHR/CAL/selector/logging/shipping) cannot consume this safely unless cross‑Context reuse is:
...
[[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] packages this into a kit: `BCT` + `BridgeCard` publication + `RegressionSet`/`SentinelSet` wiring, so that later patterns can satisfy core invariants without re‑inventing cross‑Tradition machinery.

## [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:2 - Problem
...
## [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:3 - Forces
...
## [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:4 - Solution — Bridge calibration kit (BCT + BridgeCards + RegressionSet/Sentinels)

### [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:4.1 - [[G.Core - Part G Core Invariants|G.Core]] linkage (normative)
...
* **Expansion rule.** Effective `CoreConformanceIds`, `RSCRTriggerKindIds`, and `CorePinsRequired` are obtained by expanding the cited profile/set ids and unioning with the explicit ids above (see [[G.Core - Part G Core Invariants|G.Core]] nil‑elision + expansion rule).
* **Conditional pins.**
...
### [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:4.2 - Kit objects (surface governed by this patterns)

This pattern defines the *bridge calibration kit* as a set of minimal, checkable surfaces. Semantics of `BridgeCard` and CL typing are governed by **[[F.9 - Alignment & Bridge across Contexts|F.9]]**; [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] adds calibration records and publication/wiring surfaces.
...
**Source provenance (when sourced from [[G.2 - SoTA Harvester & Synthesis|G.2]]).** If the BCT is derived from a [[G.2 - SoTA Harvester & Synthesis|G.2]] BridgeMatrix, publish `BridgeMatrixId` (+ `BridgeMatrixRef.edition` when editioned) and row‑level linkage via `G.7:Ext.MatrixIntake` (wiring‑only), rather than duplicating [[G.2 - SoTA Harvester & Synthesis|G.2]] semantics in core.
...
#### [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:4.2.1 - CL / CL^k admissibility regime and plane guard (kit‑local; normative)

This subsection is kit-governed ([[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]) and complements (but does not duplicate) [[G.Core - Part G Core Invariants|G.Core]] penalty routing and tri‑state guard semantics.

**Admissibility regimes (row‑level, minimal).**
* `RowCL_min` MUST take a value in `{3,2,1,0}` (value set and CL meaning are governed by [[F.9 - Alignment & Bridge across Contexts|F.9]]; [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] governs the admissibility regime).
...
* any “blocking” outcome must be representable downstream via [[G.Core - Part G Core Invariants|G.Core]] tri‑state guard (`abstain` or a policy‑bound `degrade(mode=…)`), without introducing additional statuses in [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]];
* plane effects MUST NOT rewrite `CL/CL^k`; their impact is routed via the pinned policy ids and [[G.Core - Part G Core Invariants|G.Core]] penalty semantics.
...
### [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:4.3 - Minimal calibration procedure (auditable; table‑backed; bridge‑first)

For each Tradition‑pair and each comparable construct row from **[[G.2 - SoTA Harvester & Synthesis|G.2]]**:

1. **Materialise bridge artefacts.** Produce (or reuse) **[[F.9 - Alignment & Bridge across Contexts|F.9]]** `BridgeCard`s for the concrete `SenseCell`‑level alignments required by the row scope.
   *Note.* “SenseCell anchoring” is a kit requirement: if a row is authored at a coarser token level, the SenseCell anchors must be explicitly cited ([[F.3 - Intra‑Context Sense Clustering|F.3]] discipline).
...
4. **Policy pins for penalty routing.** Record the policy id pins needed to audit penalty routing (ids only). Penalty semantics cite `CC‑GCORE‑PEN‑1` through [[G.Core - Part G Core Invariants|G.Core]]; [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]’s responsibility is to make the pins explicit and published.
5. **Row bottleneck discipline.** When a row aggregates multiple bridge cells, row summarisation uses bottleneck semantics ([[F.7 - Concept‑Set Table|F.7]]) and carries a counterexample citation whenever any cell is loss‑noted.
...
### [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:4.4 - Publication surfaces (UTS + GateCrossing harness)

A conformant [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] publication:
...
* ensures evidence-facing citations are pin-complete: whenever bridge calibration is cited in SCR/Evidence surfaces, the citation MUST include `{BCT.id, RegressionSetId}` and the active policy id pins `{Φ(CL), Ψ(CL^k)?, Φ_plane?}` (ids only; representation is governed by [[G.6 - Evidence Graph & Provenance Ledger|G.6]]/SCR).

### [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:4.5 - Worked mini‑examples (informative; post‑2015; row scopes + loss notes)
...
### [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:4.6 - Extensions (pattern‑scoped; non‑core)
...
* **GoverningPatternId:** [[G.2 - SoTA Harvester & Synthesis|G.2]] *(BridgeMatrix semantics and comparable-construct inventory)*
* **Uses:** `{[[G.2 - SoTA Harvester & Synthesis|G.2]], [[F.9 - Alignment & Bridge across Contexts|F.9]]}`
* **⊑/⊑⁺:** `∅`
* **RequiredPins/EditionPins/PolicyPins (minimum):**
  ...
  * `ComparableConstructId[]` *(row keys; if the source does not supply a stable id, [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] mints one while preserving `BridgeMatrixRowRef` as the provenance anchor)*
  * `LossNoteRef[]?` *(if exported by [[G.2 - SoTA Harvester & Synthesis|G.2]]; otherwise authored in [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] and cited from the `CalibrationLedger`)*
...
* **Notes (wiring‑only):** This module binds “row candidates” from [[G.2 - SoTA Harvester & Synthesis|G.2]] to the BCT/Ledger intake without copying [[G.2 - SoTA Harvester & Synthesis|G.2]] semantics into [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]].
...
* **GoverningPatternId:** [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]] *(DHC metric semantics, including AlignmentDensity)*
* **Uses:** `{[[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]}`
* **⊑/⊑⁺:** `∅`
...
* **Notes (wiring‑only):**
  * [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] stores the *counts and declared units* as a surface; [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]] governs the meaning and legality constraints.
  * When reporting AlignmentDensity, the counted bridge set is typically restricted to `CL ≥ 2` (treat `CL=3` as “free substitution”, `CL=2` as “guarded” for reporting); conformance is enforced by `CC‑G7‑DHC‑Units‑1` while semantics remain governed by [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]].
...
* **GPatternExtensionKind:** `InteropSpecific`
* **GoverningPatternId:** [[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]] *(QD artefact semantics; uses [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]] for exploration/logging pins as needed)*
...
* **GPatternExtensionKind:** `InteropSpecific`
* **GoverningPatternId:** [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]] *(SoS‑LOG rule and branch semantics; [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] does not redefine meaning)*
* **Uses:** `{[[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]], [[G.6 - Evidence Graph & Provenance Ledger|G.6]]}`
* **⊑/⊑⁺:** `∅`
* **RequiredPins/EditionPins/PolicyPins (minimum; conditional on use):**
  * `SoSLogRuleId[]` *(or governing definition‑equivalent ids)*
  * `FailureBehaviorPolicyId?` *(policy id, when degrade behavior is bound)*
  * `PathId/PathSliceId` citations for explainability (via [[G.6 - Evidence Graph & Provenance Ledger|G.6]])
...
* **Notes (wiring‑only):** Ensures cross‑Tradition bridge reuse decisions can be justified by citing SoS‑LOG clauses and evidence paths, without embedding SoS‑LOG semantics into [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]].
...
* **GoverningPatternId:** [[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]] *(Acceptance/threshold/unknown handling; [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] does not define thresholds)*
...
## [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:7 - Conformance Checklist (normative) — **CC‑G7**

| ConformanceId             | Requirement                                                                                                                                                                                                                                                                               | Purpose                                                                        |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| **CC‑G7‑CoreRef**         | [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] is conformant only if it satisfies the effective [[G.Core - Part G Core Invariants|G.Core]] obligations declared by the `GCoreLinkageManifest` in **§4.1** (after nil‑elision and expansion of profile/set/pinset ids), including any explicit deltas listed there. | Make universal invariants one governing definition and enforce citation‑based reuse.       |
...
| **CC‑G7‑BridgeCard‑1**    | Any bridge published by [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] **MUST** be consumable as an **[[F.9 - Alignment & Bridge across Contexts|F.9]]** `BridgeCard` and **MUST** be SenseCell‑anchored (directly or via explicit SenseCell anchor refs).                                                                                                                        | Prevent “Context‑only” or ambiguous bridges.                                   |
| **CC‑G7‑UTS‑1**           | [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] outputs **MUST** mint/publish UTS‑citable ids (NameCards/twin labels as applicable) for (a) each BridgeCard (or its NameCard) and (b) each GateCrossing/crossing row that makes bridge use checkable; and **MUST** expose the resulting `UTSRowId[]` in the BCT/Ledger/crossing bundles. *(UTS discipline is delegated to `CC‑GCORE‑UTS‑1`.)* | Make bridge calibration externally citable and checkable.                      |
...
| **CC‑G7‑SoSLOG‑Pins‑1**   | When `G.7:Ext.SoSLogClauses` is in use, [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] outputs **MUST** expose the cited SoS‑LOG rule ids and the relevant `PathId/PathSliceId` evidence citations; any change in those pins **MUST** be RSCR‑relevant per `CC‑GCORE‑TRIG‑1…TRIG‑4`.                                               | Keep cross‑Tradition reuse explainable without embedding [[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]] semantics.        |
| **CC‑G7‑Acceptance‑1**    | When `G.7:Ext.AcceptanceHooks` is in use, [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] outputs **MUST** expose the Acceptance clause ids/policy ids used as gates; thresholds/unknown handling remain governed by Acceptance; any change **MUST** be RSCR‑relevant per `CC‑GCORE‑TRIG‑1…TRIG‑4`.                                           | Keep thresholds and unknowns out of bridges while preserving auditability.     |
| **CC‑G7‑RowBottleneck‑1** | If a comparable construct row aggregates multiple bridge cells, row summaries (e.g., `RowCL_min`) **MUST** follow bottleneck discipline ([[F.7 - Concept‑Set Table|F.7]]) and cite a counterexample whenever a cell carries a loss note.                                                                              | Forbid “CL averaging” and enforce loss‑aware summaries.                        |
| **CC‑G7‑PolicyPins‑1**    | [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] outputs **MUST** publish the *policy id pins* required to audit penalty routing and plane effects (ids only), as required by `CC‑GCORE‑LINK‑1/2` and `CC‑GCORE‑PEN‑1`. [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] MUST NOT duplicate policy tables or redefine penalty semantics.                                           | Keep penalty routing auditable while preserving single‑governing-pattern policy semantics. |
...
| **CC‑G7‑Sentinels‑1**     | [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] **MUST** register `BridgeSentinel` entries for bridges used by live scopes and **MUST** emit typed RSCR triggers (canonical `RSCRTriggerKindId`; see `CC‑GCORE‑TRIG‑1…TRIG‑4`) on calibration‑relevant edits, scoped to the watched `PathSliceId[]` or `PatternScopeId[]`, with the minimum payload pins from §4.1. | Enable targeted refresh rather than pack‑wide reruns.                          |
| **CC‑G7‑QD‑Pins‑1**       | When `G.7:Ext.QDParityPins` is in use, [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] outputs **MUST** include `{DescriptorMapRef.edition, DistanceDefRef.edition, InsertionPolicyRef}` and treat any change to those pins as RSCR‑relevant per `CC‑GCORE‑TRIG‑1…TRIG‑4`.                                                          | Prevent silent QD telemetry drift.                                             |
| **CC‑G7‑DHC‑Units‑1**     | When AlignmentDensity (or related DHC accounts) are reported, [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] outputs **MUST** (a) restrict the counted bridge set to rows with `RowCL_min ≥ 2` (treat `CL=3` as “free substitution”, `CL=2` as “guarded” for reporting), (b) include declared units, and (c) cite the relevant DHC method semantics ([[C.21 - Field Health & Structure (Discipline-CHR)|C.21]]). [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]] MUST NOT invent arithmetic over ordinal/illegal surfaces. | Keep dashboards and discipline‑health metrics lawful and interpretable.        |

## [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:8 - Common Anti-Patterns and How to Avoid Them
...
* **SenseFamily jump (scope‑bridge used as kind‑bridge).**
  **Avoid:** keep channel/sense‑family constraints governed by **[[F.9 - Alignment & Bridge across Contexts|F.9]]** visible; use `RowScopeId` to state which channel is claimed, and require `CL^k` + `Ψ(CL^k)` pins when a kind‑channel bridge is invoked (do not “upgrade” a scope‑channel bridge into kind substitution).
...
## [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:9 - Consequences
...
## [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:10 - Rationale
...
## [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:11 - SoTA‑Echoing (post‑2015, for orientation; non‑normative)
...
## [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:12 - Relations

**Builds on:** [[G.Core - Part G Core Invariants|G.Core]], [[G.2 - SoTA Harvester & Synthesis|G.2]], [[F.3 - Intra‑Context Sense Clustering|F.3]], [[F.7 - Concept‑Set Table|F.7]], [[F.9 - Alignment & Bridge across Contexts|F.9]], [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]], [[E.10 - Unified Lexical Rules for FPF|E.10]], [[E.18 - Transformation Flow Structure|E.18]], [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]], [[G.6 - Evidence Graph & Provenance Ledger|G.6]], [[C.21 - Field Health & Structure (Discipline-CHR)|C.21]].
**Optionally uses via Extensions:** **[[G.4 - CAL Authoring for a CG-Frame- Operators, Acceptance Clauses, Evidence Wiring|G.4]]** (Acceptance hooks), **[[C.23 - MethodFamily Evidence & Maturity (Method‑SoS‑LOG)|C.23]]** (SoS‑LOG clauses), **[[C.18 - Open‑Ended Search Calculus (NQD‑CAL)|C.18]] and [[C.19 - Explore-Exploit Governor (E-E‑LOG)|C.19]]** (QD/OEE pins).
**Used by / prerequisite for:** **[[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]]** (cross‑Tradition eligibility/selection), **[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]** (refresh orchestration), **[[G.9 - Parity and Benchmark Harness|G.9]]** (parity across Traditions where bridges are required), **[[G.10 - SoTA Pack Shipping|G.10]]** (shipping surfaces that must cite bridge calibration ids), **[[G.12 - DHC Dashboards (Discipline‑Health time‑series; admissible telemetry; generation‑first)|G.12]]** (DHC dashboards when bridge counts/units are surfaced).
**Publishes to:** **UTS** (bridge and crossing rows; twin labels as applicable) and emits RSCR‑ready telemetry/trigger payloads for **[[G.11 - Telemetry-Driven Refresh & Decay Orchestrator|G.11]]**.
**Constrains:** Any downstream consumer that claims cross‑Context/Tradition reuse must use the calibrated bridge artefacts/pins surfaced by this kit (governed by [[G.Core - Part G Core Invariants|G.Core]] crossing invariants apply).

## [[G.7 - Cross‑Tradition Bridge Calibration Kit (BridgeMatrix → BridgeCards + BCT-Sentinels)|G.7]]:End
