...
## [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:0 - Use this when

Use [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] when you are writing, revising, or reviewing one FPF pattern and need to know what shape, voice, reader-recognition role, and assurance material the pattern must carry before it can be treated as mature FPF text.
...
**Not this pattern when.** Use [[E.9 - Design‑Rationale Record (DRR) Method|E.9]] when the main work is deciding why FPF should change and how that decision is distributed across patterns. Use [[E.19 - Pattern Quality Gates- Review and Refresh Profiles|E.19]] when the main work is an admission or refresh review. Use the local domain pattern when the question is what FPF says inside that domain rather than how a pattern should be authored.

## [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:0.1 - What goes wrong if missed
...
When an authoring pass claims quality improvement rather than ordinary drafting, keep these roles distinct: [[E.22 - Improvement-Oriented Quality Evaluation Question Framing|E.22]] frames the improvement-oriented quality-evaluation question, the object-under-improvement evaluation such as [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] or [[E.9.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]] supplies value meanings and stop meanings, [[C.16.Q - Quality-Term Precision Restoration|C.16.Q]] repairs overloaded quality and evaluative-characterization wording, [[C.25 - Q-Bundle- Authoring --ilities- as Structured Quality Bundles|C.25]] carries engineering quality-family endpoints when those endpoints are claimed, and [[E.23 - Quality Improvement Loop Method|E.23]] governs any repeated quality-improvement method. Closing checklist rows or satisfying a review profile is not by itself quality improvement.

When a pattern claims practical payoff or uses a score, coordinate value, checklist result, benchmark, projection signal, review result, or release posture as evidence of value, name the intended value and the visible proxy relation. If the visible proxy is being treated as the value itself, apply [[E.13 - Pragmatic Utility and Value Alignment|E.13]] and repair the proxy-to-value substitution before the payoff claim is admitted.
...
**Primary EntityOfConcern in plain terms.** The primary `EntityOfConcern` of [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] is the authored FPF pattern: its canonical sections, reader-recognition role, wording discipline, examples, rationale, anti-patterns, SoTA-Echoing, and relations.
...
## [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:0.3 - Pattern Kind In Plain Terms
...
## [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:2 - Problem
...
## [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:3 - Forces
...
## [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:4 - Solution — One template, enriched by style principles

### [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:4.1 - Canonical Pattern Template
...
#### [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:4.1.1 - Heading & ID discipline (human tooling + retrieval)
...
For a **pattern heading** it is the pattern ID (e.g., [[A.2 - Role Taxonomy|A.2]], [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]).
...
* Inside a pattern, each nested section **MUST** add exactly one `#` per level (e.g., `## [[A.2 - Role Taxonomy|A.2]] - …`, `### [[A.2 - Role Taxonomy|A.2]]:2 - …`, `#### [[A.2 - Role Taxonomy|A.2]]:2.1 - …`).
...
### [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:4.2 - Stylistic Principles (S-0 ... S-19)
...
### [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:4.2.1 - Recognition text and assurance text
...
Under [[F.18 - Local‑First Unification Naming Protocol|F.18]] local-first naming, the canonical pair here is **recognition text** and **assurance text**.
...
When a compact matrix helps, [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]] is the preferred template for showing that breadth.
...
### [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:4.2.2 - Package-form and governing-pattern relation role-word discipline
...
This is a cross-cutting review discipline, not a replacement for local pattern lexica. For example, [[A.6.7 - MechSuiteDescription - Description of a set of distinct mechanisms|A.6.7]] / [[A.19.CHR - CHRMechanismSuite|A.19.CHR]] already carry the suite/kit/pack distinction, and [[E.17.1 - U.ViewpointBundleLibrary - Reusable Viewpoint Bundles|E.17.1]] already carries the viewpoint bundle/family/library distinction.
...
#### [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:4.2.2.1 - Precision-restoration placement discipline

When a pattern or companion text is drafted from [[E.10 - Unified Lexical Rules for FPF|E.10]] or [[E.10.ARCH - Wording-Use Ontological Precision Restoration Architecture|E.10.ARCH]], distinguish two authoring objects:
...
These are not synonyms. A precision-restoration pattern is placed in the pattern nest whose primary `EntityOfConcern`, relation record, or claim record it repairs. Its `semanticArea` states the Part-F semantic unit it repairs, while its `ontologicalNeighborhood` may name several relation governing the asserted uses. For example, quality-term repair lives in the [[C.16 - Measurement & Metrics Characterization (MM‑CHR)|C.16]] characterization nest, even though its neighbouring relations can include relation construction, action invitation, evidence, assurance, source-use assignment, engineering quality bundles, pattern-quality evaluation, or mathematical-lens use.

Affected patterns should use a thin pointer when the first-stage wording repair belongs elsewhere. The pointer names the selected restoration pattern and the condition that triggers it; it does not copy the trigger registry, the full [[E.10.ARCH - Wording-Use Ontological Precision Restoration Architecture|E.10.ARCH]] recovery algorithm, or a second local architecture for the same repair. The affected pattern then keeps its own subject matter: the characteristic, structure, view, episteme, relation, evidence, assurance, gate, work, decision, or adequacy question it already governs.
...
### [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:4.2.3 - Reader-role discipline for pattern prose
...
[[E.8 - FPF Authoring Conventions & Style Guide|E.8]] reader and reviewer wording is FPF pattern-authoring wording. Project-side publication readers, explanation readers, comparative review units, and participants in named project-side review relations are governed by the publication or project-side patterns that name those publication units, explanation-use relations, comparative review units, evidence paths, work records, or gate records, such as [[E.17 - Multi‑View Publication Kit|E.17]], [[E.17.ID.CR - ComparativeReviewUnit - bounded comparison over comparative review units|E.17.ID.CR]], [[E.17.EFP - ExplanationFaithfulnessProfile - explanation-use discipline over existing MVPK faces|E.17.EFP]], [[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]], [[A.15.4 - Work-Relevant Source Restoration|A.15.4]], [[A.20 - U.Flow.ConstraintValidity - Eulerian|A.20]], or [[A.21 - GateProfilization- OperationalGate(profile) (GateFit core)|A.21]].
...
### [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:4.2.4 - Human-facing fit beyond role correctness
...
**P-2 epistemic precision check.** When [[E.10 - Unified Lexical Rules for FPF|E.10]] selects epistemic precision restoration for pattern prose, the first admissible action-guiding move must survive as a remaining admissible reader move or be replaced by an neighboring FPF pattern governing that claim application that now carries that claim. This is a direct [[E.2 - The Eleven Pillars|E.2]] `P-2` and [[E.12 - Didactic Primacy & Cognitive Ergonomics|E.12]] requirement, not an optional style preference. Intentional didactic metaphors and vivid Plain recognition lines are admissible when they are ordinary recognition aids or when their claim kind or admissible-use boundary maps back to Tech under `E.10:6.2`. A precision-corrected rewrite that leaves the recognition text inert is still under-authored.
...
separate navigation block. When an [[E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline|E.11]] pattern-entry discoverability problem
...
- the first admissible action-guiding result; when an [[E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline|E.11]] discoverability
...
Compact candidate-pattern comparison belongs in [[E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline|E.11]]-distributed entry material; expanded entry-disambiguation cases belong in [[I.2 - Expanded Entry Disambiguation Cases|I.2]].

If the prose points to neighbouring patterns or other governing companion roles, it should present them as neighboring FPF patterns, project-side FPF kinds and references named by value, or [[E.11 - First-Practical Entry and Pattern-Use Discoverability Discipline|E.11]] entry-recognition reclassifications rather than as hidden co-authorities of the current pattern.
...
When a compact matrix helps, [[F.16 - Worked‑Example Template (Cross‑Domain)|F.16]] is the preferred template for making that breadth legible.
...
Neighboring pattern relations / entry-recognition reclassifications: [[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]], [[A.6.C - Contract Unpacking for Boundaries|A.6.C]].
```
...
First admissible action-guiding result: one [[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]]-governed atomic claim set or one Claim Register whose claim/use questions are explicit enough for the governing FPF pattern or named project-side FPF kind and reference to inspect.
```

### [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:4.2.5 - Design-time and run-time referents stay separated in pattern prose
...
1) Add a subsection **“Autonomy (RoC‑[[E.16 - RoC‑Autonomy Budget & Enforcement|E.16]])”** that lists:
...
## [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:5 - Archetypal Grounding (System / Episteme)
...
| Hook‑Weave‑Ground | Opens with field anecdote → weaves in Γ‑core → ties the claim to motor torque. | Opens with historical paradox → weaves in **[[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]** evidence refs → ties the claim to peer‑review data. |
...
## [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:6 - Bias‑Annotation
...
## [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:7 - Conformance Checklist
...
| **CC-SG.9 (Lexical hygiene).** | The term **mapping** **SHALL NOT** appear in SoTA-Echoing except in the precise [[E.10 - Unified Lexical Rules for FPF|E.10]] sense; use **alignment/Bridge/relation** instead. | Avoids overloading reserved vocabulary. |
...
| **CC-SG.12 (Boundary claim-set discipline).** | If a pattern’s subject is a boundary, interface, API, protocol, connector, SLA, or other published boundary description, it **MUST** either (a) provide an **[[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]]**-governed atomic claim set (`L-*`/`A-*`/`D-*`/`E-*`, with stable IDs), or (b) explicitly cite an existing **[[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]] Claim Register** / scoped claim set that it reuses. | Pulls [[A.6.B - Boundary Norm Square (Laws - Admissibility - Deontics - Work‑Effects)|A.6.B]] into the authoring contour, prevents boundary-kind soup, and makes review more explicit and repeatable. |
...
## [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:8 - Common Anti-Patterns and How to Avoid Them
...
## [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:9 - Consequences
...
## [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:10 - Rationale
...
## [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:11 - SoTA-Echoing  *(normative; lineage and deltas to contemporary State-of-the-Art)*
...
### [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:11.1 - SoTA alignment for this pattern ([[E.8 - FPF Authoring Conventions & Style Guide|E.8]] self-echo)

| Claim ([[E.8 - FPF Authoring Conventions & Style Guide|E.8]] need) | SoTA practice (post-2015) | Use of source | Primary source (post-2015) | Alignment with [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] | Adoption status |
|---|---|---|---|---|---|
| Pattern texts must be teachable, not just correct. | Use a stable skeleton with context, problem, forces, solution, actions, and consequences, plus illustration and checks, to keep patterns readable and actionable. | **Current-practice writing-guidance use.** This row is used for [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]'s canonical pattern skeleton and didactic ordering; it is not treated as external authority over FPF ontology. | Iba (2021), “How to Write Patterns: A Practical Guide for Creating a Pattern Language on Human Actions” (PLoP 2021 PLoPourri). | Canonical Template mirrors the skeleton and adds Archetypal Grounding plus Conformance Checklist as first-class sections. | **Adopt and adapt.** Adopt the skeleton; adapt by making bias and conformance explicit sections. |
| Pattern quality needs explicit validation beyond folklore. | Critique of ad hoc validation, including the rule of three, and push toward more rigorous discovery and validation methods. | **Current-best source use for pattern discovery and validation rigor in this row's narrow use.** The source changes [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] by requiring validation to be explicit rather than folklore-based. | Riehle, Harutyunyan, Barcomb (2020), “Pattern Discovery and Validation Using Scientific Research Methods”. | [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] encodes validation as Conformance Checklist plus SoTA-Echoing with adoption status and evidence binding. | **Adopt.** Adopt auditability goals; keep the mechanism lightweight through checks and evidence binding. |
| Governance should constrain structure, not mandate tools. | Specify conformance and structure; do not prescribe processes, notations, tools, or recording media. | **Current-standard reference use.** The architecture-description standard supplies a useful conformance-vs-tooling distinction, but [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] does not import architecture-description ontology as pattern-authoring ontology. | ISO/IEC/IEEE 42010:2022, *Software, systems and enterprise - Architecture description*. | [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] is template-centric and conformance-centric, with guardrails against tool and notation lock-in in core narrative. | **Adopt.** Directly adopt the conformance-not-tooling discipline for authoring shape. |
| Pattern languages are networks; visuals often mislead. | Systematic surveys report low consensus on what to visualise and ambiguous or inexpressive visuals; relations need clear definition in text. | **Current-practice rationale use.** The source is used as rationale for the text-first relation discipline; it is not current-best source material for all pattern-language visualization. | Quirino, Barcellos, Falbo (2018), “Visual Notations for Software Pattern Languages: A Mapping Study”. | [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] requires a Relations section and keeps diagrams optional, placing primacy on textual structure and explicit links. | **Adapt.** Use the finding as rationale for text-first, relation-explicit authoring. |
| Controlled technical writing should be plain without losing necessary terms. | Plain-language practice writes for a specific audience, removes unnecessary words and hidden verbs, avoids synonym churn for the same object, and keeps necessary technical or legal terms when they carry the meaning. | **Current-practice source use for kind-safe plain wording.** The source changes [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] by making plain wording a precision-preserving diagnostic, not decorative simplification. | U.S. Plain Writing Act implementation and Digital.gov plain-language guides; SEC, *A Plain English Handbook*. | [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] encodes kind-safe plain rewriting: remove phrase-level role/carrier/locus/flow/status apparatus only when it adds no kind, relation, evidence value, or user move; preserve established FPF terms unless [[E.10 - Unified Lexical Rules for FPF|E.10]]/[[F.18 - Local‑First Unification Naming Protocol|F.18]] changes them. | **Adapt.** Adopt audience-first, concise, consistent wording; adapt it to FPF kind preservation. |

## [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:12 - Relations
* **Coordinates with:** [[E.9.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]] when an authored pattern body is drafted from a concrete `DRR` and the blocker is whether the `DRR` selected, distributed, carried source use, carried accepted decisions, or supplied a first drafting move sufficiently for that authoring use. [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] still governs the pattern body; [[E.9.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]] is not a mandatory authoring section, review card, or substitute for writing the Solution.

* **Builds on:** [[E.6 - Didactic Architecture of the Specification|E.6]], [[E.7 - Archetypal Grounding Principle|E.7]]
* **Constrained by:** Guard‑Rails [[E.5.1 - DevOps Lexical Firewall|E.5.1]]–[[E.5.4 - Cross‑Disciplinary Bias Audit|E.5.4]] (lexical firewall, notation independence, etc.)
* **Coordinates with:** [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] when one authored FPF pattern version is evaluated as a scoped pattern-quality claim. [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] governs authoring shape, recognition text, action guidance, worked cases, SoTA grounding, and conformance material; [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] governs the pattern-quality evaluation, required coordinate values, `PatternQualityStatus`, and stop condition. Do not import [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] as a mandatory authoring section or full review card.
* **Coordinates with:** [[E.23 - Quality Improvement Loop Method|E.23]] when an authored FPF pattern body is being improved through repeated passes. [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] still governs the authored pattern body; [[E.23 - Quality Improvement Loop Method|E.23]] governs the repeated quality-improvement method; the object-under-improvement evaluation such as [[E.21 - FPF Pattern-Quality Evaluation CharacteristicSpace|E.21]] or [[E.9.DA - DRR Decision-Adequacy Evaluation CharacteristicSpace|E.9.DA]] supplies value meanings and stop meanings.
* **Coordinates with:** [[E.13 - Pragmatic Utility and Value Alignment|E.13]] when an authored pattern claims practical payoff or uses a visible quality value, metric, checklist result, review result, or release posture as if it were the intended value. [[E.8 - FPF Authoring Conventions & Style Guide|E.8]] keeps the payoff in user-facing prose; [[E.13 - Pragmatic Utility and Value Alignment|E.13]] repairs proxy-to-value substitution.


* **Constrains:** All patterns; the DRR template references the same section order.

## [[E.8 - FPF Authoring Conventions & Style Guide|E.8]]:End
