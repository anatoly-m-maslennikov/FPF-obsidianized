...
**[[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] compliance —** Only **Work** carries resource deltas; quantitative splits/consumption use **PortionOf** against pre‑consumption stocks; run histories use **PhaseOf** on Work; `MemberOf` MUST NOT be used for resource mereology; SCR/RSCR stay outside (use EPV‑DAG anchors).

## [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:1 - Problem frame

FPF distinguishes **what is done** from **what it costs** to do it.

* **Method, MethodDescription, and design-time process:**
  A **Method** is the abstract **way‑of‑doing** inside a bounded context ([[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]). A **MethodDescription** is a design‑time `U.Episteme` that describes a Method (SOP, algorithm, proof, simulator configuration, etc.).
  A **Process** is a *view* that represents a MethodDescription as an ordered/partially‑ordered composition (steps, branches, synchronization). In Cluster B, that ordering/coordination is handled by **Γ\_method** ([[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]]). **Not every MethodDescription admits a step decomposition**; Γ\_method applies only when a step/process view is chosen.

* **Work (run‑time; this pattern focuses on the resource facet):**
  **Work** is the dated run‑time **occurrence** of enacting a MethodDescription by a performer under a `U.RoleAssignment` ([[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]). In this pattern we treat Work under its **spent‑resource facet**: the typed delta we can account for across a declared boundary and time window. Γ\_work defines how those deltas compose across parts and phases.
...
## [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:2 - Problem
...
4. **Category errors in mereology:** Collection membership (MemberOf) is misused as if it were parthood for resource stocks, polluting Γ proofs ([[B.1 - Universal Algebra of Aggregation (Γ)|B.1]]).

## [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:3 - Forces
...
| **Safety vs. Synergy**                              | Weakest‑link bounds must cap availability ↔ redundancy or substitution can improve feasibility but belongs to emergence ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]). |

## [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:4 - Terminology guard‑rails ([[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] — Strict Distinction)
...
* **Transformer (T)** — a `U.System` playing the executing and/or auditing role for Work’s accounting ([[A.12 - External Transformer & Reflexive Split|A.12]]); transformer identity belongs in the **Boundary Ledger**.
* **Mereology for resources ([[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]):** use `PortionOf` for **quantitative splits** and `PhaseOf` for **time‑slices**; **do not** use `MemberOf` for resource stocks.

## [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:5 - Solution — The Γ\_work Operator
...
### [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:5.1 - Operator signature

```
Γ_work : (S : Set[U.Work], M_spec : U.MethodDescription?) → W_tot : U.Work
```

* **S — Work set.** A finite set of `U.Work` instances to be rolled up (parts, phases, episodes, or boundary partitions). Each Work MUST carry (or reference) a **Boundary Ledger** (§5.3) and a typed resource ledger on an explicit basis. Where a stock is subdivided, the split uses `PortionOf`; where a run is time‑sliced, the slices use `PhaseOf` ([[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]]).

  If `S` contains overlaps (shared stocks, shared ports, or overlapping time windows), the fold MUST apply an explicit **overlap / de‑duplication policy** declared in the relevant `U.BoundedContext` ([[A.15.1 - U.Work|A.15.1]]:5.3); otherwise the result is undefined (double counting).
...
### [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:5.2 - What counts as “Work”
...
### [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:5.3 - Boundary Ledger (normative output metadata)
...
This ledger is what makes cross‑model Work totals comparable and auditable ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]).

### [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:5.4 - The invariant quintet instantiated (overview)

Γ\_work preserves [[B.1 - Universal Algebra of Aggregation (Γ)|B.1]] invariants; the detailed proofs and corner cases are in Part 2.
...
### [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:5.5 - How Γ\_work relates to Methods (and to Γ\_method)
...
## [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:6 - Fold rules (how Γ\_work composes)

### [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:6.1 - Boundary partition (across parts of a whole)
...
### [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:6.2 - Time slicing (serial runs / phases)
...
### [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:6.3 - Concurrent branches (parallel activity)
...
### [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:6.4 - Multi‑resource vectors and declared equivalences
...
### [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:6.5 - Availability gates (weakest‑link discipline)
...
If any inequality is violated, the fold **must fail** or the modeller must declare a **Meta‑Holon Transition ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]])** that introduces redundancy/substitution as a new structural capability (changing Q\* or the equivalence map). This is WLNK in resource form.

## [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:7 - Embodiment and dissipation (reporting scheme)
...
## [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:8 - Invariants — edge cases and proof sketches

### [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:8.1 - IDEM (idempotence)
...
### [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:8.2 - COMM/LOC (local commutativity / locality)
...
### [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:8.3 - WLNK (weakest‑link)
...
### [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:8.4 - MONO (monotonicity)
...
### [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:8.5 - Compatibility with Γ\_method
...
## [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:9 - Archetypal grounding (System / Episteme)
...
## [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:10 - Conformance Checklist (complete)

| ID            | Requirement                                                                                                                                     | Purpose                                               |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| **CC‑B1.6.1** | Every Γ\_work result SHALL include a **Boundary Ledger**: boundary, time window, basis, method context, transformer identity.                   | Make Work statements comparable and auditable ([[A.10 - Evidence Graph Referring- Claim-Bound Evidence and Provenance Graph|A.10]]). |
...
| **CC‑B1.6.3** | Resource stocks SHALL be structured with `PortionOf` and `PhaseOf`; `MemberOf` MUST NOT be used for resource mereology.                         | Align with [[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] and prevent category errors.          |
...
| **CC‑B1.6.8** | When `M_spec` declares a yield η, the report SHALL separate **planned** (ex‑ante) and **measured** (ex‑post) Work.                              | Keep planning distinct from accounting ([[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]).        |
| **CC‑B1.6.9** | Γ\_work SHALL provide proofs of the invariant quintet under the independence assumptions used, or explicitly state where MHT is required.       | Maintain [[B.1 - Universal Algebra of Aggregation (Γ)|B.1]] guarantees.                              |

## [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:11 - Consequences
...
* **Boundary modelling effort:** Requires explicit ports and stock deltas. *Mitigation:* use [[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] templates for common boundary patterns.
...
## [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:12 - Rationale (informative)

Γ\_work is a conservative algebra of **spent resources**. It respects physical conservation (mass/energy), supports information‑centric resources without conflation, and keeps the **design‑time** (MethodDescription) separate from **run‑time** (Work) facts ([[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]). Additivity over disjoint boundaries and non‑overlapping phases is the minimal set of rules that yields stable cross‑scale accounting while remaining faithful to the universal invariants of [[B.1 - Universal Algebra of Aggregation (Γ)|B.1]]. Emergent efficiency (redundancy, substitution) is not “free”: it is made structural via **Meta‑Holon Transition** ([[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]]), after which the same algebra applies at the new level.

## [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:13 - Relations
...
- [[C.27 - Temporal Claim Adequacy- State Readings, Temporal Trends, and Intervention-Sensitive Temporal Change|C.27]] may flag: an authored claim that planned effort, actual effort trace, resource burn, effort window, resistance, or cost changes a temporal outcome.
...
- Exit: use [[C.27 - Temporal Claim Adequacy- State Readings, Temporal Trends, and Intervention-Sensitive Temporal Change|C.27]] only for the temporal-claim adequacy question; use work/resource patterns for actual work evidence and cite dynamics, causal/evaluation, or benchmark patterns when those other questions are live.

* **Builds on:** [[A.12 - External Transformer & Reflexive Split|A.12]] **Transformer Principle**; [[A.14 - Advanced Mereology- Components, Portions, Aspects & Phases|A.14]] **Mereology Extension** (PortionOf, PhaseOf); [[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]] **Strict Distinction** (MethodDescription / Method / Work).
* **Coordinates with:** [[B.1.5 - Γmethod - Order‑Sensitive Method Composition & Work Enactment|B.1.5]] **Γ\_method** (order and concurrency), [[B.1.4 - Contextual & Temporal Aggregation (Γ-ctx & Γ-time)|B.1.4]] **Γ\_time** (temporal coverage), [[B.1.2 - System‑specific Aggregation Γ-sys|B.1.2]] **Γ\_sys** (system assembly).
* **Triggers:** [[B.2 - Meta‑Holon Transition (MHT)- Recognizing Emergence and Re‑identifying Wholes|B.2]] **Meta‑Holon Transition (MHT): Recognizing Emergence and Re‑identifying Wholes** when feasibility constraints (WLNK) are beaten by structural redundancy/substitution.
* **Feeds:** [[B.3 - Trust and Assurance Calculus (F-G-R with Congruence)|B.3]] **Trust & Assurance Calculus (F–G–R with Congruence)** (cost‑aware confidence overlays) — informative only, without altering Γ\_work’s conservation semantics.
...
## [[B.1.6 - Γ-work - Work as Spent Resource|B.1.6]]:End
