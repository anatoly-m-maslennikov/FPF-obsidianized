...
**Status.** Normative in **Part C**. Identifier **[[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]**.
**Audience.** Engineering managers, architects, reviewers, editors.

**Depends on.**

* **[[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]] — U.Kind & SubkindOf (Core):** kinds are intensional; `⊑` is a partial order; kinds **carry no Scope**.
* **[[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]] — KindSignature (+F) & Extension/MemberOf:** signature F; deterministic `MemberOf(e,k,slice)`; `EntitySet(slice)`.
* **[[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]] — KindBridge & CL^k:** Cross‑context kind mapping; `CL^k` penalties → **R** only.
* **[[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]] — USM (Context slices & Scopes):** Claim/Work scope (**G**) over `U.ContextSlice`; bridges and **CL** for scope.
* **[[C.2.2 - Reliability R in the F-G-R triad|C.2.2]] — F–G–R; [[C.2.3 - Unified Formality Characteristic F|C.2.3]] — U.Formality (F).**
...
## [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:1 - Purpose (manager’s view)
...
## [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:2 - Context

Kinds ([[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]].1/3.2) name **what** claims quantify over; USM ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]) governs **where** claims hold. In practice, procedures need **local tailoring** of kinds for a role/process (compliance profile, product line, cohort). RoleMask gives that tailoring **without** mutating entityOfConcern (Kind) or applicability (Scope).

## [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:3 - Problem
...
## [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:4 - Forces
...
## [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:5 - Solution — **RoleMask** (overview)
...
3. **May declare context expectations** (selectors over `U.ContextSlice`, e.g., jurisdiction, API version). **These are enforced via USM Scope guards** ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]) and **do not** change mask membership.
...
## [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:6 - Norms & Invariants (normative)

> The following formalize and expand **RM‑01…RM‑08** referenced in [[C.3 - Kinds, Intent-Extent, and Typed Reasoning (Kind‑CAL)|C.3]].

### [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:6.1 - Definition & Shape
...
**RM‑02 (Not a new kind).** A RoleMask **MUST NOT** introduce a new `U.Kind`. If the domain needs a stable refinement, Contexts **SHALL** publish an explicit `SubkindOf` node ([[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]]).
...
### [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:6.2 - Separation of channels
...
* Predicates about **ContextSlice** (jurisdiction, Standards, Γ\_time) **SHALL** be enforced via **USM Scope** guards ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]). Masks **MUST NOT** hide Scope requirements inside membership checks.

**Guard routing.** Enforce ContextSlice predicates via **USM Scope** ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]) and entity predicates via **membership**; see **Annex [[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]] §4.3 (Guard_MaskedUse)** and **§5 (E‑01)** for the normative order of checks.
...
### [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:6.3 - Promotion & Catalog
...
### [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:6.4 - Cross‑context use
...
## [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:7 - Invariants & Non‑goals (normative)
...
## [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:8 - Interactions (informative)

### [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:8.1 - With Kinds & Subkinds ([[C.3.1 - U.Kind & SubkindOf (Core)|C.3.1]])
...
### [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:8.2 - With Membership & Signature ([[C.3.2 - KindSignature (+F) & Extension-MemberOf|C.3.2]])
...
### [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:8.3 - With KindBridge ([[C.3.3 - KindBridge & CL^k - Cross‑context Mapping of Kinds|C.3.3]])
...
### [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:8.4 - With Guards (Annex [[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]])

Use **`Guard_MaskedUse`** (Annex **[[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]] §4.3**). It requires:
— mask registration & determinism;
— Scope coverage ([[A.2.6 - Unified Scope Mechanism (USM)- Context Slices & Scopes|A.2.6]]), **Γ\_time** explicit;
— where Cross‑context: KindBridge (`CL^k`) + MaskAdapter + penalties → **R**.
— **Cross‑context combo.** When masks cross Contexts, use **`Guard_MaskedUse`** together with **`Guard_XContext_Typed`** (Annex **[[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]] §4.5**) so both bridges are checked and both penalties (**Φ(CL)** and **Ψ(CLᵏ)**) land in **R**.
— **Order of checks.** Follow **Annex [[C.3.A - Typed Guard Macros for Kinds + USM (Annex)|C.3.A]] §5 (E‑01)**: typed compatibility first, then Scope coverage, then penalties to **R** and freshness.

## [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:9 - Anti‑patterns & Remedies (informative)
...
## [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:10 - Worked Examples (informative)

### [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:10.1 - `Vehicle@ABSOnly` (constraint mask)
...
### [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:10.2 - `AuthenticatedRequest@Frontend` (vocabulary mask)
...
### [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:10.3 - `AdultPatient@Clinic` (composite mask) across jurisdictions
...
## [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:11 - Authoring & Review Guidance (informative)

### [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:11.1 - Authoring a RoleMask card
...
### [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:11.2 - Reviewer 7‑point checklist
...
## [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:12 - Conformance Checklist (normative)
...
## [[C.3.4 - RoleMask - Contextual Adaptation of Kinds (without cloning)|C.3.4]]:End
