...
## [[B.4 - Canonical Evolution Loop|B.4]]:3 - **Forces**
...
## [[B.4 - Canonical Evolution Loop|B.4]]:4 - **Solution**

FPF defines the **Canonical Evolution Loop**, a four-phase cycle that serves as the universal engine for all principled, open-ended evolution. This loop is a direct implementation of the **Explore → Shape → Evidence → Operate** state machine (Pattern [[B.5.1 - Explore → Shape → Evidence → Operate|B.5.1]]) and is powered by the **Canonical Reasoning Cycle** (Pattern [[B.5 - Canonical Reasoning Cycle|B.5]]).

The loop creates a closed, auditable circuit between the two temporal scopes. Crucially, transitions between phases are performed by an **external `Transformer`** (Pattern [[A.12 - External Transformer & Reflexive Split|A.12]]). A holon does not evolve itself; it is evolved by an external agent acting upon it.
...
## [[B.4 - Canonical Evolution Loop|B.4]]:5 - **Archetypal Grounding**
...
*   **[[B.4.1 - Observe -- Notice -- Stabilize -- Route|B.4.1]] - Observe -> Notice -> Stabilize -> Route (pre-abductive seam):**
...
*   **B.4.2 - Knowledge Instantiation (Theory Refinement Loop):**
    ...
    *   **Loop Example:**
        ...
        4.  **Deploy:** The team (`Transformer`) publishes the revised theory in a journal. The scientific community begins to use the new version. **Note.** The *chart* and any CG‑frame readings derived from this episteme MUST cite the updated `MethodDescription` (per [[A.19.CN - CN‑frame (comparability & normalization)|A.19.CN]] CC‑A19.D1‑3) to keep comparability auditable.

      **Adaptive-specialization note.** Knowledge instantiation for one declared task family **SHALL** name the prior basis being refined from, the named work-measure threshold being pursued, the adaptation budget being spent, and the freshness or provenance basis for claiming the specialization is reusable. If the refinement is claimed as one specialization step, it **SHALL** also cite the declared `TaskFamily` or `TaskSignature` anchor that later [[C.22.1 - Task-family adaptation signature|C.22.1]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], and [[G.9 - Parity and Benchmark Harness|G.9]] will consume. This keeps the refinement legible as contextual task-family specialization rather than vague general capability growth.
...
## [[B.4 - Canonical Evolution Loop|B.4]]:6 - **Conformance Checklist**
...
*   **CC-B4.3 (Transformer Mandate):** The *Observe*, *Refine*, and *Deploy* transitions **MUST** be performed by an explicitly identified external `Transformer` (Pattern [[A.12 - External Transformer & Reflexive Split|A.12]]). A holon cannot observe, refine, or deploy itself.
...
*   **CC-B4.5 (Adaptive-specialization boundary):** `B.4.2` and `B.4.3` **SHALL NOT** silently re-govern selector or parity semantics. If transfer, retention, downstream exploitation efficiency, corridor entry, or downside cost are comparison-relevant, the pattern-local note **MUST** leave those fields recoverable by the downstream [[C.22.1 - Task-family adaptation signature|C.22.1]], [[G.5 - Multi‑Method Dispatcher & MethodFamily Registry|G.5]], and [[G.9 - Parity and Benchmark Harness|G.9]] governing patterns.

## [[B.4 - Canonical Evolution Loop|B.4]]:7 - **Common Anti-Patterns and How to Avoid Them**
...
## [[B.4 - Canonical Evolution Loop|B.4]]:8 - **Consequences**
...
## [[B.4 - Canonical Evolution Loop|B.4]]:9 - **Rationale**
...
## [[B.4 - Canonical Evolution Loop|B.4]]:10 - **Relations**
...
### [[B.4 - Canonical Evolution Loop|B.4]]:12.1 - Pre-abductive seam compatibility
For early language-state routing, `Observe` does not have to jump directly into anomaly or hypothesis forms. `Observe` may publish `U.PreArticulationCuePack` and a `RoutedCueSet` via [[B.4.1 - Observe -- Notice -- Stabilize -- Route|B.4.1]], after which later loops consume that routed cue publication directly or a downstream typed publication such as `U.AbductivePrompt`, as appropriate.
## [[B.4 - Canonical Evolution Loop|B.4]]:End
