...
**Builds on:** [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **D.CTX** (Context discipline); A.3/**[[A.3.1 - U.Method- Context-Defined Way of Doing|A.3.1]]**/**[[A.3.2 - U.MethodDescription- Description Episteme for a Way of Doing|A.3.2]]** (Transformer Constitution; `U.Method`, `U.MethodDescription`); A.15/**[[A.15.1 - U.Work|A.15.1]]** (`U.Work` as record of occurrence); Sys‑CAL (control/actuation semantics); KD‑CAL (observation).
**Coordinates with.** [[F.1 - Domain‑Family Landscape Survey|F.1]]–[[F.3 - Intra‑Context Sense Clustering|F.3]] (Contexts, Seeds → SenseCells), [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] (Role Description), [[F.5 - Naming Discipline for U.Types & Roles|F.5]] (Naming), [[F.6 - Role Assignment & Enactment Cycle (Six-Step)|F.6]] (Role Assignment & Enactment Cycle (Six-Step)), F.7/F.9 (Bridges), [[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] (Status families & Windows).
**Aliases (informative).** *Method/Spec/Work or Actuation split*; *DesignRunTag harmonisation*.

## [[F.11 - Method Quartet Harmonisation|F.11]]:1 - Intent & applicability
...
## [[F.11 - Method Quartet Harmonisation|F.11]]:2 - Problem frame
...
## [[F.11 - Method Quartet Harmonisation|F.11]]:3 - Forces
...
## [[F.11 - Method Quartet Harmonisation|F.11]]:4 - Core idea (didactic)
...
## [[F.11 - Method Quartet Harmonisation|F.11]]:5 - Minimal vocabulary (this pattern only)

* **Context** = `U.BoundedContext` (per D.CTX).
* **Local‑Sense** → **SenseCell** ([[F.3 - Intra‑Context Sense Clustering|F.3]]): the address *(Context × sense)* for a term like *process*, *task*, *activity*, *command*.
* **Concept‑Set** (F.7/F.8): row aligning multiple SenseCells as “what we regard as the same” (after Bridges & losses are declared).
* **Window** ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]): temporal/conditional envelope (e.g., *July*, *during test run T42*, *under load ≥ 70%*).
* **StatusCell** ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]): laddered status **about** methods/specs/works (e.g., *Approved (spec)*; *Observed/Measured (work)*).

## [[F.11 - Method Quartet Harmonisation|F.11]]:6 - Solution — the quartet lens (notation‑free)

> *Not steps for a team—**lenses for a thinker**. Use them to sanity‑check any statement about “how”, “script”, “run”, or “signal”.*

### [[F.11 - Method Quartet Harmonisation|F.11]]:6.1 - The **stance split** (design vs run)
...
* **Guard rule.** Never let a conclusion cross stances without (a) an explicit Bridge kind (*interpretation* vs *substitution*), and (b) an acceptable CL (F.7/F.9, [[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]).

### [[F.11 - Method Quartet Harmonisation|F.11]]:6.2 - The **recipe/idea split**
...
### [[F.11 - Method Quartet Harmonisation|F.11]]:6.3 - The **happening** (Work) with **signal** (Actuation)
...
### [[F.11 - Method Quartet Harmonisation|F.11]]:6.4 - The **Role Assignment & Enactment touch-points**

* **Roles** ([[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]) bind **who enacts** the Method at run‑time (behavioural masks), **not** what permissions they hold (RBAC is a different Context).
* **Statuses** ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]) bind to the right box: *Approved* → MethodDescription; *Measured/Observed* → Work; *Satisfied/Violated* → Requirement clause about the Work’s outcomes within a **Window**.

## [[F.11 - Method Quartet Harmonisation|F.11]]:7 - Harmonisation map (Context‑first)

> Examples of **local SenseCells** and **safe Bridges**. *You may keep the exact Contexts from your [[F.1 - Domain‑Family Landscape Survey|F.1]] cut.*
...
* `SOSA:Observation` **interprets** `Requirement clause` ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]) about **Work outcomes** (**cross‑StatusModality: epistemic→deontic; never substitution**; declare **Bridge(kind=Interpretation, CL, Loss)**).

## [[F.11 - Method Quartet Harmonisation|F.11]]:8 - Invariants (normative)
...
3. **Context locality.** Terms (*process*, *activity*, *task*, *command*) **MUST** be read as **SenseCells** in their Contexts ([[F.3 - Intra‑Context Sense Clustering|F.3]]); Cross‑context equivalence is a matter for F.7/F.9 Bridges.
4. **Status placement.** *Approved* attaches to MethodDescription; *Observed/Measured* attach to Work; *Satisfied/Violated* attach to clauses about Work outcomes within a **Window** ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]).
...
6. **Naming clarity.** Technical/Plain labels for the quartet **SHOULD** be distinct ([[F.5 - Naming Discipline for U.Types & Roles|F.5]]); avoid homonymous single‑word labels when Contexts collide.
7. **Bridge guard.** Cross‑context moves **MUST** declare **kind** (≈, ⊑, ⊒, ⋂, ⊥, Interpretation), **CL**, and **Loss** (F.7/F.9).

## [[F.11 - Method Quartet Harmonisation|F.11]]:9 - Micro‑examples (didactic)
...
1. **Data pipeline deploy (software).**
   ...
   *Statuses*: Spec **Approved** (governance Context); Work **Measured** (rows processed) → Evidence for SLO ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]).
...
| **A8**  | **Recipe-as-Role**             | “The Spec assigns responsibility” (mixes MethodDescription with Role constructs — `U.RoleDescription`/`U.RoleAssignment`).  | Conflates the `U.MethodDescription` episteme with behavioural masks.         | Use **[[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] Role Description**; let **MethodDescription** only **describe** a Method.                                                                            |
| **A9**  | **One‑Context Scope**        | A single Context (e.g., BPMN) used as if it covered control/measurement. | Scope mirage; silent cross‑domain generalisation.                           | Re‑cut Contexts ([[F.1 - Domain‑Family Landscape Survey|F.1]]) to include control and sensing. Re‑express statements with the quartet across those Contexts.                                                                                                        |
...
## [[F.11 - Method Quartet Harmonisation|F.11]]:11 - Worked examples (extended)

> Each scenario names Contexts (from your [[F.1 - Domain‑Family Landscape Survey|F.1]] cut), identifies the quartet boxes, and shows safe Cross‑context moves.

### [[F.11 - Method Quartet Harmonisation|F.11]]:11.1 - ML service rollout (software + services + sensing)
...
* **Statuses ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]):** *Spec Approved*; *Work Observed* (latency/err‑rate via SOSA Observations); *SLO clause Satisfied* in Window if measured ≤ thresholds.
...
### [[F.11 - Method Quartet Harmonisation|F.11]]:11.2 - Industrial furnace control (control + sensing + services)
...
### [[F.11 - Method Quartet Harmonisation|F.11]]:11.3 - Clinical assay
...
### [[F.11 - Method Quartet Harmonisation|F.11]]:11.4 Incident response (services + enactment)
...
## [[F.11 - Method Quartet Harmonisation|F.11]]:12 - Reasoning primitives (judgement schemas)
...
7. **Clause evaluation** ([[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] handshake)
...
## [[F.11 - Method Quartet Harmonisation|F.11]]:13 - Relations

**Builds on:**
[[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **D.CTX** (Context ≡ `U.BoundedContext`); A.3/**[[A.3.1 - U.Method- Context-Defined Way of Doing|A.3.1]]**/**[[A.3.2 - U.MethodDescription- Description Episteme for a Way of Doing|A.3.2]]**/**[[A.15 - Role-Method-Work Alignment (Contextual Enactment)|A.15]]** (Method/Spec/Work foundations); Sys‑CAL (Actuation semantics); KD‑CAL (Observation); [[F.1 - Domain‑Family Landscape Survey|F.1]]–[[F.3 - Intra‑Context Sense Clustering|F.3]] (Contexts → SenseCells); [[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]] (Status families & Windows).

**Constrains:**

* **[[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] Role Description:** Roles or Statuses **must** point to the right box (e.g., *Approved* → MethodDescription; *Observed* → Work).
* **[[F.5 - Naming Discipline for U.Types & Roles|F.5]] Naming:** Enforce distinct Tech/Plain labels for Method/Spec/Work or Actuation where homonyms threaten.
...
## [[F.11 - Method Quartet Harmonisation|F.11]]:14 - Migration notes (conceptual)
...
5. **Name collisions.** Where *task/activity/process* appear with mixed meanings, prefix with Contexts and relabel per **[[F.5 - Naming Discipline for U.Types & Roles|F.5]]** (Tech/Plain).
6. **Backfill Bridges.** If earlier text implied Cross‑context equivalence, add explicit Bridges (F.7/F.9) declaring **kind/CL/Loss**.

## [[F.11 - Method Quartet Harmonisation|F.11]]:15 - Acceptance tests (SCR/RSCR — concept level)

### [[F.11 - Method Quartet Harmonisation|F.11]]:15.1 - Static conformance checks (SCR)
...
### [[F.11 - Method Quartet Harmonisation|F.11]]:15.2 - Regression checks (RSCR)
...
## [[F.11 - Method Quartet Harmonisation|F.11]]:16 - Didactic distillation (90‑second script)
...
> For compliance, don’t point at the plan—point at **Work**, show **Observations**, and judge clauses in **[[F.10 - Status Families Mapping (Evidence • Standard • Requirement)|F.10]]**.
> Hold this quartet in your head and you’ll stop mixing plans with facts, signals with outcomes, and names across Contexts. + Everything else—naming ([[F.5 - Naming Discipline for U.Types & Roles|F.5]]), `U.RoleDescription` ([[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]) and `U.RoleAssignment`/`U.RoleEnactment` ([[A.2 - Role Taxonomy|A.2]].1/F.6), Bridges (F.7/F.9)—falls into place.

## [[F.11 - Method Quartet Harmonisation|F.11]]:End
