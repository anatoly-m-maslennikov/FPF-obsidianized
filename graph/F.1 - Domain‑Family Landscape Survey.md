...
**Depends on.** [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **Lexical Discipline for “Context” (D.CTX)**; **[[F.0.1 - Contextual Lexicon Principles|F.0.1]] Contextual Lexicon Principles**; [[A.7 - Strict Distinction (Clarity Lattice)|A.7]] **Strict Distinction (Clarity Lattice)**; [[A.11 - Ontological Parsimony (C‑5)|A.11]] **Ontological Parsimony**.
**Coordinates with.** [[F.2 - Term Harvesting & Normalisation|F.2]] **Term Harvesting & Normalisation**; [[F.3 - Intra‑Context Sense Clustering|F.3]] **Intra‑Context Sense Clustering**; [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] **Role Description**; [[F.9 - Alignment & Bridge across Contexts|F.9]] **Alignment & Bridge Across Contexts**; **[[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]–[[G.1 - CG‑Frame‑Ready Generator|G.1]]** *(Scope/entityOfConcern handoff)*.  *(Bridges live only in [[F.9 - Alignment & Bridge across Contexts|F.9]].)*

**Aliases (informative).** *Contexts‑first survey*; *Context cut*.

## [[F.1 - Domain‑Family Landscape Survey|F.1]]:1 - Intent & applicability
...
## [[F.1 - Domain‑Family Landscape Survey|F.1]]:2 - Problem frame
...
## [[F.1 - Domain‑Family Landscape Survey|F.1]]:3 - Forces
...
## [[F.1 - Domain‑Family Landscape Survey|F.1]]:4 - Core idea (didactic)

**Think in Contexts, not in words.**
A *Context of meaning* is a **U.BoundedContext** (per D.CTX) that encloses a coherent vocabulary and its rules from a **specific, citable canon** (standard, BoK, seminal paper, textbook tradition). You **name and reason** *inside the Context*. When you must step between Contexts, you will **declare a bridge later** ([[F.9 - Alignment & Bridge across Contexts|F.9]]) with explicit losses or mismatches.

## [[F.1 - Domain‑Family Landscape Survey|F.1]]:5 - Minimal vocabulary (this pattern only)
...
* **SenseCell** *(appears downstream)*. A **(Context × Local‑Sense)** address; [[F.3 - Intra‑Context Sense Clustering|F.3]] will mint these after clustering. Mentioned here only to keep the destination in view.

## [[F.1 - Domain‑Family Landscape Survey|F.1]]:6 - Solution — the Contexts‑first survey (conceptual, notation‑free)
...
If two Contexts seem “close”, **resist** collapsing. Note the tension and leave any bridge claim to **[[F.9 - Alignment & Bridge across Contexts|F.9]] Alignment & Bridge**.

## [[F.1 - Domain‑Family Landscape Survey|F.1]]:7 - What to record (conceptual, not clerical)
...
## [[F.1 - Domain‑Family Landscape Survey|F.1]]:8 - Invariants (normative, lightweight)

1. **Context ≡ U.BoundedContext.** In this pattern, *Context* always means **U.BoundedContext** (per [[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]]).
...
5. **No bridging here.** No equivalence or mapping is asserted between Contexts in [[F.1 - Domain‑Family Landscape Survey|F.1]]. (Bridges live in **[[F.9 - Alignment & Bridge across Contexts|F.9]]**.)
...
9. **Scope naming separation.** `Scope gist` on Cards is **didactic only**; formal *Scope/entityOfConcern* (=`USM.ScopeSlice(G)` ⊕ `entityOfConcern(GroundingHolon, ReferencePlane)`) is declared **in [[G.0 - Frame Standard and Comparability Governance - CG‑Spec|G.0]]–[[G.1 - CG‑Frame‑Ready Generator|G.1]]**, not in [[F.1 - Domain‑Family Landscape Survey|F.1]].
...
## [[F.1 - Domain‑Family Landscape Survey|F.1]]:9 - Self‑checks (mental, not procedural)
...
* **The bridge itch.** Feel the temptation to say “these are the same”? Good. **Write the itch down** and refuse to scratch it here. That’s [[F.9 - Alignment & Bridge across Contexts|F.9]]’s job.
* **The memory rule.** If your entire survey cannot be recalled **without opening a document**, it is too large.

## [[F.1 - Domain‑Family Landscape Survey|F.1]]:10 - Micro‑examples (illustrative only)
...
> With only these Contexts fixed, later steps become almost mechanical: [[F.2 - Term Harvesting & Normalisation|F.2]] harvests terms **inside** each Context; [[F.3 - Intra‑Context Sense Clustering|F.3]] clusters **within** each Context; [[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]] names roles or statuses pointing to **SenseCells**; [[F.9 - Alignment & Bridge across Contexts|F.9]] draws the bridges you refused to draw here.

## [[F.1 - Domain‑Family Landscape Survey|F.1]]:11 - Anti‑patterns & remedies
...
| **A5**  | **Bridge‑by‑stealth**      | Phrases like “these are basically the same” inside the survey.                       | Hides losses; imports meaning across Contexts without scrutiny.   | **No bridging here**; write the *itch to bridge* down as an **[[F.9 - Alignment & Bridge across Contexts|F.9]]** bridge question (§6 Step 6, §8‑5).                           |
...
| **A9**  | **Context inheritance**    | Arranging Contexts in is‑a hierarchies (“PROV is‑a BPMN”).                              | Suggests meaning flows by inheritance; erases locality.        | **No is‑a among Contexts**; relations between Contexts live in **[[F.9 - Alignment & Bridge across Contexts|F.9]] bridges** (§8‑5).                                        |
...
| **A11** | **Family‑based inference** | Treating Domain‑family membership as implying similarity/equivalence. | Smuggles semantics via shelf labels; breaks locality. | **Domain family is informative only**; locality and any Cross‑context relation must be explicit ([[F.9 - Alignment & Bridge across Contexts|F.9]]). |

## [[F.1 - Domain‑Family Landscape Survey|F.1]]:12 - Worked examples
...
### [[F.1 - Domain‑Family Landscape Survey|F.1]]:12.1 Enactment (`U.RoleAssignment` + `U.RoleEnactment`) with sensing & execution (service acceptance)
...
* You resist equating **PROV Activity** with **BPMN workflow**; later **[[F.9 - Alignment & Bridge across Contexts|F.9]]** may relate them with explicit loss.

### [[F.1 - Domain‑Family Landscape Survey|F.1]]:12.2 Method quartet with types & measurement (model state graph)
...
* You avoid treating **FCA “concept”** as a **U.Type**; later [[F.9 - Alignment & Bridge across Contexts|F.9]] can bridge OWL classes to FCA concepts with cautions.

### [[F.1 - Domain‑Family Landscape Survey|F.1]]:12.3 Control & actuation with services (operational SLOs in plants)
...
## [[F.1 - Domain‑Family Landscape Survey|F.1]]:13 - Reasoning primitives (judgement schemas, notation‑free)
...
7. **Bridge embargo**
   ...
   *Reading:* [[F.1 - Domain‑Family Landscape Survey|F.1]] never asserts equivalence across Contexts; postponement is principled, not procrastination.
...
## [[F.1 - Domain‑Family Landscape Survey|F.1]]:14 - F1‑Card example (informative)
```
F1-Card v2025‑Q3:
...
## [[F.1 - Domain‑Family Landscape Survey|F.1]]:15 - Relations (with other patterns)

**Builds on:**
[[E.10.D1 - Lexical Discipline for “Context” (D.CTX)|E.10.D1]] **Lexical Discipline for “Context” (D.CTX)** — ensures *Context* ≡ *U.BoundedContext* and reserves “Problem Frame” for narrative use.
[[A.7 - Strict Distinction (Clarity Lattice)|A.7]] **Strict Distinction** — guards EntityOfConcern/Description-episteme/publication-carrier and DesignRunTag splits while you cut Contexts.
[[A.11 - Ontological Parsimony (C‑5)|A.11]] **Ontological Parsimony** — motivates the small cut.

**Constrains:**
**[[F.2 - Term Harvesting & Normalisation|F.2]]** (Term Harvesting): harvest **inside** Contexts named here; every occurrence carries a Context name.
**[[F.3 - Intra‑Context Sense Clustering|F.3]]** (Intra‑Context Sense Clustering): cluster **per Context**; no Cross‑context sense claims.
**[[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]** (Role Descriptions): any role template or status template must cite a **SenseCell** that lives in a Context from this cut.
**[[F.9 - Alignment & Bridge across Contexts|F.9]]** (Alignment & Bridge): only [[F.9 - Alignment & Bridge across Contexts|F.9]] may relate Contexts; never [[F.1 - Domain‑Family Landscape Survey|F.1]]–[[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]].
...
## [[F.1 - Domain‑Family Landscape Survey|F.1]]:16 - Migration notes (conceptual)
...
5. **Bridges discovered later.** Do not back‑port bridges into [[F.1 - Domain‑Family Landscape Survey|F.1]]; leave the Cards untouched and record the mapping in **[[F.9 - Alignment & Bridge across Contexts|F.9]]**.
...
## [[F.1 - Domain‑Family Landscape Survey|F.1]]:17 - Acceptance tests (SCR/RSCR — concept‑level)

### [[F.1 - Domain‑Family Landscape Survey|F.1]]:17.1 - Static conformance checks (SCR)
...
* **SCR‑F1‑S03 (Locality pledge).** Nowhere in [[F.1 - Domain‑Family Landscape Survey|F.1]] are Cross‑context equivalences or merges asserted.
...
* **SCR‑F1‑S07 (Family neutrality).** No claim, classification, or relation in [[F.1 - Domain‑Family Landscape Survey|F.1]] relies on Domain‑family membership; families appear only as shelf labels on cards.
...
### [[F.1 - Domain‑Family Landscape Survey|F.1]]:17.2 - Regression checks (RSCR)
...
## [[F.1 - Domain‑Family Landscape Survey|F.1]]:18 - Didactic distillation (90‑second teaching script)

> “Before you name anything, **fix the context of meaning**. A *Context* is a **U.BoundedContext** tied to a specific canon—*BPMN 2.0*, *PROV‑O*, *ITIL 4*, *SOSA/SSN*, *IEC 61131‑3*, *OWL 2*. Words are **local to Contexts**: *process (BPMN)* is a workflow graph, *activity (PROV)* is a run‑time occurrence, *service (ITIL)* is a promise vocabulary. Cut the landscape so each unification line sees **at least three domain families**, with **one‑screen Cards** per Context (scope gist, time stance, trip‑wires). **Do not bridge** Contexts here—just write down the itch to bridge as an **[[F.9 - Alignment & Bridge across Contexts|F.9]]** bridge question. Keep the cut **small enough to remember**. With Contexts fixed, harvesting ([[F.2 - Term Harvesting & Normalisation|F.2]]), local clustering ([[F.3 - Intra‑Context Sense Clustering|F.3]]), role template or status templates ([[F.4 - Role Description (RCS + RoleStateGraph + Checklists)|F.4]]), and explicit Cross‑context bridges ([[F.9 - Alignment & Bridge across Contexts|F.9]]) become straightforward—and you avoid naming ghosts that come from words floating without walls.”

## [[F.1 - Domain‑Family Landscape Survey|F.1]]:End
